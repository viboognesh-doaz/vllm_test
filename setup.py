...
if sys.platform.startswith('darwin') and VLLM_TARGET_DEVICE != 'cpu':
    ...
    logger.warning('vLLM only supports Linux platform (including WSL) and MacOS.Building on %s, so vLLM may not be able to run correctly', sys.platform)
    VLLM_TARGET_DEVICE = 'empty'
elif sys.platform.startswith('linux') and torch.version.cuda is None and (os.getenv('VLLM_TARGET_DEVICE') is None) and (torch.version.hip is None):
    VLLM_TARGET_DEVICE = 'cpu'
...
class cmake_build_ext(build_ext):
    ...
    def configure(self, ext: CMakeExtension) -> None:
        ...
        cmake_args = ['-DCMAKE_BUILD_TYPE={}'.format(cfg), '-DVLLM_TARGET_DEVICE={}'.format(VLLM_TARGET_DEVICE)]
    ...
    def run(self):
        super().run()
        files = glob.glob(os.path.join(self.build_lib, 'vllm', 'vllm_flash_attn', '**', '*.py'), recursive=True)
        for file in files:
            dst_file = os.path.join('vllm/vllm_flash_attn', file.split('vllm/vllm_flash_attn/')[-1])
...
class repackage_wheel(build_ext):
    ...
    def get_base_commit_in_main_branch(self) -> str:
        if envs.VLLM_TEST_USE_PRECOMPILED_NIGHTLY_WHEEL:
            return 'nightly'
        try:
            resp_json = subprocess.check_output(['curl', '-s', 'https://api.github.com/repos/vllm-project/vllm/commits/main']).decode('utf-8')
            upstream_main_commit = json.loads(resp_json)['sha']
            try:
                subprocess.check_output(['git', 'cat-file', '-e', f'{upstream_main_commit}'])
            except subprocess.CalledProcessError:
                subprocess.check_call(['git', 'fetch', 'https://github.com/vllm-project/vllm', 'main'])
            current_branch = subprocess.check_output(['git', 'branch', '--show-current']).decode('utf-8').strip()
            base_commit = subprocess.check_output(['git', 'merge-base', f'{upstream_main_commit}', current_branch]).decode('utf-8').strip()
            return base_commit
        except ValueError as err:
            raise ValueError(err) from None
        except Exception as err:
            logger.warning('Failed to get the base commit in the main branch. Using the nightly wheel. The libraries in this wheel may not be compatible with your dev branch: %s', err)
            return 'nightly'
    ...
    def run(self) -> None:
        ...
        if os.path.isfile(wheel_location):
            ...
            try:
                ...
                raise SetupError(f'Failed to get vLLM wheel from {wheel_location}') from e
        with zipfile.ZipFile(wheel_path) as wheel:
            files_to_copy = ['vllm/_C.abi3.so', 'vllm/_moe_C.abi3.so', 'vllm/_flashmla_C.abi3.so', 'vllm/vllm_flash_attn/_vllm_fa2_C.abi3.so', 'vllm/vllm_flash_attn/_vllm_fa3_C.abi3.so', 'vllm/cumem_allocator.abi3.so']
            file_members = list(filter(lambda x: x.filename in files_to_copy, wheel.filelist))
            compiled_regex = re.compile('vllm/vllm_flash_attn/(?:[^/.][^/]*/)*(?!\\.)[^/]*\\.py')
            file_members += list(filter(lambda x: compiled_regex.match(x.filename), wheel.filelist))
...
package_data = {'vllm': ['py.typed', 'model_executor/layers/fused_moe/configs/*.json', 'model_executor/layers/quantization/utils/configs/*.json']}
if _no_device():
    ext_modules = []
if not ext_modules:
    cmdclass = {}
else:
    cmdclass = {'build_ext': repackage_wheel if envs.VLLM_USE_PRECOMPILED else cmake_build_ext}
setup(version=get_vllm_version(), ext_modules=ext_modules, install_requires=get_requirements(), extras_require={'bench': ['pandas', 'datasets'], 'tensorizer': ['tensorizer==2.10.1'], 'fastsafetensors': ['fastsafetensors >= 0.1.10'], 'runai': ['runai-model-streamer', 'runai-model-streamer-s3', 'boto3'], 'audio': ['librosa', 'soundfile', 'mistral_common[audio]'], 'video': []}, cmdclass=cmdclass, package_data=package_data)