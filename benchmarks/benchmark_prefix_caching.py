...
import random
import time
'\nBenchmark the efficiency of prefix caching.\n\nThis script allows you to benchmark the performance of\na model with and without prefix caching using either fixed prompts\nor prompts sampled from the ShareGPT dataset.\n\nFixed example usage:\n    python benchmark_prefix_caching.py         --model meta-llama/Llama-2-7b-chat-hf         --enable-prefix-caching         --num-prompts 1         --repeat-count 100         --input-length-range 128:256\n\nShareGPT example usage:\n    # This command samples 20 prompts with input lengths\n    # between 128 and 256 tokens from the ShareGPT dataset,\n    # then replicates each prompt 5 times.\n    python benchmark_prefix_caching.py         --model meta-llama/Llama-2-7b-chat-hf         --dataset-path /path/to/ShareGPT_V3_unfiltered_cleaned_split.json         --enable-prefix-caching         --num-prompts 20         --repeat-count 5         --input-length-range 128:256\n'
try:
except ImportError:
...
def sample_tokens(tokenizer: PreTrainedTokenizerBase, length: int) -> list[int]:
    ...
    return random.choices([v for k, v in vocab.items() if k not in all_special_ids], k=length)

def sample_requests_from_dataset(dataset_path: str, num_requests: int, tokenizer: PreTrainedTokenizerBase, input_length_range: tuple[int, int], fixed_output_len: Optional[int]) -> list[Request]:
...
def main(args):
    ...
    if args.dataset_path is not None:
        ...
        filtered_requests = sample_requests_from_dataset(dataset_path=args.dataset_path, num_requests=args.num_prompts, tokenizer=tokenizer, input_length_range=input_length_range, fixed_output_len=args.output_len)
    else:
        print(f'Start to sample {args.num_prompts} prompts from random')
        filtered_requests = sample_requests_from_random(num_requests=args.num_prompts, tokenizer=tokenizer, input_length_range=input_length_range, fixed_output_len=args.output_len, prefix_len=args.prefix_len)
    ...
    sampling_params = SamplingParams(temperature=0, max_tokens=args.output_len, detokenize=not args.disable_detokenize)
...
def create_argument_parser():
    ...
    parser.add_argument('--num-prompts', type=int, required=True, help='Number of the prompts sampled from dataset')
    parser.add_argument('--repeat-count', type=int, default=1, help='Number of times to repeat each prompt')
    parser.add_argument('--sort', action='store_true', help='Sort prompts by input length')
    parser.add_argument('--input-length-range', type=str, required=True, help='Range of input lengths for sampling prompts,specified as "min:max" (e.g., "128:256").')
    parser.add_argument('--prefix-len', type=int, default=0, help='Specifies the length of a common prefix to be added to the input prompt. The input-length-range will subtract this length when filtering prompts. Only used when dataset-path is not provided.')
