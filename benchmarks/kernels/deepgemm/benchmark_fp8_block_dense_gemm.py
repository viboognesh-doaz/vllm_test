...
from vllm.model_executor.layers.quantization.utils.fp8_utils import per_token_group_quant_fp8, w8a8_block_fp8_matmul
...
def per_block_cast_to_fp8(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    x_amax = x_view.abs().float().amax(dim=(1, 3), keepdim=True).clamp(0.0001)
    x_scaled = (x_view * (448.0 / x_amax)).to(torch.float8_e4m3fn)
    return (x_scaled.view_as(x_padded)[:m, :n].contiguous(), (x_amax / 448.0).view(x_view.size(0), x_view.size(2)))

def benchmark_shape(m: int, n: int, k: int, warmup: int=100, repeat: int=10000, verbose: bool=False) -> dict:
    ...
    A_vllm_cutlass, A_scale_vllm_cutlass = per_token_group_quant_fp8(A, block_size[1], column_major_scales=True)

    def deepgemm_gemm():
        deep_gemm.gemm_fp8_fp8_bf16_nt((A_deepgemm, A_scale_deepgemm), (B_deepgemm, B_scale_deepgemm), C_deepgemm)
        return C_deepgemm

    def vllm_triton_gemm():
        return w8a8_block_fp8_matmul(A_vllm, B_vllm, A_scale_vllm, B_scale_vllm, block_size, output_dtype=torch.bfloat16)

    def vllm_cutlass_gemm():
        return ops.cutlass_scaled_mm(A_vllm_cutlass, B_vllm.T, scale_a=A_scale_vllm_cutlass, scale_b=B_scale_vllm.T, out_dtype=torch.bfloat16)
    if verbose:
    ...
    if verbose:
        ...
        print(f'vLLM Triton vs DeepGEMM difference: {calc_diff(C_vllm_triton, C_deepgemm):.6f}')
        print(f'vLLM CUTLASS vs DeepGEMM difference: {calc_diff(C_vllm_cutlass, C_deepgemm):.6f}')
    implementations = {'DeepGEMM': deepgemm_gemm, 'vLLM Triton': vllm_triton_gemm, 'vLLM CUTLASS': vllm_cutlass_gemm}
    ...
    for name, func in implementations.items():
        ...
        tflops = 2 * m * n * k / (avg_time_ms * 0.001) / 1000000000000.0
        gb_s = (m * k + k * n + m * n * 2) / 1000000000.0 / (avg_time_ms * 0.001)
        benchmark_results['implementations'][name] = {'time_ms': avg_time_ms, 'time_us': avg_time_us, 'tflops': tflops, 'gb_s': gb_s, 'diff': {'DeepGEMM': 0.0 if name == 'DeepGEMM' else calc_diff(func(), C_deepgemm), 'Reference': deepgemm_diff if name == 'DeepGEMM' else vllm_triton_diff if name == 'vLLM Triton' else vllm_cutlass_diff}}
        if verbose:
    ...
    benchmark_results['implementations']['vLLM CUTLASS']['speedup_vs_triton'] = cutlass_vs_triton
    if verbose:
        print(f"vLLM CUTLASS is {cutlass_vs_triton:.2f}x {('faster' if cutlass_vs_triton > 1 else 'slower')} than vLLM Triton")
...
def run_benchmarks(verbose: bool=False):
    ...
    shapes = [(8, 4096, 7168), (8, 7168, 18432), (8, 18432, 7168), (64, 4096, 7168), (64, 7168, 18432), (64, 18432, 7168), (64, 24576, 1536), (64, 32768, 512), (64, 7168, 16384), (128, 4096, 7168), (128, 7168, 18432), (128, 18432, 7168), (1024, 4096, 7168), (1024, 18432, 7168), (2048, 4096, 7168), (4096, 4096, 7168)]
    shapes = [(64, 24576, 1536), (64, 32768, 512), (64, 7168, 16384), (64, 4096, 7168), (64, 7168, 2048), (128, 24576, 1536), (128, 32768, 512), (128, 7168, 16384), (128, 4096, 7168), (128, 7168, 2048), (4096, 24576, 1536), (4096, 32768, 512), (4096, 7168, 16384), (4096, 4096, 7168), (4096, 7168, 2048)]
    all_results = []
    for m, n, k in shapes:
    ...
    for result in all_results:
        ...
        deepgemm_rows.append([shape['m'], shape['n'], shape['k'], f"{impl_data['time_us']:.1f}", f"{impl_data['tflops']:.1f}", f"{impl_data['gb_s']:.1f}"])
    ...
    for result in all_results:
        ...
        triton_rows.append([shape['m'], shape['n'], shape['k'], f"{impl_data['time_us']:.1f}", f"{impl_data['tflops']:.1f}", f"{impl_data['gb_s']:.1f}", format_speedup(speedup)])
    print_table(triton_headers, triton_rows, title='vLLM Triton Implementation:')
    cutlass_headers = ['m', 'n', 'k', 'Time (μs)', 'TFLOPS', 'GB/s', 'vs DeepGEMM', 'vs Triton']
    cutlass_rows = []
    for result in all_results:
        shape = result['shape']
        ...
        cutlass_rows.append([shape['m'], shape['n'], shape['k'], f"{impl_data['time_us']:.1f}", f"{impl_data['tflops']:.1f}", f"{impl_data['gb_s']:.1f}", format_speedup(vs_deepgemm), format_speedup(vs_triton)])
    ...
    avg_speedups = {'DeepGEMM vs vLLM Triton': 0, 'DeepGEMM vs vLLM CUTLASS': 0, 'vLLM CUTLASS vs vLLM Triton': 0}
    for result in all_results:
        ...
        avg_speedups['vLLM CUTLASS vs vLLM Triton'] += vllm_triton_time / vllm_cutlass_time
    print('\n===== AVERAGE SPEEDUPS =====')
