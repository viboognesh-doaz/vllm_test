...
@triton.testing.perf_report(triton.testing.Benchmark(x_names=['batch_size'], x_vals=[1, 16, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384], x_log=False, line_arg='provider', line_vals=_enabled, line_names=[k for k in _enabled], ylabel='TFLOP/s (larger is better)', plot_name='BF16 vs INT8 GEMMs', args={}))
def benchmark(batch_size, provider, N, K):
    ...
    if provider == 'torch-bf16':
        ms, min_ms, max_ms = triton.testing.do_bench_cudagraph(lambda: torch.nn.functional.linear(a, b), quantiles=quantiles)
    else:
    ...
    to_tflops = lambda t_ms: 2 * M * N * K * 1e-12 / (t_ms * 0.001)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models', nargs='+', type=str, default=['meta-llama/Llama-3.1-8B-Instruct'], choices=list(WEIGHT_SHAPES.keys()), help='List of models to benchmark')
    parser.add_argument('--tp-sizes', nargs='+', type=int, default=[1], help='List of tensor parallel sizes')
    args = parser.parse_args()
    for K, N, model in prepare_shapes(args):
        print(f'{model}, N={N} K={K}, BF16 vs INT8 GEMMs TFLOP/s:')
        benchmark.run(print_data=True, show_plots=True, save_path=f'bench_int8_res_n{N}_k{K}', N=N, K=K)
