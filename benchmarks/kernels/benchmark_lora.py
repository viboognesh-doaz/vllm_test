...
DEFAULT_TP_SIZES = [1]
DEFAULT_BATCH_SIZES = [1, 16, 32, 64, 128, 192, 256, 320, 384, 448, 512, 640, 768, 896, 1024, 2048, 3072, 4096, 5120, 6144, 7168, 8192]
...
def make_rand_lora_weight_tensor(k: int, n: int, num_loras: int, dtype: torch.dtype, device: str='cuda') -> torch.Tensor:
    return torch.rand((num_loras, n, k), dtype=dtype).to(device)

def make_rand_tensors(a_shape: tuple[int], b_shape: tuple[int], c_shape: tuple[int], a_dtype: torch.dtype, b_dtype: torch.dtype, c_dtype: torch.dtype, num_slices: int, device: str='cuda') -> tuple[torch.Tensor, list[torch.Tensor], torch.Tensor]:
    """
...
def make_token_lora_mapping(num_tokens: int, num_prompts: int, prompt_lora_mapping: torch.Tensor, seq_len_tensor: torch.Tensor, device: str):
    """
...
def ref_group_gemm(ref_out: torch.Tensor, input: torch.Tensor, lora_weights: list[torch.Tensor], seq_lens_cpu: torch.Tensor, prompt_lora_mapping_cpu: torch.Tensor, scaling: float, add_inputs: Optional[bool]):
    """
...
class OpType(Enum):
    ...
    def matmul_shapes(self, batch_size: int, seq_length: int, hidden_size: int, lora_rank: int, num_loras: int, num_slices: int) -> tuple[tuple[int], tuple[int], tuple[int]]:
        """
        Given num_slices, return the shapes of the A, B, and C matrices
        in A x B = C, for the op_type
        """
        m, k, n = self.mkn(batch_size, seq_length, hidden_size, lora_rank)
        b_shape = (num_loras, n, k)
    ...
    def run_ref_group_gemm(self, output: torch.Tensor, input: torch.Tensor, lora_weights: list[torch.Tensor], **kwargs) -> Callable:
        ...
        if self in [OpType.LORA_SHRINK]:
            for slice_idx in range(num_slices):
                ref_group_gemm(ref_out=output[slice_idx, :], input=input, lora_weights=lora_weights[slice_idx], **kwargs)
        elif self in [OpType.LORA_EXPAND]:
            hidden_size = lora_weights[0].shape[1]
            for slice_idx in range(num_slices):
                slice_offset = slice_idx * hidden_size
                ref_group_gemm(ref_out=output[:, slice_offset:slice_offset + hidden_size], input=input[slice_idx].clone().to(dtype=w_dtype), lora_weights=lora_weights[slice_idx], **kwargs)
        else:
            raise ValueError(f'Unrecognized optype {self}')
...
class BenchmarkContext:
    ...
    def bench_sublabel(self, op_type: OpType) -> str:
        ...
        desc = {'bs': self.batch_size, 'sl': self.seq_length, 'm': m, 'k': k, 'n': n, 'num_loras': self.num_loras, 'sort_by_lora': self.sort_by_lora_id, 'num_slices': self.num_slices}
...
class BenchmarkTensors:
    ...
    def io_types(self) -> str:
        return f'{dtype_to_str(self.input.dtype)}x{dtype_to_str(self.lora_weights_lst[0].dtype)}=>{dtype_to_str(self.output.dtype)}'

    @staticmethod
    def make(ctx: BenchmarkContext, op_type: OpType, device: str='cuda') -> 'BenchmarkTensors':
        a_shape, b_shape, c_shape = op_type.matmul_shapes(ctx.batch_size, ctx.seq_length, ctx.hidden_size, ctx.lora_rank, ctx.num_loras, ctx.num_slices)
        a_type, b_type, c_type = op_type.matmul_dtypes(ctx.dtype)
        input_tensor, lora_weights, output_tensor = make_rand_tensors(a_shape, b_shape, c_shape, a_type, b_type, c_type, num_slices=ctx.num_slices)
        ...
        prompt_lora_indices_tensor = make_prompt_lora_mapping(ctx.batch_size, ctx.num_active_loras, ctx.sort_by_lora_id, 'cpu')
        token_lora_indices_tensor = make_token_lora_mapping(total_tokens, ctx.batch_size, prompt_lora_indices_tensor, seq_len_tensor, 'cpu')
        lora_kernel_meta = LoRAKernelMeta.make(max_loras=ctx.num_loras, max_num_tokens=token_lora_indices_tensor.size(0), device='cpu')
        lora_kernel_meta.prepare_tensors(token_lora_mapping=token_lora_indices_tensor)
        return BenchmarkTensors(input_tensor, lora_weights, output_tensor, lora_kernel_meta, seq_len_tensor, prompt_lora_indices_tensor)
    ...
    def as_lora_shrink_kwargs(self) -> dict[str, Any]:
        ...
        i_shape, lw_shape, o_shape = (self.input.shape, self.lora_weights_lst[0].shape, self.output.shape)
    ...
    def as_lora_expand_kwargs(self, add_inputs: bool) -> dict[str, Any]:
        ...
        return {'inputs': self.input, 'lora_b_weights': self.lora_weights_lst, 'output_tensor': self.output, 'token_lora_mapping': self.lora_kernel_meta.token_lora_mapping, 'token_indices_sorted_by_lora_ids': self.lora_kernel_meta.token_indices_sorted_by_lora_ids, 'num_tokens_per_lora': self.lora_kernel_meta.num_tokens_per_lora, 'lora_token_start_loc': self.lora_kernel_meta.lora_token_start_loc, 'lora_ids': self.lora_kernel_meta.active_lora_ids, 'offset_start': 0, 'add_inputs': add_inputs}
    ...
    def test_correctness(self, op_type: OpType, expand_fn_add_inputs: Optional[bool]) -> bool:
        """
        ...
        op_type.run_ref_group_gemm(ref_output, self.input, self.lora_weights_lst, seq_lens_cpu=seq_lens_cpu, prompt_lora_mapping_cpu=prompt_lora_mapping_cpu, scaling=1.0, add_inputs=expand_fn_add_inputs)
        rtol, atol = {torch.float16: (0.06, 0.06), torch.bfloat16: (0.06, 0.06), torch.float32: (0.01, 0.01)}[self.output.dtype]
        return torch.allclose(ref_output, self.output, rtol=rtol, atol=atol)

def bench_optype(ctx: BenchmarkContext, arg_pool_size: int, op_type: OpType, cuda_graph_nops: Optional[int]=None, expand_fn_add_inputs: Optional[bool]=None, test_correctness: bool=False) -> TMeasurement:
    ...
    timer = None
    with Bench(cuda_graph_params, ctx.bench_label(), ctx.bench_sublabel(op_type), description, op_type.bench_fn(), **kwargs) as bench:
        timer = bench.run()
    return timer

def bench_torch_mm(ctx: BenchmarkContext, arg_pool_size: int, op_type: OpType, cuda_graph_nops: Optional[int]=None) -> TMeasurement:
    """
    ...
    """
    batch_size, hidden_size, lora_rank, seq_length, dtype = (ctx.batch_size, ctx.hidden_size, ctx.lora_rank, ctx.seq_length, ctx.dtype)
    ...
    description = f'single-lora roofline using torch.mm ({dtype_to_str(dtype)}x{dtype_to_str(dtype)}=>{dtype_to_str(dtype)})'
    cuda_graph_params = None
    if cuda_graph_nops:
        cuda_graph_params = CudaGraphBenchParams(cuda_graph_nops)
    with Bench(cuda_graph_params, ctx.bench_label(), ctx.bench_sublabel(op_type), description, torch.mm, **mm_kwargs) as bench:
        return bench.run()
...
def print_timers(timers: list[TMeasurement], args: Optional[argparse.Namespace]=None):
    ...
    if args and args.cuda_graph_nops:
        print(f'Note : The timings reported above is for {args.cuda_graph_nops} consecutive invocations of the benchmarking functions. Please divide by {args.cuda_graph_nops} for single invocation timings.')
    print('Note on Comparison with torch.mm : The torch.mm numbers are benchmark numbers of a simple matmul emulating the single lora case. It is provided as a roofline for comparing our LoRA Kernel implementations. It is expected that the LoRA kernels will be slower than torch.mm in cases where num_loras is big. But for small num_loras the goal should be to match the torch.mm numbers.')
            ...
            for bench_op in bench_ops:
                for num_slices in bench_op.num_slices():
                    ...
                    for add_input_arg in expand_fn_add_inputs:
                        seq_len_timers.append(bench_optype(_ctx, args.arg_pool_size, bench_op, args.cuda_graph_nops, add_input_arg, args.test_correctness))
...
def as_benchmark_contexts(hidden_sizes: list[int], lora_ranks: list[int], args: argparse.Namespace) -> list[BenchmarkContext]:
    ctxs: list[BenchmarkContext] = []
    for batch_size, hidden_size, lora_rank, num_loras, sort_by_lora_id in product(args.batch_sizes, list(hidden_sizes), lora_ranks, args.num_loras, args.sort_by_lora_id):
        ctxs.append(BenchmarkContext(batch_size=batch_size, hidden_size=hidden_size, lora_rank=lora_rank, num_loras=num_loras, num_active_loras=args.num_active_loras if args.num_active_loras else num_loras, seq_length=None, sort_by_lora_id=sort_by_lora_id, dtype=args.dtype, num_slices=None))
    return ctxs

def run_list_bench(args: argparse.Namespace):
    print(args)
    print(f'List bench :\n  Hidden Sizes {args.hidden_sizes}  LoRA Ranks {args.lora_ranks}')
    bench_contexts: list[BenchmarkContext] = as_benchmark_contexts(hidden_sizes=args.hidden_sizes, lora_ranks=args.lora_ranks, args=args)
    run(args, bench_contexts)

def run_range_bench(args: argparse.Namespace):
    print(args)
    hidden_sizes = list(range(args.hidden_sizes_start, args.hidden_sizes_end + 1, args.hidden_sizes_increment))
    ...
    bench_contexts: list[BenchmarkContext] = as_benchmark_contexts(hidden_sizes=hidden_sizes, lora_ranks=lora_ranks, args=args)
    run(args, bench_contexts)
...
def run_model_bench(args: argparse.Namespace):
    ...
    bench_contexts: list[BenchmarkContext] = as_benchmark_contexts(hidden_sizes=hidden_sizes, lora_ranks=args.lora_ranks, args=args)
    run(args, bench_contexts)
if __name__ == '__main__':
    ...
    def add_common_command_args(p: argparse.ArgumentParser):
        p.add_argument('--dtype', type=to_torch_dtype, required=True, help="Available options are ['torch.float16', 'torch.bfloat16']")
        p.add_argument('--arg-pool-size', type=int, default=32, help='Run profiles with a pool of input/output/meta tensors insteadof simply reusing the same tensors for all runs. A bigger arg-poolmitigates hardware caching effects during benchmarking.')
        p.add_argument('--cuda-graph-nops', type=int, help='when set profiling is done using cudagraph, with the given number of operations in a graph.Note that the measurement returned is the time taken for N consecutive executions of the benchmarking functions, where N is the value of this argument.')
        p.add_argument('--num-loras', nargs='+', type=int, default=DEFAULT_NUM_LORAS)
        p.add_argument('--num-active-loras', type=int, default=None, help='Active LoRAs. When None, all LoRAs are active')
        p.add_argument('--sort-by-lora-id', nargs='+', type=get_bool, default=DEFAULT_SORT_BY_LORA_IDS)
