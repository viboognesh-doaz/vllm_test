...
class Bench:
    ...
    def __init__(self, cuda_graph_params: Optional[CudaGraphBenchParams], label: str, sub_label: str, description: str, fn: Callable, *args, **kwargs):
    ...
    def collapse_argpool(self, *args, **kwargs):
        argpool_args = [arg for arg in args if isinstance(arg, ArgPool)] + [arg for arg in kwargs.values() if isinstance(arg, ArgPool)]
    ...
    def run_cudagrah(self) -> TMeasurement:
        assert self.use_cuda_graph
        globals = {'g': self.g}
        return TBenchmark.Timer(stmt='g.replay()', globals=globals, label=f'{self.label} | cugraph {self.cuda_graph_params.num_ops_in_cuda_graph} ops', sub_label=self.sub_label, description=self.description).blocked_autorange(min_run_time=self.min_run_time)
    ...
    def run_eager(self) -> TMeasurement:
        ...
        return TBenchmark.Timer(stmt=stmt, setup=setup, globals=globals, label=self.label, sub_label=self.sub_label, description=self.description).blocked_autorange(min_run_time=self.min_run_time)
