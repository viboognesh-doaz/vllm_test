...
def bench_int8(dtype: torch.dtype, m: int, k: int, n: int, label: str, sub_label: str, bench_kernels: Optional[list[str]]=None) -> Iterable[TMeasurement]:
...
def bench_fp8(dtype: torch.dtype, m: int, k: int, n: int, label: str, sub_label: str, bench_kernels: Optional[list[str]]=None) -> Iterable[TMeasurement]:
...
def bench(dtype: torch.dtype, m: int, k: int, n: int, label: str, sub_label: str, bench_kernels: Optional[list[str]]=None) -> Iterable[TMeasurement]:
    if dtype == torch.int8:
...
def run(dtype: torch.dtype, MKNs: Iterable[tuple[int, int, int]], bench_kernels: Optional[list[str]]=None) -> Iterable[TMeasurement]:
    results = []
    for m, k, n in MKNs:
        timers = bench(dtype, m, k, n, f'scaled-{dtype}-gemm', f'MKN=({m}x{k}x{n})', bench_kernels=bench_kernels)
        print_timers(timers)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kernels', nargs='+', type=str, default=None, help='Exact names of the kernels to benchmark. If not set, runs all kernels.')
