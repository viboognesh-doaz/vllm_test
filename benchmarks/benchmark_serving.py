...
from backend_request_func import ASYNC_REQUEST_FUNCS, OPENAI_COMPATIBLE_BACKENDS, RequestFuncInput, RequestFuncOutput
from backend_request_func import get_tokenizer
from benchmark_dataset import AIMODataset, ASRDataset, BurstGPTDataset, ConversationDataset, CustomDataset, HuggingFaceDataset, InstructCoderDataset, MTBenchDataset, NextEditPredictionDataset, RandomDataset, SampleRequest, ShareGPTDataset, SonnetDataset, VisionArenaDataset
...
def _get_current_request_rate(ramp_up_strategy: Optional[Literal['linear', 'exponential']], ramp_up_start_rps: Optional[int], ramp_up_end_rps: Optional[int], request_index: int, total_requests: int, request_rate: float) -> float:
...
async def get_request(input_requests: list[SampleRequest], request_rate: float, burstiness: float=1.0, ramp_up_strategy: Optional[Literal['linear', 'exponential']]=None, ramp_up_start_rps: Optional[int]=None, ramp_up_end_rps: Optional[int]=None) -> AsyncGenerator[tuple[SampleRequest, float], None]:
    """
    ...
    for request in input_requests:
        current_request_rate = _get_current_request_rate(ramp_up_strategy, ramp_up_start_rps, ramp_up_end_rps, request_index, total_requests, request_rate)
...
def calculate_metrics(input_requests: list[SampleRequest], outputs: list[RequestFuncOutput], dur_s: float, tokenizer: PreTrainedTokenizerBase, selected_percentile_metrics: list[str], selected_percentiles: list[float], goodput_config_dict: dict[str, float]) -> tuple[BenchmarkMetrics, list[int]]:
...
async def benchmark(backend: str, api_url: str, base_url: str, model_id: str, model_name: str, tokenizer: PreTrainedTokenizerBase, input_requests: list[SampleRequest], logprobs: Optional[int], request_rate: float, burstiness: float, disable_tqdm: bool, profile: bool, selected_percentile_metrics: list[str], selected_percentiles: list[float], ignore_eos: bool, goodput_config_dict: dict[str, float], max_concurrency: Optional[int], lora_modules: Optional[Iterable[str]], extra_body: Optional[dict], ramp_up_strategy: Optional[Literal['linear', 'exponential']]=None, ramp_up_start_rps: Optional[int]=None, ramp_up_end_rps: Optional[int]=None):
    ...
    if profile:
        ...
        profile_input = RequestFuncInput(model=model_id, model_name=model_name, prompt=test_prompt, api_url=base_url + '/start_profile', prompt_len=test_prompt_len, output_len=test_output_len, logprobs=logprobs, multi_modal_content=test_mm_content, ignore_eos=ignore_eos, extra_body=extra_body)
        profile_output = await request_func(request_func_input=profile_input)
        if profile_output.success:
            print('Profiler started')
    distribution = 'Poisson process' if burstiness == 1.0 else 'Gamma distribution'
    if ramp_up_strategy is not None:
        print(f'Traffic ramp-up strategy: {ramp_up_strategy}. Will increase RPS from {ramp_up_start_rps} to {ramp_up_end_rps} RPS over the duration of the benchmark.')
    else:
    ...
    if ramp_up_strategy is not None and ramp_up_start_rps is not None:
        ...
        rps_change_events.append({'rps': last_int_rps, 'timestamp': datetime.now().isoformat()})
    async for request, current_request_rate in get_request(input_requests, request_rate, burstiness, ramp_up_strategy, ramp_up_start_rps, ramp_up_end_rps):
        if ramp_up_strategy is not None:
        ...
        prompt, prompt_len, output_len, mm_content = (request.prompt, request.prompt_len, request.expected_output_len, request.multi_modal_data)
        req_model_id, req_model_name = (model_id, model_name)
        if lora_modules:
            req_lora_module = next(lora_modules)
            req_model_id, req_model_name = (req_lora_module, req_lora_module)
        request_func_input = RequestFuncInput(model=req_model_id, model_name=req_model_name, prompt=prompt, api_url=api_url, prompt_len=prompt_len, output_len=output_len, logprobs=logprobs, multi_modal_content=mm_content, ignore_eos=ignore_eos, extra_body=extra_body)
        task = limited_request_func(request_func_input=request_func_input, pbar=pbar)
        tasks.append(asyncio.create_task(task))
    outputs: list[RequestFuncOutput] = await asyncio.gather(*tasks)
    if profile:
        print('Stopping profiler...')
        profile_input = RequestFuncInput(model=model_id, prompt=test_prompt, api_url=base_url + '/stop_profile', prompt_len=test_prompt_len, output_len=test_output_len, logprobs=logprobs)
        profile_output = await request_func(request_func_input=profile_input)
        if profile_output.success:
    ...
    metrics, actual_output_lens = calculate_metrics(input_requests=input_requests, outputs=outputs, dur_s=benchmark_duration, tokenizer=tokenizer, selected_percentile_metrics=selected_percentile_metrics, selected_percentiles=selected_percentiles, goodput_config_dict=goodput_config_dict)
    ...
    if args.goodput:
        ...
        for slo_name, slo_val in goodput_config_dict.items():
            if slo_name not in VALID_NAMES:
                raise ValueError(f'Invalid metric name found, {slo_name}: {slo_val}. The service level objective name should be one of {str(VALID_NAMES)}. ')
            if slo_val < 0:
                raise ValueError(f'Invalid value found, {slo_name}: {slo_val}. The service level objective value should be non-negative.')
...
def parse_goodput(slo_pairs):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError('Invalid format found for service level objectives. Specify service level objectives for goodput as "KEY:VALUE" pairs, where the key is a metric name, and the value is a number in milliseconds.') from err
    return goodput_config_dict

def save_to_pytorch_benchmark_format(args: argparse.Namespace, results: dict[str, Any], file_name: str) -> None:
    metrics = ['median_ttft_ms', 'mean_ttft_ms', 'std_ttft_ms', 'p99_ttft_ms', 'mean_tpot_ms', 'median_tpot_ms', 'std_tpot_ms', 'p99_tpot_ms', 'median_itl_ms', 'mean_itl_ms', 'std_itl_ms', 'p99_itl_ms']
    ignored_metrics = ['ttfts', 'itls', 'generated_texts', 'errors']
    pt_records = convert_to_pytorch_benchmark_format(args=args, metrics={k: [results[k]] for k in metrics}, extra_info={k: results[k] for k in results if k not in metrics and k not in ignored_metrics})
    if pt_records:
...
def main(args: argparse.Namespace):
    ...
    if args.ramp_up_strategy is not None:
        if args.request_rate != float('inf'):
            raise ValueError('When using ramp-up, do not specify --request-rate. The request rate will be controlled by ramp-up parameters. Please remove the --request-rate argument.')
        if args.ramp_up_start_rps is None or args.ramp_up_end_rps is None:
            raise ValueError('When using --ramp-up-strategy, both --ramp-up-start-rps and --ramp-up-end-rps must be specified')
    ...
    if args.dataset_name == 'custom':
        ...
        if dataset_class.IS_MULTIMODAL and backend not in ['openai-chat', 'openai-audio']:
    ...
    sampling_params = {k: v for k, v in {'top_p': args.top_p, 'top_k': args.top_k, 'min_p': args.min_p, 'temperature': args.temperature}.items() if v is not None}
    ...
    if args.save_result or args.append_result:
        ...
        if args.ramp_up_strategy is not None:
            file_name = f'{backend}-ramp-up-{args.ramp_up_strategy}-{args.ramp_up_start_rps}qps-{args.ramp_up_end_rps}qps{max_concurrency_str}-{base_model_id}-{current_dt}.json'
        else:
...
def create_argument_parser():
    ...
    parser.add_argument('--base-url', type=str, default=None, help='Server or API base url if not using http host and port.')
    ...
    parser.add_argument('--tokenizer', type=str, help='Name or path of the tokenizer, if not using the default tokenizer.')
    parser.add_argument('--use-beam-search', action='store_true')
    parser.add_argument('--num-prompts', type=int, default=1000, help='Number of prompts to process.')
    parser.add_argument('--logprobs', type=int, default=None, help='Number of logprobs-per-token to compute & return as part of the request. If unspecified, then either (1) if beam search is disabled, no logprobs are computed & a single dummy logprob is returned for each token; or (2) if beam search is enabled 1 logprob per token is computed')
    parser.add_argument('--request-rate', type=float, default=float('inf'), help='Number of requests per second. If this is inf, then all the requests are sent at time 0. Otherwise, we use Poisson process or gamma distribution to synthesize the request arrival times.')
    parser.add_argument('--burstiness', type=float, default=1.0, help='Burstiness factor of the request generation. Only take effect when request_rate is not inf. Default value is 1, which follows Poisson process. Otherwise, the request intervals follow a gamma distribution. A lower burstiness value (0 < burstiness < 1) results in more bursty requests. A higher burstiness value (burstiness > 1) results in a more uniform arrival of requests.')
    ...
    parser.add_argument('--save-detailed', action='store_true', help='When saving the results, whether to include per request information such as response, error, ttfs, tpots, etc.')
    parser.add_argument('--append-result', action='store_true', help='Append the benchmark result to the existing json file.')
    parser.add_argument('--metadata', metavar='KEY=VALUE', nargs='*', help='Key-value pairs (e.g, --metadata version=0.3.3 tp=1) for metadata of this run to be saved in the result JSON file for record keeping purposes.')
    parser.add_argument('--result-dir', type=str, default=None, help='Specify directory to save benchmark json results.If not specified, results are saved in the current directory.')
    parser.add_argument('--result-filename', type=str, default=None, help='Specify the filename to save benchmark json results.If not specified, results will be saved in {backend}-{args.request_rate}qps-{base_model_id}-{current_dt}.json format.')
    parser.add_argument('--ignore-eos', action='store_true', help='Set ignore_eos flag when sending the benchmark request.Warning: ignore_eos is not supported in deepspeed_mii and tgi.')
    parser.add_argument('--percentile-metrics', type=str, default='ttft,tpot,itl', help='Comma-separated list of selected metrics to report percentils. This argument specifies the metrics to report percentiles. Allowed metric names are "ttft", "tpot", "itl", "e2el". Default value is "ttft,tpot,itl".')
    parser.add_argument('--metric-percentiles', type=str, default='99', help='Comma-separated list of percentiles for selected metrics. To report 25-th, 50-th, and 75-th percentiles, use "25,50,75". Default value is "99". Use "--percentile-metrics" to select metrics.')
    parser.add_argument('--goodput', nargs='+', required=False, help='Specify service level objectives for goodput as "KEY:VALUE" pairs, where the key is a metric name, and the value is in milliseconds. Multiple "KEY:VALUE" pairs can be provided, separated by spaces. Allowed request level metric names are "ttft", "tpot", "e2el". For more context on the definition of goodput, refer to DistServe paper: https://arxiv.org/pdf/2401.09670 and the blog: https://hao-ai-lab.github.io/blogs/distserve')
    ...
    sampling_group.add_argument('--top-p', type=float, default=None, help='Top-p sampling parameter. Only has effect on openai-compatible backends.')
    sampling_group.add_argument('--top-k', type=int, default=None, help='Top-k sampling parameter. Only has effect on openai-compatible backends.')
    sampling_group.add_argument('--min-p', type=float, default=None, help='Min-p sampling parameter. Only has effect on openai-compatible backends.')
    sampling_group.add_argument('--temperature', type=float, default=None, help='Temperature sampling parameter. Only has effect on openai-compatible backends. If not specified, default to greedy decoding (i.e. temperature==0.0).')
    ...
    parser.add_argument('--served-model-name', type=str, default=None, help='The model name used in the API. If not specified, the model name will be the same as the ``--model`` argument. ')
    parser.add_argument('--lora-modules', nargs='+', default=None, help='A subset of LoRA module names passed in when launching the server. For each request, the script chooses a LoRA module at random.')
    parser.add_argument('--ramp-up-strategy', type=str, default=None, choices=['linear', 'exponential'], help='The ramp-up strategy. This would be used to ramp up the request rate from initial RPS to final RPS rate (specified by --ramp-up-start-rps and --ramp-up-end-rps). over the duration of the benchmark.')
    parser.add_argument('--ramp-up-start-rps', type=int, default=None, help='The starting request rate for ramp-up (RPS). Needs to be specified when --ramp-up-strategy is used.')
    parser.add_argument('--ramp-up-end-rps', type=int, default=None, help='The ending request rate for ramp-up (RPS). Needs to be specified when --ramp-up-strategy is used.')
    return parser
