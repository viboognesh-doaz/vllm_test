...
tool_functions = {'get_weather': get_weather, 'calculate_expression': calculate_expression, 'translate_text': translate_text}
            ...
            for tool_call_chunk in chunk.choices[0].delta.tool_calls:
                if hasattr(tool_call_chunk, 'id') and tool_call_chunk.id:
                    ...
                    if current_id not in tool_calls:
                        tool_calls[current_id] = {'function_name': None, 'function_args': '', 'function_id': current_id}
                if hasattr(tool_call_chunk, 'function') and current_id and (current_id in tool_calls):
                    if hasattr(tool_call_chunk.function, 'name') and tool_call_chunk.function.name:
                        tool_calls[current_id]['function_name'] = tool_call_chunk.function.name
                    ...
                    if hasattr(tool_call_chunk.function, 'arguments') and tool_call_chunk.function.arguments:
                        tool_calls[current_id]['function_args'] += tool_call_chunk.function.arguments
    ...
    for tool_id, tool_data in tool_calls.items():
        ...
        if function_name and function_args:
            try:
                ...
                follow_up_messages.append({'role': 'assistant', 'tool_calls': [{'id': function_id, 'type': 'function', 'function': {'name': function_name, 'arguments': function_args}}]})
                follow_up_messages.append({'role': 'tool', 'tool_call_id': function_id, 'content': function_result})
            except Exception as e:
                print(f'Error executing function: {e}')
    if len(follow_up_messages) > 1:
        follow_up_response = client.chat.completions.create(model=client.models.list().data[0].id, messages=follow_up_messages, stream=True)
        print('\n--- Follow-up Response ---')
...
def run_test_case(query, test_name):
    ...
    response = client.chat.completions.create(model=client.models.list().data[0].id, messages=[{'role': 'user', 'content': query}], tools=tools, tool_choice='auto', stream=True)
...
def main():
    ...
    test_cases = [('I want to know the weather in San Francisco', 'Weather Information'), ('Calculate 25 * 17 + 31', 'Math Calculation'), ("Translate 'Hello world' to Spanish", 'Text Translation'), ('What is the weather in Tokyo and New York in celsius', 'Multiple Tool Usage')]
