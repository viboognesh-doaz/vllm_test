...
def init_vectorstore(config: dict[str, Any], documents: list[Document]):
    ...
    """
    return Milvus.from_documents(documents=documents, embedding=OpenAIEmbeddings(model=config['embedding_model'], openai_api_key=config['vllm_api_key'], openai_api_base=config['vllm_embedding_endpoint']), connection_args={'uri': config['uri']}, drop_old=True)

def init_llm(config: dict[str, Any]):
    """
    Initialize llm
    """
    return ChatOpenAI(model=config['chat_model'], openai_api_key=config['vllm_api_key'], openai_api_base=config['vllm_chat_endpoint'])
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--vllm-embedding-endpoint', default='http://localhost:8000/v1', help='Base URL for embedding service')
    parser.add_argument('--vllm-chat-endpoint', default='http://localhost:8001/v1', help='Base URL for chat service')
...
def init_config(args: Namespace):
    ...
    """
    return {'vllm_api_key': args.vllm_api_key, 'vllm_embedding_endpoint': args.vllm_embedding_endpoint, 'vllm_chat_endpoint': args.vllm_chat_endpoint, 'uri': args.uri, 'embedding_model': args.embedding_model, 'chat_model': args.chat_model, 'url': args.url, 'chunk_size': args.chunk_size, 'chunk_overlap': args.chunk_overlap, 'top_k': args.top_k}
