...
class Proxy:
    ...
    def __init__(self, prefill_instances: list[str], decode_instances: list[str], model: str, scheduling_policy: SchedulingPolicy, custom_create_completion: Optional[Callable[[Request], StreamingResponse]]=None, custom_create_chat_completion: Optional[Callable[[Request], StreamingResponse]]=None):
    ...
    def setup_routes(self):
        self.router.post('/v1/completions', dependencies=[Depends(self.validate_json_request)])(self.custom_create_completion if self.custom_create_completion else self.create_completion)
        self.router.post('/v1/chat/completions', dependencies=[Depends(self.validate_json_request)])(self.custom_create_chat_completion if self.custom_create_chat_completion else self.create_chat_completion)
        self.router.get('/status', response_class=JSONResponse)(self.get_status)
        self.router.post('/instances/add', dependencies=[Depends(self.api_key_authenticate)])(self.add_instance_endpoint)
    ...
    async def add_instance_endpoint(self, request: Request):
        try:
            ...
            elif instance not in self.decode_instances:
            ...
            try:
                async with session.post(url=url, json=data, headers=headers) as response:
                    if 200 <= response.status < 300 or 400 <= response.status < 500:
                        ...
                        raise HTTPException(status_code=response.status, detail=f'Request failed with status {response.status}: {error_content}')
            except aiohttp.ClientError as e:
                logger.error('ClientError occurred: %s', str(e))
                raise HTTPException(status_code=502, detail='Bad Gateway: Error communicating with upstream server.') from e
            except Exception as e:
    ...
    async def get_status(self):
        status = {'prefill_node_count': len(self.prefill_instances), 'decode_node_count': len(self.decode_instances), 'prefill_nodes': self.prefill_instances, 'decode_nodes': self.decode_instances}
        return status
    ...
    async def create_chat_completion(self, raw_request: Request):
        try:
            ...
            prefill_instance = self.schedule(self.prefill_cycler)
            try:
                async for _ in self.forward_request(f'http://{prefill_instance}/v1/chat/completions', kv_prepare_request):
                    continue
            except HTTPException as http_exc:
                self.remove_instance_endpoint('prefill', prefill_instance)
                raise http_exc
...
class ProxyServer:
    ...
    def __init__(self, args: argparse.Namespace, scheduling_policy: Optional[SchedulingPolicy]=None, create_completion: Optional[Callable[[Request], StreamingResponse]]=None, create_chat_completion: Optional[Callable[[Request], StreamingResponse]]=None):
        self.validate_parsed_serve_args(args)
        self.port = args.port
        self.proxy_instance = Proxy(prefill_instances=[] if args.prefill is None else args.prefill, decode_instances=[] if args.decode is None else args.decode, model=args.model, scheduling_policy=scheduling_policy if scheduling_policy is not None else RoundRobinSchedulingPolicy(), custom_create_completion=create_completion, custom_create_chat_completion=create_chat_completion)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if model_cur_suffix != model_suffix:
                        raise ValueError(f'{instance} serves a different model: {model_cur} != {model}')
                else:
...
def parse_args():
    ...
    parser.add_argument('--prefill', '-p', type=str, nargs='+', help='List of prefill node URLs (host:port)')
    parser.add_argument('--decode', '-d', type=str, nargs='+', help='List of decode node URLs (host:port)')
