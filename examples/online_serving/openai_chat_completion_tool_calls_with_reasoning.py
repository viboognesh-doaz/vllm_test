...
tools = [{'type': 'function', 'function': {'name': 'get_current_weather', 'description': 'Get the current weather in a given location', 'parameters': {'type': 'object', 'properties': properties, 'required': ['city', 'state', 'unit']}}}]
messages = [{'role': 'user', 'content': 'Hi! How are you doing today?'}, {'role': 'assistant', 'content': "I'm doing well! How can I help you?"}, {'role': 'user', 'content': 'Can you tell me what the temperate will be in Dallas, in fahrenheit?'}]
...
def main():
    ...
    tool_calls = client.chat.completions.create(messages=messages, model=model, tools=tools)
    ...
    print(f'function arguments: {tool_calls.choices[0].message.tool_calls[0].function.arguments}')
    ...
    tool_calls = client.chat.completions.create(messages=messages, model=model, tools=tools, tool_choice={'type': 'function', 'function': {'name': 'get_current_weather'}})
    ...
    tool_calls_stream = client.chat.completions.create(messages=messages, model=model, tools=tools, tool_choice={'type': 'function', 'function': {'name': 'get_current_weather'}}, stream=True)
    chunks = list(tool_calls_stream)
    reasoning_content, arguments, function_names = extract_reasoning_and_calls(chunks)
    print(f'reasoning_content: {reasoning_content}')
    print(f'function name: {function_names[0]}')
