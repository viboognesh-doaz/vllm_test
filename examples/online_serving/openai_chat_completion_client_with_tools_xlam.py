    ...
    if response.choices[0].message.tool_calls:
        ...
        for tool_call in response.choices[0].message.tool_calls:
            ...
            try:
                ...
                print(f'\n--- Function Result ---\n{function_result}\n')
                assistant_tool_calls.append({'id': function_id, 'type': 'function', 'function': {'name': function_name, 'arguments': function_args}})
                tool_results.append({'role': 'tool', 'tool_call_id': function_id, 'content': function_result})
            except Exception as e:
                print(f'Error executing function: {e}')
        assistant_message['tool_calls'] = assistant_tool_calls
        follow_up_messages = [{'role': 'user', 'content': original_query}, assistant_message]
        follow_up_messages.extend(tool_results)
        follow_up_response = client.chat.completions.create(model=client.models.list().data[0].id, messages=follow_up_messages, stream=False)
        print('\n--- Follow-up Response ---')
...
def run_test_case(query, test_name):
    ...
    response = client.chat.completions.create(model=client.models.list().data[0].id, messages=[{'role': 'user', 'content': query}], tools=tools, tool_choice='auto', stream=False)
