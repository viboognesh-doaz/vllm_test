...
question_per_audio_count = {0: 'What is 1+1?', 1: 'What is recited in the audio?', 2: 'What sport and what nursery rhyme are referenced?'}
...
def run_voxtral(question: str, audio_count: int) -> ModelRequestData:
    ...
    engine_args = EngineArgs(model=model_name, max_model_len=8192, max_num_seqs=2, limit_mm_per_prompt={'audio': audio_count}, config_format='mistral', load_format='mistral', tokenizer_mode='mistral', enforce_eager=True, enable_chunked_prefill=False)
    ...
    return ModelRequestData(engine_args=engine_args, prompt_token_ids=prompt_ids, multi_modal_data=multi_modal_data)

def run_granite_speech(question: str, audio_count: int) -> ModelRequestData:
    model_name = 'ibm-granite/granite-speech-3.3-8b'
    engine_args = EngineArgs(model=model_name, trust_remote_code=True, max_model_len=2048, max_num_seqs=2, enable_lora=True, max_lora_rank=64, limit_mm_per_prompt={'audio': audio_count})
    ...
    return ModelRequestData(engine_args=engine_args, prompt=prompts, lora_requests=[LoRARequest('speech', 1, speech_lora_path)])

def run_minicpmo(question: str, audio_count: int) -> ModelRequestData:
    model_name = 'openbmb/MiniCPM-o-2_6'
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
    engine_args = EngineArgs(model=model_name, trust_remote_code=True, max_model_len=4096, max_num_seqs=2, limit_mm_per_prompt={'audio': audio_count})
    ...
    return ModelRequestData(engine_args=engine_args, prompt=prompt, stop_token_ids=stop_token_ids)
...
def run_phi4mm(question: str, audio_count: int) -> ModelRequestData:
    ...
    engine_args = EngineArgs(model=model_path, trust_remote_code=True, max_model_len=12800, max_num_seqs=2, enable_lora=True, max_lora_rank=320, limit_mm_per_prompt={'audio': audio_count})
    return ModelRequestData(engine_args=engine_args, prompt=prompts, lora_requests=[LoRARequest('speech', 1, speech_lora_path)])

def run_qwen2_audio(question: str, audio_count: int) -> ModelRequestData:
    model_name = 'Qwen/Qwen2-Audio-7B-Instruct'
    engine_args = EngineArgs(model=model_name, max_model_len=4096, max_num_seqs=5, limit_mm_per_prompt={'audio': audio_count})
...
def run_ultravox(question: str, audio_count: int) -> ModelRequestData:
    ...
    engine_args = EngineArgs(model=model_name, max_model_len=4096, max_num_seqs=5, trust_remote_code=True, limit_mm_per_prompt={'audio': audio_count})
    return ModelRequestData(engine_args=engine_args, prompt=prompt)
...
def run_whisper(question: str, audio_count: int) -> ModelRequestData:
    ...
    engine_args = EngineArgs(model=model_name, max_model_len=448, max_num_seqs=5, limit_mm_per_prompt={'audio': audio_count})
    return ModelRequestData(engine_args=engine_args, prompt=prompt)
model_example_map = {'voxtral': run_voxtral, 'granite_speech': run_granite_speech, 'minicpmo': run_minicpmo, 'phi4_mm': run_phi4mm, 'qwen2_audio': run_qwen2_audio, 'qwen2_5_omni': run_qwen2_5_omni, 'ultravox': run_ultravox, 'whisper': run_whisper}

def parse_args():
    parser = FlexibleArgumentParser(description='Demo on using vLLM for offline inference with audio language models')
    parser.add_argument('--model-type', '-m', type=str, default='ultravox', choices=model_example_map.keys(), help='Huggingface "model_type".')
    parser.add_argument('--num-prompts', type=int, default=1, help='Number of prompts to run.')
    parser.add_argument('--num-audios', type=int, default=1, choices=[0, 1, 2], help='Number of audio items per prompt.')
...
def main(args):
    ...
    req_data = model_example_map[model](question_per_audio_count[audio_count], audio_count)
