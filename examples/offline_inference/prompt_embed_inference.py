...
def batch_prompt_inference(llm: LLM, tokenizer: PreTrainedTokenizer, embedding_layer: torch.nn.Module):
    chats = [[{'role': 'user', 'content': 'Please tell me about the capital of France.'}], [{'role': 'user', 'content': 'When is the day longest during the year?'}], [{'role': 'user', 'content': 'Where is bigger, the moon or the sun?'}]]
