...
def main(args: argparse.Namespace):
    ...
    xp.trace_detached('localhost:9012', profile_dir, delay_ms=DELAY_MS, duration_ms=DURATION_MS)
    if DELAY_MS == 0:
        time.sleep(1.0)
    profile_latencies = []
...
def parse_args():
    ...
    parser.add_argument('--num-iters-warmup', type=int, default=5, help='Number of iterations to run for warmup.')
    parser.add_argument('--num-iters', type=int, default=1, help='Number of iterations to run for profiling.')
    parser.add_argument('--profile-result-dir', type=str, default='profiles', help='path to save the pytorch profiler output. Can be visualized with ui.perfetto.dev or Tensorboard (https://cloud.google.com/tpu/docs/pytorch-xla-performance-profiling-tpu-vm).')
