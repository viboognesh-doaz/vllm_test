...
def compute_request_output_lengths(batch_size: int, step_requests: list[int]) -> OutputLen_NumReqs_Map:
    """
    ...
    for num_requests_at_step in reversed(step_requests):
        ...
        output_length -= 1
    assert sum(ol_nr.values()) == batch_size, f'Number of requests in output-length assignment does not match batch-size.\n batch size {batch_size} - step requests {step_requests} - assignments {ol_nr}'
    assert all((ol >= 1 and ol <= num_steps for ol in ol_nr)), f'Output lengths of requests should be in range [1, num-engine-steps].\n batch size {batch_size} - step requests {step_requests} - assignments {ol_nr}'
    return ol_nr
...
def determine_requests_per_step(context: ProfileContext) -> list[int]:
    ...
    assert context.complete_num_requests_per_step and context.complete_num_requests_per_step > 0, f'Expected a positive complete_num_requests_per_step argument.Instead got {context.complete_num_requests_per_step}'
    step_requests = [context.batch_size, context.batch_size]
...
def run_profile(context: ProfileContext, csv_output: Optional[str], json_output: Optional[str]):
    print('Run profile with:')
    for key, value in asdict(context).items():
        print(f'  {key} = {value}')
    requests_per_step: list[int] = determine_requests_per_step(context)
    ol_nr: OutputLen_NumReqs_Map = compute_request_output_lengths(context.batch_size, requests_per_step)
    num_steps_to_profile: int = len(requests_per_step)
    max_output_len: int = max(ol_nr.keys())
    assert max_output_len >= 1
    sampling_params = SamplingParams(temperature=0.8, top_p=0.95, max_tokens=None, ignore_eos=True)
    ...
    if batch_size * prompt_len > max_num_batched_tokens:
        print(f'ERROR: chosen batch_size * prompt_len ({batch_size} * {prompt_len} = {batch_size * prompt_len}) is  larger than max_num_batched_tokens ({max_num_batched_tokens}) and therefore cannot be run in a single profile step, please choose a smaller batch size or prompt length, or increase --max-num-batched-tokens')
        sys.exit(-1)
    if batch_size > max_num_seqs:
        print(f'ERROR: chosen batch_size ({batch_size}) is larger than max_num_seqs ({max_num_seqs}) and therefore cannot be run in a single profile step, please choose a smaller batch size')
        sys.exit(-1)
    ...
    if prompt_len + max_output_len > llm.llm_engine.model_config.max_model_len:
        print(f"ERROR: chosen prompt_len + max_output_len ({prompt_len} + {max_output_len} = {prompt_len + max_output_len}) is larger than the model's max_model_len ({max_model_len}), please choose a smaller prompt_len or max_output_len, or increase --max-model-len")
        sys.exit(-1)
    ...
    def add_requests():
        ...
        for i in range(batch_size):
            ...
            prompt_token_ids = torch.randint(llm.get_tokenizer().vocab_size, size=(prompt_len,)).tolist()
            llm.llm_engine.add_request(request_id=f'seq{i}', prompt={'prompt_token_ids': prompt_token_ids}, params=sampling_params)
    ...
    if csv_output:
        csv_filename_base = csv_output[:-4] if csv_output.endswith('.csv') else csv_output
    ...
    if json_output:
        ...
        json_dict = {'context': {'python_version': f'{sys.version}', 'torch_version': f'{torch.__version__}', 'torch_cuda_version': f'{torch.version.cuda}', 'cuda_devices': f'{cuda_devices}', **asdict(context)}, 'prefill': prefill_results.convert_stats_to_dict()}
        if has_decode:
...
def parse_args():
    ...
    parser.add_argument('--csv', type=str, default=None, help='Export the results as multiple csv file. This should be the root filename, will create <filename>_prefill_model_table.csv, <filename>_prefill_summary_table.csv, <filename>_decode_model_table.csv, and <filename>_decode_summary_table.csv')
    parser.add_argument('--json', type=str, default=None, help='Export the results as a json file. This should be the filename')
    parser.add_argument('--save-chrome-traces-folder', type=str, help='Save chrome traces for the prefill and decode will save traces as prefill.json and decode_1.json, etc. inside this folder')
    parser.add_argument('--prompt-len', type=int, default=PROMPT_LEN_DEFAULT, help=f'Length of the random prompt to use when profiling, all batched requests use the same prompt_len, default={PROMPT_LEN_DEFAULT}')
    parser.add_argument('--batch-size', type=int, default=BATCH_SIZE_DEFAULT, help=f'Number of requests to run as a single batch, default={BATCH_SIZE_DEFAULT}')
    subparsers = parser.add_subparsers(dest='cmd')
    run_num_steps_parser = subparsers.add_parser('run_num_steps', help='This variation profiles n engine.step() invocations.')
    run_num_steps_parser.add_argument('-n', '--num-steps', type=int, help='Number of engine steps to profile.\nSetting it to 1, profiles only the prefill step.\nSetting it to 2, profiles the prefill and first decode step\nSetting it to 3, profiles the prefill, 1st and 2nd decode steps\nand so on ...')
    run_to_completion_parser = subparsers.add_parser('run_to_completion', help='This variation profiles all the engine.step() invocationsuntil the engine exhausts all submitted requests.')
    run_to_completion_parser.add_argument('-n', '--complete-num-requests-per-step', type=int, help='Complete complete_num_requests_per_step requests every decode step.For e.g., with batch_size 128 and complete_num_requests_per_step 32,the profiler is run for 6 engine steps, with the steps processing, 128, 128, 96, 64, 32, 1 requests respectively.\nNote that we tack-on a one-request step at the end as it is often useful.')
    EngineArgs.add_cli_args(parser)
    return parser.parse_args()

def main(args):
    context = ProfileContext(engine_args=EngineArgs.from_cli_args(args), **{k: v for k, v in vars(args).items() if k in inspect.signature(ProfileContext).parameters})
