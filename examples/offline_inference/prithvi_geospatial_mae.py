...
class PrithviMAE:
    ...
    def run(self, input_data, location_coords):
        ...
        mm_data = {'pixel_values': torch.empty(0) if input_data is None else input_data, 'location_coords': torch.empty(0) if location_coords is None else location_coords}
...
def generate_datamodule():
    datamodule = Sen1Floods11NonGeoDataModule(data_root=datamodule_config['data_root'], batch_size=datamodule_config['batch_size'], num_workers=datamodule_config['num_workers'], bands=datamodule_config['bands'], drop_last=datamodule_config['drop_last'], test_transform=datamodule_config['test_transform'])
    return datamodule
...
def load_example(file_paths: list[str], mean: list[float]=None, std: list[float]=None, indices: Union[list[int], None]=None):
    ...
    for file in file_paths:
        ...
        try:
            match = re.search('(\\d{7,8}T\\d{6})', file)
            if match:
...
def run_model(input_data, temporal_coords, location_coords, model, datamodule, img_size, lightning_model=None):
    ...
    pred_imgs = rearrange(pred_imgs, '(b h1 w1) c h w -> b c (h1 h) (w1 w)', h=img_size, w=img_size, b=1, c=1, h1=h1, w1=w1)
...
def parse_args():
    ...
    parser.add_argument('--data_file', type=str, default='./India_900498_S2Hand.tif', help='Path to the file.')
    parser.add_argument('--output_dir', type=str, default='output', help='Path to the directory where to save outputs.')
    parser.add_argument('--input_indices', default=[1, 2, 3, 8, 11, 12], type=int, nargs='+', help='0-based indices of the six Prithvi channels to be selected from the  input. By default selects [1,2,3,8,11,12] for S2L1C data.')
    parser.add_argument('--rgb_outputs', action='store_true', help='If present, output files will only contain RGB channels. Otherwise, all bands will be saved.')

def main(data_file: str, output_dir: str, rgb_outputs: bool, input_indices: list[int]=None):
    os.makedirs(output_dir, exist_ok=True)
    model_obj = PrithviMAE()
    datamodule = generate_datamodule()
    img_size = 256
    input_data, temporal_coords, location_coords, meta_data = load_example(file_paths=[data_file], indices=input_indices)
    meta_data = meta_data[0]
    if input_data.mean() > 1:
        input_data = input_data / 10000
    channels = [datamodule_config['bands'].index(b) for b in ['RED', 'GREEN', 'BLUE']]
    ...
    if input_data.mean() < 1:
        input_data = input_data * 10000
    rgb_orig = process_channel_group(orig_img=torch.Tensor(input_data[0, :, 0, ...]), channels=channels)
    ...
    save_geotiff(image=_convert_np_uint8(img_pred), output_path=img_pred_file, meta=meta_data)
    if rgb_outputs:
        rgb_file = os.path.join(output_dir, f'original_rgb_{os.path.splitext(os.path.basename(data_file))[0]}.tiff')
        save_geotiff(image=_convert_np_uint8(rgb_orig), output_path=rgb_file, meta=meta_data)
