...
from vllm.model_executor.model_loader.tensorizer import TensorizerArgs, TensorizerConfig, tensorize_lora_adapter, tensorize_vllm_model, tensorizer_kwargs_arg
...
def get_parser():
    ...
    serialize_parser.add_argument('--serialization-kwargs', type=tensorizer_kwargs_arg, required=False, help="A JSON string containing additional keyword arguments to pass to Tensorizer's TensorSerializer during serialization.")
    ...
    deserialize_parser.add_argument('--path-to-tensors', type=str, required=False, help='The local path or S3 URI to the model tensors to deserialize. ')
    deserialize_parser.add_argument('--serialized-directory', type=str, required=False, help='Directory with model artifacts for loading. Assumes a model.tensors file exists therein. Can supersede --path-to-tensors.')
    ...
    deserialize_parser.add_argument('--deserialization-kwargs', type=tensorizer_kwargs_arg, required=False, help="A JSON string containing additional keyword arguments to pass to Tensorizer's `TensorDeserializer` during deserialization.")
...
def merge_extra_config_with_tensorizer_config(extra_cfg: dict, cfg: TensorizerConfig):
    for k, v in extra_cfg.items():
        if hasattr(cfg, k):
            setattr(cfg, k, v)
            logger.info('Updating TensorizerConfig with %s from --model-loader-extra-config provided', k)

def deserialize(args, tensorizer_config):
    if args.lora_path:
        tensorizer_config.lora_dir = tensorizer_config.tensorizer_dir
        llm = LLM(model=args.model, load_format='tensorizer', tensor_parallel_size=args.tensor_parallel_size, model_loader_extra_config=tensorizer_config, enable_lora=True)
        ...
        print(llm.generate(prompts, sampling_params, lora_request=LoRARequest('sql-lora', 1, args.lora_path, tensorizer_config_dict=tensorizer_config.to_serializable())))
    else:
...
def main():
    ...
    s3_endpoint = getattr(args, 's3_endpoint', None) or os.environ.get('S3_ENDPOINT_URL', None)
    ...
    if args.command == 'serialize':
        ...
        tensorizer_config = TensorizerConfig(tensorizer_uri=model_path, encryption_keyfile=keyfile, serialization_kwargs=args.serialization_kwargs or {}, **credentials)
        if args.lora_path:
            tensorizer_config.lora_dir = tensorizer_config.tensorizer_dir
        ...
        tensorizer_config = TensorizerConfig(tensorizer_uri=args.path_to_tensors, tensorizer_dir=args.serialized_directory, encryption_keyfile=keyfile, deserialization_kwargs=args.deserialization_kwargs or {}, **credentials)
        merge_extra_config_with_tensorizer_config(extra_config, tensorizer_config)
