...
class OvisProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {'text_kwargs': {'padding': False}, 'images_kwargs': {'max_partition': 9, 'covering_threshold': 0.9, 'convert_to_rgb': True, 'return_tensors': 'pt'}}
...
class OvisProcessor(ProcessorMixin):
    ...
    def __init__(self, image_processor=None, tokenizer=None, chat_template=None, image_pad_token=None, image_segment_len=255, **kwargs):
    ...
    def extra_special_tokens(self):
        ...
        extra_special_tokens = {'image_token': -200, 'image_atom': -300, 'image_start': -301, 'image_prefix': -302, 'image_col_sep': -303, 'image_row_sep': -304, 'image_end': -305, 'image_pad': image_pad_token_id}
        return extra_special_tokens

    def __call__(self, images: ImageInput=None, text: Union[TextInput, PreTokenizedInput, list[TextInput], list[PreTokenizedInput]]=None, **kwargs: Unpack[OvisProcessorKwargs]) -> BatchFeature:
        """
        ...
        """
        output_kwargs = self._merge_kwargs(OvisProcessorKwargs, tokenizer_init_kwargs=self.tokenizer.init_kwargs, **kwargs)
        image_features = {}
        if images is not None:
            ...
            for image in images if isinstance(images, list) else [images]:
                pixel_values, image_placeholders, grid = self.preprocess_image(image=image, **output_kwargs['images_kwargs'])
            ...
            for ids_tensor in tokenized_batched_text:
                if image_token_id in ids_tensor and 'image_placeholders' in image_features:
                    if idx < len(image_features['image_placeholders']):
                        ...
                        raise RuntimeError('Mismatch between the images you provided and the number of placeholder present in the text')
    ...
    def construct_image_indicators(self, grid):
        image_placeholders = [self.get_token_value('image_start'), self.get_token_value('image_atom'), self.get_token_value('image_prefix')]
        if grid[0] * grid[1] > 1:
    ...
    def post_process_image_text_to_text(self, generated_outputs):
        ...
        """
        return self.tokenizer.batch_decode(generated_outputs, skip_special_tokens=True, clean_up_tokenization_spaces=False)
