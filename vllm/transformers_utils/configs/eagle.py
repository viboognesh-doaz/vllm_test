...
class EAGLEConfig(PretrainedConfig):
    ...
    def __init__(self, model: Union[PretrainedConfig, dict, None]=None, truncated_vocab_size: Optional[int]=None, method: Optional[str]='eagle', **kwargs):
        ...
        if method == 'eagle':
            ...
            kwargs['architectures'] = [f'Eagle{arch}' if not arch.startswith('Eagle') else arch for arch in self.model.architectures]
        elif method == 'eagle3':
            assert self.model is not None, 'model should not be None when method is eagle3'
            kwargs['architectures'] = [f'Eagle3{arch}' if not arch.startswith('Eagle3') else arch for arch in self.model.architectures]
        else:
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike], **kwargs) -> 'EAGLEConfig':
