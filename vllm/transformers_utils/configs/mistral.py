...
def _remap_mistral_vision_args(config: dict) -> dict:
    ...
    config = {'model_type': 'pixtral', 'architectures': ['PixtralForConditionalGeneration'], 'text_config': PretrainedConfig.from_dict(config), 'vision_config': PretrainedConfig.from_dict(vision_config)}
    if quant_config:
        config['quantization_config'] = quant_config
    return config

def _remap_mistral_yarn_args(config: dict) -> dict:
    _map = {'beta': 'beta_fast', 'alpha': 'beta_slow'}
    yarn_config = config.get('yarn') or {}
    renamed_yarn_config = {_map.get(k, k): v for k, v in yarn_config.items()}
    config['rope_scaling'] = {'rope_type': 'yarn', 'mscale_all_dim': 1, **renamed_yarn_config}
    return config

def _remap_general_mistral_args(config: dict) -> dict:
    config_mapping = {'dim': 'hidden_size', 'norm_eps': 'rms_norm_eps', 'n_kv_heads': 'num_key_value_heads', 'n_layers': 'num_hidden_layers', 'n_heads': 'num_attention_heads', 'hidden_dim': 'intermediate_size'}
    top_level_mapping_with_default = {'model_type': ('model_type', 'transformer'), 'hidden_act': ('activation', 'silu'), 'tie_word_embeddings': ('tied_embeddings', False), 'max_seq_len': ('max_seq_len', 128000), 'max_position_embeddings': ('max_position_embeddings', 128000)}
