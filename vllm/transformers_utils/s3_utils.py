...
def glob(s3=None, path: str='', allow_pattern: Optional[list[str]]=None) -> list[str]:
    """
...
def list_files(s3, path: str, allow_pattern: Optional[list[str]]=None, ignore_pattern: Optional[list[str]]=None) -> tuple[str, str, list[str]]:
    """
...
class S3Model:
    ...
    def pull_files(self, s3_model_path: str='', allow_pattern: Optional[list[str]]=None, ignore_pattern: Optional[list[str]]=None) -> None:
        """
        ...
        bucket_name, base_dir, files = list_files(self.s3, s3_model_path, allow_pattern, ignore_pattern)
        if len(files) == 0:
            return
        for file in files:
            destination_file = os.path.join(self.dir, file.removeprefix(base_dir).lstrip('/'))
