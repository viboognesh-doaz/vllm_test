    ...
    if partition_list_str is not None:
        ...
        if (remaining_layers := (num_hidden_layers % pp_size)):
            for i in range(2, remaining_layers + 2):
                partitions[-i] += 1
            logger.info('Hidden layers were unevenly partitioned: [%s]. This can be manually overridden using the VLLM_PP_LAYER_PARTITION environment variable', ','.join(str(p) for p in partitions))
