...
class XlaQKVParallelLinear(nn.Module):
    ...
    def _shard_weight(self, mesh: 'xs.Mesh'):
        ...
        if self.q_bias is not None:
            assert self.k_bias is not None and self.v_bias is not None, 'QKVParallelLinear should have q, k, and v biases together.'
    ...
    def _load_weights_from_qkv_linear(self, qkv_linear: nn.Module):
        ...
        if qkv_linear.bias is not None:
            ...
            k_bias = Parameter(qkv_linear.bias[q_proj_size:q_proj_size + k_proj_size], requires_grad=False)
...
def partition_qkv_parallel_linear(layer: torch.nn.Module, mesh: xs.Mesh) -> torch.nn.Module:
    ...
    return xla_layer
MODULE_TYPE_TO_WRAPPING_FUNC = OrderedDict([('QKVParallelLinear', partition_qkv_parallel_linear), ('ColumnParallelLinear', partition_column_parallel_linear), ('RowParallelLinear', partition_row_parallel_linear)])
