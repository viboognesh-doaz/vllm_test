...
from vllm.platforms import current_platform
from vllm.platforms import current_platform
from vllm.utils import direct_register_custom_op, get_distributed_init_method, resolve_obj_by_qualname, supports_custom_op
import contextlib
import gc
import pickle
import ray
...
def _split_tensor_dict(tensor_dict: dict[str, Union[torch.Tensor, Any]]) -> tuple[list[tuple[str, Any]], list[torch.Tensor]]:
...
if supports_custom_op():
    direct_register_custom_op(op_name='all_reduce', op_func=all_reduce, mutates_args=[], fake_impl=all_reduce_fake, dispatch_key=current_platform.dispatch_key)
    direct_register_custom_op(op_name='reduce_scatter', op_func=reduce_scatter, mutates_args=[], fake_impl=reduce_scatter_fake, dispatch_key=current_platform.dispatch_key)
    direct_register_custom_op(op_name='all_gather', op_func=all_gather, mutates_args=[], fake_impl=all_gather_fake, dispatch_key=current_platform.dispatch_key)
...
class GroupCoordinator:
    ...
    mq_broadcaster: Optional[Any]

    def __init__(self, group_ranks: list[list[int]], local_rank: int, torch_distributed_backend: Union[str, Backend], use_device_communicator: bool, use_message_queue_broadcaster: bool=False, group_name: Optional[str]=None):
        ...
        self.device_communicator: DeviceCommunicatorBase = None
        if use_device_communicator and self.world_size > 1:
            device_comm_cls = resolve_obj_by_qualname(current_platform.get_device_communicator_cls())
            self.device_communicator = device_comm_cls(cpu_group=self.cpu_group, device=self.device, device_group=self.device_group, unique_name=self.unique_name)
    ...
    def all_gather(self, input_: torch.Tensor, dim: int=-1) -> torch.Tensor:
        ...
        if self.use_custom_op_call:
            return torch.ops.vllm.all_gather(input_, dim, world_size, group_name=self.unique_name)
        else:
    ...
    def reduce_scatter(self, input_: torch.Tensor, dim: int=-1) -> torch.Tensor:
        ...
        assert -input_.dim() <= dim < input_.dim(), f'Invalid dim ({dim}) for input tensor with shape {input_.size()}'
        if self.use_custom_op_call:
            return torch.ops.vllm.reduce_scatter(input_, dim, world_size, group_name=self.unique_name)
        else:
    ...
    def broadcast_tensor_dict(self, tensor_dict: Optional[dict[str, Union[torch.Tensor, Any]]]=None, src: int=0, group: Optional[ProcessGroup]=None, metadata_group: Optional[ProcessGroup]=None) -> Optional[dict[str, Union[torch.Tensor, Any]]]:
    ...
    def send_tensor_dict(self, tensor_dict: dict[str, Union[torch.Tensor, Any]], dst: Optional[int]=None, all_gather_group: Optional['GroupCoordinator']=None) -> Optional[dict[str, Union[torch.Tensor, Any]]]:
    ...
    def recv_tensor_dict(self, src: Optional[int]=None, all_gather_group: Optional['GroupCoordinator']=None) -> Optional[dict[str, Union[torch.Tensor, Any]]]:
    ...
    def dispatch(self, hidden_states: torch.Tensor, router_logits: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        if self.device_communicator is not None:
            return self.device_communicator.dispatch(hidden_states, router_logits)
        else:
...
def init_model_parallel_group(group_ranks: list[list[int]], local_rank: int, backend: str, use_message_queue_broadcaster: bool=False, group_name: Optional[str]=None) -> GroupCoordinator:
    return GroupCoordinator(group_ranks=group_ranks, local_rank=local_rank, torch_distributed_backend=backend, use_device_communicator=True, use_message_queue_broadcaster=use_message_queue_broadcaster, group_name=group_name)
...
def init_distributed_environment(world_size: int=-1, rank: int=-1, distributed_init_method: str='env://', local_rank: int=-1, backend: str='nccl'):
    ...
    if config is not None and config.parallel_config.data_parallel_size > 1:
        ...
        logger.info('Adjusting world_size=%d rank=%d distributed_init_method=%s for DP', world_size, rank, distributed_init_method)
...
def initialize_model_parallel(tensor_model_parallel_size: int=1, pipeline_model_parallel_size: int=1, backend: Optional[str]=None) -> None:
    """
    ...
    all_ranks = torch.arange(world_size).reshape(-1, data_parallel_size, pipeline_model_parallel_size, tensor_model_parallel_size)
    global _TP
    ...
    logger.info('rank %s in world size %s is assigned as DP rank %s, PP rank %s, TP rank %s, EP rank %s', rank, world_size, _DP.rank_in_group, _PP.rank_in_group, _TP.rank_in_group, _EP.rank_in_group)

def ensure_model_parallel_initialized(tensor_model_parallel_size: int, pipeline_model_parallel_size: int, backend: Optional[str]=None) -> None:
    ...
    assert pp_world_size == pipeline_model_parallel_size, f'pipeline parallel group already initialized, but of unexpected size: pp_world_size={pp_world_size!r} vs. pipeline_model_parallel_size={pipeline_model_parallel_size!r}'
...
def in_the_same_node_as(pg: Union[ProcessGroup, StatelessProcessGroup], source_rank: int=0) -> list[bool]:
    ...
    if isinstance(pg, ProcessGroup):
        assert torch.distributed.get_backend(pg) != torch.distributed.Backend.NCCL, 'in_the_same_node_as should be tested with a non-NCCL group.'
