...
def get_kv_connector_cache_layout():
    ...
    if kv_config is not None and vllm_config.model_config is None:
        logger.warning_once('Unable to detect current VLLM config. Defaulting to NHD kv cache layout.')
    elif kv_config is not None:
        use_mla = vllm_config.model_config.use_mla
        if not use_mla and kv_config.kv_connector == 'NixlConnector':
            logger.info_once('NixlConnector detected. Setting KV cache layout to HND for better xfer performance.')
            return 'HND'
    return 'NHD'
