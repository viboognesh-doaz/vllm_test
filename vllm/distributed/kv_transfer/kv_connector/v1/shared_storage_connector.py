...
class ReqMeta:
    ...
    @staticmethod
    def make_meta(token_ids: list[int], block_ids: list[int], block_size: int, is_store: bool) -> 'ReqMeta':
        ...
        return ReqMeta(token_ids=token_ids_tensor, slot_mapping=slot_mapping, is_store=is_store)
...
class SharedStorageConnectorMetadata(KVConnectorMetadata):
    ...
    def add_request(self, token_ids: list[int], block_ids: list[int], block_size: int, is_store: bool) -> None:
        self.requests.append(ReqMeta.make_meta(token_ids, block_ids, block_size, is_store))
...
class SharedStorageConnector(KVConnectorBase_V1):
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> KVConnectorMetadata:
        ...
        for new_req in scheduler_output.scheduled_new_reqs:
            if new_req.req_id in self._requests_need_load:
                meta.add_request(token_ids=new_req.prompt_token_ids, block_ids=new_req.block_ids[0], block_size=self._block_size, is_store=False)
                total_need_load += 1
            elif not self._found_match_for_request(new_req):
                meta.add_request(token_ids=new_req.prompt_token_ids, block_ids=new_req.block_ids[0], block_size=self._block_size, is_store=True)
        ...
        for i, req_id in enumerate(cached_reqs.req_ids):
            ...
            if req_id in self._requests_need_load:
                ...
                meta.add_request(token_ids=token_ids, block_ids=block_ids, block_size=self._block_size, is_store=False)
                total_need_load += 1
    ...
    def _found_match_for_request(self, request: 'Request') -> bool:
        ...
        """
        num_tokens_to_check = align_to_block_size(len(request.prompt_token_ids) - 1, self._block_size)
        foldername = self._generate_foldername_debug(torch.tensor(request.prompt_token_ids)[:num_tokens_to_check], create_folder=False)
        return os.path.exists(foldername)

    def _generate_foldername_debug(self, input_ids: torch.Tensor, create_folder=False) -> str:
    ...
    def _generate_filename_debug(self, layer_name: str, input_ids: torch.Tensor) -> str:
