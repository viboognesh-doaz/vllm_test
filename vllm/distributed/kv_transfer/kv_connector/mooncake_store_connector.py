...
class MooncakeStoreConnector(KVConnectorBase):
    ...
    def __init__(self, rank: int, local_rank: int, config: VllmConfig):
        ...
        if self.kv_transfer_config.kv_connector == 'MooncakeStoreConnector':
            ...
            if not use_mooncake_store:
                raise ValueError("To use MooncakeStoreConnector, you need to pass the ENV: 'MOONCAKE_CONFIG_PATH=/path/to/mooncake_config.json'.")
            else:
                logger.info('Initializing KVStoreConnector under kv_transfer_config %s', self.kv_transfer_config)
    ...
    def send_kv_caches_and_hidden_states(self, model_executable: torch.nn.Module, model_input: 'ModelInputForGPUWithSamplingMetadata', kv_caches: list[torch.Tensor], hidden_or_intermediate_states: Union[torch.Tensor, IntermediateTensors]) -> None:
        ...
        for idx, slen in enumerate(seq_lens):
            ...
            self.kv_store.put(hidden_key, hidden_or_intermediate_states[start_pos:end_pos])
