...
class MooncakeStoreConfig:
    ...
    def from_file(file_path: str) -> 'MooncakeStoreConfig':
        ...
        return MooncakeStoreConfig(local_hostname=config.get('local_hostname'), metadata_server=config.get('metadata_server'), global_segment_size=config.get('global_segment_size', DEFAULT_GLOBAL_SEGMENT_SIZE), local_buffer_size=config.get('local_buffer_size', DEFAULT_LOCAL_BUFFER_SIZE), protocol=config.get('protocol', 'tcp'), device_name=config.get('device_name', ''), master_server_address=config.get('master_server_address'))
...
class MooncakeStore(KVStoreBufferBase):
    ...
    def __init__(self, config: VllmConfig):
        try:
            ...
            self.store.setup(self.config.local_hostname, self.config.metadata_server, self.config.global_segment_size, self.config.local_buffer_size, self.config.protocol, self.config.device_name, self.config.master_server_address)
    ...
    def _get_impl(self, key: str) -> Optional[torch.Tensor]:
