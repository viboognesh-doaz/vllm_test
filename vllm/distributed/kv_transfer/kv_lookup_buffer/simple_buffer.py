...
class SimpleBuffer(KVLookupBufferBase):
    ...
    def __init__(self, signal_pipe: KVPipeBase, data_pipe: KVPipeBase, buffer_size_thresh: float):
        """
        signal_pipe: on CPU
    ...
    def _matches(self, tokens_roi_sender: list[torch.Tensor], tokens_roi_recver: list[torch.Tensor]):
    ...
    def _add_to_buffer(self, input_tokens: torch.Tensor, roi: torch.Tensor, key: torch.Tensor, value: torch.Tensor, hidden: torch.Tensor):
    ...
    def drop_select_handler(self):
        try:
            while True:
                ...
                assert roi is not None, 'Please provide the roi when sending drop-select request'
                roi = roi > 0.5
    ...
    def drop_select(self, input_tokens: Optional[torch.Tensor], roi: Optional[torch.Tensor]) -> list[Optional[torch.Tensor]]:
        assert self.request_handling_thread is None, 'drop_select should be called by the KV cache consumer (e.g. the decode vLLM instance)'
        if isinstance(input_tokens, torch.Tensor):
            input_tokens = input_tokens.clone()
        if isinstance(roi, torch.Tensor):
            roi = roi.clone().float()
