...
class MooncakeTransferEngineConfig:
    ...
    def from_file(file_path: str) -> 'MooncakeTransferEngineConfig':
        ...
        return MooncakeTransferEngineConfig(prefill_url=config.get('prefill_url'), decode_url=config.get('decode_url'), metadata_backend=config.get('metadata_backend', None), metadata_server=config.get('metadata_server'), protocol=config.get('protocol', 'tcp'), device_name=config.get('device_name', ''))
...
class MooncakeTransferEngine:
    ...
    def __init__(self, kv_rank: int, local_rank: int):
        try:
            ...
            raise ImportError('Please install mooncake by following the instructions at https://github.com/kvcache-ai/Mooncake/blob/main/doc/en/build.md to run vLLM with MooncakeConnector.') from e
        ...
        self.initialize(self.prefill_url if kv_rank == 0 else self.decode_url, self.config.metadata_server, self.config.protocol, self.config.device_name, self.config.metadata_backend)
        ...
        self._setup_metadata_sockets(kv_rank, prefill_host, base_prefill_port, decode_host, base_decode_port)

    def _setup_metadata_sockets(self, kv_rank: int, p_host: str, p_port: int, d_host: str, d_port: int) -> None:
        ...
        if kv_rank == 0:
            ...
            self.receiver_socket.connect(make_zmq_path('tcp', d_host, d_rank_offset + 1))
            ...
            self.receiver_socket.connect(make_zmq_path('tcp', p_host, p_rank_offset + 1))
    ...
    def initialize(self, local_hostname: str, metadata_server: str, protocol: str, device_name: str, metadata_backend: Union[str, None]) -> None:
        ...
        if metadata_backend is None:
            ...
            if metadata_backend not in supported_backend:
                raise ValueError(f'Mooncake Configuration error. `metadata_backend` should be one of {supported_backend}.')
            self.engine.initialize_ext(local_hostname, metadata_server, protocol, device_name, metadata_backend)
    ...
    def transfer_sync(self, buffer: int, peer_buffer_address: int, length: int) -> int:
        ...
        ret = self.engine.transfer_sync_read(self.remote_url, buffer, peer_buffer_address, length)
        if ret < 0:
    ...
    def send_bytes(self, user_data: bytes) -> None:
        ...
        self.sender_socket.send_multipart([struct.pack('!Q', src_ptr), struct.pack('!Q', length)])
...
class MooncakePipe(KVPipeBase):
    ...
    def __init__(self, local_rank: int, config: KVTransferConfig, device: Optional[str]=None):
    ...
    def close(self) -> None:
        ...
        self.transfer_engine.context.term()
