...
class CpuCommunicator(DeviceCommunicatorBase):
    ...
    def __init__(self, cpu_group: ProcessGroup, device: Optional[torch.device]=None, device_group: Optional[ProcessGroup]=None, unique_name: str=''):
        ...
        if current_platform.get_cpu_architecture() == CpuArchEnum.X86 and hasattr(torch.ops._C, 'init_shm_manager') and unique_name.startswith('tp'):
...
class _CPUSHMDistributed:
    ...
    def _init_cpu_shm(self) -> int:
        handle = torch.ops._C.init_shm_manager(self.group_name, self.communicator.world_size, self.communicator.rank)
        torch.distributed.barrier(self.communicator.device_group)
        torch.ops._C.join_shm_manager(handle, self.group_name)
        torch.distributed.barrier(self.communicator.device_group)
        return handle

    def all_reduce(self, input: torch.Tensor, group: Optional[ProcessGroup]=None) -> None:
        torch.ops._C.shm_allreduce(self.handle, input)

    def gather(self, input: torch.Tensor, gather_list: Optional[list[torch.Tensor]], dst: int=-1, group: Optional[ProcessGroup]=None) -> None:
        torch.ops._C.shm_gather(self.handle, input, gather_list, torch.distributed.get_group_rank(group, dst))

    def all_gather_into_tensor(self, output: torch.Tensor, input: torch.Tensor, group: Optional[ProcessGroup]=None) -> None:
