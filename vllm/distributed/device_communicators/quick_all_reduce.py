...
class QuickAllReduce:
    ...
    _QR_MIN_SIZE = {(torch.float16, 2): [1 * MB, 2 * MB, 2 * MB, 1 * MB], (torch.float16, 4): [1 * MB, 16 * MB, 4 * MB, 2 * MB], (torch.float16, 8): [16 * MB, 4 * MB, 4 * MB, 2 * MB], (torch.bfloat16, 2): [2 * MB, 8 * MB, 8 * MB, 8 * MB], (torch.bfloat16, 4): [8 * MB, 64 * MB, 64 * MB, 16 * MB], (torch.bfloat16, 8): [16 * MB, 2048 * MB, 2048 * MB, 2048 * MB]}
    ...
    def __init__(self, group: ProcessGroup, device: Union[int, str, torch.device]) -> None:
        ...
        if not quick_ar:
            ...
            return
        self.group = group
        assert dist.get_backend(group) != dist.Backend.NCCL, 'Custom quick allreduce should be attached to a non-NCCL group.'
        ...
        if world_size not in QuickAllReduce._SUPPORTED_WORLD_SIZES:
            logger.warning('Custom quick allreduce is disabled due to an unsupported world size: %d. Supported world sizes: %s.', world_size, str(QuickAllReduce._SUPPORTED_WORLD_SIZES))
            return
        ...
        if self.world_size > 2 and (not self.fully_connected):
            logger.debug("Custom quick allreduce is disabled because it's not supported on more than two PCIe-only GPUs. ")
            return
    ...
    def should_quick_allreduce(self, inp: torch.Tensor):
        ...
        if self.use_fp16_kernels:
            dtype = torch.float16
        return inp_size <= self.qr_max_size and inp_size >= self._QR_MIN_SIZE[dtype, self.world_size][self.qr_quant_level.value]
