...
def can_actually_p2p(batch_src: Sequence[int], batch_tgt: Sequence[int]) -> Sequence[bool]:
    ...
    for src, tgt in zip(batch_src, batch_tgt):
        ...
        b = result_queue.get()
        if a != b:
            logger.warning('Two processes do not agree on the P2P access status on %d -> %d, treat as disabled.', src, tgt)
            result.append(False)
        else:
            result.append(a)
    return result
...
def gpu_p2p_access_check(src: int, tgt: int) -> bool:
    ...
    path = os.path.join(envs.VLLM_CACHE_ROOT, f'gpu_p2p_access_cache_for_{cuda_visible_devices}.json')
