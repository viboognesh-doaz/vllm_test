...
class CustomAllreduce:
    ...
    def __init__(self, group: ProcessGroup, device: Union[int, str, torch.device], max_size=8192 * 1024) -> None:
        ...
        self.disabled = True
        if not custom_ar:
            logger.info('Custom allreduce is disabled because of missing custom allreduce library')
            return
        self.group = group
        assert dist.get_backend(group) != dist.Backend.NCCL, 'CustomAllreduce should be attached to a non-NCCL group.'
        if not all(in_the_same_node_as(group, source_rank=0)):
            logger.warning('Custom allreduce is disabled because this process group spans across nodes.')
            return
        ...
        if world_size not in CustomAllreduce._SUPPORTED_WORLD_SIZES:
            logger.warning('Custom allreduce is disabled due to an unsupported world size: %d. Supported world sizes: %s. To silence this warning, specify disable_custom_all_reduce=True explicitly.', world_size, str(CustomAllreduce._SUPPORTED_WORLD_SIZES))
            return
        ...
        if not current_platform.is_rocm() and (not _can_p2p(rank, world_size)):
            logger.warning('Custom allreduce is disabled because your platform lacks GPU P2P capability or P2P test failed. To silence this warning, specify disable_custom_all_reduce=True explicitly.')
            return
        self.disabled = False
    ...
    @staticmethod
    def create_shared_buffer(size_in_bytes: int, group: Optional[ProcessGroup]=None, uncached: Optional[bool]=False) -> list[int]:
    ...
    @staticmethod
    def free_shared_buffer(pointers: list[int], group: Optional[ProcessGroup]=None, rank: Optional[int]=0) -> None:
        if rank is None:
            rank = dist.get_rank(group=group)
        if ops is not None:
