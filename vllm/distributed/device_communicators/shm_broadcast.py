...
from vllm.utils import get_ip, get_open_port, get_open_zmq_ipc_path, is_valid_ipv6_address
...
class MessageQueue:
    ...
    def __init__(self, n_reader, n_local_reader, local_reader_ranks: Optional[list[int]]=None, max_chunk_bytes: int=1024 * 1024 * 10, max_chunks: int=10, connect_ip: Optional[str]=None):
        ...
        self.handle = Handle(local_reader_ranks=local_reader_ranks, buffer_handle=self.buffer.handle() if self.buffer is not None else None, local_subscribe_addr=local_subscribe_addr, remote_subscribe_addr=remote_subscribe_addr, remote_addr_ipv6=remote_addr_ipv6)
    ...
    def create_from_handle(handle: Handle, rank) -> 'MessageQueue':
        ...
        if rank in handle.local_reader_ranks:
            ...
            self.remote_socket = None
            self._read_spin_timer = SpinSleepTimer() if envs.VLLM_SLEEP_WHEN_IDLE else SpinTimer()
        else:
            self.buffer = None
            self.current_idx = -1
            self.local_reader_rank = -1
        ...
        while True:
            with self.buffer.get_metadata(self.current_idx) as metadata_buffer:
                ...
                if written_flag and read_count != self.buffer.n_reader:
                    sched_yield()
                    if time.monotonic() - start_time > VLLM_RINGBUFFER_WARNING_INTERVAL * n_warning:
    ...
    @contextmanager
    def acquire_read(self, timeout: Optional[float]=None, cancel: Optional[Event]=None):
    ...
    @staticmethod
    def create_from_process_group(pg: Union[ProcessGroup, StatelessProcessGroup], max_chunk_bytes, max_chunks, writer_rank=0) -> 'MessageQueue':
        ...
        if group_rank == writer_rank:
            buffer_io = MessageQueue(n_reader=n_reader, n_local_reader=n_local_reader, local_reader_ranks=local_reader_ranks, max_chunk_bytes=max_chunk_bytes, max_chunks=max_chunks)
