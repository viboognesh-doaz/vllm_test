...
from vllm.core.block.common import BlockPool, CopyOnWriteTracker, RefCounter, get_all_blocks_recursively
...
class NaiveBlockAllocator(BlockAllocator):
    ...
    def __init__(self, create_block: Block.Factory, num_blocks: int, block_size: int, block_ids: Optional[Iterable[int]]=None, block_pool: Optional[BlockPool]=None):
        ...
        if block_pool is None:
            extra_factor = 4
            self._block_pool = BlockPool(self._block_size, create_block, self, num_blocks * extra_factor)
        else:
    ...
    def allocate_immutable_blocks(self, prev_block: Optional[Block], block_token_ids: List[List[int]], extra_hash: Optional[int]=None, device: Optional[Device]=None) -> List[Block]:
        ...
        blocks = []
        for i in range(num_blocks):
            prev_block = self._block_pool.init_block(prev_block=prev_block, token_ids=block_token_ids[i], block_size=self._block_size, physical_block_id=block_ids[i])
    ...
    def allocate_mutable_block(self, prev_block: Optional[Block], extra_hash: Optional[int]=None, device: Optional[Device]=None) -> Block:
        ...
        block = self._block_pool.init_block(prev_block=prev_block, token_ids=[], block_size=self._block_size, physical_block_id=block_id)
        return block
    ...
    def fork(self, last_block: Block) -> List[Block]:
        ...
        for block in source_blocks:
            ...
            forked_block = self._block_pool.init_block(prev_block=prev_block, token_ids=block.token_ids, block_size=self._block_size, physical_block_id=block.block_id)
    ...
    def swap_in(self, blocks: List[Block]) -> None:
        for block in blocks:
            if block.is_full:
                tmp_block = self.allocate_immutable_block(prev_block=block.prev_block, token_ids=block.token_ids)
            else:
...
class NaiveBlock(Block):
    ...
    def __init__(self, prev_block: Optional[Block], token_ids: List[int], block_size: int, allocator: BlockAllocator, block_id: Optional[int]=None, _cow_target: Optional[Block]=None, extra_hash: Optional[int]=None):
    ...
    def append_token_ids(self, token_ids: List[int]) -> None:
        ...
        if self._block_id is not None:
            self._block_id = self._allocator.cow_block_if_not_appendable(self._cow_target)
