...
class MultiModalHasher:
    ...
    def serialize_item(cls, obj: object) -> Union[bytes, memoryview]:
        ...
        if isinstance(obj, np.ndarray):
            ...
            return cls.item_to_bytes('ndarray', {'dtype': obj.dtype.str, 'shape': obj.shape, 'data': arr_data})
        logger.warning('No serialization method found for %s. Falling back to pickle.', type(obj))
        return pickle.dumps(obj)

    @classmethod
    def item_to_bytes(cls, key: str, obj: object) -> bytes:
        return b''.join((kb + vb for kb, vb in cls.iter_item_to_bytes(key, obj)))

    @classmethod
    def iter_item_to_bytes(cls, key: str, obj: object) -> Iterable[tuple[bytes, Union[bytes, memoryview]]]:
