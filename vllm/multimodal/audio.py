...
try:
    ...
    soundfile = PlaceholderModule('soundfile')

def resample_audio_librosa(audio: npt.NDArray[np.floating], *, orig_sr: float, target_sr: float) -> npt.NDArray[np.floating]:
    return librosa.resample(audio, orig_sr=orig_sr, target_sr=target_sr)

def resample_audio_scipy(audio: npt.NDArray[np.floating], *, orig_sr: float, target_sr: float):
...
class AudioResampler:
    ...
    def __init__(self, target_sr: Optional[float]=None, method: Literal['librosa', 'scipy']='librosa'):
        self.target_sr = target_sr
        self.method = method

    def resample(self, audio: npt.NDArray[np.floating], *, orig_sr: float) -> npt.NDArray[np.floating]:
