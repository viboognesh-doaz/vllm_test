...
class OpenCVVideoBackend(VideoLoader):
    ...
    def load_bytes(cls, data: bytes, num_frames: int=-1, **kwargs) -> tuple[npt.NDArray, dict[str, Any]]:
        ...
        if full_read:
            ...
            uniform_sampled_frames = np.linspace(0, total_frames_num - 1, num_frames, dtype=int)
        ...
        for idx in range(total_frames_num):
            ...
            if idx in frame_idx:
                ...
                if ret:
                    ...
                    i += 1
        assert i == num_frames, f'Expected reading {num_frames} frames, but only loaded {i} frames from video.'
        metadata = {'total_num_frames': total_frames_num, 'fps': original_fps, 'duration': duration, 'video_backend': 'opencv'}
...
class VideoMediaIO(MediaIO[npt.NDArray]):
    ...
    def encode_base64(self, media: npt.NDArray, *, video_format: str='JPEG') -> str:
        ...
        if video_format == 'JPEG':
            encode_frame = partial(self.image_io.encode_base64, image_format=video_format)
