...
from typing import TYPE_CHECKING, Any, Generic, Literal, NamedTuple, Optional, TypeVar, Union
import numpy as np
import torch
from typing_extensions import TypeAlias, TypeGuard, assert_never
from vllm.utils import LazyLoader, is_list_of
from .audio import AudioResampler
from .inputs import AudioItem, HfAudioItem, HfImageItem, HfVideoItem, ImageItem, ModalityData, MultiModalDataDict, MultiModalFieldConfig, MultiModalKwargs, VideoItem
_T = TypeVar('_T')
_I = TypeVar('_I')
if TYPE_CHECKING:
...
class EmbeddingItems(ModalityDataItems[Union[torch.Tensor, list[torch.Tensor]], torch.Tensor]):
    """
...
class DictEmbeddingItems(ModalityDataItems[Mapping[str, torch.Tensor], Mapping[str, torch.Tensor]]):
    ...
    def __init__(self, data: Mapping[str, torch.Tensor], modality: str, required_fields: set[str], fields_factory: Callable[[Mapping[str, torch.Tensor]], Mapping[str, MultiModalFieldConfig]]) -> None:
        super().__init__(data, modality)
        missing_required_data_keys = required_fields - data.keys()
        if missing_required_data_keys:
            data_keys = set(data.keys())
            msg = f'The data should contain the fields: {required_fields}, but only found the following keys: {data_keys}'
        ...
        if missing_required_fields:
            ...
            msg = f'required_fields={required_fields!r} should be a subset of fields={fields!r}'
            raise ValueError(msg)
        self.fields_config = fields_config
        self.required_fields = required_fields
        self._kwargs = MultiModalKwargs.from_hf_inputs(BatchFeature(dict(data)), fields_config)

    def get_count(self) -> int:
        return self._kwargs.get_item_count(self.modality)

    def get(self, index: int) -> Mapping[str, torch.Tensor]:
        return {k: v.data for k, v in self._kwargs.get_item(self.modality, index).items()}
...
class VideoProcessorItems(ProcessorBatchItems[HfVideoItem]):
    ...
    def __init__(self, data: Sequence[HfVideoItem], metadata: Optional[Union[dict[str, Any], list[Optional[dict[str, Any]]]]]=None) -> None:
...
class MultiModalDataItems(UserDict[str, ModalityDataItems[Any, Any]]):
    ...
    def get_items(self, modality: str, typ: Union[type[_D], tuple[type[_D], ...]]) -> _D:
        """
        ...
        if not isinstance(items, typ):
            raise TypeError(f'Invalid type of data items for modality={modality!r}. Expected type: {typ}, but found type: {type(items)}')
        return items
ModalityDataParser: TypeAlias = Callable[[ModalityData[Any]], Optional[ModalityDataItems[Any, Any]]]
...
class MultiModalDataParser:
    ...
    def __init__(self, *, target_sr: Optional[float]=None, audio_resample_method: Literal['librosa', 'scipy']='librosa', video_needs_metadata: bool=False) -> None:
        super().__init__()
        self.audio_resampler = AudioResampler(target_sr=target_sr, method=audio_resample_method)
        self.video_needs_metadata = video_needs_metadata

    def _is_embeddings(self, data: object) -> TypeGuard[Union[torch.Tensor, list[torch.Tensor]]]:
    ...
    def _get_audio_with_sr(self, audio: AudioItem) -> tuple[np.ndarray, Optional[float]]:
    ...
    def _get_video_with_metadata(self, video: VideoItem) -> tuple[np.ndarray, Optional[dict[str, Any]]]:
    ...
    def _parse_audio_data(self, data: ModalityData[AudioItem]) -> Optional[ModalityDataItems[Any, Any]]:
        if self._is_empty(data) or (isinstance(data, tuple) and self._is_empty(data[0])):
            return None
        if self._is_embeddings(data):
            return AudioEmbeddingItems(data)
        if is_list_of(data, float) or (isinstance(data, (np.ndarray, torch.Tensor)) and data.ndim == 1) or isinstance(data, tuple):
            data_items = [data]
    ...
    def _parse_image_data(self, data: ModalityData[ImageItem]) -> Optional[ModalityDataItems[Any, Any]]:
    ...
    def _parse_video_data(self, data: ModalityData[VideoItem]) -> Optional[ModalityDataItems[Any, Any]]:
        if self._is_empty(data):
            return None
        if self._is_embeddings(data):
    ...
    def _get_subparsers(self) -> Mapping[str, ModalityDataParser]:
        return {'audio': self._parse_audio_data, 'image': self._parse_image_data, 'video': self._parse_video_data}
