...
class PunicaWrapperTPU(PunicaWrapperBase):
    ...
    def shrink(self, x: torch.Tensor, w_t_all: torch.Tensor, scale: float):
    ...
    def expand_slice(self, y: torch.Tensor, x: torch.Tensor, w_t_all: torch.Tensor, y_offset: int, y_slice_size: int, add_inputs: bool) -> torch.Tensor:
        return bgmv_expand_slice(x, w_t_all, y, self._get_token_lora_indices(x), y_offset, y_slice_size, add_inputs)
    ...
    def add_expand(self, y: torch.Tensor, x: Union[tuple[torch.Tensor, ...], torch.Tensor], lora_b_stacked: tuple[torch.Tensor, ...], lora_bias_stacked: Optional[tuple[torch.Tensor, ...]], output_slices: tuple[int, ...], offset_start: int=0, add_inputs=True, **kwargs) -> torch.Tensor:
        """
        ...
        if lora_bias_stacked is not None:
            y = self._apply_bias(self._get_token_lora_indices(y), y, output_slices, lora_bias_stacked)
        for slice_idx in range(len(lora_b_stacked)):
            y = self.expand_slice(y, x[slice_idx], lora_b_stacked[slice_idx], offset_left, output_slices[slice_idx], add_inputs=add_inputs)
            offset_left += output_slices[slice_idx]
        return y.view_as(y_org)

    def add_lora_embedding(self, y: torch.Tensor, x: torch.Tensor, lora_b_stacked: torch.Tensor, add_inputs: bool=True, **kwargs) -> torch.Tensor:
        """
    ...
    def add_lora_linear(self, y: torch.Tensor, x: torch.Tensor, lora_a_stacked: tuple[torch.Tensor, ...], lora_b_stacked: tuple[torch.Tensor, ...], lora_bias_stacked: Optional[tuple[torch.Tensor, ...]], scale: float, output_slices: tuple[int, ...], *, buffer: Optional[tuple[torch.Tensor, ...]]=None, **kwargs) -> torch.Tensor:
        """
        ...
        if buffer is None:
            ...
            T = x.size(0)
            buffer = torch.zeros((len(output_slices), T, r), dtype=x.dtype, device=x.device)
        buffer = self.add_shrink(buffer, x, lora_a_stacked, scale, **kwargs)
        return self.add_expand(y, buffer, lora_b_stacked, None, output_slices, add_inputs=True, **kwargs)

    def add_lora_logits(self, y: torch.Tensor, x: torch.Tensor, lora_a_stacked: torch.Tensor, lora_b_stacked: torch.Tensor, scale, *, buffer: Optional[torch.Tensor]=None, **kwargs) -> torch.Tensor:
        """
    ...
    def _apply_bias(self, indices: torch.Tensor, output: torch.Tensor, output_slices: tuple[int, ...], lora_bias_stacked: tuple[Optional[torch.Tensor], ...]):
        ...
        for slice_idx, slice in enumerate(output_slices):
            ...
            if bias is not None:
                ...
                bias = F.pad(bias, (offset_left, output.shape[1] - (offset_left + slice), 0, 0))
                output += bias
            offset_left += slice
    ...
    def _update_base_metadata(self, mapping: 'LoRAMapping', lora_index_to_id: list[Optional[int]], max_loras: int, vocab_size: int, extra_vocab_size: int):
        ...
        base_indices, sampler_indices, sampler_indices_padded, embeddings_indices, indices_len = convert_mapping(mapping, lora_index_to_id, max_loras, vocab_size, extra_vocab_size, 'cpu')
        self._token_lora_indices = self._pad_to_shape(base_indices, self._token_lora_indices.shape, dims=1).to(self.device)
        self._sampler_indices = self._pad_to_shape(sampler_indices, self._sampler_indices.shape, dims=1).to(self.device)
        self._sampler_indices_padded = self._pad_to_shape(sampler_indices_padded, self._sampler_indices_padded.shape, dims=1).to(self.device)
        self._embeddings_indices = self._pad_to_shape(embeddings_indices, self._embeddings_indices.shape, dims=2).to(self.device)
