...
def bgmv_jax(inputs, loras, idxs):
    return jnp.einsum('td,tX,Xld->tl', inputs, jax.nn.one_hot(idxs, loras.shape[0], dtype=inputs.dtype), loras)
...
def bgmv_shrink(inputs: torch.Tensor, lora_b_weights: torch.Tensor, lora_indices_tensor: torch.Tensor, scaling: float=1.0):
    """
    Args:
        inputs (torch.Tensor): Input tensor of shape [num_tokens, hidden_size].
        lora_b_weights (torch.Tensor): LoRA weights of shape
...
def bgmv_expand_slice(inputs: torch.Tensor, lora_b_weights: torch.Tensor, output_tensor: torch.Tensor, lora_indices_tensor: torch.Tensor, slice_offset: int, slice_size: int, add_inputs: bool=True):
    ...
    """
    outputs = torch.ops.xla.bgmv(inputs, lora_b_weights, lora_indices_tensor)
    outputs = F.pad(outputs, (slice_offset, output_tensor.shape[1] - (slice_offset + slice_size), 0, 0))
    if add_inputs:
