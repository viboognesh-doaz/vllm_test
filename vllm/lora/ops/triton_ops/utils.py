...
def _get_lora_b_ptr(lora_weights: list[torch.Tensor], offset_start: int, device: torch.device):
    ...
    if (len(set(lora_strides_d0)) == 1 and len(set(lora_strides_d1)) == 1 and (len(set(lora_strides_d2)) == 1)) and len(set(hidden_sizes)) == 1:
    ...
    _LORA_B_PTR_DICT[key] = (slice_start_tensor, lora_ptr_tensor, lora_strides_d0_tensor, lora_strides_d1_tensor, lora_strides_d2_tensor, hidden_sizes_tensor, same_stride, MAX_N)
