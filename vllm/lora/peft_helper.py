...
class PEFTHelper:
    ...
    def from_dict(cls, config_dict: dict) -> 'PEFTHelper':
        ...
        required_fields = {name for name, f in class_fields.items() if f.default is MISSING and f.default_factory is MISSING}
    ...
    @classmethod
    def from_local_dir(cls, lora_path: str, max_position_embeddings: Optional[int], tensorizer_config_dict: Optional[dict]=None) -> 'PEFTHelper':
        ...
        if tensorizer_config_dict:
            ...
            with open_stream(lora_config_path, mode='rb', **tensorizer_args.stream_kwargs) as f:
                config = json.load(f)
            logger.info('Successfully deserialized LoRA config from %s', tensorizer_config.tensorizer_dir)
        else:
