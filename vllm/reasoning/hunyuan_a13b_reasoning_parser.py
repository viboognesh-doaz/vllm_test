...
": [198, 524, 27963, 397, 27, 9399, 397]
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase):
        ...
        self.think_start_expr = '<think>\\n'
        self.think_end_expr = '\\n</think>\\n'
        self.response_start_expr = '\\n</think>\\n<answer>\\n'
        self.response_end_expr = '\\n</answer>'
        self.full_match_reasoning_regex = re.compile(f'(?:{self.think_start_expr}(.*?){self.response_start_expr})?(.*?){self.response_end_expr}', re.DOTALL)
        self.half_match_reasoning_regex = re.compile(f'{self.think_start_expr}(.*?){self.response_start_expr}(.*)', re.DOTALL)
    ...
    def extract_reasoning_content_streaming(self, previous_text: str, current_text: str, delta_text: str, previous_token_ids: Sequence[int], current_token_ids: Sequence[int], delta_token_ids: Sequence[int]) -> Union[DeltaMessage, None]:
        ...
        def check_token_with_sequence(token):
            if self.current_state == 'idle' or self.current_state == 'think':
                return token == self.expected_sequence[self.sequence_index] or token == self.expected_sequence_side[self.sequence_index]
            else:
                return token == self.expected_sequence[self.sequence_index]

        def check_last_token(token):
            if self.current_state == 'idle' or self.current_state == 'think':
                if self.sequence_index - 1 < len(self.expected_sequence_side) and token == self.expected_sequence_side[self.sequence_index - 1]:
        ...
        if token_in_state_seq:
            ...
            if check_last_token(token):
                ...
                self.token_buffer = []
                self.text_buffer = ''
        elif self.token_buffer and len(self.token_buffer) > 0:
