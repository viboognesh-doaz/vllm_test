...
from vllm.prompt_adapter.layers import VocabParallelEmbeddingWithPromptAdapter
...
class PromptAdapterModel(AdapterModel):
    ...
    @classmethod
    def from_local_checkpoint(cls, adapter_model_path: str, prompt_adapter_id: int, num_virtual_tokens: int, config: PromptAdapterConfig, device: str='cuda') -> 'PromptAdapterModel':
        if num_virtual_tokens > config.max_prompt_adapter_token:
            raise ValueError(f'num_virtual_tokens ({num_virtual_tokens}) should be <= max_prompt_adapter_token({config.max_prompt_adapter_token})')
...
class PromptAdapterModelManager(AdapterModelManager):
    ...
    def __init__(self, model: nn.Module, max_num_seqs: int, max_num_batched_tokens: int, prompt_adapter_config: PromptAdapterConfig):
        ...
        self.prompt_adapter_index_to_id: List[Optional[int]] = [None] * self.prompt_adapter_slots
    ...
    def activate_adapter(self, prompt_adapter_id: int) -> bool:
        """Move PromptAdapter into a GPU buffer 
            to be used in the forward pass."""
        if prompt_adapter_id in self._active_adapters:
            return False
        first_free_slot = next(((i, prompt_adapter_id) for i, prompt_adapter_id in enumerate(self.prompt_adapter_index_to_id) if prompt_adapter_id is None), None)
        ...
        logger.debug('Activating prompt_adapter. int id: %d, slot index: %d', prompt_adapter_model.id, index)
    ...
    def pin_adapter(self, prompt_adapter_id: int) -> bool:
        ...
        raise NotImplementedError('Pinning is not supported in PromptAdapterModelManager. Use LRUCachePromptAdapterModelManager for pinning')
...
def create_prompt_adapter_manager(model: nn.Module, max_num_seqs: int, max_num_batched_tokens: int, prompt_adapter_config: PromptAdapterConfig, prompt_adapter_manager_cls: Type[PromptAdapterModelManager]=PromptAdapterModelManager, **kwargs) -> PromptAdapterModelManager:
    """Create a PromptAdapterModel for a given model."""
    prompt_adapter_manager = prompt_adapter_manager_cls(model=model, max_num_seqs=max_num_seqs, max_num_batched_tokens=max_num_batched_tokens, prompt_adapter_config=prompt_adapter_config, **kwargs)
