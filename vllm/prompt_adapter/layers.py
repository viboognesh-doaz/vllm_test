...
class VocabParallelEmbeddingWithPromptAdapter(nn.Module):
    ...
    def create_prompt_adapter_weights(self, prompt_adapter_config: PromptAdapterConfig):
        self.embeddings_tensors = torch.zeros((prompt_adapter_config.max_prompt_adapters, prompt_adapter_config.max_prompt_adapter_token, self.emb_layer.embedding_dim), dtype=self.emb_layer.weight.dtype, device=self.emb_layer.weight.device)
        self.adapter_lengths = torch.zeros(prompt_adapter_config.max_prompt_adapters, dtype=torch.long, device=self.emb_layer.weight.device)
    ...
    def set_prompt_adapter(self, index: int, adapter_model: Optional[torch.Tensor]):
    ...
    def set_mapping(self, prompt_indices: torch.Tensor, prompt_embedding_indices: torch.Tensor):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.embedding_indices_gpu.ndim > 1:
            ...
            gathered_embeddings = self.embeddings_tensors[self.embedding_indices_gpu[:, 0], self.embedding_indices_gpu[:, 1]]
