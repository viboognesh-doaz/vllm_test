...
class GuidanceLogitsProcessor:
    ...
    def __init__(self, grammar: str, tokenizer: PreTrainedTokenizerBase) -> None:
        """Base Guidance Logits Processor
    ...
    def clone(self) -> 'GuidanceLogitsProcessor':
        ...
        if self.initialized:
            cloned.ll_matcher = llguidance.LLMatcher(self.ll_tokenizer, self.grammar, log_level=int(os.environ.get('LLGUIDANCE_LOG_LEVEL', '1')))
            self.bitmask = llguidance.torch.allocate_token_bitmask(1, self.ll_tokenizer.vocab_size)
        return cloned
    ...
    def _initialize(self):
        ...
        self.ll_matcher = llguidance.LLMatcher(self.ll_tokenizer, self.grammar, log_level=int(os.environ.get('LLGUIDANCE_LOG_LEVEL', '1')))
        self.bitmask = llguidance.torch.allocate_token_bitmask(1, self.ll_tokenizer.vocab_size)
    ...
    def __call__(self, input_ids: list[int], scores: torch.Tensor) -> torch.Tensor:
        ...
        llguidance.torch.apply_token_bitmask_inplace(scores, self.bitmask.to(scores.device))
