...
def has_xgrammar_unsupported_json_features(schema: dict) -> bool:
    ...
    def check_object(obj: dict) -> bool:
        ...
        if obj.get('type') in ('integer', 'number') and 'multipleOf' in obj:
            return True
        if obj.get('type') == 'array' and any((key in obj for key in ['uniqueItems', 'contains', 'minContains', 'maxContains', 'minItems', 'maxItems'])):
            return True
        if obj.get('type') == 'string' and any((key in obj for key in ['minLength', 'maxLength', 'format'])):
            return True
