...
from vllm.utils import PyObjectCache, async_tensor_h2d, is_pin_memory_available, make_tensor_with_pad
import torch
_SAMPLING_EPS = 1e-05
...
def gen_seq_group_to_sample_builder(num_seqs: int):
    return lambda: SequenceGroupToSample(seq_ids=[0] * num_seqs, sampling_params=None, seq_data=None, seq_len=0, query_len=0, generator=None, is_prompt=True, prompt_logprob_indices=[], sample_indices=[])
...
class SamplingMetadata:
    ...
    def __init__(self, seq_groups: list[SequenceGroupToSample], selected_token_indices: torch.Tensor, categorized_sample_indices: dict[SamplingType, torch.Tensor], num_prompts: int, skip_sampler_cpu_output: bool=False, reuse_sampling_tensors: bool=False) -> None:
    ...
    @staticmethod
    def prepare(seq_group_metadata_list: list[SequenceGroupMetadata], seq_lens: list[int], query_lens: list[int], device: str, pin_memory: bool, generators: Optional[dict[str, torch.Generator]]=None, cache: Optional[SamplingMetadataCache]=None) -> 'SamplingMetadata':
        seq_groups, selected_token_indices, categorized_sample_indices, num_prompts = _prepare_seq_groups(seq_group_metadata_list, seq_lens, query_lens, device, generators, cache)
        selected_token_indices = async_tensor_h2d(selected_token_indices, dtype=torch.long, target_device=device, pin_memory=pin_memory)
        categorized_sample_indices = {t: async_tensor_h2d(seq_ids, dtype=torch.int, target_device=device, pin_memory=pin_memory) for t, seq_ids in categorized_sample_indices.items()}
        sampling_metadata = SamplingMetadata(seq_groups=seq_groups, selected_token_indices=selected_token_indices, categorized_sample_indices=categorized_sample_indices, num_prompts=num_prompts)
        return sampling_metadata

    def __repr__(self) -> str:
        return f'SamplingMetadata(seq_groups={self.seq_groups}, selected_token_indices={self.selected_token_indices}, categorized_sample_indices={self.categorized_sample_indices})'

def _prepare_seq_groups(seq_group_metadata_list: list[SequenceGroupMetadata], seq_lens: list[int], query_lens: list[int], device: str, generators: Optional[dict[str, torch.Generator]]=None, cache: Optional[SamplingMetadataCache]=None) -> tuple[list[SequenceGroupToSample], list[int], dict[SamplingType, list[int]], int]:
    ...
    for i, seq_group_metadata in enumerate(seq_group_metadata_list):
        ...
        if cache is not None:
            ...
            sample_obj = SequenceGroupToSample(seq_ids=list(seq_ids), sampling_params=sampling_params, seq_data=seq_group_metadata.seq_data, seq_len=seq_len, query_len=query_len, generator=generator, is_prompt=is_prompt, prompt_logprob_indices=list(prompt_logprob_indices), sample_indices=list(sample_indices))
...
class SamplingTensors:
        ...
        if do_penalties:
            for seq_group in sampling_metadata.seq_groups:
                ...
                if seq_group.is_prompt and sampling_params.prompt_logprobs is not None:
                    prefill_len = len(seq_group.prompt_logprob_indices)
                    prompt_tokens.extend((array(VLLM_TOKEN_ID_ARRAY_TYPE) for _ in range(prefill_len)))
                    output_tokens.extend((array(VLLM_TOKEN_ID_ARRAY_TYPE) for _ in range(prefill_len)))
                if seq_group.do_sample:
    ...
    def from_lists(cls, temperatures: list[float], top_ps: list[float], top_ks: list[int], min_ps: list[float], presence_penalties: list[float], frequency_penalties: list[float], repetition_penalties: list[float], prompt_tokens: list[array], output_tokens: list[array], vocab_size: int, device: torch.device, dtype: torch.dtype) -> 'SamplingTensors':
        ...
        if do_penalties:
            prompt_t = make_tensor_with_pad(prompt_tokens, vocab_size, device='cpu', dtype=torch.int64, pin_memory=pin_memory)
            output_t = make_tensor_with_pad(output_tokens, vocab_size, device='cpu', dtype=torch.int64, pin_memory=pin_memory)
        else:
