...
class CustomOp(nn.Module):
    ...
    def __new__(cls, *args, **kwargs):
        try:
            ...
            raise TypeError(f"Cannot instantiate '{cls.__name__}': its 'name' attribute was not set, possibly because it was not decorated with @CustomOp.register, or it's the CustomOp base class itself.") from None
        if op_name not in cls.op_registry_oot:
            op_cls_to_instantiate = cls
        else:
            op_cls_to_instantiate = cls.op_registry_oot[op_name]
            logger.debug('Instantiating custom op: %s using %s', op_name, str(op_cls_to_instantiate))
    ...
    def enabled(cls) -> bool:
        ...
        if not hasattr(cls, 'name'):
            logger.warning_once("Custom op %s was not registered, which means it won't appear in the op registry. It will be enabled/disabled based on the global settings.", cls.__name__)
    ...
    def default_on() -> bool:
        ...
        """
        compilation_config = get_current_vllm_config().compilation_config
        default_on = compilation_config.level < CompilationLevel.PIECEWISE or not compilation_config.use_inductor
