...
from vllm.model_executor.model_loader.weight_utils import download_safetensors_index_file_from_hf, download_weights_from_hf, runai_safetensors_weights_iterator
...
class RunaiModelStreamerLoader(BaseModelLoader):
    ...
    def _prepare_weights(self, model_name_or_path: str, revision: Optional[str]) -> list[str]:
        ...
        hf_folder = model_name_or_path if is_local or is_s3_path else download_weights_from_hf(model_name_or_path, self.load_config.download_dir, [safetensors_pattern], revision, ignore_patterns=self.load_config.ignore_patterns)
        if is_s3_path:
            hf_weights_files = s3_glob(path=hf_folder, allow_pattern=[safetensors_pattern])
        else:
            hf_weights_files = glob.glob(os.path.join(hf_folder, safetensors_pattern))
    ...
    def _get_weights_iterator(self, model_or_path: str, revision: str) -> Generator[tuple[str, torch.Tensor], None, None]:
        """Get an iterator for the model weights based on the load format."""
        hf_weights_files = self._prepare_weights(model_or_path, revision)
        return runai_safetensors_weights_iterator(hf_weights_files, self.load_config.use_tqdm_on_load)
