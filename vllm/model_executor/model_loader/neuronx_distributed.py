...
class NeuronCausalLM(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, input_block_ids: torch.Tensor, sampling_params: torch.Tensor, prev_hidden: Optional[torch.Tensor]=None, adapter_ids: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        output = self.model(input_ids, attention_mask=None, position_ids=positions, seq_ids=sorted_input_block_ids, sampling_params=sampling_params, prev_hidden=prev_hidden, adapter_ids=adapter_ids)
    ...
    def load_weights(self, model_name_or_path: str, **kwargs):
        ...
        neuronx_module_path, neuronx_model_cls_name = _NEURON_SUPPORTED_MODELS[arch]
        ...
        config = neuronx_model_cls.get_config_cls()(neuron_config, load_config=load_pretrained_config(model_name_or_path))
        hashed_config = hashlib.md5(config.to_json_string().encode('utf-8'), usedforsecurity=False).hexdigest()
        if os.getenv('NEURON_COMPILED_ARTIFACTS') is not None:
            compiled_model_path = os.getenv('NEURON_COMPILED_ARTIFACTS')
        elif os.path.exists(model_name_or_path):
            compiled_model_path = os.path.join(model_name_or_path, 'neuron-compiled-artifacts', hashed_config)
            shutil.rmtree(compiled_model_path, ignore_errors=True)
        else:
            compiled_model_path = os.path.join('local-models', model_name_or_path, 'neuron-compiled-artifacts', hashed_config)
            shutil.rmtree(compiled_model_path, ignore_errors=True)
        try:
            ...
            logger.warning('Failed to load the model from %s, Recompiling...', compiled_model_path)
...
class NeuronMllamaForCausalLM(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, on_device_sampling_disabled: bool=False) -> None:
        ...
        self.config = config
        self.logits_processor = LogitsProcessor(config.get_text_config().vocab_size, logits_as_input=True)
    ...
    def write_to_has_image_cache(self, seq_ids: torch.Tensor, has_image: torch.Tensor):
        for index in range(len(seq_ids)):
            seq_id = seq_ids[index].item()
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, seq_ids: torch.Tensor, pixel_values: torch.Tensor, aspect_ratios: torch.Tensor, num_chunks: torch.Tensor, has_image: torch.Tensor, sampling_params) -> torch.Tensor:
        ...
        if self.is_reorder_needed:
            ...
            aspect_ratios = torch.index_select(aspect_ratios, 0, sorted_indices)
    ...
    def load_weights(self, model_name_or_path: str, **kwargs):
        ...
        neuron_config = neuronx_model_cls.get_neuron_config_cls()(**kwargs['neuron_config'])
        self.config.neuron_config = neuron_config
        logger.info('neuron_config buckets: %s', self.config.neuron_config.buckets)
        config = neuronx_model_cls.get_config_cls()(neuron_config, load_config=load_pretrained_config(model_name_or_path))
        hashed_config = hashlib.md5(config.to_json_string().encode('utf-8'), usedforsecurity=False).hexdigest()
        if os.getenv('NEURON_COMPILED_ARTIFACTS') is not None:
            compiled_model_path = os.getenv('NEURON_COMPILED_ARTIFACTS')
        ...
        logger.info('Successfully compiled and saved the model in %s', compiled_model_path)
        self.vision_token_id = tokenizer('<|image|>', add_special_tokens=False).input_ids[0]
...
class NeuronSpeculationCausalLM(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, input_block_ids: torch.Tensor, sampling_params: torch.Tensor) -> torch.Tensor:
        sorted_input_block_ids, sorted_indices = torch.sort(input_block_ids)
        input_ids = torch.index_select(input_ids, 0, sorted_indices)
        positions = torch.index_select(positions, 0, sorted_indices)
        sampling_params = torch.index_select(sampling_params, 0, sorted_indices)
        output = self.model(input_ids, attention_mask=None, position_ids=positions, seq_ids=sorted_input_block_ids, sampling_params=sampling_params)
        restored_indices = torch.argsort(sorted_indices)
        ...
        if input_block_ids.shape[0] != 1:
            accepted_tokens_with_padding = torch.index_select(accepted_tokens_with_padding, 0, restored_indices)
    ...
    def load_weights(self, model_name_or_path: str, draft_model_name_or_path: str, **kwargs):
        ...
        if getattr(config.neuron_config, 'draft_model_modules_to_not_convert', None):
            draft_neuron_config.modules_to_not_convert = draft_neuron_config.draft_model_modules_to_not_convert
        if config.neuron_config.enable_eagle_speculation:
            draft_neuron_config.is_eagle_draft = True
            draft_neuron_config.sequence_parallel_enabled = False
        draft_config = neuronx_model_cls.get_config_cls()(draft_neuron_config, load_config=load_pretrained_config(draft_model_name_or_path))
...
def _get_default_neuron_config(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig, lora_serving_config: LoraServingConfig):
...
def get_neuron_model(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig, lora_serving_config: LoraServingConfig) -> nn.Module:
    ...
    default_neuron_config_args = _get_default_neuron_config(model_config, parallel_config, scheduler_config, lora_serving_config)
    ...
    model.load_weights(model_config.model, neuron_config=neuron_config, override_neuron_config=override_neuron_config)
    return model.eval()
...
def get_neuron_speculation_model(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig, speculation_config: SpeculativeConfig):
    ...
    default_neuron_config_args = _get_default_speculation_config(model_config, parallel_config, scheduler_config, speculation_config)
    neuron_config = _get_neuron_config_after_override(default_neuron_config_args, model_config.override_neuron_config)
    override_neuron_config = model_config.override_neuron_config
    model.load_weights(model_config.model, speculation_config.draft_model_config.model, neuron_config=neuron_config, override_neuron_config=override_neuron_config)
    return model.eval()