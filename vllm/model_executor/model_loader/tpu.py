...
class TPUModelLoader(DefaultModelLoader):
    ...
    def load_model(self, vllm_config: VllmConfig, model_config: ModelConfig, mesh: Optional[xs.Mesh]=None) -> nn.Module:
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            if load_format != 'dummy':
                ...
                logger.info('Loading weights took %.2f seconds', self.counter_after_loading_weights - self.counter_before_loading_weights)
        ...
        logger.info('Partition model took %.2f seconds', counter_after_partition - counter_before_partition)
    ...
    def _check_model_is_loaded(self, mesh: Optional[xs.Mesh], model: nn.Module) -> None:
        ...
        for name, param in model.named_parameters():
            assert param.device.type == device_type, f'Parameter {name} is on                 {param.device.type} instead of {device_type}'
        for name, buffer in model.named_buffers():
            assert buffer.device.type == device_type, f'Buffer {name} is on {buffer.device.type} instead of                     {device_type}'
        for module in model.modules():
            if mesh is not None and get_fqn(module) == 'QKVParallelLinear':
                raise AssertionError('QKVParallelLinear should be replaced by                             XlaQKVParallelLinear under SPMD mode.')