...
__all__ = ['EncryptionParams', 'DecryptionParams', 'TensorDeserializer', 'TensorSerializer', 'open_stream', 'convert_bytes', 'get_mem_usage', 'no_init_or_tensor', 'TensorizerConfig']
...
def tensorizer_kwargs_arg(value):
    ...
    if not isinstance(loaded, dict):
        raise argparse.ArgumentTypeError(f'Not deserializable to dict: {value}. serialization_kwargs and deserialization_kwargs must be deserializable from a JSON string to a dictionary. ')
    return loaded
...
def meta_tensor_mode(loading_code=None):
    if loading_code is None:
        ...
        raise TypeError(f'expected a callable to evaluate, or None if being used as a context manager; got an object of type "{type(loading_code).__name__}" instead.')
...
class TensorizerConfig(MutableMapping):
    ...
    def __post_init__(self):
        self._is_sharded = isinstance(self.tensorizer_uri, str) and re.search('%0\\dd', self.tensorizer_uri) is not None
        if self.tensorizer_dir and self.lora_dir:
            raise ValueError('Only one of tensorizer_dir or lora_dir may be specified. Use lora_dir exclusively when serializing LoRA adapters, and tensorizer_dir or tensorizer_uri otherwise.')
        if self.tensorizer_dir and self.tensorizer_uri:
            logger.warning_once('Provided both tensorizer_dir and tensorizer_uri. Inferring tensorizer_dir from tensorizer_uri as the latter takes precedence.')
        ...
        if not self.tensorizer_uri:
            if self.lora_dir:
                ...
                raise ValueError('Unable to resolve tensorizer_uri. A valid tensorizer_uri or tensorizer_dir must be provided for deserialization, and a valid tensorizer_uri, tensorizer_uri, or lora_dir for serialization.')
        else:
            self.tensorizer_dir = os.path.dirname(self.tensorizer_uri)
    ...
    def verify_with_parallel_config(self, parallel_config: 'ParallelConfig') -> None:
        if parallel_config.tensor_parallel_size > 1 and (not self._is_sharded):
            raise ValueError("For a sharded model, tensorizer_uri should include a string format template like '%04d' to be formatted with the rank of the shard")

    def verify_with_model_config(self, model_config: 'ModelConfig') -> None:
        if model_config.quantization is not None and self.tensorizer_uri is not None:
            logger.warning('Loading a model using Tensorizer with quantization on vLLM is unstable and may lead to errors.')
...
class TensorizerArgs:
    ...
    def __init__(self, tensorizer_config: TensorizerConfig):
        ...
        self.stream_kwargs = {'s3_access_key_id': tensorizer_config.s3_access_key_id, 's3_secret_access_key': tensorizer_config.s3_secret_access_key, 's3_endpoint': tensorizer_config.s3_endpoint, **(tensorizer_config.stream_kwargs or {})}
        self.deserialization_kwargs = {'verify_hash': tensorizer_config.verify_hash, 'encryption': tensorizer_config.encryption_keyfile, 'num_readers': tensorizer_config.num_readers, **(tensorizer_config.deserialization_kwargs or {})}
        if self.encryption_keyfile:
            with open_stream(tensorizer_config.encryption_keyfile, **self.stream_kwargs) as stream:
                key = stream.read()
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        group = parser.add_argument_group('tensorizer options', description='Options for configuring the behavior of the tensorizer deserializer when load_format=tensorizer is specified when initializing an LLMEngine, either via the CLI when running the vLLM OpenAI inference server with a JSON string passed to --model-loader-extra-config or as arguments given to TensorizerConfig when passed to model_loader_extra_config in the constructor for LLMEngine.')
        group.add_argument('--tensorizer-uri', type=str, help='Path to serialized model tensors. Can be a local file path, or an HTTP(S) or S3 URI.')
        group.add_argument('--verify-hash', action='store_true', help='If enabled, the hashes of each tensor will be verified against the hashes stored in the file metadata. An exception will be raised if any of the hashes do not match.')
        group.add_argument('--encryption-keyfile', type=str, default=None, help='The file path to a binary file containing a binary key to use for decryption. Can be a file path or S3 network URI.')
        group.add_argument('--num-readers', default=None, type=int, help='Controls how many threads are allowed to read concurrently from the source file. Default is `None`, which will dynamically set the number of readers based on the available resources and model size. This greatly increases performance.')
        group.add_argument('--s3-access-key-id', type=str, default=None, help='The access key for the S3 bucket. Can also be set via the S3_ACCESS_KEY_ID environment variable.')
        group.add_argument('--s3-secret-access-key', type=str, default=None, help='The secret access key for the S3 bucket. Can also be set via the S3_SECRET_ACCESS_KEY environment variable.')
        group.add_argument('--s3-endpoint', type=str, default=None, help='The endpoint for the S3 bucket. Can also be set via the S3_ENDPOINT_URL environment variable.')
        return parser
...
def _check_tensors_on_meta_device(model: nn.Module) -> None:
    for tensor in model.state_dict().values():
        if tensor.device.type == 'meta':
            raise ValueError('The serialized model contains tensors on the meta device, indicating that some tensors were not loaded properly. Please check that the parameters of the model being specified match that of the serialized model, such as its quantization.')
...
def _resize_lora_embeddings(model: nn.Module):
    ...
    for child in model.modules():
        if isinstance(child, VocabParallelEmbedding) and child.weight.shape[0] < child.num_embeddings_per_partition:
            new_weight = torch.empty(child.num_embeddings_per_partition, child.embedding_dim, dtype=child.weight.dtype, device=child.weight.device)
            new_weight[:child.weight.shape[0]].copy_(child.weight.data)
            new_weight[child.weight.shape[0]:].fill_(0)
            child.weight.data = new_weight

def init_tensorizer_model(tensorizer_config: TensorizerConfig, vllm_config: VllmConfig) -> nn.Module:
...
def tensorizer_weights_iterator(tensorizer_args: 'TensorizerArgs') -> Generator[tuple[str, torch.Tensor], None, None]:
    logger.warning('Deserializing HuggingFace models is not optimized for loading on vLLM, as tensorizer is forced to load to CPU. Consider deserializing a vLLM model instead for faster load times. See the examples/others/tensorize_vllm_model.py example script for serializing vLLM models.')
...
def is_vllm_tensorized(tensorizer_config: 'TensorizerConfig') -> bool:
    ...
    """
    tensorizer_args = tensorizer_config._construct_tensorizer_args()
    deserializer = TensorDeserializer(open_stream(tensorizer_args.tensorizer_uri, **tensorizer_args.stream_kwargs), **tensorizer_args.deserialization_kwargs, lazy_load=True)
    if tensorizer_config.vllm_tensorized:
        logger.warning('Please note that newly serialized vLLM models are automatically inferred as vLLM models, so setting vllm_tensorized=True is only necessary for models serialized prior to this change.')
        return True
    return '.vllm_tensorized_marker' in deserializer

def serialize_extra_artifacts(tensorizer_args: TensorizerArgs, served_model_name: Union[str, list[str], None]) -> None:
    if not isinstance(served_model_name, str):
        raise ValueError(f'served_model_name must be a str for serialize_extra_artifacts, not {type(served_model_name)}.')
    with tempfile.TemporaryDirectory() as tmpdir:
        snapshot_download(served_model_name, local_dir=tmpdir, ignore_patterns=['*.pt', '*.safetensors', '*.bin', '*.cache', '*.gitattributes', '*.md'])
        for artifact in os.scandir(tmpdir):
            if not artifact.is_file():
                continue
            with open(artifact.path, 'rb') as f, open_stream(f'{tensorizer_args.tensorizer_dir}/{artifact.name}', mode='wb+', **tensorizer_args.stream_kwargs) as stream:
                logger.info('Writing artifact %s', artifact.name)
                stream.write(f.read())

def serialize_vllm_model(model: nn.Module, tensorizer_config: TensorizerConfig, model_config: 'ModelConfig') -> nn.Module:
    model.register_parameter('vllm_tensorized_marker', nn.Parameter(torch.tensor((1,), device='meta'), requires_grad=False))
    tensorizer_args = tensorizer_config._construct_tensorizer_args()
...
def tensorize_vllm_model(engine_args: 'EngineArgs', tensorizer_config: TensorizerConfig, generate_keyfile: bool=True):
    ...
    if generate_keyfile and (keyfile := tensorizer_config.encryption_keyfile) is not None:
        ...
        with open_stream(keyfile, mode='wb+', s3_access_key_id=tensorizer_config.s3_access_key_id, s3_secret_access_key=tensorizer_config.s3_secret_access_key, s3_endpoint=tensorizer_config.s3_endpoint) as stream:
            stream.write(encryption_params.key)
    if not envs.VLLM_USE_V1:
        engine = LLMEngine.from_engine_args(engine_args)
        engine.model_executor.collective_rpc('save_tensorized_model', kwargs={'tensorizer_config': tensorizer_config.to_serializable()})
    else:
        engine = V1LLMEngine.from_vllm_config(engine_config)
        engine.collective_rpc('save_tensorized_model', kwargs={'tensorizer_config': tensorizer_config.to_serializable()})
...
def tensorize_lora_adapter(lora_path: str, tensorizer_config: TensorizerConfig):
    ...
    with open(config_path) as f:
        config = json.load(f)
    tensorizer_args = tensorizer_config._construct_tensorizer_args()
    with open_stream(f'{tensorizer_config.tensorizer_dir}/adapter_config.json', mode='wb+', **tensorizer_args.stream_kwargs) as f:
    ...
    with open_stream(lora_uri, mode='wb+', **tensorizer_args.stream_kwargs) as f:
        ...
        serializer.close()
    logger.info('Successfully serialized LoRA files to %s', str(tensorizer_config.tensorizer_dir))