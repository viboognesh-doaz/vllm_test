...
from transformers_neuronx.config import ContinuousBatchingConfig
from transformers_neuronx.config import ContinuousBatchingConfig, GenerationConfig, KVCacheQuantizationConfig, NeuronConfig, QuantizationConfig, SparseAttnConfig
...
TORCH_DTYPE_TO_NEURON_AMP = {'auto': 'f32', 'half': 'f16', 'float16': 'f16', 'bfloat16': 'bf16', 'float': 'f32', 'float32': 'f32', torch.float16: 'f16', torch.bfloat16: 'bf16', torch.float32: 'f32'}
_NEURON_SUPPORTED_MODELS: dict[str, tuple[str, str, str]] = {'LlamaForCausalLM': ('transformers_neuronx.llama.model', 'LlamaForSampling', 'LlamaForCausalLM'), 'MistralForCausalLM': ('transformers_neuronx.mistral.model', 'MistralForSampling', 'MistralForCausalLM')}

class NeuronCausalLM(nn.Module):

    def __init__(self, config: PretrainedConfig, on_device_sampling_disabled: bool=False) -> None:
        super().__init__()
        self.config = config
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, input_block_ids: torch.Tensor) -> torch.Tensor:
    ...
    def sample(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> Optional[SamplerOutput]:
        ...
        for seq_group in sampling_metadata.seq_groups:
            ...
            for seq_id in seq_group.seq_ids:
                ...
                samples.append(SequenceOutput(parent_seq_id=seq_id, output_token=token_id, logprobs={token_id: Logprob(token_id)}))
                sample_idx += 1
    ...
    def load_weights(self, model_name_or_path: str, **kwargs):
        ...
        neuronx_module_path, neuronx_model_cls_name, hf_model_cls_name = _NEURON_SUPPORTED_MODELS[arch]
...
class NeuronSpeculationCausalLM(nn.Module):
    ...
    def sample(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> Optional[list[SamplerOutput]]:
        ...
        for step_index in range(num_steps):
            if all((token_id == self.SPECULATION_TERMINATION_ID for token_id in accepted_token_ids_by_step[step_index])):
                break
...
def _get_model_architecture(config: PretrainedConfig) -> str:
    ...
    for arch in architectures:
        if arch in _NEURON_SUPPORTED_MODELS:
            return arch
    raise ValueError(f'Model architectures {architectures} are not supported on Neuron for now. Supported architectures: {list(_NEURON_SUPPORTED_MODELS.keys())}')

def _get_buckets(env: str, default_value: list[int]) -> list[int]:
    env_value = os.getenv(env)
    if env_value is None:
        return default_value
    buckets_remove_empty = filter(lambda x: x is not None and len(x.strip()) > 0, env_value.split(','))
    buckets_int = map(int, buckets_remove_empty)
    buckets_list = list(buckets_int)
    return buckets_list

def _get_default_neuron_config(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig):
    ...
    quant_config = dict(dequant_dtype=TORCH_DTYPE_TO_NEURON_AMP[model_config.dtype], quantize_method='vector_dynamic')
    neuron_quantization_config_builder = lambda quant: get_quantization_config(quant).from_config(quant_config).get_quant_method(None, '')
    default_neuron_args = dict(collectives_layout=LAYOUT_BSH, attention_layout=LAYOUT_BSH, fuse_qkv=True, quant=neuron_quantization_config_builder(model_config.quantization) if model_config.quantization else None, continuous_batching=continuous_batching_config, weight_tiling=bool(model_config.quantization), on_device_generation=_get_neuron_on_device_generation_config(model_config))
    return default_neuron_args

def _get_default_neuron_config_for_speculation(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig):
    """Generate a neuron config for speculative decoding based on
    vllm config args."""
    continuous_batching_config = ContinuousBatchingConfig(batch_size_for_shared_caches=scheduler_config.max_num_seqs)
    default_neuron_args = dict(collectives_layout=LAYOUT_BSH, attention_layout=LAYOUT_BSH, fuse_qkv=True, on_device_embedding=True, continuous_batching=continuous_batching_config, on_device_generation=copy.deepcopy(model_config.neuron_sampling_params))
    return default_neuron_args
...
def get_neuron_model(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig) -> nn.Module:
    """Initializes a neuron-optimized model for inference."""
    model = NeuronCausalLM(model_config.hf_config, _is_neuron_on_device_sampling_disabled(model_config))
    default_neuron_config_args = _get_default_neuron_config(model_config, parallel_config, scheduler_config)
    neuron_config = _get_neuron_config_after_override(default_neuron_config_args, model_config.override_neuron_config)
    context_length_estimates = _get_buckets('NEURON_CONTEXT_LENGTH_BUCKETS', [scheduler_config.max_model_len])
    n_positions = _get_buckets('NEURON_TOKEN_GEN_BUCKETS', [scheduler_config.max_model_len])
    model.load_weights(model_config.model, tp_degree=parallel_config.tensor_parallel_size, amp=TORCH_DTYPE_TO_NEURON_AMP[model_config.dtype], neuron_config=neuron_config, context_length_estimate=context_length_estimates, n_positions=n_positions, batch_size=scheduler_config.max_num_seqs)
    return model.eval()

def get_neuron_speculation_model(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig, speculation_config: SpeculativeConfig):
    ...
    default_neuron_config_args = _get_default_neuron_config_for_speculation(model_config, parallel_config, scheduler_config)
    if is_eagle:
        default_neuron_config_args['is_eagle_target'] = True
    neuron_config = _get_neuron_config_after_override(default_neuron_config_args, model_config.override_neuron_config)
    context_length_estimates = _get_buckets('NEURON_CONTEXT_LENGTH_BUCKETS', [scheduler_config.max_model_len])
    n_positions = _get_buckets('NEURON_TOKEN_GEN_BUCKETS', [scheduler_config.max_model_len])
    target_model.load_weights(model_config.model, tp_degree=parallel_config.tensor_parallel_size, amp=TORCH_DTYPE_TO_NEURON_AMP[model_config.dtype], neuron_config=neuron_config, context_length_estimate=context_length_estimates, n_positions=n_positions, batch_size=scheduler_config.max_num_seqs)
    target_model.eval()
    draft_model = NeuronCausalLM(speculation_config.draft_model_config.hf_config)
    default_draft_neuron_config_args = _get_default_neuron_config_for_speculation(speculation_config.draft_model_config, parallel_config, scheduler_config)
    if is_eagle:
        default_draft_neuron_config_args['is_eagle_draft'] = True
        default_draft_neuron_config_args['has_pre_attention_norm'] = False
    draft_neuron_config = _get_neuron_config_after_override(default_draft_neuron_config_args, speculation_config.draft_model_config.override_neuron_config)
    draft_model.load_weights(speculation_config.draft_model_config.model, tp_degree=speculation_config.draft_parallel_config.tensor_parallel_size, amp=TORCH_DTYPE_TO_NEURON_AMP[speculation_config.draft_model_config.dtype], neuron_config=draft_neuron_config, context_length_estimate=context_length_estimates, n_positions=n_positions, batch_size=scheduler_config.max_num_seqs)
    draft_model.eval()
    num_speculative_tokens = speculation_config.num_speculative_tokens
    speculation_model = FusedSpeculativeDecoder(draft_model.model, target_model.model, num_speculative_tokens)
    speculation_model.to_neuron()
    return NeuronSpeculationCausalLM(speculation_model)

def get_neuron_eagle_speculation_model(model_config: ModelConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig, speculation_config: SpeculativeConfig):
