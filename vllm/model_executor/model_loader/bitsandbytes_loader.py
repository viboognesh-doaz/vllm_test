...
from vllm.model_executor.layers.linear import LinearBase, MergedColumnParallelLinear, QKVParallelLinear, ReplicatedLinear, RowParallelLinear
from vllm.model_executor.model_loader.base_loader import BaseModelLoader
from vllm.model_executor.model_loader.utils import ParamMapping, set_default_torch_dtype
from vllm.model_executor.model_loader.weight_utils import download_safetensors_index_file_from_hf, download_weights_from_hf, filter_duplicate_safetensors_files, filter_files_not_needed_for_inference, pt_weights_iterator, safetensors_weights_iterator
from vllm.model_executor.models import is_pooling_model
from vllm.model_executor.models.utils import is_pp_missing_parameter
from vllm.model_executor.utils import get_packed_modules_mapping, set_weight_attrs
...
class BitsAndBytesModelLoader(BaseModelLoader):
    ...
    def _get_weight_files(self, model_name_or_path: str, allowed_patterns: list[str], revision: Optional[str]=None) -> tuple[str, list[str], str]:
        ...
        if is_local:
            ...
            for pattern in allowed_patterns:
                ...
                if matching_files:
                    hf_folder = download_weights_from_hf(model_name_or_path, self.load_config.download_dir, [pattern], revision, ignore_patterns=self.load_config.ignore_patterns)
                    return (hf_folder, glob.glob(os.path.join(hf_folder, pattern)), pattern)
        raise RuntimeError(f'No model weights found in: `{model_name_or_path}`')

    def _prepare_weights(self, model_name_or_path: str, revision: Optional[str]) -> tuple[list[str], bool]:
        """Prepare weight files for the model."""
        allowed_patterns = ['*.safetensors', '*.bin', '*.pt']
        hf_folder, hf_weights_files, matched_pattern = self._get_weight_files(model_name_or_path, allowed_patterns, revision)
    ...
    def _hf_weight_iter(self, hf_weights_files, use_safetensors: bool):
        ...
        def _maybe_pool_model(module_name: str):
            ...
            return module_name
        if use_safetensors:
            iterator = safetensors_weights_iterator(hf_weights_files, self.load_config.use_tqdm_on_load)
        else:
            iterator = pt_weights_iterator(hf_weights_files, self.load_config.use_tqdm_on_load, self.load_config.pt_load_map_location)
    ...
    def _get_quantized_weights_iterator(self, model_name_or_path: str, revision: Optional[str]) -> tuple[Generator[tuple[str, torch.Tensor], None, None], dict[str, Any]]:
        ...
        try:
            ...
            raise ImportError('Please install bitsandbytes>=0.46.1 via `pip install bitsandbytes>=0.46.1` to use bitsandbytes quantizer.') from err
        hf_weights_files, use_safetensors = self._prepare_weights(model_name_or_path, revision)
        quant_state_dict: dict[str, Any] = {}
        if self.pre_quant:
            if self.load_8bit:
                return (self._quantized_8bit_generator(hf_weights_files, use_safetensors, quant_state_dict), quant_state_dict)
            else:
                return (self._quantized_4bit_generator(hf_weights_files, use_safetensors, quant_state_dict), quant_state_dict)
    ...
    def _is_4bit_weight_name(self, weight_name: str):
        quantized_suffix = {'absmax', 'quant_map', 'nested_absmax', 'nested_quant_map', 'bitsandbytes'}
        suffix = weight_name.split('.')[-1]
        return any((q_suffix in suffix for q_suffix in quantized_suffix))

    def _quantized_8bit_generator(self, hf_weights_files, use_safetensors, quant_state_dict) -> Generator:
        for org_weight_name, mapped_weight_name, weight_tensor in self._hf_weight_iter(hf_weights_files, use_safetensors):
    ...
    def _quantized_4bit_generator(self, hf_weights_files, use_safetensors, quant_state_dict) -> Generator:
        ...
        for org_weight_name, mapped_weight_name, weight_tensor in self._hf_weight_iter(hf_weights_files, use_safetensors):
            if self._is_4bit_weight_name(mapped_weight_name):
                continue
            if f'{mapped_weight_name}.quant_state.bitsandbytes__nf4' in temp_state_dict or f'{mapped_weight_name}.quant_state.bitsandbytes__fp4' in temp_state_dict:
    ...
    def _unquantized_generator(self, hf_weights_files, use_safetensors, quant_state_dict) -> Generator:
        ...
        for org_weight_name, mapped_weight_name, weight_tensor in self._hf_weight_iter(hf_weights_files, use_safetensors):
            if any((target_module in mapped_weight_name for target_module in self.target_modules)) and mapped_weight_name.endswith('.weight'):
                if any((mapped_weight_name.startswith(module) for module in self.unsharded_weights_modules)):
                    weight_sub_tensor = weight_tensor
                elif any((mapped_weight_name.startswith(module) for module in self.column_sharded_weights_modules)):
                    total_size = weight_tensor.size(-1)
                    start_index = total_size // tp_size * tp_rank
                    end_index = total_size // tp_size * (tp_rank + 1)
                    weight_sub_tensor = weight_tensor[..., start_index:end_index]
                elif any((mapped_weight_name.startswith(module) for module in self.maybe_fused_weights_modules)):
                    total_shard_sizes = next((sizes for module, sizes in self.maybe_fused_weights_modules.items() if mapped_weight_name.startswith(module)))
                    total_size = weight_tensor.size(0)
                    assert total_size == sum(total_shard_sizes)
                    total_start_index = list(itertools.accumulate([0] + total_shard_sizes))[:-1]
                    shard_weights_index = [(idx + size // tp_size * tp_rank, idx + size // tp_size * (tp_rank + 1)) for idx, size in zip(total_start_index, total_shard_sizes)]
                    weight_tensor = [weight_tensor[start_index:end_index, ...] for start_index, end_index in shard_weights_index]
                ...
                with set_default_torch_dtype(torch.float32):
                    processed_weight, quant_state = quantize_4bit(loaded_weight, compress_statistics=True, quant_type='nf4')
                quant_state_dict[mapped_weight_name] = quant_state
            else:
    ...
    def _get_bnb_target_modules(self, model: nn.Module) -> None:
            ...
            if isinstance(module, LinearBase) and hasattr(module.quant_method, 'quant_config'):
                if (modules_info := self.modules_mapping.get_sub_modules(name)):
                    ...
                    for sub_name in sub_modules:
                        self.target_modules.append(name.replace(rep_name, sub_name))
                ...
                if not hasattr(model, 'get_expert_mapping'):
                    raise AttributeError(f"MoE Model {type(model).__name__} does not support BitsAndBytes quantization yet. Ensure this model has 'get_expert_mapping' method.")
                if self.pre_quant:
                    raise ValueError('Prequant BitsAndBytes models with FusedMoE is not supported yet.')
                if self.load_8bit:
                    raise ValueError('BitsAndBytes 8bit quantization with FusedMoE is not supported yet.')
        ...
        assert self.target_modules, 'vLLM currently does not support BNB quantization for'
    ...
    def _verify_model_compatibility(self, model: nn.Module, model_config: ModelConfig) -> None:
        ...
        if not hasattr(model, 'packed_modules_mapping'):
            raise AttributeError(f"Model {type(model).__name__} does not support BitsAndBytes quantization yet. No 'packed_modules_mapping' found.")
        ...
        if self.pre_quant and get_tensor_model_parallel_world_size() > 1:
            raise ValueError('Prequant BitsAndBytes models with tensor parallelism is not supported. Please try with pipeline parallelism.')
        if self.pre_quant:
    ...
    def _fuse_moe_quant_states(self, model: nn.Module, quant_states_dict: dict) -> dict:
        ...
        for name, module in model.named_modules():
            ...
            w13_qs = QuantState(absmax=w13_absmax, shape=(w13_total_dim0, w1_states_lst[0].shape[1]), code=w1_states_lst[0].code, blocksize=w1_states_lst[0].blocksize, quant_type='nf4', dtype=w1_states_lst[0].dtype)
            w2_qs = QuantState(absmax=w2_absmax, shape=(w2_total_dim0, w2_states_lst[0].shape[1]), code=w2_states_lst[0].code, blocksize=w2_states_lst[0].blocksize, quant_type='nf4', dtype=w2_states_lst[0].dtype)
    ...
    def _stack_quantization_states(self, model: nn.Module, quant_state_dict: dict) -> dict[str, dict[int, Any]]:
        ...
        for quant_param_name in quant_state_dict:
            ...
            shard_index = 0
            for shard_name, (weight_name, index) in self.modules_mapping.inverse_packed_mapping.items():
    ...
    def load_weights(self, model: nn.Module, model_config: ModelConfig) -> None:
        ...
        stacked_quant_state_dict = {**expert_quant_state_dict, **stacked_quant_state_dict}
