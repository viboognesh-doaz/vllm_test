...
from vllm.model_executor.model_loader.tensorizer import TensorizerConfig, deserialize_tensorizer_model, init_tensorizer_model, is_vllm_tensorized, serialize_vllm_model, tensorizer_weights_iterator
from vllm.model_executor.model_loader.utils import get_model_architecture, initialize_model, set_default_torch_dtype
import copy
import torch
logger = init_logger(__name__)
BLACKLISTED_TENSORIZER_ARGS = {'device', 'dtype', 'mode'}
...
class TensorizerLoader(BaseModelLoader):
    ...
    def _load_model_serialized_cpu(self, vllm_config: VllmConfig) -> nn.Module:
    ...
    def load_model(self, vllm_config: VllmConfig, model_config: ModelConfig) -> nn.Module:
        ...
        if parallel_config.tensor_parallel_size > 1:
            self.tensorizer_config.tensorizer_uri = self.tensorizer_config.tensorizer_uri % get_tensor_model_parallel_rank()
        if is_vllm_tensorized(self.tensorizer_config):
            tensorizer_config = self._patch_tensorizer_config(model_config)
            device_config = vllm_config.device_config
            with set_default_torch_dtype(model_config.dtype):
                with torch.device(device_config.device):
                    model = init_tensorizer_model(tensorizer_config=tensorizer_config, vllm_config=vllm_config)
            self.load_weights(model, model_config)
            return model
        return self._load_model_serialized_cpu(vllm_config=vllm_config)

    @staticmethod
    def save_model(model: torch.nn.Module, tensorizer_config: Union[TensorizerConfig, dict], model_config: ModelConfig) -> None:
        if isinstance(tensorizer_config, dict):
            tensorizer_config = TensorizerConfig(**tensorizer_config)
        serialize_vllm_model(model=model, tensorizer_config=tensorizer_config, model_config=model_config)