...
class SmolVLMProcessingInfo(Idefics3ProcessingInfo):
    ...
    def get_hf_processor(self, *, max_image_size: Optional[dict[str, int]]=None, **kwargs: object) -> SmolVLMProcessor:
        if max_image_size is not None:
            kwargs['max_image_size'] = max_image_size
        return self.ctx.get_hf_processor(SmolVLMProcessor, **kwargs)

    def _get_image_token(self, processor: Optional[SmolVLMProcessor]) -> tuple[str, str]:
...
@MULTIMODAL_REGISTRY.register_processor(SmolVLMMultiModalProcessor, info=SmolVLMProcessingInfo, dummy_inputs=SmolVLMDummyInputsBuilder)
