...
class MiMoMultiTokenPredictorLayer(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, prefix: str, model_config: ModelConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None) -> None:
        ...
        self.mtp_block = Qwen2DecoderLayer(config=config, cache_config=cache_config, quant_config=quant_config, prefix=prefix)
        self.final_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)

    def forward(self, inputs_embeds: torch.Tensor, positions: torch.Tensor, previous_hidden_states: torch.Tensor, spec_step_index: int=0) -> torch.Tensor:
...
class MiMoMultiTokenPredictor(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.mtp_layers = torch.nn.ModuleDict({str(idx): MiMoMultiTokenPredictorLayer(config, f'{prefix}.layers.{idx}', model_config=vllm_config.model_config, cache_config=vllm_config.cache_config, quant_config=vllm_config.quant_config) for idx in range(self.mtp_start_layer_idx, self.mtp_start_layer_idx + self.num_mtp_layers)})
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, previous_hidden_states: torch.Tensor, inputs_embeds: Optional[torch.Tensor]=None, spec_step_idx: int=0) -> torch.Tensor:
        ...
        return self.mtp_layers[str(self.mtp_start_layer_idx + spec_step_idx)](inputs_embeds, positions, previous_hidden_states, spec_step_idx)

    def compute_logits(self, hidden_states: torch.Tensor, lm_head: ParallelLMHead, sampling_metadata: SamplingMetadata, spec_step_idx: int=0) -> torch.Tensor:
...
class MiMoMTP(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.model = MiMoMultiTokenPredictor(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
    ...
    def compute_logits(self, hidden_states: torch.Tensor, sampling_metadata: SamplingMetadata, spec_step_idx: int=0) -> Optional[torch.Tensor]:
        return self.model.compute_logits(hidden_states, self.lm_head, sampling_metadata, spec_step_idx)
    ...
    def map_model_name_to_mtp_param_name(self, name: str) -> str:
        name_without_prefix = ['token_layernorm', 'hidden_layernorm', 'input_proj', 'final_layernorm']
        for sub_name in name_without_prefix:
            if sub_name in name:
                return name
        pattern = 'model.mtp_layers.(\\d+).'
