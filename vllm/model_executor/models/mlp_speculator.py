...
class MLPSpeculator(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='') -> None:
        ...
        if self.tie_weights:
            ...
            self.emb = nn.ModuleList([VocabParallelEmbedding(config.vocab_size, self.inner_dim, org_num_embeddings=config.vocab_size) for _ in range(self.max_speculative_tokens)])
            self.proj = nn.ModuleList([nn.Linear(self.emb_dim if i == 0 else self.inner_dim, self.inner_dim, bias=False) for i in range(self.max_speculative_tokens)])
            self.head = nn.ModuleList([ParallelLMHead(self.vocab_size, self.inner_dim, bias=False) for _ in range(self.max_speculative_tokens)])
            self.ln = nn.ModuleList([MLPSpeculatorLayerNorm(self.inner_dim, elementwise_scale_and_shift=True) for _ in range(self.max_speculative_tokens)])
        if self.scale_input:
    ...
    def generate_proposals(self, input_ids: torch.Tensor, previous_hidden_states: torch.Tensor, num_predict_tokens: int, sampling_metadata: SamplingMetadata) -> list[SamplerOutput]:
        if num_predict_tokens > self.max_speculative_tokens:
            raise ValueError(f'Max speculative tokens for model is {self.max_speculative_tokens}, but {num_predict_tokens} were requested')
        ...
        for head_index in range(num_predict_tokens):
            z = self.emb[head_index](last_tokens)
            states = self.proj[head_index](previous_hidden_states)
            states.add_(z, alpha=self.emb_weight / self.state_weight)
            states = self.activation(self.ln[head_index](states))
            previous_hidden_states = states
            states = states.flatten(0, 1)
            logits = self.logits_processor(self.head[head_index], states, sampling_metadata)
