...
class DbrxExperts(FusedMoE):
    ...
    def __init__(self, config: DbrxConfig, quant_config: Optional[QuantizationConfig]=None, params_dtype: Optional[torch.dtype]=None, prefix: str=''):
        super().__init__(num_experts=config.ffn_config.moe_num_experts, top_k=config.ffn_config.moe_top_k, hidden_size=config.d_model, intermediate_size=config.ffn_config.ffn_hidden_size, params_dtype=params_dtype, reduce_results=True, renormalize=True, quant_config=quant_config, tp_size=get_tensor_model_parallel_world_size(), prefix=prefix)
        self.config = config
        self.d_model = config.d_model
        self.intermediate_size = self.config.ffn_config.ffn_hidden_size // self.tp_size

    def weight_loader(self, param: nn.Parameter, loaded_weight: torch.Tensor, weight_name: str, param_name: str):
...
class DbrxMoE(nn.Module):
    ...
    def __init__(self, config: DbrxConfig, quant_config: Optional[QuantizationConfig]=None, params_dtype: Optional[torch.dtype]=None, prefix: str=''):
        ...
        self.experts = DbrxExperts(config=config, quant_config=quant_config, params_dtype=self.params_dtype, prefix=f'{prefix}.experts')
...
class DbrxAttention(nn.Module):
    ...
    def __init__(self, config: DbrxConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.d_model = config.d_model
...
class DbrxBlock(nn.Module):
    ...
    def __init__(self, config: DbrxConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.norm_attn_norm = DbrxFusedNormAttention(config, cache_config, quant_config, prefix=f'{prefix}.norm_attn_norm')
        self.ffn = DbrxMoE(config, quant_config, prefix=f'{prefix}.ffn')

    def forward(self, position_ids: torch.Tensor, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states, residual = self.norm_attn_norm(position_ids=position_ids, hidden_states=hidden_states)
...
class DbrxModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.blocks = make_layers(config.n_layers, lambda prefix: DbrxBlock(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.blocks')
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        expert_params_mapping = [('w13' if weight_name in ['w1', 'v1'] else 'w2', f'mlp.{weight_name}') for weight_name in ['w1', 'v1', 'w2']]
...
class DbrxForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.transformer = DbrxModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'transformer'))
        self.lm_head = ParallelLMHead(config.vocab_size, config.d_model, org_num_embeddings=config.vocab_size, padding_size=DEFAULT_VOCAB_PADDING_SIZE, quant_config=quant_config)
