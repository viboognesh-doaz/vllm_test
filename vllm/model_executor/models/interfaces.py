...
def supports_lora(model: Union[type[object], object]) -> Union[TypeIs[type[SupportsLoRA]], TypeIs[SupportsLoRA]]:
    ...
    if not result:
        ...
        if getattr(model, 'supports_lora', False):
            if missing_attrs:
                logger.warning('The model (%s) sets `supports_lora=True`, but is missing LoRA-specific attributes: %s', model, missing_attrs)
        elif not missing_attrs:
            logger.warning('The model (%s) contains all LoRA-specific attributes, but does not set `supports_lora=True`.', model)
    return result
...
def supports_pp(model: Union[type[object], object]) -> Union[bool, TypeIs[type[SupportsPP]], TypeIs[SupportsPP]]:
    ...
    if supports_attributes and (not supports_inspect):
        logger.warning('The model (%s) sets `supports_pp=True`, but does not accept `intermediate_tensors` in its `forward` method', model)
    if not supports_attributes:
        pp_attrs = ('make_empty_intermediate_tensors',)
        missing_attrs = tuple(attr for attr in pp_attrs if not hasattr(model, attr))
        if getattr(model, 'supports_pp', False):
            if missing_attrs:
                logger.warning('The model (%s) sets `supports_pp=True`, but is missing PP-specific attributes: %s', model, missing_attrs)
        elif not missing_attrs:
            logger.warning('The model (%s) contains all PP-specific attributes, but does not set `supports_pp=True`.', model)
    ...
    def __new__(cls, *args, **kwargs) -> Self:
        ...
        if quant_config is not None:
            ...
            if getattr(instance, 'hf_to_vllm_mapper', None) is not None:
                instance.quant_config.apply_vllm_mapper(instance.hf_to_vllm_mapper)
...
class SupportsTranscription(Protocol):
    ...
    @classmethod
    def get_generation_prompt(cls, audio: np.ndarray, stt_config: SpeechToTextConfig, model_config: ModelConfig, language: str, task_type: str, request_prompt: str) -> PromptType:
