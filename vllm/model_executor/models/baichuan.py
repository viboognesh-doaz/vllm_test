...
from .utils import AutoWeightsLoader, is_pp_missing_parameter, make_empty_intermediate_tensors_factory, make_layers
...
class BaiChuanAttention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, position_embedding: str, rope_theta: float=10000, max_position_embeddings: int=8192, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        ...
        if self.position_embedding == 'ALIBI':
            ...
            self.attn = Attention(self.num_heads, self.head_dim, scaling, alibi_slopes=alibi_slopes, quant_config=quant_config, prefix=f'{prefix}.attn')
        else:
            ...
            self.attn = Attention(self.num_heads, self.head_dim, self.scaling, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.attn')
...
class BaiChuanDecoderLayer(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, position_embedding: str, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.hidden_size = config.hidden_size
        rope_theta = getattr(config, 'rope_theta', 10000)
        max_position_embeddings = getattr(config, 'max_position_embeddings', 8192)
        self.self_attn = BaiChuanAttention(hidden_size=self.hidden_size, num_heads=config.num_attention_heads, position_embedding=position_embedding, rope_theta=rope_theta, max_position_embeddings=max_position_embeddings, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.self_attn')
...
class BaiChuanModel(nn.Module):
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str='', position_embedding: str='ROPE') -> None:
        super().__init__()
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: BaiChuanDecoderLayer(config, position_embedding, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.layers')
...
class BaiChuanBaseForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsQuant):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='', position_embedding: str='ROPE'):
        super().__init__()
        config = vllm_config.model_config.hf_config
        quant_config = vllm_config.quant_config
        ...
        self.model = BaiChuanModel(vllm_config=vllm_config, prefix=prefix, position_embedding=position_embedding)
...
class BaichuanForCausalLM(BaiChuanBaseForCausalLM):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        if config.hidden_size == 4096:
            super().__init__(vllm_config=vllm_config, prefix=prefix, position_embedding='ROPE')
        else:
            super().__init__(vllm_config=vllm_config, prefix=prefix, position_embedding='ALIBI')
