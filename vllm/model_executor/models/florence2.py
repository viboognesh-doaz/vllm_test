...
from vllm.model_executor.models.bart import BartDecoder, BartEncoder, BartParallelLMHead, BartScaledWordEmbedding
...
from vllm.multimodal.processing import BaseProcessingInfo, EncDecMultiModalProcessor, PromptIndexTargets, PromptInsertion, PromptUpdate
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU):
        ...
        self.net = nn.Sequential(OrderedDict([('fc1', nn.Linear(in_features, hidden_features)), ('act', act_layer()), ('fc2', nn.Linear(hidden_features, out_features))]))
...
class DepthWiseConv2d(nn.Module):
    ...
    def __init__(self, dim_in, kernel_size, padding, stride, bias=True):
        super().__init__()
        self.dw = nn.Conv2d(dim_in, dim_in, kernel_size=kernel_size, padding=padding, groups=dim_in, stride=stride, bias=bias)
...
class ChannelBlock(nn.Module):
    ...
    def __init__(self, dim, groups, mlp_ratio=4.0, qkv_bias=True, drop_path_rate=0.0, act_layer=nn.GELU, norm_layer=nn.LayerNorm, conv_at_attn=True, conv_at_ffn=True):
        super().__init__()
        self.conv1 = PreNorm(None, DepthWiseConv2d(dim, 3, 1, 1)) if conv_at_attn else None
        self.channel_attn = PreNorm(norm_layer(dim), ChannelAttention(dim, groups=groups, qkv_bias=qkv_bias))
        self.conv2 = PreNorm(None, DepthWiseConv2d(dim, 3, 1, 1)) if conv_at_ffn else None
        self.ffn = PreNorm(norm_layer(dim), Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer))
...
class SpatialBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size, mlp_ratio=4.0, qkv_bias=True, drop_path_rate=0.0, act_layer=nn.GELU, norm_layer=nn.LayerNorm, conv_at_attn=True, conv_at_ffn=True):
        super().__init__()
        self.conv1 = PreNorm(None, DepthWiseConv2d(dim, 3, 1, 1)) if conv_at_attn else None
        self.window_attn = PreNorm(norm_layer(dim), WindowAttention(dim, num_heads, window_size, qkv_bias=qkv_bias))
        self.conv2 = PreNorm(None, DepthWiseConv2d(dim, 3, 1, 1)) if conv_at_ffn else None
        self.ffn = PreNorm(norm_layer(dim), Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer))
...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=(1, 1, 3, 1), patch_size=(7, 2, 2, 2), patch_stride=(4, 2, 2, 2), patch_padding=(3, 0, 0, 0), patch_prenorm=(False, False, False, False), embed_dims=(64, 128, 192, 256), num_heads=(3, 6, 12, 24), num_groups=(3, 6, 12, 24), window_size=7, mlp_ratio=4.0, qkv_bias=True, drop_path_rate=0.1, norm_layer=nn.LayerNorm, enable_checkpoint=False, conv_at_attn=True, conv_at_ffn=True):
        ...
        for i in range(num_stages):
            conv_embed = ConvEmbed(patch_size=patch_size[i], stride=patch_stride[i], padding=patch_padding[i], in_chans=in_chans if i == 0 else self.embed_dims[i - 1], embed_dim=self.embed_dims[i], norm_layer=norm_layer, pre_norm=patch_prenorm[i])
            convs.append(conv_embed)
            block = MySequential(*[MySequential(OrderedDict([('spatial_block', SpatialBlock(embed_dims[i], num_heads[i], window_size, drop_path_rate=dpr[depth_offset + j * 2], qkv_bias=qkv_bias, mlp_ratio=mlp_ratio, conv_at_attn=conv_at_attn, conv_at_ffn=conv_at_ffn)), ('channel_block', ChannelBlock(embed_dims[i], num_groups[i], drop_path_rate=dpr[depth_offset + j * 2 + 1], qkv_bias=qkv_bias, mlp_ratio=mlp_ratio, conv_at_attn=conv_at_attn, conv_at_ffn=conv_at_ffn))])) for j in range(depths[i])])
            blocks.append(block)
    ...
    def from_config(cls, config):
        return cls(depths=config.depths, embed_dims=config.dim_embed, num_heads=config.num_heads, num_groups=config.num_groups, patch_size=config.patch_size, patch_stride=config.patch_stride, patch_padding=config.patch_padding, patch_prenorm=config.patch_prenorm, drop_path_rate=config.drop_path_rate, window_size=config.window_size)
...
class Florence2LanguageModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.encoder = BartEncoder(config, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.encoder')
        self.decoder = BartDecoder(config, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.decoder')
        if self.config.tie_word_embeddings:
            self.encoder.embed_tokens.weight = self.shared.weight
            self.decoder.embed_tokens.weight = self.shared.weight

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, encoder_input_ids: torch.Tensor, encoder_positions: torch.Tensor, inputs_embeds: Optional[torch.Tensor]=None) -> torch.Tensor:
        """
        Args:
            input_ids
        ...
        if inputs_embeds is not None or encoder_input_ids.numel() > 0:
            encoder_hidden_states = self.encoder(input_ids=encoder_input_ids, positions=encoder_positions, inputs_embeds=inputs_embeds)
        decoder_outputs = self.decoder(decoder_input_ids=input_ids, decoder_positions=positions, encoder_hidden_states=encoder_hidden_states)
...
class Florence2LanguageForConditionalGeneration(nn.Module, SupportsV0Only):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        config = vllm_config.model_config.hf_config
        self.config = config
        self.model = Florence2LanguageModel(vllm_config=vllm_config, prefix=f'{prefix}.model')
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, encoder_input_ids: torch.Tensor, encoder_positions: torch.Tensor, inputs_embeds: Optional[torch.Tensor]=None, **kwargs) -> torch.Tensor:
        """
        Args:
            input_ids
...
class Florence2MultiModalProcessor(EncDecMultiModalProcessor[Florence2ProcessingInfo]):
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptInsertion(modality='image', target=PromptIndexTargets.start(), insertion=image_tokens)]

@MULTIMODAL_REGISTRY.register_processor(Florence2MultiModalProcessor, info=Florence2ProcessingInfo, dummy_inputs=Florence2DummyInputsBuilder)
class Florence2ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsV0Only):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.language_model = Florence2LanguageForConditionalGeneration(vllm_config=vllm_config.with_hf_config(config.text_config), prefix=f'{prefix}.language_model')
    ...
    def _build_image_projection_layers(self, config: PretrainedConfig):
        ...
        if image_pos_embed_config['type'] == 'learned_abs_2d':
            self.image_pos_embed = LearnedAbsolutePositionEmbedding2D(embedding_dim=image_dim_out, num_pos=image_pos_embed_config['max_pos_embeddings'])
        else:
        ...
        if visual_temporal_embedding_config['type'] == 'COSINE':
            self.visual_temporal_embed = PositionalEmbeddingCosine1D(embed_dim=image_dim_out, max_seq_len=visual_temporal_embedding_config['max_temporal_embeddings'])
        else:
    ...
    def _validate_pixel_values(self, data: Union[torch.Tensor, list[torch.Tensor]]) -> Union[torch.Tensor, list[torch.Tensor]]:
        ...
        def _validate_shape(d: torch.Tensor):
            ...
            if actual_dims != expected_dims:
                ...
                raise ValueError(f'The expected shape of pixel values per batch is {expected_expr}. You supplied {tuple(d.shape)}.')
        for d in data:
            _validate_shape(d)
        return data
    ...
    def _parse_and_validate_image_input(self, **kwargs: object):
        ...
        if pixel_values is not None:
            return Florence2ImagePixelInputs(type='pixel_values', data=self._validate_pixel_values(flatten_bn(pixel_values, concat=True)))
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.pad_token_id)
        return inputs_embeds

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, *, encoder_input_ids: torch.Tensor, encoder_positions: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        Args:
            input_ids
                torch.Tensor of *decoder* input token ids.
        ...
        if encoder_input_ids.numel() > 0 or vision_embeddings is not None:
            ...
            inputs_embeds = None
        hidden_states = self.language_model(input_ids, positions, encoder_input_ids, encoder_positions, inputs_embeds=inputs_embeds)
