...
class Glm4Attention(nn.Module):
    ...
    def __init__(self, config: Glm4Config, hidden_size: int, num_heads: int, num_kv_heads: int, max_position: int=4096 * 32, head_dim: Optional[int]=None, qkv_bias: bool=False, rope_theta: float=10000, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, rope_scaling: Optional[tuple]=None, prefix: str='', attn_type: str=AttentionType.DECODER) -> None:
        super().__init__()
        ...
        self.rotary_emb = get_rope(self.head_dim, rotary_dim=self.rotary_dim, max_position=max_position, base=self.rope_theta, rope_scaling=rope_scaling, partial_rotary_factor=partial_rotary_factor, is_neox_style=False)
...
class Glm4DecoderLayer(nn.Module):
    ...
    def __init__(self, config: Glm4Config, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        self.hidden_size = config.hidden_size
        rope_theta = getattr(config, 'rope_theta', 1000000)
        rope_scaling = getattr(config, 'rope_scaling', None)
        self.self_attn = Glm4Attention(config=config, hidden_size=self.hidden_size, num_heads=config.num_attention_heads, max_position=config.max_position_embeddings, num_kv_heads=config.num_key_value_heads, rope_theta=rope_theta, qkv_bias=getattr(config, 'attention_bias', False), head_dim=getattr(config, 'head_dim', None), cache_config=cache_config, quant_config=quant_config, rope_scaling=rope_scaling, prefix=f'{prefix}.self_attn', attn_type=AttentionType.DECODER)
        self.mlp = Glm4MLP(hidden_size=self.hidden_size, intermediate_size=config.intermediate_size, hidden_act=config.hidden_act, quant_config=quant_config, prefix=f'{prefix}.mlp')
...
class Glm4Model(LlamaModel):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        super().__init__(vllm_config=vllm_config, prefix=prefix, layer_type=Glm4DecoderLayer)
...
class Glm4ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.model = Glm4Model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
