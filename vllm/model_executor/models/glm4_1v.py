...
from vllm.multimodal.inputs import MultiModalDataDict, MultiModalFieldConfig, MultiModalKwargs, VideoItem
...
class Glm4vVisionMLP(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: int, bias: bool=False, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.gate_up_proj = MergedColumnParallelLinear(input_size=in_features, output_sizes=[hidden_features] * 2, bias=bias, quant_config=quant_config, prefix=f'{prefix}.gate_up_proj')
        self.down_proj = RowParallelLinear(hidden_features, in_features, bias=bias, quant_config=quant_config, prefix=f'{prefix}.down_proj')
...
class Glm4vVisionAttention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, projection_size: int, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        ...
        self.qkv = QKVParallelLinear(hidden_size=embed_dim, head_size=self.hidden_size_per_attention_head, total_num_heads=num_heads, total_num_kv_heads=num_heads, bias=False, quant_config=quant_config, prefix=f'{prefix}.qkv')
        self.proj = RowParallelLinear(input_size=projection_size, output_size=embed_dim, quant_config=quant_config, prefix=f'{prefix}.proj', bias=False)
...
class Glm4vVisionBlock(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, mlp_hidden_dim: int, norm_layer: Optional[Callable[[int], nn.Module]]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        if norm_layer is None:
            norm_layer = partial(nn.LayerNorm, eps=1e-06)
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.attn = Glm4vVisionAttention(embed_dim=dim, num_heads=num_heads, projection_size=dim, quant_config=quant_config, prefix=f'{prefix}.attn')
        self.mlp = Glm4vVisionMLP(dim, mlp_hidden_dim, bias=False, quant_config=quant_config, prefix=f'{prefix}.mlp')
...
class Glm4vVisionPatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size: int=14, temporal_patch_size: int=1, in_channels: int=3, hidden_size: int=1536) -> None:
        super().__init__()
...
class Glm4vPatchMerger(nn.Module):
    ...
    def __init__(self, d_model: int, context_dim: int, quant_config: Optional[QuantizationConfig]=None, bias: bool=False) -> None:
        super().__init__()
        self.hidden_size = d_model
        self.proj = ColumnParallelLinear(self.hidden_size, self.hidden_size, bias=bias, gather_output=True)
        self.post_projection_norm = nn.LayerNorm(self.hidden_size)
        self.gate_up_proj = MergedColumnParallelLinear(input_size=self.hidden_size, output_sizes=[context_dim] * 2, bias=bias, quant_config=quant_config)
        self.down_proj = RowParallelLinear(context_dim, self.hidden_size, bias=bias, quant_config=quant_config)
        self.act_fn = SiluAndMul()
...
class Glm4vVisionEmbeddings(nn.Module):
    ...
    def forward(self, embeddings, lengths, image_shapes, h_coords, w_coords) -> torch.Tensor:
        ...
        if total_seq == 0:
            ...
            pos_embed_2d = pos_embed_weight.view(orig_size, orig_size, hidden_size).permute(2, 0, 1).unsqueeze(0).to(device=device, dtype=torch.float32)
            ...
            interpolated_embed_fp32 = F.grid_sample(pos_embed_2d, grid, mode='bicubic', align_corners=False, padding_mode='border')
...
class Glm4vVisionTransformer(nn.Module):
    ...
    def __init__(self, vision_config: Glm4vVisionConfig, norm_eps: float=1e-06, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        self.patch_embed = Glm4vVisionPatchEmbed(patch_size=patch_size, temporal_patch_size=temporal_patch_size, in_channels=in_channels, hidden_size=self.hidden_size)
        norm_layer = partial(RMSNorm, eps=norm_eps)
        head_dim = self.hidden_size // self.num_heads
        self.rotary_pos_emb = Glm4vVisionRotaryEmbedding(head_dim // 2)
        self.blocks = nn.ModuleList([Glm4vVisionBlock(dim=self.hidden_size, num_heads=self.num_heads, mlp_hidden_dim=vision_config.out_hidden_size, norm_layer=norm_layer, quant_config=quant_config, prefix=f'{prefix}.blocks.{layer_idx}') for layer_idx in range(depth)])
        self.merger = Glm4vPatchMerger(d_model=vision_config.out_hidden_size, context_dim=vision_config.intermediate_size, quant_config=quant_config, bias=False)
        self.embeddings = Glm4vVisionEmbeddings(vision_config)
        self.post_conv_layernorm = RMSNorm(vision_config.hidden_size, eps=vision_config.rms_norm_eps)
        self.downsample = nn.Conv2d(in_channels=vision_config.hidden_size, out_channels=vision_config.out_hidden_size, kernel_size=vision_config.spatial_merge_size, stride=vision_config.spatial_merge_size)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        stacked_params_mapping = [('attn.qkv.', 'attn.q.', 'q'), ('attn.qkv.', 'attn.k.', 'k'), ('attn.qkv.', 'attn.v.', 'v'), ('gate_up_proj', 'gate_proj', 0), ('gate_up_proj', 'up_proj', 1)]
...
class Glm4vProcessingInfo(BaseProcessingInfo):
    ...
    def _get_vision_info(self, *, image_width: int, image_height: int, num_frames: int=16, do_resize: bool=True, max_image_pixels: int=28 * 28 * 2 * 30000) -> tuple[ImageSize, int]:
        ...
        temporal_patch_size = vision_config.temporal_patch_size
        if do_resize:
            resized_height, resized_width = smart_resize(num_frames=num_frames if num_frames > temporal_patch_size else temporal_patch_size, height=image_height, width=image_width, factor=patch_size * merge_size, max_pixels=max_image_pixels)
    ...
    def get_num_image_tokens(self, *, image_width: int, image_height: int) -> int:
        _, num_image_tokens = self._get_vision_info(image_width=image_width, image_height=image_height, max_image_pixels=28 * 28 * 2 * 6144)
    ...
    def get_num_video_tokens(self, *, image_width: int, image_height: int, num_frames: int) -> int:
        _, num_video_tokens = self._get_vision_info(image_width=image_width, image_height=image_height, num_frames=num_frames, max_image_pixels=28 * 28 * 2 * 30000)
    ...
    def _get_max_video_frames(self, max_tokens: int) -> int:
        ...
        while True:
            ...
            next_max_tokens = self.get_num_video_tokens(image_width=target_width, image_height=target_height, num_frames=next_num_frames)
...
class Glm4vDummyInputsBuilder(BaseDummyInputsBuilder[Glm4vProcessingInfo]):
    ...
    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        ...
        video_token_ids = [hf_config.video_start_token_id, hf_processor.video_token_id, hf_config.video_end_token_id]
    ...
    def _get_dummy_videos(self, *, width: int, height: int, num_frames: int, num_videos: int) -> list[VideoItem]:
        video = np.full((num_frames, width, height, 3), 255, dtype=np.uint8)
        video_items = []
        for i in range(num_videos):
            video_metadata = {'fps': 2.0, 'duration': num_frames / 2.0, 'total_num_frames': num_frames, 'video_backend': 'opencv'}
...
class Glm4vMultiModalProcessor(BaseMultiModalProcessor[Glm4vProcessingInfo]):
        ...
        if 'videos' in mm_data and isinstance(mm_data['videos'], list) and (len(mm_data['videos']) > 0):
            ...
            for item in mm_data.pop('videos', []):
                ...
                if metadata['total_num_frames'] != len(video_array):
                    logger.warning('Total frames in metadata (%s) does not match the length of video array %s. This can be because the video is resampled in advance. This may cause a divergence with HF implementation.', metadata['total_num_frames'], len(video_array))
                    metadata['total_num_frames'] = len(video_array)
                metadata = VideoMetadata(**metadata)
                video_mm_data = dict()
                video_mm_data['videos'] = [[video_array]]
                video_mm_data['video_metadata'] = [[metadata]]
                video_outputs = super()._call_hf_processor(prompt='<|begin_of_video|><|video|><|end_of_video|>', mm_data=video_mm_data, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
                input_ids = video_outputs.pop('input_ids')
                input_ids[input_ids == processor.image_token_id] = processor.video_token_id
                video_placeholder = processor.tokenizer.batch_decode(input_ids)[0]
                prompt = prompt.replace('<|begin_of_video|><|video|><|end_of_video|>', video_placeholder)
            ...
            video_outputs = dict(pixel_values_videos=torch.cat(pixel_values_videos_lst), video_grid_thw=torch.cat(video_grid_thw_lst))
        else:
            video_outputs = dict()
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, Any], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_video_replacement_glm4v(item_idx: int):
            ...
            return PromptUpdateDetails.select_token_id(placeholder, embed_token_id=hf_processor.video_token_id)
        return [PromptReplacement(modality='image', target=hf_processor.image_token, replacement=get_image_replacement_glm4v), PromptReplacement(modality='video', target='<|begin_of_video|><|video|><|end_of_video|>', replacement=get_video_replacement_glm4v)]

@MULTIMODAL_REGISTRY.register_processor(Glm4vMultiModalProcessor, info=Glm4vProcessingInfo, dummy_inputs=Glm4vDummyInputsBuilder)
class Glm4vForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsLoRA, SupportsPP):
    ...
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_prefix={'lm_head.': 'language_model.lm_head.', 'model.language_model.': 'language_model.model.', 'model.visual.': 'visual.'})
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.visual = Glm4vVisionTransformer(config.vision_config, norm_eps=getattr(config, 'rms_norm_eps', 1e-05), quant_config=quant_config, prefix=maybe_prefix(prefix, 'visual'))
        self.language_model = init_vllm_registered_model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, ''), architectures=['Glm4ForCausalLM'])
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[Glm4vImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return Glm4vImagePixelInputs(type='pixel_values', pixel_values=pixel_values, image_grid_thw=image_grid_thw)
        if image_embeds is not None:
            ...
            return Glm4vImageEmbeddingInputs(type='image_embeds', image_embeds=image_embeds, image_grid_thw=image_grid_thw)

    def _parse_and_validate_video_input(self, **kwargs: object) -> Optional[Glm4vVideoInputs]:
        ...
        if pixel_values_videos is not None:
            ...
            return Glm4vVideoPixelInputs(type='pixel_values_videos', pixel_values_videos=pixel_values_videos, video_grid_thw=video_grid_thw)
        if video_embeds is not None:
            ...
            return Glm4vVideoEmbeddingInputs(type='video_embeds', video_embeds=video_embeds, video_grid_thw=video_grid_thw)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0 and all(embed.numel() > 0 for embed in multimodal_embeddings):
    ...
    def get_input_embeddings_v0(self, input_ids: torch.Tensor, image_input: Optional[Glm4vImageInputs]=None, video_input: Optional[Glm4vVideoInputs]=None) -> torch.Tensor:
