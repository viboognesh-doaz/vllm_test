...
def _get_alibi_slopes(n):
    ...
    if math.log2(n).is_integer():
        ...
        return get_slopes_power_of_2(closest_power_of_2) + _get_alibi_slopes(2 * closest_power_of_2)[0::2][:n - closest_power_of_2]

class JAISAttention(nn.Module):

    def __init__(self, config: JAISConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        ...
        self.c_attn = QKVParallelLinear(self.hidden_size, self.head_dim, total_num_heads, bias=True, quant_config=quant_config)
        ...
        self.attn = Attention(self.num_heads, self.head_dim, scale=self.scale, alibi_slopes=alibi_slopes, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.attn')
...
class JAISMLP(nn.Module):
    ...
    def __init__(self, intermediate_size: int, config: JAISConfig, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        ...
        self.c_fc2 = ColumnParallelLinear(hidden_size, intermediate_size, bias=True, quant_config=quant_config) if self.swiglu else None
...
class JAISModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.h = make_layers(config.num_hidden_layers, lambda prefix: JAISBlock(config=config, cache_config=cache_config, quant_config=quant_config, prefix=prefix), prefix=f'{prefix}.h')
    ...
    def forward(self, input_ids: torch.Tensor, position_ids: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None) -> Union[IntermediateTensors, torch.Tensor]:
...
class JAISLMHeadModel(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.transformer = JAISModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'transformer'))
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None) -> Union[IntermediateTensors, torch.Tensor]:
