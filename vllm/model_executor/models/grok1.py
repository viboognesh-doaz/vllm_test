...
class Grok1MoE(nn.Module):
    ...
    def __init__(self, num_experts: int, top_k: int, hidden_size: int, intermediate_size: int, params_dtype: Optional[torch.dtype]=None, quant_config: Optional[QuantizationConfig]=None, tp_size: Optional[int]=None, prefix: str=''):
        ...
        self.gate = ReplicatedLinear(hidden_size, num_experts, bias=False, params_dtype=params_dtype, quant_config=None, prefix=f'{prefix}.gate')
        self.experts = FusedMoE(num_experts=num_experts, top_k=top_k, hidden_size=hidden_size, intermediate_size=intermediate_size, params_dtype=params_dtype, reduce_results=True, renormalize=True, quant_config=quant_config, tp_size=tp_size, activation='gelu', prefix=f'{prefix}.experts')
...
class Grok1Attention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, num_kv_heads: int, max_position: int=4096 * 32, rope_theta: float=10000, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='', config=None) -> None:
        super().__init__()
        self.hidden_size = hidden_size
        self.config = config
        ...
        self.attn = Attention(self.num_heads, self.head_dim, self.scaling, num_kv_heads=self.num_kv_heads, cache_config=cache_config, quant_config=quant_config, logits_soft_cap=attn_logits_soft_cap, prefix=f'{prefix}.attn')
...
class Grok1DecoderLayer(nn.Module):
    ...
    def __init__(self, config, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        rope_theta = 10000
        self.attn = Grok1Attention(hidden_size=self.hidden_size, num_heads=config.num_attention_heads, max_position=config.max_position_embeddings, num_kv_heads=config.num_key_value_heads, rope_theta=rope_theta, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.attn', config=config)
        num_experts = getattr(config, 'num_experts', 8)
        num_experts_per_tok = getattr(config, 'num_experts_per_tok', 2)
        self.moe_block = Grok1MoE(num_experts=num_experts, top_k=num_experts_per_tok, hidden_size=config.hidden_size, intermediate_size=config.intermediate_size, quant_config=quant_config, prefix=f'{prefix}.moe_block')
...
class Grok1Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.embedding_multiplier_scale = getattr(config, 'embedding_multiplier_scale', DEFAULT_EMBEDDING_MULTIPLIER_SCALE)
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: Grok1DecoderLayer(config, cache_config, quant_config=quant_config, prefix=prefix), prefix=f'{prefix}.layers')
    ...
    def get_expert_mapping(self) -> list[tuple[str, str, int, str]]:
        ...
        return FusedMoE.make_expert_params_mapping(ckpt_gate_proj_name='linear', ckpt_down_proj_name='linear_1', ckpt_up_proj_name='linear_v', num_experts=num_experts)
...
class Grok1ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.quant_config = quant_config
        self.model = Grok1Model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
        self.unpadded_vocab_size = config.vocab_size
        if lora_config:
            self.unpadded_vocab_size += lora_config.lora_extra_vocab_size
        self.lm_head = ParallelLMHead(self.unpadded_vocab_size, config.hidden_size, org_num_embeddings=config.vocab_size, padding_size=DEFAULT_VOCAB_PADDING_SIZE, quant_config=quant_config, prefix=maybe_prefix(prefix, 'lm_head'))
        if self.config.tie_word_embeddings:
            self.lm_head.weight = self.model.embed_tokens.weight
        self.output_multiplier_scale = getattr(config, 'output_multiplier_scale', DEFAULT_OUTPUT_MULTIPLIER_SCALE)
        self.logits_processor = LogitsProcessor(self.unpadded_vocab_size, config.vocab_size, self.output_multiplier_scale)
