...
from .utils import AutoWeightsLoader, WeightsMapper, flatten_bn, is_pp_missing_parameter, maybe_prefix, merge_multimodal_embeddings
...
class AriaVisionTransformer(Idefics3VisionTransformer, SupportsQuant):
    ...
    def __init__(self, config: Idefics2VisionConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
...
class AriaProjectorMLP(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: int, output_dim: int) -> None:
        super().__init__()
...
class AriaTextMoELayer(nn.Module):
    ...
    def __init__(self, config: AriaTextConfig, quant_config: Optional[QuantizationConfig], prefix: str='') -> None:
        super().__init__()
        self.config = config
        self.router_weight = nn.Parameter(torch.empty((self.config.moe_num_experts, self.config.hidden_size)))
        self.experts = AriaFusedMoE(num_experts=config.moe_num_experts, top_k=config.moe_topk, hidden_size=config.hidden_size, intermediate_size=config.intermediate_size, quant_config=quant_config, reduce_results=True, prefix=f'{prefix}.experts')
        self.shared_experts = LlamaMLP(config.hidden_size, config.intermediate_size * config.moe_num_shared_experts, 'silu', quant_config=quant_config, bias=config.mlp_bias)
...
class AriaTextDecoderLayer(LlamaDecoderLayer):
    ...
    def __init__(self, config: AriaTextConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
...
class AriaTextModel(LlamaModel, SupportsQuant):
    ...
    """
    packed_modules_mapping = {'qkv_proj': ['q_proj', 'k_proj', 'v_proj'], 'gate_up_proj': ['gate_proj', 'up_proj'], 'experts.w13_weight': ['experts.fc1.weight'], 'experts.w2_weight': ['experts.fc2.weight']}

    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        super().__init__(vllm_config=vllm_config, prefix=prefix, layer_type=AriaTextDecoderLayer)

    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        stacked_params_mapping = [('.qkv_proj', '.q_proj', 'q'), ('.qkv_proj', '.k_proj', 'k'), ('.qkv_proj', '.v_proj', 'v'), ('.gate_up_proj', '.gate_proj', 0), ('.gate_up_proj', '.up_proj', 1), ('experts.w13_weight', 'experts.fc1.weight', 'w13'), ('experts.w2_weight', 'experts.fc2.weight', 'w2')]
        params_dict = dict(self.named_parameters())
        loaded_params: set[str] = set()
        for name, loaded_weight in weights:
...
class AriaDummyInputsBuilder(BaseDummyInputsBuilder[AriaProcessingInfo]):
    ...
    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        ...
        image_token: str = processor.tokenizer.image_token
...
class AriaMultiModalProcessor(BaseMultiModalProcessor[AriaProcessingInfo]):
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(pixel_values=MultiModalFieldConfig.batched('image'), pixel_mask=MultiModalFieldConfig.batched('image'))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptReplacement(modality='image', target=[image_token_id], replacement=[image_token_id] * num_image_tokens)]

@MULTIMODAL_REGISTRY.register_processor(AriaMultiModalProcessor, info=AriaProcessingInfo, dummy_inputs=AriaDummyInputsBuilder)
class AriaForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    """
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_prefix={'model.language_model.': 'language_model.model.', 'model.vision_tower.': 'vision_tower.', 'model.multi_modal_projector.': 'multi_modal_projector.', 'language_model.model': 'language_model', 'language_model.lm_head': 'lm_head'}, orig_to_new_suffix={'router.weight': 'router_weight'})
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.config = config
        self.vision_tower = AriaVisionTransformer(config.vision_config, quant_config=quant_config, prefix=f'{prefix}.vision_tower')
        self.multi_modal_projector = AriaProjector(config)
        self.vocab_size = config.text_config.vocab_size
        self.language_model = AriaTextModel(vllm_config=vllm_config.with_hf_config(config.text_config), prefix=maybe_prefix(prefix, 'language_model.model'))
        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1
        self.unpadded_vocab_size = config.text_config.vocab_size
        self.lm_head = ParallelLMHead(self.unpadded_vocab_size, config.text_config.hidden_size, org_num_embeddings=self.language_model.org_vocab_size, quant_config=quant_config)
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[AriaImagePixelInputs]:
        ...
        if pixel_mask is not None:
            if not isinstance(pixel_mask, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of pixel mask. Got type: {type(pixel_mask)}')
            pixel_mask = flatten_bn(pixel_mask, concat=True)
        return AriaImagePixelInputs(pixel_values=pixel_values, pixel_mask=pixel_mask)

    def _create_patch_attention_mask(self, pixel_mask: Optional[torch.Tensor]) -> torch.Tensor:
    ...
    def _process_image_input(self, image_input: AriaImagePixelInputs) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        image_outputs = self.vision_tower(pixel_values=pixel_values, patch_attention_mask=patch_attention_mask)
