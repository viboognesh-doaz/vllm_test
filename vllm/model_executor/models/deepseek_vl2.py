...
class DeepseekVL2ProcessingInfo(BaseProcessingInfo):
    ...
    def get_image_size_with_most_features(self) -> ImageSize:
        ...
        height, width = max(candidate_resolutions, key=lambda x: self.get_num_image_tokens(image_width=x[1], image_height=x[0]))
...
class DeepseekVL2DummyInputsBuilder(BaseDummyInputsBuilder[DeepseekVL2ProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'image': self._get_dummy_images(width=max_image_size.width, height=max_image_size.height, num_images=num_images)}
...
class DeepseekVL2MultiModalProcessor(BaseMultiModalProcessor[DeepseekVL2ProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        if mm_data:
            processed_outputs = self.info.ctx.call_hf_processor(self.info.get_hf_processor(**mm_kwargs), dict(prompt=prompt, **mm_data), dict(**mm_kwargs, **tok_kwargs))
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(pixel_values=MultiModalFieldConfig.batched('image'), images_spatial_crop=MultiModalFieldConfig.batched('image'), image_embeds=MultiModalFieldConfig.batched('image'))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement_deepseek_vl2(item_idx: int):
            ...
            if isinstance(images, ImageEmbeddingItems):
                ...
                num_image_tokens = self.info.get_num_image_tokens(image_width=image_size.width, image_height=image_size.height, cropping=len(images) <= 2)
            return [image_token_id] * num_image_tokens
        return [PromptReplacement(modality='image', target=[image_token_id], replacement=get_replacement_deepseek_vl2)]
...
@MULTIMODAL_REGISTRY.register_processor(DeepseekVL2MultiModalProcessor, info=DeepseekVL2ProcessingInfo, dummy_inputs=DeepseekVL2DummyInputsBuilder)
class DeepseekVLV2ForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_prefix={'language.': 'language_model.'})
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.language_model = init_vllm_registered_model(vllm_config=vllm_config, hf_config=self.text_config, prefix=maybe_prefix(prefix, 'language'), architectures=architectures)
        self.make_empty_intermediate_tensors = self.language_model.make_empty_intermediate_tensors

    def _init_vision_module(self, vision_config: VisionEncoderConfig, quant_config: Optional[QuantizationConfig], prefix: str='') -> nn.Module:
        try:
        except ImportError:
    ...
    def _validate_images_spatial_crop(self, data: Union[torch.Tensor, list[torch.Tensor]]) -> Union[torch.Tensor, list[torch.Tensor]]:
        expected_dims = 2
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[DeepseekVL2ImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return DeepseekVL2ImagePixelInputs(type='pixel_values', data=self._validate_pixel_values(flatten_bn(pixel_values)), images_spatial_crop=self._validate_images_spatial_crop(flatten_bn(images_spatial_crop, concat=True)))
        if image_embeds is not None:
            if not isinstance(image_embeds, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of image embeddings. Got type: {type(image_embeds)}')
            return DeepseekVL2VImageEmbeddingInputs(type='image_embeds', data=flatten_bn(image_embeds))
        raise AssertionError('This line should be unreachable.')

    def _pixel_values_to_embedding(self, pixel_values: NestedTensors, images_spatial_crop: torch.Tensor) -> NestedTensors:
        ...
        for jdx in range(images_spatial_crop.size(0)):
            ...
            if self.global_view_pos == 'head':
                ...
                global_local_features = torch.cat([local_features, self.view_seperator[None, :], global_features])
