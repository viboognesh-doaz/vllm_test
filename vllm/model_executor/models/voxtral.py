...
from mistral_common.protocol.instruct.messages import AudioChunk, RawAudio, TextChunk, UserMessage
...
from vllm.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, MultiModalHashes, PromptReplacement, PromptUpdate
...
class VoxtralProcessorAdapter:
    ...
    def __call__(self, text: Optional[Union[TextInput, list[TextInput]]]=None, audios: Optional[Union[np.ndarray, list[np.ndarray]]]=None, return_tensors: Optional[Union[str, TensorType]]=None, **kwargs) -> Mapping[str, NestedTensors]:
        if text is None:
            text = []
        ...
        return {'input_ids': torch.cat(audios_tokens)[None].expand(len(text), -1), 'audio_arrays': audios_processed}
...
class VoxtralDummyInputsBuilder(BaseDummyInputsBuilder[VoxtralProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'audio': self._get_dummy_audios(length=target_length, num_audios=num_audios)}
    ...
    def get_dummy_processor_inputs(self, seq_len: int, mm_counts: Mapping[str, int]) -> ProcessorInputs:
        ...
        for audio in dummy_audios:
            audio_item = Audio(audio_array=audio, sampling_rate=self.info.get_hf_processor().sampling_rate, format=format)
            chunk = AudioChunk(input_audio=RawAudio.from_audio(audio_item))
            audio_chunks.append(chunk)
        request = ChatCompletionRequest(messages=[UserMessage(content=[TextChunk(text=dummy_text), *audio_chunks])])
...
class VoxtralMultiModalProcessor(BaseMultiModalProcessor[VoxtralProcessingInfo]):
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptReplacement(modality='audio', target='', replacement=get_replacement)]
...
@MULTIMODAL_REGISTRY.register_processor(VoxtralMultiModalProcessor, info=VoxtralProcessingInfo, dummy_inputs=VoxtralDummyInputsBuilder)
class VoxtralForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsTranscription):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.whisper_encoder = VoxtralEncoderModel(vllm_config.with_hf_config(config.audio_config), prefix=maybe_prefix(prefix, 'whisper_encoder'))
        self.audio_language_adapter = AudioLanguageAdapter(hidden_size=config.audio_config.d_model * self.downsample_factor, dim=config.text_config.hidden_size)
    ...
    def get_multimodal_embeddings(self, **kwargs) -> Union[list[torch.Tensor], torch.Tensor, tuple[torch.Tensor, ...], None]:
        ...
        for i, audio_embedding in enumerate(audio_embeddings):
            ...
            audio_embedding = torch.nn.functional.pad(audio_embedding, (0, 0, 0, target_seq_len - seq_len))
            audio_embeddings[i] = audio_embedding.reshape(target_seq_len // self.downsample_factor, dim * self.downsample_factor)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, audio_tok_id)
        return inputs_embeds

    def _parse_and_validate_audio_arrays(self, **kwargs: object) -> Union[list[torch.Tensor], None]:
    ...
    def get_speech_to_text_config(cls, model_config: ModelConfig, task_type: str) -> SpeechToTextConfig:
        ...
        return SpeechToTextConfig(max_audio_clip_s=max_audio_clip_s, sample_rate=sample_rate, min_energy_split_window_size=None)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        remapping_rules = [('mm_whisper_embeddings\\.(.*)', '\\1'), ('audio_language_projection\\.(.*)', 'audio_language_adapter.\\1'), ('audio_language_adapter\\.0\\.weight', 'audio_language_adapter.w_in.weight'), ('audio_language_adapter\\.2\\.weight', 'audio_language_adapter.w_out.weight')]
        audio_params = dict(nn.ModuleDict({'audio_language_adapter': self.audio_language_adapter}).named_parameters())
        loaded_weights = set()

        def llm_weights_generator():
            nonlocal loaded_weights
            for name, w in weights:
                is_encoder = name.startswith('mm_whisper_embeddings') and (not name.startswith('mm_whisper_embeddings.tok_embeddings')) and (not name.startswith('mm_whisper_embeddings.audio_language_projection'))
...
class VoxtralEncoderModel(nn.Module):
    ...
    mistral_remapping = [('whisper_encoder\\.conv_layers\\.0\\.(weight|bias)', 'whisper_encoder.conv1.\\1'), ('whisper_encoder\\.conv_layers\\.1\\.(weight|bias)', 'whisper_encoder.conv2.\\1'), ('whisper_encoder\\.transformer\\.layers\\.(\\d+)\\.attention\\.w([qkv])\\.(weight|bias)', 'whisper_encoder.layers.\\1.self_attn.\\2_proj.\\3'), ('whisper_encoder\\.transformer\\.layers\\.(\\d+)\\.attention\\.wo\\.(weight|bias)', 'whisper_encoder.layers.\\1.self_attn.out_proj.\\2'), ('whisper_encoder\\.transformer\\.layers\\.(\\d+)\\.attention_norm\\.(weight|bias)', 'whisper_encoder.layers.\\1.self_attn_layer_norm.\\2'), ('whisper_encoder\\.transformer\\.layers\\.(\\d+)\\.feed_forward\\.w1\\.(weight|bias)', 'whisper_encoder.layers.\\1.mlp.fc1.\\2'), ('whisper_encoder\\.transformer\\.layers\\.(\\d+)\\.feed_forward\\.w2\\.(weight|bias)', 'whisper_encoder.layers.\\1.mlp.fc2.\\2'), ('whisper_encoder\\.transformer\\.layers\\.(\\d+)\\.ffn_norm\\.(weight|bias)', 'whisper_encoder.layers.\\1.final_layer_norm.\\2'), ('whisper_encoder\\.transformer\\.norm\\.(weight|bias)', 'whisper_encoder.layer_norm.\\1')]
    ...
    def __init__(self, vllm_config: VllmConfig, *, prefix: str='') -> None:
        ...
        self.whisper_encoder = WhisperEncoder(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'whisper_encoder'), is_standalone_encoder=True, init_in_fp32=True)
        mel_filters = mel_filter_bank(num_frequency_bins=1 + self.config.window_size // 2, num_mel_bins=self.config.num_mel_bins, min_frequency=0.0, max_frequency=8000.0, sampling_rate=self.config.sampling_rate)
    ...
    def compute_whisper_melspec(self, audio_waveforms: torch.Tensor) -> torch.Tensor:
        ...
        stft = torch.stft(audio_waveforms, self.config.window_size, self.config.hop_length, window=window, return_complex=True)
    ...
    def prepare_inputs_for_conv(self, audio_waveforms: list[torch.Tensor]) -> tuple[torch.Tensor, list[int]]:
    ...
    def forward(self, input_features: Union[torch.Tensor, list[torch.Tensor]]) -> list[torch.Tensor]:
