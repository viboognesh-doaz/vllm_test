...
class Exaone4Attention(nn.Module):
    ...
    def __init__(self, config: Exaone4Config, hidden_size: int, num_heads: int, num_kv_heads: int, rope_theta: float=1000000, rope_scaling: Optional[dict[str, Any]]=None, max_position_embeddings: int=8192, quant_config: Optional[QuantizationConfig]=None, bias: bool=False, cache_config: Optional[CacheConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        if quant_config is not None and quant_config.get_name() == 'gguf':
            is_neox_style = False
        self.apply_all_layers = False
        layer_idx = extract_layer_index(prefix)
        interleaved_sliding_window = getattr(config, 'interleaved_sliding_window', 4096)
        sliding_window_pattern = getattr(config, 'sliding_window_pattern', 'LLLG')
        if sliding_window_pattern:
            layer_has_sliding_window = (layer_idx + 1) % sliding_window_pattern.__len__() != 0
        else:
...
class Exaone4DecoderLayer(nn.Module):
    ...
    def __init__(self, config: Exaone4Config, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        self.self_attn = Exaone4Attention(config=config, hidden_size=self.hidden_size, num_heads=config.num_attention_heads, num_kv_heads=getattr(config, 'num_key_value_heads', config.num_attention_heads), rope_theta=rope_theta, rope_scaling=rope_scaling, max_position_embeddings=max_position_embeddings, quant_config=quant_config, bias=attention_bias, cache_config=cache_config, prefix=f'{prefix}.self_attn')
        self.mlp = Exaone4GatedMLP(hidden_size=self.hidden_size, intermediate_size=config.intermediate_size, hidden_act=config.hidden_act, quant_config=quant_config, bias=getattr(config, 'mlp_bias', False), prefix=f'{prefix}.mlp')
...
class Exaone4Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: Exaone4DecoderLayer(config=config, cache_config=cache_config, quant_config=quant_config, prefix=prefix), prefix=f'{prefix}.layers')
