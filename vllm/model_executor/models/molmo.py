...
from .utils import AutoWeightsLoader, WeightsMapper, flatten_bn, is_pp_missing_parameter, make_empty_intermediate_tensors_factory, make_layers, maybe_prefix, merge_multimodal_embeddings
...
from vllm.distributed import get_pp_group, get_tensor_model_parallel_rank, get_tensor_model_parallel_world_size, split_tensor_along_last_dim, tensor_model_parallel_all_gather
...
from vllm.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, PromptIndexTargets, PromptInsertion, PromptUpdate, PromptUpdateDetails
...
class ViTMLP(nn.Module):
    ...
    def __init__(self, config: VisionBackboneConfig, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        self.w1 = ColumnParallelLinear(config.image_emb_dim, config.image_mlp_dim, bias=True, quant_config=quant_config)
        assert config.image_mlp_activations == 'quick_gelu'
        self.act = QuickGELU()
        self.w2 = RowParallelLinear(config.image_mlp_dim, config.image_emb_dim, bias=True, quant_config=quant_config)
...
class MultiHeadDotProductAttention(nn.Module):
    ...
    def __init__(self, config: VisionBackboneConfig, use_bias: bool=True, nlayers: int=1, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        ...
        self.wq = ColumnParallelLinear(nlayers * self.hidden_size, self.total_num_heads * self.head_dim, bias=use_bias, quant_config=quant_config)
        self.wk = ColumnParallelLinear(nlayers * self.hidden_size, self.total_num_kv_heads * self.head_dim, bias=use_bias, quant_config=quant_config)
        self.wv = ColumnParallelLinear(nlayers * self.hidden_size, self.total_num_kv_heads * self.head_dim, bias=use_bias, quant_config=quant_config)
        self.wo = RowParallelLinear(self.total_num_heads * self.head_dim, self.hidden_size, bias=use_bias, quant_config=quant_config)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def __init__(self, config: VisionBackboneConfig, quant_config: Optional[QuantizationConfig]=None):
        ...
        self.attention_norm = nn.LayerNorm(config.image_emb_dim, eps=config.image_norm_eps)
...
class BlockCollection(nn.Module):
    ...
    def __init__(self, config: VisionBackboneConfig, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        self.resblocks = nn.ModuleList([ResidualAttentionBlock(config, quant_config) for _ in range(config.image_num_layers)])
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, config: VisionBackboneConfig, quant_config: Optional[QuantizationConfig]=None):
        ...
        self.patch_embedding = nn.Linear(image_patch_size * image_patch_size * 3, config.image_emb_dim, bias=False)
    ...
    def add_pos_emb(self, x: torch.Tensor, patch_num: int) -> torch.Tensor:
        ...
        if pos_emb.shape[0] != patch_num_0 or pos_emb.shape[1] != patch_num_1:
            ...
            pos_emb = F.interpolate(pos_emb, size=(patch_num_0, patch_num_1), mode='bicubic', align_corners=False, antialias=True)
...
class MolmoAttention(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        self.qkv_proj = QKVParallelLinear(self.hidden_size, self.head_dim, self.total_num_heads, self.total_num_kv_heads, bias=config.qkv_bias, quant_config=quant_config)
...
class LanguageModelMLP(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, input_dim: Optional[int]=None, quant_config: Optional[QuantizationConfig]=None) -> None:
        super().__init__()
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size // 2
        self.gate_up_proj = MergedColumnParallelLinear(input_dim or self.hidden_size, [self.intermediate_size] * 2, bias=False, quant_config=quant_config)
...
class ImageProjectorMLP(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, input_dim: Optional[int]=None, quant_config: Optional[QuantizationConfig]=None) -> None:
        ...
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size // 2
        self.merged_linear = MergedColumnParallelLinear(input_dim or self.hidden_size, [self.intermediate_size] * 2, bias=False, quant_config=quant_config)
...
class MolmoDecoderLayer(nn.Module):
    ...
    def forward(self, positions: torch.Tensor, hidden_states: torch.Tensor, residual: Optional[torch.Tensor]) -> tuple[torch.Tensor, Optional[tuple[torch.Tensor, torch.Tensor]]]:
        if residual is None:
...
class MolmoVisionBackbone(nn.Module, SupportsQuant):
    ...
    def __init__(self, config: PretrainedConfig, vision_config: VisionBackboneConfig, quant_config: Optional[QuantizationConfig]=None) -> None:
        super().__init__()
        self.vit_layers = VIT_LAYERS
        self.image_num_patch = vision_config.image_num_patch
        self.llm_patches_per_crop = ((self.image_num_patch[0] + 1) // POOLING_SIZE, (self.image_num_patch[1] + 1) // POOLING_SIZE)
        ...
        self.image_pooling_2d = MultiHeadDotProductAttention(vision_config, nlayers=len(self.vit_layers), quant_config=quant_config)
        self.image_projector = ImageProjectorMLP(config, input_dim=vision_config.image_emb_dim, quant_config=quant_config)
    ...
    def forward(self, images: torch.Tensor, image_masks: torch.Tensor) -> torch.Tensor:
        ...
        image_features = image_features.reshape((batch_size, num_image) + self.image_num_patch + (-1,))
        if (missing_w := (self.image_num_patch[0] % POOLING_SIZE)):
            image_features = F.pad(image_features, (0, 0, 0, missing_w, 0, missing_w, 0, 0, 0, 0))
        image_features = rearrange(image_features, 'b n (h dh) (w dw) c -> (b n h w) (dh dw) c', dh=POOLING_SIZE, dw=POOLING_SIZE)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        stacked_params_mapping = [('merged_linear', 'gate_proj', 0), ('merged_linear', 'up_proj', 1)]
...
class MolmoModel(nn.Module, SupportsQuant):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.embed_tokens = VocabParallelEmbedding(self.embedding_size, config.hidden_size, quant_config=quant_config)
        decoder_layer = MolmoDecoderNormAfterLayer if config.norm_after else MolmoDecoderLayer
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: decoder_layer(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.layers')
        assert config.layer_norm_type == 'rms'
...
def get_num_patches(num_tiles: int, *, crop_patches: int, left_margin: int, right_margin: int, pooling_size: int) -> int:
    if num_tiles == 1:
    ...
    left_num = _lowest_multiple(crop_window_patches + left_margin + pooling_size - 1, pooling_size)
    middle_num = _lowest_multiple(crop_window_patches + pooling_size - 1, pooling_size)
    right_num = _lowest_multiple(crop_window_patches + right_margin + pooling_size - 1, pooling_size)
    return left_num + (num_tiles - 2) * middle_num + right_num

def get_patches_grid_size(*, tiling_h: int, tiling_w: int, crop_patches: int, left_margin: int, right_margin: int, pooling_size: int) -> tuple[int, int]:
    nrows = get_num_patches(tiling_h, crop_patches=crop_patches, left_margin=left_margin, right_margin=right_margin, pooling_size=pooling_size)
    ncols = get_num_patches(tiling_w, crop_patches=crop_patches, left_margin=left_margin, right_margin=right_margin, pooling_size=pooling_size)
    return (nrows, ncols)
...
def select_tiling(*, height: int, width: int, patch_size: int, max_num_patches: int):
    ...
    if (required_scale < 1).all():
        ...
        ix = np.where(required_scale < 1.0, 10000000000.0, required_scale).argmin()
...
class MolmoProcessorWrapper:
    ...
    def select_tiling(self, *, image_width: int, image_height: int) -> tuple[int, int]:
        ...
        tiling_h, tiling_w = select_tiling(height=image_height - total_margin_pixels, width=image_width - total_margin_pixels, patch_size=crop_window_size, max_num_patches=max_crops)
        return (tiling_w, tiling_h)

    def get_patches_grid_size(self, *, image_width: int, image_height: int) -> tuple[int, int]:
        left_margin, right_margin = self.overlap_margins
        base_image_input_size = self.base_image_input_size
        base_image_input_d = self.image_patch_size
        pooling_size = self.pooling_size
        crop_patches = base_image_input_size[0] // base_image_input_d
        tiling_w, tiling_h = self.select_tiling(image_height=image_height, image_width=image_width)
        nrows, ncols = get_patches_grid_size(tiling_h=tiling_h, tiling_w=tiling_w, crop_patches=crop_patches, left_margin=left_margin, right_margin=right_margin, pooling_size=pooling_size)
        return (ncols, nrows)

    def __call__(self, text: Optional[Union[TextInput, list[TextInput]]]=None, images: Optional[Union[ImageInput, list[ImageInput]]]=None, return_tensors: Optional[Union[str, TensorType]]=None, **kwargs) -> BatchFeature:
        outputs = self.processor.process(text, images, **kwargs)
        if images is None:
            images = []
        ...
        if image_input_idx is not None:
            ...
            tilings = [self.select_tiling(image_width=image.size[0], image_height=image.size[1]) for image in images]
...
class MolmoProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self, *, image_width: int, image_height: int, processor: Optional[MolmoProcessorWrapper]) -> int:
        if processor is None:
            processor = self.get_hf_processor()
        ncols, nrows = processor.get_patches_grid_size(image_width=image_width, image_height=image_height)
...
class MolmoMultiModalProcessor(BaseMultiModalProcessor[MolmoProcessingInfo]):
    ...
    def _apply_hf_processor_tokens_only(self, prompt_tokens: list[int]) -> list[int]:
        ...
        tokens = processor.processor.get_tokens_input(self.info.get_tokenizer().decode(prompt_tokens), message_format=processor.message_format, always_start_with_space=processor.always_start_with_space)
        processed_data = self.info.ctx.call_hf_processor(processor, dict(tokens=tokens))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_insertion_molmo(item_idx: int):
            ...
            ncols, nrows = processor.get_patches_grid_size(image_width=image_size.width, image_height=image_size.height)
            joint_row = [img_patch_id] * ((ncols + 1) // pooling_size) + [img_col_id]
            joint = [img_start_id] + joint_row * ((nrows + 1) // pooling_size) + [img_end_id]
            return PromptUpdateDetails.select_token_id(extra_joint + joint, embed_token_id=img_patch_id)
        return [PromptInsertion(modality='image', target=PromptIndexTargets.prefix('<|endoftext|>'), insertion=get_insertion_molmo)]

@MULTIMODAL_REGISTRY.register_processor(MolmoMultiModalProcessor, info=MolmoProcessingInfo, dummy_inputs=MolmoDummyInputsBuilder)
class MolmoForCausalLM(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA, SupportsQuant):
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_substr={'image_projector.w1.': 'image_projector.gate_proj.', 'image_projector.w3.': 'image_projector.up_proj.', 'image_projector.w2.': 'image_projector.down_proj.', 'att_proj': 'self_attn.qkv_proj', 'attn_out': 'self_attn.o_proj', 'q_norm': 'self_attn.q_norm', 'k_norm': 'self_attn.k_norm', 'ff_proj': 'mlp.gate_up_proj', 'ff_out': 'mlp.down_proj', 'attn_norm': 'input_layernorm', 'ff_norm': 'post_attention_layernorm'}, orig_to_new_prefix={'model.vision_backbone.': 'vision_backbone.', 'model.transformer.blocks.': 'model.layers.', 'model.transformer.ln_f.': 'model.norm.', 'model.transformer.mlp.down_proj.': 'lm_head.'})
    packed_modules_mapping = {'qkv_proj': ['qkv_proj'], 'gate_up_proj': ['gate_up_proj'], 'merged_linear': ['gate_proj', 'up_proj']}
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.model = MolmoModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
        self.img_patch_id = None
        if self.config.weight_tying:
            self.lm_head = self.model.transformer.wte
        else:
            self.lm_head = ParallelLMHead(config.embedding_size or config.vocab_size, config.hidden_size, quant_config=quant_config)
        self.logits_processor = LogitsProcessor(config.embedding_size or config.vocab_size)
        self.make_empty_intermediate_tensors = self.model.make_empty_intermediate_tensors

    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[MolmoImageInputs]:
        images = kwargs.pop('images', None)
        if images is None:
            return None
        if not isinstance(images, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of images. Got type: {type(images)}')
        image_masks = kwargs.pop('image_masks', None)
        if not (image_masks is None or isinstance(image_masks, (torch.Tensor, list))):
            raise ValueError(f'Incorrect type of image_masks. Got type: {type(image_masks)}')
        feat_is_patch = kwargs.pop('feat_is_patch', None)
        if not isinstance(feat_is_patch, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of feat_is_patch. Got type: {type(feat_is_patch)}')
        num_crops = kwargs.pop('num_crops', None)
        if not isinstance(num_crops, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of num_crops. Got type: {type(num_crops)}')
        img_patch_id = kwargs.pop('img_patch_id', None)
        if not isinstance(img_patch_id, torch.Tensor):
            raise ValueError(f'Incorrect type of img_patch_id. Got type: {type(img_patch_id)}')
        self.img_patch_id = img_patch_id.flatten().unique().item()
        num_crops = flatten_bn(num_crops, concat=True)
        return MolmoImageInputs(images=images, image_masks=image_masks, feat_is_patch=feat_is_patch, num_crops=num_crops)

    def _process_image_input(self, image_input: MolmoImageInputs) -> list[torch.Tensor]:
        ...
        image_features_flat = self.vision_backbone(images=images_flat.unsqueeze(0), image_masks=None if image_masks_flat is None else image_masks_flat.unsqueeze(0)).squeeze(0)
        return [feats[f_is_patch] for feats, f_is_patch in zip(image_features_flat.split(num_crops.tolist()), feat_is_patch_flat.split(num_crops.tolist()))]
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            ...
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.img_patch_id)
        return inputs_embeds

    def forward(self, input_ids: torch.LongTensor, positions: torch.LongTensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: object) -> torch.Tensor:
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        """
        return MultiModelKeys.from_string_field(language_model='model', connector='vision_backbone.image_projector', tower_model='vision_backbone')

def _get_weights_with_merged_embedding(weights: Iterable[tuple[str, torch.Tensor]]) -> Iterable[tuple[str, torch.Tensor]]:
    embedding_weights = {}
    for name, weight in weights:
        if 'wte.embedding' in name:
            ...
            yield (name, weight)
    embedding_weights = torch.cat([embedding_weights['embedding'], embedding_weights['new_embedding']], dim=0)
