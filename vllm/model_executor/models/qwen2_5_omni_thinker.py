...
from vllm.model_executor.models.qwen2_5_vl import Qwen2_5_VisionTransformer, Qwen2_5_VLImageEmbeddingInputs, Qwen2_5_VLImageInputs, Qwen2_5_VLImagePixelInputs, Qwen2_5_VLProcessingInfo, Qwen2_5_VLVideoEmbeddingInputs, Qwen2_5_VLVideoInputs, Qwen2_5_VLVideoPixelInputs
...
class Qwen2_5OmniThinkerProcessingInfo(Qwen2AudioProcessingInfo, Qwen2_5_VLProcessingInfo):
    ...
    def get_hf_processor(self, *, sampling_rate: Optional[int]=None, min_pixels: Optional[int]=None, max_pixels: Optional[int]=None, size: Optional[dict[str, int]]=None, fps: Optional[Union[float, list[float]]]=None, **kwargs: object) -> Qwen2_5OmniProcessor:
        if fps is not None:
            kwargs['fps'] = fps
        ...
        processor = self.ctx.get_hf_processor(processor_class, image_processor=self.get_image_processor(min_pixels=min_pixels, max_pixels=max_pixels, size=size, use_fast=kwargs.get('use_fast', True)), **kwargs)
    ...
    def get_feature_extractor(self, *, sampling_rate: Optional[int]=None, **kwargs: object):
...
class Qwen2_5OmniThinkerDummyInputsBuilder(BaseDummyInputsBuilder[Qwen2_5OmniThinkerProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        mm_data = {'audio': self._get_dummy_audios(length=target_audio_length, num_audios=num_audios), 'image': self._get_dummy_images(width=target_width, height=target_height, num_images=num_images), 'video': self._get_dummy_videos(width=target_width, height=target_height, num_frames=target_num_frames, num_videos=num_videos)}
        return mm_data
...
class Qwen2_5OmniThinkerMultiModalProcessor(BaseMultiModalProcessor[Qwen2_5OmniThinkerProcessingInfo]):
    ...
    def _get_data_parser(self) -> MultiModalDataParser:
        feature_extractor = self.info.get_feature_extractor()
        return Qwen2_5OmniThinkerMultiModalDataParser(target_sr=feature_extractor.sampling_rate)
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        hf_inputs = super()._call_hf_processor(prompt=prompt, mm_data=mm_data, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
    ...
    def _maybe_apply_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], prompt_ids: list[int], mm_kwargs: MultiModalKwargs, is_update_applied: bool) -> tuple[list[int], str, Mapping[str, list[PlaceholderFeaturesInfo]]]:
        ...
        if is_update_applied:
            ...
            self._validate_mm_placeholders(mm_placeholders, mm_item_counts, use_audio_in_video=use_audio_in_video)
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, Any], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement_qwen2_use_audio_in_video(item_idx: int):
            ...
            return MRotaryEmbedding.omni_get_updates_use_audio_in_video(thinker_config=thinker_config, audio_len=audio_num_features, video_grid_thw=video_grid_thw, video_second_per_grid_t=video_second_per_grid_t)
        ...
        return [PromptReplacement(modality='audio', target=audio_token, replacement=get_replacement_qwen2_audio), PromptReplacement(modality='image', target=image_token, replacement=partial(get_replacement_qwen2_vision, modality='image')), PromptReplacement(modality='video', target=video_token, replacement=video_replacement_fn)]
    ...
    def _validate_mm_placeholders(self, mm_placeholders: Mapping[str, list[PlaceholderFeaturesInfo]], mm_item_counts: Mapping[str, int], use_audio_in_video: bool=False) -> None:
        if use_audio_in_video:
...
class Qwen2_5OmniConditionalGenerationMixin:
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> Optional[Qwen2AudioInputs]:
        ...
        return Qwen2AudioInputs(input_features=input_audio_features, audio_feature_lengths=audio_feature_lengths, feature_attention_mask=feature_attention_mask)

    def _parse_and_validate_image_input(self, **kwargs: dict[str, Any]) -> Optional[Qwen2_5_VLImageInputs]:
    ...
    def _parse_and_validate_video_input(self, **kwargs: dict[str, Any]) -> Optional[Qwen2_5_VLVideoInputs]:
        ...
        if pixel_values_videos is not None:
            ...
            return Qwen2_5_VLVideoPixelInputs(type='pixel_values_videos', pixel_values_videos=pixel_values_videos, video_grid_thw=video_grid_thw)
    ...
    def _process_audio_input(self, audio_input: Qwen2AudioInputs, audio_hashes: list[str]=None, cached_audio_features: torch.Tensor=None) -> torch.Tensor:
        ...
        audio_outputs = self.audio_tower(input_features.to(self.audio_tower.dtype), feature_lens=audio_feature_lengths, aftercnn_lens=audio_feat_lengths)
    ...
    def _process_video_input(self, video_input: Qwen2_5_VLVideoInputs, video_hashes: list[str]=None, cached_video_embeds: torch.Tensor=None) -> torch.Tensor:
...
@MULTIMODAL_REGISTRY.register_processor(Qwen2_5OmniThinkerMultiModalProcessor, info=Qwen2_5OmniThinkerProcessingInfo, dummy_inputs=Qwen2_5OmniThinkerDummyInputsBuilder)
class Qwen2_5OmniThinkerForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, Qwen2_5OmniConditionalGenerationMixin):
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_prefix={'thinker.lm_head.': 'language_model.lm_head.', 'thinker.model.': 'language_model.model.', 'thinker.': ''})
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        super().__init__()
        thinker_config: Qwen2_5OmniThinkerConfig = vllm_config.model_config.hf_config.thinker_config
        ...
        if flash_attn is not None:
            ...
            logger.warning('flash_attn is not available, the model may not yield the exactly same result as the transformers implementation in the audio tower part.')
        self.audio_tower = Qwen2_5OmniAudioEncoder(thinker_config.audio_config)
        self.visual = Qwen2_5_VisionTransformer(vision_config=thinker_config.vision_config, norm_eps=getattr(thinker_config.text_config, 'rms_norm_eps', 1e-06), quant_config=quant_config, prefix=maybe_prefix(prefix, 'visual'))
        self.quant_config = quant_config
        self.language_model = init_vllm_registered_model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'language_model'), hf_config=thinker_config.text_config, architectures=['Qwen2ForCausalLM'])
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, [self.config.image_token_index, self.config.video_token_index, self.config.audio_token_index])
    ...
    def get_input_embeddings_v0(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[NestedTensors]=None) -> torch.Tensor:
        inputs_embeds = self.language_model.get_input_embeddings(input_ids)
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: object) -> Union[torch.Tensor, IntermediateTensors]:
        if intermediate_tensors is not None:
            ...
            inputs_embeds = self.get_input_embeddings_v0(input_ids, multimodal_embeddings)
            input_ids = None
