...
class SambaYAttention(nn.Module):
    ...
    def __init__(self, config, layer_idx: Optional[int]=None, yoco_cross: bool=False, cache_config: Optional[CacheConfig]=None, prefix: str=''):
        super().__init__()
        ...
        self.subln = nn.RMSNorm(2 * self.head_dim, eps=1e-05, elementwise_affine=True)
        params = {'differential_flash_attention_config': {'lambda_init': self.lambda_init, 'lambda_q1': self.lambda_q1, 'lambda_k1': self.lambda_k1, 'lambda_q2': self.lambda_q2, 'lambda_k2': self.lambda_k2, 'subln': self.subln}}
        if yoco_cross:
            kv_shared_layer_index = config.num_hidden_layers // 2 + 1
            kv_sharing_target_layer_name = f'model.layers.{kv_shared_layer_index}.self_attn.attn'
        else:
            kv_sharing_target_layer_name = None
        self.attn = Attention(self.num_heads, self.head_dim, self.head_dim ** (-0.5), num_kv_heads=self.num_key_value_heads, cache_config=cache_config, per_layer_sliding_window=sliding_window, prefix=f'{prefix}.attn', attn_type=AttentionType.DECODER, kv_sharing_target_layer_name=kv_sharing_target_layer_name, **params)
        assert self.attn.backend == _Backend.DIFFERENTIAL_FLASH_ATTN, 'DIFFERENTIAL_FLASH_ATTN required'
...
class Phi4Mamba(nn.Module):
    ...
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2, dt_rank='auto', dt_min=0.001, dt_max=0.1, dt_init='random', dt_scale=1.0, dt_init_floor=0.0001, conv_bias=True, bias=False, use_fast_path=True, layer_idx=None, device=None, dtype=None, yoco_cross=False, yoco_kv=False):
        factory_kwargs = {'params_dtype': dtype}
        super().__init__()
        self.yoco_cross = yoco_cross
        ...
        if self.yoco_cross:
            ...
            return
        self.conv1d = ColumnParallelLinear(input_size=d_conv, output_size=self.d_inner, bias=conv_bias, params_dtype=dtype)
        self.conv1d.weight.data = self.conv1d.weight.data.unsqueeze(1)
        self.in_proj = MergedColumnParallelLinear(self.d_model, [self.d_inner] * 2, bias=bias, params_dtype=dtype)
        self.x_proj = RowParallelLinear(self.d_inner, self.dt_rank + self.d_state * 2, bias=False, params_dtype=dtype)
        self.dt_proj = ColumnParallelLinear(self.dt_rank, self.d_inner, bias=True, skip_bias_add=True, params_dtype=dtype)
        self.A = nn.Parameter(torch.empty(self.d_inner, self.d_state, dtype=torch.float32))
        self.D = nn.Parameter(torch.ones(self.d_inner, dtype=torch.float32))
        self.out_proj = RowParallelLinear(self.d_inner, self.d_model, bias=bias, input_is_parallel=True, params_dtype=dtype)
        self.activation = 'silu'

    def forward(self, hidden_states: torch.Tensor, attn_metadata: AttentionMetadata, mamba_cache_params: MambaCacheParams, yoco_key_values=None) -> torch.Tensor:
        if self.yoco_cross:
        ...
        time_step, B, C = torch.split(ssm_parameters, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        discrete_time_step = self.dt_proj(time_step)[0].transpose(-2, -1)
        time_proj_bias = self.dt_proj.bias.float() if hasattr(self.dt_proj, 'bias') else None
        if attn_metadata.query_start_loc is not None and attn_metadata.context_lens_tensor is not None:
            scan_outputs = selective_scan_fn(hidden_states, mamba_cache_params.ssm_state, discrete_time_step, self.A, B.transpose(-2, -1), C.transpose(-2, -1), self.D.float(), None if self.yoco_kv else gate, time_proj_bias, delta_softplus=True, cache_indices=mamba_cache_params.state_indices_tensor, has_initial_state=attn_metadata.context_lens_tensor > 0, query_start_loc=attn_metadata.query_start_loc)
        else:
            scan_outputs = selective_state_update(mamba_cache_params.ssm_state, hidden_states.transpose(0, 1), discrete_time_step.transpose(0, 1), self.A, B, C, self.D, None if self.yoco_kv else gate.transpose(0, 1), time_proj_bias, dt_softplus=True, state_batch_indices=mamba_cache_params.state_indices_tensor)
...
class SambaYDecoderLayer(nn.Module):
    ...
    def __init__(self, config, layer_idx, cache_config, prefix: str='') -> None:
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        ...
        if self.use_mamba:
            ...
            self.attn = Phi4Mamba(config.hidden_size, layer_idx=layer_idx, yoco_cross=self.yoco_cross, yoco_kv=self.yoco_mb, **factory_kwargs)
        else:
    ...
    def forward(self, hidden_states: torch.Tensor, positions: torch.Tensor, attn_metadata: AttentionMetadata, mamba_cache_params: MambaCacheParams, ssm_output: Optional[torch.LongTensor]=None) -> Union[torch.Tensor, IntermediateTensors]:
        if self.use_mamba:
        ...
        if self.use_mamba:
            attn_outputs, ssm_output = self.attn(hidden_states, attn_metadata, mamba_cache_params, yoco_key_values=ssm_output)
...
class SambaYModel(nn.Module):
    ...
    def __init__(self, config, cache_config=None, quant_config=None, lora_config=None, prefix: str='') -> None:
        super().__init__()
        self.config = config
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: SambaYDecoderLayer(config, int(prefix.split('.')[-1]), cache_config, prefix=prefix), prefix=f'{prefix}.layers')
        self.final_layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)

    def get_input_embeddings(self, input_ids: torch.Tensor) -> torch.Tensor:
        return self.embed_tokens(input_ids)

    def forward(self, input_ids: Optional[torch.Tensor], positions: torch.Tensor, attn_metadata: AttentionMetadata, mamba_cache_params: MambaCacheParams, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None) -> Union[torch.Tensor, IntermediateTensors]:
        ...
        for i in range(self.start_layer, self.end_layer):
            ...
            if layer.use_mamba:
                if i < self.config.num_hidden_layers // 2 or not layer.yoco_cross:
                    mamba_cache = mamba_cache_params.at_layer_idx(mamba_state_idx)
                    mamba_state_idx += 1
                else:
                    mamba_cache = mamba_cache_params.at_layer_idx(mamba_state_idx - 1)
                hidden_states, ssm_output = layer(hidden_states, positions, attn_metadata, mamba_cache, ssm_output=ssm_output)
            else:
                hidden_states, ssm_output = layer(hidden_states, positions, attn_metadata, None, ssm_output=ssm_output)
...
class Phi4FlashForCausalLM(nn.Module, HasInnerState, IsHybrid, SupportsV0Only):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        assert not cache_config.enable_prefix_caching, 'Phi4flash currently does not support prefix caching'
        assert not scheduler_config.chunked_prefill_enabled, 'Phi4Flash currently does not support prefix caching'
        super().__init__()
        self.config = config
        self.model_config = vllm_config.model_config
        self.scheduler_config = scheduler_config
        self.model = SambaYModel(config, cache_config=cache_config, prefix=maybe_prefix(prefix, 'model'))
        ...
        self.logits_processor = LogitsProcessor(self.unpadded_vocab_size, config.vocab_size, logits_as_input=False)
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs) -> Union[torch.Tensor, IntermediateTensors]:
        ...
        hidden_states = self.model(input_ids, positions, attn_metadata, mamba_cache_params, intermediate_tensors, inputs_embeds)
        return hidden_states
    ...
    def _get_mamba_cache_shape(self) -> tuple[Optional[tuple[int, int]], Optional[tuple[int, int]]]:
        ...
        conv_state_shape = (mamba_expand * hidden_size // world_size, mamba_d_conv - 1)
        temporal_state_shape = (mamba_expand * hidden_size // world_size, mamba_d_state)
    ...
    def compute_logits(self, hidden_states: torch.Tensor, sampling_metadata: SamplingMetadata) -> Optional[torch.Tensor]:
        ...
        processed_logits = self.logits_processor(self.lm_head, hidden_states, sampling_metadata, self.embedding_bias, prune_hidden_states=prune_hidden_states)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        for name, param in self.named_parameters():
            ...
            if weight is not None and weight.shape != param.shape:
                logger.warning('Shape mismatch: %s %s %s', name, weight.shape, param.shape)
