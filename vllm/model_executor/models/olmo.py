...
class OlmoAttention(nn.Module):
    ...
    def __init__(self, config: OlmoConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.config = config
        ...
        self.qkv_proj = QKVParallelLinear(self.hidden_size, self.head_dim, self.total_num_heads, bias=config.attention_bias, quant_config=quant_config)
        ...
        self.o_proj = RowParallelLinear(self.hidden_size, self.hidden_size, bias=config.attention_bias, quant_config=quant_config)
...
class OlmoDecoderLayer(nn.Module):
    ...
    def forward(self, positions: torch.Tensor, hidden_states: torch.Tensor) -> tuple[torch.Tensor, Optional[tuple[torch.Tensor, torch.Tensor]]]:
...
class OlmoModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: OlmoDecoderLayer(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.layers')
...
class OlmoForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        quant_config = vllm_config.quant_config
        self.config = config
        self.model = OlmoModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
