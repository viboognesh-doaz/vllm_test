...
class PaliGemmaMultiModalProcessor(BaseMultiModalProcessor[PaliGemmaProcessingInfo]):
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_insertion(item_idx: int):
            ...
            return PromptUpdateDetails.select_token_id(image_tokens + [bos_token_id], embed_token_id=image_token_id)
        return [PromptInsertion(modality='image', target=PromptIndexTargets.prefix([bos_token_id] if tokenizer.add_bos_token else []), insertion=get_insertion)]
    ...
    def apply(self, prompt: Union[str, list[int]], mm_data: MultiModalDataDict, hf_processor_mm_kwargs: Mapping[str, object], tokenization_kwargs: Optional[Mapping[str, object]]=None, return_mm_hashes: bool=False) -> MultiModalInputs:
        ...
        tokenizer = self.info.get_tokenizer()
        newline_prompt = '\n'
        newline_token_id = tokenizer.encode(newline_prompt)[-1]
...
@MULTIMODAL_REGISTRY.register_processor(PaliGemmaMultiModalProcessor, info=PaliGemmaProcessingInfo, dummy_inputs=PaliGemmaDummyInputsBuilder)
class PaliGemmaForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.multi_modal_projector = PaliGemmaMultiModalProjector(vision_hidden_size=config.vision_config.hidden_size, projection_dim=config.vision_config.projection_dim)
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[PaliGemmaImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return PaliGemmaImagePixelInputs(type='pixel_values', data=self._validate_pixel_values(pixel_values))
