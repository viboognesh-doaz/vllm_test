...
class VisualAttention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, bias: bool=True, kdim: Optional[int]=None, vdim: Optional[int]=None):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        query_layer = query_layer.view(sq, b * self.num_attention_heads_per_partition, self.hidden_size_per_attention_head).transpose(0, 1)
        key_layer = key_layer.view(sq, b * self.num_attention_heads_per_partition, self.hidden_size_per_attention_head).transpose(0, 1)
        ...
        value_layer = value_layer.view(sq, b * self.num_attention_heads_per_partition, self.hidden_size_per_attention_head).transpose(0, 1)
...
class QwenVLMLP(nn.Module):
    ...
    def __init__(self, hidden_size: int, intermediate_size: int, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
...
class VisualAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, mlp_ratio: float=4.0, norm_layer: Callable[[int], nn.Module]=nn.LayerNorm, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        self.ln_1 = norm_layer(d_model)
        self.ln_2 = norm_layer(d_model)
        mlp_width = int(d_model * mlp_ratio)
        self.attn = VisualAttention(d_model, n_head)
        self.mlp = QwenVLMLP(hidden_size=d_model, intermediate_size=mlp_width, quant_config=quant_config)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, width: int, layers: int, heads: int, mlp_ratio: float=4.0, norm_layer: Callable[[int], nn.Module]=nn.LayerNorm, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.ModuleList([VisualAttentionBlock(width, heads, mlp_ratio, norm_layer=norm_layer, quant_config=quant_config) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, image_size: int, patch_size: int, width: int, layers: int, heads: int, mlp_ratio: float, n_queries: int=256, output_dim: int=512, image_start_id: int=151857, quant_config: Optional[QuantizationConfig]=None, **kwargs):
        super().__init__()
        ...
        self.transformer = TransformerBlock(width, layers, heads, mlp_ratio, norm_layer=norm_layer, quant_config=quant_config)
        self.attn_pool = Resampler2(grid_size=int(math.sqrt(n_queries)), embed_dim=output_dim, num_heads=output_dim // 128, kv_dim=width, norm_layer=norm_layer, adaptive=False, do_post_projection=False).to(device=self.positional_embedding.device, dtype=self.positional_embedding.dtype)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x.to(dtype=self.transformer.get_cast_dtype(), device=self.transformer.get_cast_device())
        x = self.conv1(x)
...
@lru_cache(maxsize=1)
def _get_tokenizer_without_image_pad(tokenizer: PreTrainedTokenizer) -> PreTrainedTokenizer:
    """
    ...
    class TokenizerWithoutImagePad(tokenizer.__class__):

        def tokenize(self, text: str, allowed_special: Union[Set[str], str]='all', disallowed_special: Union[Collection[str], str]=(), **kwargs) -> list[Union[bytes, str]]:
            text = unicodedata.normalize('NFC', text)
            return [self.decoder[t] for t in self.tokenizer.encode(text, allowed_special=allowed_special, disallowed_special=disallowed_special)]

        def _decode(self, token_ids: Union[int, list[int]], skip_special_tokens: bool=False, errors: Optional[str]=None, **kwargs) -> str:
...
class QwenVLProcessor:
    ...
    def __init__(self, config: PretrainedConfig, tokenizer: PreTrainedTokenizer) -> None:
        super().__init__()
        self.config = config
        self.tokenizer = tokenizer
        vision_config = config.visual
        image_size = vision_config['image_size']
        self.image_transform = transforms.Compose([transforms.Resize((image_size, image_size), interpolation=InterpolationMode.BICUBIC), transforms.ToTensor(), transforms.Normalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711))])
    ...
    def __call__(self, text: Optional[Union[TextInput, list[TextInput]]]=None, images: Optional[Union[ImageInput, list[ImageInput]]]=None, return_tensors: Optional[Union[str, TensorType]]=None) -> BatchFeature:
        if text is None:
            text = []
...
class QwenVLProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, **kwargs: object) -> QwenVLProcessor:
        return self.ctx.init_processor(QwenVLProcessor, config=self.get_hf_config(), tokenizer=self.get_tokenizer(), **kwargs)
...
class QwenVLMultiModalProcessor(BaseMultiModalProcessor[QwenVLProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        prompt, num_matched_images = re.subn('(Picture \\d*: <img>).*?(<\\/img>\\n)', '\\1\\2', prompt)
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        special_tokens: dict[str, int] = tokenizer.special_tokens
        ...
        return [PromptReplacement(modality='image', target=[img_start_id, img_end_id], replacement=PromptUpdateDetails.select_token_id([img_start_id] + image_tokens + [img_end_id], embed_token_id=img_pad_id))]

@MULTIMODAL_REGISTRY.register_processor(QwenVLMultiModalProcessor, info=QwenVLProcessingInfo, dummy_inputs=QwenVLDummyInputsBuilder)
class QwenVLForConditionalGeneration(QWenBaseModel, SupportsPP, SupportsLoRA, SupportsMultiModal):
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        """
        return MultiModelKeys.from_string_field(language_model='transformer.h', connector='transformer.visual.attn_pool', tower_model='transformer.visual.transformer')
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='', transformer_type: type[QwenVLModel]=QwenVLModel) -> None:
        super().__init__(vllm_config=vllm_config, prefix=prefix, transformer_type=transformer_type)
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[QwenImageInputs]:
        pixel_values = kwargs.pop('pixel_values', None)
        image_embeds = kwargs.pop('image_embeds', None)
        if pixel_values is not None:
            if not isinstance(pixel_values, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of pixel values. Got type: {type(pixel_values)}')
            return QwenImagePixelInputs(type='pixel_values', data=self._validate_pixel_values(flatten_bn(pixel_values, concat=True)))
        if image_embeds is not None:
            if not isinstance(image_embeds, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of image embeddings. Got type: {type(image_embeds)}')
            return QwenImageEmbeddingInputs(type='image_embeds', data=flatten_bn(image_embeds, concat=True))
        return None
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.transformer.visual.image_pad_id)
