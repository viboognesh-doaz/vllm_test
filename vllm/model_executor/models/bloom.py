...
class BloomAttention(nn.Module):
    ...
    def __init__(self, config: BloomConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        ...
        self.dense = RowParallelLinear(self.hidden_size, self.hidden_size, bias=True, quant_config=quant_config)
        ...
        self.attn = Attention(self.num_heads, self.head_dim, scaling, alibi_slopes=alibi_slopes, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.attn')
...
class BloomMLP(nn.Module):
    ...
    def __init__(self, config: BloomConfig, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        hidden_size = config.hidden_size
        self.dense_h_to_4h = ColumnParallelLinear(hidden_size, 4 * hidden_size, quant_config=quant_config)
        self.gelu_impl = get_act_fn('gelu')
        self.dense_4h_to_h = RowParallelLinear(4 * hidden_size, hidden_size, quant_config=quant_config)
...
class BloomBlock(nn.Module):
    ...
    def forward(self, position_ids: torch.Tensor, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        attention_output = self.self_attention(position_ids=position_ids, hidden_states=layernorm_output)
...
class BloomModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.h = make_layers(config.num_hidden_layers, lambda prefix: BloomBlock(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.h')
...
class BloomForCausalLM(nn.Module, SupportsPP, SupportsV0Only, SupportsQuant):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.transformer = BloomModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'transformer'))
...
def _add_transformer_prefix(weights: Iterable[tuple[str, torch.Tensor]]) -> Iterable[tuple[str, torch.Tensor]]:
