...
@cache
def _get_gemma_act_fn(hidden_act: Optional[str], hidden_activation: Optional[str]) -> nn.Module:
...
class GemmaMLP(nn.Module):
    ...
    def __init__(self, hidden_size: int, intermediate_size: int, hidden_act: Optional[str]=None, hidden_activation: Optional[str]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
...
class GemmaAttention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_position_embeddings: int=8192, rope_theta: float=10000, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
...
class GemmaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: GemmaConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = GemmaAttention(hidden_size=self.hidden_size, num_heads=config.num_attention_heads, num_kv_heads=config.num_key_value_heads, head_dim=config.head_dim, max_position_embeddings=config.max_position_embeddings, rope_theta=config.rope_theta, cache_config=cache_config, quant_config=quant_config, prefix=f'{prefix}.self_attn')
        self.mlp = GemmaMLP(hidden_size=self.hidden_size, intermediate_size=config.intermediate_size, hidden_act=config.hidden_act, hidden_activation=getattr(config, 'hidden_activation', None), quant_config=quant_config, prefix=f'{prefix}.mlp')
...
class GemmaModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: GemmaDecoderLayer(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.layers')
...
class GemmaForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.model = GemmaModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
