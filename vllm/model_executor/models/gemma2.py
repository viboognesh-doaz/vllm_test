...
class Gemma2Attention(nn.Module):
    ...
    def __init__(self, config: Gemma2Config, hidden_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_position_embeddings: int, rope_theta: float, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, attn_logits_soft_cap: Optional[float]=None, prefix: str='') -> None:
        super().__init__()
        self.config = config
        ...
        use_sliding_window = layer_idx % 2 == 0 and getattr(config, 'interleaved_sliding_window', None) is not None
        sliding_window = config.interleaved_sliding_window if use_sliding_window else None
        self.attn = Attention(self.num_heads, self.head_dim, self.scaling, num_kv_heads=self.num_kv_heads, cache_config=cache_config, quant_config=quant_config, logits_soft_cap=attn_logits_soft_cap, per_layer_sliding_window=sliding_window, prefix=f'{prefix}.attn')
...
class Gemma2DecoderLayer(nn.Module):
    ...
    def __init__(self, config: Gemma2Config, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Gemma2Attention(config=config, hidden_size=self.hidden_size, num_heads=config.num_attention_heads, num_kv_heads=config.num_key_value_heads, head_dim=config.head_dim, max_position_embeddings=config.max_position_embeddings, rope_theta=config.rope_theta, cache_config=cache_config, quant_config=quant_config, attn_logits_soft_cap=config.attn_logit_softcapping, prefix=f'{prefix}.self_attn')
        self.hidden_size = config.hidden_size
...
class Gemma2Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: Gemma2DecoderLayer(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.layers')
...
class Gemma2ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.quant_config = quant_config
        self.model = Gemma2Model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
