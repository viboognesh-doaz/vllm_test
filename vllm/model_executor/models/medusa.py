...
class Medusa(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='') -> None:
        ...
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList([ResidualBlock(config=config, hidden_size=self.config.hidden_size, num_layers=self.config.num_hidden_layers) for _ in range(self.config.num_heads)])
        self.orig_vocab_size = config.vocab_size
        self.truncated_vocab_size = config.truncated_vocab_size
        self.unpadded_vocab_size = self.truncated_vocab_size
        if getattr(config, 'original_lm_head', False):
            self.lm_head = ParallelLMHead(self.unpadded_vocab_size, config.hidden_size, org_num_embeddings=self.truncated_vocab_size, padding_size=DEFAULT_VOCAB_PADDING_SIZE)
            self.lm_heads = [self.lm_head for _ in range(self.config.num_heads)]
        else:
            self.lm_heads = nn.ModuleList([ParallelLMHead(self.unpadded_vocab_size, config.hidden_size, org_num_embeddings=self.truncated_vocab_size, padding_size=DEFAULT_VOCAB_PADDING_SIZE) for _ in range(self.config.num_heads)])
    ...
    def compute_logits(self, hidden_states: list[torch.Tensor], sampling_metadata: SamplingMetadata) -> list[torch.Tensor]:
        ...
        for hs, lm_head in zip(hidden_states, self.lm_heads):
            ...
            if self.token_map is None:
                ...
                logits_lst.append(-torch.inf * torch.ones(size=(*_logits.shape[:-1], self.orig_vocab_size), device=_logits.device, dtype=_logits.dtype))
                logits_lst[-1][..., self.token_map] = _logits
        return logits_lst

    def sample(self, logits: list[torch.Tensor], sampling_metadata: SamplingMetadata) -> list[SamplerOutput]:
        ...
        for idx in range(len(sampling_metadata.seq_groups)):
            outputs.append(SamplerOutput(outputs=None, sampled_token_probs=token_prob_list[idx].squeeze(1), logprobs=token_logprob_list[idx].squeeze(1), sampled_token_ids=token_id_list[idx].squeeze(1)))
        return outputs

    def generate_proposals(self, previous_hidden_states: torch.Tensor, sampling_metadata: SamplingMetadata) -> Optional[list[SamplerOutput]]:
        if previous_hidden_states.size(0) == 0:
            return None
        return self.sample(logits=self.compute_logits(hidden_states=self.forward(previous_hidden_states), sampling_metadata=sampling_metadata), sampling_metadata=sampling_metadata)
