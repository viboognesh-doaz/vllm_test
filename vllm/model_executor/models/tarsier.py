...
from transformers.processing_utils import ProcessingKwargs, Unpack, _validate_images_text_input_order
...
from vllm.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, ProcessingCache, PromptReplacement, PromptUpdate
...
class TarsierProcessor(LlavaProcessor):
    ...
    def __call__(self, images: ImageInput=None, text: Union[TextInput, PreTokenizedInput, list[TextInput], list[PreTokenizedInput]]=None, audio=None, videos=None, **kwargs: Unpack[TarsierProcessorKwargs]) -> BatchFeature:
        if images is None and text is None:
            raise ValueError('You have to specify at least one of `images` or `text`.')
        images, text = _validate_images_text_input_order(images, text)
        output_kwargs = self._merge_kwargs(TarsierProcessorKwargs, tokenizer_init_kwargs=self.tokenizer.init_kwargs, **kwargs)
        ...
        if image_inputs.get('pixel_values') is not None:
            ...
            num_image_tokens = height // self.patch_size * (width // self.patch_size + 1) + self.num_additional_image_tokens + 1
...
class TarsierProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self, *, image_width: int, image_height: int) -> int:
        ...
        num_projected_patches = self._apply_feature_select_strategy(hf_config.vision_feature_select_strategy, vision_encoder_info.get_num_image_tokens(image_width=image_width, image_height=image_height))
        if num_projected_patches <= 0:
            default_size = self.get_image_size_with_most_features()
            num_projected_patches_default = self._apply_feature_select_strategy(hf_config.vision_feature_select_strategy, vision_encoder_info.get_num_image_tokens(image_width=default_size.width, image_height=default_size.height))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement(item_idx: int):
            ...
            if isinstance(images, ImageEmbeddingItems):
                ...
                num_final_image_tokens = self.info.get_num_image_tokens(image_width=image_size.width, image_height=image_size.height)
...
def _build_tarsier_hf_processor(info: _I_Tarsier, dummy_inputs: BaseDummyInputsBuilder[_I_Tarsier], *, cache: Optional[ProcessingCache]=None) -> BaseMultiModalProcessor:
    if isinstance(info, TarsierProcessingInfo):
        return TarsierMultiModalProcessor(info, dummy_inputs, cache=cache)
    raise NotImplementedError(type(info))

def init_vision_tower_for_tarsier(hf_config: TarsierHfConfig, quant_config: Optional[QuantizationConfig], *, require_post_norm: Optional[bool]=None, prefix: str='') -> Union[CLIPVisionModel, SiglipVisionModel]:
    ...
    if isinstance(vision_config, CLIPVisionConfig):
        return CLIPVisionModel(vision_config, quant_config=quant_config, num_hidden_layers_override=num_hidden_layers_to_init, require_post_norm=require_post_norm, prefix=prefix)
    elif isinstance(vision_config, SiglipVisionConfig):
        return SiglipVisionModel(vision_config, quant_config=quant_config, num_hidden_layers_override=num_hidden_layers_to_init, require_post_norm=require_post_norm, prefix=prefix)
    msg = f'Unsupported vision config for Tarsier: {type(vision_config)}'
    raise NotImplementedError(msg)

@MULTIMODAL_REGISTRY.register_processor(_build_tarsier_hf_processor, info=_build_tarsier_hf_info, dummy_inputs=TarsierDummyInputsBuilder)
class TarsierForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='') -> None:
        ...
        self.config = config
        self.vision_tower = init_vision_tower_for_tarsier(config, quant_config, require_post_norm=False, prefix=maybe_prefix(prefix, 'vision_tower'))
        projector_bias = getattr(config, 'multimodal_projector_bias', True)
        self.multi_modal_projector = TarsierMultiModalProjector(vision_hidden_size=config.vision_config.hidden_size, text_hidden_size=config.text_config.hidden_size, projector_hidden_act=config.projector_hidden_act, multimodal_projector_bias=projector_bias, quant_config=quant_config, prefix=maybe_prefix(prefix, 'multi_modal_projector'))
        ...
        self.register_buffer('image_newline_idx_tensor', torch.tensor([config.image_newline_idx], dtype=torch.long), persistent=False)
        self.register_buffer('image_new_idx_tensor', torch.tensor([config.image_new_idx], dtype=torch.long), persistent=False)
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[TarsierImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return TarsierImagePixelInputs(type='pixel_values', pixel_values=self._validate_pixel_values(flatten_bn(pixel_values, concat=True)))
        if image_embeds is not None:
            if not isinstance(image_embeds, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of image embeddings. Got type: {type(image_embeds)}')
            return TarsierImageEmbeddingInputs(type='image_embeds', data=flatten_bn(image_embeds, concat=True))
    ...
    def _image_pixels_to_features(self, vision_tower: Union[CLIPVisionModel, SiglipVisionModel], pixel_values: Union[torch.Tensor, list[torch.Tensor]]) -> Union[torch.Tensor, tuple[torch.Tensor, ...]]:
        image_hidden_states = vision_tower(pixel_values)
        if not isinstance(image_hidden_states, torch.Tensor):
            raise TypeError(f'image_hidden_states type: {type(image_hidden_states)} is not supported')
        ...
        selected_features = cast(Union[torch.Tensor, tuple[torch.Tensor, ...]], json_map_leaves(select_features_fn, image_hidden_states))
    ...
    def _add_tarsier_split_tokens(self, projected_image_features: torch.Tensor) -> torch.Tensor:
        ...
        try:
            current_image_features_grid = projected_image_features.view(num_images, num_height_patches, num_width_patches, embed_dim)
        except RuntimeError as e:
            raise RuntimeError(f'Cannot reshape projected_image_features with shape {projected_image_features.shape} to ({num_images}, {num_height_patches}, {num_width_patches}, {embed_dim}). Ensure num_projected_patches is compatible with a grid structure. num_projected_patches={num_projected_patches}, derived num_height_patches={num_height_patches}. ') from e
        image_newline_expanded = image_newline_emb.expand((num_images, num_height_patches, 1, embed_dim))
        features_with_newlines = torch.cat([current_image_features_grid, image_newline_expanded], dim=2)
        ...
        final_image_features = torch.cat([features_with_newlines_flat, image_new_expanded], dim=1)
        return final_image_features

    def _process_image_pixels(self, inputs: TarsierImagePixelInputs) -> Union[torch.Tensor, tuple[torch.Tensor, ...]]:
        assert self.vision_tower is not None
        pixel_values = inputs['pixel_values']
        image_features_selected = self._image_pixels_to_features(self.vision_tower, pixel_values)
    ...
    def _process_image_input(self, image_input: TarsierImageInputs) -> Union[torch.Tensor, tuple[torch.Tensor, ...]]:
