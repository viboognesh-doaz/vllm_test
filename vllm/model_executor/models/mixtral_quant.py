...
class MixtralMoE(nn.Module):
    ...
    def __init__(self, config: MixtralConfig, quant_config: Optional[QuantizationConfig]=None):
        ...
        self.expert_indices = np.array_split(range(self.num_total_experts), self.tp_size)[self.rank].tolist()
        if not self.expert_indices:
            raise ValueError(f'Rank {self.rank} has no experts assigned to it.')
        self.experts = nn.ModuleList([MixtralMLP(self.num_total_experts, config.hidden_size, config.intermediate_size, quant_config=quant_config) if idx in self.expert_indices else None for idx in range(self.num_total_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        return tensor_model_parallel_all_reduce(final_hidden_states).view(num_tokens, hidden_dim)

class MixtralAttention(nn.Module):

    def __init__(self, config: MixtralConfig, hidden_size: int, num_heads: int, num_kv_heads: int, max_position: int=4096 * 32, rope_theta: float=10000, quant_config: Optional[QuantizationConfig]=None, cache_config: Optional[CacheConfig]=None, prefix: str='') -> None:
        super().__init__()
