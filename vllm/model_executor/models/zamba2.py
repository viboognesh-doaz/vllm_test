...
class Zamba2ForCausalLM(nn.Module, HasInnerState, IsHybrid):
    ...
    """
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_substr={'A_log': 'A', '0.weight': 'A.weight', '1.weight': 'B.weight'})
    ...
    def get_mamba_state_shape_from_config(cls, vllm_config: 'VllmConfig', use_v1: bool=True) -> tuple[tuple[int, int], tuple[int, int, int]]:
        ...
        return get_mamba_state_shape(intermediate_size=intermediate_size, tp_world_size=parallel_config.tensor_parallel_size, n_groups=hf_config.mamba_ngroups, num_heads=hf_config.n_mamba_heads, head_dim=hf_config.mamba_headdim, state_size=hf_config.mamba_d_state, conv_kernel=hf_config.mamba_d_conv, use_v1=use_v1)
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='') -> None:
        ...
        self.model = Zamba2Model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
