...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, num_kv_heads: int, max_position: int=4096 * 32, rope_theta: float=10000, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, rope_scaling: Optional[tuple]=None, prefix: str='', attn_type: str=AttentionType.DECODER, dual_chunk_attention_config: Optional[dict[str, Any]]=None) -> None:
        ...
        self.rotary_emb = get_rope(self.head_dim, rotary_dim=self.head_dim, max_position=max_position, base=self.rope_theta, rope_scaling=rope_scaling, dual_chunk_attention_config=dual_chunk_attention_config)
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        self.mlp = Qwen2MLP(hidden_size=self.hidden_size, intermediate_size=config.intermediate_size, hidden_act=config.hidden_act, quant_config=quant_config, prefix=f'{prefix}.mlp')
...
class Qwen2Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='', decoder_layer_type: type[nn.Module]=Qwen2DecoderLayer):
        super().__init__()
        config = vllm_config.model_config.hf_config
        cache_config = vllm_config.cache_config
        quant_config = vllm_config.quant_config
        if cache_config.sliding_window is not None and hasattr(config, 'max_window_layers'):
            assert config.max_window_layers == config.num_hidden_layers, 'Sliding window for some but all layers is not supported. This model uses sliding window but `max_window_layers` = {} is less than `num_hidden_layers` = {}. Please open an issue to discuss this feature.'.format(config.max_window_layers, config.num_hidden_layers)
        self.config = config
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: decoder_layer_type(config=config, cache_config=cache_config, quant_config=quant_config, prefix=prefix), prefix=f'{prefix}.layers')
