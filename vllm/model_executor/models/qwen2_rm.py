...
class Qwen2RewardBaseModel(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.score = nn.Sequential(ColumnParallelLinear(config.hidden_size, config.hidden_size, quant_config=quant_config, return_bias=False), nn.ReLU(), RowParallelLinear(config.hidden_size, config.num_labels, quant_config=quant_config, return_bias=False))
...
class Qwen2ForProcessRewardModel(Qwen2RewardBaseModel):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        super().__init__(vllm_config=vllm_config, prefix=prefix)
        pooler_config = vllm_config.model_config.pooler_config
        self.pooler = Pooler.from_config_with_defaults(pooler_config, pooling_type=PoolingType.STEP, normalize=False, softmax=True, step_tag_id=151651)