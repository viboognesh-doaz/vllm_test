...
class EagleMiniCPMDecoderLayer(nn.Module):
    ...
    def _init_ffn_block(self):
        ...
        if self.num_experts == 0:
            self.mlp = EagleMiniCPMMLP(hidden_size=self.hidden_size, intermediate_size=self.config.intermediate_size, hidden_act=self.config.hidden_act, hidden_act_param=getattr(self.config, 'hidden_act_param', 0.0), quant_config=self.quant_config)
        else:
            self.mlp = EagleMiniCPMMoE(num_experts=self.config.num_experts, top_k=self.config.num_experts_per_tok, hidden_size=self.config.hidden_size, intermediate_size=self.config.intermediate_size)
...
class EagleMiniCPMModel(nn.Module):
    ...
    def _init_layers(self, prefix: str, config: PretrainedConfig, cache_config: Optional[CacheConfig], quant_config: Optional[QuantizationConfig], start_layer: int):
        self.eagle_layers = nn.ModuleList([EagleMiniCPMDecoderLayer(config, cache_config, quant_config, f'{prefix}.eagle_layers.{i + start_layer}') for i in range(self.config.num_hidden_layers)])

    def get_input_embeddings(self, input_ids: torch.Tensor) -> torch.Tensor:
        embedding = self.embed_tokens(input_ids)
        return embedding * self.config.scale_emb

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, hidden_states: torch.Tensor) -> Union[torch.Tensor, IntermediateTensors]:
...
class EagleMiniCPMForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.model = self._init_model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'), start_layer=target_layer_num)
