...
from .utils import AutoWeightsLoader, PPMissingLayer, WeightsMapper, flatten_bn, is_pp_missing_parameter, make_empty_intermediate_tensors_factory, maybe_prefix
...
from vllm.multimodal.inputs import MultiModalDataDict, MultiModalFieldConfig, MultiModalInputs, PlaceholderRange
...
def vllm_flash_attention_forward(module: torch.nn.Module, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attention_mask: torch.Tensor, scaling: Optional[float]=None, attention_instances: Optional[dict[Attention]]=None, **kwargs):
...
def replace_linear_class(linear: nn.Linear, style: Literal['colwise', 'rowwise'], quant_config: QuantizationConfig) -> Union[ColumnParallelLinear, RowParallelLinear]:
    """
    ...
    vllm_linear_cls = {'colwise': ColumnParallelLinear, 'rowwise': RowParallelLinear}.get(style, ReplicatedLinear)
    return vllm_linear_cls(input_size=linear.in_features, output_size=linear.out_features, bias=linear.bias is not None, quant_config=quant_config, return_bias=False)
...
class MultiModalProcessor(BaseMultiModalProcessor[MultiModalProcessingInfo]):
    ...
    def apply(self, prompt: Union[str, list[int]], mm_data: MultiModalDataDict, hf_processor_mm_kwargs: Mapping[str, object], tokenization_kwargs: Optional[Mapping[str, object]]=None, return_mm_hashes: bool=False) -> MultiModalInputs:
        ...
        """
        if return_mm_hashes:
            raise ValueError("TransformersForMultimodalLM doesn't support mm hashing yet! Probably you didn't set `disable_mm_preprocessor_cache=True`")
        ...
        return MultiModalInputs(type='multimodal', prompt=prompt, prompt_token_ids=prompt_ids, mm_kwargs=mm_kwargs, mm_hashes=None, mm_placeholders=mm_placeholders)
...
class TransformersModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        if hasattr(config, 'interleaved_sliding_window'):
            config_override = ConfigOverride(config, sliding_window=config.interleaved_sliding_window)
        self.text_config._attn_implementation = 'vllm'
        with init_on_device_without_buffers('meta'), config_override:
            self.model: PreTrainedModel = AutoModel.from_config(config, torch_dtype=model_config.dtype, trust_remote_code=model_config.trust_remote_code)
        self.pipeline_parallel()
        self.tensor_parallel()
        text_config = config.get_text_config()
        if not isinstance(self.model.get_input_embeddings(), PPMissingLayer):
            self.model.set_input_embeddings(VocabParallelEmbedding(text_config.vocab_size, text_config.hidden_size, org_num_embeddings=text_config.vocab_size, quant_config=quant_config))
        self.attention_instances = self.create_attention_instances()
        self.init_parameters(self.model)
        self.make_empty_intermediate_tensors = make_empty_intermediate_tensors_factory(['hidden_states'], text_config.hidden_size)
...
class TransformersForCausalLM(nn.Module, SupportsQuant, SupportsLoRA, SupportsPP):
    ...
    def hf_to_vllm_mapper(self):
        prefix_mapper = {name: 'model.' + name for name, _ in self.model.model.named_children()}
        return WeightsMapper(orig_to_new_substr={'model.': 'model.model.'}, orig_to_new_prefix=prefix_mapper)
...
class TransformersForMultimodalLM(nn.Module, SupportsQuant, SupportsLoRA, SupportsPP, SupportsMultiModal):
    embedding_padding_modules = ['lm_head']
    embedding_modules = ['embed_tokens']
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        if get_pp_group().is_last_rank:
            ...
            self.lm_head = ParallelLMHead(text_config.vocab_size, text_config.hidden_size, quant_config=quant_config, prefix=maybe_prefix(prefix, 'lm_head'))
            if text_config.tie_word_embeddings:
                self.lm_head = self.lm_head.tie_weights(self.model.get_input_embeddings())
            logit_scale = getattr(config, 'logit_scale', 1.0)
            self.logits_processor = LogitsProcessor(self.unpadded_vocab_size, text_config.vocab_size, logit_scale)
        else:
            self.lm_head = PPMissingLayer()
        self.make_empty_intermediate_tensors = self.model.make_empty_intermediate_tensors
    ...
    @property
    def hf_to_vllm_mapper(self):
        prefix_mapper = {'language_model.model': 'model.language_model', 'text_model.model': 'model.text_model', 'vision_tower': 'model.vision_tower', 'vqmodel': 'model.vqmodel', 'vision_model': 'model.vision_model', 'vision_embed_tokens': 'model.vision_embed_tokens', 'image_newline': 'model.image_newline', 'multi_modal_projector': 'model.multi_modal_projector', 'text_model.lm_head': 'lm_head', 'language_model.lm_head': 'lm_head'}
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        loader = AutoWeightsLoader(self, skip_prefixes=['lm_head.'] if self.config.get_text_config().tie_word_embeddings else None)
        return loader.load_weights(weights, mapper=self.hf_to_vllm_mapper)
    ...
    def get_multimodal_embeddings(self, **kwargs):
        ...
        if pixel_values is not None:
            ...
            vision_embeddings = self.model.model.get_image_features(pixel_values, **{k: v.flatten(0, 1) for k, v in kwargs.items()})
