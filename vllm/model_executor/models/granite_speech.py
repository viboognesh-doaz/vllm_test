...
class GraniteSpeechMultiModalProcessor(BaseMultiModalProcessor[GraniteSpeechMultiModalProcessingInfo]):
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(input_features=MultiModalFieldConfig.batched('audio'), audio_embed_sizes=MultiModalFieldConfig.batched('audio'))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> list[PromptUpdate]:
        ...
        return [PromptReplacement(modality='audio', target=[audio_token_id], replacement=get_replacement)]
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        if 'audio' in mm_data:
            ...
            processed_outputs['audio_embed_sizes'] = [torch.sum(indices == audio_token_index).item() for indices in processed_outputs['input_ids']]
...
class GraniteSpeechDummyInputsBuilder(BaseDummyInputsBuilder[GraniteSpeechMultiModalProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'audio': self._get_dummy_audios(length=self.info.get_max_audio_len(), num_audios=num_audios)}
...
class GraniteSpeechEncoderProjector(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, cache_config: CacheConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        ...
        self.qformer = Blip2QFormerModel(config.projector_config, quant_config=quant_config, cache_config=cache_config, prefix=f'{prefix}.qformer')
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        last_hidden_state = self.qformer(query_embeds=self.query.data, encoder_hidden_states=hidden_states)
        query_proj = self.linear(last_hidden_state.view(batch_size, nblocks * self.window_size // self.downsample_rate, -1))
        return query_proj
...
class GraniteSpeechConformerFeedForward(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        ...
        self.up_proj = ColumnParallelLinear(input_size=config.hidden_dim, output_size=config.hidden_dim * config.feedforward_mult, quant_config=quant_config, prefix=f'{prefix}.up_proj')
        self.silu = nn.SiLU()
        self.down_proj = RowParallelLinear(input_size=config.hidden_dim * config.feedforward_mult, output_size=config.hidden_dim, quant_config=quant_config, prefix=f'{prefix}.down_proj')
...
class GraniteSpeechConformerAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_dists: torch.Tensor) -> torch.Tensor:
        ...
        with torch.nn.attention.sdpa_kernel(torch.nn.attention.SDPBackend.MATH):
            out = F.scaled_dot_product_attention(query_states, key_states, value_states, attn_mask=pos_attn, scale=self.scale)
...
class GraniteSpeechConformerConvModule(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, prefix: str=''):
        ...
        self.depth_conv = GraniteSpeechConformerDepthWiseConv1d(inner_dim, inner_dim, kernel_size=config.conv_kernel_size, prefix=f'{prefix}.depth_conv')
        self.silu = nn.SiLU()
...
class GraniteSpeechCTCEncoder(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, prefix: str, quant_config: Optional[QuantizationConfig]=None):
        super().__init__()
        self.config = config
        ...
        self.layers = nn.ModuleList([GraniteSpeechConformerBlock(config, prefix=f'{prefix}.layers.{idx}') for idx in range(config.num_layers)])
        self.out = ColumnParallelLinear(input_size=config.hidden_dim, output_size=config.output_dim, bias=True, quant_config=quant_config, prefix=f'{prefix}.out')
        self.out_mid = RowParallelLinear(input_size=config.output_dim, output_size=config.hidden_dim, bias=True, quant_config=quant_config, prefix=f'{prefix}.out_mid')
...
@MULTIMODAL_REGISTRY.register_processor(GraniteSpeechMultiModalProcessor, info=GraniteSpeechMultiModalProcessingInfo, dummy_inputs=GraniteSpeechDummyInputsBuilder)
class GraniteSpeechForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str):
        ...
        self.encoder = GraniteSpeechCTCEncoder(config=config.encoder_config, quant_config=quant_config, prefix=f'{prefix}.encoder')
        self.projector = GraniteSpeechEncoderProjector(config=config, quant_config=quant_config, cache_config=cache_config, prefix=f'{prefix}.projector')
        self.make_empty_intermediate_tensors = self.language_model.make_empty_intermediate_tensors

    def _parse_and_validate_audio_input(self, **kwargs: object) -> Optional[GraniteSpeechAudioInputs]:
        ...
        if isinstance(input_features, torch.Tensor):
            ...
            input_features = self._pad_and_stack_input_features(input_features).to(self.encoder.input_linear.weight.dtype)
        return GraniteSpeechAudioInputs(input_features=input_features, input_features_mask=input_features_mask, audio_embed_sizes=audio_embed_sizes.flatten().tolist())

    def _build_input_features_mask(self, audio_embed_sizes: torch.Tensor) -> torch.Tensor:
        ...
        mask_indices = torch.arange(most_audio_features, device=audio_embed_sizes.device).view(1, -1)
        input_features_mask = mask_indices < audio_embed_sizes.view(-1, 1)
        return input_features_mask

    def _pad_and_stack_input_features(self, input_features: list[torch.Tensor]) -> torch.Tensor:
    ...
    def _process_audio_input(self, audio_input: GraniteSpeechAudioInputs) -> tuple[torch.Tensor]:
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        inputs_embeds = embed_multimodal(input_ids, self.config.audio_token_index, self.language_model.model.get_input_embeddings, multimodal_embeddings)
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        return MultiModelKeys.from_string_field(language_model='language_model', connector='projector', tower_model='encoder')