...
class LlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def __init__(self, config: LlamaConfig, disable_input_layernorm: bool, prefix: str='') -> None:
...
class LlamaModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='', start_layer_id: int=0) -> None:
        super().__init__()
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer(self.config, i == 0, prefix=maybe_prefix(prefix, f'layers.{i + start_layer_id}')) for i in range(self.config.num_hidden_layers)])
