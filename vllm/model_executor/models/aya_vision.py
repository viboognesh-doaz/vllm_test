...
from vllm.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, MultiModalFieldConfig, PromptReplacement, PromptUpdate, PromptUpdateDetails
...
class AyaVisionMultiModalProjector(nn.Module):
    ...
    def pixel_shuffle(self, image_features: torch.Tensor) -> torch.Tensor:
        ...
        image_features = image_features.reshape(batch_size, int(height / self.downsample_factor), int(width / self.downsample_factor), -1)
        image_features = image_features.permute(0, 2, 1, 3)
...
class AyaVisionProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_patches(self, *, image_width: int, image_height: int, size: dict, min_patches: int, max_patches: int) -> int:
        """
        ...
        num_columns, num_rows = get_optimal_tiled_canvas((image_height, image_width), (size['height'], size['width']), min_patches, max_patches)
...
class AyaVisionDummyInputsBuilder(BaseDummyInputsBuilder[AyaVisionProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'image': self._get_dummy_images(width=image_size.width, height=image_size.height, num_images=num_images)}
...
class AyaVisionMultiModalProcessor(BaseMultiModalProcessor[AyaVisionProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        if (images := mm_data.get('images')) is not None:
            ...
            num_patches = [self.info.get_num_patches(image_width=image_size.width, image_height=image_size.height, size=image_processor.size, min_patches=image_processor.min_patches, max_patches=image_processor.max_patches) for image_size in image_sizes]
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        ...
        return dict(pixel_values=MultiModalFieldConfig.flat_from_sizes('image', num_patches), num_patches=MultiModalFieldConfig.batched('image'), image_embeds=MultiModalFieldConfig.batched('image'))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement(item_idx: int):
            ...
            num_patches = self.info.get_num_patches(image_width=image_size.width, image_height=image_size.height, size=image_processor.size, min_patches=image_processor.min_patches, max_patches=image_processor.max_patches)
...
@MULTIMODAL_REGISTRY.register_processor(AyaVisionMultiModalProcessor, info=AyaVisionProcessingInfo, dummy_inputs=AyaVisionDummyInputsBuilder)
class AyaVisionForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.vision_tower = SiglipVisionModel(config.vision_config, quant_config, num_hidden_layers_override=num_hidden_layers, prefix=maybe_prefix(prefix, 'vision_model'))
        self.vocab_size = config.text_config.vocab_size
        self.multi_modal_projector = AyaVisionMultiModalProjector(config)
        self.language_model = init_vllm_registered_model(vllm_config=vllm_config, hf_config=config.text_config, prefix=maybe_prefix(prefix, 'model'), architectures=['Cohere2ForCausalLM'])
    ...
    def _image_pixels_to_features(self, vision_tower: SiglipVisionModel, pixel_values: torch.Tensor, **kwargs) -> torch.Tensor:
    ...
    def _validate_pixel_values(self, data: torch.Tensor) -> torch.Tensor:
        ...
        def _validate_shape(d: torch.Tensor):
            if d.shape != expected_dims:
                raise ValueError(f'The expected shape of pixel values per image per batch is {expected_dims}. You supplied {tuple(d.shape)}.')
        for d in data:
            _validate_shape(d)
        return data
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[AyaVisionImagePixelInputs]:
        ...
        return AyaVisionImagePixelInputs(type='pixel_values', pixel_values=self._validate_pixel_values(pixel_values), num_patches=num_patches)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids=input_ids, inputs_embeds=inputs_embeds, multimodal_embeddings=multimodal_embeddings, placeholder_token_id=self.config.image_token_index)
