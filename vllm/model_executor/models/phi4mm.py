...
from vllm.multimodal.parse import AudioProcessorItems, ImageEmbeddingItems, ImageProcessorItems, ImageSize, MultiModalDataItems, MultiModalDataParser
...
VISION_ENCODER_TO_PROCESSING_CONFIG = {'siglip-so400m-patch14-448': {'vit_image_size': 448, 'vit_patch_size': 14, 'token_compression_factor': 2}}
...
def get_navit_vision_model(layer_idx: int=-1, **kwargs):
    vision_config = {'hidden_size': 1152, 'image_size': 448, 'intermediate_size': 4304, 'model_type': 'siglip_vision_model', 'num_attention_heads': 16, 'num_hidden_layers': 27, 'patch_size': 14}
    model_config = SiglipVisionConfig(**vision_config, **kwargs)
    if layer_idx < 0:
        num_hidden_layers = model_config.num_hidden_layers + layer_idx + 1
    else:
        num_hidden_layers = layer_idx + 1
    vision_model = Idefics2VisionTransformer(config=model_config, require_post_norm=False, num_hidden_layers_override=num_hidden_layers)
    return vision_model

class Phi4MMImageEncoder(nn.Module):
    """Image embedding."""

    def __init__(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig], prefix: str='', model_dir: str='') -> None:
        super().__init__()
    ...
    def forward(self, pixel_values: torch.FloatTensor, image_sizes: torch.Tensor, image_attention_mask: torch.Tensor) -> list[torch.FloatTensor]:
        """
        ...
        img_features = self.get_img_features(pixel_values, image_attention_mask.type(torch.BoolTensor).flatten(0, 1).to(target_device))
        base_feat_height_target = self.base_feat_height_target
        base_resolution = self.crop_size
        base_feat_height_reduction = self.base_feat_height_reduction
        base_feat_height = base_feat_width = int(np.sqrt(img_features.shape[1]))
        assert base_feat_height == base_feat_height_target and base_feat_width == base_feat_height_target, f'base_feat_height: {base_feat_height},"                f" base_feat_width: {base_feat_width}, "                f"expect {base_feat_height_target} features for hd transform'
...
class Phi4MMProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, *, dynamic_hd: Optional[int]=None, **kwargs: object) -> ProcessorMixin:
    ...
    def get_dynamic_hd(self, processor: Optional[ProcessorMixin]=None) -> int:
    ...
    def _find_target_aspect_ratio(self, orig_width: int, orig_height: int, image_size: int, max_num: int, min_num: int):
        ...
        if w_crop_num * h_crop_num > max_num:
            ...
            target_aspect_ratio = image_processor.find_closest_aspect_ratio(aspect_ratio, target_ratios, orig_width, orig_height, image_size)
    ...
    def _compute_num_image_tokens(self, orig_width: int, orig_height: int, dynamic_hd_size: int, vit_image_size: int, vit_patch_size: int, token_compression_factor: int=2):
        """
        ...
        target_aspect_ratio, target_height, target_width = self._find_target_aspect_ratio(orig_width, orig_height, vit_image_size, dynamic_hd_size, min_num=1)
        assert target_aspect_ratio[0] * vit_image_size == target_width, f'{target_aspect_ratio[0]} * {vit_image_size} != {target_width}'
        assert target_aspect_ratio[1] * vit_image_size == target_height, f'{target_aspect_ratio[1]} * {vit_image_size} != {target_height}'
        ...
        return num_global_image_tokens + num_sep_tokens + num_hd_patch_tokens + num_hd_newline_tokens + num_global_image_newline_tokens

    def get_num_image_tokens(self, *, image_width: int, image_height: int, processor: Optional[ProcessorMixin]=None) -> int:
        ...
        image_num_tokens = self._compute_num_image_tokens(image_width, image_height, dynamic_hd_size=dynamic_hd_size, vit_image_size=vit_image_size, vit_patch_size=vit_patch_size, token_compression_factor=token_compression_factor)
        return image_num_tokens

    def get_image_size_with_most_features(self, processor: Optional[ProcessorMixin]=None) -> ImageSize:
        hf_config = self.get_hf_config()
        vision_encoder_name = hf_config.img_processor
        if vision_encoder_name is None:
            vision_encoder_name = SIGLIP_NAME
...
class Phi4MMDummyInputsBuilder(BaseDummyInputsBuilder[Phi4MMProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        mm_data = {'image': self._get_dummy_images(width=target_width, height=target_height, num_images=num_images), 'audio': self._get_dummy_audios(length=_AUDIO_MAX_SOUNDFILE_SIZE, num_audios=num_audios)}
        return mm_data
...
class Phi4MMMultiModalProcessor(BaseMultiModalProcessor[Phi4MMProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        num_img_tokens = [self.info.get_num_image_tokens(image_width=img_size[0], image_height=img_size[1]) for img_size in processed_outputs['image_sizes']]
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(input_image_embeds=MultiModalFieldConfig.batched('image'), image_attention_mask=MultiModalFieldConfig.batched('image'), image_sizes=MultiModalFieldConfig.batched('image'), num_img_tokens=MultiModalFieldConfig.batched('image'), input_audio_embeds=MultiModalFieldConfig.batched('audio'))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, Any], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_image_replacement_phi4mm(item_idx: int):
            ...
            if isinstance(images, ImageEmbeddingItems):
                ...
                num_image_tokens = self.info.get_num_image_tokens(image_width=image_size.width, image_height=image_size.height, processor=hf_processor)
        ...
        image_repl = [PromptReplacement(modality='image', target=image_token, replacement=get_image_replacement_phi4mm) for image_token in image_tokens[:num_images]]
        audio_repl = [PromptReplacement(modality='audio', target=audio_token, replacement=get_audio_replacement_phi4mm) for audio_token in audio_tokens[:num_audios]]
        return image_repl + audio_repl

@MULTIMODAL_REGISTRY.register_processor(Phi4MMMultiModalProcessor, info=Phi4MMProcessingInfo, dummy_inputs=Phi4MMDummyInputsBuilder)
class Phi4MMForCausalLM(nn.Module, SupportsLoRA, SupportsMultiModal):
    """
    Implements the Phi-4-multimodal-instruct model in vLLM.
    """
    packed_modules_mapping = {'qkv_proj': ['qkv_proj'], 'gate_up_proj': ['gate_up_proj']}
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_substr={'base_layer.': ''}, orig_to_new_prefix={'model.embed_tokens_extend.audio_embed.audio_projection.vision.': 'embed_tokens_extend.audio_projection_for_vision.', 'model.embed_tokens_extend.audio_embed.audio_projection.speech.': 'embed_tokens_extend.audio_projection.', 'model.embed_tokens_extend.audio_embed.': 'embed_tokens_extend.', 'model.embed_tokens_extend.image_embed.': 'vision_encoder.'})
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.vision_encoder = Phi4MMImageEncoder(config, quant_config, prefix='model.vision_embed_tokens', model_dir=config._name_or_path)
        if isinstance(config.embd_layer['audio_embd_layer'], dict):
            embedding_config = {'embedding_cls': config.embd_layer['audio_embd_layer']['embedding_cls'], **config.embd_layer['audio_embd_layer']}
        else:
        ...
        self.lm_head = ParallelLMHead(self.unpadded_vocab_size, config.hidden_size, org_num_embeddings=config.vocab_size, padding_size=DEFAULT_VOCAB_PADDING_SIZE, quant_config=quant_config)
        if config.tie_word_embeddings:
            self.lm_head = self.lm_head.tie_weights(self.model.embed_tokens)
        logit_scale = getattr(config, 'logit_scale', 1.0)
        self.logits_processor = LogitsProcessor(self.unpadded_vocab_size, config.vocab_size, logit_scale)

    def _parse_and_validate_audio_input(self, **kwargs: object) -> Optional[Phi4MMAudioInputs]:
        """
        ...
        if audio_features is not None:
            if not isinstance(audio_features, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of audio features. Got type: {type(audio_features)}')
            return Phi4MMAudioFeatureInputs(type='audio_features', data=flatten_bn(audio_features))
    ...
    def _process_audio_input(self, audio_input: Phi4MMAudioInputs, audio_projection_mode: str) -> NestedTensors:
        ...
        audio_embeds = [self.embed_tokens_extend(features.to(dtype), audio_projection_mode=audio_projection_mode) for features in audio_features]
        return audio_embeds
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[dict]:
        ...
        assert image_sizes is not None and image_attention_mask is not None and (num_img_tokens is not None), 'Missing image inputs'
        ...
        if isinstance(num_img_tokens, list):
            num_img_tokens = [n for num_tensor in num_img_tokens for n in num_tensor.tolist()]
        elif isinstance(num_img_tokens, torch.Tensor):
            num_img_tokens = num_img_tokens.flatten(0, 1).tolist()
        else:
            raise ValueError('Incorrect image_attention_mask inputs')
        return Phi4MMImagePixelInputs(type='pixel_values', data=input_image_embeds, image_sizes=image_sizes, image_attention_mask=image_attention_mask, num_img_tokens=num_img_tokens)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, [_IMAGE_PLACEHOLDER_TOKEN_ID, _AUDIO_PLACEHOLDER_TOKEN_ID])
        return inputs_embeds

    def get_input_embeddings_v0(self, input_ids: torch.Tensor, image_input: Optional[Phi4MMImagePixelInputs]=None, audio_input: Optional[Phi4MMAudioFeatureInputs]=None) -> torch.Tensor:
        audio_projection_mode = 'speech'
        inputs_embeds = self.get_input_embeddings(input_ids)
        if image_input is not None:
            image_embeds = self._process_image_input(image_input)
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, image_embeds, placeholder_token_id=_IMAGE_PLACEHOLDER_TOKEN_ID)
            audio_projection_mode = 'vision'
        if audio_input is not None:
            audio_embeds = self._process_audio_input(audio_input, audio_projection_mode=audio_projection_mode)
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, audio_embeds, placeholder_token_id=_AUDIO_PLACEHOLDER_TOKEN_ID)
        return inputs_embeds

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: object) -> torch.Tensor:
        if intermediate_tensors is not None:
            ...
            if image_input is None and audio_input is None:
                ...
                inputs_embeds = self.get_input_embeddings_v0(input_ids, image_input=image_input, audio_input=audio_input)
                input_ids = None
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        """
        return MultiModelKeys.from_string_field(language_model='model.', connector=['audio_projection_for_vision', 'audio_projection'], tower_model=['vision_encoder', 'embed_tokens_extend'])
