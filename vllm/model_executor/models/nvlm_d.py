...
from .internvl import BaseInternVLDummyInputsBuilder, BaseInternVLMultiModalProcessor, BaseInternVLProcessingInfo, BaseInternVLProcessor, InternVLChatModel
...
class NVLMProcessingInfo(BaseInternVLProcessingInfo):
    ...
    def get_hf_processor(self, *, min_dynamic_patch: Optional[int]=None, max_dynamic_patch: Optional[int]=None, dynamic_image_size: Optional[bool]=None, **kwargs: object) -> NVLMProcessor:
        ...
        return self.ctx.init_processor(NVLMProcessor, config=self.get_hf_config(), tokenizer=self.get_tokenizer(), **kwargs)
...
class NVLMMultiModalProcessor(BaseInternVLMultiModalProcessor[NVLMProcessingInfo]):
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptReplacement(modality='image', target='<image>\n', replacement=get_replacement_nvlm)]

@MULTIMODAL_REGISTRY.register_processor(NVLMMultiModalProcessor, info=NVLMProcessingInfo, dummy_inputs=NVLMDummyInputsBuilder)
class NVLM_D_Model(InternVLChatModel):
    ...
    def _init_mlp1(self, config: PretrainedConfig) -> nn.Sequential:
        ...
        return nn.Sequential(nn.LayerNorm(vit_hidden_size * int(1 / self.downsample_ratio) ** 2), nn.Linear(vit_hidden_size * int(1 / self.downsample_ratio) ** 2, llm_intermediate_size, bias=False), nn.GELU(), nn.Linear(llm_intermediate_size, llm_hidden_size, bias=False))
    ...
    def _init_vision_model(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig], *, is_mono: bool, prefix: str):
        if not is_mono:
            ...
            return InternVisionModel(config.vision_config, quant_config=quant_config, num_hidden_layers_override=num_hidden_layers, num_dummy_heads=7, prefix=prefix)
        else:
