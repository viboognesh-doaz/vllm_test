...
from vllm.model_executor.layers.pooler import Pooler, PoolerHead, PoolerNormalize, PoolingParamsUpdate, build_output, get_prompt_lens, get_prompt_token_ids
...
class GritLMMeanPool(nn.Module):
    ...
    def __init__(self, model_config: ModelConfig):
        ...
        self.token_ids = {tok: tokenizer.convert_tokens_to_ids([tok])[0] for tok in ['<s>', '▁<', '<', '|', 'embed', '>', '<0x0A>', 'user']}
    ...
    def _find_array(self, arr: np.ndarray, target: np.ndarray, start_idx: int=0, end_idx: Optional[int]=None) -> int:
        """
    ...
    def _get_instruction_len(self, prompt_token_ids: np.ndarray) -> int:
        ...
        if prompt_token_ids[0] != self.token_ids['<s>']:
            logger.warning('BOS token not found in prompt, thus using empty string for instruction. GritLM requires BOS token in prompt.')
        ...
        if found_embed_pattern_idx != -1:
            ...
            logger.warning('Query instruction not found in prompt, thus using BOS token as instruction instead. GritLM requires query instruction in prompt.')
            instruction_len = 1
        return instruction_len
    ...
    def forward_one(self, hidden_states: torch.Tensor, prompt_len: Optional[torch.Tensor]=None, instr_len: Optional[torch.Tensor]=None) -> torch.Tensor:
    ...
    def forward_all(self, hidden_states: torch.Tensor, prompt_lens: torch.Tensor, instr_lens: torch.Tensor) -> Union[list[torch.Tensor], torch.Tensor]:
        offset = 0
    ...
    def forward(self, hidden_states: Union[torch.Tensor, list[torch.Tensor]], pooling_metadata: PoolingMetadata) -> Union[list[torch.Tensor], torch.Tensor]:
        ...
        instr_lens = torch.tensor([self._get_instruction_len(token_ids.cpu().numpy()) for token_ids in get_prompt_token_ids(pooling_metadata)], device=prompt_lens.device)
        if isinstance(hidden_states, list):
            return [self.forward_one(h, prompt_len, instr_len) for h, prompt_len, instr_len in zip(hidden_states, prompt_lens, instr_lens)]
