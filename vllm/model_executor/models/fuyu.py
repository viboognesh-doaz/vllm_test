...
class FuyuProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self, *, image_width: int, image_height: int) -> int:
        ncols, nrows = self.get_image_feature_grid_size(image_width=image_width, image_height=image_height)
        return ncols * nrows
...
class FuyuMultiModalProcessor(BaseMultiModalProcessor[FuyuProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        if image_patches is not None:
            ...
            if isinstance(image_patches, list):
                ...
                assert isinstance(image_patches[0], torch.Tensor) and len(image_patches[0]) == len(images)
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement_fuyu(item_idx: int):
            ...
            ncols, nrows = self.info.get_image_feature_grid_size(image_width=image_size.width, image_height=image_size.height)
            image_tokens = ([_IMAGE_TOKEN_ID] * ncols + [_NEWLINE_TOKEN_ID]) * nrows
            return PromptUpdateDetails.select_token_id(image_tokens + [bos_token_id], embed_token_id=_IMAGE_TOKEN_ID)
        return [PromptReplacement(modality='image', target=[eot_token_id], replacement=get_replacement_fuyu)]

@MULTIMODAL_REGISTRY.register_processor(FuyuMultiModalProcessor, info=FuyuProcessingInfo, dummy_inputs=FuyuDummyInputsBuilder)
class FuyuForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_prefix={'model.vision_embed_tokens.': 'vision_embed_tokens.', 'model.language_model.': 'language_model.model.', 'lm_head.': 'language_model.lm_head.'})
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.vision_embed_tokens = ColumnParallelLinear(self.image_feature_size, config.hidden_size, quant_config=quant_config, gather_output=True)
        self.language_model = PersimmonForCausalLM(vllm_config=vllm_config.with_hf_config(config.text_config), prefix=maybe_prefix(prefix, 'language_model'))
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[FuyuImagePatchInputs]:
        image_patches = kwargs.pop('image_patches', None)
        if image_patches is not None:
            if not isinstance(image_patches, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of image patches. Got type: {type(image_patches)}')
            image_patches_flat = flatten_bn(image_patches)
            return FuyuImagePatchInputs(type='image_patches', flat_data=self._validate_pixel_values(flatten_bn(image_patches_flat, concat=True)), patches_per_image=[x.size(0) for x in image_patches_flat])
        return None
    ...
    def compute_logits(self, hidden_states: torch.Tensor, sampling_metadata: SamplingMetadata) -> Optional[torch.Tensor]:
        logits = self.language_model.logits_processor(self.language_model.lm_head, hidden_states, sampling_metadata)
        return logits
