...
class DeciLMAttention(LlamaAttention):
    ...
    def __init__(self, config: LlamaConfig, hidden_size: int, num_heads: int, num_kv_heads: int, rope_theta: float=10000, rope_scaling: Optional[dict[str, Any]]=None, max_position_embeddings: int=8192, quant_config: Optional[QuantizationConfig]=None, bias: bool=False, bias_o_proj: bool=False, cache_config: Optional[CacheConfig]=None, prefix: str='', attn_type: str=AttentionType.DECODER) -> None:
        super().__init__(config, hidden_size, num_heads, num_kv_heads, rope_theta, rope_scaling, max_position_embeddings, quant_config, bias, bias_o_proj, cache_config, prefix, attn_type)

    def _init_rotary_emb(self, config, rope_scaling: Optional[dict[str, Any]], quant_config: Optional[QuantizationConfig]) -> None:
        is_neox_style = True
        if hasattr(config, 'position_embedding_type'):
            is_neox_style = config.position_embedding_type not in ['mistral_yarn', 'rope_llama4']
...
class DeciLMDecoderLayer(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        if not self._is_no_op_attention:
            ...
            self.self_attn = DeciLMAttention(config=config, hidden_size=self.hidden_size, num_heads=config.num_attention_heads, num_kv_heads=num_kv_heads, rope_theta=rope_theta, rope_scaling=rope_scaling, max_position_embeddings=max_position_embeddings, quant_config=quant_config, bias=attention_bias, bias_o_proj=bias_o_proj, cache_config=cache_config, prefix=f'{prefix}.self_attn')
        ...
        if not self._is_no_op_ffn:
            ...
            self.mlp = LlamaMLP(hidden_size=self.hidden_size, intermediate_size=intermediate_size, hidden_act=config.hidden_act, quant_config=quant_config, bias=getattr(config, 'mlp_bias', False), prefix=f'{prefix}.mlp')
...
class DeciModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str='', layer_type: type[DeciLMDecoderLayer]=DeciLMDecoderLayer):
        super().__init__()
        ...
        def get_layer(prefix: str):
            ...
            return layer_type(config, layer_idx, cache_config, quant_config=quant_config, prefix=prefix)
...
class DeciLMForCausalLM(nn.Module, SupportsLoRA, SupportsPP, HasNoOps):
    ...
    mistral_mapping = {'layers': 'model.layers', 'attention': 'self_attn', 'wq': 'q_proj', 'wk': 'k_proj', 'wv': 'v_proj', 'wo': 'o_proj', 'attention_norm': 'input_layernorm', 'feed_forward': 'mlp', 'w1': 'gate_proj', 'w2': 'down_proj', 'w3': 'up_proj', 'ffn_norm': 'post_attention_layernorm', 'tok_embeddings': 'model.embed_tokens', 'output': 'lm_head', 'norm': 'model.norm'}
