from .minicpmv import _MAX_FRAMES_PER_VIDEO, MiniCPMV2_6, MiniCPMVDummyInputsBuilder, MiniCPMVMultiModalDataParser, MiniCPMVMultiModalProcessor, MiniCPMVProcessingInfo, _minicpmv_field_config
...
from vllm.multimodal.parse import AudioItem, AudioProcessorItems, DictEmbeddingItems, ModalityData, ModalityDataItems, MultiModalDataItems, MultiModalDataParser
...
def _minicpmo_field_config(hf_inputs: Mapping[str, torch.Tensor]):
    ...
    return dict(**_minicpmv_field_config(hf_inputs), audio_features=MultiModalFieldConfig.batched('audio'), audio_feature_lens=MultiModalFieldConfig.batched('audio'), audio_embeds=MultiModalFieldConfig.batched('audio'), audio_token_id=MultiModalFieldConfig.shared('audio', num_audios))
...
class MiniCPMOAudioEmbeddingItems(DictEmbeddingItems):
    ...
    def __init__(self, data: Mapping[str, torch.Tensor], fields_factory: Callable[[Mapping[str, torch.Tensor]], Mapping[str, MultiModalFieldConfig]]) -> None:
        super().__init__(data, modality='image', required_fields={'audio_embeds'}, fields_factory=fields_factory)

class MiniCPMOMultiModalDataParser(MiniCPMVMultiModalDataParser):

    def _parse_audio_data(self, data: Union[dict[str, torch.Tensor], ModalityData[AudioItem]]) -> Optional[ModalityDataItems[Any, Any]]:
        if isinstance(data, dict):
            return MiniCPMOAudioEmbeddingItems(data, fields_factory=_minicpmo_field_config)
...
class MiniCPMOProcessingInfo(MiniCPMVProcessingInfo):
    ...
    def get_audio_placeholder(self, audio_lens: int, chunk_input: bool=True, chunk_length: int=1) -> str:
        hf_processor = self.get_hf_processor()
        return hf_processor.get_audio_placeholder(audio_lens, chunk_input=chunk_input, chunk_length=chunk_length)
...
class MiniCPMODummyInputsBuilder(MiniCPMVDummyInputsBuilder[MiniCPMOProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        audio_mm_data = {'audio': self._get_dummy_audios(length=audio_len, num_audios=num_audios)}
        return {**super().get_dummy_mm_data(seq_len, mm_counts), **audio_mm_data}
...
class MiniCPMOMultiModalProcessor(MiniCPMVMultiModalProcessor[MiniCPMOProcessingInfo]):
    ...
    def get_audio_prompt_texts(self, audio_lens: int, chunk_input: bool=True, chunk_length: int=1) -> str:
        return self.info.get_audio_placeholder(audio_lens, chunk_input=chunk_input, chunk_length=chunk_length)

    def process_audios(self, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> Mapping[str, NestedTensors]:
        if (audios := mm_data.get('audios')) is None:
            return {}
        parsed_audios = self._get_data_parser().parse_mm_data({'audio': audios}).get_items('audio', (MiniCPMOAudioEmbeddingItems, AudioProcessorItems))
        if isinstance(parsed_audios, MiniCPMOAudioEmbeddingItems):
            audio_inputs = {}
        else:
            audio_inputs = self._base_call_hf_processor(prompts=[self.info.audio_pattern] * len(parsed_audios), mm_data={'audios': [[audio] for audio in parsed_audios]}, mm_kwargs={**mm_kwargs, 'chunk_input': True}, tok_kwargs=tok_kwargs, out_keys={'audio_features', 'audio_feature_lens'})
            unpadded_audio_features = [feat[:, :feature_len] for feat, feature_len in zip(audio_inputs['audio_features'], audio_inputs['audio_feature_lens'])]
    ...
    def process_mm_inputs(self, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> Mapping[str, NestedTensors]:
        return {**super().process_mm_inputs(mm_data, mm_kwargs, tok_kwargs), **self.process_audios(mm_data, mm_kwargs, tok_kwargs)}
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        base_updates = super()._get_prompt_updates(mm_items=mm_items, hf_processor_mm_kwargs=hf_processor_mm_kwargs, out_mm_kwargs=out_mm_kwargs)
        ...
        def get_audio_replacement(item_idx: int):
            ...
            return PromptUpdateDetails.select_text(self.get_audio_prompt_texts(audio_len), '<unk>')
        return [*base_updates, PromptReplacement(modality='audio', target=audio_placeholder, replacement=get_audio_replacement)]
...
class MiniCPMWhisperEncoderLayer(nn.Module):
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int):
        ...
        self.self_attn = WhisperAttention(embed_dim=self.embed_dim, num_heads=config.encoder_attention_heads, dropout=config.attention_dropout, config=config, layer_idx=layer_idx)
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states, attn_weights, past_key_values = self.self_attn(hidden_states=hidden_states, attention_mask=attention_mask, past_key_value=past_key_values)
...
class MiniCPMWhisperEncoder(WhisperEncoder):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.layers = nn.ModuleList([MiniCPMWhisperEncoderLayer(config, layer_idx=i) for i in range(config.encoder_layers)])

    def forward(self, input_features: torch.Tensor, attention_mask: Optional[torch.Tensor]=None) -> BaseModelOutputWithPast:
        ...
        encoder_states = encoder_states + (hidden_states,)
        return BaseModelOutputWithPast(last_hidden_state=hidden_states, hidden_states=encoder_states)

@MULTIMODAL_REGISTRY.register_processor(MiniCPMOMultiModalProcessor, info=MiniCPMOProcessingInfo, dummy_inputs=MiniCPMODummyInputsBuilder)
class MiniCPMO(MiniCPMV2_6):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.apm = self.init_audio_module(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'apm'))
    ...
    def init_resampler(self, embed_dim: int, vision_dim: int, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> nn.Module:
        quant_config = self._maybe_ignore_quant_config(quant_config)
        return super().init_resampler(embed_dim, vision_dim, quant_config, prefix)
    ...
    def subsequent_chunk_mask(self, size: int, chunk_size: int, num_left_chunks: int=-1, device: torch.device=CPU_DEVICE, num_lookhead: int=0) -> torch.Tensor:
    ...
    def get_audio_hidden_states(self, data: MiniCPMOAudioFeatureInputs) -> list[torch.Tensor]:
        ...
        seq_range = torch.arange(0, max_seq_len, dtype=audio_feature_lens.dtype, device=audio_feature_lens.device).unsqueeze(0).expand(batch_size, max_seq_len)
        ...
        audio_attention_mask = audio_attention_mask_.to(dtype=self.apm.conv1.weight.dtype, device=self.apm.conv1.weight.device)
        if chunk_length > 0:
            chunk_num_frame = int(chunk_length * 50)
            chunk_mask = self.subsequent_chunk_mask(size=max_seq_len, chunk_size=chunk_num_frame, num_left_chunks=-1, device=audio_attention_mask_.device)
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> Optional[MiniCPMOAudioInputs]:
        ...
        if audio_embeds is not None:
            if not isinstance(audio_embeds, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of audio_embeds. Got type: {type(audio_embeds)}')
            audio_embeds_flat = flatten_bn(audio_embeds)
            return MiniCPMOAudioEmbeddingInputs(type='audio_embeds', audio_embeds=audio_embeds_flat)
        if not isinstance(audio_features, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of audio_features. Got type: {type(audio_features)}')
        audio_feature_lens = kwargs.pop('audio_feature_lens')
        if not isinstance(audio_feature_lens, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of audio_feature_lens. Got type: {type(audio_feature_lens)}')
        audio_features_flat = flatten_bn(audio_features)
        audio_feature_lens_flat = flatten_bn(audio_feature_lens)
        return MiniCPMOAudioFeatureInputs(type='audio_features', audio_features=audio_features_flat, audio_feature_lens=audio_feature_lens_flat)
    ...
    def _process_audio_input(self, audio_input: MiniCPMOAudioInputs) -> Union[torch.Tensor, list[torch.Tensor]]:
