...
from vllm.model_executor.models.utils import AutoWeightsLoader, flatten_bn, init_vllm_registered_model, maybe_prefix
...
from vllm.transformers_utils.configs.ovis import BaseVisualTokenizerConfig, OvisConfig
...
IMAGE_PAD_TOKEN_MAP = {'gemma2': '<unused0>', 'llama': '<|reserved_special_token_0|>', 'qwen2': '<|image_pad|>'}
IMAGE_PAD_TOKEN_ID_MAP = {'gemma2': 7, 'llama': 128002, 'qwen2': 151655}

def st_argmax(y_soft: torch.Tensor, dim: int):
    index = y_soft.argmax(dim, keepdim=True)
    return torch.zeros_like(y_soft, memory_format=torch.legacy_contiguous_format).scatter_(dim, index, 1.0)

class VisualTokenizer(torch.nn.Module):

    def __init__(self, config: BaseVisualTokenizerConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.config = config
        self.backbone = self._init_backbone(config=config, quant_config=quant_config, prefix=f'{prefix}.backbone')
        head_dim = config.vocab_size - len(IMAGE_INDICATOR_IDS)
        self.head = torch.nn.Sequential(ReplicatedLinear(config.backbone_config.hidden_size * config.hidden_stride * config.hidden_stride, head_dim, bias=False, return_bias=False), torch.nn.LayerNorm(head_dim))

    def _init_backbone(self, config: BaseVisualTokenizerConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> nn.Module:
        model_type = config.backbone_config.model_type
        if model_type == 'aimv2':
            return AIMv2Model(config=config.backbone_config, quant_config=quant_config, require_post_norm=False, prefix=prefix)
        elif model_type == 'siglip_vision_model':
            return SiglipVisionModel(config=config.backbone_config, quant_config=quant_config, prefix=prefix)
    ...
    def encode(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.hidden_stride > 1:
            ...
            sqrt_l += pl
            features = features.reshape(n, sqrt_l // self.config.hidden_stride, self.config.hidden_stride, sqrt_l // self.config.hidden_stride, self.config.hidden_stride, d)
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        tokens = torch.nn.functional.pad(tokens, (0, len(IMAGE_INDICATOR_IDS)), mode='constant', value=0)
        return tokens
...
class OvisProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, **kwargs):
        return self.ctx.get_hf_processor(OvisProcessor, image_pad_token=self.get_image_pad_token(), image_segment_len=self.get_image_segment_len())
    ...
    def get_image_segment_len(self) -> int:
        ...
        assert patch_grid_length % hidden_stride == 0, f'patch_grid_length {patch_grid_length} is not divisible by hidden_stride {hidden_stride}'
...
class OvisDummyInputsBuilder(BaseDummyInputsBuilder[OvisProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        mm_data = {'image': self._get_dummy_images(width=target_width, height=target_height, num_images=num_images)}
        return mm_data

class OvisMultiModalProcessor(BaseMultiModalProcessor[OvisProcessingInfo]):

    def image_indicators_to_visual_tokens(self, image_indicators: list[int]) -> list[int]:
        """
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(pixel_values=MultiModalFieldConfig.batched('image'), grids=MultiModalFieldConfig.batched('image'), indicator_tokens=MultiModalFieldConfig.batched('image'))

    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> list[PromptReplacement]:

        def get_replacement_ovis(item_idx):
            grid = out_mm_kwargs['grids'][item_idx]
            hf_processor = self.info.get_hf_processor()
            return hf_processor.construct_image_placeholders(grid)
        return [PromptReplacement(modality='image', target=IMAGE_TOKEN, replacement=get_replacement_ovis)]

@MULTIMODAL_REGISTRY.register_processor(OvisMultiModalProcessor, info=OvisProcessingInfo, dummy_inputs=OvisDummyInputsBuilder)
class Ovis(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.llm = init_vllm_registered_model(vllm_config=vllm_config.with_hf_config(config.get_text_config()), prefix=maybe_prefix(prefix, 'llm'))
        self.visual_tokenizer = VisualTokenizer(config=config.visual_tokenizer_config, quant_config=self._maybe_ignore_quant_config(quant_config), prefix=f'{prefix}.visual_tokenizer')
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[OvisImagePatchInputs]:
        ...
        if pixel_values is not None and indicator_tokens is not None:
            ...
            return OvisImagePatchInputs(type='image_patches', flat_data=flatten_bn(flatten_bn(pixel_values), concat=True), patches_per_image=[x.shape[0] for x in flatten_bn(pixel_values)], indicator_tokens=flatten_bn(flatten_bn(indicator_tokens), concat=True))
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.image_pad_token_id)
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: object) -> Union[torch.Tensor, IntermediateTensors]:
        if intermediate_tensors is not None:
            ...
            input_ids = None
        hidden_states = self.llm(input_ids=input_ids, positions=positions, intermediate_tensors=intermediate_tensors, inputs_embeds=inputs_embeds)
