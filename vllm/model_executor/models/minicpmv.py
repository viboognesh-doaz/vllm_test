...
from vllm.model_executor.layers.resampler import BaseResampler, Resampler2, get_2d_sincos_pos_embed
...
from vllm.multimodal.parse import DictEmbeddingItems, ImageItem, ImageProcessorItems, ImageSize, ModalityData, ModalityDataItems, MultiModalDataItems, MultiModalDataParser, VideoItem, VideoProcessorItems
...
class Resampler2_5(BaseResampler):
    ...
    def __init__(self, num_queries: int, embed_dim: int, num_heads: int, kv_dim: Optional[int]=None, norm_layer: Callable[[int], nn.LayerNorm]=DEFAULT_LN, max_size: tuple[int, int]=(70, 70), quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__(num_queries, embed_dim, num_heads, kv_dim, norm_layer, quant_config=quant_config, prefix=prefix)
...
def _minicpmv_field_config(hf_inputs: Mapping[str, torch.Tensor]):
    ...
    return dict(pixel_values=MultiModalFieldConfig.batched('image'), image_sizes=MultiModalFieldConfig.batched('image'), tgt_sizes=MultiModalFieldConfig.batched('image'), image_embeds=MultiModalFieldConfig.batched('image'), video_pixel_values=MultiModalFieldConfig.batched('video'), video_image_sizes=MultiModalFieldConfig.batched('video'), video_tgt_sizes=MultiModalFieldConfig.batched('video'), video_embeds=MultiModalFieldConfig.batched('video'), image_token_id=MultiModalFieldConfig.shared('image', num_images), video_token_id=MultiModalFieldConfig.shared('video', num_videos))

class MiniCPMVImageEmbeddingItems(DictEmbeddingItems):

    def __init__(self, data: Mapping[str, torch.Tensor], fields_factory: Callable[[Mapping[str, torch.Tensor]], Mapping[str, MultiModalFieldConfig]]) -> None:
        super().__init__(data, modality='image', required_fields={'image_embeds', 'image_sizes'}, fields_factory=fields_factory)
...
class MiniCPMVVideoEmbeddingItems(DictEmbeddingItems):
    ...
    def __init__(self, data: Mapping[str, torch.Tensor], fields_factory: Callable[[Mapping[str, torch.Tensor]], Mapping[str, MultiModalFieldConfig]]) -> None:
        super().__init__(data, modality='video', required_fields={'video_embeds', 'video_image_sizes'}, fields_factory=fields_factory)
...
class MiniCPMVMultiModalDataParser(MultiModalDataParser):
    ...
    def _parse_image_data(self, data: Union[dict[str, torch.Tensor], ModalityData[ImageItem]]) -> Optional[ModalityDataItems[Any, Any]]:
        if isinstance(data, dict):
            return MiniCPMVImageEmbeddingItems(data, fields_factory=_minicpmv_field_config)
    ...
    def _parse_video_data(self, data: Union[dict[str, torch.Tensor], ModalityData[VideoItem]]) -> Optional[ModalityDataItems[Any, Any]]:
        if isinstance(data, dict):
            return MiniCPMVVideoEmbeddingItems(data, fields_factory=_minicpmv_field_config)
...
class MiniCPMVProcessingInfo(BaseProcessingInfo):
    ...
    def get_slice_image_placeholder(self, image_size: ImageSize, image_idx: int=0, max_slice_nums: Optional[int]=None, use_image_id: bool=True) -> str:
        image_processor = self.get_image_processor()
        version = self.get_model_version()
        if version == (2, 0) or version == (2, 5):
            return image_processor.get_slice_image_placeholder(image_size)
        return image_processor.get_slice_image_placeholder(image_size, image_idx=image_idx, max_slice_nums=max_slice_nums, use_image_id=use_image_id)

    def get_sliced_grid(self, image_size: ImageSize, max_slice_nums: Optional[int]=None) -> Optional[tuple[int, int]]:
        image_processor = self.get_image_processor()
        version = self.get_model_version()
        if version == (2, 0) or version == (2, 5):
        ...
        return image_processor.get_sliced_grid(image_size, max_slice_nums=max_slice_nums)

    def get_num_image_tokens(self, image_size: ImageSize, max_slice_nums: Optional[int]=None) -> int:
        image_processor = self.get_image_processor()
    ...
    def get_max_video_frame_tokens(self) -> int:
        ...
        return self.get_num_image_tokens(frame_size, max_slice_nums=self.get_video_max_slice_num())
...
class MiniCPMVDummyInputsBuilder(BaseDummyInputsBuilder[_I]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'image': self._get_dummy_images(width=image_width, height=image_height, num_images=num_images), 'video': [self._get_dummy_images(width=video_width, height=video_height, num_images=num_video_frames)] * num_videos}
...
class MiniCPMVMultiModalProcessor(BaseMultiModalProcessor[_I]):
    ...
    def get_video_prompt_texts(self, image_size: ImageSize, num_frames: int) -> str:
        return self.info.get_slice_image_placeholder(image_size=image_size, image_idx=0, max_slice_nums=self.info.get_video_max_slice_num(), use_image_id=False) * num_frames

    def process_images(self, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> Mapping[str, NestedTensors]:
        if (images := mm_data.get('images')) is None:
            return {}
        parsed_images = self._get_data_parser().parse_mm_data({'image': images}).get_items('image', (MiniCPMVImageEmbeddingItems, ImageProcessorItems))
        if isinstance(parsed_images, MiniCPMVImageEmbeddingItems):
            image_inputs = {}
        else:
            image_inputs = self._base_call_hf_processor(prompts=[self.info.image_pattern] * len(parsed_images), mm_data={'images': [[image] for image in parsed_images]}, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs, out_keys={'pixel_values', 'image_sizes', 'tgt_sizes'})
        tokenizer = self.info.get_tokenizer()
        unk_token_id = tokenizer.get_vocab()['<unk>']
        image_inputs['image_token_id'] = torch.tensor(unk_token_id)
        return image_inputs

    def process_videos(self, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> Mapping[str, NestedTensors]:
        if (videos := mm_data.get('videos')) is None:
            return {}
        parsed_videos = self._get_data_parser().parse_mm_data({'video': videos}).get_items('video', (MiniCPMVVideoEmbeddingItems, VideoProcessorItems))
        if isinstance(parsed_videos, MiniCPMVVideoEmbeddingItems):
            video_inputs = {}
        else:
            video_inputs = self._base_call_hf_processor(prompts=[self.info.image_pattern * len(video) for video in parsed_videos], mm_data={'images': list(parsed_videos)}, mm_kwargs={**mm_kwargs, 'max_slice_nums': self.info.get_video_max_slice_num()}, tok_kwargs=tok_kwargs, out_keys={'pixel_values', 'image_sizes', 'tgt_sizes'})
    ...
    def process_mm_inputs(self, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> Mapping[str, NestedTensors]:
        return {**self.process_images(mm_data, mm_kwargs, tok_kwargs), **self.process_videos(mm_data, mm_kwargs, tok_kwargs)}

    def _base_call_hf_processor(self, prompts: list[str], mm_data: Mapping[str, Sequence[object]], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object], *, out_keys: set[str]) -> dict[str, NestedTensors]:
        if self.info.get_model_version() == (2, 6):
            inputs = super()._call_hf_processor(prompt=prompts, mm_data=mm_data, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
        else:
            inputs = defaultdict[str, list[torch.Tensor]](list)
            for i, prompt in enumerate(prompts):
                inputs_one = super()._call_hf_processor(prompt=prompt, mm_data={k: v[i] for k, v in mm_data.items()}, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        placeholder = {'image': self.info.image_pattern, 'video': self.info.video_pattern}

        def get_image_replacement(item_idx: int):
            images = mm_items.get_items('image', (MiniCPMVImageEmbeddingItems, ImageProcessorItems))
            image_size = images.get_image_size(item_idx)
            return PromptUpdateDetails.select_text(self.get_image_prompt_texts(image_size, item_idx), '<unk>')
        ...
        def get_video_replacement(item_idx: int):
            ...
            return PromptUpdateDetails.select_text(self.get_video_prompt_texts(frame_size, num_frames), '<unk>')
        get_replacement = {'image': get_image_replacement, 'video': get_video_replacement}
        return [PromptReplacement(modality=modality, target=placeholder[modality], replacement=get_replacement[modality]) for modality in ('image', 'video')]
...
class MiniCPMVBaseModel(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.llm = self.init_llm(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'llm'))
        ...
        self.resampler = self.init_resampler(self.embed_dim, self.vision_dim, quant_config=quant_config, prefix=maybe_prefix(prefix, 'resampler'))
        self.mm_token_ids = set[int]()
        self.make_empty_intermediate_tensors = self.llm.make_empty_intermediate_tensors

    def _parse_and_validate_vision_input(self, modality: str, **kwargs: object) -> Optional[MiniCPMVImageInputs]:
        ...
        if image_embeds is not None:
            if not isinstance(image_embeds, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of image_embeds for modality={modality!r}. Got type: {type(image_embeds)}')
            image_embeds_flat = flatten_bn(image_embeds)
            return MiniCPMVImageEmbeddingInputs(type='image_embeds', image_embeds=image_embeds_flat)
        if not isinstance(pixel_values, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of pixel_values for modality={modality!r}. Got type: {type(pixel_values)}')
        tgt_sizes = kwargs.pop('tgt_sizes')
        if not isinstance(tgt_sizes, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of tgt_sizes for modality={modality!r}. Got type: {type(tgt_sizes)}')
        ...
        if len(pixel_values_flat) != len(tgt_sizes_flat):
            raise ValueError(f'Inconsistent flattened lengths, found: {len(pixel_values_flat)} vs. {len(tgt_sizes_flat)}')
        return MiniCPMVImagePixelInputs(type='pixel_values', pixel_values=pixel_values_flat, tgt_sizes=tgt_sizes_flat, num_slices=num_slices_flat)
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> dict:
        ...
        for input_key in kwargs:
            ...
            if input_key in ('video_pixel_values', 'video_embeds') and 'videos' not in modalities:
                ...
                modalities['videos'] = self._parse_and_validate_vision_input('videos', **{_image_key(k): v for k, v in kwargs.items()})
        return modalities

    def _process_vision_input(self, image_input: MiniCPMVImageInputs) -> Union[torch.Tensor, list[torch.Tensor], tuple[torch.Tensor, ...]]:
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            ...
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, list(self.mm_token_ids))
        return inputs_embeds

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: Any) -> torch.Tensor:
        if intermediate_tensors is not None:
            ...
            input_ids = None
        hidden_states = self.llm.model(input_ids=input_ids, positions=positions, intermediate_tensors=intermediate_tensors, inputs_embeds=inputs_embeds)
    ...
    def init_vision_module(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig], prefix: str='') -> nn.Module:
        raise NotImplementedError

    def init_resampler(self, embed_dim: int, vision_dim: int, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> nn.Module:
        raise NotImplementedError
...
class MiniCPMV2_0(MiniCPMVBaseModel):
    ...
    def init_vision_module(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig], prefix: str='') -> nn.Module:
        ...
        with set_default_torch_dtype(torch.float16):
            model = timm.create_model('vit_so400m_patch14_siglip_384.webli', pretrained=False, num_classes=0, dynamic_img_size=True, dynamic_img_pad=True)
    ...
    def init_resampler(self, embed_dim: int, vision_dim: int, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> nn.Module:
        with set_default_torch_dtype(torch.float16):
            resampler = Resampler2(embed_dim=embed_dim, num_heads=embed_dim // 128, grid_size=int(math.sqrt(self.config.query_num)), kv_dim=vision_dim, adaptive=False, do_post_projection=True, quant_config=quant_config, prefix=prefix)
...
class MiniCPMV2_5(MiniCPMVBaseModel, SupportsLoRA):
    ...
    def init_vision_module(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig], prefix: str='') -> nn.Module:
        model = Idefics2VisionTransformer(config.vision_config, quant_config=quant_config, prefix=prefix)
        if self.config.drop_vision_last_layer:
    ...
    def init_resampler(self, embed_dim: int, vision_dim: int, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> nn.Module:
        with set_default_torch_dtype(torch.float16):
            resampler = Resampler2_5(num_queries=self.config.query_num, embed_dim=embed_dim, num_heads=embed_dim // 128, kv_dim=vision_dim, quant_config=quant_config, prefix=prefix)
        return resampler.to(device=current_platform.device_type, dtype=torch.get_default_dtype())
    ...
    def get_vision_hidden_states(self, data: MiniCPMVImagePixelInputs) -> torch.Tensor:
        ...
        vision_embedding = self.vpm(all_pixel_values, patch_attention_mask=patch_attn_mask.unsqueeze(1), tgt_sizes=None)
...
class MiniCPMV2_6(MiniCPMVBaseModel, SupportsLoRA):
    ...
    def init_vision_module(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> nn.Module:
        model = Idefics2VisionTransformer(config.vision_config, quant_config=quant_config, prefix=prefix)
        if self.config.drop_vision_last_layer:
            model.encoder.layers = model.encoder.layers[:-1]
        return model
    ...
    def get_vision_hidden_states(self, data: MiniCPMVImagePixelInputs) -> torch.Tensor:
        ...
        for i, num_patches_item in enumerate(num_patches):
            patch_attn_mask[i, :num_patches_item] = True
        vision_embedding = self.vpm(all_pixel_values, patch_attention_mask=patch_attn_mask.unsqueeze(1), tgt_sizes=tgt_sizes)
        return self.resampler(vision_embedding, tgt_sizes)
_SUPPORT_VERSION = {(2, 0): MiniCPMV2_0, (2, 5): MiniCPMV2_5, (2, 6): MiniCPMV2_6}

@MULTIMODAL_REGISTRY.register_processor(MiniCPMVMultiModalProcessor, info=MiniCPMVProcessingInfo, dummy_inputs=MiniCPMVDummyInputsBuilder)
class MiniCPMV(MiniCPMVBaseModel, SupportsMultiModal, SupportsLoRA):
    ...
    def __new__(cls, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        if instance_cls is None:
            raise ValueError('Currently, MiniCPMV only supports versions 2.0, 2.5, and 2.6')
