...
from .utils import flatten_bn, is_pp_missing_parameter, make_empty_intermediate_tensors_factory, make_layers, maybe_prefix, merge_multimodal_embeddings
...
from vllm.model_executor.model_loader.weight_utils import default_weight_loader, row_parallel_weight_loader
...
class ChameleonDummyInputsBuilder(BaseDummyInputsBuilder[ChameleonProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        num_images = mm_counts.get('image', 0)
        return {'image': self._get_dummy_images(width=width, height=height, num_images=num_images)}
...
class ChameleonMultiModalProcessor(BaseMultiModalProcessor[ChameleonProcessingInfo]):
    ...
    def _apply_hf_processor_tokens_only(self, prompt_tokens: list[int]) -> list[int]:
        ...
        sep_token_id = vocab[tokenizer.sep_token]
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptReplacement(modality='image', target=[image_token_id], replacement=PromptUpdateDetails.select_token_id([image_start_id] + image_tokens + [image_end_id], embed_token_id=image_token_id))]
...
class ChameleonMLP(nn.Module):
    ...
    def __init__(self, hidden_size: int, intermediate_size: int, hidden_act: str, quant_config: Optional[QuantizationConfig]=None, bias: bool=False) -> None:
        super().__init__()
        self.gate_up_proj = MergedColumnParallelLinear(input_size=hidden_size, output_sizes=[intermediate_size] * 2, bias=bias, quant_config=quant_config)
        self.down_proj = RowParallelLinear(input_size=intermediate_size, output_size=hidden_size, bias=bias, quant_config=quant_config)
        if hidden_act != 'silu':
...
class ChameleonAttention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_heads: int, num_kv_heads: int, rope_theta: float=10000, rope_scaling: Optional[dict[str, Any]]=None, max_position_embeddings: int=4096, quant_config: Optional[QuantizationConfig]=None, bias: bool=False, cache_config: Optional[CacheConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        self.qkv_proj = QKVParallelLinear(hidden_size=hidden_size, head_size=self.head_dim, total_num_heads=self.total_num_heads, total_num_kv_heads=self.total_num_kv_heads, bias=bias, quant_config=quant_config)
        self.o_proj = RowParallelLinear(input_size=self.total_num_heads * self.head_dim, output_size=hidden_size, bias=bias, quant_config=quant_config)
...
class ChameleonDecoderLayer(nn.Module):
    ...
    def __init__(self, config: ChameleonConfig, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str='') -> None:
        super().__init__()
        ...
        self.self_attn = ChameleonAttention(hidden_size=self.hidden_size, num_heads=config.num_attention_heads, num_kv_heads=getattr(config, 'num_key_value_heads', config.num_attention_heads), rope_theta=rope_theta, rope_scaling=rope_scaling, max_position_embeddings=max_position_embeddings, quant_config=quant_config, bias=False, cache_config=cache_config, prefix=f'{prefix}.self_attn')
        self.mlp = ChameleonMLP(hidden_size=self.hidden_size, intermediate_size=config.intermediate_size, hidden_act=config.hidden_act, quant_config=quant_config, bias=getattr(config, 'mlp_bias', False))
        self.input_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)

    def forward(self, positions: torch.Tensor, hidden_states: torch.Tensor, residual: Optional[torch.Tensor]) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        if residual is None:
...
class ChameleonVQVAEEncoderResnetBlock(nn.Module):
    ...
    def __init__(self, config: ChameleonVQVAEConfig, in_channels: int, out_channels=None, conv_shortcut=False):
        super().__init__()
...
class ChameleonModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(config.num_hidden_layers, lambda prefix: decoder_layer(config=config, cache_config=cache_config, quant_config=quant_config, prefix=prefix), prefix=f'{prefix}.layers')
...
@MULTIMODAL_REGISTRY.register_processor(ChameleonMultiModalProcessor, info=ChameleonProcessingInfo, dummy_inputs=ChameleonDummyInputsBuilder)
class ChameleonForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsQuant):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.model = ChameleonModel(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'model'))
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[ChameleonImagePixelInputs]:
        ...
        return ChameleonImagePixelInputs(type='pixel_values', data=self._validate_pixel_values(pixel_values))
    ...
    def get_multimodal_embeddings(self, **kwargs: object) -> MultiModalEmbeddings:
        ...
        image_tokens = self.model.get_image_tokens(image_input['data'].to(self.config.torch_dtype))
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.model.vocabulary_mapping.image_token_id)
                ...
                for param_name, weight_name, shard_id in stacked_params_mapping:
                    ...
                    if name.endswith('kv_scale'):
                        ...
                        if remapped_kv_scale_name not in params_dict:
                            logger.warning_once('Found kv scale in the checkpoint (e.g. %s), but not found the expected name in the model (e.g. %s). kv-scale is not loaded.', name, remapped_kv_scale_name)
                            continue
                        else:
