...
from .utils import AutoWeightsLoader, WeightsMapper, flatten_bn, init_vllm_registered_model, maybe_prefix, merge_multimodal_embeddings, merge_multimodal_embeddings_from_map
...
class UltravoxProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, *, sampling_rate: Optional[int]=None, **kwargs: object) -> ProcessorMixin:
...
class UltravoxMultiModalProcessor(BaseMultiModalProcessor[UltravoxProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        feature_extractor = self.info.get_feature_extractor()
        mm_kwargs = dict(**mm_kwargs, sampling_rate=feature_extractor.sampling_rate, include_audio_num_chunks=True)
        item_processor_data = dict(**mm_data, audios=audios)
        tok_kwargs.pop('padding', None)
        tok_kwargs.pop('truncation', None)
        output = super()._call_hf_processor(prompt=prompt, mm_data=item_processor_data, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        ...
        return dict(audio_features=MultiModalFieldConfig.flat_from_sizes('audio', num_chunks), audio_token_len=MultiModalFieldConfig.flat_from_sizes('audio', num_chunks), audio_lens=MultiModalFieldConfig.flat_from_sizes('audio', num_chunks), audio_num_chunks=MultiModalFieldConfig.batched('audio'), audio_embeds=MultiModalFieldConfig.batched('audio'))
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, Any], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptReplacement(modality='audio', target='<|audio|>', replacement=get_replacement_ultravox)]
...
class ModifiedWhisperEncoder(WhisperEncoder):
    ...
    def get_attention_mask_by_audio_len(self, audio_lens: Optional[torch.Tensor], hidden_states: torch.Tensor):
        """
    ...
    def forward(self, input_features: torch.Tensor, audio_lens: Optional[torch.Tensor]=None):
        expected_seq_length = self.max_context_length
        if input_features.shape[-1] > expected_seq_length:
            raise ValueError(f'Whisper expects the mel input features to be of length {expected_seq_length} or less, but found {input_features.shape[-1]}. Make sure to pad the input mel features to {expected_seq_length}.')
        ...
        for encoder_layer in self.layers:
            layer_outputs = encoder_layer(hidden_states, attention_mask, layer_head_mask=None)
            hidden_states = layer_outputs[0]
        hidden_states = self.layer_norm(hidden_states)
        return hidden_states

@MULTIMODAL_REGISTRY.register_processor(UltravoxMultiModalProcessor, info=UltravoxProcessingInfo, dummy_inputs=UltravoxDummyInputsBuilder)
class UltravoxModel(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA):
    packed_modules_mapping = {'qkv_proj': ['q_proj', 'k_proj', 'v_proj'], 'gate_up_proj': ['gate_proj', 'up_proj']}
    hf_to_vllm_mapper = WeightsMapper(orig_to_new_prefix={'audio_tower.model.encoder.': 'audio_tower.'})
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        if config.audio_model_id is not None:
            self.secondary_weights.append(DefaultModelLoader.Source(model_or_path=config.audio_model_id, revision=None, prefix='audio_tower.'))
        ...
        if config.text_model_id is not None:
            self.secondary_weights.append(DefaultModelLoader.Source(model_or_path=config.text_model_id, revision=None, prefix='language_model.'))
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        """
        return MultiModelKeys.from_string_field(language_model='language_model.', connector='multi_modal_projector.', tower_model='audio_tower.')
    ...
    def _parse_and_validate_audio_input(self, **kwargs: object) -> Optional[UltravoxAudioInputs]:
        ...
        if audio_features is not None:
            ...
            if not isinstance(audio_lens, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of audio_lens. Got type: {type(audio_features)}')
            if not isinstance(audio_token_len, (torch.Tensor, list)):
                raise ValueError(f'Incorrect type of audio_token_len. Got type: {type(audio_features)}')
            return UltravoxAudioFeatureInputs(type='audio_features', data=audio_features, lens=audio_lens, token_len=audio_token_len)
    ...
    def _process_audio_input(self, audio_input: UltravoxAudioInputs) -> Union[NestedTensors, tuple[torch.Tensor, ...]]:
        ...
        embed_lens = [token_len_item.sum().item() for token_len_item in audio_input['token_len']]
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            if not envs.VLLM_USE_V1:
                ...
                merge_multimodal_embeddings_from_map(inputs_embeds, multimodal_embeddings, attn_metadata.multi_modal_placeholder_index_maps['audio'])
            else:
                inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, _AUDIO_PLACEHOLDER_TOKEN)
        return inputs_embeds

    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[torch.Tensor]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs) -> Union[torch.Tensor, IntermediateTensors]:
...
def pad_and_concat_to_dim3(features: Union[torch.Tensor, list[torch.Tensor], list[list[torch.Tensor]]]) -> torch.Tensor:
    """
