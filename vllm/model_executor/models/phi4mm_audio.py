...
from vllm.model_executor.models.phi4mm_utils import AbsolutePositionalEncoding, ConvModule, FeedForward, MeanVarianceNormLayer, MultiHeadedAttention, MultiSequential, NemoConvSubsampling, T5RelativeAttentionLogitBias, adaptive_enc_mask, get_offset, unfold_tensor
import abc
import math
import numpy as np
import torch
...
class ConformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model=512, ext_pw_out_channel=0, depthwise_seperable_out_channel=256, depthwise_multiplier=1, n_head=4, d_ffn=2048, ext_pw_kernel_size=1, kernel_size=3, dropout_rate=0.1, causal=False, batch_norm=False, activation='relu', chunk_se=0, chunk_size=18, conv_activation='relu', conv_glu_type='sigmoid', bias_in_glu=True, linear_glu_in_convm=False, attention_inner_dim=-1, attention_glu_type='swish', activation_checkpointing='', export=False, use_pt_scaled_dot_product_attention=False, attn_group_sizes: int=1):
        ...
        self.self_attn = MultiHeadedAttention(n_head, d_model, dropout_rate, attention_inner_dim, attention_glu_type, bias_in_glu, use_pt_scaled_dot_product_attention=use_pt_scaled_dot_product_attention, group_size=attn_group_sizes)
    ...
    def forward(self, x, pos_k, pos_v, mask, relative_attention_bias: Optional[Tensor]=None):
...
class TransformerEncoderBase(abc.ABC, nn.Module):
    ...
    def __init__(self, input_size, chunk_size, left_chunk, attention_dim=256, attention_heads=4, input_layer='nemo_conv', cnn_out=-1, cnn_layer_norm=False, time_reduction=4, dropout_rate=0.0, padding_idx=-1, relative_attention_bias_args=None, positional_dropout_rate=0.0, nemo_conv_settings=None, conv2d_extra_padding: Literal['feat', 'feat_time', 'none', True]='none', attention_group_size=1, encoder_embedding_config=None):
        ...
        if self.input_layer == 'nemo_conv':
            default_nemo_conv_settings = {'subsampling': 'dw_striding', 'subsampling_factor': self.time_reduction, 'feat_in': input_size, 'feat_out': attention_dim, 'conv_channels': 256, 'subsampling_conv_chunking_factor': 1, 'activation': nn.ReLU(), 'is_causal': False}
        ...
        if self.relative_attention_bias_type == 't5':
            ...
            self.relative_attention_bias_layer = T5RelativeAttentionLogitBias(self.num_heads // self.attention_group_size, max_distance=relative_attention_bias_args.get('t5_bias_max_distance', 1000), symmetric=relative_attention_bias_args.get('t5_bias_symmetric', False))
        else:
    ...
    def compute_lens_change(self, feature_lens):
        ...
        """
        if self.input_layer == 'nemo_conv':
            subsampling_causal_cond = self.nemo_conv_settings.get('subsampling', 'dw_striding') in ['dw_striding', 'striding', 'striding_conv1d']
            is_causal = self.nemo_conv_settings.get('is_causal', False)
            if is_causal and subsampling_causal_cond:
                lens_change = torch.ceil(feature_lens / self.time_reduction).long() if isinstance(feature_lens, Tensor) else math.ceil(feature_lens / self.time_reduction)
...
class WindowQformer(nn.Module):
    ...
    def __init__(self, window_size: int=8, num_queries: int=1, num_blocks: int=2, attention_dim: int=512, attention_heads: int=8, linear_units: int=2048, dropout_rate: float=0.0, normalize_before: bool=True):
        super().__init__()
        self.decoders = nn.ModuleList([nn.TransformerDecoderLayer(d_model=attention_dim, nhead=attention_heads, dim_feedforward=linear_units, dropout=dropout_rate, activation='relu', batch_first=True, norm_first=normalize_before) for _ in range(num_blocks)])
    ...
    def forward(self, audio_embed, mask, embed_len=None):
        ...
        embed_chunk = F.unfold(audio_embed[..., None, :], kernel_size=(1, self.window_size), stride=(1, self.window_size))
...
class AudioEmbedding(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, **kwargs) -> None:
        ...
        if kwargs.get('use_conv_downsample', False):
            ...
            default_nemo_conv_settings = {'subsampling': 'dw_striding', 'subsampling_factor': self.downsample_rate, 'feat_in': audio_dim_out, 'feat_out': audio_dim_out, 'conv_channels': 256, 'subsampling_conv_chunking_factor': 1, 'activation': nn.ReLU(), 'is_causal': False}
            if nemo_conv_settings:
                default_nemo_conv_settings.update(nemo_conv_settings)
                for i in ['subsampling_factor', 'feat_in', 'feat_out']:
                    assert i not in nemo_conv_settings, '{i} should be specified outside of the NeMo dictionary'
    ...
    def get_audio_features(self, input_embeds: torch.FloatTensor, audio_attention_mask: torch.Tensor=None, audio_projection_mode: str='speech') -> torch.FloatTensor:
        """
        arguments:
    ...
    def forward(self, audio_features: torch.FloatTensor, audio_attention_mask: torch.Tensor=None, audio_projection_mode: str='speech') -> torch.FloatTensor:
        """
        arguments:
