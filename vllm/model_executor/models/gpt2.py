...
class GPT2Attention(nn.Module):
    ...
    def __init__(self, config: GPT2Config, cache_config: Optional[CacheConfig]=None, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
...
class GPT2Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.wte = VocabParallelEmbedding(config.vocab_size, self.embed_dim, quant_config=quant_config, prefix=f'{prefix}.wte')
        self.wpe = nn.Embedding(config.max_position_embeddings, self.embed_dim)
        self.start_layer, self.end_layer, self.h = make_layers(config.num_hidden_layers, lambda prefix: GPT2Block(config, cache_config, quant_config, prefix=prefix), prefix=f'{prefix}.h')
    ...
    def forward(self, input_ids: torch.Tensor, position_ids: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors], inputs_embeds: Optional[torch.Tensor]) -> Union[torch.Tensor, IntermediateTensors]:
...
class GPT2LMHeadModel(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.lm_head = ParallelLMHead(self.config.vocab_size, self.config.hidden_size, quant_config=quant_config, prefix=f'{prefix}.lm_head')
...
class GPT2ForSequenceClassification(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        super().__init__()
        config = vllm_config.model_config.hf_config
        self.transformer = GPT2Model(vllm_config=vllm_config, prefix=maybe_prefix(prefix, 'gpt2'))
        self.score = nn.Linear(config.n_embd, config.num_labels, bias=False)
        pooler_config = vllm_config.model_config.pooler_config
        self.pooler = Pooler.from_config_with_defaults(pooler_config, pooling_type=PoolingType.LAST, normalize=False, softmax=True)
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None) -> torch.Tensor:
        hidden_states = self.transformer(input_ids=input_ids, position_ids=positions, inputs_embeds=inputs_embeds, intermediate_tensors=intermediate_tensors)
