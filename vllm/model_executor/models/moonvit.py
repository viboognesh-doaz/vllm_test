...
def multihead_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, q_cu_seqlens: Optional[torch.Tensor]=None, k_cu_seqlens: Optional[torch.Tensor]=None):
    ...
    assert q.dtype in [torch.bfloat16, torch.float16], f'unsupported dtype {q.dtype} for multihead attn'
...
def sdpa_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, q_cu_seqlens: Optional[torch.Tensor]=None, k_cu_seqlens: Optional[torch.Tensor]=None) -> torch.Tensor:
    """SDPA attention.
    ...
    return attn_output
VL_VISION_ATTENTION_FUNCTIONS = {'flash_attention_2': multihead_attention, 'sdpa': sdpa_attention}
    ...
    def forward(self, x: torch.Tensor, grid_hws: torch.Tensor) -> torch.Tensor:
        ...
        for shape in grid_hws.tolist():
            if shape == self.weight.shape[:-1]:
                ...
                pos_embs.append(F.interpolate(self.weight.permute((2, 0, 1)).unsqueeze(0), size=shape, mode=self.interpolation_mode).squeeze(0).permute((1, 2, 0)).flatten(end_dim=1))
        out = x + torch.cat(pos_embs)
        return out

class MoonVisionPatchEmbed(nn.Module):

    def __init__(self, out_dim: int, in_dim: int=3, patch_size: Union[int, tuple[int, int]]=(14, 14), pos_emb_height: int=14, pos_emb_width: int=14):
        super().__init__()
...
class Rope2DPosEmb(nn.Module):
    ...
    def precomputed_freqs_cis(self) -> torch.Tensor:
        ...
        dim_range = torch.arange(0, self.dim, 4)[:self.dim // 4].float().to(self.device)
    ...
    def get_freqs_cis_by_seqlens(self, grid_hws: torch.Tensor) -> torch.Tensor:
        ...
        assert all((1 <= h <= self.max_height and 1 <= w <= self.max_width for h, w in shapes)), (shapes, self.max_height, self.max_width)
        freqs_cis = torch.cat([self.precomputed_freqs_cis[:h, :w].reshape(-1, self.dim // 2) for h, w in shapes], dim=0)
        return freqs_cis
    ...
    def get_freqs_cis_by_idx(self, pos_idx: torch.Tensor, pos_idx_mask: torch.Tensor) -> torch.Tensor:
        ...
        """
        assert pos_idx.shape[:-1] == pos_idx_mask.shape and pos_idx.shape[-1] == 2 and (pos_idx.ndim == pos_idx_mask.ndim + 1), (pos_idx.shape, pos_idx_mask.shape)
        assert pos_idx_mask.dtype == torch.bool, pos_idx_mask.dtype
        shp = pos_idx_mask.shape + (self.dim // 2,)
        freqs_cis = torch.ones(shp, dtype=torch.complex64, device=self.device)
        freqs_cis[pos_idx_mask] = self.precomputed_freqs_cis[pos_idx[..., 0][pos_idx_mask], pos_idx[..., 1][pos_idx_mask]]
        return freqs_cis
...
class MoonVitEncoderLayer(nn.Module):
    ...
    def __init__(self, num_heads: int, hidden_dim: int, mlp_dim: int, *, attn_implementation: str='sdpa', activation=F.gelu, attn_bias: bool=False):
        super().__init__()
    ...
    def attention_qkvpacked(self, x: torch.Tensor, cu_seqlens: torch.Tensor, rope_freqs_cis: Optional[torch.Tensor]=None):
        """
        Args:
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor, rope_freqs_cis: Union[torch.Tensor, None]=None) -> torch.Tensor:
        """
        Args:
...
class MoonVitEncoder(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_hw: torch.Tensor) -> torch.Tensor:
        ...
        lengths = torch.cat((torch.zeros(1, device=hidden_states.device, dtype=grid_hw.dtype), grid_hw[:, 0] * grid_hw[:, 1]))
...
def patch_merger(x: torch.Tensor, grid_hw: torch.Tensor, merge_kernel_size: list[int, int]=(2, 2)) -> list[torch.Tensor]:
    d_model = x.size(-1)
    outputs = []
    pre_sum = 0
...
class MoonVitVLProjector(nn.Module):
    ...
    def __init__(self, in_channels: int, merge_kernel_size: list[int, int], hidden_act: str='gelu', ln_eps: float=1e-05, out_dim: int=4096):
        super().__init__()
...
class MoonVitPretrainedModel(PreTrainedModel):
    ...
    def __init__(self, config: MoonViTConfig, *inputs, **kwargs):
        ...
        self.patch_embed = MoonVisionPatchEmbed(out_dim=config.hidden_size, patch_size=config.patch_size, pos_emb_height=config.init_pos_emb_height, pos_emb_width=config.init_pos_emb_width)
        self.encoder = MoonVitEncoder(hidden_dim=config.hidden_size, num_layers=config.num_hidden_layers, block_cfg={'num_heads': config.num_attention_heads, 'hidden_dim': config.hidden_size, 'mlp_dim': config.intermediate_size, 'activation': PytorchGELUTanh(), 'attn_bias': True, 'attn_implementation': config._attn_implementation})
