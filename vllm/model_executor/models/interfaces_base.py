...
def _check_vllm_model_forward(model: Union[type[object], object]) -> bool:
    ...
    if missing_kws and (isinstance(model, type) and issubclass(model, nn.Module)):
        logger.warning('The model (%s) is missing vLLM-specific keywords from its `forward` method: %s', model, missing_kws)
...
def is_vllm_model(model: Union[type[object], object]) -> Union[TypeIs[type[VllmModel]], TypeIs[VllmModel]]:
...
class VllmModelForTextGeneration(VllmModel[T], Protocol[T]):
    ...
    def compute_logits(self, hidden_states: T, sampling_metadata: 'SamplingMetadata') -> Optional[T]:
        """Return `None` if TP rank > 0."""
        ...

@overload
def is_text_generation_model(model: type[object]) -> TypeIs[type[VllmModelForTextGeneration]]:
    ...
...
def is_text_generation_model(model: Union[type[object], object]) -> Union[TypeIs[type[VllmModelForTextGeneration]], TypeIs[VllmModelForTextGeneration]]:
...
def is_pooling_model(model: Union[type[object], object]) -> Union[TypeIs[type[VllmModelForPooling]], TypeIs[VllmModelForPooling]]:
    if not is_vllm_model(model):
        return False
