...
from vllm.model_executor.layers.linear import ColumnParallelLinear, QKVParallelLinear, ReplicatedLinear, RowParallelLinear
...
class Llama4VisionMLP(nn.Module):
    ...
    def __init__(self, input_size: int, intermediate_size: int, output_size: int, bias: bool, output_activation: bool, quant_config: Optional[QuantizationConfig]=None, prefix: str='', use_data_parallel: bool=False):
        super().__init__()
        cls_fc1 = ReplicatedLinear if use_data_parallel else ColumnParallelLinear
        self.fc1 = cls_fc1(input_size=input_size, output_size=intermediate_size, bias=bias, quant_config=quant_config, prefix=f'{prefix}.fc1')
        cls_fc2 = ReplicatedLinear if use_data_parallel else RowParallelLinear
        self.fc2 = cls_fc2(input_size=intermediate_size, output_size=output_size, bias=bias, quant_config=quant_config, prefix=f'{prefix}.fc2')
...
class Llama4MultiModalProjector(nn.Module):
    ...
    def __init__(self, config, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.linear_1 = ColumnParallelLinear(input_size=config.vision_config.vision_output_dim, output_size=config.text_config.hidden_size, bias=False, quant_config=quant_config, gather_output=True, prefix=f'{prefix}.linear_1')
...
def pixel_shuffle(input_tensor, shuffle_ratio):
    ...
    reshaped_tensor = reshaped_tensor.view(batch_size, int(height * shuffle_ratio), int(width * shuffle_ratio), int(channels / shuffle_ratio ** 2))
    reshaped_tensor = reshaped_tensor.permute(0, 2, 1, 3).contiguous()
...
class Llama4VisionPixelShuffleMLP(nn.Module):
    ...
    def __init__(self, config, quant_config: Optional[QuantizationConfig]=None, prefix: str='', use_data_parallel: bool=False):
        super().__init__()
        self.pixel_shuffle_ratio = config.pixel_shuffle_ratio
        self.inner_dim = int(config.projector_input_dim // self.pixel_shuffle_ratio ** 2)
        self.output_dim = config.projector_output_dim
        self.mlp = Llama4VisionMLP(input_size=config.intermediate_size, intermediate_size=config.projector_input_dim, output_size=config.projector_output_dim, bias=config.multi_modal_projector_bias, output_activation=True, quant_config=quant_config, prefix=f'{prefix}.mlp', use_data_parallel=use_data_parallel)
...
class Llama4VisionAttention(nn.Module):
    ...
    def __init__(self, config: Llama4VisionConfig, quant_config: Optional[QuantizationConfig], prefix: str='', use_data_parallel: bool=False):
        super().__init__()
        self.config = config
        ...
        if use_data_parallel:
            self.qkv_proj = ReplicatedLinear(self.embed_dim, self.q_size + 2 * self.kv_size, bias=True, quant_config=quant_config, prefix=f'{prefix}.qkv_proj')
            self.o_proj = ReplicatedLinear(self.num_heads * self.head_dim, self.embed_dim, bias=True, quant_config=quant_config, prefix=f'{prefix}.o_proj')
        else:
            self.qkv_proj = QKVParallelLinear(self.embed_dim, self.head_dim, self.num_heads, bias=True, quant_config=quant_config, prefix=f'{prefix}.qkv_proj')
            self.o_proj = RowParallelLinear(self.num_heads * self.head_dim, self.embed_dim, bias=True, input_is_parallel=True, quant_config=quant_config, prefix=f'{prefix}.o_proj')
        self.rotary_emb = get_rope(head_size=self.head_dim, rotary_dim=config.hidden_size // config.num_attention_heads // 2, max_position=(config.image_size // config.patch_size) ** 2, base=config.rope_theta, rope_scaling={'rope_type': 'mllama4'}, is_neox_style=False, dtype=torch.complex64)
...
class Llama4VisionEncoderLayer(nn.Module):
    ...
    def __init__(self, config: Llama4VisionConfig, quant_config: Optional[QuantizationConfig], prefix: str='', use_data_parallel: bool=False):
        ...
        self.self_attn = Llama4VisionAttention(config, quant_config=quant_config, prefix=f'{prefix}.self_attn', use_data_parallel=use_data_parallel)
        self.mlp = Llama4VisionMLP(input_size=config.hidden_size, intermediate_size=config.intermediate_size, output_size=config.hidden_size, bias=True, output_activation=False, quant_config=quant_config, prefix=f'{prefix}.mlp', use_data_parallel=use_data_parallel)
...
class Llama4VisionEncoder(nn.Module):
    ...
    def __init__(self, config: Llama4VisionConfig, quant_config: Optional[QuantizationConfig], prefix: str='', use_data_parallel: bool=False):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([Llama4VisionEncoderLayer(config, quant_config=quant_config, prefix=f'{prefix}.layers.{layer_idx}', use_data_parallel=use_data_parallel) for layer_idx in range(config.num_hidden_layers)])
...
class Llama4UnfoldConvolution(nn.Module):
    ...
    def __init__(self, config: Llama4VisionConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str='', use_data_parallel: bool=False):
        super().__init__()
        kernel_size = config.patch_size
        if isinstance(kernel_size, int):
            kernel_size = (kernel_size, kernel_size)
        self.unfold = torch.nn.Unfold(kernel_size=kernel_size, stride=config.patch_size)
        params = {'input_size': config.num_channels * kernel_size[0] * kernel_size[1], 'output_size': config.hidden_size, 'bias': False, 'quant_config': quant_config, 'prefix': f'{prefix}.linear'}
        if use_data_parallel:
...
class Llama4VisionModel(nn.Module):
    ...
    def __init__(self, config: Llama4VisionConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str='', use_data_parallel: bool=False):
        ...
        self.patch_embedding = Llama4UnfoldConvolution(config, quant_config=quant_config, prefix=f'{prefix}.patch_embedding', use_data_parallel=use_data_parallel)
        ...
        self.model = Llama4VisionEncoder(config, quant_config=quant_config, prefix=f'{prefix}.model', use_data_parallel=use_data_parallel)
        self.vision_adapter = Llama4VisionPixelShuffleMLP(config, quant_config, prefix=f'{prefix}.vision_adapter', use_data_parallel=use_data_parallel)
...
class Mllama4MultiModalProcessor(BaseMultiModalProcessor[Mllama4ProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        if processed_outputs.get('pixel_values') is not None:
            ...
            possible_resolutions = find_supported_resolutions(max_num_chunks=self.info.get_max_num_tiles(), patch_size=SizeDict(height=tile_size, width=tile_size))
            best_fit_sizes = [get_best_fit((image.size[1], image.size[0]), torch.tensor(possible_resolutions), resize_to_max_canvas=image_processor.resize_to_max_canvas) for image in parsed_images]
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        ...
        return dict(pixel_values=MultiModalFieldConfig.flat_from_sizes('image', patches_per_image), patches_per_image=MultiModalFieldConfig.batched('image'), aspect_ratios=MultiModalFieldConfig.batched('image'))

    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> list[PromptUpdate]:
        assert mm_items.get_count('image', strict=False) == 0 or 'aspect_ratios' in out_mm_kwargs, 'Transformers expect to include aspect_ratios in out_mm_kwargs'
        ...
        def get_replacement(item_idx: int):
            ...
            repl = hf_processor._prompt_split_image(aspect_ratio=aspect_ratio, num_patches_per_chunk=num_patches_per_chunk)
            return PromptUpdateDetails.select_text(repl, img_patch_token)
        return [PromptReplacement(modality='image', target=image_token, replacement=get_replacement)]
...
@MULTIMODAL_REGISTRY.register_processor(Mllama4MultiModalProcessor, info=Mllama4ProcessingInfo, dummy_inputs=Mllama4DummyInputsBuilder)
class Llama4ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str=''):
        ...
        self.vision_model = Llama4VisionModel(config.vision_config, None, prefix=maybe_prefix(prefix, 'vision_model'), use_data_parallel=self.use_data_parallel)
        self.multi_modal_projector = Llama4MultiModalProjector(self.config, None, prefix=maybe_prefix(prefix, 'multi_modal_projector'))
        self.language_model = initialize_model(vllm_config=vllm_config.with_hf_config(config.text_config, ['LlamaForCausalLM']), prefix=maybe_prefix(prefix, 'language_model'), model_class=Llama4ForCausalLM)
        self.make_empty_intermediate_tensors = self.language_model.make_empty_intermediate_tensors

    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[Llama4ImagePatchInputs]:
        ...
        if not isinstance(aspect_ratios, (torch.Tensor, list)):
            raise ValueError(f'Incorrect type of aspect_ratios. Got type: {type(aspect_ratios)}')
        return Llama4ImagePatchInputs(type='pixel_values', flat_data=flat_pixel_values, patches_per_image=patches_per_image, aspect_ratios=aspect_ratios)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[NestedTensors]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None and len(multimodal_embeddings) != 0:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, self.config.image_token_index)
    ...
    def separate_weights(self, weights: Iterable[tuple[str, torch.Tensor]], prefix: str) -> tuple[Iterable[tuple[str, torch.Tensor]], Iterable[tuple[str, torch.Tensor]]]:
    ...
    def _consolidate_qkv_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> Iterable[tuple[str, torch.Tensor]]:
        qkv_idx_mappings = {'.self_attn.q_proj': 0, '.self_attn.k_proj': 1, '.self_attn.v_proj': 2}
        qkv_weights = {}
