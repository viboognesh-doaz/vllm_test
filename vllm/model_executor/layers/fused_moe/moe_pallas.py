...
def fused_moe(hidden_states: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, gating_output: torch.Tensor, topk: int, global_num_experts: int, expert_map: torch.Tensor=None, renormalize: bool=False) -> torch.Tensor:
    """
    Args:
    ...
    assert num_tokens * topk % 16 == 0, f'The Pallas GMM kernel requires num_tokens * topk to be a multiple of 16 but got {num_tokens * topk}'
