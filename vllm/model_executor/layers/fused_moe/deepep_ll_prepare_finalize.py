...
from vllm.model_executor.layers.fused_moe.utils import moe_kernel_quantize_input, normalize_batched_scales_shape
import deep_ep
import torch
...
class DeepEPLLPrepareAndFinalize(mk.FusedMoEPrepareAndFinalize):
    ...
    def __init__(self, buffer: deep_ep.Buffer, max_tokens_per_rank: int, num_dispatchers: int, use_fp8_dispatch: bool=False):
        super().__init__()
        self.buffer = buffer
    ...
    def _do_quant(self, x: Union[torch.Tensor, tuple[torch.Tensor, torch.Tensor]], a1_scale: Optional[torch.Tensor], a2_scale: Optional[torch.Tensor], a1_dtype: torch.dtype, quant_dtype: Optional[torch.dtype], per_act_token_quant: bool, block_shape: Optional[list[int]]) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        x, x_scales = moe_kernel_quantize_input(x, a1_scale, quant_dtype, per_act_token_quant, block_shape)
    ...
    def prepare(self, a1: torch.Tensor, a1_scale: Optional[torch.Tensor], a2_scale: Optional[torch.Tensor], topk_weights: torch.Tensor, topk_ids: torch.Tensor, num_experts: int, expert_map: Optional[torch.Tensor], apply_router_weight_on_input: bool, quant_config: FusedMoEQuantConfig, extra_prepare_args: Optional[dict[str, Any]]) -> tuple[torch.Tensor, Optional[torch.Tensor], Optional[mk.ExpertTokensMetadata], Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        assert hidden_size in self.SUPPORTED_HIDDEN_SIZES, f'Hidden Size {hidden_size} not in supported list of hidden sizes{self.SUPPORTED_HIDDEN_SIZES}'
        if self.use_fp8_dispatch:
            assert hidden_size % 128 == 0, 'DeepEP kernels quantize the inputs in blocks of shape 128'
        has_per_token_scales = a1_scale.numel() != 1 if a1_scale is not None else a2_scale.numel() != 1 if a2_scale is not None else False
        ...
        expert_x, expert_num_tokens, self.handle, event, hook = self.buffer.low_latency_dispatch(a1, topk_ids, self.max_tokens_per_rank, num_experts, use_fp8=self.use_fp8_dispatch, async_finish=False, return_recv_hook=False)
        expert_x, expert_x_scale = self._do_quant(expert_x, a1_scale, a2_scale, a1.dtype, quant_config.quant_dtype, quant_config.per_act_token_quant, quant_config.block_shape)
    ...
    def finalize(self, output: torch.Tensor, fused_expert_output: torch.Tensor, topk_weights: torch.Tensor, topk_ids: torch.Tensor, apply_router_weight_on_input: bool, weight_and_reduce_impl: mk.TopKWeightAndReduce, extra_finalize_args: Optional[dict[str, Any]]) -> None:
        ...
        _, event, hook = self.buffer.low_latency_combine(fused_expert_output, topk_ids, combine_topk_weights, self.handle, async_finish=False, zero_copy=False, return_recv_hook=False, out=output)