...
from vllm.model_executor.layers.fused_moe.fused_moe import get_config_dtype_str, try_get_optimal_moe_config
from vllm.model_executor.layers.fused_moe.topk_weight_and_reduce import TopKWeightAndReduceDelegate, TopKWeightAndReduceNaiveBatched
from vllm.model_executor.layers.fused_moe.utils import _resize_cache, moe_kernel_quantize_input, normalize_batched_scales_shape, normalize_scales_shape
...
@triton.jit
def moe_mmk(a_ptrs, b_ptrs, K, expert_id, a_scale_ptr, b_scale_ptr, stride_ak: tl.int64, stride_bk: tl.int64, stride_ase: tl.int64, stride_asm: tl.int64, stride_ask: tl.int64, stride_bse: tl.int64, stride_bsk: tl.int64, stride_bsn: tl.int64, offs_m, offs_n, offs_bn, mask_m, group_n: tl.constexpr, group_k: tl.constexpr, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr, compute_type: tl.constexpr, use_w8a8: tl.constexpr, use_w8a16: tl.constexpr, per_act_token_quant: tl.constexpr):
    ...
    for k in range(0, tl.cdiv(K, BLOCK_K)):
        a = tl.load(a_ptrs, mask=mask_m[:, None] & (offs_k[None, :] < K - k * BLOCK_K), other=0.0)
...
class BatchedPrepareAndFinalize(mk.FusedMoEPrepareAndFinalize):
    ...
    def prepare(self, a1: torch.Tensor, a1_scale: Optional[torch.Tensor], a2_scale: Optional[torch.Tensor], topk_weights: torch.Tensor, topk_ids: torch.Tensor, num_experts: int, expert_map: Optional[torch.Tensor], apply_router_weight_on_input: bool, quant_config: FusedMoEQuantConfig, extra_prepare_args: Optional[dict[str, Any]]) -> tuple[torch.Tensor, Optional[torch.Tensor], Optional[mk.ExpertTokensMetadata], Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        b_a1 = torch.zeros((num_local_experts, self.max_num_tokens, hidden_dim), dtype=b_type, device=a1.device)
        ...
        for expert_id in range(first_expert, last_expert):
            ...
            if quant_config.quant_dtype is not None:
                if a1_scale is not None:
                    ...
                    rhs_a1_scale = None
                b_a1[idx, :rows, :], b_s = moe_kernel_quantize_input(rhs, rhs_a1_scale, quant_config.quant_dtype, quant_config.per_act_token_quant, quant_config.block_shape)
    ...
    def finalize(self, output: torch.Tensor, fused_expert_output: torch.Tensor, topk_weights: torch.Tensor, topk_ids: torch.Tensor, apply_router_weight_on_input: bool, weight_and_reduce_impl: mk.TopKWeightAndReduce, extra_finalize_args: Optional[dict[str, Any]]) -> None:
        ...
        weight_and_reduce_impl.apply(output=output, fused_expert_output=fused_expert_output, topk_weights=topk_weights, topk_ids=topk_ids, apply_router_weight_on_input=apply_router_weight_on_input)
...
class NaiveBatchedExperts(mk.FusedMoEPermuteExpertsUnpermute):
    ...
    def __init__(self, max_num_tokens: int, num_dispatchers: int, use_fp8_w8a8: bool=False, use_int8_w8a8: bool=False, use_int8_w8a16: bool=False, use_int4_w4a16: bool=False, use_mxfp4_w4a4: bool=False, block_shape: Optional[list[int]]=None, per_act_token_quant: bool=False):
...
def batched_moe_kernel_quantize_input(A: torch.Tensor, A_scale: Optional[torch.Tensor], num_tokens: int, E: int, N: int, expert_num_tokens: torch.Tensor, qtype: Optional[torch.dtype], per_act_token_quant: bool, block_shape: Optional[list[int]]=None) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
    if torch.compiler.is_compiling() or torch.cuda.is_current_stream_capturing():
        hidden_dim = A.size(-1)
...
class BatchedTritonExperts(mk.FusedMoEPermuteExpertsUnpermute):
    ...
    def __init__(self, max_num_tokens: int, num_dispatchers: int, use_fp8_w8a8: bool=False, use_int8_w8a8: bool=False, use_int8_w8a16: bool=False, use_int4_w4a16: bool=False, use_mxfp4_w4a4: bool=False, per_act_token_quant: bool=False, block_shape: Optional[list[int]]=None):
        super().__init__(FusedMoEQuantConfig.make(use_fp8_w8a8=use_fp8_w8a8, use_int8_w8a8=use_int8_w8a8, use_int8_w8a16=use_int8_w8a16, use_int4_w4a16=use_int4_w4a16, use_mxfp4_w4a4=use_mxfp4_w4a4, per_act_token_quant=per_act_token_quant, block_shape=block_shape))
        assert not use_int8_w8a8, 'NYI'
        assert not use_int8_w8a16, 'NYI'
        assert not use_int4_w4a16, 'NYI'
    ...
    def apply(self, output: torch.Tensor, hidden_states: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, topk_weights: torch.Tensor, topk_ids: torch.Tensor, activation: str, global_num_experts: int, expert_map: Optional[torch.Tensor], w1_scale: Optional[torch.Tensor], w2_scale: Optional[torch.Tensor], w1_zp: Optional[torch.Tensor], w2_zp: Optional[torch.Tensor], a1q_scale: Optional[torch.Tensor], a2_scale: Optional[torch.Tensor], workspace13: torch.Tensor, workspace2: torch.Tensor, expert_tokens_meta: Optional[mk.ExpertTokensMetadata], apply_router_weight_on_input: bool, extra_expert_args: Optional[dict[str, Any]]):
        ...
        assert expert_tokens_meta is not None
        expert_num_tokens = expert_tokens_meta.expert_num_tokens
        E, max_num_tokens, N, K, top_k_num = mk._moe_problem_size(hidden_states, w1, w2, topk_ids)
        assert w1.size(0) == E
        assert w2.size(0) == E
        ...
        config = try_get_optimal_moe_config(w1.size(), w2.size(), top_k_num, config_dtype, max_num_tokens, block_shape=self.block_shape)
        ...
        invoke_moe_batched_triton_kernel(A=hidden_states, B=w1, C=intermediate_cache1, expert_num_tokens=expert_num_tokens, compute_type=compute_type, A_scale=a1q_scale, B_scale=w1_scale, B_zp=w1_zp, use_fp8_w8a8=self.use_fp8_w8a8, use_int8_w8a16=self.use_int8_w8a16, use_int4_w4a16=self.use_int4_w4a16, config=config, per_act_token_quant=self.per_act_token_quant, block_shape=self.block_shape)
        intermediate_cache2.fill_(0)
        self.activation(activation, intermediate_cache2.view(-1, N // 2), intermediate_cache1.view(-1, N))
        qintermediate_cache2, a2q_scale = batched_moe_kernel_quantize_input(intermediate_cache2, a2_scale, max_num_tokens, E, N, expert_num_tokens, self.quant_dtype, self.per_act_token_quant, self.block_shape)
