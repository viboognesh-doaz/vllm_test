...
class TopKWeightAndReduceNoOP(mk.TopKWeightAndReduce):
    ...
    def apply(self, output: Optional[torch.Tensor], fused_expert_output: torch.Tensor, topk_weights: torch.Tensor, topk_ids: torch.Tensor, apply_router_weight_on_input: bool) -> torch.Tensor:
        ...
        assert output.size() == fused_expert_output.size(), f'output shape is expected to match the fused_expert_output shape. But got output={output.size()}, used_expert_output={fused_expert_output.size()}'
...
class TopKWeightAndReduceContiguous(mk.TopKWeightAndReduce):
    ...
    def apply(self, output: Optional[torch.Tensor], fused_expert_output: torch.Tensor, topk_weights: torch.Tensor, topk_ids: torch.Tensor, apply_router_weight_on_input: bool) -> torch.Tensor:
        ...
        assert fused_expert_output.size() == (m, num_topk, k), f'Expected fused_expert_output size {(m, num_topk, k)}. But got {fused_expert_output.size()}'
        if not apply_router_weight_on_input:
            fused_expert_output.mul_(topk_weights.view(m, -1, 1))
        if output is None:
            output = torch.empty((m, k), device=fused_expert_output.device, dtype=fused_expert_output.dtype)
