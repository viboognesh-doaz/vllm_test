...
def _chunk_scan_fwd_kernel(cb_ptr, x_ptr, z_ptr, out_ptr, out_x_ptr, dt_ptr, dA_cumsum_ptr, seq_idx_ptr, C_ptr, states_ptr, D_ptr, initstates_ptr, chunk_indices_ptr, chunk_offsets_ptr, chunk_meta_num, chunk_size, hdim, dstate, batch, seqlen, nheads_ngroups_ratio, stride_cb_batch, stride_cb_chunk, stride_cb_head, stride_cb_csize_m, stride_cb_csize_k, stride_x_batch, stride_x_seqlen, stride_x_head, stride_x_hdim, stride_z_batch, stride_z_seqlen, stride_z_head, stride_z_hdim, stride_out_batch, stride_out_seqlen, stride_out_head, stride_out_hdim, stride_dt_batch, stride_dt_chunk, stride_dt_head, stride_dt_csize, stride_dA_cs_batch, stride_dA_cs_chunk, stride_dA_cs_head, stride_dA_cs_csize, stride_seq_idx_batch, stride_seq_idx_seqlen, stride_C_batch, stride_C_seqlen, stride_C_head, stride_C_dstate, stride_states_batch, stride_states_chunk, stride_states_head, stride_states_hdim, stride_states_dstate, stride_init_states_batch, stride_init_states_head, stride_init_states_hdim, stride_init_states_dstate, stride_D_head, IS_CAUSAL: tl.constexpr, HAS_D: tl.constexpr, D_HAS_HDIM: tl.constexpr, HAS_Z: tl.constexpr, HAS_SEQ_IDX: tl.constexpr, BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, BLOCK_SIZE_K: tl.constexpr, BLOCK_SIZE_DSTATE: tl.constexpr, IS_TRITON_22: tl.constexpr, HAS_INITSTATES: tl.constexpr):
    ...
    cb_ptr += pid_b * stride_cb_batch + c_idx * stride_cb_chunk + pid_h // nheads_ngroups_ratio * stride_cb_head
    ...
    C_ptr += pid_b * stride_C_batch + c_idx * chunk_size * stride_C_seqlen + pid_h // nheads_ngroups_ratio * stride_C_head
    ...
    if HAS_SEQ_IDX:
        ...
        seq_idx_prev = tl.load(seq_idx_ptr - stride_seq_idx_seqlen, mask=c_idx >= 1, other=0)
        if HAS_INITSTATES:
            if pid_m * BLOCK_SIZE_M + c_off < chunk_size_limit:
                seq_idx_m = tl.load(seq_idx_ptr + (pid_m * BLOCK_SIZE_M + c_off) * stride_seq_idx_seqlen)
    ...
    if HAS_INITSTATES:
        ...
        c_idx_n = tl.load(chunk_indices_ptr + (pid_c + 1), mask=pid_c > -1 and pid_c + 1 < chunk_meta_num, other=-1)
        if c_idx == c_idx_n or c_off > 0:
            c_off_n = tl.load(chunk_offsets_ptr + (pid_c + 1), mask=pid_c > -1 and pid_c + 1 < chunk_meta_num, other=chunk_size)
            if c_idx == c_idx_n:
                chunk_size_limit = min(c_off_n, chunk_size_limit)
            dA_cs_m_boundary = tl.load(dA_cumsum_ptr + (pid_m * BLOCK_SIZE_M + c_off - 1) * stride_dA_cs_csize, mask=pid_m * BLOCK_SIZE_M + c_off - 1 > -1 and pid_m * BLOCK_SIZE_M + c_off < chunk_size, other=0.0).to(tl.float32)
    if HAS_SEQ_IDX:
        if not HAS_INITSTATES:
...
def _chunk_scan_fwd(cb, x, dt, dA_cumsum, C, states, D=None, z=None, seq_idx=None, chunk_indices=None, chunk_offsets=None, initial_states=None):
    ...
    if seq_idx is not None:
        ...
        if initial_states is not None:
            ...
            if initial_states.shape[0] == 1:
                ...
                assert chunk_indices is not None and chunk_offsets is not None, 'chunk_indices and chunk_offsets should have been set'
        else:
