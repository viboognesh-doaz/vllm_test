...
class IPEXConfig(QuantizationConfig):
    ...
    including AWQ, GPTQ.
    """
    IPEX_QUANT_METHOD_MAP = {'awq': 1, 'gptq': 0}

    def __init__(self, method: str, weight_bits: int, group_size: int, modules_to_not_convert: Optional[list[str]]=None, desc_act: Optional[bool]=None, lm_head_quantized: Optional[bool]=None) -> None:
        super().__init__()
        self.method = method
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        try:
            if ipex.__version__ < MIN_IPEX_VERSION:
                raise ImportError(f'intel_extension_for_pytorch version is wrong. Please install intel_extension_for_pytorch>={MIN_IPEX_VERSION}.')
        except ImportError as err:
            raise ImportError(f'Please install intel_extension_for_pytorch>={MIN_IPEX_VERSION} via `pip install intel_extension_for_pytorch>={MIN_IPEX_VERSION}` to use IPEX-AWQ linear method.') from err
        lowp_mode = ipex.quantization.WoqLowpMode.INT8
        weight_dtype = ipex.quantization.WoqWeightDtype.INT4
        act_quant_mode = ipex.quantization.WoqActQuantMode.PER_BATCH_IC_BLOCK
        qconfig = ipex.quantization.get_weight_only_quant_qconfig_mapping(weight_dtype=weight_dtype, lowp_mode=lowp_mode, act_quant_mode=act_quant_mode, group_size=self.quant_config.group_size)
...
class IPEXAWQLinearMethod(AWQLinearMethod):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        layer.ipex_qlinear = ipex.llm.quantization.woq_linear.IPEXWeightOnlyQuantizedLinear.from_weight(layer.qweight, layer.scales, layer.qzeros, layer.qweight.size(0), layer.ipex_output_size, qconfig=qconfig, bias=bias, group_size=self.quant_config.group_size, quant_method=IPEXConfig.IPEX_QUANT_METHOD_MAP['awq'])
