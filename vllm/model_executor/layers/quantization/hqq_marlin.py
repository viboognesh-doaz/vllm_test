...
from vllm.model_executor.layers.quantization.utils.marlin_utils import GPTQ_MARLIN_MAX_PARALLEL, GPTQ_MARLIN_MIN_THREAD_N, marlin_make_empty_g_idx, marlin_permute_scales
...
from vllm.model_executor.parameter import BasevLLMParameter, GroupQuantScaleParameter, PackedvLLMParameter
...
class HQQMarlinConfig(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int, group_size: int, skip_modules: Optional[list[str]]=None) -> None:
        super().__init__()
...
class HQQweightParameter(PackedvLLMParameter):
    ...
    def unpack_4bit_u8(self, W_q: torch.Tensor) -> torch.Tensor:
...
class HQQMarlinMethod(LinearMethodBase):
    ...
    def __init__(self, quant_config: HQQMarlinConfig):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs) -> None:
        ...
        qweight = HQQweightParameter(data=torch.empty(self.input_size_per_partition // self.quant_config.pack_factor, self.output_size_per_partition, dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=0, packed_factor=self.quant_config.pack_factor, weight_bits=self.quant_config.weight_bits, weight_loader=weight_loader)
        zeros = HQQZeroScaleParameter(data=torch.empty(self.output_size_per_partition, self.scales_and_zp_size, dtype=params_dtype), input_dim=1, output_dim=0, weight_loader=weight_loader)
        scales = HQQZeroScaleParameter(data=torch.empty(self.output_size_per_partition, self.scales_and_zp_size, dtype=params_dtype), input_dim=1, output_dim=0, weight_loader=weight_loader)
        layer.register_parameter('W_q', qweight)
        layer.register_parameter('zero', zeros)
        layer.register_parameter('scale', scales)
        ignore_parameters = ('axis', 'channel_wise', 'compute_dtype', 'encoded_state_dict', 'group_size', 'nbits', 'offload_meta', 'optimize', 'packing', 'quant_scale', 'quant_zero', 'round_zero', 'shape', 'stores_quant_config', 'unpack_view_dtype', 'view_as_float')
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        marlin_w_q = ops.gptq_marlin_repack(layer.W_q, sort_indices, self.input_size_per_partition, self.output_size_per_partition, self.quant_config.weight_bits).to(dev)
        marlin_s = marlin_permute_scales(layer.scale.transpose(1, 0), self.input_size_per_partition, self.output_size_per_partition, self.quant_config.group_size).to(dev)
        marlin_zp = marlin_permute_scales(layer.zero.transpose(1, 0), self.input_size_per_partition, self.output_size_per_partition, self.quant_config.group_size).to(dev)
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        workspace = MarlinWorkspace(self.output_size_per_partition, GPTQ_MARLIN_MIN_THREAD_N, GPTQ_MARLIN_MAX_PARALLEL)
        ...
        marlin_out = ops.gptq_marlin_gemm(x, None, layer.marlin_qweight, scales, None, zeros, layer.g_idx, layer.g_idx_sort_indices, workspace.scratch, scalar_types.uint4, x.shape[0], self.output_size_per_partition, self.input_size_per_partition, True, False, True, True)
        if orig_type != torch.float16:
