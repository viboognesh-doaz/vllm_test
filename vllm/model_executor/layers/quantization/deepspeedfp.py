...
class DeepSpeedFPConfig(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int=8, group_size: int=512) -> None:
        super().__init__()
        self.weight_bits = weight_bits
        self.group_size = group_size
        self.valid_types = [torch.bfloat16, torch.float16]
        if self.weight_bits not in (6, 8):
            raise ValueError(f'Currently, only 6-bit or 8-bit weight quantization are supported for DeepSpeed FP quantizaiton, but got {self.weight_bits} bits.')
...
class DeepSpeedFPLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, weight_loader=None, **extra_weight_attrs):
        del output_size
        del input_size
        output_size_per_partition = sum(output_partition_sizes)
        weight = DeepSpeedFPParameter(torch.Size((output_size_per_partition, input_size_per_partition)), params_dtype=params_dtype, quant_config=self.quant_config)
...
class DeepSpeedFPParameter(nn.Parameter):
    ...
    def __new__(cls, orig_shape: torch.Size, params_dtype: torch.dtype, quant_config: DeepSpeedFPConfig):
        try:
            if deepspeed.__version__ < '0.14.2':
                raise ImportError('deepspeed version is wrong. Please install deepspeed>=0.14.2.')
        except ImportError as err:
            raise ImportError('Please install deepspeed>=0.14.2 via `pip install deepspeed>=0.14.2` to use deepspeedfp quantizer.') from err
        data = torch.empty((orig_shape.numel() // quant_config.group_size, quant_config.group_size * quant_config.weight_bits // 8 + 4), dtype=torch.int8)
    ...
    def ds_quantize_(self, tensor: torch.Tensor):
        ...
        return self.data.copy_(self.fp_quantizer.quantize(tensor.data, q_bits=self.quant_config.weight_bits))
    ...
    def ds_selective_dequantize(self, indices, fp_out=None) -> torch.Tensor:
        ...
        """
        assert self.data.device.type == 'cuda' and self.data.dtype == torch.int8
        return self.fp_quantizer.selective_dequantize(self.data, indices, fp_out=fp_out, q_bits=self.quant_config.weight_bits)