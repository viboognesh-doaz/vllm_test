...
class QuarkMoEMethod(FusedMoEMethodBase):
    ...
    @staticmethod
    def get_moe_method(quant_config: 'QuarkConfig', module: torch.nn.Module, layer_name: str) -> 'QuarkMoEMethod':
        ...
        if layer_quant_config.get('output_tensors') or layer_quant_config.get('bias'):
            raise NotImplementedError('Currently, Quark models with output_tensors and bias quantized are not supported')
...
class QuarkW8A8Fp8MoEMethod(QuarkMoEMethod):
    ...
    def __init__(self, weight_config: dict[str, Any], input_config: dict[str, Any]):
        ...
        if not (weight_qscheme == 'per_tensor' and input_qscheme == 'per_tensor'):
            raise ValueError(f'For FP8 Fused MoE layers, only per-tensor scales for weights and activations are supported. Found {weight_qscheme}, {input_qscheme}')
...
class QuarkW4A4MXFp4MoEMethod(QuarkMoEMethod):
    ...
    def __init__(self, weight_config: dict[str, Any], input_config: dict[str, Any]):
        ...
        if not (weight_qscheme == 'per_group' and input_qscheme == 'per_group'):
            raise ValueError(f'For MX(FP4) Fused MoE layers, only per-group scales for weights and activations are supported. Found {weight_qscheme}, {input_qscheme}')
        self.static_input_scales = not self.input_quant.get('is_dynamic')
        if self.static_input_scales:
            raise NotImplementedError('QuarkW4A4MXFp4MoEMethod with static input scales is currently not implemented. Please open an issue.')
        if not current_platform.supports_mx():
            self.emulate = True
            logger.warning_once('The current platform does not support native MXFP4 computation. Simulated weight dequantization and activation QDQ (quantize and dequantize) will be used, with the linear layers computed in high precision.')
        else:
            self.emulate = True
            logger.warning_once('The current platform supports native MXFP4 computation, but kernels are not yet integrated in vLLM. Simulated weight dequantization and activation QDQ (quantize and dequantize) will be used, with the linear layers computed in high precision.')

    def create_weights(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
        extra_weight_attrs.update({'quant_method': FusedMoeWeightScaleSupported.BLOCK.value})
        params_dtype = torch.uint8
        w13_weight = torch.nn.Parameter(torch.empty(num_experts, 2 * intermediate_size_per_partition, hidden_size // 2, dtype=params_dtype), requires_grad=False)
        layer.register_parameter('w13_weight', w13_weight)
        set_weight_attrs(w13_weight, extra_weight_attrs)
        w2_weight = torch.nn.Parameter(torch.empty(num_experts, hidden_size, intermediate_size_per_partition // 2, dtype=params_dtype), requires_grad=False)
        layer.register_parameter('w2_weight', w2_weight)
        set_weight_attrs(w2_weight, extra_weight_attrs)
        w13_weight_scale = torch.nn.Parameter(torch.ones(num_experts, 2 * intermediate_size_per_partition, hidden_size // OCP_MX_BLOCK_SIZE, dtype=params_dtype), requires_grad=False)
        w2_weight_scale = torch.nn.Parameter(torch.ones(num_experts, hidden_size, intermediate_size_per_partition // OCP_MX_BLOCK_SIZE, dtype=params_dtype), requires_grad=False)
        set_weight_attrs(w2_weight_scale, extra_weight_attrs)
        set_weight_attrs(w13_weight_scale, extra_weight_attrs)
        layer.register_parameter('w13_weight_scale', w13_weight_scale)
        layer.register_parameter('w2_weight_scale', w2_weight_scale)
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, router_logits: torch.Tensor, top_k: int, renormalize: bool, use_grouped_topk: bool=False, topk_group: Optional[int]=None, num_expert_group: Optional[int]=None, global_num_experts: int=-1, expert_map: Optional[torch.Tensor]=None, custom_routing_function: Optional[Callable]=None, scoring_func: str='softmax', e_score_correction_bias: Optional[torch.Tensor]=None, apply_router_weight_on_input: bool=False, activation: str='silu', enable_eplb: bool=False, expert_load_view: Optional[torch.Tensor]=None, logical_to_physical_map: Optional[torch.Tensor]=None, logical_replica_count: Optional[torch.Tensor]=None) -> torch.Tensor:
        if enable_eplb:
            raise NotImplementedError('EPLB not supported for `QuarkW4A4MXFp4MoEMethod` yet.')
        topk_weights, topk_ids = FusedMoE.select_experts(hidden_states=x, router_logits=router_logits, use_grouped_topk=use_grouped_topk, top_k=top_k, renormalize=renormalize, topk_group=topk_group, num_expert_group=num_expert_group, custom_routing_function=custom_routing_function, scoring_func=scoring_func, e_score_correction_bias=e_score_correction_bias)
        out = fused_experts(x, layer.w13_weight, layer.w2_weight, topk_weights=topk_weights, topk_ids=topk_ids, inplace=True, use_mxfp4_w4a4=True, global_num_experts=global_num_experts, apply_router_weight_on_input=apply_router_weight_on_input, expert_map=expert_map, w1_scale=layer.w13_weight_scale, w2_scale=layer.w2_weight_scale, a1_scale=None, a2_scale=None, block_shape=None, activation=activation)
        return out