...
def query_marlin_supported_quant_types(has_zp: Optional[bool]=None, include_fp_type: bool=True, device_capability: Optional[int]=None):
...
def _check_marlin_supported(quant_type: ScalarType, group_size: Optional[int], has_zp: bool, device_capability: Optional[int]=None) -> tuple[bool, Optional[str]]:
    if device_capability is None:
        capability_tuple = current_platform.get_device_capability()
        device_capability = -1 if capability_tuple is None else capability_tuple.to_int()
    supported_types = query_marlin_supported_quant_types(has_zp, True, device_capability)
    if quant_type not in supported_types:
        return (False, f'Marlin does not support weight_bits = {quant_type}. Only types = {supported_types} are supported (for group_size = {group_size}, device_capability = {device_capability}, zp = {has_zp}).')
    if group_size is None or group_size not in MARLIN_SUPPORTED_GROUP_SIZES:
        return (False, f'Marlin does not support group_size = {group_size}. Only group_sizes = {MARLIN_SUPPORTED_GROUP_SIZES} are supported.')
    return (True, None)

def check_marlin_supported(quant_type: ScalarType, group_size: int, has_zp: bool=False, device_capability: Optional[int]=None) -> bool:
...
def verify_marlin_supports_shape(output_size_per_partition: int, input_size_per_partition: int, input_size: int, group_size: int) -> None:
    if output_size_per_partition % GPTQ_MARLIN_MIN_THREAD_N != 0:
        raise ValueError(f'Weight output_size_per_partition = {output_size_per_partition} is not divisible by  min_thread_n = {GPTQ_MARLIN_MIN_THREAD_N}. Consider reducing tensor_parallel_size or running with --quantization gptq.')
    if input_size_per_partition % GPTQ_MARLIN_MIN_THREAD_K != 0:
        raise ValueError(f'Weight input_size_per_partition = {input_size_per_partition} is not divisible by min_thread_k = {GPTQ_MARLIN_MIN_THREAD_K}. Consider reducing tensor_parallel_size or running with --quantization gptq.')
    if group_size < input_size and input_size_per_partition % group_size != 0:
        raise ValueError(f'Weight input_size_per_partition = {input_size_per_partition} is not divisible by group_size = {group_size}. Consider reducing tensor_parallel_size or running with --quantization gptq.')

def check_marlin_supports_shape(output_size_per_partition: int, input_size_per_partition: int, input_size: int, group_size: int) -> tuple[bool, Optional[str]]:
    try:
...
def check_marlin_supports_layer(layer: LinearBase, group_size: int) -> bool:
    ...
    return check_marlin_supports_shape(output_size_per_partition=output_size_per_partition, input_size_per_partition=input_size_per_partition, input_size=layer.input_size, group_size=group_size)[0]
...
def marlin_moe_permute_scales(s: torch.Tensor, size_k: int, size_n: int, group_size: int):
    num_experts = s.shape[0]
    output = torch.empty((num_experts, s.shape[1], s.shape[2]), device=s.device, dtype=s.dtype)
...
def moe_awq_to_marlin_zero_points(q_zp_packed: torch.Tensor, size_k: int, size_n: int, num_bits: int):
    ...
    output = torch.empty((num_experts, q_zp_packed.shape[1], q_zp_packed.shape[2]), device=q_zp_packed.device, dtype=q_zp_packed.dtype)
    for e in range(num_experts):
...
def maybe_warn_marlin_atomic_add(device, dtype):
    ...
    if device_capability[0] < 9 and dtype == torch.bfloat16:
        logger.info_once('You are running Marlin kernel with bf16 on GPUs before SM90. You can consider change to fp16 to achieve better performance if possible.')
...
def maybe_warn_marlin_atomic_add_env():
    ...
    if envs.VLLM_MARLIN_USE_ATOMIC_ADD:
        return
    logger.info_once('Marlin kernel can achieve better performance for small size_n with experimental use_atomic_add feature. You can consider set environment variable VLLM_MARLIN_USE_ATOMIC_ADD to 1 if possible.')
...
def apply_gptq_marlin_linear(input: torch.Tensor, weight: torch.Tensor, weight_scale: torch.Tensor, weight_zp: torch.Tensor, g_idx: torch.Tensor, g_idx_sort_indices: torch.Tensor, workspace: torch.Tensor, wtype: ScalarType, output_size_per_partition: int, input_size_per_partition: int, is_k_full: bool, bias: Optional[torch.Tensor]=None, use_fp32_reduce: bool=USE_FP32_REDUCE_DEFAULT) -> torch.Tensor:
    ...
    use_atomic_add = should_use_atomic_add_reduce(m=reshaped_x.size(0), n=output_size_per_partition, k=reshaped_x.size(1), device=input.device, dtype=input.dtype)
    output = ops.gptq_marlin_gemm(reshaped_x, None, weight, weight_scale, None, weight_zp, g_idx, g_idx_sort_indices, workspace, wtype, size_m=reshaped_x.shape[0], size_n=output_size_per_partition, size_k=input_size_per_partition, is_k_full=is_k_full, use_atomic_add=use_atomic_add, use_fp32_reduce=use_fp32_reduce, is_zp_float=False)
    if bias is not None:
        output.add_(bias)
    return output.reshape(out_shape)

def apply_awq_marlin_linear(input: torch.Tensor, weight: torch.Tensor, weight_scale: torch.Tensor, weight_zp: torch.Tensor, g_idx: torch.Tensor, g_idx_sort_indices: torch.Tensor, workspace: torch.Tensor, quant_type: ScalarType, output_size_per_partition: int, input_size_per_partition: int, bias: Optional[torch.Tensor]=None, use_fp32_reduce: bool=USE_FP32_REDUCE_DEFAULT) -> torch.Tensor:
    reshaped_x = input.reshape(-1, input.shape[-1])
    out_shape = input.shape[:-1] + (output_size_per_partition,)
    use_atomic_add = should_use_atomic_add_reduce(m=reshaped_x.size(0), n=output_size_per_partition, k=reshaped_x.size(1), device=input.device, dtype=input.dtype)
    output = ops.gptq_marlin_gemm(reshaped_x, None, weight, weight_scale, None, weight_zp, g_idx, g_idx_sort_indices, workspace, quant_type, size_m=reshaped_x.shape[0], size_n=output_size_per_partition, size_k=input_size_per_partition, use_atomic_add=use_atomic_add, use_fp32_reduce=use_fp32_reduce, is_zp_float=False)
    if bias is not None:
        output.add_(bias)
    return output.reshape(out_shape)