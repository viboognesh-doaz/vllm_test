...
def _normalize_quant_group_shape(x: torch.Tensor, group_shape: GroupShape):
    return (group_shape[0] if group_shape[0] > 0 else x.shape[-2], group_shape[1] if group_shape[1] > 0 else x.shape[-1])
...
def scaled_quantize(x: torch.Tensor, group_shape: GroupShape, quant_dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
    group_shape = _normalize_quant_group_shape(x, group_shape)
    assert quant_dtype.is_floating_point, 'currently `scaled_quantize` only supports floating point dtypes but could be extended to support other dtypes'
    ...
    x_scl_sat = (x_blkd_permd * scale.unsqueeze(-1)).clamp(min=finfo.min, max=finfo.max).reshape(blk_m, blk_n, group_shape[0], group_shape[1]).permute(0, 2, 1, 3).reshape(x.shape)
    return (x_scl_sat.to(quant_dtype).contiguous(), scale.float().reciprocal())

def scaled_dequantize(x_q: torch.Tensor, x_s: torch.Tensor, group_shape: Optional[GroupShape]=None, out_dtype: torch.dtype=torch.float32) -> tuple[torch.Tensor, torch.Tensor]:
    if group_shape is not None:
        group_shape = _normalize_quant_group_shape(x_q, group_shape)
    if x_s.ndim == 0:
        x_s = x_s.unsqueeze(-1).unsqueeze(-1)
    if x_s.ndim == 1:
        if group_shape is None:
            raise AssertionError('if x_s is 1D tensor, group_shape must be provided otherwise its ambiguous which dimension to broadcast x_s to')
        if group_shape[0] == x_q.shape[-2]:
            x_s = x_s.unsqueeze(-2)
        elif group_shape[1] == x_q.shape[-1]:
            x_s = x_s.unsqueeze(-1)
        else:
            raise AssertionError('if x_s is a vector we should be broadcasting it to the full extent of one of the dimensions')
    if group_shape is not None:
...
def is_layer_skipped(prefix: str, ignored_layers: list[str], fused_mapping: Mapping[str, list[str]]=MappingProxyType({})) -> bool:
...
def permute_rows(q_w: torch.Tensor, w_ref: torch.Tensor, group_size: int, test_perm: Optional[torch.Tensor]=None):
    ...
    return (w_ref.to(device=orig_device), q_w.to(device=orig_device), g_idx.to(device=orig_device), rand_perm.to(device=orig_device))

def quantize_weights(w: torch.Tensor, quant_type: ScalarType, group_size: Optional[int], zero_points: bool=False, ref_zero_points_after_scales: bool=False):
    assert quant_type.is_integer(), 'Floating point quantization may work but has not been tested'
    assert not zero_points or group_size is not None, 'to have group zero points, group_size must be provided (-1 group_size is channelwise)'
    ...
    if group_size is not None:
        if zero_points:
            ...
            w_s = (max_val - min_val).clamp(min=1e-05) / quant_type.max()
            maybe_w_zp = torch.round(torch.abs(min_val / w_s)).clamp(min_q_val, max_q_val).int()
        else:
            w_s = torch.max(abs(max_val / (max_q_val if max_q_val != 0 else torch.inf)), abs(min_val / (min_q_val if min_q_val != 0 else torch.inf)))
    ...
    return (w_ref.to(device=orig_device), w_q.to(device=orig_device), w_s if group_size is not None else None, maybe_w_zp)
SUPPORTED_GPTQ_QUANT_TYPES = [scalar_types.uint4b8, scalar_types.uint8b128]
SUPPORTED_GROUP_SIZES = [-1, 32, 64, 128]

def gptq_quantize_weights(w: torch.Tensor, quant_type: ScalarType, group_size: int, act_order: bool, test_perm: Optional[torch.Tensor]=None):
    size_k, _ = w.shape
    assert w.is_floating_point(), 'w must be float'
    ...
    if act_order:
        assert group_size < size_k, 'For act_order, groupsize = {} must be less than size_k = {}'.format(group_size, size_k)
...
def qqq_quantize_weights(w: torch.Tensor, num_bits: int, group_size: int):
    ...
    size_k, size_n = w.shape
    assert w.is_floating_point(), 'w must be float'
    assert num_bits in MARLIN_QQQ_SUPPORTED_NUM_BITS, f'Unsupported num_bits = {num_bits}'
    assert group_size in SUPPORTED_GROUP_SIZES + [size_k], f'Unsupported groupsize = {group_size}'
    if group_size == -1:
        group_size = size_k
    ...
    if group_size < size_k:
        ...
        s_group = (s_group.reshape(-1, size_n).contiguous() / s_channel).to(dtype=torch.half)
    else:
    ...
    return (w_ref.to(device=orig_device), q_w.to(device=orig_device), s_group.to(device=orig_device), s_channel.to(device=orig_device))
...
def sort_weights(q_w: torch.Tensor, g_idx: torch.Tensor):
    ...
    return (q_w.to(device=orig_device), g_idx.to(device=orig_device), sort_indices.to(device=orig_device))
...
def unpack_cols(packed_q_w: torch.Tensor, num_bits: int, size_k: int, size_n: int):
    pack_factor = get_pack_factor(num_bits)
    assert size_n % pack_factor == 0
    assert packed_q_w.shape == (size_k, size_n // pack_factor), 'packed_q_w.shape = {} size_k = {}, size_n = {} pack_Factor = {}'.format(packed_q_w.shape, size_k, size_n, pack_factor)
