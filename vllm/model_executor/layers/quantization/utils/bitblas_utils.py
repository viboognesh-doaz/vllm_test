...
def query_bitblas_supported_quant_types(has_zp: bool, device_capability: Optional[int]=None):
...
def _check_bitblas_supported(quant_type: ScalarType, group_size: Optional[int], has_zp: bool, device_capability: Optional[int]=None) -> tuple[bool, Optional[str]]:
    if device_capability is None:
        capability_tuple = current_platform.get_device_capability()
        device_capability = -1 if capability_tuple is None else capability_tuple.to_int()
    supported_types = query_bitblas_supported_quant_types(has_zp, device_capability)
    if quant_type not in supported_types:
        return (False, f'BitBLAS does not support weight_bits = {quant_type}. Only types = {supported_types} are supported (for group_size = {group_size}, device_capability = {device_capability}, zp = {has_zp}).')
    if group_size is None or group_size not in BITBLAS_SUPPORTED_GROUP_SIZES:
        return (False, f'BitBLAS does not support group_size = {group_size}. Only group_sizes = {BITBLAS_SUPPORTED_GROUP_SIZES} are supported.')
    try:
...
def check_bitblas_supported(quant_type: ScalarType, group_size: int, has_zp: bool=False, device_capability: Optional[int]=None) -> bool:
...
def verify_bitblas_supports_shape(output_size_per_partition: int, input_size_per_partition: int, input_size: int, group_size: int) -> None:
    if output_size_per_partition % BITBLAS_MIN_WEIGHT_SIZE_N != 0:
        raise ValueError(f'Weight output_size_per_partition = {output_size_per_partition} is not divisible by  min_thread_n = {BITBLAS_MIN_WEIGHT_SIZE_N}. Consider reducing tensor_parallel_size or running with --quantization gptq.')
    if input_size_per_partition % BITBLAS_MIN_WEIGHT_SIZE_K != 0:
        raise ValueError(f'Weight input_size_per_partition = {input_size_per_partition} is not divisible by min_thread_k = {BITBLAS_MIN_WEIGHT_SIZE_K}. Consider reducing tensor_parallel_size or running with --quantization gptq.')
...
def check_bitblas_supports_shape(output_size_per_partition: int, input_size_per_partition: int, input_size: int, group_size: int) -> tuple[bool, Optional[str]]:
    try:
...
def unpack_gptq_qzeros(qzeros, bits, is_gptq_v2=False) -> torch.Tensor:
    ...
    unpacked_zeros = torch.zeros((qzeros.shape[0], qzeros.shape[1] * elems_per_int32), dtype=torch.int8, device=qzeros.device, requires_grad=False)
    for col in range(unpacked_zeros.shape[1]):
        i = col % elems_per_int32
        unpacked_zeros[:, col] = qzeros[:, col // elems_per_int32] >> bits * i & 15
    if not is_gptq_v2:
...
def unpack_gptq_qweight(qweight, bits):
    ...
    unpacked_weight = torch.zeros((qweight.shape[0], qweight.shape[1] * elems_per_int8), dtype=torch.int8, device=qweight.device, requires_grad=False)
