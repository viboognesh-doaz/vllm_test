...
def _dequant_mxfp4(x: torch.Tensor, scale: torch.Tensor, float_dtype: torch.dtype) -> torch.Tensor:
    try:
        ...
        raise ImportError('The package `amd-quark` is required to use MX-FP4 models. Please install it with `pip install amd-quark`.') from err
...
try:
    direct_register_custom_op(op_name='dequant_mxfp4', op_func=_dequant_mxfp4, mutates_args=[], fake_impl=_dequant_mxfp4_fake)
    dequant_mxfp4 = torch.ops.vllm.dequant_mxfp4
except AttributeError as error:
    raise error
try:
    direct_register_custom_op(op_name='quant_dequant_mxfp4', op_func=_quant_dequant_mxfp4, mutates_args=[], fake_impl=_quant_dequant_mxfp4_fake)
