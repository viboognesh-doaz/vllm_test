...
import torch
__all__ = ['break_fp4_bytes', 'dequantize_to_dtype', 'ref_nvfp4_quant', 'cutlass_fp4_supported']
...
def break_fp4_bytes(a, dtype):
    ...
    signs = (combined & 8).to(torch.bool)
    abs_vals = (combined & 7).to(torch.long)
...
def run_nvfp4_emulations(x: torch.Tensor, input_global_scale: torch.Tensor, weight: torch.Tensor, weight_scale_swizzled: torch.Tensor, weight_global_scale: torch.Tensor):
    group_size = 16
    x_m, x_k = x.shape
    ...
    w_dq = dequantize_to_dtype(w_fp4, weight_scale_swizzled.data, weight_global_scale, output_dtype, x.device, group_size)
