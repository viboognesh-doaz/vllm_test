...
from vllm.model_executor.layers.quantization.utils.w8a8_utils import Fp8LinearOp, maybe_create_device_identity, normalize_e4m3fn_to_e4m3fnuz
...
class FBGEMMFp8Config(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional['QuantizeMethodBase']:
        if isinstance(layer, LinearBase):
            if is_layer_skipped(prefix=prefix, ignored_layers=self.ignore_list, fused_mapping=self.packed_modules_mapping):
...
class FBGEMMFp8LinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        input_scale_ub = torch.nn.Parameter(torch.tensor(self.quant_config.input_scale_ub, dtype=torch.float32), requires_grad=False)
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        if current_platform.is_fp8_fnuz():
            weight, weight_scale, input_scale = normalize_e4m3fn_to_e4m3fnuz(weight=weight, weight_scale=layer.weight_scale, input_scale=None)
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        return self.fp8_linear.apply(input=x, weight=layer.weight, weight_scale=layer.weight_scale, out_dtype=self.out_dtype, input_scale=None, input_scale_ub=layer.input_scale_ub, bias=bias)