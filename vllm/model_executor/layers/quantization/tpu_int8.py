try:
    ...
    raise ImportError('Please install torch_xla by following the instructions at https://docs.vllm.ai/en/latest/getting_started/tpu-installation.html to run vLLM on TPU.') from err
...
class Int8TpuConfig(QuantizationConfig):
    ...
    def __init__(self, activation_scheme: str='none') -> None:
        super().__init__()
...
class TPUInt8LinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        weight_loader = extra_weight_attrs.get('weight_loader')
        weight = ModelWeightParameter(data=torch.empty(sum(output_partition_sizes), input_size_per_partition, dtype=params_dtype), input_dim=1, output_dim=0, weight_loader=weight_loader)
