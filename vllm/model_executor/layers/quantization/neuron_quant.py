...
class NeuronQuantConfig(QuantizationConfig):
    ...
    def __init__(self, dequant_dtype: str='f16', quantize_method: str='vector_dynamic') -> None:
        super().__init__()
        self.quant_dtype = os.getenv('NEURON_QUANT_DTYPE', 's8')
        if self.quant_dtype not in SUPPORTED_QUANT_DTYPE_LIST:
            raise ValueError(f'Neuron quantization datatype {self.quant_dtype} is not valid, the quantization datatype should match one of the below types {SUPPORTED_QUANT_DTYPE_LIST}')
    ...
    def get_quantization_config(self):
        return QuantizationConfig(quant_dtype=self.quant_dtype, dequant_dtype=self.dequant_dtype, quantize_method=self.quantize_method)