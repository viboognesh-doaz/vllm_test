...
class TritonScaledMMLinearKernel(CutlassScaledMMLinearKernel):
    ...
    def can_implement(cls, c: ScaledMMLinearLayerConfig) -> tuple[bool, Optional[str]]:
        if current_platform.is_cpu():
            return (False, 'TritonScaledMMLinearKernel requires Triton which is not ' + 'currently supported on CPU.')
        if not c.input_symmetric:
            return (False, 'TritonScaledMMLinearKernel only supports symmetric ' + 'quantization.')
        return (True, None)
