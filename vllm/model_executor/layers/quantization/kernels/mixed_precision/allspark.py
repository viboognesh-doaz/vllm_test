...
from vllm.model_executor.layers.quantization.utils.allspark_utils import ALLSPARK_AMPERE_M_CUBLAS_THRESHOLD, check_allspark_supported_dtype_shape
...
class AllSparkLinearKernel(MPLinearKernel):
    ...
    def can_implement(cls, c: MPLinearLayerConfig) -> tuple[bool, Optional[str]]:
        ...
        return check_allspark_supported_dtype_shape(c.partition_weight_shape[0], c.partition_weight_shape[1], c.group_size, c.weight_type, c.act_type)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        new_weight_param.data, new_scale_param.data, _ = ops.allspark_repack_weight(new_weight_param.data, new_scale_param.data, None, c.zero_points)
    ...
    def apply_weights(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        output = ops.allspark_w8a16_gemm(a=reshaped_x, b_qweight=w_q, b_scales=w_s, b_qzeros=None, n=c.partition_weight_shape[1], group_size=c.group_size, sm_count=gemm_args['sm_count'], sm_version=gemm_args['sm_version'], CUBLAS_M_THRESHOLD=ALLSPARK_AMPERE_M_CUBLAS_THRESHOLD, has_zp=c.zero_points, n32k16_reorder=True)
        if bias is not None:
            output.add_(bias)
