...
from vllm.model_executor.layers.quantization.utils.machete_utils import check_machete_supports_shape, query_machete_supported_group_sizes, query_machete_supported_quant_types
from vllm.model_executor.layers.quantization.utils.quant_utils import pack_quantized_values_into_int32, unpack_quantized_values_into_int32
...
class MacheteLinearKernel(MPLinearKernel):
    ...
    def can_implement(cls, c: MPLinearLayerConfig) -> tuple[bool, Optional[str]]:
        ...
        if c.has_g_idx and c.partition_weight_shape[0] != c.full_weight_shape[0]:
            return (False, 'Act reordering currently not supported by Machete, when the input features are partitioned across devices')
        if c.weight_type not in query_machete_supported_quant_types(c.zero_points):
            return (False, f'Quant type ({c.weight_type}) not supported by Machete, supported types are: {query_machete_supported_quant_types(c.zero_points)}')
        if c.group_size not in query_machete_supported_group_sizes(c.act_type):
            return (False, f'Group size ({c.group_size}) not supported by Machete, supported group sizes are: {query_machete_supported_group_sizes(c.act_type)}')
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module):
        ...
        def transform_w_q(x):
            ...
            x.data = ops.machete_prepack_B(x.data.t().contiguous().t(), a_type=c.act_type, b_type=c.weight_type, group_scales_type=c.act_type)
            return x
    ...
    def apply_weights(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if c.zero_points:
            ...
            w_zp = None
        output = ops.machete_mm(a=x_2d, b_q=w_q, b_type=c.weight_type, b_group_zeros=w_zp, b_group_scales=w_s, b_group_size=c.group_size)
        if bias is not None:
            output.add_(bias)
