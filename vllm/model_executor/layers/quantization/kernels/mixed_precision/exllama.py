...
class ExllamaLinearKernel(MPLinearKernel):
    ...
    def can_implement(cls, c: MPLinearLayerConfig) -> tuple[bool, Optional[str]]:
        if c.has_g_idx and c.partition_weight_shape[0] != c.full_weight_shape[0]:
            return (False, 'Act reordering currently not supported by Exllama, when the input features are partitioned across devices')
        if c.partition_weight_shape[1] % (32 // c.weight_type.size_bits) != 0:
            return (False, 'Output features must be a multiple of the pack factor (32 / num_bits) so that we can correctly pack the zero points')
        if c.act_type != torch.float16:
            return (False, 'Exllama only supports float16 activations')
        if c.weight_type not in cls.SUPPORTED_QUANT_TYPES:
            return (False, f'Quant type ({c.weight_type}) not supported by Exllama, supported types are: {cls.SUPPORTED_QUANT_TYPES}')
        if c.full_weight_shape[0] % c.group_size != 0:
            return (False, f'Group size ({c.group_size}) does not evenly divide the number of input features ({c.full_weight_shape[0]})')
        return (True, None)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module):
        ...
        if not c.zero_points:
            ...
            if c.weight_type.has_bias():
                zeros = torch.full((groups, out_features), c.weight_type.bias - 1, dtype=torch.int32, device=device)
            else:
                raise NotImplementedError('A 0 zero-point is not supported by Exllama due to a bug in the original GPTQ checkpoint format leading to exllama kernel adding 1 to the zero points during inference')
