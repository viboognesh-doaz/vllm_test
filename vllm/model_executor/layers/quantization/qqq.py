...
class QQQConfig(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int, group_size: int, is_sym: bool=True) -> None:
        super().__init__()
        self.weight_bits = weight_bits
        self.group_size = group_size
        self.is_sym = is_sym
        if self.weight_bits not in MARLIN_QQQ_SUPPORTED_NUM_BITS:
            raise ValueError(f'QQQ does not support weight_bits = {self.weight_bits}. Only weight_bits = {MARLIN_QQQ_SUPPORTED_NUM_BITS} are supported.')
        if self.group_size not in MARLIN_QQQ_SUPPORTED_GROUP_SIZES:
            raise ValueError(f'QQQ does not support group_size = {self.group_size}. Only group_sizes = {MARLIN_QQQ_SUPPORTED_GROUP_SIZES} are supported.')
        if self.is_sym not in MARLIN_QQQ_SUPPORTED_SYM:
            raise ValueError(f'QQQ does not support is_sym = {self.is_sym}. Only sym = {MARLIN_QQQ_SUPPORTED_SYM} are supported.')
...
class QQQLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        qweight = PackedvLLMParameter(data=torch.empty(input_size_per_partition // self.quant_config.tile_size, output_size_per_partition * self.quant_config.tile_size // self.quant_config.pack_factor, device='cuda', dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=1, packed_factor=self.quant_config.pack_factor, marlin_tile_size=self.quant_config.tile_size, weight_loader=weight_loader)
        s_channel = ChannelQuantScaleParameter(data=torch.empty(1, output_size_per_partition, device='cuda', dtype=torch.float), weight_loader=weight_loader, output_dim=1)
        if self.quant_config.group_size == -1:
            s_group_data = torch.tensor([], device='cuda', dtype=torch.half)
        else:
            s_group_data = torch.empty(input_size_per_partition // self.quant_config.group_size, output_size_per_partition, device='cuda', dtype=torch.half)
