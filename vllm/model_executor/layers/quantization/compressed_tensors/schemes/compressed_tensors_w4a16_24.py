...
from vllm.model_executor.layers.quantization.gptq_marlin_24 import GPTQ_MARLIN_24_MAX_PARALLEL, GPTQ_MARLIN_24_MIN_THREAD_N
...
W4A16SPARSE24_SUPPORTED_TYPES_MAP = {4: scalar_types.uint4b8}
...
class CompressedTensorsW4A16Sparse24(CompressedTensorsScheme):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size: int, output_partition_sizes: list[int], input_size_per_partition: int, params_dtype: torch.dtype, weight_loader: Callable, **kwargs):
        assert params_dtype == torch.float16, 'float16 is required for marlin24 compressed models. Set dtype=torch.float16'
        pack_factor = 32 // self.quant_type.size_bits
        output_size_per_partition = sum(output_partition_sizes)
        qweight = PackedvLLMParameter(data=torch.empty(input_size_per_partition // self.tile_size // 2, output_size_per_partition * self.tile_size // pack_factor, dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=1, packed_factor=pack_factor, marlin_tile_size=self.tile_size, weight_loader=weight_loader)
        input_groups = 1 if self.group_size is None else input_size_per_partition // self.group_size
        weight_scale_args = {'data': torch.empty(input_groups, output_size_per_partition, dtype=params_dtype), 'weight_loader': weight_loader}
        ...
        meta = PackedvLLMParameter(data=torch.empty(input_size_per_partition // 8 // 2 // 2, output_size_per_partition * 2, dtype=torch.int16), input_dim=0, output_dim=1, packed_dim=1, packed_factor=1, marlin_tile_size=2, weight_loader=weight_loader)
    ...
    def apply_weights(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        output_2d = ops.gptq_marlin_24_gemm(x_2d, qweight, meta, scales, workspace, self.quant_type, size_m, size_n, size_k)
