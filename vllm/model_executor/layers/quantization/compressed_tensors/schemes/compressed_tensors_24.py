...
from vllm.model_executor.layers.quantization.utils.w8a8_utils import convert_to_channelwise, sparse_cutlass_supported
from vllm.model_executor.parameter import BasevLLMParameter, ChannelQuantScaleParameter, ModelWeightParameter, PerTensorScaleParameter
...
class CompressedTensors24(CompressedTensorsScheme):
    ...
    def __init__(self, quantized: bool=False, weight_quant: Optional[QuantizationArgs]=None, input_quant: Optional[QuantizationArgs]=None, model_compression_config: Optional[dict[str, Any]]=None):
        self.quantized = quantized
        self.weight_quant = weight_quant
        self.input_quant = input_quant
        self.model_compressor = ModelCompressor.from_compression_config(model_compression_config) if model_compression_config is not None else None
        self.do_sparse_decompress = self.model_compressor is not None and self.model_compressor.sparsity_config.format == CompressionFormat.sparse_24_bitmask.value
    ...
    def create_weights(self, layer: torch.nn.Module, input_size: int, output_partition_sizes: list[int], input_size_per_partition: int, params_dtype: torch.dtype, weight_loader: Callable, **kwargs):
        if not sparse_cutlass_supported():
            raise ValueError('Sparse CUTLASS not supported. vLLM must be built with CUDA 12.2 or later to use this feature')
        layer.logical_widths = output_partition_sizes
        layer.input_size = input_size
        layer.input_size_per_partition = input_size_per_partition
        self.weights_dtype: torch.dtype = self._get_params_dtype(params_dtype)
        weight = ModelWeightParameter(data=torch.empty(sum(output_partition_sizes), input_size_per_partition, dtype=self.weights_dtype), input_dim=1, output_dim=0, weight_loader=weight_loader)
        if self.do_sparse_decompress:
            assert all((partition_size % 8 == 0 for partition_size in output_partition_sizes)), 'All partitions must be divisible by 8 for '
            '2:4 sparse compressed models'
            shape = BasevLLMParameter(data=torch.empty(2, 1, dtype=torch.int64), weight_loader=weight_loader)
            compressed_weight = ModelWeightParameter(data=torch.empty(sum(output_partition_sizes), input_size_per_partition // 2, dtype=self.weights_dtype), input_dim=1, output_dim=0, weight_loader=weight_loader)
            bitmask = ModelWeightParameter(data=torch.empty(sum(output_partition_sizes), input_size_per_partition // 8, dtype=torch.uint8), input_dim=1, output_dim=0, weight_loader=weight_loader)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        """
        if self.do_sparse_decompress:
            layer.weight.data = self._decompress_bitmask_compressed_weight(compressed=layer.compressed, bitmask=layer.bitmask, layer=layer)
        ...
        if self.weight_quant:
            if self.weight_quant.strategy == QuantizationStrategy.TENSOR.value:
                layer.weight_scale = torch.nn.Parameter(convert_to_channelwise(weight_scale=layer.weight_scale, logical_widths=layer.logical_widths), requires_grad=False)
            else:
    ...
    def apply_weights(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if self.quantized:
            ...
            q_input = x
        out = ops.cutlass_scaled_sparse_mm(a=q_input, bt_nzs=layer.weight, bt_meta=layer.meta, scale_a=input_scale, scale_b=layer.weight_scale, out_dtype=x.dtype, bias=bias)
    ...
    def _decompress_bitmask_compressed_weight(self, compressed: torch.Tensor, bitmask: torch.Tensor, layer: torch.nn.Module) -> torch.Tensor:
        """
        ...
        def _process_split(bitmask_compressed_weight: torch.Tensor, shape, bitmask: torch.Tensor) -> torch.Tensor:
            weight_data = dict(compressed=bitmask_compressed_weight, shape=shape, bitmask=bitmask)
        ...
        if split_weights:
            decompressed_shards = [_process_split(compressed_weight, shape, bitmask) for compressed_weight, shape, bitmask in zip(split_weights, split_shape, split_bitmask)]
            decompressed = combine_shards(decompressed_shards)
        else:
            decompressed = sparsity_compressor.decompress_weight(dict(compressed=compressed, shape=(layer.logical_widths[0], layer.input_size_per_partition), bitmask=bitmask))
        return decompressed