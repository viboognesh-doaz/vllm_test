...
def dequantize_weight(codes: torch.Tensor, codebooks: torch.Tensor, scales: Optional[torch.Tensor]=None) -> torch.Tensor:
    """
    ...
    codebook_offsets = torch.arange(0, num_codebooks * codebook_size, codebook_size, device=codes.device)
    reconstructed_weight_flat = F.embedding_bag(codes.flatten(0, -2) + codebook_offsets, codebooks.flatten(0, 1).flatten(-2, -1), mode='sum')
    reconstructed_weight_groupwise = reconstructed_weight_flat.view(list(codes.shape[:-3]) + [num_out_groups, num_in_groups, out_group_size, in_group_size])
...
def dequantize_gemm(input: torch.Tensor, codes: torch.IntTensor, codebooks: torch.Tensor, scales: torch.Tensor, bias: Optional[torch.Tensor]) -> torch.Tensor:
    dequantized_weight = dequantize_weight(unpack_int_data(codes, codebooks.shape[1].bit_length() - 1), codebooks, scales)
    return F.linear(input, dequantized_weight, bias)

def generic_dequantize_gemm(input: torch.Tensor, codes: torch.IntTensor, codebooks: torch.Tensor, scales: torch.Tensor, output_partition_sizes: list[int], bias: Optional[torch.Tensor]) -> torch.Tensor:
    ...
    for output_size in output_partition_sizes:
        shard_output = dequantize_gemm(input, codes.narrow(0, output_offset, output_size), codebooks.narrow(0, codebooks_offset, num_codebooks), scales.narrow(0, output_offset, output_size), None if bias is None else bias.narrow(0, output_offset, output_size))
...
def optimized_dequantize_gemm(input: torch.Tensor, codes: torch.IntTensor, codebooks: torch.Tensor, scales: torch.Tensor, output_partition_sizes: list[int], bias: Optional[torch.Tensor]) -> torch.Tensor:
...
class AQLMConfig(QuantizationConfig):
    ...
    def __init__(self, in_group_size: int, nbits_per_codebook: int, num_codebooks: int, out_group_size: int) -> None:
        super().__init__()
    ...
    def __repr__(self) -> str:
        return f'AQLMConfig(in_group_size={self.in_group_size}, nbits_per_codebook={self.nbits_per_codebook}, num_codebooks={self.num_codebooks}, out_group_size={self.out_group_size})'
...
class AQLMLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        codes = Parameter(torch.empty(output_size_per_partition, input_size_per_partition // self.quant_config.pack_factor, self.quant_config.num_codebooks, dtype=get_int_dtype(self.quant_config.nbits_per_codebook)), requires_grad=False)
        set_weight_attrs(codes, {'input_dim': 1, 'output_dim': 0, 'packed_dim': 1, 'pack_factor': self.quant_config.pack_factor})
        codebooks = Parameter(torch.empty(self.quant_config.num_codebooks * len(output_partition_sizes), 2 ** self.quant_config.nbits_per_codebook, self.quant_config.out_group_size, self.quant_config.in_group_size, dtype=params_dtype), requires_grad=False)
        set_weight_attrs(codebooks, {'is_metadata': True, 'output_partition_sizes': output_partition_sizes})
        scales = Parameter(torch.empty((output_size_per_partition // self.quant_config.out_group_size, 1, 1, 1), dtype=params_dtype), requires_grad=False)
        set_weight_attrs(scales, {'output_dim': 0, 'packed_dim': 0, 'pack_factor': self.quant_config.out_group_size})
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if ingroups == 8 and outgroups == 1 and (bits == 256 and nbooks == 2 or (bits == 65536 and nbooks == 1)):
            ...
            return ops.aqlm_gemm(x, codes, codebooks, scales, output_partition_sizes, bias) if use_gemv else optimized_dequantize_gemm(x, codes, codebooks, scales, output_partition_sizes, bias)
        return generic_dequantize_gemm(x, codes, codebooks, scales, output_partition_sizes, bias)