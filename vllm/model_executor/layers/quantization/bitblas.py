...
from vllm.model_executor.layers.quantization.utils.bitblas_utils import BITBLAS_OPTIMIZE_FEATURES, BITBLAS_SUPPORTED_NUM_BITS, BITBLAS_SUPPORTED_SYM, MINIMUM_BITBLAS_VERSION
from vllm.model_executor.layers.vocab_parallel_embedding import ParallelLMHead
from vllm.model_executor.parameter import BasevLLMParameter, ChannelQuantScaleParameter, GroupQuantScaleParameter, PackedvLLMParameter
...
class BitBLASConfig(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int, group_size: Optional[int], desc_act: Optional[bool], is_sym: Optional[bool], quant_method: Optional[str], lm_head_quantized: bool) -> None:
        try:
        ...
        if self.weight_bits not in BITBLAS_SUPPORTED_NUM_BITS:
            raise ValueError(f'BitBLAS does not support weight_bits = {self.weight_bits}. Only weight_bits = {BITBLAS_SUPPORTED_NUM_BITS} are supported.')
        if self.is_sym not in BITBLAS_SUPPORTED_SYM:
            raise ValueError(f'BitBLAS does not support is_sym = {self.is_sym}. Only sym = {BITBLAS_SUPPORTED_SYM} are supported.')
    ...
    def __repr__(self) -> str:
        return f'BitBLASConfig(weight_bits={self.weight_bits}, group_size={self.group_size}, desc_act={self.desc_act}, is_sym={self.is_sym}, quant_method={self.quant_method})'
    ...
    def override_quantization_method(cls, hf_quant_cfg, user_quant) -> Optional[QuantizationMethods]:
        is_bitblas_format = hf_quant_cfg.get('checkpoint_format') == 'bitblas' or hf_quant_cfg.get('is_bitblas_format', False)
...
class BitBLASLinearMethod(LinearMethodBase):
    ...
    ENABLE_TUNING = True
    BITBLAS_DTYPES = {torch.float32: 'float32', torch.float16: 'float16', torch.bfloat16: 'bfloat16', torch.half: 'float16', torch.int8: 'int8'}

    def __init__(self, quant_config: BitBLASConfig):
        self.quant_config = quant_config

    def create_weights_gptq(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        if group_size != -1 and input_size_per_partition % group_size != 0:
            raise ValueError(f'Input size per partition ({input_size_per_partition}) must be divisible by group size ({group_size}).')
        self._configure_bitblas_matmul(input_size_per_partition, output_size_per_partition, params_dtype=params_dtype, enable_tuning=self.ENABLE_TUNING, bias=False, layout='nt', bits=self.quant_config.weight_bits)
        qweight = PackedvLLMParameter(data=torch.empty(self.bitblas_matmul.retrieve_weight_shape(), device='cuda', dtype=self.quant_config.storage_torch_dtype, requires_grad=False), input_dim=1, output_dim=0, packed_dim=1, packed_factor=self.quant_config.pack_factor, bitblas_tile_size=self.bitblas_matmul.retrieve_weight_shape()[-2] if self.bitblas_matmul.propagate_b else None, weight_loader=weight_loader)
        input_groups = 1 if group_size == -1 else input_size_per_partition // group_size
        weight_scale_args = {'data': torch.empty(output_size_per_partition, input_groups, device='cuda', dtype=params_dtype), 'weight_loader': weight_loader}
        if input_groups == 1:
        ...
        if self.quant_config.zeros_mode == 'quantized':
            zeros = PackedvLLMParameter(data=torch.empty(input_groups, output_size_per_partition // self.quant_config.pack_factor, device='cuda', dtype=self.quant_config.storage_torch_dtype, requires_grad=False), input_dim=0, output_dim=1, packed_dim=1, packed_factor=self.quant_config.pack_factor, weight_loader=weight_loader)
        else:
            zeros = BasevLLMParameter(torch.empty(output_size_per_partition, input_groups, device='cuda', dtype=params_dtype), weight_loader=weight_loader)
            set_weight_attrs(zeros, {'input_dim': None if input_groups == 1 else 1, 'output_dim': 0})
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        if self.quant_config.quant_method == 'gptq':
            return self.create_weights_gptq(layer, input_size_per_partition, output_partition_sizes, input_size, output_size, params_dtype, **extra_weight_attrs)
        else:
            raise ValueError(f'Unsupported quant_method {self.quant_config.quant_method}')

    def _configure_bitblas_matmul(self, infeatures, outfeatures, params_dtype, enable_tuning, bias, layout, bits, out_dtype='float16'):
    ...
    def apply_gptq(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
