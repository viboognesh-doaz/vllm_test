...
from vllm.model_executor.layers.quantization.fp8 import Fp8Config, Fp8KVCacheMethod, Fp8LinearMethod
...
class PTPCFp8Config(Fp8Config):
    ...
    def __init__(self, activation_scheme: str='dynamic', ignored_layers: Optional[list[str]]=None) -> None:
        if not current_platform.is_rocm():
            raise ValueError('ptpc_fp8 quantization is supported only on ROCm.')
        if not current_platform.has_device_capability(94):
            raise ValueError('ptpc_fp8 quantization is supported only on AMD Instinct MI300 GPUs and newer.')
        if activation_scheme == 'static':
            raise ValueError('ptpc_fp8 as of now only support dynamic quantization.')
        super().__init__(is_checkpoint_fp8_serialized=False, activation_scheme=activation_scheme, ignored_layers=ignored_layers)
...
class PTPCFp8LinearMethod(Fp8LinearMethod):
    ...
    def __init__(self, quant_config: PTPCFp8Config):
        ...
        self.fp8_linear = Fp8LinearOp(act_quant_static=False, cutlass_fp8_supported=False, act_quant_group_shape=GroupShape.PER_TOKEN)

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        layer.weight = torch.nn.Parameter(layer.weight.data, requires_grad=False)
        assert layer.weight.data.dtype == torch.bfloat16, f'Currently torch._scaled_mm (hipBLASLt) rowwise gemm only support output dtype of bfloat16. {str(layer.weight.data.dtype)} is specified.'
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        return self.fp8_linear.apply(input=x, weight=layer.weight, weight_scale=layer.weight_scale, input_scale=None, input_scale_ub=None, bias=bias)