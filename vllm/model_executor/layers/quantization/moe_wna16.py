...
from vllm.model_executor.layers.quantization.gptq_marlin import GPTQMarlinConfig
from vllm.model_executor.layers.quantization.utils.marlin_utils import check_marlin_supports_layer
...
class MoeWNA16Config(QuantizationConfig):
    ...
    def __init__(self, linear_quant_method: str, weight_bits: int, group_size: int, has_zp: bool, lm_head_quantized: bool, modules_to_not_convert: Optional[list[str]], full_config: dict[str, Any]) -> None:
        super().__init__()
        ...
        if self.linear_quant_method == 'gptq':
            ...
            if device_capability < awq_min_capability:
                raise ValueError(f'The quantization method moe_wna16 + awq is not supported for the current GPU. Minimum capability: {awq_min_capability}. Current capability: {device_capability}.')
    ...
    def from_config(cls, config: dict[str, Any]) -> 'MoeWNA16Config':
        ...
        if linear_quant_method == 'gptq':
            ...
            raise ValueError('moe_wna16 only support gptq and awq.')
        return cls(linear_quant_method, weight_bits, group_size, has_zp, lm_head_quantized, modules_to_not_convert, config)
...
class MoeWNA16Method(FusedMoEMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        w13_qweight = torch.nn.Parameter(torch.empty(num_experts, 2 * intermediate_size_per_partition, hidden_size // bit8_pack_factor, dtype=torch.uint8), requires_grad=False)
        layer.register_parameter('w13_qweight', w13_qweight)
        set_weight_attrs(w13_qweight, extra_weight_attrs)
        w2_qweight = torch.nn.Parameter(torch.empty(num_experts, hidden_size, intermediate_size_per_partition // bit8_pack_factor, dtype=torch.uint8), requires_grad=False)
        layer.register_parameter('w2_qweight', w2_qweight)
        set_weight_attrs(w2_qweight, extra_weight_attrs)
        w13_scales = torch.nn.Parameter(torch.zeros(num_experts, 2 * intermediate_size_per_partition, hidden_size // group_size, dtype=params_dtype), requires_grad=False)
        layer.register_parameter('w13_scales', w13_scales)
        set_weight_attrs(w13_scales, extra_weight_attrs)
        w2_scales = torch.nn.Parameter(torch.zeros(num_experts, hidden_size, intermediate_size_per_partition // group_size, dtype=params_dtype), requires_grad=False)
        layer.register_parameter('w2_scales', w2_scales)
        set_weight_attrs(w2_scales, extra_weight_attrs)
        if self.quant_config.has_zp:
            w13_qzeros = torch.nn.Parameter(torch.zeros(num_experts, 2 * intermediate_size_per_partition // bit8_pack_factor, hidden_size // group_size, dtype=torch.uint8), requires_grad=False)
            layer.register_parameter('w13_qzeros', w13_qzeros)
            set_weight_attrs(w13_qzeros, extra_weight_attrs)
            w2_qzeros = torch.nn.Parameter(torch.zeros(num_experts, hidden_size // bit8_pack_factor, intermediate_size_per_partition // group_size, dtype=torch.uint8), requires_grad=False)
    ...
    def get_weight_loader(layer, weight_loader):
        ...
        def moe_wna16_weight_loader(param: torch.nn.Parameter, loaded_weight: torch.Tensor, weight_name: str, shard_id: str, expert_id: int):
