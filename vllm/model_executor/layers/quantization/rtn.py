...
class RTNConfig(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int=int(NUM_BITS), group_size: int=int(GROUP_SIZE)) -> None:
        self.weight_bits = weight_bits
        self.group_size = group_size
        if self.weight_bits != 4 and self.weight_bits != 8:
            raise ValueError(f'Currently, only 4-bit or 8-bit weight quantization is supported for RTN, but got {self.weight_bits} bits.')
...
class RTNTensor:
    ...
    def narrow(self, dim, start, length):
        ...
        return RTNTensor(self.data.narrow(dim, start // factor, length // factor), self.scale.narrow(dim, start, length), self.quant_config)
    ...
    def copy_(self, loaded_weight: torch.Tensor) -> None:
        qweight, weight_scale = rtn_quantize(loaded_weight.cuda(), self.quant_config.weight_bits, self.quant_config.group_size)
...
class RTNLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        num_groups_per_col = input_size_per_partition // self.quant_config.group_size if self.quant_config.group_size != -1 else 1
        scale = Parameter(torch.empty(output_size_per_partition, num_groups_per_col, dtype=params_dtype), requires_grad=False)
        factor = 1 if self.quant_config.weight_bits == 8 else 2
        weight = RTNParameter(data=torch.empty(output_size_per_partition // factor, input_size_per_partition, dtype=torch.int8), scale=scale, quant_config=self.quant_config)
        layer.register_parameter('weight', weight)
        set_weight_attrs(weight, {**extra_weight_attrs, 'input_dim': 1, 'output_dim': 0})
...
def rtn_quantize(tensor: torch.Tensor, num_bits: int, group_size: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    q_range = 2 ** num_bits
    num_groups = tensor.shape[0] * tensor.shape[1] // group_size if group_size != -1 else tensor.shape[0]
    ...
    if num_bits == 4:
        ...
        inputs_q = inputs_q[:, 1::2] << 4 | inputs_q[:, ::2] & 15
