...
class GPTQMarlin24Config(QuantizationConfig):
    ...
    def __init__(self, weight_bits: int, group_size: int) -> None:
        super().__init__()
        quant_type = {4: scalar_types.uint4b8, 8: scalar_types.uint8b128}.get(weight_bits)
        self.group_size = group_size
        if quant_type is None or quant_type not in GPTQ_MARLIN_24_SUPPORTED_QUANT_TYPES:
            raise ValueError(f'Marlin_24 does not support quant_type = {quant_type}. Only weight_bits = {GPTQ_MARLIN_24_SUPPORTED_QUANT_TYPES} are supported.')
        if self.group_size not in GPTQ_MARLIN_24_SUPPORTED_GROUP_SIZES:
            raise ValueError(f'Marlin_24 does not support group_size = {self.group_size}. Only group_sizes = {GPTQ_MARLIN_24_SUPPORTED_GROUP_SIZES} are supported.')
...
class GPTQMarlin24LinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        if self.quant_config.group_size != -1 and input_size_per_partition % self.quant_config.group_size != 0:
            raise ValueError(f'Weight input_size_per_partition = {input_size_per_partition} is not divisible by group_size = {self.quant_config.group_size}.')
        ...
        qweight = PackedvLLMParameter(data=torch.empty(input_size_per_partition // self.quant_config.tile_size // 2, output_size_per_partition * self.quant_config.tile_size // self.quant_config.pack_factor, device='cuda', dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=1, packed_factor=self.quant_config.pack_factor, marlin_tile_size=self.quant_config.tile_size, weight_loader=weight_loader)
        meta = PackedvLLMParameter(data=torch.empty(input_size_per_partition // 8 // 2 // 2, output_size_per_partition * 2, device='cuda', dtype=torch.int16), input_dim=0, output_dim=1, packed_dim=1, packed_factor=1, marlin_tile_size=2, weight_loader=weight_loader)
        input_groups = 1 if self.quant_config.group_size == -1 else input_size_per_partition // self.quant_config.group_size
        weight_scale_args = {'data': torch.empty(input_groups, output_size_per_partition, device='cuda', dtype=params_dtype), 'weight_loader': weight_loader}
        if input_groups == 1:
        ...
        workspace = BasevLLMParameter(data=torch.zeros(max_workspace_size, device='cuda', dtype=torch.int), weight_loader=weight_loader)
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        output_2d = ops.gptq_marlin_24_gemm(x_2d, qweight, meta, scales, workspace, self.quant_config.quant_type, size_m, size_n, size_k)
