...
class BitsAndBytesConfig(QuantizationConfig):
    ...
    def __repr__(self) -> str:
        return f'BitsAndBytesConfig(load_in_8bit={self.load_in_8bit}, load_in_4bit={self.load_in_4bit}, bnb_4bit_compute_dtype={self.bnb_4bit_compute_dtype}, bnb_4bit_quant_storage={self.bnb_4bit_quant_storage}, bnb_4bit_quant_type={self.bnb_4bit_quant_type}, llm_int8_skip_modules={self.llm_int8_skip_modules})'
    ...
    def from_config(cls, config: dict[str, Any]) -> 'BitsAndBytesConfig':
        ...
        llm_int8_enable_fp32_cpu_offload = get_safe_value(config, ['llm_int8_enable_fp32_cpu_offload'], default_value=False)
        ...
        return cls(load_in_8bit=load_in_8bit, load_in_4bit=load_in_4bit, bnb_4bit_compute_dtype=bnb_4bit_compute_dtype, bnb_4bit_quant_storage=bnb_4bit_quant_storage, bnb_4bit_quant_type=bnb_4bit_quant_type, bnb_4bit_use_double_quant=bnb_4bit_use_double_quant, llm_int8_enable_fp32_cpu_offload=llm_int8_enable_fp32_cpu_offload, llm_int8_has_fp16_weight=llm_int8_has_fp16_weight, llm_int8_skip_modules=llm_int8_skip_modules, llm_int8_threshold=llm_int8_threshold)

    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional[Union['LinearMethodBase', 'BitsAndBytesMoEMethod']]:
...
class BitsAndBytesLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        def create_qweight_for_8bit():
            qweight = Int8Params(data=torch.empty(sum(output_partition_sizes), input_size_per_partition, dtype=torch.int8), has_fp16_weights=self.quant_config.llm_int8_has_fp16_weight, requires_grad=False)
            set_weight_attrs(qweight, {'input_dim': 0, 'output_dim': 0, 'pack_factor': 1, 'use_bitsandbytes_8bit': True, 'generation': 0})
            return qweight
        ...
        def create_qweight_for_4bit():
            ...
            qweight = torch.nn.Parameter(torch.empty(total_size // quant_ratio, 1, dtype=torch.uint8), requires_grad=False)
            set_weight_attrs(qweight, {'input_dim': 0, 'output_dim': 0, 'pack_factor': quant_ratio, 'use_bitsandbytes_4bit': True})
            return qweight
    ...
    def _apply_8bit_weight(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        for i in range(len(quant_states)):
            ...
            if generation > 0 and (not self.quant_config.llm_int8_has_fp16_weight) and (matmul_states[i].CB is not None) and (matmul_states[i].CxB is not None):
    ...
    def _apply_4bit_weight(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        original_type = x.dtype
        original_shape = x.shape
        reshape_after_matmul = False
        if x.ndim > 2:
...
def _apply_bnb_4bit(x: torch.Tensor, weight: torch.Tensor, offsets: torch.Tensor, out: torch.Tensor) -> None:
    quant_states = weight.bnb_quant_state
    current_index = 0
    for i in range(len(quant_states)):
        output_size = quant_states[i].shape[0]
        out[:, current_index:current_index + output_size] = matmul_4bit(x, weight[offsets[i]:offsets[i + 1]].t(), quant_states[i])
        current_index += output_size

def _apply_bnb_4bit_fake(x: torch.Tensor, weight: torch.Tensor, offsets: torch.Tensor, out: torch.Tensor) -> None:
    return
try:
    direct_register_custom_op(op_name='apply_bnb_4bit', op_func=_apply_bnb_4bit, mutates_args=['out'], fake_impl=_apply_bnb_4bit_fake, dispatch_key=current_platform.dispatch_key)
...
class BitsAndBytesMoEMethod(FusedMoEMethodBase):
    ...
    def __init__(self, quant_config: BitsAndBytesConfig):
        try:
            if bitsandbytes.__version__ < '0.45.3':
                raise ImportError('bitsandbytes version is wrong. Please install bitsandbytes>=0.45.3.')
        except ImportError as err:
            raise ImportError('Please install bitsandbytes>=0.45.3 via `pip install bitsandbytes>=0.45.3` to use bitsandbytes quantizer.') from err
    ...
    def create_weights(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        call_fun(layer, num_experts, hidden_size, intermediate_size_per_partition, params_dtype, **extra_weight_attrs)
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, router_logits: torch.Tensor, top_k: int, renormalize: bool, use_grouped_topk: bool=False, topk_group: Optional[int]=None, num_expert_group: Optional[int]=None, global_num_experts: int=-1, expert_map: Optional[torch.Tensor]=None, custom_routing_function: Optional[Callable]=None, scoring_func: str='softmax', e_score_correction_bias: Optional[torch.Tensor]=None, apply_router_weight_on_input: bool=False, activation: str='silu', enable_eplb: bool=False, expert_load_view: Optional[torch.Tensor]=None, logical_to_physical_map: Optional[torch.Tensor]=None, logical_replica_count: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        return fused_experts(hidden_states=x, w1=w13, w2=w2, topk_weights=topk_weights, topk_ids=topk_ids, inplace=True, activation=activation, apply_router_weight_on_input=apply_router_weight_on_input, global_num_experts=global_num_experts, expert_map=expert_map)

    def _create_weights_4bit(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
        quant_ratio = calculate_quant_ratio(params_dtype)
        w13_total_size = hidden_size * 2 * intermediate_size_per_partition // quant_ratio
        w13_qweight = torch.nn.Parameter(torch.empty(num_experts, w13_total_size, 1, dtype=torch.uint8), requires_grad=False)
        layer.register_parameter('w13_weight', w13_qweight)
        set_weight_attrs(w13_qweight, extra_weight_attrs)
        set_weight_attrs(w13_qweight, {'num_experts': num_experts, 'input_dim': hidden_size, 'output_dim': 2 * intermediate_size_per_partition, 'experts_shape': (num_experts, intermediate_size_per_partition * 2, hidden_size), 'pack_factor': quant_ratio, 'use_bitsandbytes_4bit': True})
        w2_total_size = hidden_size * intermediate_size_per_partition // quant_ratio
        w2_qweight = torch.nn.Parameter(torch.empty(num_experts, w2_total_size, 1, dtype=torch.uint8), requires_grad=False)
        set_weight_attrs(w2_qweight, {'num_experts': num_experts, 'input_dim': intermediate_size_per_partition, 'output_dim': hidden_size, 'experts_shape': (num_experts, hidden_size, intermediate_size_per_partition), 'pack_factor': quant_ratio, 'use_bitsandbytes_4bit': True})
        layer.register_parameter('w2_weight', w2_qweight)
        set_weight_attrs(w2_qweight, extra_weight_attrs)

    def _create_weights_8bit(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
        raise NotImplementedError

    def _apply_4bit_dequnt(self, layer: torch.nn.Module) -> tuple[torch.Tensor, torch.Tensor]:
        w13 = dequantize_4bit(layer.w13_weight.reshape(-1, 1), layer.w13_weight.bnb_quant_state)
        w2 = dequantize_4bit(layer.w2_weight.reshape(-1, 1), layer.w2_weight.bnb_quant_state)
