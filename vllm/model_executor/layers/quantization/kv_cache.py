    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        if layer.kv_cache_dtype != 'auto' and (not layer.calculate_kv_scales):
            ...
            if layer.q_scale < 0.0:
                logger.warning_once('Checkpoint does not provide a q scaling factor. Setting it to k_scale. This only matters for the flash-attn backend.')
            ...
            if k_scale == 1.0 and v_scale == 1.0 and ('e5m2' not in layer.kv_cache_dtype):
                logger.warning_once('Using KV cache scaling factor 1.0 for fp8_e4m3. This may cause accuracy issues. Please make sure k/v_scale scaling factors are available in the fp8 checkpoint.')
        if layer.q_scale > 0.0:
        ...
        if layer.kv_cache_dtype == 'fp8' and (q_scale == 1.0 or prob_scale == 1.0):
            logger.warning_once(f'Using uncalibrated q_scale {q_scale} and/or prob_scale {prob_scale} with fp8 attention. This may cause accuracy issues. Please make sure q/prob scaling factors are available in the fp8 checkpoint.')
        del layer.k_scale
        del layer.v_scale
        del layer.q_scale
