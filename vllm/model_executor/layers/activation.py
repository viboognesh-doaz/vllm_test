...
_ACTIVATION_REGISTRY = LazyDict({'gelu': lambda: nn.GELU(), 'gelu_fast': lambda: FastGELU(), 'gelu_new': lambda: NewGELU(), 'gelu_pytorch_tanh': lambda: nn.GELU(approximate='tanh'), 'relu': lambda: nn.ReLU(), 'relu2': lambda: ReLUSquaredActivation(), 'silu': lambda: nn.SiLU(), 'quick_gelu': lambda: QuickGELU()})
...
_ACTIVATION_AND_MUL_REGISTRY = LazyDict({'gelu': lambda: GeluAndMul(), 'silu': lambda: SiluAndMul(), 'geglu': lambda: GeluAndMul()})
