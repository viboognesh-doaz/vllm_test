...
from vllm.sequence import VLLM_INVALID_TOKEN_ID, CompletionSequenceGroupOutput, Logprob, PromptLogprobs, SampleLogprobs, SequenceOutput
...
class SamplerOutput(msgspec.Struct, omit_defaults=True, array_like=True):
    ...
    def __repr__(self) -> str:
        ...
        return f'SamplerOutput(outputs={self.outputs}, sampled_token_probs={sampled_token_probs_repr}, sampled_token_ids={sampled_token_ids_repr})'
...
class Sampler(nn.Module):
    ...
    def _init_sampling_tensors(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata):
        ...
        sampling_tensors, do_penalties, do_top_p_top_k, do_min_p = SamplingTensors.from_sampling_metadata(sampling_metadata, vocab_size, logits.device, logits.dtype)
    ...
    def forward(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> Optional[SamplerOutput]:
        ...
        if do_penalties:
            logits = apply_penalties(logits, sampling_tensors.prompt_tokens, sampling_tensors.output_tokens, sampling_tensors.presence_penalties, sampling_tensors.frequency_penalties, sampling_tensors.repetition_penalties)
        ...
        maybe_deferred_sample_results, maybe_sampled_tokens_tensor = _sample(probs, logprobs, sampling_metadata, sampling_tensors, include_gpu_probs_tensor=self.include_gpu_probs_tensor, modify_greedy_probs=self._should_modify_greedy_probs_inplace)
        ...
        return _build_sampler_output(maybe_deferred_sample_results, sampling_metadata, prompt_logprobs, sample_logprobs, on_device_tensors=on_device_tensors, skip_sampler_cpu_output=sampling_metadata.skip_sampler_cpu_output)
...
def _apply_min_tokens_penalty(logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
...
def _greedy_sample(selected_seq_groups: list[SequenceGroupToSample], samples: torch.Tensor) -> SampleResultType:
...
def _random_sample(selected_seq_groups: list[SequenceGroupToSample], random_samples: torch.Tensor) -> SampleResultType:
...
def _multinomial(probs: torch.Tensor, num_samples: int, seq_groups: Optional[list[SequenceGroupToSample]]=None) -> torch.Tensor:
    if num_samples > 1:
...
def _top_k_top_p_multinomial_with_flashinfer(probs: torch.Tensor, top_ks: torch.Tensor, top_ps: torch.Tensor, num_samples: int, seq_groups: Optional[list[SequenceGroupToSample]]):
    if num_samples > 1:
        probs = probs.repeat_interleave(num_samples, dim=0)
        top_ks = top_ks.repeat_interleave(num_samples)
        top_ps = top_ps.repeat_interleave(num_samples)
    batch_next_token_ids = flashinfer_top_k_top_p_sampling(probs, top_ks, top_ps)
...
def get_pythonized_sample_results(sample_result_args: SampleResultArgsType) -> SampleResultType:
    ...
    """
    sample_metadata, sampling_metadata, greedy_samples, multinomial_samples, sample_results_dict = (sample_result_args.sample_metadata, sample_result_args.sampling_metadata, sample_result_args.greedy_samples, sample_result_args.multinomial_samples, sample_result_args.sample_results_dict)
...
def _sample_with_torch(probs: torch.Tensor, logprobs: torch.Tensor, sampling_metadata: SamplingMetadata, sampling_tensors: SamplingTensors, include_gpu_probs_tensor: bool, modify_greedy_probs: bool) -> SampleReturnType:
    ...
    * Defer Pythonization & preserve GPU-side
    ...
    if include_gpu_probs_tensor:
        sampled_token_ids_tensor = torch.full((logprobs.shape[0], 1), VLLM_INVALID_TOKEN_ID, dtype=torch.long, device=logprobs.device)
    else:
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            multinomial_samples[sampling_type] = _multinomial(probs[long_sample_indices], max_n_in_batch, seq_groups=seq_groups_arg)
            if sampled_token_ids_tensor is not None:
                sampled_token_ids_tensor[long_sample_indices] = multinomial_samples[sampling_type].to(torch.long)
        else:
            raise ValueError(f'Unsupported sampling type: {sampling_type}')
    maybe_deferred_args = SampleResultArgsType(sampling_metadata=sampling_metadata, sample_metadata=sample_metadata, multinomial_samples=multinomial_samples, greedy_samples=greedy_samples, sample_results_dict=sample_results_dict)
    if not sampling_metadata.skip_sampler_cpu_output:
...
def _sample(probs: torch.Tensor, logprobs: torch.Tensor, sampling_metadata: SamplingMetadata, sampling_tensors: SamplingTensors, include_gpu_probs_tensor: bool, modify_greedy_probs: bool) -> SampleReturnType:
    """
    Args:
    ...
    """
    return _sample_with_torch(probs, logprobs, sampling_metadata, sampling_tensors, include_gpu_probs_tensor=include_gpu_probs_tensor, modify_greedy_probs=modify_greedy_probs)
...
def get_logprobs(logprobs: torch.Tensor, sampling_metadata: SamplingMetadata, sample_results: SampleResultType) -> tuple[list[Optional[PromptLogprobs]], list[SampleLogprobs]]:
    ...
    for seq_group, sample_result in zip(sampling_metadata.seq_groups, sample_results):
        prompt_logprobs, top_logprob_idx, selected_logprobs_idx = _get_prompt_logprob_if_needed(seq_group, selected_logprobs, ranks, top_token_ids, top_logprobs, selected_logprobs_idx, top_logprob_idx)
        prompt_logprobs_per_seq_group.append(prompt_logprobs)
        sampled_logprobs, top_logprob_idx, selected_logprobs_idx = _get_sampled_logprob_if_needed(seq_group, sample_result, selected_logprobs, ranks, top_token_ids, top_logprobs, selected_logprobs_idx, top_logprob_idx)
        sample_logprobs_per_seq_group.append(sampled_logprobs)
    return (prompt_logprobs_per_seq_group, sample_logprobs_per_seq_group)

def _get_prompt_logprob_if_needed(seq_group: SequenceGroupToSample, selected_logprobs: torch.Tensor, ranks: torch.Tensor, top_token_ids: torch.Tensor, top_logprobs: torch.Tensor, selected_logprobs_idx: int, top_logprob_idx: int):
    ...
    if is_prompt and sampling_params.prompt_logprobs is not None:
        ...
        next_prompt_tokens = _get_next_prompt_tokens(seq_group)
        selected_logprob_items = selected_logprobs[selected_logprobs_idx:selected_logprobs_idx + len(next_prompt_tokens)].tolist()
        ...
        for idx, token_id in enumerate(next_prompt_tokens):
            prompt_logprobs_dict: dict[int, tuple[float, int]] = {token_id: (selected_logprob_items[idx], rank_items[idx])}
            if num_logprobs > 0:
                ...
                prompt_logprobs_dict.update({top_id: (top_prob, rank) for top_id, top_prob, rank in zip(top_ids, top_probs, top_ranks)})
            prompt_logprobs.append({token_id: Logprob(*logprob_and_rank) for token_id, logprob_and_rank in prompt_logprobs_dict.items()})
            top_logprob_idx += 1
        selected_logprobs_idx += len(next_prompt_tokens)
    return (prompt_logprobs, top_logprob_idx, selected_logprobs_idx)

def _get_sampled_logprob_if_needed(seq_group: SequenceGroupToSample, sample_result: tuple[list[int], list[int]], selected_logprobs: torch.Tensor, ranks: torch.Tensor, top_token_ids: torch.Tensor, top_logprobs: torch.Tensor, selected_logprobs_idx: int, top_logprob_idx: int):
    ...
    if seq_group.do_sample:
        ...
        if num_logprobs is None:
            ...
            selected_logprob_items = selected_logprobs[selected_logprobs_idx:selected_logprobs_idx + len(next_token_ids)].tolist()
            ...
            for idx, (next_token_id, parent_id) in enumerate(zip(next_token_ids, parent_seq_ids)):
                ...
                sampled_logprobs.append({token_id: Logprob(*logprob_and_rank) for token_id, logprob_and_rank in sampled_logprobs_dict.items()})
...
def _modify_greedy_probs_inplace(logprobs: torch.Tensor, probs: torch.Tensor, sample_indices: torch.Tensor, greedy_samples: torch.Tensor) -> None:
...
def _build_sampler_output(maybe_deferred_sample_results: MaybeDeferredSampleResultType, sampling_metadata: SamplingMetadata, prompt_logprobs: Optional[list[Optional[PromptLogprobs]]], sample_logprobs: Optional[list[SampleLogprobs]], on_device_tensors: Optional[tuple[torch.Tensor, torch.Tensor, torch.Tensor]], skip_sampler_cpu_output: bool=False) -> SamplerOutput:
    ...
    if skip_sampler_cpu_output:
        ...
        assert not isinstance(maybe_deferred_sample_results, SampleResultArgsType)
        assert len(sampling_metadata.seq_groups) == len(maybe_deferred_sample_results) == len(prompt_logprobs) == len(sample_logprobs)
        deferred_sample_results_args = None
        for seq_group, sample_result, group_prompt_logprobs, group_sample_logprobs in zip(sampling_metadata.seq_groups, maybe_deferred_sample_results, prompt_logprobs, sample_logprobs):
            seq_ids = seq_group.seq_ids
    ...
    return SamplerOutput(outputs=sampler_output, sampled_token_probs=sampled_token_probs, sampled_token_ids=sampled_token_ids, logprobs=logprobs_tensor, deferred_sample_results_args=deferred_sample_results_args)
