...
def download_video_asset(filename: str) -> str:
    ...
    if not video_path.exists():
        video_path_str = hf_hub_download(repo_id='raushan-testing-hf/videos-test', filename=filename, repo_type='dataset', cache_dir=video_directory)
...
def video_to_ndarrays(path: str, num_frames: int=-1) -> npt.NDArray:
    ...
    if len(frames) < num_frames:
        raise ValueError(f'Could not read enough frames from video file {path} (expected {num_frames} frames, got {len(frames)})')
    return frames
...
def video_get_metadata(path: str) -> dict[str, Any]:
    ...
    metadata = {'total_num_frames': total_frames, 'fps': fps, 'duration': duration, 'video_backend': 'opencv'}
    return metadata
...
class VideoAsset:
    name: VideoAssetName
    num_frames: int = -1
    _NAME_TO_FILE: ClassVar[dict[VideoAssetName, str]] = {'baby_reading': 'sample_demo_1.mp4'}
