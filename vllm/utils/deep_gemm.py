...
def is_blackwell_deep_gemm_used() -> bool:
    ...
    Blackwell-class GPU.
    """
    if not (envs.VLLM_USE_DEEP_GEMM and has_deep_gemm() and (_per_block_cast_impl is not None)):
        return False
    return cuda_get_device_properties(0, ('major',))[0] == 10

def _missing(*_: Any, **__: Any) -> NoReturn:
    """Placeholder for unavailable DeepGEMM backend."""
    raise RuntimeError('DeepGEMM backend is not available. Please install the `deep_gemm` package to enable FP8 kernels.')
...
if not has_deep_gemm():
    ...
    _fp8_gemm_nt_impl = _resolve_symbol(_dg, 'fp8_gemm_nt', 'gemm_fp8_fp8_bf16_nt')
    _grouped_impl = _resolve_symbol(_dg, 'm_grouped_fp8_gemm_nt_contiguous', 'm_grouped_gemm_fp8_fp8_bf16_nt_contiguous')
    _grouped_masked_impl = _resolve_symbol(_dg, 'fp8_m_grouped_gemm_nt_masked', 'm_grouped_gemm_fp8_fp8_bf16_nt_masked')
    try:
        _math_mod = importlib.import_module('deep_gemm.utils.math')
        _per_block_cast_impl = getattr(_math_mod, 'per_block_cast_to_fp8', None)
...
def calc_diff(x: torch.Tensor, y: torch.Tensor):
    ...
    return 1 - sim
__all__ = ['calc_diff', 'fp8_gemm_nt', 'm_grouped_fp8_gemm_nt_contiguous', 'fp8_m_grouped_gemm_nt_masked', 'per_block_cast_to_fp8', 'is_blackwell_deep_gemm_used']