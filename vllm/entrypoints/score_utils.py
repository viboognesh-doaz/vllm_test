...
from vllm.entrypoints.chat_utils import BaseMultiModalItemTracker, ChatCompletionContentPartImageEmbedsParam, ChatCompletionContentPartImageParam, ChatCompletionContentPartTextParam, MultiModalItemTracker, _ContentPart, _parse_chat_message_content_part
from vllm.inputs import TokensPrompt
from vllm.model_executor.models.interfaces import supports_score_template
from vllm.multimodal.inputs import MultiModalDataDict
from vllm.outputs import PoolingRequestOutput
from vllm.transformers_utils.tokenizer import AnyTokenizer, PreTrainedTokenizer, PreTrainedTokenizerFast
...
def _cosine_similarity(tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast], embed_1: list[PoolingRequestOutput], embed_2: list[PoolingRequestOutput]) -> list[PoolingRequestOutput]:
    ...
    for emb_1, emb_2 in zip(embed_1, embed_2):
        ...
        scores.append(PoolingRequestOutput(request_id=f'{emb_1.request_id}_{emb_2.request_id}', outputs=pair_score, prompt_token_ids=tokens, finished=True))
    return scores

def _validate_score_input_lens(data_1: Union[list[str], list[ScoreContentPartParam]], data_2: Union[list[str], list[ScoreContentPartParam]]):
    len_1 = len(data_1)
    len_2 = len(data_2)
...
def parse_score_data(data_1: Union[str, ScoreContentPartParam], data_2: Union[str, ScoreContentPartParam], model_config: ModelConfig, tokenizer: AnyTokenizer) -> tuple[str, str, Optional[MultiModalDataDict]]:
...
def _parse_score_content(data: Union[str, ScoreContentPartParam], mm_tracker: BaseMultiModalItemTracker) -> Optional[_ContentPart]:
    if isinstance(data, str):
        data = ChatCompletionContentPartTextParam(type='text', text=data)
    mm_parser = mm_tracker.create_parser()
    parse_res = _parse_chat_message_content_part(data, mm_parser, wrap_dicts=False, interleave_strings=False)
    if parse_res:
        return parse_res
...
def apply_score_template(model_config: ModelConfig, prompt_1: str, prompt_2: str) -> str:
...
def get_score_prompt(model_config: ModelConfig, tokenizer: AnyTokenizer, tokenization_kwargs: dict[str, Any], data_1: Union[str, ScoreContentPartParam], data_2: Union[str, ScoreContentPartParam]) -> tuple[str, TokensPrompt]:
    prompt_1, prompt_2, mm_data = parse_score_data(data_1, data_2, model_config, tokenizer)
