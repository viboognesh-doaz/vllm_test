...
async def init_app(args: Namespace, llm_engine: Optional[AsyncLLMEngine]=None) -> FastAPI:
    ...
    engine = llm_engine if llm_engine is not None else AsyncLLMEngine.from_engine_args(engine_args, usage_context=UsageContext.API_SERVER)
...
async def run_server(args: Namespace, llm_engine: Optional[AsyncLLMEngine]=None, **uvicorn_kwargs: Any) -> None:
    ...
    assert engine is not None
    shutdown_task = await serve_http(app, sock=None, enable_ssl_refresh=args.enable_ssl_refresh, host=args.host, port=args.port, log_level=args.log_level, timeout_keep_alive=envs.VLLM_HTTP_TIMEOUT_KEEP_ALIVE, ssl_keyfile=args.ssl_keyfile, ssl_certfile=args.ssl_certfile, ssl_ca_certs=args.ssl_ca_certs, ssl_cert_reqs=args.ssl_cert_reqs, **uvicorn_kwargs)
    await shutdown_task
if __name__ == '__main__':
    ...
    parser.add_argument('--enable-ssl-refresh', action='store_true', default=False, help='Refresh SSL Context when SSL certificate files change')
