...
class HunyuanA13BToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.answer_tool_calls_pattern = re.compile('<tool_calls>([\\s\\S]*?)</tool_calls>', re.DOTALL)
        self.tool_name_reg = re.compile('"name"\\s*:\\s*"([^"]+)"')
        self.tool_empty_arg_reg = re.compile('"name"\\s*:\\s*"[^"]+"\\s*,\\s*"arguments"\\s*:\\s*\\{\\s*\\}')
        self.tool_non_empty_arg_reg = re.compile('"name"\\s*:\\s*"[^"]+"\\s*,\\s*"arguments"\\s*:\\s*(\\{(?:[^{}]|(?:\\{[^{}]*\\}))*\\})')
    ...
    def preprocess_model_output(self, model_output: str) -> tuple[Optional[str], Optional[str]]:
        for match in self.answer_tool_calls_pattern.finditer(model_output):
            ...
            think_regions = [(m.start(), m.end()) for m in re.finditer('<think>(.*?)</think>', model_output, flags=re.DOTALL)]
        ...
        try:
            ...
            for idx, call in enumerate(tool_calls_data):
                if not isinstance(call, dict) or 'name' not in call or 'arguments' not in call:
                    continue
                tool_call = ToolCall(id=f'call_{random_uuid()}', type='function', function=FunctionCall(name=call['name'], arguments=json.dumps(call['arguments']) if isinstance(call['arguments'], dict) else call['arguments']))
