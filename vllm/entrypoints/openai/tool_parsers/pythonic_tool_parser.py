...
class PythonicToolParser(ToolParser):
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        try:
            is_tool_call_pattern = self.TOOL_CALL_REGEX.match(model_output, timeout=envs.VLLM_TOOL_PARSE_REGEX_TIMEOUT_SECONDS) is not None
        except TimeoutError:
        ...
        try:
            ...
            if isinstance(parsed, ast.List) and all((isinstance(e, ast.Call) for e in parsed.elts)):
                return ExtractedToolCallInformation(tools_called=True, tool_calls=[_handle_single_tool(e) for e in parsed.elts], content=None)
            else:
...
def _get_parameter_value(val: ast.expr) -> Any:
    if isinstance(val, ast.Constant):
        ...
        return {k.value: _get_parameter_value(v) for k, v in zip(val.keys, val.values)}
...
def _handle_single_tool(call: ast.Call) -> ToolCall:
    ...
    return ToolCall(type='function', function=FunctionCall(name=function_name, arguments=json.dumps(arguments, ensure_ascii=False)))
...
def _compute_tool_delta(previously_sent_args: str, new_call: ToolCall, index: int, withheld_suffix: str) -> Union[DeltaToolCall, None]:
    new_call_args = new_call.function.arguments
    if withheld_suffix:
        assert new_call_args.endswith(withheld_suffix)
        new_call_args = new_call_args[:-len(withheld_suffix)]
    if not previously_sent_args:
        return DeltaToolCall(id=new_call.id, type='function', index=index, function=DeltaFunctionCall(name=new_call.function.name, arguments=new_call_args))
