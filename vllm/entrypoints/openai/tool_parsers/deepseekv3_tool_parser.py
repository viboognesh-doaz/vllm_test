...
class DeepSeekV3ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.tool_call_regex = re.compile('<｜tool▁call▁begin｜>(?P<type>.*)<｜tool▁sep｜>(?P<function_name>.*)\\n```json\\n(?P<function_arguments>.*)\\n```<｜tool▁call▁end｜>')
        self.stream_tool_call_portion_regex = re.compile('(?P<type>.*)<｜tool▁sep｜>(?P<function_name>.*)\\n```json\\n(?P<function_arguments>.*[^\\n`])')
        self.stream_tool_call_name_regex = re.compile('(?P<type>.*)<｜tool▁sep｜>(?P<function_name>.*)\\n')
        ...
        if self.tool_calls_start_token not in model_output:
            ...
            try:
                ...
                for match in function_call_tuples:
                    ...
                    tool_calls.append(ToolCall(type=tool_type, function=FunctionCall(name=function_name, arguments=function_args)))
        ...
        try:
            ...
            if not self.current_tool_name_sent:
                ...
                if function_name:
                    ...
                    return DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, type='function', id=random_tool_call_id(), function=DeltaFunctionCall(name=function_name).model_dump(exclude_none=True))])
                else:
                    return None
