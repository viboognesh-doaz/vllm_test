...
class MinimaxToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.tool_call_regex = re.compile('<tool_calls>(.*?)</tool_calls>|<tool_calls>(.*)', re.DOTALL)
        self.thinking_tag_pattern = '<think>(.*?)</think>'
        ...
        if self.tool_call_start_token_id is None or self.tool_call_end_token_id is None:
            logger.warning('Minimax Tool parser could not locate tool call start/end tokens in the tokenizer. Falling back to string matching.')
    ...
    def preprocess_model_output(self, model_output: str) -> str:
        ...
        def remove_tool_calls_from_think(match):
            ...
            cleaned_content = re.sub('<tool_calls>.*?</tool_calls>', '', think_content, flags=re.DOTALL)
            return f'<think>{cleaned_content}</think>'
        processed_output = re.sub(self.thinking_tag_pattern, remove_tool_calls_from_think, model_output, flags=re.DOTALL)
        ...
        try:
            ...
            for function_call in raw_function_calls:
                if 'name' in function_call and 'arguments' in function_call:
                    tool_calls.append(ToolCall(type='function', function=FunctionCall(name=function_call['name'], arguments=json.dumps(function_call['arguments'], ensure_ascii=False))))
            ...
            return ExtractedToolCallInformation(tools_called=len(tool_calls) > 0, tool_calls=tool_calls, content=content.strip() if content.strip() else None)
        except Exception:
    ...
    def extract_tool_calls_streaming(self, previous_text: str, current_text: str, delta_text: str, previous_token_ids: Sequence[int], current_token_ids: Sequence[int], delta_token_ids: Sequence[int], request: ChatCompletionRequest) -> Union[DeltaMessage, None]:
        ...
        if self.tool_call_start_token_id is not None and self.tool_call_start_token_id in delta_token_ids and (len(delta_token_ids) == 1):
            return None
        ...
        try:
            parsable_content = processed_current_text.split(self.tool_call_start_token)[-1].split(self.tool_call_end_token)[0]
            tool_call_arr = []
                ...
                for line in lines:
                    ...
                    if line and (line.startswith('{') or '"name"' in line):
                        try:
                            ...
                            continue
            current_tool_call: dict = tool_call_arr[self.current_tool_id] if len(tool_call_arr) > self.current_tool_id >= 0 else {}
            ...
            if not self.current_tool_name_sent:
                ...
                prev_arguments = None
                if self.current_tool_id < len(self.prev_tool_call_arr) and self.prev_tool_call_arr[self.current_tool_id]:
                ...
                if not cur_arguments and (not prev_arguments):
                    ...
                    delta = DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, function=DeltaFunctionCall(arguments=cur_arguments_json).model_dump(exclude_none=True))])
