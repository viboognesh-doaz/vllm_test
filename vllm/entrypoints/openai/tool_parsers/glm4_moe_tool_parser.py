...
class Glm4MoeModelToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.tool_call_regex = re.compile('<tool_call>\\s*(?P<function_name>[^\\n<]+)\\s*(?P<arguments>(?:\\s*<arg_key>[^<]+</arg_key>\\s*<arg_value>[^<]*</arg_value>\\s*)*)\\s*</tool_call>', re.DOTALL)
        self.arg_regex = re.compile('<arg_key>(?P<key>[^<]+)</arg_key>\\s*<arg_value>(?P<value>[^<]*)</arg_value>', re.DOTALL)
        self.stream_tool_call_portion_regex = re.compile('(?P<function_name>[^\\n<]+)\\s*(?P<arguments>(?:\\s*<arg_key>[^<]+</arg_key>\\s*<arg_value>[^<]*</arg_value>\\s*)*)', re.DOTALL)
        self.stream_tool_call_name_regex = re.compile('(?P<function_name>[^\\n<]+)', re.DOTALL)
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        try:
            ...
            for i, match in enumerate(function_call_matches):
                ...
                tool_calls.append(ToolCall(id=f'call_{i}', type='function', function=FunctionCall(name=function_name, arguments=function_args_json)))
            content = model_output[:model_output.find(self.tool_calls_start_token)]
            return ExtractedToolCallInformation(tools_called=bool(tool_calls), tool_calls=tool_calls, content=content.strip() if content.strip() else None)
        except Exception:
    ...
    def extract_tool_calls_streaming(self, previous_text: str, current_text: str, delta_text: str, previous_token_ids: Sequence[int], current_token_ids: Sequence[int], delta_token_ids: Sequence[int], request: ChatCompletionRequest) -> Union[DeltaMessage, None]:
        ...
        try:
            ...
            text_portion = None
            if cur_tool_start_count == cur_tool_end_count and prev_tool_end_count == cur_tool_end_count and (self.tool_call_end_token not in delta_text):
                logger.debug('Generating text content! skipping tool parsing.')
                return DeltaMessage(content=delta_text)
            if self.tool_call_end_token in delta_text:
                logger.debug('tool_call_end_token in delta_text')
                full_text = current_text + delta_text
                tool_call_portion = full_text.split(self.tool_call_start_token)[-1].split(self.tool_call_end_token)[0].rstrip()
            ...
            if cur_tool_start_count > cur_tool_end_count and cur_tool_start_count > prev_tool_start_count:
                ...
                if diff:
                    ...
                    return DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, function=DeltaFunctionCall(arguments=diff).model_dump(exclude_none=True))])
            else:
            ...
            if not self.current_tool_name_sent:
                ...
                if function_name:
                    ...
                    return DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, type='function', id=tool_id, function=DeltaFunctionCall(name=function_name).model_dump(exclude_none=True))])
                else:
                    return None
            ...
            if not cur_arguments and (not prev_arguments):
                ...
                delta = DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, function=DeltaFunctionCall(arguments=cur_arguments).model_dump(exclude_none=True))])
                self.streamed_args_for_tool[self.current_tool_id] = cur_arguments
            elif cur_arguments and prev_arguments:
                if isinstance(delta_text, str) and cur_arguments != prev_arguments and (len(cur_arguments) > len(prev_arguments)) and cur_arguments.startswith(prev_arguments):
                    delta_arguments = cur_arguments[len(prev_arguments):]
                    logger.debug('got diff %s', delta_text)
                    delta = DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, function=DeltaFunctionCall(arguments=delta_arguments).model_dump(exclude_none=True))])
                    self.streamed_args_for_tool[self.current_tool_id] = cur_arguments
                else:
                    delta = None
