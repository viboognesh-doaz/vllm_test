...
class Internlm2ToolParser(ToolParser):
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        if '<|action_start|><|plugin|>' in text:
            ...
            name, parameters = (action_dict['name'], json.dumps(action_dict.get('parameters', action_dict.get('arguments', {})), ensure_ascii=False))
            ...
            return ExtractedToolCallInformation(tools_called=True, tool_calls=tool_calls, content=text if len(text) > 0 else None)
