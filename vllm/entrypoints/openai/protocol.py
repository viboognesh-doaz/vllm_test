...
class OpenAIBaseModel(BaseModel):
    ...
    def __log_extra_fields__(cls, data, handler):
        ...
        if any((k not in field_names for k in data)):
            logger.warning('The following fields were present in the request but ignored: %s', data.keys() - field_names)
        return result
...
class ResponsesRequest(OpenAIBaseModel):
    ...
    def to_sampling_params(self, default_max_tokens: int, default_sampling_params: Optional[dict]=None) -> SamplingParams:
        ...
        return SamplingParams.from_optional(temperature=temperature, top_p=top_p, max_tokens=max_tokens, logprobs=self.top_logprobs, output_kind=RequestOutputKind.DELTA if self.stream else RequestOutputKind.FINAL_ONLY, guided_decoding=guided_decoding)
    ...
    def check_cache_salt_support(cls, data):
        if data.get('cache_salt') is not None:
            if not envs.VLLM_USE_V1:
                raise ValueError("Parameter 'cache_salt' is not supported with this instance of vLLM, which uses engine V0.")
...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    tool_choice: Optional[Union[Literal['none'], Literal['auto'], Literal['required'], ChatCompletionNamedToolChoiceParam]] = 'none'
    ...
    vllm_xargs: Optional[dict[str, Union[str, int, float]]] = Field(default=None, description='Additional request parameters with string or numeric values, used by custom extensions.')
    ...
    def to_sampling_params(self, max_tokens: int, logits_processor_pattern: Optional[str], default_sampling_params: dict) -> SamplingParams:
    ...
    def check_tool_usage(cls, data):
        ...
        if 'tool_choice' in data:
            ...
            if data['tool_choice'] not in ['auto', 'required'] and (not isinstance(data['tool_choice'], dict)):
                raise NotImplementedError(f"""Invalid value for `tool_choice`: {data['tool_choice']}! Only named tools, "none", "auto" or "required" are supported.""")
...
class CompletionRequest(OpenAIBaseModel):
    ...
    response_format: Optional[AnyResponseFormat] = Field(default=None, description="Similar to chat completion, this parameter specifies the format of output. Only {'type': 'json_object'}, {'type': 'json_schema'}, {'type': 'structural_tag'}, or {'type': 'text' } is supported.")
    guided_json: Optional[Union[str, dict, BaseModel]] = Field(default=None, description='If specified, the output will follow the JSON schema.')
    guided_regex: Optional[str] = Field(default=None, description='If specified, the output will follow the regex pattern.')
    guided_choice: Optional[list[str]] = Field(default=None, description='If specified, the output will be exactly one of the choices.')
    guided_grammar: Optional[str] = Field(default=None, description='If specified, the output will follow the context free grammar.')
    ...
    def to_sampling_params(self, max_tokens: int, logits_processor_pattern: Optional[str], default_sampling_params: Optional[dict]=None) -> SamplingParams:
        if default_sampling_params is None:
            default_sampling_params = {}
        if (repetition_penalty := self.repetition_penalty) is None:
            repetition_penalty = default_sampling_params.get('repetition_penalty', self._DEFAULT_SAMPLING_PARAMS['repetition_penalty'])
