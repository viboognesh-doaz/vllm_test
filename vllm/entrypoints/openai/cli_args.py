...
from vllm.entrypoints.chat_utils import ChatTemplateContentFormatOption, validate_chat_template
...
class FrontendArgs:
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        frontend_kwargs['prompt_adapters']['action'] = PromptAdapterParserAction
...
def make_arg_parser(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
    ...
    parser.add_argument('--data-parallel-start-rank', '-dpr', type=int, default=0, help='Starting data parallel rank for secondary nodes. Requires --headless.')
    parser.add_argument('--api-server-count', '-asc', type=int, default=1, help='How many API server processes to run.')
    parser.add_argument('--config', help='Read CLI options from a config file. Must be a YAML with the following options: https://docs.vllm.ai/en/latest/configuration/serve_args.html')
...
def validate_parsed_serve_args(args: argparse.Namespace):
    ...
    if args.enable_prompt_embeds and args.enable_prompt_adapter:
        raise ValueError('Cannot use prompt embeds and prompt adapter at the same time.')
...
def create_parser_for_docs() -> FlexibleArgumentParser:
    parser_for_docs = FlexibleArgumentParser(prog='-m vllm.entrypoints.openai.api_server')
