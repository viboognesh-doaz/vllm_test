...
from vllm.entrypoints.openai.protocol import ErrorResponse, LoadLoRAAdapterRequest, ModelCard, ModelList, ModelPermission, UnloadLoRAAdapterRequest
...
class OpenAIServingModels:
    ...
    def __init__(self, engine_client: EngineClient, model_config: ModelConfig, base_model_paths: list[BaseModelPath], *, lora_modules: Optional[list[LoRAModulePath]]=None, prompt_adapters: Optional[list[PromptAdapterPath]]=None):
        super().__init__()
        ...
        if prompt_adapters is not None:
            for i, prompt_adapter in enumerate(prompt_adapters, start=1):
                with pathlib.Path(prompt_adapter.local_path, 'adapter_config.json').open() as f:
                    adapter_config = json.load(f)
                    num_virtual_tokens = adapter_config['num_virtual_tokens']
                self.prompt_adapter_requests.append(PromptAdapterRequest(prompt_adapter_name=prompt_adapter.name, prompt_adapter_id=i, prompt_adapter_local_path=prompt_adapter.local_path, prompt_adapter_num_virtual_tokens=num_virtual_tokens))
    ...
    async def show_available_models(self) -> ModelList:
        ...
        prompt_adapter_cards = [ModelCard(id=prompt_adapter.prompt_adapter_name, root=self.base_model_paths[0].name, permission=[ModelPermission()]) for prompt_adapter in self.prompt_adapter_requests]
            ...
            for resolver in self.lora_resolvers:
                ...
                if lora_request is not None:
                    ...
                    try:
                        await self.engine_client.add_lora(lora_request)
                        self.lora_requests[lora_name] = lora_request
                        logger.info("Resolved and loaded LoRA adapter '%s' using %s", lora_name, resolver.__class__.__name__)
                        return lora_request
                    except BaseException as e:
                        logger.warning("Failed to load LoRA '%s' resolved by %s: %s. Trying next resolver.", lora_name, resolver.__class__.__name__, e)
                        continue
            if found_adapter:
