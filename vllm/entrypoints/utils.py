...
async def listen_for_disconnect(request: Request) -> None:
    ...
    while True:
        ...
        if message['type'] == 'http.disconnect':
            if getattr(request.app.state, 'enable_server_load_tracking', False) and hasattr(request.app.state, 'server_load_metrics'):
...
def _validate_truncation_size(max_model_len: int, truncate_prompt_tokens: Optional[int], tokenization_kwargs: Optional[dict[str, Any]]=None) -> Optional[int]:
    if truncate_prompt_tokens is not None:
        if truncate_prompt_tokens <= -1:
            truncate_prompt_tokens = max_model_len
        if truncate_prompt_tokens > max_model_len:
            raise ValueError(f'truncate_prompt_tokens value ({truncate_prompt_tokens}) is greater than max_model_len ({max_model_len}). Please, select a smaller truncation size.')
        if tokenization_kwargs is not None:
            tokenization_kwargs['truncation'] = True
            tokenization_kwargs['max_length'] = truncate_prompt_tokens
    elif tokenization_kwargs is not None:
...
def get_max_tokens(max_model_len: int, request: Union[ChatCompletionRequest, CompletionRequest], input_length: int, default_sampling_params: dict) -> int:
    max_tokens = getattr(request, 'max_completion_tokens', None) or request.max_tokens
    default_max_tokens = max_model_len - input_length
    max_output_tokens = current_platform.get_max_output_tokens(input_length)
    return min((val for val in (default_max_tokens, max_tokens, max_output_tokens, default_sampling_params.get('max_tokens')) if val is not None))