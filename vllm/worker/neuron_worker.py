...
class NeuronWorker(LocalOrDistributedWorkerBase):
    ...
    def __init__(self, vllm_config: VllmConfig, local_rank: int, rank: int, distributed_init_method: str, is_driver_worker: bool=False) -> None:
        ...
        if neuron_framework == NeuronFramework.TRANSFORMERS_NEURONX:
            ...
            self.model_runner = self.get_neuronx_distributed_model_runner(vllm_config)
        else:
            raise NotImplementedError('Specified framework' + f" {os.environ.get('VLLM_NEURON_FRAMEWORK')}" + ' is either not installed or not supported.' + ' Supported frameworks: ' + '[transformers-neuronx, neuronx-distributed-inference]')

    def get_tnx_model_runner(self, vllm_config):
        assert self.lora_config is None, 'LoRA is not supported for TransformersNeuronX framework.'
    ...
    def get_neuronx_distributed_model_runner(self, vllm_config):
        if self.speculative_config is not None:
            assert self.lora_config is None, 'LoRA is not supported for Speculative Decoding'
            return MultiStepNeuronxDistributedModelRunner(vllm_config=vllm_config)
        else:
    ...
    def prepare_worker_input(self, execute_model_req: ExecuteModelRequest) -> WorkerInput:
        return WorkerInput(num_seq_groups=len(execute_model_req.seq_group_metadata_list))
    ...
    def init_distributed_environment(self):
        ...
        """
        init_distributed_environment(world_size=1, rank=self.rank, local_rank=self.local_rank, distributed_init_method=self.distributed_init_method, backend=current_platform.dist_backend)
        ensure_model_parallel_initialized(1, 1)

    def add_lora(self, lora_request: LoRARequest) -> bool:
        if current_platform.use_transformers_neuronx():
            raise NotImplementedError(f'{type(self)} does not support LoRA with Neuron Framework Transformers NeuronX')
