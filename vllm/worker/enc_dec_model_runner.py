...
from vllm.worker.model_runner import GPUModelRunnerBase, ModelInputForGPUBuilder, ModelInputForGPUWithSamplingMetadata
...
class EncoderDecoderModelRunner(GPUModelRunnerBase[EncoderDecoderModelInput]):
    ...
    def __init__(self, vllm_config: VllmConfig, kv_cache_dtype: Optional[str]='auto', is_driver_worker: bool=False, input_registry: InputRegistry=INPUT_REGISTRY, mm_registry: MultiModalRegistry=MULTIMODAL_REGISTRY):
        """
        ...
        super().__init__(vllm_config=vllm_config, kv_cache_dtype=kv_cache_dtype, is_driver_worker=is_driver_worker, input_registry=input_registry, mm_registry=mm_registry)
    ...
    def execute_model(self, model_input: EncoderDecoderModelInput, kv_caches: List[torch.Tensor], intermediate_tensors: Optional[IntermediateTensors]=None, num_steps: int=1) -> Optional[List[PoolerOutput]]:
        if num_steps > 1:
            raise ValueError('num_steps > 1 is not supported in EncoderDecoderModelRunner')
        if self.lora_config:
            assert model_input.lora_requests is not None
            assert model_input.lora_mapping is not None
            self.set_active_loras(model_input.lora_requests, model_input.lora_mapping)
        if model_input.attn_metadata is not None and model_input.attn_metadata.prefill_metadata is None and model_input.attn_metadata.decode_metadata.use_cuda_graph:
            if model_input.inputs_embeds is None:
                assert model_input.input_tokens is not None
                graph_batch_size = model_input.input_tokens.shape[0]
                model_executable = self.graph_runners[model_input.virtual_engine][graph_batch_size, False]
            else:
                graph_batch_size = model_input.inputs_embeds.shape[0]
                model_executable = self.graph_runners[model_input.virtual_engine][graph_batch_size, True]
        else:
        ...
        with set_forward_context(model_input.attn_metadata, self.vllm_config, model_input.virtual_engine):
            hidden_or_intermediate_states = model_executable(input_ids=model_input.input_tokens, inputs_embeds=model_input.inputs_embeds, positions=model_input.input_positions, encoder_input_ids=model_input.encoder_input_tokens, encoder_positions=model_input.encoder_input_positions, intermediate_tensors=intermediate_tensors, **MultiModalKwargs.as_kwargs(multi_modal_kwargs, device=self.device), **seqlen_agnostic_kwargs)
    ...
    def make_model_input_from_broadcasted_tensor_dict(self, tensor_dict: Dict[str, Any]) -> EncoderDecoderModelInput:
        return EncoderDecoderModelInput.from_broadcasted_tensor_dict(tensor_dict, attn_backend=self.attn_backend)

    def prepare_model_input(self, seq_group_metadata_list: List[SequenceGroupMetadata], virtual_engine: int=0, finished_requests_ids: Optional[List[str]]=None) -> EncoderDecoderModelInput:
        ...
        attn_metadata, encoder_input_tokens_tensor, encoder_input_positions_tensor = self._prepare_encoder_model_input_tensors(seq_group_metadata_list, model_input)
        model_input = dataclasses.replace(model_input, attn_metadata=attn_metadata, encoder_input_tokens=encoder_input_tokens_tensor, encoder_input_positions=encoder_input_positions_tensor)
        generators = self.get_generators(finished_requests_ids)
        sampling_metadata = SamplingMetadata.prepare(seq_group_metadata_list, model_input.seq_lens, model_input.query_lens, self.device, self.pin_memory, generators=generators)
    ...
    def profile_run(self) -> None:
        ...
        for group_id in range(max_num_seqs):
            ...
            batch_size += seq_len
            decoder_dummy_data = self.input_registry.dummy_data_for_profiling(self.model_config, seq_len, self.mm_registry, is_encoder_data=False)
            encoder_dummy_data = self.input_registry.dummy_data_for_profiling(self.model_config, seq_len, self.mm_registry, is_encoder_data=True)
            assert len(decoder_dummy_data.seq_data.prompt_token_ids) >= seq_len, f'Expected at least {seq_len} dummy tokens for profiling, but got: {len(decoder_dummy_data.seq_data.prompt_token_ids)}'
            assert decoder_dummy_data.multi_modal_data is None or encoder_dummy_data.multi_modal_data is None, "Multi-modal data can't be provided in both encoder and decoder"
    ...
    def _prepare_encoder_model_input_tensors(self, seq_group_metadata_list: List[SequenceGroupMetadata], model_input: EncoderDecoderModelInput) -> Tuple[AttentionMetadata, Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        * Updated model inputs data structure
        ...
        if is_prompt:
            cross_block_tables = self._empty_int32_tensor().view(len(seq_group_metadata_list), -1)
            encoder_input_tokens, encoder_input_positions, cross_slot_mapping = ([], [], [])
            for seq_group_metadata in seq_group_metadata_list:
                ...
                if is_profile_run:
                    ...
                    for i in range(0, seq_len):
                        block_number = seq_group_metadata.cross_block_table[i // self.block_size]
            ...
            for seq_group_metadata in seq_group_metadata_list:
                for _ in range(len(seq_group_metadata.seq_data)):
                    ...
                    cross_block_tables.append([] if cross_block_table is None else cross_block_table)
