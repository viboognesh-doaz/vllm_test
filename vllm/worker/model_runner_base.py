...
from typing import TYPE_CHECKING, Any, Dict, Generic, List, Optional, Type, TypeVar, get_args
...
def _add_attn_metadata_broadcastable_dict(tensor_dict: Dict[str, Any], attn_metadata: Optional['AttentionMetadata']) -> None:
    """
...
def _init_attn_metadata_from_tensor_dict(attn_backend: 'AttentionBackend', tensor_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def _init_sampling_metadata_from_tensor_dict(tensor_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if selected_token_indices is not None:
        tensor_dict['sampling_metadata'] = SamplingMetadata(seq_groups=None, selected_token_indices=selected_token_indices, categorized_sample_indices=None, num_prompts=0)
    return tensor_dict

def _add_sampling_metadata_broadcastable_dict(tensor_dict: Dict[str, Any], sampling_metadata: Optional['SamplingMetadata']) -> None:
    """
    Helper method to update tensor_dict with broadcastable
    SamplingMetadata fields.
    """
...
def _init_frozen_model_input_from_tensor_dict(frozen_model_input_cls: Type['ModelRunnerInputBase'], tensor_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
...
class BroadcastableModelInput(ABC):
    ...
    @classmethod
    @abstractmethod
    def from_broadcasted_tensor_dict(cls: Type[T], tensor_dict: Dict[str, Any], attn_backend: Optional['AttentionBackend']=None) -> T:
        """
...
class ModelRunnerBase(ABC, Generic[T]):
    ...
    @abstractmethod
    def make_model_input_from_broadcasted_tensor_dict(self, tensor_dict: Dict[str, Any]) -> T:
        """
    ...
    @abstractmethod
    def prepare_model_input(self, seq_group_metadata_list: List[SequenceGroupMetadata], virtual_engine: int=0, finished_requests_ids: Optional[List[str]]=None) -> T:
        """
    ...
    def execute_model(self, model_input: T, kv_caches: Optional[List[torch.Tensor]], intermediate_tensors: Optional[IntermediateTensors]=None, num_steps: int=1, **kwargs) -> Optional[List[SamplerOutput]]:
        """
...
class ModelRunnerWrapperBase:
    ...
    def __init__(self, model_runner: ModelRunnerBase) -> None:
...
class InputProcessingError(Exception):
    ...
    def __str__(self):
        return f'Failed to prepare inputs for sequence group with request id: {self.request_id}, Error: {self.message}'