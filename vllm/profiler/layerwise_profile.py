...
from vllm.profiler.utils import TablePrinter, event_has_module, event_is_torch_op, event_module_repr, event_torch_op_stack_trace, indent_string
import copy
import pandas as pd
...
class LayerwiseProfileResults(profile):
    ...
    def print_model_table(self, column_widths: dict[str, int]=None):
        ...
        filtered_model_table = [(depth, row) for depth, row in self._flatten_stats_tree(self._model_stats_tree) if row.cuda_time_us > 0 or row.cpu_time_us > 0]
        TablePrinter(ModelStatsEntry, _column_widths).print_table(self._indent_row_names_based_on_depth(filtered_model_table, indent_style=lambda indent: '|' + '-' * indent + ' '))
    ...
    def print_summary_table(self, column_widths: dict[str, int]=None):
        ...
        if column_widths:
            _column_widths.update(**column_widths)
        filtered_summary_table = [(depth, row) for depth, row in self._flatten_stats_tree(self._summary_stats_tree) if row.cuda_time_us > 0]
        TablePrinter(SummaryStatsEntry, _column_widths).print_table(self._indent_row_names_based_on_depth(filtered_summary_table, indent_style=lambda indent: '|' + '-' * indent + ' '))
    ...
    def convert_stats_to_dict(self) -> dict[str, Any]:
        return {'metadata': {'num_running_seqs': self.num_running_seqs}, 'summary_stats': self._convert_stats_tree_to_dict(self._summary_stats_tree), 'model_stats': self._convert_stats_tree_to_dict(self._model_stats_tree)}

    @staticmethod
    def _indent_row_names_based_on_depth(depths_rows: list[tuple[int, StatsEntry]], indent_style: Union[Callable[[int], str], str]=' '):
        indented_rows = []
    ...
    def _build_module_tree(self):
        ...
        def _df_traversal(event: _ProfilerEvent, curr_node: Optional[_ModuleTreeNode]=None):
            ...
            if is_leaf and curr_node:
                node = _ModuleTreeNode(event=event, parent=curr_node, trace=event_torch_op_stack_trace(event, until=lambda x: event_has_module(x)))
                curr_node.children.append(node)
                curr_node = node
    ...
    def _get_kineto_gpu_event(self, node: _ModuleTreeNode):
        ...
        iterator = (x for x in correlated_kineto_events if x.device_type() == DeviceType.CUDA and x.name() == node.event.name)
    ...
    def _build_stats_trees(self):
        ...
        def build_summary_stats_tree_df(node: _ModuleTreeNode, parent: Optional[_StatsTreeNode]=None, summary_trace: tuple[str]=()):
            ...
            if summary_trace in summary_dict:
                ...
                new_node = _StatsTreeNode(entry=SummaryStatsEntry(name=name, cuda_time_us=cuda_time_us, pct_cuda_time=pct_cuda_time(cuda_time_us), invocations=1), children=[], parent=parent)
                if parent:
        ...
        def build_model_stats_tree_df(node: _ModuleTreeNode, parent: Optional[_StatsTreeNode]=None):
            if event_has_module(node.event):
                name = event_module_repr(node.event)
                cuda_time_us = self._cumulative_cuda_time(node)
                ...
                return None
            new_node = _StatsTreeNode(entry=ModelStatsEntry(name=name, cpu_time_us=cpu_time_us, cuda_time_us=cuda_time_us, pct_cuda_time=pct_cuda_time(cuda_time_us), trace=trace), parent=parent, children=[])
            if parent:
                parent.children.append(new_node)
            for child in node.children:
...
class layerwise_profile(profile):
    ...
    def __init__(self, num_running_seqs: Optional[int]=None):
        ...
        """
        super().__init__(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True, with_stack=True, with_modules=True, experimental_config=_ExperimentalConfig(verbose=True))
