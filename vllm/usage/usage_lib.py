...
_USAGE_ENV_VARS_TO_COLLECT = ['VLLM_USE_MODELSCOPE', 'VLLM_USE_TRITON_FLASH_ATTN', 'VLLM_ATTENTION_BACKEND', 'VLLM_USE_FLASHINFER_SAMPLER', 'VLLM_PP_LAYER_PARTITION', 'VLLM_USE_TRITON_AWQ', 'VLLM_USE_V1', 'VLLM_ENABLE_V1_MULTIPROCESSING']
...
def _get_current_timestamp_ns() -> int:
    return int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000000000.0)

def _detect_cloud_provider() -> str:
    vendor_files = ['/sys/class/dmi/id/product_version', '/sys/class/dmi/id/bios_vendor', '/sys/class/dmi/id/product_name', '/sys/class/dmi/id/chassis_asset_tag', '/sys/class/dmi/id/sys_vendor']
    cloud_identifiers = {'amazon': 'AWS', 'microsoft corporation': 'AZURE', 'google': 'GCP', 'oraclecloud': 'OCI'}
...
class UsageMessage:
    ...
    def report_usage(self, model_architecture: str, usage_context: UsageContext, extra_kvs: Optional[dict[str, Any]]=None) -> None:
        t = Thread(target=self._report_usage_worker, args=(model_architecture, usage_context, extra_kvs or {}), daemon=True)
        t.start()

    def _report_usage_worker(self, model_architecture: str, usage_context: UsageContext, extra_kvs: dict[str, Any]) -> None:
        self._report_usage_once(model_architecture, usage_context, extra_kvs)
        self._report_continuous_usage()

    def _report_usage_once(self, model_architecture: str, usage_context: UsageContext, extra_kvs: dict[str, Any]) -> None:
        ...
        self.cpu_family_model_stepping = ','.join([str(info.get('family', '')), str(info.get('model', '')), str(info.get('stepping', ''))])
