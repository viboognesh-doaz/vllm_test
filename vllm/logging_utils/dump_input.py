...
def _dump_engine_exception(config: VllmConfig, scheduler_output: SchedulerOutput, scheduler_stats: Optional[SchedulerStats]):
    logger.error('Dumping input data for V1 LLM engine (v%s) with config: %s, ', VLLM_VERSION, config)
    try:
