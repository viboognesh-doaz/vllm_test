...
from vllm.multimodal import MultiModalDataDict, MultiModalPlaceholderDict, MultiModalRegistry
...
class InputProcessingContext(InputContext):
    ...
    def call_hf_processor(self, hf_processor: ProcessorMixin, data: Mapping[str, object], kwargs: Mapping[str, object]={}) -> Union[BatchFeature, JSONTree]:
        ...
        if base_kwargs is None:
            base_kwargs = {}
        merged_kwargs = resolve_mm_processor_kwargs(base_kwargs, kwargs, hf_processor, requires_kw_only=False, allow_var_kwargs=True)
        ...
        try:
            ...
            logger.warning_once(f'{type(hf_processor).__name__} did not return `BatchFeature`. Make sure to match the behaviour of `ProcessorMixin` when implementing custom processors.')
            return cast_output
        except Exception as exc:
            msg = f'Failed to apply {type(hf_processor).__name__} on data={data} with kwargs={merged_kwargs}'
