...
class ProcessWorkerWrapper:
    ...
    def __init__(self, result_handler: ResultHandler, worker_factory: Callable[[VllmConfig, int], Any], vllm_config: VllmConfig, rank: int) -> None:
        self.mp = get_mp_context()
        self._task_queue = self.mp.Queue()
        self.result_queue = result_handler.result_queue
        self.tasks = result_handler.tasks
        self.process: BaseProcess = self.mp.Process(target=_run_worker_process, name='VllmWorkerProcess', kwargs=dict(worker_factory=worker_factory, task_queue=self._task_queue, result_queue=self.result_queue, vllm_config=vllm_config, rank=rank), daemon=True)
        self.process.start()

    def _enqueue_task(self, future: Union[ResultFuture, asyncio.Future], method: Union[str, bytes], args, kwargs):
        task_id = uuid.uuid4()
...
def _run_worker_process(worker_factory: Callable[[VllmConfig, int], Any], task_queue: Queue, result_queue: Queue, vllm_config: VllmConfig, rank: int) -> None:
    ...
    try:
        for items in iter(task_queue.get, _TERMINATE):
            ...
            try:
                ...
                logger.exception('Exception in worker %s while processing method %s.', process_name, method)
                exception = e
    ...
    if torch.cuda.is_available():
        if tunable.is_enabled() and tunable.tuning_is_enabled() and (not tunable.record_untuned_is_enabled()):
            tunable.write_file()
...
def set_multiprocessing_worker_envs(parallel_config):
    ...
    if 'OMP_NUM_THREADS' not in os.environ and (current_parallelism := torch.get_num_threads()) > default_omp_num_threads:
        logger.warning('Reducing Torch parallelism from %d threads to %d to avoid unnecessary CPU contention. Set OMP_NUM_THREADS in the external environment to tune this value as needed.', current_parallelism, default_omp_num_threads)
