...
from vllm.utils import _run_task_with_lock, get_distributed_init_method, get_ip, get_open_port, make_async
if ray is not None:
...
class RayDistributedExecutor(DistributedExecutorBase):
    ...
    WORKER_SPECIFIC_ENV_VARS = {'VLLM_HOST_IP', 'VLLM_HOST_PORT', 'LOCAL_RANK', 'CUDA_VISIBLE_DEVICES'}
    uses_ray: bool = True
    ...
    def _init_workers_ray(self, placement_group: 'PlacementGroup', **ray_remote_kwargs):
        ...
        if envs.VLLM_RAY_BUNDLE_INDICES:
            ...
            assert len(bundle_indices) == self.parallel_config.world_size, f'VLLM_RAY_BUNDLE_INDICES must have the same size as the world size, but got bundle_indices={bundle_indices!r} and self.parallel_config.world_size={self.parallel_config.world_size!r}'
            assert len(set(bundle_indices)) == len(bundle_indices), f'VLLM_RAY_BUNDLE_INDICES cannot have duplicate values, but got bundle_indices={bundle_indices!r}'
        else:
            bundle_indices = []
        ...
        for rank, bundle_id in enumerate(bundle_indices):
            ...
            if current_platform.ray_device_key == 'GPU':
                worker = ray.remote(num_cpus=0, num_gpus=num_gpus, scheduling_strategy=scheduling_strategy, **ray_remote_kwargs)(RayWorkerWrapper).remote(vllm_config=self.vllm_config, rpc_rank=rank)
            else:
                worker = ray.remote(num_cpus=0, num_gpus=0, resources={current_platform.ray_device_key: num_gpus}, scheduling_strategy=scheduling_strategy, **ray_remote_kwargs)(RayWorkerWrapper).remote(vllm_config=self.vllm_config, rpc_rank=rank)
        ...
        if n_nodes != n_ips:
            raise RuntimeError(f'Every node should have a unique IP address. Got {n_nodes} nodes with node ids {list(node_workers.keys())} and {n_ips} unique IP addresses {all_ips}. Please check your network configuration. If you set `VLLM_HOST_IP` environment variable, make sure it is unique for each node.')
        ...
        env_vars_to_copy = get_env_vars_to_copy(exclude_vars=self.WORKER_SPECIFIC_ENV_VARS, additional_vars=set(current_platform.additional_env_vars), destination='workers')
    ...
    def _check_ray_cgraph_installation(self):
        ...
        if cupy_spec is None and envs.VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE == 'nccl':
            raise ValueError("cupy is not installed but required since VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE is set to 'nccl'. Run `pip install ray[cgraph]` and check cupy installation.")

    def _compiled_ray_dag(self, enable_asyncio: bool):
        assert self.parallel_config.use_ray
        self._check_ray_cgraph_installation()
        os.environ.setdefault('RAY_CGRAPH_get_timeout', '300')
        logger.info('RAY_CGRAPH_get_timeout is set to %s', os.environ['RAY_CGRAPH_get_timeout'])
        logger.info('VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE = %s', envs.VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE)
        logger.info('VLLM_USE_RAY_COMPILED_DAG_OVERLAP_COMM = %s', envs.VLLM_USE_RAY_COMPILED_DAG_OVERLAP_COMM)
        channel_type = envs.VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE
        if channel_type not in ('auto', 'nccl', 'shm'):
            raise ValueError(f"Invalid value for VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE: {channel_type}. Valid values are: 'auto', 'nccl', or 'shm'.")
        with InputNode() as input_data:
            ...
            for pp_rank, tp_group in enumerate(self.pp_tp_workers):
                ...
                if pp_rank < last_pp_rank and envs.VLLM_USE_RAY_COMPILED_DAG_CHANNEL_TYPE != 'shm':
        ...
        return forward_dag.experimental_compile(enable_asyncio=enable_asyncio, _overlap_gpu_communication=envs.VLLM_USE_RAY_COMPILED_DAG_OVERLAP_COMM)
