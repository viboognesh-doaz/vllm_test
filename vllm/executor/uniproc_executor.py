...
class UniProcExecutor(ExecutorBase):
    ...
    def _init_executor(self) -> None:
        ...
        kwargs = dict(vllm_config=self.vllm_config, local_rank=local_rank, rank=rank, distributed_init_method=distributed_init_method, is_driver_worker=is_driver_worker)
...
class ExecutorWithExternalLauncher(UniProcExecutor):
    ...
    def _init_executor(self) -> None:
        """Initialize the worker and load the model.
        """
        assert self.vllm_config.scheduler_config.delay_factor == 0.0, 'ExecutorWithExternalLauncher needs deterministic execution, so itdoes not support delay_factor in scheduling'
        if envs.VLLM_USE_V1:
            assert not envs.VLLM_ENABLE_V1_MULTIPROCESSING, 'To get deterministic execution in V1, please set VLLM_ENABLE_V1_MULTIPROCESSING=0'
        self.driver_worker = WorkerWrapperBase(vllm_config=self.vllm_config, rpc_rank=0)
