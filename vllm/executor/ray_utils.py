...
try:
    ...
    class RayWorkerWrapper(WorkerWrapperBase):
        ...
        def __init__(self, *args, **kwargs) -> None:
            ...
            self.input_decoder = msgspec.msgpack.Decoder(ExecuteModelRequest, dec_hook=decode_hook)
        ...
        def get_node_and_gpu_ids(self) -> Tuple[str, List[int]]:
            ...
            if not device_key:
                raise RuntimeError('current platform %s does not support ray.', vllm.platforms.current_platform.device_name)
            gpu_ids = ray.get_runtime_context().get_accelerator_ids()[device_key]
            return (node_id, gpu_ids)

        def execute_model_spmd(self, req_or_tuple: Union[bytes, Tuple[bytes, Optional[IntermediateTensors]]]) -> bytes:
        ...
        def execute_model_ray(self, scheduler_output: Union['SchedulerOutput', Tuple['SchedulerOutput', 'IntermediateTensors']]) -> Union['ModelRunnerOutput', Tuple['SchedulerOutput', 'IntermediateTensors']]:
...
def assert_ray_available():
    ...
    if ray is None:
        raise ValueError('Failed to import Ray, please install Ray with `pip install ray`.') from ray_import_err
...
def _wait_until_pg_ready(current_placement_group: 'PlacementGroup'):
    ...
    while time.time() - s < PG_WAIT_TIMEOUT:
        ...
        if len(ready) > 0:
            break
        wait_interval *= 2
        logger.info('Waiting for creating a placement group of specs for %d seconds. specs=%s. Check `ray status` and `ray list nodes` to see if you have enough resources, and make sure the IP addresses used by ray cluster are the same as VLLM_HOST_IP environment variable specified in each node if you are running on a multi-node.', int(time.time() - s), placement_group_specs)
    try:
        ray.get(pg_ready_ref, timeout=0)
    except ray.exceptions.GetTimeoutError:
        raise ValueError(f'Cannot provide a placement group of placement_group_specs={placement_group_specs!r} within {PG_WAIT_TIMEOUT} seconds. See `ray status` and `ray list nodes` to make sure the cluster has enough resources.') from None
...
def _wait_until_pg_removed(current_placement_group: 'PlacementGroup'):
    ...
    while time.time() - s < PG_WAIT_TIMEOUT:
        ...
        if pg is None:
            break
        wait_interval *= 2
        logger.info('Waiting for removing a placement group of specs for %d seconds.', int(time.time() - s))
        time.sleep(wait_interval)

def initialize_ray_cluster(parallel_config: ParallelConfig, ray_address: Optional[str]=None):
    ...
    if current_placement_group:
        ...
        if parallel_config.world_size > device_bundles:
            raise ValueError(f'The number of required {device_str}s exceeds the total number of available {device_str}s in the placement group. Required number of devices: {parallel_config.world_size}. Total number of devices: {device_bundles}.')
    else:
        logger.info('No current placement group found. Creating a new placement group.')
        num_devices_in_cluster = ray.cluster_resources().get(device_str, 0)
        if parallel_config.world_size > num_devices_in_cluster:
            logger.warning('The number of required %ss exceeds the total number of available %ss in the placement group.', device_str, device_str)
        ...
        if current_node_resource.get(device_str, 0) < 1:
            raise ValueError(f'Current node has no {device_str} available. current_node_resource={current_node_resource!r}. vLLM engine cannot start without {device_str}. Make sure you have at least 1 {device_str} available in a node current_node_id={current_node_id!r} current_ip={current_ip!r}.')
