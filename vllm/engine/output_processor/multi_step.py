...
from vllm.sequence import VLLM_INVALID_TOKEN_ID, CompletionSequenceGroupOutput, Sequence, SequenceGroup, SequenceGroupOutput, SequenceOutput, SequenceStatus
...
class MultiStepOutputProcessor(SequenceGroupOutputProcessor):
    ...
    def __init__(self, detokenizer: Detokenizer, scheduler: List[Scheduler], seq_counter: Counter, get_tokenizer_for_seq: Callable[[Sequence], AnyTokenizer], stop_checker: StopChecker):
    ...
    def _log_prompt_logprob_unsupported_warning_once():
        logger.warning('Prompt logprob is not supported by multi step workers. (e.g., speculative decode uses multi step workers).')

    def process_outputs(self, sequence_group: SequenceGroup, outputs: List[SequenceGroupOutput], is_async: bool=False) -> None:
        ...
        seq = seqs[0]
        seq_id = seq.seq_id
        assert all([isinstance(output, CompletionSequenceGroupOutput) for output in outputs])
        compl_outputs = cast(List[CompletionSequenceGroupOutput], outputs)
        assert all([seq_id == output.samples[0].parent_seq_id for output in compl_outputs])
        if is_async:
            self._process_decode_and_stop(seq, sequence_group.sampling_params)
        else:
            samples = [output.samples[0] for output in compl_outputs]
            valid_samples = [sample for sample in samples if sample.output_token != VLLM_INVALID_TOKEN_ID]
            if valid_samples:
                self._process_seq_outputs(seq, valid_samples, sequence_group.sampling_params)

    def _process_decode_and_stop(self, seq: Sequence, sampling_params: SamplingParams) -> None:
        new_char_count = 0
        if sampling_params.detokenize and self.detokenizer:
            new_char_count = self.detokenizer.decode_sequence_inplace(seq, sampling_params)
        self.stop_checker.maybe_stop_sequence(seq, new_char_count=new_char_count, sampling_params=sampling_params)
    ...
    def _process_seq_outputs(self, seq: Sequence, valid_samples: List[SequenceOutput], sampling_params: SamplingParams) -> None:
        ...
        remaining_tokens = sampling_params.max_tokens - (seq.get_output_len() + len(output_token_ids))
        ...
        for output_token_id, output_logprob, output_embed in zip(output_token_ids, output_logprobs, output_embeds):
            seq.append_token_id(token_id=output_token_id, logprobs=output_logprob, token_embed=output_embed)
