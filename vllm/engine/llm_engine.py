...
from vllm.config import DecodingConfig, LoRAConfig, ModelConfig, ObservabilityConfig, ParallelConfig, SchedulerConfig, VllmConfig
...
from vllm.sequence import ExecuteModelRequest, ParallelSampleSequenceGroup, PoolingSequenceGroupOutput, Sequence, SequenceGroup, SequenceGroupBase, SequenceGroupMetadata, SequenceGroupOutput, SequenceStatus
...
class LLMEngine:
    ...
    def __init__(self, vllm_config: VllmConfig, executor_class: Type[ExecutorBase], log_stats: bool, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT, stat_loggers: Optional[Dict[str, StatLoggerBase]]=None, mm_registry: MultiModalRegistry=MULTIMODAL_REGISTRY, use_cached_outputs: bool=False) -> None:
        if envs.VLLM_USE_V1:
            raise ValueError('Using V0 LLMEngine, but envs.VLLM_USE_V1=True. This should not happen. As a workaround, try using LLMEngine.from_vllm_config(...) or explicitly set VLLM_USE_V1=0 or 1 and report this issue on Github.')
        ...
        logger.info('Initializing a V0 LLM engine (v%s) with config: %s, use_cached_outputs=%s, ', VLLM_VERSION, vllm_config, use_cached_outputs)
        ...
        if is_usage_stats_enabled():
            usage_message.report_usage(get_architecture_class_name(self.model_config), usage_context, extra_kvs={'dtype': str(self.model_config.dtype), 'tensor_parallel_size': self.parallel_config.tensor_parallel_size, 'block_size': self.cache_config.block_size, 'gpu_memory_utilization': self.cache_config.gpu_memory_utilization, 'quantization': self.model_config.quantization, 'kv_cache_dtype': str(self.cache_config.cache_dtype), 'enable_lora': bool(self.lora_config), 'enable_prompt_adapter': bool(self.prompt_adapter_config), 'enable_prefix_caching': self.cache_config.enable_prefix_caching, 'enforce_eager': self.model_config.enforce_eager, 'disable_custom_all_reduce': self.parallel_config.disable_custom_all_reduce})
        ...
        if isinstance(self.vllm_config.scheduler_config.scheduler_cls, str):
            Scheduler = resolve_obj_by_qualname(self.vllm_config.scheduler_config.scheduler_cls)
        else:
        ...
        if self.log_stats:
            if stat_loggers is not None:
                ...
                self.stat_loggers = {'logging': LoggingStatLogger(local_interval=_LOCAL_LOGGING_INTERVAL_SEC, vllm_config=vllm_config), 'prometheus': PrometheusStatLogger(local_interval=_LOCAL_LOGGING_INTERVAL_SEC, labels=dict(model_name=self.model_config.served_model_name), vllm_config=vllm_config)}
    ...
    def _initialize_kv_caches(self) -> None:
        ...
        if self.cache_config.num_gpu_blocks_override is not None:
            ...
            logger.info('Overriding num_gpu_blocks=%d with num_gpu_blocks_override=%d', num_gpu_blocks, num_gpu_blocks_override)
    ...
    def _get_executor_cls(cls, engine_config: VllmConfig) -> Type[ExecutorBase]:
        ...
        if isinstance(distributed_executor_backend, type):
            ...
            assert not envs.VLLM_USE_RAY_SPMD_WORKER, 'multiprocessing distributed executor backend does not support VLLM_USE_RAY_SPMD_WORKER=1'
    ...
    @classmethod
    def from_vllm_config(cls, vllm_config: VllmConfig, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT, stat_loggers: Optional[Dict[str, StatLoggerBase]]=None, disable_log_stats: bool=False) -> 'LLMEngine':
        return cls(vllm_config=vllm_config, executor_class=cls._get_executor_cls(vllm_config), log_stats=not disable_log_stats, usage_context=usage_context, stat_loggers=stat_loggers)

    @classmethod
    def from_engine_args(cls, engine_args: EngineArgs, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT, stat_loggers: Optional[Dict[str, StatLoggerBase]]=None) -> 'LLMEngine':
        """Creates an LLM engine from the engine arguments."""
        vllm_config = engine_args.create_engine_config(usage_context)
        engine_cls = cls
        if envs.VLLM_USE_V1:
            engine_cls = V1LLMEngine
        return engine_cls.from_vllm_config(vllm_config=vllm_config, usage_context=usage_context, stat_loggers=stat_loggers, disable_log_stats=engine_args.disable_log_stats)
    ...
    def _add_processed_request(self, request_id: str, processed_inputs: ProcessorInputs, params: Union[SamplingParams, PoolingParams], arrival_time: float, lora_request: Optional[LoRARequest], prompt_adapter_request: Optional[PromptAdapterRequest], trace_headers: Optional[Mapping[str, str]]=None, priority: int=0) -> Optional[SequenceGroup]:
        ...
        seq = Sequence(seq_id, decoder_inputs, block_size, eos_token_id, lora_request, prompt_adapter_request)
        encoder_seq = None if encoder_inputs is None else Sequence(seq_id, encoder_inputs, block_size, eos_token_id, lora_request, prompt_adapter_request)
        if isinstance(params, SamplingParams):
            seq_group = self._create_sequence_group_with_sampling(request_id, seq, params, arrival_time=arrival_time, lora_request=lora_request, trace_headers=trace_headers, prompt_adapter_request=prompt_adapter_request, encoder_seq=encoder_seq, priority=priority)
        elif isinstance(params, PoolingParams):
            seq_group = self._create_sequence_group_with_pooling(request_id, seq, params, arrival_time=arrival_time, lora_request=lora_request, prompt_adapter_request=prompt_adapter_request, encoder_seq=encoder_seq, priority=priority)
        else:
    ...
    def add_request(self, request_id: str, prompt: PromptType, params: Union[SamplingParams, PoolingParams], arrival_time: Optional[float]=None, lora_request: Optional[LoRARequest]=None, tokenization_kwargs: Optional[dict[str, Any]]=None, trace_headers: Optional[Mapping[str, str]]=None, prompt_adapter_request: Optional[PromptAdapterRequest]=None, priority: int=0) -> None:
        ...
        if isinstance(params, SamplingParams) and (params.guided_decoding or params.logits_processors) and (self.scheduler_config.num_scheduler_steps > 1):
            raise ValueError('Guided decoding and logits processors are not supported in multi-step decoding')
        ...
        processed_inputs = self.input_preprocessor.preprocess(prompt, tokenization_kwargs=tokenization_kwargs, lora_request=lora_request, prompt_adapter_request=prompt_adapter_request)
        self._add_processed_request(request_id=request_id, processed_inputs=processed_inputs, params=params, arrival_time=arrival_time, lora_request=lora_request, prompt_adapter_request=prompt_adapter_request, trace_headers=trace_headers, priority=priority)
    ...
    def _create_sequence_group_with_sampling(self, request_id: str, seq: Sequence, sampling_params: SamplingParams, arrival_time: float, lora_request: Optional[LoRARequest], trace_headers: Optional[Mapping[str, str]]=None, prompt_adapter_request: Optional[PromptAdapterRequest]=None, encoder_seq: Optional[Sequence]=None, priority: int=0) -> SequenceGroup:
        ...
        if self.vllm_config.speculative_config is not None:
            draft_size = self.vllm_config.speculative_config.num_speculative_tokens + 1
        seq_group = SequenceGroup(request_id=request_id, seqs=[seq], arrival_time=arrival_time, sampling_params=sampling_params, lora_request=lora_request, trace_headers=trace_headers, prompt_adapter_request=prompt_adapter_request, encoder_seq=encoder_seq, priority=priority, draft_size=draft_size)
        return seq_group

    def _create_sequence_group_with_pooling(self, request_id: str, seq: Sequence, pooling_params: PoolingParams, arrival_time: float, lora_request: Optional[LoRARequest], prompt_adapter_request: Optional[PromptAdapterRequest], encoder_seq: Optional[Sequence]=None, priority: int=0) -> SequenceGroup:
        """Creates a SequenceGroup with PoolingParams."""
        pooling_params = pooling_params.clone()
        seq_group = SequenceGroup(request_id=request_id, seqs=[seq], arrival_time=arrival_time, lora_request=lora_request, pooling_params=pooling_params, prompt_adapter_request=prompt_adapter_request, encoder_seq=encoder_seq, priority=priority)
        return seq_group
    ...
    def step(self) -> List[Union[RequestOutput, PoolingRequestOutput]]:
        ...
        if self.parallel_config.pipeline_parallel_size > 1:
            raise NotImplementedError('Pipeline parallelism is only supported through AsyncLLMEngine as performance will be severely degraded otherwise.')
        virtual_engine = 0
    ...
    def sleep(self, level: int=1) -> None:
        assert self.vllm_config.model_config.enable_sleep_mode, 'Sleep mode is not enabled in the model config'
    ...
    def _build_logits_processors(self, sampling_params: SamplingParams, lora_request: Optional[LoRARequest]) -> SamplingParams:
        ...
        if sampling_params.guided_decoding is not None:
            ...
            logger.debug('Building guided decoding logits processor in LLMEngine. Params: %s', guided_decoding)
            tokenizer = self.get_tokenizer(lora_request=lora_request)
            guided_decoding.backend = guided_decoding.backend or self.decoding_config.backend
            if self.decoding_config.reasoning_backend:
                logger.debug('Building with reasoning backend %s', self.decoding_config.reasoning_backend)
            processor = get_local_guided_decoding_logits_processor(guided_params=guided_decoding, tokenizer=tokenizer, model_config=self.model_config, reasoning_backend=self.decoding_config.reasoning_backend)
            if processor:
