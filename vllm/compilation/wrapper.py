    ...
    def __init__(self, compiled_callable: Optional[Callable]=None, compilation_level: int=0):
        ...
        if compiled_callable is None:
            ...
            if isinstance(backend, str) and backend == 'inductor':
                options = get_current_vllm_config().compilation_config.inductor_compile_config
            compiled_callable = torch.compile(self.forward, fullgraph=envs.VLLM_TEST_DYNAMO_FULLGRAPH_CAPTURE, backend=backend, options=options)
        ...
        if isinstance(debug_dump_dir, str) and debug_dump_dir != '':
            ...
            if not os.path.exists(decompiled_file):
                try:
                    ...
                    pass
        if self.vllm_config.compilation_config.use_cudagraph and 'update' in new_code.co_names:
            src = depyf.decompile(new_code)
