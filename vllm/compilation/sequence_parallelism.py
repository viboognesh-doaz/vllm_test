...
class _RMSNormAndQuantOpHelper:
    ...
    def __init__(self, epsilon: float, dtype: torch.dtype, device: str, quant_op: Optional[torch._ops.OpOverload]=None, **kwargs):
    ...
    def _functional_rmsnorm(self, result_buffer, input_tensor, weight_tensor):
        return torch.ops.higher_order.auto_functionalized(torch.ops._C.rms_norm.default, result=result_buffer, input=input_tensor, weight=weight_tensor, epsilon=self.epsilon)

    def _functional_fused_add_rmsnorm(self, input_tensor, residual_tensor, weight_tensor):
        return torch.ops.higher_order.auto_functionalized(torch.ops._C.fused_add_rms_norm.default, input=input_tensor, residual=residual_tensor, weight=weight_tensor, epsilon=self.epsilon)

    def _functional_rmsnorm_then_quant(self, rmsnorm_result_buffer, quant_result_buffer, input_tensor, weight_tensor, scale_tensor):
        ...
        quant_out_tuple = torch.ops.higher_order.auto_functionalized(self.quant_op, result=quant_result_buffer, input=rmsnorm_out_tuple[1], scale=scale_tensor)
        return quant_out_tuple

    def _functional_fused_add_rmsnorm_then_quant(self, quant_result_buffer, input_tensor, residual_tensor, weight_tensor, scale_tensor):
        if self.quant_op is None:
            raise RuntimeError('_RMSNormAndQuantOpHelper was not initialized with a quant_op.')
        fused_add_rmsnorm_out_tuple = self._functional_fused_add_rmsnorm(input_tensor, residual_tensor, weight_tensor)
        quant_out_tuple = torch.ops.higher_order.auto_functionalized(self.quant_op, result=quant_result_buffer, input=fused_add_rmsnorm_out_tuple[1], scale=scale_tensor)
...
class _SequenceParallelPatternHelper(_RMSNormAndQuantOpHelper):
    ...
    def _reduce_scatter(self, x: torch.Tensor) -> torch.Tensor:
        return torch.ops.vllm.reduce_scatter.default(x, dim=0, world_size=self.tp_size, group_name=self.tp_group.unique_name)

    def _all_gather(self, x: torch.Tensor) -> torch.Tensor:
        return torch.ops.vllm.all_gather.default(x, dim=0, world_size=self.tp_size, group_name=self.tp_group.unique_name)
...
class FirstAllReduceRMSNormPattern(_SequenceParallelPatternHelper):
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def pattern(input: torch.Tensor, permute: torch.Tensor, arg3_1: torch.Tensor):
            all_reduce = self._all_reduce(input)
            rmsnorm = self._functional_rmsnorm(permute, all_reduce, arg3_1)
            return (rmsnorm[1], all_reduce)

        def replacement(input: torch.Tensor, permute: torch.Tensor, arg3_1: torch.Tensor):
...
class MiddleAllReduceRMSNormPattern(_SequenceParallelPatternHelper):
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def pattern(residual: torch.Tensor, mm_1: torch.Tensor, rms_norm_weights: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            all_reduce = self._all_reduce(mm_1)
            rmsnorm = self._functional_fused_add_rmsnorm(all_reduce, residual, rms_norm_weights)
            return (rmsnorm[1], rmsnorm[2])

        def replacement(residual: torch.Tensor, mm_1: torch.Tensor, rms_norm_weights: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
class FirstAllReduceRMSNormStaticFP8Pattern(_SequenceParallelPatternHelper):
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def pattern(input: torch.Tensor, rmsnorm_result: torch.Tensor, quant_result: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor):
            all_reduce = self._all_reduce(input)
        ...
        def replacement(input: torch.Tensor, rmsnorm_result: torch.Tensor, quant_result: torch.Tensor, weight: torch.Tensor, scale: torch.Tensor):
            reduce_scatter = self._reduce_scatter(input)
            rmsnorm_result = torch.empty_like(reduce_scatter, dtype=rmsnorm_result.dtype)
            quant_result = torch.empty_like(rmsnorm_result, dtype=quant_result.dtype)
            static_fp8 = self._functional_rmsnorm_then_quant(rmsnorm_result, quant_result, reduce_scatter, weight, scale)
...
class MiddleAllReduceRMSNormStaticFP8Pattern(_SequenceParallelPatternHelper):
    ...
    def get_inputs(self):
        ...
        return [result, residual, mm_1, rms_norm_weights, scale]

    def register(self, pm_pass: PatternMatcherPass):

        def pattern(result: torch.Tensor, residual: torch.Tensor, mm_1: torch.Tensor, rms_norm_weights: torch.Tensor, scale: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            all_reduce = self._all_reduce(mm_1)
            static_fp8, rmsnorm_residual_out = self._functional_fused_add_rmsnorm_then_quant(result, all_reduce, residual, rms_norm_weights, scale)
            return (static_fp8[1], rmsnorm_residual_out)

        def replacement(result: torch.Tensor, residual: torch.Tensor, mm_1: torch.Tensor, rms_norm_weights: torch.Tensor, scale: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            reduce_scatter = self._reduce_scatter(mm_1)
            quant_result_buf = torch.empty_like(reduce_scatter, dtype=result.dtype)
            static_fp8, rmsnorm_residual_out = self._functional_fused_add_rmsnorm_then_quant(quant_result_buf, reduce_scatter, residual, rms_norm_weights, scale)
            all_gather = self._all_gather(static_fp8[1])
...
class LastAllReduceRMSNormStaticFP8Pattern(_SequenceParallelPatternHelper):
    ...
    def register(self, pm_pass: PatternMatcherPass):
        ...
        def pattern(result: torch.Tensor, residual: torch.Tensor, mm_1: torch.Tensor, rms_norm_weights: torch.Tensor, scale: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            all_reduce = self._all_reduce(mm_1)
            static_fp8, _ = self._functional_fused_add_rmsnorm_then_quant(result, all_reduce, residual, rms_norm_weights, scale)
            return static_fp8[1]

        def replacement(result: torch.Tensor, residual: torch.Tensor, mm_1: torch.Tensor, rms_norm_weights: torch.Tensor, scale: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            reduce_scatter = self._reduce_scatter(mm_1)
            quant_result_buf = torch.empty_like(reduce_scatter, dtype=result.dtype)
            static_fp8, _ = self._functional_fused_add_rmsnorm_then_quant(quant_result_buf, reduce_scatter, residual, rms_norm_weights, scale)
...
class SequenceParallelismPass(VllmInductorPass):
    ...
    def __init__(self, config: VllmConfig):
        ...
        for epsilon in [1e-05, 1e-06]:
            ...
            FirstAllReduceRMSNormStaticFP8Pattern(epsilon, self.model_dtype, self.device, fp8_quant_op).register(self.patterns)
            MiddleAllReduceRMSNormStaticFP8Pattern(epsilon, self.model_dtype, self.device, fp8_quant_op).register(self.patterns)
            LastAllReduceRMSNormStaticFP8Pattern(epsilon, self.model_dtype, self.device, fp8_quant_op).register(self.patterns)
