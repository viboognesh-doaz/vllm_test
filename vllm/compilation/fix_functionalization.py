...
class FixFunctionalizationPass(VllmInductorPass):
    ...
    def __call__(self, graph: torch.fx.Graph):
        ...
        for node in graph.nodes:
            ...
            elif at_target == torch.ops._C.fused_add_rms_norm_static_fp8_quant.default:
                mutated_args = {1: 'result', 2: 'residual'}
                self.defunctionalize(graph, node, mutated_args)
            elif at_target == torch.ops._C.rms_norm_dynamic_per_token_quant.default:
    ...
    def defunctionalize(self, graph: torch.fx.Graph, node: torch.fx.Node, mutated_args: dict[int, Union[torch.fx.Node, str]], args: Optional[tuple[Union[torch.fx.Node, str], ...]]=None):
        """
    ...
    def replace_users_with_mutated_args(self, node: torch.fx.Node, mutated_args: dict[int, Union[torch.fx.Node, str]]):
        """
    ...
    def insert_defunctionalized(self, graph: torch.fx.Graph, node: torch.fx.Node, args: Optional[tuple[Union[torch.fx.Node, str], ...]]=None):
        """
