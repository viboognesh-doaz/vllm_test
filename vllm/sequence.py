...
class SequenceData(msgspec.Struct, omit_defaults=True):
    ...
    @staticmethod
    def from_seqs(prompt_token_ids: GenericSequence[int], output_token_ids: Optional[GenericSequence[int]]=None, *, prompt_embeds: Optional[torch.Tensor]=None) -> 'SequenceData':
        """
        Construct a [`SequenceData`][vllm.sequence.SequenceData] instance
    ...
    def _update_cached_all_token_embeds(self):
        ...
        if self._output_embeds is not None:
            self._cached_all_token_embeds = torch.cat((self._cached_all_token_embeds, self._output_embeds), dim=0)
    ...
    def append_token_id(self, token_id: int, logprob: float, token_embed: Optional[torch.Tensor]=None) -> None:
        ...
        if token_embed is not None:
            ...
            if self._output_embeds is None:
                ...
                self._output_embeds = torch.cat((self._output_embeds, token_embed), dim=0)
            assert self._cached_all_token_embeds is not None
            self._cached_all_token_embeds = torch.cat((self._cached_all_token_embeds, token_embed.to(device=self._cached_all_token_embeds.device)), dim=0)
    ...
    def get_delta_and_reset(self) -> SequenceDataDelta:
        delta = SequenceDataDelta(self._new_appended_tokens, self._cumulative_logprob, self.get_num_computed_tokens(), self.stage)
        self._new_appended_tokens = []
        return delta
    ...
    def __repr__(self) -> str:
        return f"SequenceData(prompt_token_ids={self._prompt_token_ids}, prompt_embeds.shape={getattr(self._prompt_embeds, 'shape', None)}, output_token_ids={self.output_token_ids}, cumulative_logprob={self.cumulative_logprob}, get_num_computed_tokens={self.get_num_computed_tokens()})"
...
class Sequence:
    ...
    def __init__(self, seq_id: int, inputs: SingletonInputs, block_size: int, eos_token_id: Optional[int]=None, lora_request: Optional[LoRARequest]=None, prompt_adapter_request: Optional[PromptAdapterRequest]=None) -> None:
        self.seq_id = seq_id
        self.inputs = inputs
        ...
        self.data = SequenceData.from_seqs(self.prompt_token_ids, prompt_embeds=self.inputs['prompt_embeds'] if self.inputs['type'] == 'embeds' else None)
    ...
    def append_token_id(self, token_id: int, logprobs: dict[int, Logprob], token_embed: Optional[torch.Tensor]=None) -> None:
...
class SequenceGroup:
    ...
    def init_multi_step_from_lookahead_slots(self, num_lookahead_slots: int, num_scheduler_steps: int, is_multi_step: bool, enable_chunking: bool) -> None:
    ...
    def set_last_token_time(self, now: float) -> None:
        ...
        assert not self.is_prefill(), 'seq_group.set_last_token_time() should not be called if the seq_group is in prefill phase.'
    ...
    def get_last_token_latency(self) -> float:
        ...
        assert not self.is_prefill(), 'seq_group.get_last_token_latency() should not be called if the seq_group is in prefill phase.'
    ...
    def __repr__(self) -> str:
        return f'SequenceGroup(request_id={self.request_id}, sampling_params={self.sampling_params}, num_seqs={len(self.seqs)})'
...
class SequenceGroupMetadata(msgspec.Struct, tag=True, array_like=True, omit_defaults=True):
    ...
    def finish_step(self) -> None:
        ...
        assert self.state.current_step < self.state.num_steps, f'current step {self.state.current_step}, num_steps {self.state.num_steps}'
...
class SequenceOutput(msgspec.Struct, omit_defaults=True, array_like=True):
    ...
    def __repr__(self) -> str:
        ...
        return f'SequenceOutput(parent_seq_id={self.parent_seq_id}, output_token={self.output_token}, output_embed.shape={output_embed_shape}, logprobs={self.logprobs})'
...
class HiddenStates(msgspec.Struct, array_like=True, omit_defaults=True):
    ...
    def update(self, hidden_states: torch.Tensor, seq_group_metadata_list: list[SequenceGroupMetadata], second_last_token_hidden_states: Optional[torch.Tensor]=None):
        ...
        if self.second_last_token_hidden_states is not None:
            self.second_last_token_hidden_states = torch.cat([self.second_last_token_hidden_states, torch.zeros_like(hidden_states) if second_last_token_hidden_states is None else second_last_token_hidden_states])
...
class ParallelSampleSequenceGroup(SequenceGroupBase):
    ...
    def add_request(request_id: str, engine, params, **kwargs):
        ...
        group.assembled_seq_group = SequenceGroup(request_id=request_id, seqs=seqs, arrival_time=seq_group.arrival_time, sampling_params=original_params, lora_request=seq_group.lora_request, pooling_params=seq_group.pooling_params, pooled_data=seq_group.pooled_data, encoder_seq=seq_group.encoder_seq, trace_headers=seq_group.trace_headers, prompt_adapter_request=seq_group.prompt_adapter_request, priority=seq_group.priority)
