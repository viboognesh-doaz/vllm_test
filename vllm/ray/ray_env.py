...
try:
    ...
    logger.warning('Failed to parse %s. Using an empty set for non-carry-over env vars.', RAY_NON_CARRY_OVER_ENV_VARS_FILE)
    RAY_NON_CARRY_OVER_ENV_VARS = set()
...
def get_env_vars_to_copy(exclude_vars: Optional[set[str]]=None, additional_vars: Optional[set[str]]=None, destination: Optional[str]=None) -> set[str]:
    ...
    env_vars_to_copy = {v for v in envs.environment_variables if v not in exclude_vars and v not in RAY_NON_CARRY_OVER_ENV_VARS}
    ...
    logger.info('Copying the following environment variables%s: %s', to_destination, [v for v in env_vars_to_copy if v in os.environ])
    logger.info('If certain env vars should NOT be copied, add them to %s file', RAY_NON_CARRY_OVER_ENV_VARS_FILE)
