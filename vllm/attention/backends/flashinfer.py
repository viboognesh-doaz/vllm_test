...
try:
    ...
    from flashinfer.decode import CUDAGraphBatchDecodeWithPagedKVCacheWrapper, trtllm_batch_decode_with_kv_cache
    ...
    raise ImportError('FlashInfer is not installed. Please install it from https://github.com/flashinfer-ai/flashinfer') from None
import torch
import vllm.envs as envs
from vllm import _custom_ops as ops
from vllm.attention.backends.abstract import AttentionBackend, AttentionImpl, AttentionLayer, AttentionMetadata, AttentionMetadataBuilder, AttentionState, AttentionType
...
class FlashInferBackend(AttentionBackend):
    ...
    @staticmethod
    def use_trtllm_decode_attention(batch_size: int, max_seq_len: int, kv_cache_dtype: str, num_qo_heads: Optional[int], num_kv_heads: Optional[int], attn_head_size: Optional[int]) -> bool:
        if FlashInferBackend.cached_sm100a_supported is None:
            FlashInferBackend.cached_sm100a_supported = current_platform.has_device_capability(100)
        if not FlashInferBackend.cached_sm100a_supported:
            return False
        if attn_head_size is None or num_qo_heads is None or num_kv_heads is None or (num_qo_heads // num_kv_heads > 8) or (num_qo_heads % num_kv_heads != 0) or (attn_head_size != 128):
            return False
        ...
        if env_value is not None:
            ...
            use_trtllm = FlashInferBackend.cached_sm100a_supported and batch_size <= 256 and (max_seq_len < 131072) and (kv_cache_dtype == 'auto')
            if use_trtllm:
...
class FlashInferState(AttentionState):
    ...
    def _get_workspace_buffer(self):
        if self._workspace_buffer is None:
            self._workspace_buffer = torch.empty(FLASHINFER_WORKSPACE_BUFFER_SIZE, dtype=torch.uint8, device=self.runner.device)
        return self._workspace_buffer

    @staticmethod
    def get_kv_cache_layout():
        if _KV_CACHE_LAYOUT_OVERRIDE is not None:
            logger.info_once('Using KV cache layout %s', _KV_CACHE_LAYOUT_OVERRIDE)
    ...
    def _get_prefill_wrapper(self):
        if self._prefill_wrapper is None:
            self._prefill_wrapper = BatchPrefillWithPagedKVCacheWrapper(self._get_workspace_buffer(), self.get_kv_cache_layout())
        return self._prefill_wrapper

    def _get_decode_wrapper(self):
        if self._decode_wrapper is None:
            num_qo_heads = self.runner.model_config.get_num_attention_heads(self.runner.parallel_config)
            num_kv_heads = self.runner.model_config.get_num_kv_heads(self.runner.parallel_config)
            use_tensor_cores = envs.VLLM_FLASHINFER_FORCE_TENSOR_CORES or num_qo_heads // num_kv_heads > 4
            self._decode_wrapper = BatchDecodeWithPagedKVCacheWrapper(self._get_workspace_buffer(), self.get_kv_cache_layout(), use_tensor_cores=use_tensor_cores)
    ...
    def graph_capture(self, max_batch_size: int):
        ...
        self._graph_indices_buffer = torch.empty(max_batch_size * self.runner.cache_config.num_gpu_blocks, dtype=torch.int32, device=self.runner.device)
        self._graph_indptr_buffer = torch.empty(max_batch_size + 1, dtype=torch.int32, device=self.runner.device)
        self._graph_last_page_len_buffer = torch.empty(max_batch_size, dtype=torch.int32, device=self.runner.device)
        yield
        self._is_graph_capturing = False
    ...
    def graph_capture_get_metadata_for_batch(self, batch_size: int, is_encoder_decoder_model: bool=False):
        assert self._is_graph_capturing
        ...
        num_kv_heads = self.runner.model_config.get_num_kv_heads(self.runner.parallel_config)
        use_tensor_cores = envs.VLLM_FLASHINFER_FORCE_TENSOR_CORES or num_qo_heads // num_kv_heads > 4
        self._graph_decode_wrapper = CUDAGraphBatchDecodeWithPagedKVCacheWrapper(self._graph_decode_workspace_buffer, _indptr_buffer, self._graph_indices_buffer, _last_page_len_buffer, self.get_kv_cache_layout(), use_tensor_cores)
        if self.runner.kv_cache_dtype.startswith('fp8'):
            kv_cache_dtype = FlashInferBackend.get_fp8_dtype_for_flashinfer(self.runner.kv_cache_dtype)
        else:
            kv_cache_dtype = get_kv_cache_torch_dtype(self.runner.kv_cache_dtype, self.runner.model_config.dtype)
        ...
        paged_kv_last_page_len_tensor_host = torch.full((batch_size,), self.runner.block_size, dtype=torch.int32)
        query_start_loc_host = torch.arange(0, batch_size + 1, dtype=torch.int32)
        global_params = infer_global_hyperparameters(get_per_layer_parameters(self.vllm_config))
        attn_metadata = self.runner.attn_backend.make_metadata(num_prefills=0, slot_mapping=self._graph_slot_mapping[:batch_size], multi_modal_placeholder_index_maps=None, enable_kv_scales_calculation=False, num_prefill_tokens=0, num_decode_tokens=batch_size, max_prefill_seq_len=0, max_decode_seq_len=0, seq_lens_tensor=self._graph_seq_lens, block_tables=self._graph_block_tables, paged_kv_indptr=paged_kv_indptr_tensor_host, paged_kv_indices=paged_kv_indices_tensor_host, paged_kv_last_page_len=paged_kv_last_page_len_tensor_host, num_qo_heads=num_qo_heads, num_kv_heads=num_kv_heads, head_dim=self.runner.model_config.get_head_size(), page_size=self.runner.block_size, seq_start_loc=None, query_start_loc=query_start_loc_host, device=self.runner.device, data_type=kv_cache_dtype, q_data_type=self.runner.model_config.dtype, use_cuda_graph=True, decode_wrapper=self._graph_decode_wrapper, prefill_wrapper=None, **dataclasses.asdict(global_params))
        attn_metadata.begin_forward()
        return attn_metadata

    def get_graph_input_buffers(self, attn_metadata, is_encoder_decoder_model: bool=False):
        return {'block_tables': attn_metadata.block_tables, 'seq_lens_tensor': attn_metadata.seq_lens_tensor, 'slot_mapping': attn_metadata.slot_mapping}

    def prepare_graph_input_buffers(self, input_buffers, attn_metadata, is_encoder_decoder_model: bool=False):
        num_total_blocks = attn_metadata.decode_metadata.seq_lens_tensor.shape[0]
        input_buffers['seq_lens_tensor'][:num_total_blocks].copy_(attn_metadata.seq_lens_tensor, non_blocking=True)
        input_buffers['block_tables'][:num_total_blocks].copy_(attn_metadata.block_tables, non_blocking=True)
    ...
    def begin_forward(self, model_input):
        ...
        if use_cuda_graph and is_decode:
            if model_input.inputs_embeds is None:
                ...
                state = self.runner.graph_runners[model_input.virtual_engine][batch_size, False].attn_state
            else:
                batch_size = model_input.inputs_embeds.shape[0]
                state = self.runner.graph_runners[model_input.virtual_engine][batch_size, True].attn_state
    ...
    def begin_forward(self):
        if self.num_prefill_tokens > 0:
            ...
            if not self.is_profile_run:
                ...
                self.prefill_wrapper.plan(self.query_start_loc, self.paged_kv_indptr[:self.num_prefills + 1], self.paged_kv_indices, self.paged_kv_last_page_len[:self.num_prefills], self.num_qo_heads, self.num_kv_heads, self.head_dim, self.page_size, causal=True, sm_scale=self.sm_scale, window_left=self.window_left, logits_soft_cap=self.logits_soft_cap, q_data_type=self.q_data_type, kv_data_type=self.data_type)
        if self.num_decode_tokens > 0:
            ...
            self.decode_wrapper.plan(self.paged_kv_indptr[self.num_prefills:], self.paged_kv_indices, self.paged_kv_last_page_len[self.num_prefills:], self.num_qo_heads, self.num_kv_heads, self.head_dim, self.page_size, pos_encoding_mode='NONE', window_left=self.window_left, logits_soft_cap=self.logits_soft_cap, sm_scale=self.sm_scale, kv_data_type=self.data_type, q_data_type=self.q_data_type)
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def prepare(self):
        ...
        if self.global_hyperparameters is None:
            inferred_params = infer_global_hyperparameters(get_per_layer_parameters(self.vllm_config))
    ...
    def _add_seq_group(self, inter_data: 'ModelInputForGPUBuilder.InterDataForSeqGroup', chunked_prefill_enabled: bool):
        ...
        for seq_id, token_len, seq_len, curr_seq_len, query_len, context_len, curr_sliding_window_block in zip(inter_data.seq_ids, [len(t) for t in inter_data.input_tokens], inter_data.orig_seq_lens, inter_data.seq_lens, inter_data.query_lens, inter_data.context_lens, inter_data.curr_sliding_window_blocks):
            ...
            if is_prompt:
                ...
                assert query_len == 1, 'seq_len: {}, context_len: {}, query_len: {}'.format(seq_len, context_len, query_len)
    ...
    def build(self, seq_lens: List[int], query_lens: List[int], cuda_graph_pad_size: int, batch_size: int):
        ...
        if use_captured_graph:
            ...
            block_tables = torch.from_numpy(input_block_tables).to(device, non_blocking=True)
        ...
        placeholder_index_maps = {modality: placeholder_map.index_map() for modality, placeholder_map in self.multimodal_placeholder_maps.items()}
        ...
        if len(self.paged_kv_indptr) > 0:
            ...
            paged_kv_indices_tensor = torch.tensor(self.paged_kv_indices, device='cpu', dtype=torch.int)
            paged_kv_indptr_tensor = torch.tensor(self.paged_kv_indptr, device='cpu', dtype=torch.int)
            paged_kv_last_page_len_tensor = torch.tensor(self.paged_kv_last_page_len, device='cpu', dtype=torch.int)
            block_table_bound_tensor = torch.zeros(len(self.paged_kv_indptr) - 1, device='cpu', dtype=torch.int)
        else:
...
class FlashInferImpl(AttentionImpl):
    ...
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[List[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: str=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[str]=None, use_irope: bool=False) -> None:
        if kv_sharing_target_layer_name is not None:
            raise NotImplementedError('KV sharing is not supported in V0 FLASHINFER backend.')
        if use_irope:
            logger.warning_once('Using irope in FlashInfer is not supported yet, it will fall back to global attention for long context.')
        ...
        if attn_type != AttentionType.DECODER:
            raise NotImplementedError('Encoder self-attention and encoder/decoder cross-attention are not implemented for FlashInferImpl')

    def forward(self, layer: AttentionLayer, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: FlashInferMetadata, output: Optional[torch.Tensor]=None, output_scale: Optional[torch.Tensor]=None) -> torch.Tensor:
        if output_scale is not None:
            raise NotImplementedError('fused output quantization is not yet supported for FlashInferImpl')
        ...
        if kv_cache.numel() > 0:
            ops.reshape_and_cache_flash(key, value, kv_cache[:, 0], kv_cache[:, 1], attn_metadata.slot_mapping.flatten(), kv_cache_dtype, layer._k_scale, layer._v_scale)
            if kv_cache_dtype.startswith('fp8'):
                torch_dtype = FlashInferBackend.get_fp8_dtype_for_flashinfer(kv_cache_dtype)
                kv_cache = kv_cache.view(torch_dtype)
        num_prefill_tokens = attn_metadata.num_prefill_tokens
        num_decode_tokens = attn_metadata.num_decode_tokens
        assert key.shape[0] == num_prefill_tokens + num_decode_tokens, f'key : {key.shape} : #prefill tokens {num_prefill_tokens} : #decode tokens {num_decode_tokens}'
        assert value.shape[0] == num_prefill_tokens + num_decode_tokens, f'value : {value.shape} : #prefill toks {num_prefill_tokens} : #decode toks {num_decode_tokens}'
        ...
        if (prefill_meta := attn_metadata.prefill_metadata):
            if kv_cache.numel() == 0:
                prefill_output = flash_attn_varlen_func(q=query, k=key, v=value, cu_seqlens_q=prefill_meta.seq_start_loc, cu_seqlens_k=prefill_meta.seq_start_loc, max_seqlen_q=prefill_meta.max_prefill_seq_len, max_seqlen_k=prefill_meta.max_prefill_seq_len, softmax_scale=softmax_scale, causal=True, window_size=window_size, alibi_slopes=alibi_slopes)
            else:
                ...
                prefill_output = prefill_meta.prefill_wrapper.run(query, kv_cache.permute(*stride_order), k_scale=layer._k_scale_float, v_scale=layer._v_scale_float)
        if (decode_meta := attn_metadata.decode_metadata):
            ...
            if not FlashInferBackend.use_trtllm_decode_attention(num_decode_tokens, attn_metadata.max_decode_seq_len, kv_cache_dtype, attn_metadata.num_qo_heads, attn_metadata.num_kv_heads, attn_metadata.head_dim):
                decode_output = decode_meta.decode_wrapper.run(decode_query, kv_cache.permute(*stride_order), k_scale=layer._k_scale_float, v_scale=layer._v_scale_float)
            else:
                workspace_buffer = decode_meta.decode_wrapper._int_workspace_buffer
                assert FlashInferState.get_kv_cache_layout() == 'HND'
                decode_output = trtllm_batch_decode_with_kv_cache(query=decode_query, kv_cache=kv_cache.permute(*stride_order), workspace_buffer=workspace_buffer, num_heads=num_heads, num_kv_heads=num_kv_heads, scale=softmax_scale, block_tables=attn_metadata.block_tables, seq_lens=decode_meta.seq_lens_tensor, block_size=attn_metadata.page_size, max_seq_len=attn_metadata.max_decode_seq_len, kv_cache_dtype=kv_cache_dtype, k_scale=layer._k_scale_float, v_scale=layer._v_scale_float)
