...
if TYPE_CHECKING:
STR_NOT_IMPL_ENC_DEC_ROCM_HIP = 'ROCm/HIP is not currently supported with encoder/decoder models.'
PAD_SLOT_ID = -1
...
def is_block_tables_empty(block_tables: Union[None, Dict]):
    ...
    if block_tables is None:
        return True
    return isinstance(block_tables, dict) and all((value is None for value in block_tables.values()))

def compute_slot_mapping_start_idx(is_prompt: bool, query_len: int, context_len: int, sliding_window: int):
    """
...
def _compute_slot_mapping_python(slot_mapping: List[int], block_table: List[int], range_start: int, range_end: int, block_size: int):
...
def _compute_slot_mapping_numpy(slot_mapping: List[int], block_table: List[int], range_start: int, range_end: int, block_size: int):
...
def compute_slot_mapping(is_profile_run: bool, slot_mapping: List[int], seq_id: int, seq_len: int, context_len: int, start_idx: int, block_size: int, block_tables: Dict[int, List[int]]):
    """
    ...
    if numel < _COMPUTE_SLOT_MAPPING_NUMPY_NUMEL:
        _compute_slot_mapping_python(slot_mapping, block_table, range_start, range_end, block_size)
    else:
        _compute_slot_mapping_numpy(slot_mapping, block_table, range_start, range_end, block_size)
...
class CommonAttentionState(AttentionState):
    ...
    def graph_capture_get_metadata_for_batch(self, batch_size: int, is_encoder_decoder_model: bool=False):
        assert self._is_graph_capturing
        attn_metadata = self.runner.attn_backend.make_metadata(num_prefills=0, num_prefill_tokens=0, num_decode_tokens=batch_size, slot_mapping=self._graph_slot_mapping[:batch_size], multi_modal_placeholder_index_maps=None, enable_kv_scales_calculation=True, seq_lens=None, seq_lens_tensor=self._graph_seq_lens[:batch_size], max_query_len=1, max_decode_query_len=1, max_prefill_seq_len=0, max_decode_seq_len=self.runner.max_seq_len_to_capture, query_start_loc=None, seq_start_loc=None, context_lens_tensor=None, block_tables=self._graph_block_tables[:batch_size], use_cuda_graph=True)
        if is_encoder_decoder_model:
            assert self.runner.attn_backend.get_name() in ['XFORMERS', 'FLASH_ATTN', 'ROCM_FLASH'], f"Expected attn_backend name to be either 'XFORMERS','ROCM_FLASH', or 'FLASH_ATTN', but got '{self.runner.attn_backend.get_name()}'"
            self._update_captured_metadata_for_enc_dec_model(batch_size=batch_size, attn_metadata=attn_metadata)
        return attn_metadata

    def get_graph_input_buffers(self, attn_metadata, is_encoder_decoder_model: bool=False) -> Dict[str, Any]:
        ...
        if is_encoder_decoder_model:
            assert self.runner.attn_backend.get_name() in ['XFORMERS', 'FLASH_ATTN', 'ROCM_FLASH'], f"Expected attn_backend name to be either 'XFORMERS','ROCM_FLASH', or 'FLASH_ATTN', but got '{self.runner.attn_backend.get_name()}'"
            self._add_additional_input_buffers_for_enc_dec_model(attn_metadata=attn_metadata, input_buffers=input_buffers)
        return input_buffers

    def prepare_graph_input_buffers(self, input_buffers, attn_metadata, is_encoder_decoder_model: bool=False) -> None:
        ...
        if is_encoder_decoder_model:
            assert self.runner.attn_backend.get_name() in ['XFORMERS', 'FLASH_ATTN'], f"Expected attn_backend name to be either 'XFORMERS' or 'FLASH_ATTN', but got '{self.runner.attn_backend.get_name()}'"
            self._prepare_input_buffers_for_enc_dec_model(attn_metadata, input_buffers)

    def begin_forward(self, model_input) -> None:
        return

    def _update_captured_metadata_for_enc_dec_model(self, batch_size: int, attn_metadata):
        """
        ...
        attn_metadata.encoder_seq_lens = torch.full((batch_size,), 1, dtype=torch.int).cuda()
        attn_metadata.encoder_seq_lens_tensor = torch.full((batch_size,), 1, dtype=torch.int).cuda()
        attn_metadata.max_encoder_seq_len = self.runner.max_seq_len_to_capture
        attn_metadata.num_encoder_tokens = 0

    def _add_additional_input_buffers_for_enc_dec_model(self, attn_metadata, input_buffers: Dict[str, Any]):
        """
        ...
        """
        input_buffers['encoder_seq_lens_tensor'] = attn_metadata.decode_metadata.encoder_seq_lens_tensor
        input_buffers['cross_slot_mapping'] = attn_metadata.decode_metadata.cross_slot_mapping
        input_buffers['cross_block_tables'] = attn_metadata.decode_metadata.cross_block_tables

    def _prepare_input_buffers_for_enc_dec_model(self, attn_metadata, input_buffers: Dict[str, Any]):
        """
        ...
        """
        input_buffers['encoder_seq_lens_tensor'].copy_(attn_metadata.decode_metadata.encoder_seq_lens_tensor, non_blocking=True)
        input_buffers['cross_slot_mapping'].copy_(attn_metadata.decode_metadata.cross_slot_mapping, non_blocking=True)
        input_buffers['cross_block_tables'].copy_(attn_metadata.decode_metadata.cross_block_tables, non_blocking=True)

def is_all_encoder_attn_metadata_set(attn_metadata):
    """
    All attention metadata required for encoder attention is set.
    """
    return attn_metadata.encoder_seq_lens is not None and attn_metadata.encoder_seq_lens_tensor is not None and (attn_metadata.max_encoder_seq_len is not None)
...
def is_all_cross_attn_metadata_set(attn_metadata):
    ...
    """
    return attn_metadata.is_all_encoder_attn_metadata_set and attn_metadata.cross_slot_mapping is not None and (attn_metadata.cross_block_tables is not None)

def get_seq_len_block_table_args(attn_metadata, is_prompt: bool, attn_type: str) -> tuple:
    """
    ...
    * Appropriate block tables (or None)
    ...
    if attn_type == AttentionType.DECODER:
        ...
        return (attn_metadata.encoder_seq_lens_tensor, attn_metadata.max_encoder_seq_len, attn_metadata.cross_block_tables)
    elif attn_type == AttentionType.ENCODER:
        return (attn_metadata.encoder_seq_lens_tensor, attn_metadata.max_encoder_seq_len, None)
    else:
        raise AttributeError(f'Invalid attention type {str(attn_type)}')

def get_num_prefill_decode_query_kv_tokens(attn_metadata, attn_type: str) -> Tuple[int, int, int]:
    """
...
def get_mla_dims(model_config: ModelConfig) -> MLADims:
    ...
    return MLADims(q_lora_rank=getattr(hf_text_config, 'q_lora_rank', None), kv_lora_rank=hf_text_config.kv_lora_rank, qk_nope_head_dim=hf_text_config.qk_nope_head_dim, qk_rope_head_dim=hf_text_config.qk_rope_head_dim, v_head_dim=hf_text_config.v_head_dim)