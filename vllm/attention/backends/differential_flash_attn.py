...
from vllm.attention.backends.abstract import AttentionBackend, AttentionImpl, AttentionLayer, AttentionMetadata, AttentionMetadataBuilder, AttentionType, is_quantized_kv_cache
from vllm.attention.backends.flash_attn import FlashAttentionBackend
from vllm.attention.backends.utils import PAD_SLOT_ID, CommonAttentionState, compute_slot_mapping, compute_slot_mapping_start_idx, is_all_cross_attn_metadata_set, is_all_encoder_attn_metadata_set, is_block_tables_empty
from vllm.attention.utils.fa_utils import flash_attn_supports_fp8, get_flash_attn_version
...
class DifferentialFlashAttentionMetadata(AttentionMetadata):
    ...
    def prefill_metadata(self) -> Optional['DifferentialFlashAttentionMetadata']:
        ...
        cross_layer_shared_block_tables = None if self.cross_layer_shared_block_tables is None else self.cross_layer_shared_block_tables[:self.num_prefills]
        self._cached_prefill_metadata = DifferentialFlashAttentionMetadata(num_prefills=self.num_prefills, num_prefill_tokens=self.num_prefill_tokens, num_decode_tokens=0, slot_mapping=slot_mapping, multi_modal_placeholder_index_maps=self.multi_modal_placeholder_index_maps, enable_kv_scales_calculation=self.enable_kv_scales_calculation, seq_lens=seq_lens, seq_lens_tensor=seq_lens_tensor, max_query_len=self.max_query_len, max_prefill_seq_len=self.max_prefill_seq_len, max_decode_query_len=0, max_decode_seq_len=0, query_start_loc=query_start_loc, seq_start_loc=seq_start_loc, context_lens_tensor=context_lens_tensor, block_tables=block_tables, cross_layer_shared_block_tables=cross_layer_shared_block_tables, use_cuda_graph=False, encoder_seq_lens=self.encoder_seq_lens, encoder_seq_lens_tensor=self.encoder_seq_lens_tensor, encoder_seq_start_loc=self.encoder_seq_start_loc, max_encoder_seq_len=self.max_encoder_seq_len, cross_slot_mapping=self.cross_slot_mapping, cross_block_tables=self.cross_block_tables)
        return self._cached_prefill_metadata
    ...
    def decode_metadata(self) -> Optional['DifferentialFlashAttentionMetadata']:
        ...
        block_tables = None if self.block_tables is None else self.block_tables[self.num_prefills:]
        cross_layer_shared_block_tables = None if self.cross_layer_shared_block_tables is None else self.cross_layer_shared_block_tables[self.num_prefills:]
        self._cached_decode_metadata = DifferentialFlashAttentionMetadata(num_prefills=0, num_prefill_tokens=0, num_decode_tokens=self.num_decode_tokens, slot_mapping=slot_mapping, multi_modal_placeholder_index_maps=None, enable_kv_scales_calculation=True, seq_lens=None, seq_lens_tensor=seq_lens_tensor, max_decode_query_len=self.max_decode_query_len, max_query_len=self.max_query_len, max_prefill_seq_len=0, max_decode_seq_len=self.max_decode_seq_len, query_start_loc=self.query_start_loc[self.num_prefills:] - self.query_start_loc[self.num_prefills] if self.query_start_loc is not None else None, seq_start_loc=self.seq_start_loc[self.num_prefills:] if self.seq_start_loc is not None else None, context_lens_tensor=None, block_tables=block_tables, cross_layer_shared_block_tables=cross_layer_shared_block_tables, use_cuda_graph=self.use_cuda_graph, encoder_seq_lens=self.encoder_seq_lens, encoder_seq_lens_tensor=self.encoder_seq_lens_tensor, encoder_seq_start_loc=self.encoder_seq_start_loc, max_encoder_seq_len=self.max_encoder_seq_len, cross_slot_mapping=self.cross_slot_mapping, cross_block_tables=self.cross_block_tables)
        return self._cached_decode_metadata
...
class DifferentialFlashAttentionMetadataBuilder(AttentionMetadataBuilder[DifferentialFlashAttentionMetadata]):
    ...
    def _add_seq_group(self, inter_data: 'ModelInputForGPUBuilder.InterDataForSeqGroup', chunked_prefill_enabled: bool, prefix_cache_hit: bool):
        ...
        3. slot mapping.
        """
        assert not chunked_prefill_enabled, 'chunked prefill is not supported for now'
        ...
        for seq_id, token_len, seq_len, curr_seq_len, query_len, context_len, curr_sliding_window_block in zip(inter_data.seq_ids, [len(t) for t in inter_data.input_tokens], inter_data.orig_seq_lens, inter_data.seq_lens, inter_data.query_lens, inter_data.context_lens, inter_data.curr_sliding_window_blocks):
            ...
            if prefix_cache_hit:
                ...
                if curr_sliding_window_block == 0:
                    ...
                    cross_layer_shared_block_table = block_tables[seq_id][-curr_sliding_window_block:]
    ...
    def build(self, seq_lens: List[int], query_lens: List[int], cuda_graph_pad_size: int, batch_size: int):
        ...
        if use_captured_graph:
            ...
            block_tables = self._get_graph_runner_block_tables(num_seqs, self.block_tables, self.runner.graph_block_tables)
            cross_layer_shared_block_tables = self._get_graph_runner_block_tables(num_seqs, self.cross_layer_shared_block_tables, self.runner.cross_layer_shared_graph_block_tables)
        else:
            block_tables = make_tensor_with_pad(self.block_tables, pad=0, dtype=torch.int, device=device)
            cross_layer_shared_block_tables = make_tensor_with_pad(self.cross_layer_shared_block_tables, pad=0, dtype=torch.int, device=device)
...
class DifferentialFlashAttentionImpl(AttentionImpl):
    ...
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[List[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: str=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[str]=None, use_irope: bool=False, differential_flash_attention_config: Optional[Dict[str, Any]]=None) -> None:
        ...
        self.vllm_flash_attn_version = get_flash_attn_version(requires_alibi=self.alibi_slopes is not None)
        if is_quantized_kv_cache(self.kv_cache_dtype) and (not self.kv_cache_dtype.startswith('fp8') or not flash_attn_supports_fp8()):
            raise NotImplementedError(f'FlashAttention does not support {self.kv_cache_dtype} kv-cache on this device (FA supports fp8 = {flash_attn_supports_fp8()}).')
    ...
    def populate_kv_cache(self, layer: AttentionLayer, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: DifferentialFlashAttentionMetadata):
        if kv_cache.numel() > 0 and key is not None and (value is not None):
            updated_slot_mapping = attn_metadata.slot_mapping
            torch.ops._C_cache_ops.reshape_and_cache_flash(key, value, kv_cache[0], kv_cache[1], updated_slot_mapping.flatten(), self.kv_cache_dtype, layer._k_scale, layer._v_scale)

    def forward_generate_kv_cache(self, query: torch.Tensor, key: Optional[torch.Tensor], value: Optional[torch.Tensor], k_cache: torch.Tensor, v_cache: torch.Tensor, attn_metadata: DifferentialFlashAttentionMetadata) -> torch.Tensor:
        ...
        if (prefill_meta := attn_metadata.prefill_metadata):
            if k_cache.numel() == 0 or prefill_meta.block_tables is None or prefill_meta.block_tables.numel() == 0:
                prefill_output = flash_attn_varlen_func(q=query, k=key, v=value, cu_seqlens_q=prefill_meta.seq_start_loc, cu_seqlens_k=prefill_meta.seq_start_loc, max_seqlen_q=prefill_meta.max_prefill_seq_len, max_seqlen_k=prefill_meta.max_prefill_seq_len, softmax_scale=self.scale, causal=True, window_size=self.sliding_window, alibi_slopes=self.alibi_slopes, softcap=self.logits_soft_cap)
        ...
        if (decode_meta := attn_metadata.decode_metadata):
            ...
            try:
                output[num_prefill_tokens:] = flash_attn_with_kvcache(q=decode_query.unsqueeze(1), k_cache=k_cache, v_cache=v_cache, block_table=block_tables_arg, cache_seqlens=decode_meta.seq_lens_tensor, softmax_scale=self.scale, causal=True, window_size=self.sliding_window, alibi_slopes=self.alibi_slopes, softcap=self.logits_soft_cap).squeeze(1)
            except Exception as e:
                logger.error('Error in PagedAttention.forward_decode: %s', str(e))
                raise e
        return output.view(-1, num_heads, head_size)

    def forward_with_kv_cache_only(self, query: torch.Tensor, k_cache: torch.Tensor, v_cache: torch.Tensor, attn_metadata: DifferentialFlashAttentionMetadata):
        ...
        output = flash_attn_with_kvcache(q=query.unsqueeze(1), k_cache=k_cache, v_cache=v_cache, block_table=block_tables_arg, cache_seqlens=attn_metadata.seq_lens_tensor, softmax_scale=self.scale, causal=True, window_size=self.sliding_window, alibi_slopes=self.alibi_slopes, softcap=self.logits_soft_cap).squeeze(1)
        return output

    def forward(self, layer: AttentionLayer, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: DifferentialFlashAttentionMetadata, output: Optional[torch.Tensor]=None, output_scale: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if not self.used_shared_kv_cache:
