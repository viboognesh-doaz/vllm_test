...
from vllm.attention.backends.abstract import AttentionBackend, AttentionImpl, AttentionMetadata, AttentionMetadataBuilder
...
class PlaceholderAttentionMetadata(AttentionMetadata):
    ...
    def advance_step(self, model_input: 'ModelInputForGPUWithSamplingMetadata', sampled_token_ids: Optional[torch.Tensor], block_size: int, num_seqs: int, num_queries: int, turn_prefills_into_decodes: bool=False):
        ...
        assert not turn_prefills_into_decodes, 'Multi-Step + Chunked-Prefill is not supported for attention-freemodels. turn_prefills_into_decodes is a Multi-Step + Chunked-Prefill specific parameter.'
        ...
        if sampled_token_ids is not None:
            model_input.input_tokens.masked_scatter_(mask, sampled_token_ids[:num_queries])

class PlaceholderAttentionMetadataBuilder(AttentionMetadataBuilder[PlaceholderAttentionMetadata]):
