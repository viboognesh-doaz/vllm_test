...
from vllm.attention.backends.flash_attn import FlashAttentionBackend, FlashAttentionImpl, FlashAttentionMetadata, FlashAttentionMetadataBuilder
from vllm.distributed.parallel_state import get_tensor_model_parallel_rank
from vllm.logger import init_logger
from vllm.utils import async_tensor_h2d
from vllm.vllm_flash_attn import flash_attn_varlen_func, flash_attn_with_kvcache, sparse_attn_func
...
class DualChunkFlashAttentionMetadata(FlashAttentionMetadata):
    ...
    def prefill_metadata(self) -> Optional['DualChunkFlashAttentionMetadata']:
        ...
        prefill_metadata.orig_seq_lens = None if self.orig_seq_lens is None else self.orig_seq_lens[:self.num_prefills]
        prefill_metadata.orig_seq_lens_tensor = None if self.orig_seq_lens_tensor is None else self.orig_seq_lens_tensor[:self.num_prefills]
        if self.original_max_position_embeddings > 0:
            assert prefill_metadata.orig_seq_lens_tensor is not None
            prefill_metadata.scaling_factor = (0.1 * torch.log(prefill_metadata.orig_seq_lens_tensor / self.original_max_position_embeddings) + 1.0).clip(min=1)
    ...
    def decode_metadata(self) -> Optional['DualChunkFlashAttentionMetadata']:
        ...
        if decode_metadata is None:
            return None
        decode_metadata = DualChunkFlashAttentionMetadata(**decode_metadata.asdict_zerocopy())
        decode_metadata.orig_seq_lens_tensor = None if self.orig_seq_lens_tensor is None else self.orig_seq_lens_tensor[self.num_prefills:]
        ...
        if self.original_max_position_embeddings > 0:
            decode_metadata.scaling_factor = (0.1 * torch.log(cache_seq_lens / self.original_max_position_embeddings) + 1.0).clip(min=1)
        ...
        block_tables_intra = torch.zeros(batch_size, (max_seq_len_intra - 1) // self.block_size + 1, dtype=decode_metadata.block_tables.dtype, device=decode_metadata.block_tables.device)
        for i in range(batch_size):
            st = chunk_num_curr[i] * chunk_len // self.block_size
            ed = min(st + (max_seq_len_intra - 1) // self.block_size + 1, (cache_seq_lens[i] - 1) // self.block_size + 1)
        ...
        if max_seq_len_succ:
            block_tables_succ = torch.zeros(batch_size, (max_seq_len_succ - 1) // self.block_size + 1, dtype=decode_metadata.block_tables.dtype, device=decode_metadata.block_tables.device)
            for i in range(batch_size):
                start = (chunk_num_curr[i] - 1).clip(min=0) * chunk_len // self.block_size
                end = min(start + (max_seq_len_succ - 1) // self.block_size + 1, (cache_seq_lens[i] - 1) // self.block_size + 1)
                block_tables_succ[i, :end - start] = decode_metadata.block_tables[i, start:end]
...
class DualChunkFlashAttentionMetadataBuilder(FlashAttentionMetadataBuilder):
    ...
    def build(self, seq_lens: List[int], query_lens: List[int], cuda_graph_pad_size: int, batch_size: int):
        attn_metadata = super().build(seq_lens, query_lens, cuda_graph_pad_size, batch_size)
        attn_metadata = DualChunkFlashAttentionMetadata(**attn_metadata.asdict_zerocopy())
        device = self.runner.device
        attn_metadata.orig_seq_lens = self.orig_seq_lens
        attn_metadata.orig_seq_lens_tensor = async_tensor_h2d(self.orig_seq_lens, torch.int, device, self.runner.pin_memory)
        attn_metadata.block_size = self.runner.block_size
        dual_chunk_attn_config = getattr(self.runner.model_config.hf_config, 'dual_chunk_attention_config', {})
        attn_metadata.original_max_position_embeddings = dual_chunk_attn_config.get('original_max_position_embeddings', 0)
...
class DualChunkFlashAttentionImpl(FlashAttentionImpl):
    ...
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[List[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: str=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[str]=None, layer_idx: int=-1, dual_chunk_attention_config: Optional[Dict[str, Any]]=None) -> None:
        if kv_sharing_target_layer_name is not None:
            raise NotImplementedError('KV sharing is not supported in V0 DUAL_CHUNK_FLASH_ATTN backend.')
        ...
        if head_size not in support_head_sizes:
            raise ValueError(f'Head size {head_size} is not supported by FlashAttention. Supported head sizes are: {support_head_sizes}.')
        ...
        self.original_max_position_embeddings = dual_chunk_attention_config.get('original_max_position_embeddings', 0)
        self.sparse_attention_config = dual_chunk_attention_config.get('sparse_attention_config', None)
        if not self.sparse_attention_config:
            logger.warning_once('Sparse attention will not be enabled as sparse attention config is not provided.')
        self.sparse_attention_enabled = dual_chunk_attention_config.get('sparse_attention_enabled', self.sparse_attention_config is not None)
        self.sparse_attention_threshold = dual_chunk_attention_config.get('sparse_attention_threshold', 32768)
        self.sparse_attention_last_q = dual_chunk_attention_config.get('sparse_attention_last_q', 64)
        self.layer_idx = layer_idx
        self.dual_chunk_attention_config = dual_chunk_attention_config
        if self.sparse_attention_config:
            self.sparse_attention_config = {int(i): j for i, j in self.sparse_attention_config[self.layer_idx].items()}
            start_head = self.num_heads * get_tensor_model_parallel_rank()
            end_head = start_head + self.num_heads
            self.sparse_attention_config = [self.sparse_attention_config[i] for i in range(start_head, end_head)]
        if self.sparse_attention_enabled:
            self.arange = torch.arange(self.sparse_attention_last_q, device='cuda')
            self.last_q_mask = self.arange[None, None, :, None] >= self.arange[None, None, None, :]

    def forward(self, layer: AttentionLayer, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: DualChunkFlashAttentionMetadata, output: Optional[torch.Tensor]=None, output_scale: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        query, query_succ, query_inter, query_succ_critical, query_inter_critical = torch.split(query, query.shape[-1] // 5, dim=-1)
        assert query_succ is not None and query_inter is not None, 'query_succ and query_inter are required in Dual Chunk Attention.'
        ...
        if self.original_max_position_embeddings > 0:
            if (prefill_meta := attn_metadata.prefill_metadata):
                ...
                for i in range(len(prefill_meta.orig_seq_lens)):
                    current_end = current_start + (query_start_loc_cpu[i + 1] - query_start_loc_cpu[i]).item()
            ...
            if (decode_meta := attn_metadata.decode_metadata):
                ...
                key[attn_metadata.num_prefill_tokens:].mul_(scaling_factor.unsqueeze(-1).unsqueeze(-1))
        if kv_cache is not None and kv_cache.numel() > 0:
            key_cache = kv_cache[0]
            value_cache = kv_cache[1]
            ops.reshape_and_cache_flash(key, value, key_cache, value_cache, attn_metadata.slot_mapping.flatten(), self.kv_cache_dtype, layer._k_scale, layer._v_scale)
    ...
    def _dual_chunk_flash_attn_prefill(self, q, q_succ, q_inter, q_succ_critical, q_inter_critical, k, v, cu_seqlens_q, cu_seqlens_k, orig_seq_lens: List[int], scaling_factor: torch.Tensor, softmax_scale: float, causal: Optional[bool]=True, window_size: Tuple[int, int]=(-1, -1), alibi_slopes: Optional[torch.Tensor]=None, block_table: Optional[torch.Tensor]=None, chunk_size: int=8192, local_size: int=1024):
        ...
        for i in range(0, len(cu_seqlens_q_cpu) - 1):
            ...
            if block_table is None:
                ...
                current_k = k
                current_v = v
            sparse_attn_enabled = self.sparse_attention_enabled and current_orig_seq_len > self.sparse_attention_threshold
            if current_q.shape[0] == 0:
                continue
            if current_k.shape[0] == 0:
                all_outputs.append(torch.zeros((current_q.shape[0], current_q.shape[1], v.shape[2]), device=q.device, dtype=q.dtype))
                continue
            ...
            if sparse_attn_enabled:
                ...
                for head_id in range(current_q.size(-2)):
                    ty, vertical_size, slash_size, _ = self.sparse_attention_config[head_id]
                ...
                current_output = self._dual_chunk_flash_attn_prefill_func(current_q, current_q_succ, current_q_inter, current_q_succ_critical, current_q_inter_critical, current_k, current_v, current_block_table, softmax_scale, chunk_size, local_size, scaling_factor[i].item(), ke - ks, sparse_attn_enabled=sparse_attn_enabled, heads_vertical_size=heads_vertical_size, heads_slash_size=heads_slash_size, group_size=group_size)
            else:
                for head_id in range(current_q.size(-2)):
                    ...
                    current_q_succ_head_critical = current_q_succ_critical[:, head_id, :].unsqueeze(1)
                    current_q_inter_head_critical = current_q_inter_critical[:, head_id, :].unsqueeze(1)
                    if block_table is not None:
                        current_k_head = current_k[..., head_id // group_size, :].unsqueeze(2)
                        current_v_head = current_v[..., head_id // group_size, :].unsqueeze(2)
                    else:
                        current_k_head = current_k[:, head_id, :].unsqueeze(1)
                        current_v_head = current_v[:, head_id, :].unsqueeze(1)
                    current_out = self._dual_chunk_flash_attn_prefill_func(current_q_head, current_q_succ_head, current_q_inter_head, current_q_succ_head_critical, current_q_inter_head_critical, current_k_head, current_v_head, current_block_table, softmax_scale, chunk_size, local_size, scaling_factor[i].item(), ke - ks, sparse_attn_enabled=sparse_attn_enabled)
                    current_output[:, head_id:head_id + 1, :] = current_out
            all_outputs.append(current_output)
        return torch.cat(all_outputs, dim=0)

    def _dual_chunk_flash_attn_prefill_func(self, q, q_succ, q_inter, q_succ_critical, q_inter_critical, k, v, block_table, softmax_scale: float, chunk_size: int, local_size: int, scaling_factor: float, k_length: int, sparse_attn_enabled: Optional[bool]=True, heads_vertical_size=None, heads_slash_size=None, group_size=None):
        flash_results = []
        ...
        while begin < k_length:
            ...
            if block_table is not None:
                block_tables_intra = _get_block(block_table, block_size, prev_chunk_end_pos, end)
                k_states_intra = k[block_tables_intra].view(-1, *k.shape[-2:])[:end - prev_chunk_end_pos]
                v_states_intra = v[block_tables_intra].view(-1, *v.shape[-2:])[:end - prev_chunk_end_pos]
            else:
            ...
            if sparse_attn_enabled:
                ...
                k_states_intra = k_states_intra.unsqueeze(2).repeat(1, 1, group_size, 1).reshape(-1, num_device_k_heads * group_size, head_dim)
                v_states_intra = v_states_intra.unsqueeze(2).repeat(1, 1, group_size, 1).reshape(-1, num_device_k_heads * group_size, head_dim)
                qk_chunks.append(q_states_intra.transpose(0, 1)[:, -last_q_size:] * softmax_scale @ k_states_intra.permute(1, 2, 0))
            if prev_chunk_end_pos - chunk_len >= 0:
                q_states_succ = q_succ[qbegin:qend]
                q_states_succ_critical = q_succ_critical[qbegin:qend]
                if block_table is not None:
                    block_tables_succ = _get_block(block_table, block_size, prev_chunk_end_pos - chunk_len, prev_chunk_end_pos)
                    k_states_succ = k[block_tables_succ].view(-1, *k.shape[-2:])[:chunk_len]
                    v_states_succ = v[block_tables_succ].view(-1, *v.shape[-2:])[:chunk_len]
                else:
                    k_states_succ = k[prev_chunk_end_pos - chunk_len:prev_chunk_end_pos]
                    v_states_succ = v[prev_chunk_end_pos - chunk_len:prev_chunk_end_pos]
                if sparse_attn_enabled:
                    k_states_succ = k_states_succ.unsqueeze(2).repeat(1, 1, group_size, 1).reshape(-1, num_device_k_heads * group_size, head_dim)
                    v_states_succ = v_states_succ.unsqueeze(2).repeat(1, 1, group_size, 1).reshape(-1, num_device_k_heads * group_size, head_dim)
                    qk_chunks.append(q_states_succ_critical.transpose(0, 1)[:, -last_q_size:] * softmax_scale @ k_states_succ.permute(1, 2, 0))
            if prev_chunk_end_pos - chunk_len * 2 >= 0:
                ...
                if block_table is not None:
                    block_tables_inter = _get_block(block_table, block_size, 0, prev_chunk_end_pos - chunk_len)
                    k_states_inter = k[block_tables_inter].view(-1, *k.shape[-2:])[:prev_chunk_end_pos - chunk_len]
                    v_states_inter = v[block_tables_inter].view(-1, *v.shape[-2:])[:prev_chunk_end_pos - chunk_len]
                else:
                    k_states_inter = k[:prev_chunk_end_pos - chunk_len]
                    v_states_inter = v[:prev_chunk_end_pos - chunk_len]
                if sparse_attn_enabled:
                    k_states_inter = k_states_inter.unsqueeze(2).repeat(1, 1, group_size, 1).reshape(-1, num_device_k_heads * group_size, head_dim)
                    v_states_inter = v_states_inter.unsqueeze(2).repeat(1, 1, group_size, 1).reshape(-1, num_device_k_heads * group_size, head_dim)
                    qk_chunks.append(q_states_inter_critical.transpose(0, 1)[:, -last_q_size:] * softmax_scale @ k_states_inter.permute(1, 2, 0))
            if sparse_attn_enabled:
                reversed_qk = qk_chunks[::-1]
                qk = torch.cat(reversed_qk, dim=-1)
                qk[:, :, -last_q_size:] = torch.where(self.last_q_mask[..., -last_q_size:, -last_q_size:].to(qk.device), qk[:, :, -last_q_size:], -torch.inf)
                ...
                slash_topk_buffer = torch.empty(size=(n_heads, max_slash_topk), dtype=torch.int64, device=qk.device)
                ...
                vertical_buffer = torch.full((n_heads, max_vertical_topk), int32_max, dtype=torch.int64, device=q.device)
                slash_buffer = torch.full((n_heads, max_slash_topk), int32_min, dtype=torch.int64, device=q.device)
                succ_vertical_buffer = torch.full((n_heads, max_vertical_topk), int32_max, dtype=torch.int64, device=q.device)
                succ_slash_buffer = torch.full((n_heads, max_slash_topk), int32_min, dtype=torch.int64, device=q.device)
                inter_vertical_buffer = torch.full((n_heads, max_vertical_topk), int32_max, dtype=torch.int64, device=q.device)
                inter_slash_buffer = torch.full((n_heads, max_slash_topk), int32_min, dtype=torch.int64, device=q.device)
                vertical_size_buffer = torch.empty(size=(n_heads,), dtype=torch.int32, device=q.device)
                slash_sizes_buffer = torch.empty(size=(n_heads,), dtype=torch.int32, device=q.device)
                succ_vertical_size_buffer = torch.empty(size=(n_heads,), dtype=torch.int32, device=q.device)
                succ_slash_sizes_buffer = torch.empty(size=(n_heads,), dtype=torch.int32, device=q.device)
                inter_vertical_size_buffer = torch.empty(size=(n_heads,), dtype=torch.int32, device=q.device)
                inter_slash_sizes_buffer = torch.empty(size=(n_heads,), dtype=torch.int32, device=q.device)
                for head_i in range(n_heads):
                    vertical_topk = vertical_topk_buffer[head_i, :heads_vertical_size[head_i]]
                    intra_vertical_indices = vertical_topk[vertical_topk >= prev_chunk_end_pos] - prev_chunk_end_pos
                    if intra_vertical_indices.nelement() == 0:
                        intra_vertical_indices = torch.cat([intra_vertical_indices, torch.arange(0, k_states_intra.size(0), max(1, k_states_intra.size(0) / 5), dtype=torch.int32, device=intra_vertical_indices.device)])
                    slash_topk = slash_topk_buffer[head_i, :heads_slash_size[head_i]]
                    intra_slash_indices = qk.size(-1) - 1 - slash_topk[slash_topk >= prev_chunk_end_pos]
                    ...
                    if prev_chunk_end_pos - chunk_len >= 0:
                        succ_vertical_indices = vertical_topk[(vertical_topk < prev_chunk_end_pos) & (vertical_topk >= prev_chunk_end_pos - chunk_len)] - (prev_chunk_end_pos - chunk_len)
                        if succ_vertical_indices.nelement() == 0:
                            succ_vertical_indices = torch.cat([succ_vertical_indices, torch.arange(0, k_states_succ.size(0), max(1, k_states_succ.size(0) / 5), dtype=torch.int32, device=intra_vertical_indices.device)])
                        succ_slash_indices = prev_chunk_end_pos + (qend - qbegin) - 1 - slash_topk[(slash_topk >= prev_chunk_end_pos - chunk_len) & (slash_topk < prev_chunk_end_pos + (qend - qbegin))]
                        if succ_slash_indices.nelement() == 0:
                            succ_slash_indices = torch.cat([succ_slash_indices, torch.arange(0, k_states_succ.size(0), max(1, k_states_succ.size(0) / 5), dtype=torch.int32, device=intra_vertical_indices.device)])
                        ...
                        succ_vertical_buffer[head_i, :v_count].copy_(succ_vertical_indices)
                        succ_slash_buffer[head_i, :s_count].copy_(succ_slash_indices)
                    if prev_chunk_end_pos - 2 * chunk_len >= 0:
                        inter_vertical_indices = vertical_topk[vertical_topk < prev_chunk_end_pos - chunk_len]
                        if inter_vertical_indices.nelement() == 0:
                            inter_vertical_indices = torch.cat([inter_vertical_indices, torch.arange(0, k_states_inter.size(0), max(1, k_states_inter.size(0) / 5), dtype=torch.int32, device=intra_vertical_indices.device)])
                        inter_slash_indices = prev_chunk_end_pos - chunk_len + (qend - qbegin) - 1 - slash_topk[slash_topk < prev_chunk_end_pos - chunk_len + (qend - qbegin)]
                        if inter_slash_indices.nelement() == 0:
                            inter_slash_indices = torch.cat([inter_slash_indices, torch.arange(0, k_states_inter.size(0), max(1, k_states_inter.size(0) / 5), dtype=torch.int32, device=intra_vertical_indices.device)])
                        ...
                        inter_vertical_buffer[head_i, :v_count].copy_(inter_vertical_indices)
            ...
            if sparse_attn_enabled:
                flash_result = self._do_flash_attn(q_states_intra, k_states_intra, v_states_intra, softmax_scale=softmax_scale, causal=True, block_table=block_table, stage='intra', vertical_indices=vertical_buffer, slash_indices=slash_buffer, vertical_indices_count=vertical_size_buffer, slash_indices_count=slash_sizes_buffer, mergehead_softmax_scale=softmax_scale, sparse_attn_enabled=sparse_attn_enabled)
            else:
                flash_result = self._do_flash_attn(q_states_intra, k_states_intra, v_states_intra, softmax_scale=softmax_scale, causal=True, block_table=block_table, stage='intra', vertical_indices=intra_vertical_indices, slash_indices=intra_slash_indices, sparse_attn_enabled=sparse_attn_enabled)
            flash_per_chunk.append(flash_result)
            if prev_chunk_end_pos - chunk_len >= 0:
                if sparse_attn_enabled:
                    flash_result = self._do_flash_attn(q_states_succ, k_states_succ, v_states_succ, softmax_scale=softmax_scale, causal=False, block_table=block_table, stage='succ', vertical_indices=succ_vertical_buffer, slash_indices=succ_slash_buffer, vertical_indices_count=succ_vertical_size_buffer, slash_indices_count=succ_slash_sizes_buffer, mergehead_softmax_scale=softmax_scale, sparse_attn_enabled=sparse_attn_enabled)
                else:
                    flash_result = self._do_flash_attn(q_states_succ, k_states_succ, v_states_succ, softmax_scale=softmax_scale, causal=False, block_table=block_table, stage='succ', vertical_indices=succ_vertical_indices, slash_indices=succ_slash_indices, sparse_attn_enabled=sparse_attn_enabled)
                flash_per_chunk.append(flash_result)
            if prev_chunk_end_pos - chunk_len * 2 >= 0:
                if sparse_attn_enabled:
                    flash_result = self._do_flash_attn(q_states_inter, k_states_inter, v_states_inter, softmax_scale=softmax_scale, causal=False, block_table=block_table, stage='inter', vertical_indices=inter_vertical_buffer, slash_indices=inter_slash_buffer, vertical_indices_count=inter_vertical_size_buffer, slash_indices_count=inter_slash_sizes_buffer, mergehead_softmax_scale=softmax_scale, sparse_attn_enabled=sparse_attn_enabled)
                else:
                    flash_result = self._do_flash_attn(q_states_inter, k_states_inter, v_states_inter, softmax_scale=softmax_scale, causal=False, block_table=block_table, stage='inter', vertical_indices=inter_vertical_indices, slash_indices=inter_slash_indices, sparse_attn_enabled=sparse_attn_enabled)
                flash_per_chunk.append(flash_result)
    ...
    def _dual_chunk_flash_attn_decoding(self, query: torch.Tensor, query_succ: torch.Tensor, query_inter: torch.Tensor, key_cache: torch.Tensor, value_cache: torch.Tensor, block_table: torch.Tensor, cache_seqlens: torch.Tensor, softmax_scale: float, causal: bool, alibi_slopes: Optional[torch.Tensor], chunk_size: int, local_size: int, original_max_position_embeddings: int, decode_meta: DualChunkFlashAttentionMetadata):
        ...
        if original_max_position_embeddings > 0:
            assert decode_meta.scaling_factor is not None
            scaling_factor = decode_meta.scaling_factor
            query = (query * scaling_factor.view(-1, 1, 1, 1)).to(query.dtype)
            query_succ = (query_succ * scaling_factor.view(-1, 1, 1, 1)).to(query.dtype)
            query_inter = (query_inter * scaling_factor.view(-1, 1, 1, 1)).to(query.dtype)
        outputs_list = []
        softmax_lses_list = []
        intra_output, intra_softmax_lse = self._dual_chunk_flash_attn_decoding_with_exp_sums(query, key_cache, value_cache, decode_meta.block_tables_intra, decode_meta.seq_lens_intra, softmax_scale, alibi_slopes, causal=False)
        outputs_list.append(intra_output)
        softmax_lses_list.append(intra_softmax_lse)
        if decode_meta.max_seq_len_succ:
            succ_output, succ_softmax_lse = self._dual_chunk_flash_attn_decoding_with_exp_sums(query_succ, key_cache, value_cache, decode_meta.block_tables_succ, decode_meta.seq_lens_succ, softmax_scale, alibi_slopes, causal=False)
            outputs_list.append(succ_output)
            softmax_lses_list.append(succ_softmax_lse)
        if decode_meta.max_seq_len_inter:
            inter_output, inter_softmax_lse = self._dual_chunk_flash_attn_decoding_with_exp_sums(query_inter, key_cache, value_cache, block_table[:, :decode_meta.max_seq_len_inter], decode_meta.seq_lens_inter, softmax_scale, alibi_slopes, causal=False)
    ...
    def _dual_chunk_flash_attn_decoding_with_exp_sums(self, query: torch.Tensor, key_cache: torch.Tensor, value_cache: torch.Tensor, block_table: torch.Tensor, cache_seqlens: torch.Tensor, softmax_scale: float, alibi_slopes: Optional[torch.Tensor], causal: bool):
        out, softmax_lse = flash_attn_with_kvcache(q=query, k_cache=key_cache, v_cache=value_cache, block_table=block_table, cache_seqlens=cache_seqlens, softmax_scale=softmax_scale, alibi_slopes=alibi_slopes, causal=causal, return_softmax_lse=True)
        mask = cache_seqlens == 0
        out[mask] = 0
        softmax_lse[mask] = -float('inf')
        return (out, softmax_lse)

def _vertical_slash_sparse_attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, v_idx: torch.Tensor, s_idx: torch.Tensor, softmax_scale: float, causal: bool=True, stage: str='intra', block_size_M: int=64, block_size_N: int=64, vertical_indices_count: torch.Tensor=None, slash_indices_count: torch.Tensor=None):
    if stage == 'intra':
        assert causal
    else:
        assert not causal
    ...
    if vertical_indices_count is not None and slash_indices_count is not None:
        block_count, block_offset, column_count, column_index = ops.convert_vertical_slash_indexes_mergehead(q_seqlens, kv_seqlens, v_idx, s_idx, vertical_indices_count, slash_indices_count, context_size, block_size_M, block_size_N, causal)
    else:
        block_count, block_offset, column_count, column_index = ops.convert_vertical_slash_indexes(q_seqlens, kv_seqlens, v_idx, s_idx, context_size, block_size_M, block_size_N, causal)
    q = query.transpose(1, 2).contiguous()
    k = key.transpose(1, 2).contiguous()
    v = value.transpose(1, 2).contiguous()
    out, lse = sparse_attn_func(q, k, v, block_count, block_offset, column_count, column_index, causal=causal, softmax_scale=softmax_scale, return_softmax_lse=True)
