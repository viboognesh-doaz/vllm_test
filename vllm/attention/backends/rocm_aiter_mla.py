...
from vllm.attention.backends.mla.common import MLACommonBackend, MLACommonImpl, MLACommonMetadata, MLACommonMetadataBuilder, MLACommonState
from vllm.attention.backends.utils import compute_slot_mapping, compute_slot_mapping_start_idx, is_block_tables_empty
from vllm.attention.ops.rocm_aiter_mla import aiter_mla_decode_fwd, get_aiter_mla_metadata
...
class AiterMLAMetadata(MLACommonMetadata):
    ...
    def prefill_metadata(self):
        ...
        if prefill_metadata is not None:
            ...
            prefill_metadata.paged_kv_last_page_lens = self.paged_kv_last_page_lens
    ...
    def decode_metadata(self):
        ...
        if decode_metadata is not None:
            ...
            decode_metadata.paged_kv_last_page_lens = self.paged_kv_last_page_lens
...
class AiterMLAMetadataBuilder(MLACommonMetadataBuilder[AiterMLAMetadata]):
    ...
    def _add_seq_group(self, inter_data, chunked_prefill_enabled: bool, prefix_cache_hit: bool):
    ...
    def build(self, seq_lens: list[int], query_lens: list[int], cuda_graph_pad_size: int, batch_size: int) -> AiterMLAMetadata:
        ...
        if len(self.paged_kv_indptr) > 0:
            ...
            paged_kv_indices_tensor = torch.tensor(self.paged_kv_indices, device=device, dtype=torch.int)
            paged_kv_indptr_tensor = torch.tensor(self.paged_kv_indptr, device=device, dtype=torch.int)
            paged_kv_last_page_lens_tensor = torch.tensor(self.paged_kv_last_page_lens, device=device, dtype=torch.int)
            block_table_bound_tensor = torch.zeros(len(self.paged_kv_indptr) - 1, device=device, dtype=torch.int)
            qo_indptr = torch.tensor(self.qo_indptr, device=device, dtype=torch.int)
        else:
...
class AiterMLAState(MLACommonState[AiterMLAMetadata]):
    ...
    def graph_capture(self, max_batch_size: int):
        kv_indices, kv_indptr, last_page_lens, qo_indptr = get_aiter_mla_metadata(max_batch_size=max_batch_size, block_size=self.runner.block_size, max_block_per_batch=self.runner.get_max_block_per_batch(), device=self.runner.device)
    ...
    def graph_capture_get_metadata_for_batch(self, batch_size: int, is_encoder_decoder_model: bool=False) -> AiterMLAMetadata:
        metadata = super().graph_capture_get_metadata_for_batch(batch_size, is_encoder_decoder_model)
    ...
    def get_graph_input_buffers(self, attn_metadata: AiterMLAMetadata, is_encoder_decoder_model: bool=False):
        input_buffers = super().get_graph_input_buffers(attn_metadata, is_encoder_decoder_model)
        input_buffers['paged_kv_indptr'] = attn_metadata.decode_metadata.paged_kv_indptr
        input_buffers['paged_kv_indices'] = attn_metadata.decode_metadata.paged_kv_indices
        input_buffers['paged_kv_last_page_lens'] = attn_metadata.decode_metadata.paged_kv_last_page_lens
        input_buffers['qo_indptr'] = attn_metadata.qo_indptr
        return input_buffers

    def prepare_graph_input_buffers(self, input_buffers, attn_metadata: AiterMLAMetadata, is_encoder_decoder_model: bool=False):
        super().prepare_graph_input_buffers(input_buffers, attn_metadata, is_encoder_decoder_model)
        num_total_blocks = attn_metadata.decode_metadata.paged_kv_indices.shape[0]
        input_buffers['paged_kv_indptr'].copy_(attn_metadata.decode_metadata.paged_kv_indptr, non_blocking=True)
        input_buffers['paged_kv_indices'][:num_total_blocks].copy_(attn_metadata.decode_metadata.paged_kv_indices, non_blocking=True)
        input_buffers['paged_kv_last_page_lens'].copy_(attn_metadata.decode_metadata.paged_kv_last_page_lens, non_blocking=True)
        input_buffers['qo_indptr'].copy_(attn_metadata.decode_metadata.qo_indptr, non_blocking=True)

class AiterMLAImpl(MLACommonImpl[AiterMLAMetadata]):

    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[list[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float], attn_type: str, kv_sharing_target_layer_name: Optional[str], **mla_args) -> None:
        ...
        if any(unsupported_features):
            raise NotImplementedError('Aiter MLA does not support one of the following: alibi_slopes, sliding_window, logits_soft_cap')
        self.flash_attn_varlen_func = flash_attn_varlen_func

    def _flash_attn_varlen_diff_headdims(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, softmax_scale: float, return_softmax_lse: bool, **kwargs) -> Union[tuple[torch.Tensor, ...], torch.Tensor]:
        output = self.flash_attn_varlen_func(q, k, v, **kwargs)
        return output

    def _forward_decode(self, q_nope: torch.Tensor, q_pe: torch.Tensor, kv_c_and_k_pe_cache: torch.Tensor, attn_metadata: AiterMLAMetadata) -> torch.Tensor:
        assert kv_c_and_k_pe_cache.numel() > 0
        decode_meta = attn_metadata.decode_metadata
        assert decode_meta is not None
        B = q_nope.shape[0]
        q = torch.cat([q_nope, q_pe], dim=-1)
        o = torch.empty(B, self.num_heads, self.kv_lora_rank, dtype=q.dtype, device=q.device)
        kv_buffer = kv_c_and_k_pe_cache.unsqueeze(2)
        aiter_mla_decode_fwd(q, kv_buffer, o, self.scale, attn_metadata.qo_indptr, attn_metadata.max_query_len, attn_metadata.paged_kv_indptr, attn_metadata.paged_kv_indices, attn_metadata.paged_kv_last_page_lens)
