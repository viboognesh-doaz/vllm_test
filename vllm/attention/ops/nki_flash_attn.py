...
def load_block_tables(block_tables_hbm, num_tiles, num_blocks_per_tile):
    ...
    block_tables_sbuf = nl.zeros((cdiv(num_tiles, B_P_SIZE), par_dim(B_P_SIZE), num_blocks_per_tile), dtype=nl.int32)
    for i in nl.affine_range(cdiv(num_tiles, B_P_SIZE)):
        i_p = nl.arange(B_P_SIZE)[:, None]
        i_f = nl.arange(num_blocks_per_tile)[None, :]
        block_tables_sbuf[i, i_p, i_f] = nl.load(block_tables_hbm[i_p + i * B_P_SIZE, i_f], dtype=nl.int32, mask=i_p + i * B_P_SIZE < num_tiles)
    return block_tables_sbuf

@nki.jit
def transform_block_tables_for_indirect_load(block_tables, block_size_tiling_factor, num_head, head_id):
    """
    ...
    assert is_power_of_2(num_blocks_per_tile), f'num_blocks_per_tile={num_blocks_per_tile!r} is not power of 2'
    num_loads = cdiv(num_blocks_per_tile, B_P_SIZE)
    block_tables_transposed = nl.ndarray((num_loads, par_dim(B_P_SIZE), num_partitions * num_tiles_per_partition), dtype=nl.int32)
    if num_head > 1:
    ...
    if block_size_tiling_factor > 1:
        broadcast_shape = (num_tiles_per_partition, num_blocks_per_tile, block_size_tiling_factor)
        offset = nisa.iota(nl.arange(block_size_tiling_factor)[None, None, :], dtype=nl.int32).broadcast_to(broadcast_shape)
    for partition_id in nl.affine_range(num_partitions):
        ...
        if block_size_tiling_factor > 1:
            ...
            block_tables_partition = (block_tables_partition * block_size_tiling_factor).reshape((num_tiles_per_partition, num_blocks_per_tile, 1)).broadcast_to(broadcast_shape)
        ...
        for i in nl.affine_range(num_loads):
            i_p = nl.arange(B_P_SIZE)[:, None]
            i_f = partition_id * num_tiles_per_partition + nl.arange(num_tiles_per_partition)[None, :]
            block_tables_transposed[i, i_p, i_f] = nl.transpose(new_block_tables[:, nl.ds(i * B_P_SIZE, B_P_SIZE)])
    return block_tables_transposed

@nki.jit
def load_kv_tile_from_cache(cur_k_tile, cur_v_tile, kv_cache, block_tables, large_k_tile_idx, num_blocks_per_large_tile, tiled_block_size, B_P_SIZE, B_D_SIZE):
    """
    ...
    for load_idx in nl.affine_range(num_loads):
        ...
        loaded = nl.load(kv_cache[0, block_tables[load_idx, i_p, large_k_tile_idx], i_f])
        if cur_k_tile.dtype != loaded.dtype:
            loaded = nl.copy(loaded, dtype=cur_k_tile.dtype)
        for tb_i in nl.affine_range(tiled_block_size):
            cur_k_tile[:, nl.ds(load_idx * B_P_SIZE * tiled_block_size + tb_i * B_P_SIZE, B_P_SIZE)] = nl.transpose(loaded[:, nl.ds(tb_i * B_D_SIZE, B_D_SIZE)])
    for load_idx in nl.affine_range(num_loads):
        loaded = nl.load(kv_cache[1, block_tables[load_idx, i_p, large_k_tile_idx], i_f])
        if cur_v_tile.dtype != loaded.dtype:
            loaded = nl.copy(loaded, dtype=cur_v_tile.dtype)
        i_p = nl.arange(B_P_SIZE)[:, None]
        i_f = nl.arange(tiled_block_size * B_D_SIZE)[None, :]
        cur_v_tile[:, nl.ds(load_idx * tiled_block_size * B_D_SIZE, tiled_block_size * B_D_SIZE)] = loaded
...
def transpose_p_local(p_local_transposed, p_local, LARGE_TILE_SZ, B_F_SIZE=512):
    for i in nl.affine_range(LARGE_TILE_SZ // B_F_SIZE):
        ...
        p_local_transposed[:, nl.ds(i * B_F_SIZE, B_F_SIZE)] = nl.copy(p_local_t_tmp, dtype=p_local_transposed.dtype)

@nki.jit
def _flash_attention_core(q_local_tile, k, v, o_buffer, l_buffer, m_buffer, kernel_dtype, acc_type, tile_mask, use_causal_mask, q_tile_idx=None, initialize=False, LARGE_TILE_SZ=2048, B_P_SIZE=128, B_F_SIZE=512, B_D_SIZE=128, qk_res_buffer=None):
    """
    ...
    for k_i in nl.affine_range(num_k_tile_per_large_tile):
        ...
        if multiplication_required_selection:
            qk_psum = nl.ndarray((par_dim(B_P_SIZE), B_F_SIZE), dtype=np.float32, buffer=nl.psum)
            qk_psum[:, :] = nl.matmul(q_local_tile, k[:, k_i_b_f_slice], transpose_x=True)
            qk_res_buf[:, k_i_b_f_slice] = nl.where(tile_mask[:, k_i_b_f_slice], qk_psum[:, nl.ds(0, B_F_SIZE)], -9984.0, dtype=acc_type)
        else:
            qk_res_buf[:, k_i_b_f_slice] = -9984.0
        max_local[:, k_i] = nisa.tensor_reduce(np.max, qk_res_buf[:, k_i_b_f_slice], axis=(1,), dtype=acc_type, negate=False)
    ...
    if initialize:
        ...
        alpha = nisa.activation(np.exp, m_previous, bias=-1 * m_current, scale=1.0)
    ...
    for k_r_i in nl.affine_range(LARGE_TILE_SZ // REDUCTION_TILE):
        ...
        p_local[:, k_r_i_reduce_slice] = nisa.activation_reduce(np.exp, qk_res_buf[:, k_r_i_reduce_slice], bias=-1 * m_current, scale=1.0, reduce_op=nl.add, reduce_res=p_partial_sum[:, k_r_i], dtype=kernel_dtype)
    ...
    transpose_p_local(p_local_transposed=p_local_transposed, p_local=p_local, LARGE_TILE_SZ=LARGE_TILE_SZ, B_F_SIZE=B_F_SIZE)
    pv_psum = nl.zeros((par_dim(B_P_SIZE), B_D_SIZE), dtype=np.float32, buffer=nl.psum)
    for k_i in nl.affine_range(LARGE_TILE_SZ // B_P_SIZE):
        pv_psum[:, :] += nl.matmul(p_local_transposed[:, nl.ds(k_i * B_P_SIZE, B_P_SIZE)], v[:, nl.ds(k_i * B_D_SIZE, B_D_SIZE)], transpose_x=True)
    if initialize:
        ...
        l_prev = l_buffer[:, 0]
        l_exp = nl.add(nl.exp(nl.subtract(l_prev, m_current)), ps)
...
def load_v_tile(v_hbm_tile, cur_v_tile, large_tile_idx, v_i, LARGE_TILE_SZ):
    ...
    loaded = nl.load(v_hbm_tile[nl.ds(large_tile_idx * LARGE_TILE_SZ + B_P_SIZE * v_i, B_P_SIZE), :])
    if cur_v_tile.dtype != loaded.dtype:
        loaded = nl.copy(loaded, dtype=cur_v_tile.dtype)
    cur_v_tile[:, nl.ds(v_i * B_D_SIZE, B_D_SIZE)] = loaded

@nki.jit
def flash_paged_attention(query, key, value, kv_cache, block_tables, mask, softmax_scale=None, mixed_precision=True, LARGE_TILE_SZ=2048, return_debug_tensors=False):
    """
    ...
    q_h_per_k_h = h // k_h
    assert b == 1, f'invalid batch size b={b!r}'
    assert d <= 128, f' we do not support head_dim > 128, got head dim d={d!r}'
    cache_shape = (2, num_blocks, k_h, block_size, d)
    assert tuple(kv_cache.shape) == cache_shape, f'kv_cache.shape={kv_cache.shape!r} mismatch, expect {cache_shape}'
    assert key is None or tuple(key.shape) == (1, k_h, d, seqlen_q), f'key shape {key.shape} mismatch!'
    assert value is None or tuple(value.shape) == (1, k_h, seqlen_q, d), f'value shape {value.shape} mismatch!'
    assert nl.program_ndim() == 2, f'Expect spmd grid with 2 dimensions, got {nl.program_ndim()} instead!'
    batch_id = nl.program_id(axis=0)
    head_id = nl.program_id(axis=1)
    num_active_blocks, = block_tables.shape
    context_kv_len = num_active_blocks * block_size
    assert LARGE_TILE_SZ % B_F_SIZE == 0, f'Need LARGE_TILE_SZ={LARGE_TILE_SZ!r} to be divisible by B_F_SIZE={B_F_SIZE!r} in transpose_p'
    assert context_kv_len % LARGE_TILE_SZ == 0, f'Need context_kv_len={context_kv_len!r} to be divisible by LARGE_TILE_SZ={LARGE_TILE_SZ!r}'
    num_blocks_per_large_tile = LARGE_TILE_SZ // block_size
    assert is_power_of_2(num_blocks_per_large_tile), f'num_blocks_per_large_tile={num_blocks_per_large_tile!r} is expected of be power of 2'
    if seqlen_q > B_F_SIZE:
        MAX_REDUCTION_TILE = 2048
        if seqlen_q // 2 > MAX_REDUCTION_TILE:
            assert seqlen_q % MAX_REDUCTION_TILE == 0, f'seqlen_q={seqlen_q!r} should be divisible by MAX_REDUCTION_TILE={MAX_REDUCTION_TILE!r}'
        else:
            assert seqlen_q % B_F_SIZE == 0, f'seqlen_q={seqlen_q!r} should be divisible by B_F_SIZE={B_F_SIZE!r})'
    ...
    hbm_l_buffer, hbm_m_buffer, hbm_qk_res, qk_res_buffer = (None, None, None, None)
    if return_debug_tensors:
        ...
        qk_res_buffer = nl.zeros((n_tile_q, q_h_per_k_h, par_dim(B_P_SIZE), seqlen_q), dtype=acc_type, buffer=nl.sbuf, lazy_initialization=True)
    block_tables_sbuf = load_block_tables(block_tables_hbm=block_tables, num_tiles=num_large_k_tile, num_blocks_per_tile=num_blocks_per_large_tile)
    ...
    block_tables_sbuf = transform_block_tables_for_indirect_load(block_tables_sbuf, block_size_tiling_factor=block_size_tiling_factor, num_head=k_h, head_id=head_id)
    new_cache_shape = (2, num_blocks * k_h * block_size_tiling_factor, tiled_block_size * d)
    kv_cache = kv_cache.reshape(new_cache_shape)
    o_buffer = nl.zeros((n_tile_q, q_h_per_k_h, par_dim(B_P_SIZE), d), dtype=acc_type, buffer=nl.sbuf, lazy_initialization=True)
    l_buffer = nl.zeros((n_tile_q, q_h_per_k_h, par_dim(B_P_SIZE), 1), dtype=acc_type, buffer=nl.sbuf, lazy_initialization=True)
    m_buffer = nl.zeros((n_tile_q, q_h_per_k_h, par_dim(B_P_SIZE), 1), dtype=acc_type, buffer=nl.sbuf, lazy_initialization=True)
    for large_k_tile_idx in nl.sequential_range(0, num_large_k_tile):
        ...
        cur_v_tile = nl.ndarray((par_dim(B_P_SIZE), num_loads * tiled_block_size * B_D_SIZE), dtype=kernel_dtype)
        load_kv_tile_from_cache(cur_k_tile=cur_k_tile, cur_v_tile=cur_v_tile, kv_cache=kv_cache, block_tables=block_tables_sbuf, large_k_tile_idx=large_k_tile_idx, num_blocks_per_large_tile=num_blocks_per_large_tile, tiled_block_size=tiled_block_size, B_P_SIZE=B_P_SIZE, B_D_SIZE=B_D_SIZE)
        for i in nl.affine_range(n_tile_q):
            cur_mask = nl.load(mask[nl.ds(i * B_P_SIZE, B_P_SIZE), nl.ds(large_k_tile_idx * LARGE_TILE_SZ, LARGE_TILE_SZ)])
    ...
    if key is not None and value is not None:
        ...
        for v_i in nl.affine_range(LARGE_TILE_SZ // B_P_SIZE):
            load_v_tile(v_hbm_tile=v_hbm_tile, cur_v_tile=cur_v_tile, large_tile_idx=0, v_i=v_i, LARGE_TILE_SZ=LARGE_TILE_SZ)
        for i in nl.affine_range(n_tile_q):
            cur_mask = nl.load(mask[nl.ds(i * B_P_SIZE, B_P_SIZE), nl.ds(context_kv_len, LARGE_TILE_SZ)])
            for i_q_h in nl.affine_range(q_h_per_k_h):
                q_tile = nl.ndarray((B_D_SIZE, B_P_SIZE), dtype=kernel_dtype)
                q_hbm_tile = query[batch_id, head_id * q_h_per_k_h + i_q_h]
                q_sbuf_tile = nl.load(q_hbm_tile[:, nl.ds(i * B_P_SIZE, B_P_SIZE)])
                if q_sbuf_tile.dtype != kernel_dtype:
                    q_sbuf_tile = nl.copy(q_sbuf_tile, dtype=kernel_dtype)
                q_tile[:, :] = q_sbuf_tile * softmax_scale
                _flash_attention_core(q_local_tile=q_tile, k=cur_k_tile, v=cur_v_tile, o_buffer=o_buffer[i, i_q_h], l_buffer=l_buffer[i, i_q_h], m_buffer=m_buffer[i, i_q_h], kernel_dtype=kernel_dtype, acc_type=acc_type, tile_mask=cur_mask, use_causal_mask=True, q_tile_idx=i, initialize=False, LARGE_TILE_SZ=LARGE_TILE_SZ, B_P_SIZE=B_P_SIZE, B_F_SIZE=B_F_SIZE, B_D_SIZE=B_D_SIZE, qk_res_buffer=qk_res_buffer[i, i_q_h] if qk_res_buffer is not None else None)
    for i_q_h in nl.affine_range(q_h_per_k_h):
        for i in nl.affine_range(n_tile_q):
            out = nl.multiply(o_buffer[i, i_q_h], nl.exp(m_buffer[i, i_q_h] - l_buffer[i, i_q_h]), dtype=kernel_dtype)
            nl.store(o[batch_id, head_id * q_h_per_k_h + i_q_h, nl.ds(i * B_P_SIZE, B_P_SIZE), :], out)
            if return_debug_tensors:
                nl.store(hbm_m_buffer[batch_id, head_id * q_h_per_k_h + i_q_h, nl.ds(i * B_P_SIZE, B_P_SIZE)], m_buffer[i, i_q_h, :, :])
                nl.store(hbm_l_buffer[batch_id, head_id * q_h_per_k_h + i_q_h, nl.ds(i * B_P_SIZE, B_P_SIZE)], l_buffer[i, i_q_h])
                nl.store(hbm_qk_res[batch_id, head_id * q_h_per_k_h + i_q_h, :, :], qk_res_buffer[batch_id, i_q_h, :, :])
    if return_debug_tensors:
...
def reorder_context_mask(mask, LARGE_TILE_SZ, block_size):
    ...
    B_P_SIZE = 128
    assert LARGE_TILE_SZ >= B_P_SIZE, f'LARGE_TILE_SZ={LARGE_TILE_SZ!r} must be larger than B_P_SIZE={B_P_SIZE!r}'
    ...
    if tiled_block_size > 1:
        ...
        context_mask = context_mask.view(total_query_len, context_kv_len // LARGE_TILE_SZ, num_tiled_blocks // B_P_SIZE, B_P_SIZE, tiled_block_size)
        context_mask = context_mask.transpose(3, 4).reshape(total_query_len, context_kv_len)
        new_mask = mask[:, context_kv_len:]
        return torch.concat([context_mask, new_mask], dim=1).to(device)
    else:
        return mask

def flash_attn_varlen_nkifunc(query, key, value, kv_cache, block_table, attn_mask, n_kv_head=None, head_size=None, LARGE_TILE_SZ=2048, mixed_precision=True):
    """
    ...
    kwargs = dict(query=query, key=key, value=value, kv_cache=kv_cache, block_tables=block_table, mask=attn_mask, softmax_scale=1.0 / head_size ** 0.5, mixed_precision=mixed_precision, LARGE_TILE_SZ=LARGE_TILE_SZ)
    o = flash_paged_attention[1, n_kv_head](**kwargs)
    return o

def reshape_and_cache(key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, slot_mapping: torch.Tensor) -> None:
    """
    ...
    kv_cache.index_put_((torch.tensor([0], device=key.device), block_indices[:, None], head_indices[None, :], block_offsets[:, None]), key)
    kv_cache.index_put_((torch.tensor([1], device=key.device), block_indices[:, None], head_indices[None, :], block_offsets[:, None]), value)