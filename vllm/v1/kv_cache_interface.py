...
class KVCacheSpec:
    ...
    def merge(cls, specs: list[Self]) -> Self:
        ...
        """
        assert all((spec.type_id == specs[0].type_id for spec in specs[1:])), 'All layers in the same KV cache group must share the same type_id.'
...
class ChunkedLocalAttentionSpec(AttentionSpec):
    ...
    @property
    def type_id(self) -> str:
        return f'local_attention_{self.attention_chunk_size}_{self.block_size}_{self.page_size_bytes}'
...
class SlidingWindowSpec(AttentionSpec):
    ...
    @property
    def type_id(self) -> str:
        return f'sliding_window_{self.sliding_window}_{self.block_size}_{self.page_size_bytes}'
