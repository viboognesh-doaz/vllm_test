...
def compute_encoder_budget(model_config: 'ModelConfig', scheduler_config: 'SchedulerConfig', mm_registry: MultiModalRegistry) -> tuple[int, int]:
    ...
    if not model_config.is_multimodal_model:
        return (0, 0)
    encoder_compute_budget, encoder_cache_size = _compute_encoder_budget_multimodal(model_config, scheduler_config, mm_registry)
    return (encoder_compute_budget, encoder_cache_size)

def _compute_encoder_budget_multimodal(model_config: 'ModelConfig', scheduler_config: 'SchedulerConfig', mm_registry: MultiModalRegistry) -> tuple[int, int]:
    """Compute the encoder cache budget based on the model and scheduler 
    ...
    Returns:
        ...
        - Space budget for encoder cache size, in unit of number of tokens 
            in the input sequence.
    """
    max_tokens_by_modality_dict = mm_registry.get_max_tokens_per_item_by_nonzero_modality(model_config)
    if not max_tokens_by_modality_dict:
        logger.warning('All non-text modalities supported by the model have been explicitly disabled via limit_mm_per_prompt. Encoder cache will not be initialized.')
        return (0, 0)
    _, max_tokens_per_mm_item = max(max_tokens_by_modality_dict.items(), key=lambda item: item[1])
    if scheduler_config.disable_chunked_mm_input and max_tokens_per_mm_item > scheduler_config.max_num_batched_tokens:
        raise ValueError(f'Chunked MM input disabled but max_tokens_per_mm_item ({max_tokens_per_mm_item}) is larger than max_num_batched_tokens ({scheduler_config.max_num_batched_tokens}). Please increase max_num_batched_tokens.')
