...
from vllm.v1.kv_cache_interface import ChunkedLocalAttentionSpec, FullAttentionSpec, KVCacheConfig, KVCacheGroupSpec, KVCacheSpec, KVCacheTensor, SlidingWindowSpec
...
def max_memory_usage_bytes(vllm_config: VllmConfig, kv_cache_specs: Iterable[KVCacheSpec]) -> int:
    """
    Get the maximum memory usage in bytes for the given KV cache specs.
    """
    return sum((spec.max_memory_usage_bytes(vllm_config) for spec in kv_cache_specs))

def estimate_max_model_len(vllm_config: VllmConfig, kv_cache_spec: dict[str, KVCacheSpec], available_memory: int) -> int:
    """
...
def check_enough_kv_cache_memory(vllm_config: VllmConfig, kv_cache_spec: dict[str, KVCacheSpec], available_memory: int):
    """
    ...
    if needed_memory > available_memory:
        estimated_max_len = estimate_max_model_len(vllm_config, kv_cache_spec, available_memory)
        estimated_msg = ''
...
def get_max_concurrency_for_kv_cache_config(vllm_config: VllmConfig, kv_cache_config: KVCacheConfig) -> float:
    """
    Get the maximum concurrency for the given KV cache configuration.
    """
    num_layer_per_group = max((len(group.layer_names) for group in kv_cache_config.kv_cache_groups))
    max_memory_usage_per_request = num_layer_per_group * max_memory_usage_bytes(vllm_config, (group.kv_cache_spec for group in kv_cache_config.kv_cache_groups))
...
def get_num_blocks(vllm_config: VllmConfig, num_layers: int, available_memory: int, page_size: int) -> int:
    """
    ...
    if vllm_config.cache_config.num_gpu_blocks_override is not None:
        ...
        logger.info('Overriding num_gpu_blocks=%d with num_gpu_blocks_override=%d', num_blocks, num_gpu_blocks_override)
...
def _get_kv_cache_config_uniform_type(vllm_config: VllmConfig, kv_cache_spec: dict[str, KVCacheSpec], available_memory: int) -> KVCacheConfig:
    """
    ...
    num_blocks = get_num_blocks(vllm_config, len(kv_cache_spec), available_memory, page_size)
    ...
    logger.info('Maximum concurrency for %s tokens per request: %.2fx', max_model_len_str, max_concurrency)
...
def _get_kv_cache_config_uniform_page_size(vllm_config: VllmConfig, kv_cache_spec: dict[str, KVCacheSpec], available_memory: int) -> KVCacheConfig:
    """
    ...
    for layers in same_type_layers.values():
        ...
        if num_padding_layers != group_size:
            logger.warning('Add %d padding layers, may waste at most %.2f%% KV cache memory', num_padding_layers, num_padding_layers / len(layers) * 100)
...
def unify_hybrid_kv_cache_specs(kv_cache_spec: dict[str, KVCacheSpec]):
    ...
    def is_hybrid(kv_cache_spec: dict[str, KVCacheSpec]) -> bool:
        type_ids = set((layer_spec.type_id for layer_spec in kv_cache_spec.values()))
...
def get_kv_cache_config(vllm_config: VllmConfig, kv_cache_spec: dict[str, KVCacheSpec], available_memory: int) -> KVCacheConfig:
    """
    ...
    if is_kv_cache_type_attention_free(kv_cache_spec):
        ...
        return _get_kv_cache_config_uniform_type(vllm_config, kv_cache_spec, available_memory)
    elif is_kv_cache_page_size_uniform(kv_cache_spec):
        return _get_kv_cache_config_uniform_page_size(vllm_config, kv_cache_spec, available_memory)
