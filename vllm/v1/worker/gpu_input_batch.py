...
from vllm.v1.sample.logits_processor import BatchUpdateBuilder, MoveDirectionality, init_builtin_logitsprocs
...
class InputBatch:
    ...
    def __init__(self, max_num_reqs: int, max_model_len: int, max_num_batched_tokens: int, device: torch.device, pin_memory: bool, vocab_size: int, block_sizes: list[int], is_spec_decode: bool=False):
        ...
        self.token_ids_cpu_tensor = torch.zeros((max_num_reqs, max_model_len), device='cpu', dtype=torch.int32, pin_memory=False)
        self.token_ids_cpu = self.token_ids_cpu_tensor.numpy()
        self.num_tokens = np.zeros(max_num_reqs, dtype=np.int32)
        self.num_tokens_no_spec = np.zeros(max_num_reqs, dtype=np.int32)
        self.num_prompt_tokens = np.zeros(max_num_reqs, dtype=np.int32)
        self.num_computed_tokens_cpu_tensor = torch.zeros((max_num_reqs,), device='cpu', dtype=torch.int32, pin_memory=pin_memory)
        self.num_computed_tokens_cpu = self.num_computed_tokens_cpu_tensor.numpy()
        self.block_table = MultiGroupBlockTable(max_num_reqs=max_num_reqs, max_model_len=max_model_len, max_num_batched_tokens=max_num_batched_tokens, pin_memory=pin_memory, device=device, block_sizes=block_sizes)
        ...
        self.logitsprocs = init_builtin_logitsprocs(pin_memory_available=pin_memory, max_num_reqs=max_num_reqs + 1, device=device)
    ...
    def _register_add_request(self, request: 'CachedRequestState') -> int:
        ...
        params = request.sampling_params if request.sampling_params else request.pooling_params
        self.batch_update_builder.added.append((req_index, params, request.output_token_ids))
        return req_index
    ...
    def swap_states(self, i1: int, i2: int) -> None:
        ...
        self.req_output_token_ids[i1], self.req_output_token_ids[i2] = (self.req_output_token_ids[i2], self.req_output_token_ids[i1])
        assert old_id_i1 is not None and old_id_i2 is not None
        self.req_id_to_index[old_id_i1], self.req_id_to_index[old_id_i2] = (self.req_id_to_index[old_id_i2], self.req_id_to_index[old_id_i1])
        self.num_tokens[i1], self.num_tokens[i2] = (self.num_tokens[i2], self.num_tokens[i1])
        self.num_tokens_no_spec[i1], self.num_tokens_no_spec[i2] = (self.num_tokens_no_spec[i2], self.num_tokens_no_spec[i1])
        self.num_prompt_tokens[i1], self.num_prompt_tokens[i2] = (self.num_prompt_tokens[i2], self.num_prompt_tokens[i1])
        self.num_computed_tokens_cpu[i1], self.num_computed_tokens_cpu[i2] = (self.num_computed_tokens_cpu[i2], self.num_computed_tokens_cpu[i1])
        ...
        self.frequency_penalties_cpu[i1], self.frequency_penalties_cpu[i2] = (self.frequency_penalties_cpu[i2], self.frequency_penalties_cpu[i1])
        self.presence_penalties_cpu[i1], self.presence_penalties_cpu[i2] = (self.presence_penalties_cpu[i2], self.presence_penalties_cpu[i1])
        self.repetition_penalties_cpu[i1], self.repetition_penalties_cpu[i2] = (self.repetition_penalties_cpu[i2], self.repetition_penalties_cpu[i1])
        tmp = self.token_ids_cpu[i1, ...].copy()
        self.token_ids_cpu[i1, ...] = self.token_ids_cpu[i2, ...]
        self.token_ids_cpu[i2, ...] = tmp
        swap_dict_values(self.generators, i1, i2)
        swap_dict_values(self.bad_words_token_ids, i1, i2)
        self.request_lora_mapping[i1], self.request_lora_mapping[i2] = (self.request_lora_mapping[i2], self.request_lora_mapping[i1])
        if self.allowed_token_ids_mask_cpu_tensor is not None:
            self.allowed_token_ids_mask_cpu_tensor[i1], self.allowed_token_ids_mask_cpu_tensor[i2] = (self.allowed_token_ids_mask_cpu_tensor[i2], self.allowed_token_ids_mask_cpu_tensor[i1])
    ...
    def condense(self) -> None:
        ...
        while empty_req_indices:
            ...
            if self.allowed_token_ids_mask_cpu_tensor is not None:
                self.allowed_token_ids_mask_cpu_tensor[empty_index] = self.allowed_token_ids_mask_cpu_tensor[last_req_index]
    ...
    def _make_sampling_metadata(self) -> SamplingMetadata:
        ...
        if not self.no_penalties:
            ...
            copy_slice(self.repetition_penalties_cpu_tensor, self.repetition_penalties, num_reqs)
    ...
    def pooling_metadata(self) -> PoolingMetadata:
        ...
        return PoolingMetadata(prompt_lens=torch.from_numpy(self.num_prompt_tokens[:self.num_reqs]).to(self.device), prompt_token_ids=self.sampling_metadata.prompt_token_ids, pooling_params=pooling_params)

    def _make_prompt_token_ids_tensor(self) -> torch.Tensor:
        max_prompt_len = self.num_prompt_tokens[:self.num_reqs].max()
        prompt_token_ids_cpu_tensor = torch.empty((self.num_reqs, max_prompt_len), device='cpu', dtype=torch.int64, pin_memory=self.pin_memory)
    ...
    def make_lora_inputs(self, num_scheduled_tokens: np.ndarray) -> tuple[tuple[int, ...], tuple[int, ...], set[LoRARequest]]:
        """
    ...
    def no_penalties(self) -> bool:
        return len(self.presence_penalties_reqs) == 0 and len(self.frequency_penalties_reqs) == 0 and (len(self.repetition_penalties_reqs) == 0)
