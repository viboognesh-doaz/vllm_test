...
class CPUWorker(Worker):
    ...
    def __init__(self, vllm_config: VllmConfig, local_rank: int, rank: int, distributed_init_method: str, is_driver_worker: bool=False):
        super().__init__(vllm_config, local_rank, rank, distributed_init_method, is_driver_worker=is_driver_worker)
    ...
    def determine_available_memory(self) -> int:
        return self.cache_config.cpu_kvcache_space_bytes
    ...
    def _get_autobind_cpu_ids(self, cpu_selector: Callable[[list[LogicalCPUInfo]], list[LogicalCPUInfo]]) -> str:
        ...
        Args:
            ...
            returned.
        """
        allowed_numa_nodes, logical_cpu_list = CpuPlatform.get_allowed_cpu_memory_node_list()
        assert len(allowed_numa_nodes) >= self.parallel_config.world_size, f'No enough allowed NUMA nodes to bind threads of {self.parallel_config.world_size} CPUWorkers. Allowed NUMA nodes are {allowed_numa_nodes}. Please try to bind threads manually.'
        ...
        assert len(logical_cpu_list) > reserve_cpu_num, f'VLLM_CPU_NUM_OF_RESERVED_CPU ({reserve_cpu_num}) should less than {len(logical_cpu_list)}.'
        if reserve_cpu_num != 0:
            logical_cpu_list = logical_cpu_list[:-reserve_cpu_num]
        logger.info('auto thread-binding list (id, physical core): %s', [(x.id, x.physical_core) for x in logical_cpu_list])
