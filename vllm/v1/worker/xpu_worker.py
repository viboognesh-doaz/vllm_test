...
class XPUWorker(Worker):
    ...
    def __init__(self, vllm_config: VllmConfig, local_rank: int, rank: int, distributed_init_method: str, is_driver_worker: bool=False):
        ...
        if envs.VLLM_TORCH_PROFILER_DIR:
            ...
            self.profiler = torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.XPU], with_stack=True, on_trace_ready=torch.profiler.tensorboard_trace_handler(torch_profiler_trace_dir, use_gzip=True))
        else:
            self.profiler = None
    ...
    def determine_available_memory(self) -> int:
        ...
        msg = f'Before memory profiling run, total GPU memory: {total_gpu_memory / 1024 ** 2:.2f} MB, model load takes {current_allocated_bytes / 1024 ** 2:.2f} MB, free gpu memory is {free_gpu_memory / 1024 ** 2:.2f} MB.'
        logger.info(msg)
        self.model_runner.profile_run()
        free_gpu_memory, _ = self.xpu_get_mem_info()
        assert self.init_gpu_memory > free_gpu_memory, f'Error in memory profiling. Initial free memory {self.init_gpu_memory}, current free memory {free_gpu_memory}. This happens when the GPU memory was not properly cleaned up before initializing the vLLM instance.'
        ...
        msg = f'After memory profiling run, peak memory usage is {peak_memory / 1024 ** 2:.2f} MB,torch mem is {torch_allocated_bytes / 1024 ** 2:.2f} MB, non-torch mem is {non_torch_allocations / 1024 ** 2:.2f} MB, free gpu memory is {free_gpu_memory / 1024 ** 2:.2f} MB.'
        logger.info(msg)
    ...
    def init_device(self):
        ...
        self.model_runner = XPUModelRunner(self.vllm_config, self.device)