...
from vllm.v1.structured_output.utils import OutlinesVocabulary, get_outlines_cache, get_outlines_vocabulary
import ast
import importlib
import json
...
class OutlinesBackend(StructuredOutputBackend):
    ...
    def compile_grammar(self, request_type: StructuredOutputOptions, grammar_spec: str) -> StructuredOutputGrammar:
        ...
        max_rollback_tokens = self.vllm_config.speculative_config.num_speculative_tokens if self.vllm_config.speculative_config is not None else 0
        return OutlinesGrammar(vocab_size=self.vocab_size, guide=oc.Guide(index, max_rollback=max_rollback_tokens))
...
def validate_regex_is_buildable(pattern: str) -> None:
    ...
    try:
        ...
        raise ValueError(f'Regex uses unsupported feature for guided decoding: {e}. Only basic matching constructs are supported—lookarounds, backreferences, and unicode boundaries are not.') from e
    if _prefix_needs_context(parsed):
        raise ValueError(f'Regex does not have a anchored universal start stateThis means that the Regex uses anchors (^) or look-arounds in a way which requires context before any token is matched.Guided decoding needs regexes that can match without needing that context. Try rewriting the pattern without using these constructs. Pattern:\n{pattern}')