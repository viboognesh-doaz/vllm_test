...
class GuidanceBackend(StructuredOutputBackend):
    ...
    def __post_init__(self):
        ...
        self.disable_additional_properties = self.vllm_config.decoding_config.disable_additional_properties
    ...
    def compile_grammar(self, request_type: StructuredOutputOptions, grammar_spec: str) -> StructuredOutputGrammar:
        self.serialized_grammar = serialize_guidance_grammar(request_type, grammar_spec, self.disable_any_whitespace, self.disable_additional_properties)
        ll_matcher = llguidance.LLMatcher(self.ll_tokenizer, self.serialized_grammar, log_level=int(os.environ.get('LLGUIDANCE_LOG_LEVEL', '1')))
        r = GuidanceGrammar(ll_matcher=ll_matcher, ll_tokenizer=self.ll_tokenizer, vocab_size=self.vocab_size)
        r.check_error()
        return r
...
def serialize_guidance_grammar(request_type: StructuredOutputOptions, grammar_spec: Union[str, dict[str, Any]], disable_any_whitespace: bool=False, disable_additional_properties: bool=False) -> str:

    def _process_schema(grammar_spec: Union[str, dict[str, Any]]) -> str:
        if disable_additional_properties:
            grammar_spec = process_for_additional_properties(grammar_spec)
        return llguidance.LLMatcher.grammar_from_json_schema(grammar_spec, defaults={'whitespace_flexible': not disable_any_whitespace})
    if request_type == StructuredOutputOptions.JSON:
        return _process_schema(grammar_spec)
    elif request_type == StructuredOutputOptions.JSON_OBJECT:
        return llguidance.LLMatcher.grammar_from_json_schema('{"type": "object"}', defaults={'whitespace_flexible': not disable_any_whitespace})
    else:
        if request_type == StructuredOutputOptions.REGEX:
            ...
            for s in s_tag['structures']:
                ...
                tags.append(llguidance.StructTag(trigger=trig, begin=s['begin'], grammar=_process_schema(s['schema']), end=s['end']))
            if not tags:
...
def validate_guidance_grammar(sampling_params: SamplingParams, tokenizer: Optional[llguidance.LLTokenizer]=None) -> None:
