...
from vllm.v1.engine import EngineCoreOutputs, EngineCoreRequest, EngineCoreRequestType, ReconfigureDistributedRequest, ReconfigureRankType, UtilityOutput
from vllm.v1.engine.coordinator import DPCoordinator
from vllm.v1.engine.core import EngineCore, EngineCoreProc
from vllm.v1.engine.exceptions import EngineDeadError
from vllm.v1.engine.utils import CoreEngineActorManager, CoreEngineProcManager, launch_core_engines
...
class EngineCoreClient(ABC):
    ...
    * AsyncMPClient: ZMQ + background proc EngineCore w/ asyncio (for AsyncLLM)
    ...
    @staticmethod
    def make_client(multiprocess_mode: bool, asyncio_mode: bool, vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool) -> 'EngineCoreClient':
        if asyncio_mode and (not multiprocess_mode):
            raise NotImplementedError('Running EngineCore in asyncio without multiprocessing is not currently supported.')
    ...
    @staticmethod
    def make_async_mp_client(vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None, client_index: int=0) -> 'MPClient':
        parallel_config = vllm_config.parallel_config
        client_args = (vllm_config, executor_class, log_stats, client_addresses, client_index)
    ...
    async def save_sharded_state_async(self, path: str, pattern: Optional[str]=None, max_size: Optional[int]=None) -> None:
        raise NotImplementedError

    async def collective_rpc_async(self, method: Union[str, Callable[..., _R]], timeout: Optional[float]=None, args: tuple=(), kwargs: Optional[dict[str, Any]]=None) -> list[_R]:
        raise NotImplementedError
...
class BackgroundResources:
    ...
    engine_manager: Optional[Union[CoreEngineProcManager, CoreEngineActorManager]] = None
    ...
    def __call__(self):
        ...
        for socket in (self.output_socket, self.input_socket, self.first_req_send_socket):
...
class MPClient(EngineCoreClient):
    ...
    def __init__(self, asyncio_mode: bool, vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None):
        ...
        try:
            ...
            engine_ranks = [dp_rank] if offline_mode or external_dp_lb else range(dp_size)
            assert parallel_config.data_parallel_size_local <= len(engine_ranks)
            self.core_engines: list[EngineIdentity] = [index.to_bytes(2, 'little') for index in engine_ranks]
            identities = set(self.core_engines)
            sync_input_socket = zmq.Socket.shadow(self.input_socket)
            while identities:
                if not sync_input_socket.poll(timeout=600000):
...
class SyncMPClient(MPClient):
    ...
    def __init__(self, vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool):
        super().__init__(asyncio_mode=False, vllm_config=vllm_config, executor_class=executor_class, log_stats=log_stats)
...
class AsyncMPClient(MPClient):
    ...
    def __init__(self, vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None, client_index: int=0):
        super().__init__(asyncio_mode=True, vllm_config=vllm_config, executor_class=executor_class, log_stats=log_stats, client_addresses=client_addresses)
    ...
    def _ensure_output_queue_task(self):
        ...
        utility_results = self.utility_results
        outputs_queue = self.outputs_queue
        output_handler: Optional[Callable[[AsyncMPClient, EngineCoreOutputs], Awaitable[None]]] = getattr(self.__class__, 'process_engine_outputs', None)
    ...
    def _send_input(self, request_type: EngineCoreRequestType, request: Any, engine: Optional[EngineIdentity]=None) -> Awaitable[Any]:
        if engine is None:
    ...
    async def _call_utility_async(self, method: str, *args, engine: EngineIdentity) -> Any:
        ...
        self.utility_results[call_id] = future
        message = (EngineCoreRequestType.UTILITY.value, *self.encoder.encode((self.client_index, call_id, method, args)))
...
class DPAsyncMPClient(AsyncMPClient):
    ...
    def __init__(self, vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None, client_index: int=0):
        self.current_wave = 0
        super().__init__(vllm_config, executor_class, log_stats, client_addresses, client_index)
        self.lb_engines: list[list[int]] = []
        self.first_req_sock_addr = get_open_zmq_inproc_path()
        self.first_req_send_socket = self.resources.first_req_send_socket = make_zmq_socket(self.ctx, self.first_req_sock_addr, zmq.PAIR, bind=True)
        try:
            asyncio.get_running_loop()
    ...
    def _ensure_stats_update_task(self):
        ...
        async def run_engine_stats_update_task():
            with make_zmq_socket(self.ctx, self.stats_update_address, zmq.XSUB) as socket, make_zmq_socket(self.ctx, self.first_req_sock_addr, zmq.PAIR, bind=False) as first_req_rcv_socket:
...
class DPLBAsyncMPClient(DPAsyncMPClient):
    ...
    async def call_utility_async(self, method: str, *args) -> Any:
        return (await asyncio.gather(*[self._call_utility_async(method, *args, engine=engine) for engine in self.core_engines]))[0]
    ...
    async def _send_reconfig_message(self, reconfig_request: ReconfigureDistributedRequest, engine: EngineIdentity) -> asyncio.Future:
        """Send reconfiguration message and return the result future without
        waiting for completion."""
        call_id = uuid.uuid1().int >> 64
        future = asyncio.get_running_loop().create_future()
        self.utility_results[call_id] = future
        message = (EngineCoreRequestType.UTILITY.value, *self.encoder.encode((self.client_index, call_id, 'reinitialize_distributed', (reconfig_request,))))
    ...
    async def scale_elastic_ep(self, new_data_parallel_size: int) -> None:
        ...
        assert new_data_parallel_size != cur_data_parallel_size, f'new_data_parallel_size {new_data_parallel_size} must be different from cur_data_parallel_size {cur_data_parallel_size}'
        assert self.vllm_config.parallel_config.data_parallel_backend == 'ray', 'Only ray DP backend supports scaling elastic EP'
    ...
    async def _scale_up_elastic_ep(self, cur_data_parallel_size: int, new_data_parallel_size: int) -> None:
        ...
        for engine in self.core_engines:
            reconfig_request = ReconfigureDistributedRequest(new_data_parallel_size=new_data_parallel_size, new_data_parallel_rank=ReconfigureRankType.KEEP_CURRENT_RANK, new_data_parallel_rank_local=ReconfigureRankType.KEEP_CURRENT_RANK, new_data_parallel_master_ip=self.vllm_config.parallel_config.data_parallel_master_ip, new_data_parallel_master_port=self.vllm_config.parallel_config.data_parallel_master_port)
            future = await self._send_reconfig_message(reconfig_request, engine)
            reconfig_futures.append(future)
        logger.info('All reconfigure messages sent, starting engine creation')
        assert isinstance(self.resources.engine_manager, CoreEngineActorManager)
        self.resources.engine_manager.scale_up_elastic_ep(self.vllm_config, new_data_parallel_size)
        ...
        logger.info('[Elastic EP] Scale up completed, new data parallel size: %s', new_data_parallel_size)
    ...
    async def _scale_down_elastic_ep(self, cur_data_parallel_size: int, new_data_parallel_size: int) -> None:
        ...
        self.vllm_config.parallel_config.data_parallel_size = new_data_parallel_size
        logger.info('[Elastic EP] Scale down completed, new data parallel size: %s', new_data_parallel_size)