...
class DPCoordinator:
    ...
    * Broadcasts the START_DP_WAVE message to engines to move them from paused
    ...
    def __init__(self, parallel_config: ParallelConfig):
        ...
        front_publish_address = get_engine_client_zmq_addr(local_only=not external_lb, host=host)
...
class CoordinatorProc:
    ...
    def run_coordinator(engine_count: int, front_publish_address: str, back_output_address: str, back_publish_address: str, min_stats_update_interval_ms: int=100):
        coordinator = CoordinatorProc(engine_count=engine_count, min_stats_update_interval_ms=min_stats_update_interval_ms)
        try:
    ...
    def process_input_socket(self, front_publish_address: str, back_output_address: str, back_publish_address: str):
        ...
        with make_zmq_socket(path=front_publish_address, ctx=self.ctx, socket_type=zmq.XPUB, bind=True) as publish_front, make_zmq_socket(path=back_output_address, ctx=self.ctx, socket_type=zmq.PULL, bind=True) as output_back, make_zmq_socket(path=back_publish_address, ctx=self.ctx, socket_type=zmq.XPUB, bind=True) as publish_back:
            ...
            while True:
                ...
                wait_for = self.stats_update_interval_ms if self.stats_changed else 4000
                events = poller.poll(timeout=max(0, wait_for - elapsed))
                if not events:
                    engine_req_counts_list = self._get_engine_counts()
                    to_publish = (engine_req_counts_list, self.current_wave, self.engines_running)
                ...
                if publish_front in events:
                    ...
                    if isinstance(decoded, (list, tuple)) and len(decoded) == 2 and (decoded[0] == 'SCALE_ELASTIC_EP'):
                        ...
                        if new_engine_count > current_count:
                            ...
                            self.engines_running = False
                            logger.info('DPCoordinator scaled up from %s to %s engines', current_count, new_engine_count)
                        else:
                            self.engines = self.engines[:new_engine_count]
                            logger.info('DPCoordinator scaled down from %s to %s engines', current_count, new_engine_count)
                        continue
                    ...
                    if not self.engines_running:
                        ...
                        self._send_start_wave(publish_back, self.current_wave, engine_to_exclude)
                if output_back in events:
                    ...
                    if (wave := outputs.wave_complete) is not None:
                        if self.current_wave <= wave:
                            new_wave = wave + 1
                            logger.debug('Moving DP wave from %d to %d.', self.current_wave, new_wave)
                            self.current_wave = new_wave
                            self.engines_running = False
                            self.stats_changed = True
                    elif (wave := outputs.start_wave) is not None and (wave > self.current_wave or (wave == self.current_wave and (not self.engines_running))):
                        logger.debug('Starting wave %d after notification of stale wave request from engine.', wave)
