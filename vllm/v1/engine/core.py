...
class EngineCore:
    ...
    def __init__(self, vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool, executor_fail_callback: Optional[Callable]=None):
        ...
        if Scheduler is not V1Scheduler:
            logger.warning('Using configured V1 scheduler class %s. This scheduler interface is not public and compatibility may not be maintained.', vllm_config.scheduler_config.scheduler_cls)
        self.scheduler: SchedulerInterface = Scheduler(vllm_config=vllm_config, kv_cache_config=kv_cache_config, structured_output_manager=self.structured_output_manager, include_finished_set=vllm_config.parallel_config.data_parallel_size > 1, log_stats=self.log_stats)
        self.mm_input_cache_server = MirroredProcessingCache(vllm_config.model_config)
    ...
    def _initialize_kv_caches(self, vllm_config: VllmConfig) -> tuple[int, int, KVCacheConfig]:
        ...
        kv_cache_configs = [get_kv_cache_config(vllm_config, kv_cache_spec_one_worker, available_gpu_memory_one_worker) for kv_cache_spec_one_worker, available_gpu_memory_one_worker in zip(kv_cache_specs, available_gpu_memory)]
    ...
    def add_request(self, request: EngineCoreRequest):
        ...
        if (pooling_params := request.pooling_params):
            supported_pooling_tasks = self.model_executor.supported_pooling_tasks
...
class EngineCoreProc(EngineCore):
    ...
    def __init__(self, vllm_config: VllmConfig, local_client: bool, handshake_address: str, executor_class: type[Executor], log_stats: bool, client_handshake_address: Optional[str]=None, engine_index: int=0):
        ...
        with self._perform_handshakes(handshake_address, identity, local_client, vllm_config, client_handshake_address) as addresses:
            self.client_count = len(addresses.outputs)
            self.has_coordinator = addresses.coordinator_output is not None
            self.frontend_stats_publish_address = addresses.frontend_stats_publish_address
            self.publish_dp_lb_stats = self.has_coordinator and (not vllm_config.parallel_config.data_parallel_external_lb)
        ...
        threading.Thread(target=self.process_input_sockets, args=(addresses.inputs, addresses.coordinator_input, identity), daemon=True).start()
    ...
    @contextmanager
    def _perform_handshakes(self, handshake_address: str, identity: bytes, local_client: bool, vllm_config: VllmConfig, client_handshake_address: Optional[str]) -> Generator[EngineZmqAddresses, None, None]:
        """
        ...
        handshake = self._perform_handshake(input_ctx, handshake_address, identity, is_local, vllm_config, vllm_config.parallel_config)
        if client_handshake_address is None:
            with handshake as addresses:
                yield addresses
        else:
            local_handshake = self._perform_handshake(input_ctx, client_handshake_address, identity, local_client, vllm_config)
    ...
    @contextmanager
    def _perform_handshake(self, ctx: zmq.Context, handshake_address: str, identity: bytes, local_client: bool, vllm_config: VllmConfig, parallel_config_to_update: Optional[ParallelConfig]=None) -> Generator[EngineZmqAddresses, None, None]:
        with make_zmq_socket(ctx, handshake_address, zmq.DEALER, identity=identity, linger=5000, bind=False) as handshake_socket:
            addresses = self.startup_handshake(handshake_socket, local_client, parallel_config_to_update)
            yield addresses
            num_gpu_blocks = vllm_config.cache_config.num_gpu_blocks
            dp_stats_address = self.frontend_stats_publish_address
            handshake_socket.send(msgspec.msgpack.encode({'status': 'READY', 'local': local_client, 'num_gpu_blocks': num_gpu_blocks, 'dp_stats_address': dp_stats_address}))

    @staticmethod
    def startup_handshake(handshake_socket: zmq.Socket, local_client: bool, parallel_config: Optional[ParallelConfig]=None) -> EngineZmqAddresses:
        handshake_socket.send(msgspec.msgpack.encode({'status': 'HELLO', 'local': local_client}))
    ...
    def _send_engine_dead(self):
        ...
        if self.output_thread.is_alive():
            logger.fatal('vLLM shutdown signal from EngineCore failed to send. Please report this issue.')
...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def __init__(self, vllm_config: VllmConfig, local_client: bool, handshake_address: str, executor_class: type[Executor], log_stats: bool, client_handshake_address: Optional[str]=None):
        self._decorate_logs()
        self.counter = 0
        self.current_wave = 0
        self.last_counts = (0, 0)
        dp_rank = vllm_config.parallel_config.data_parallel_rank
        super().__init__(vllm_config, local_client, handshake_address, executor_class, log_stats, client_handshake_address, dp_rank)
    ...
    def _init_data_parallel(self, vllm_config: VllmConfig):
        ...
        if vllm_config.kv_transfer_config is not None:
            ...
            logger.debug('Setting kv_transfer_config.engine_id to %s', vllm_config.kv_transfer_config.engine_id)
...
class DPEngineCoreActor(DPEngineCoreProc):
    ...
    def __init__(self, vllm_config: VllmConfig, local_client: bool, addresses: EngineZmqAddresses, executor_class: type[Executor], log_stats: bool, dp_rank: int=0, local_dp_rank: int=0):
    ...
    @contextmanager
    def _perform_handshakes(self, handshake_address: str, identity: bytes, local_client: bool, vllm_config: VllmConfig, client_handshake_address: Optional[str]):
        """
