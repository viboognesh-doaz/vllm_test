...
class CoreEngineProcManager:
    ...
    def __init__(self, target_fn: Callable, local_engine_count: int, start_index: int, local_start_index: int, vllm_config: VllmConfig, local_client: bool, handshake_address: str, executor_class: type[Executor], log_stats: bool, client_handshake_address: Optional[str]=None):
        context = get_mp_context()
        common_kwargs = {'vllm_config': vllm_config, 'local_client': local_client, 'handshake_address': handshake_address, 'executor_class': executor_class, 'log_stats': log_stats}
        ...
        for index in range(local_engine_count):
            ...
            self.processes.append(context.Process(target=target_fn, name=f'EngineCore_{global_index}', kwargs=common_kwargs | {'dp_rank': global_index, 'local_dp_rank': local_index}))
...
class CoreEngineActorManager:
    ...
    def __init__(self, vllm_config: VllmConfig, addresses: EngineZmqAddresses, executor_class: type[Executor], log_stats: bool, placement_groups: Optional[list['PlacementGroup']]=None, local_dp_ranks: Optional[list[int]]=None):
        ...
        if placement_groups is not None:
            ...
            assert len(placement_groups) == len(local_dp_ranks), 'placement_groups and local_dp_ranks must have the same length'
        ...
        for index in range(dp_size):
            ...
            actor = ray.remote(DPEngineCoreActor).options(scheduling_strategy=PlacementGroupSchedulingStrategy(placement_group=pg, placement_group_bundle_index=world_size), runtime_env=runtime_env).remote(vllm_config=dp_vllm_config, executor_class=executor_class, log_stats=log_stats, local_client=local_client, addresses=addresses, dp_rank=index, local_dp_rank=local_index)
            if local_client:
    ...
    @staticmethod
    def create_dp_placement_groups(vllm_config: VllmConfig) -> tuple[list['PlacementGroup'], list[int]]:
        """
        ...
        for node in nodes:
            ...
            if node_ip == dp_master_ip:
                assert available_engine_count >= local_engine_count, f'Not enough resources to allocate DP ranks on DP master node {node_ip}'
                for i in range(local_engine_count):
                    bundles = [{'GPU': 1.0, 'node:' + dp_master_ip: 0.001}] * world_size + [{'CPU': 1.0}]
                    pg = ray.util.placement_group(name=f'dp_rank_{len(placement_groups)}', strategy='STRICT_PACK', bundles=bundles)
    ...
    @staticmethod
    def add_dp_placement_groups(old_vllm_config: VllmConfig, new_data_parallel_size: int) -> tuple[list['PlacementGroup'], list[int]]:
        """
        ...
        for node in nodes:
            ...
            for i in range(available_engine_count):
                ...
                if node_ip == dp_master_ip:
                    ...
                    bundles = [{'GPU': 1.0}] * world_size + [{'CPU': 1.0}]
                pg = ray.util.placement_group(name=f'dp_rank_{rank}', strategy='STRICT_PACK', bundles=bundles)
                placement_groups.append(pg)
    ...
    def scale_up_elastic_ep(self, cur_vllm_config: VllmConfig, new_data_parallel_size: int) -> None:
        cur_data_parallel_size = len(self.local_engine_actors) + len(self.remote_engine_actors)
        assert new_data_parallel_size > cur_data_parallel_size, f'New data parallel size {new_data_parallel_size} must be greater than current data parallel size {cur_data_parallel_size} for scale up'
        placement_groups, local_dp_ranks = self.add_dp_placement_groups(cur_vllm_config, new_data_parallel_size)
        ...
        new_local_engines = 0
        runtime_env = RuntimeEnv(env_vars=self.env_vars_dict | {'VLLM_ELASTIC_EP_SCALE_UP_LAUNCH': '1'})
        for i, (pg, local_rank) in enumerate(zip(placement_groups, local_dp_ranks)):
            ...
            if local_client:
                new_local_engines += 1
                dp_vllm_config.parallel_config.data_parallel_size_local = cur_vllm_config.parallel_config.data_parallel_size_local + new_local_engines
            actor = ray.remote(DPEngineCoreActor).options(scheduling_strategy=PlacementGroupSchedulingStrategy(placement_group=pg, placement_group_bundle_index=world_size), runtime_env=runtime_env).remote(vllm_config=dp_vllm_config, executor_class=self.executor_class, log_stats=self.log_stats, local_client=local_client, addresses=self.addresses, dp_rank=rank, local_dp_rank=local_rank)
            if local_client:
                self.local_engine_actors.append(actor)
            else:
                self.remote_engine_actors.append(actor)
            ...
            self.placement_group_is_local.append(local_client)
        ray.get([actor.wait_for_init.remote() for actor in (self.local_engine_actors[-new_local_engines:] if new_local_engines > 0 else []) + self.remote_engine_actors[-(len(placement_groups) - new_local_engines):]])
        actors = (self.local_engine_actors[-new_local_engines:] if new_local_engines > 0 else []) + self.remote_engine_actors[-(len(placement_groups) - new_local_engines):]
        for actor in actors:
            self.run_refs.append(actor.run.remote())
    ...
    def scale_down_elastic_ep(self, cur_data_parallel_size: int, new_data_parallel_size: int) -> None:
        assert cur_data_parallel_size > new_data_parallel_size, f'cur_data_parallel_size {cur_data_parallel_size} must be greater than new_data_parallel_size {new_data_parallel_size} for scale down'
...
def launch_core_engines(vllm_config: VllmConfig, executor_class: type[Executor], log_stats: bool, num_api_servers: int=1) -> Iterator[tuple[Optional[Union[CoreEngineProcManager, CoreEngineActorManager]], Optional[DPCoordinator], EngineZmqAddresses]]:
    ...
    client_local_only = offline_mode or external_dp_lb or local_engine_count == dp_size
    addresses = EngineZmqAddresses(inputs=[get_engine_client_zmq_addr(client_local_only, host) for _ in range(num_api_servers)], outputs=[get_engine_client_zmq_addr(client_local_only, host) for _ in range(num_api_servers)])
    ...
    if parallel_config.data_parallel_backend == 'ray':
        ...
        engine_actor_manager = CoreEngineActorManager(vllm_config=vllm_config, addresses=addresses, executor_class=executor_class, log_stats=log_stats)
    ...
    with zmq_socket_ctx(local_handshake_address, zmq.ROUTER, bind=True) as handshake_socket:
        if local_engine_count:
            local_engine_manager = CoreEngineProcManager(EngineCoreProc.run_engine_core, vllm_config=vllm_config, executor_class=executor_class, log_stats=log_stats, handshake_address=handshake_address, client_handshake_address=client_handshake_address, local_client=True, local_engine_count=local_engine_count, start_index=dp_rank, local_start_index=local_start_index or 0)
        else:
            local_engine_manager = None
        yield (local_engine_manager, coordinator, addresses)
        wait_for_engine_startup(handshake_socket, addresses, engines_to_handshake, parallel_config, vllm_config.cache_config, local_engine_manager, coordinator.proc if coordinator else None)

def wait_for_engine_startup(handshake_socket: zmq.Socket, addresses: EngineZmqAddresses, core_engines: list[CoreEngine], parallel_config: ParallelConfig, cache_config: CacheConfig, proc_manager: Optional[CoreEngineProcManager], coord_process: Optional[Process]):
    ...
    while any(conn_pending) or any(start_pending):
        ...
        if not events:
            if any(conn_pending):
                logger.debug('Waiting for %d local, %d remote core engine proc(s) to connect.', *conn_pending)
            if any(start_pending):
                logger.debug('Waiting for %d local, %d remote core engine proc(s) to start.', *start_pending)
            continue
        if len(events) > 1 or events[0][0] != handshake_socket:
            finished = proc_manager.finished_procs() if proc_manager else {}
            if coord_process is not None and coord_process.exitcode is not None:
                finished[coord_process.name] = coord_process.exitcode
            raise RuntimeError(f'Engine core initialization failed. See root cause above. Failed core proc(s): {finished}')
        ...
        if local != engine.local:
            raise RuntimeError(f"{status} message from {('local' if local else 'remote')} engine {eng_index}, expected it to be {('local' if engine.local else 'remote')}")
        if status == 'HELLO' and engine.state == CoreEngineState.NEW:
            init_message = msgspec.msgpack.encode(EngineHandshakeMetadata(addresses=addresses, parallel_config={'data_parallel_master_ip': parallel_config.data_parallel_master_ip, 'data_parallel_master_port': parallel_config.data_parallel_master_port, 'data_parallel_size': parallel_config.data_parallel_size}))
            ...
            raise RuntimeError(f"Unexpected {status} message for {('local' if local else 'remote')} engine {eng_index} in {engine.state} state.")
        logger.debug('%s from %s core engine process %s.', status, 'local' if local else 'remote', eng_index)