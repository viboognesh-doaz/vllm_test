...
class Processor:
    ...
    def __init__(self, vllm_config: VllmConfig, tokenizer: TokenizerGroup, mm_registry: MultiModalRegistry=MULTIMODAL_REGISTRY):
        ...
        self.use_hash = self.mm_input_cache_client.use_cache or self.cache_config.enable_prefix_caching
    ...
    def _validate_logprobs(self, params: SamplingParams) -> None:
        ...
        if params.logprobs and params.logprobs > max_logprobs:
            raise ValueError(f'Requested sample logprobs of {params.logprobs}, which is greater than max allowed: {max_logprobs}')
        if params.prompt_logprobs and params.prompt_logprobs > max_logprobs:
            raise ValueError(f'Requested prompt logprobs of {params.prompt_logprobs}, which is greater than max allowed: {max_logprobs}')

    def _validate_sampling_params(self, params: SamplingParams, lora_request: Optional[LoRARequest]) -> None:
    ...
    def _validate_logit_bias(self, params: SamplingParams) -> None:
        ...
        if invalid_token_ids:
            raise ValueError(f'token_id(s) {invalid_token_ids} in logit_bias contain out-of-vocab token ids. Vocabulary size: {vocab_size}')

    def _validate_supported_sampling_params(self, params: SamplingParams) -> None:
        if params.best_of is not None and params.best_of > 1:
            raise ValueError('vLLM V1 does not yet support best_of.')
        if params.logits_processors:
            raise ValueError('vLLM V1 does not support per request user provided logits processors.')

    def _validate_params(self, params: Union[SamplingParams, PoolingParams], lora_request: Optional[LoRARequest]):
        """
    ...
    def _validate_structured_output(self, params: SamplingParams) -> None:
        ...
        if self.model_config.skip_tokenizer_init and params.guided_decoding:
            raise ValueError("Structured outputs requires a tokenizer so it can't be used with 'skip_tokenizer_init'")
        engine_level_backend = self.decoding_config.backend
        if params.guided_decoding.backend:
            if params.guided_decoding.backend != engine_level_backend and (not (engine_level_backend == 'auto' and params.guided_decoding.backend_was_auto)):
                raise ValueError(f"Request-level structured output backend selection is no longer supported. The request specified '{params.guided_decoding.backend}', but vLLM was initialised with '{engine_level_backend}'. This error can be resolved by removing backend selection from the request.")
        else:
            params.guided_decoding.backend = engine_level_backend
        if isinstance(params.guided_decoding.choice, list) and (not params.guided_decoding.choice):
            raise ValueError(f"Choice '{params.guided_decoding.choice}' cannot be an empty list")
    ...
    def process_inputs(self, request_id: str, prompt: PromptType, params: Union[SamplingParams, PoolingParams], arrival_time: Optional[float]=None, lora_request: Optional[LoRARequest]=None, tokenization_kwargs: Optional[dict[str, Any]]=None, trace_headers: Optional[Mapping[str, str]]=None, prompt_adapter_request: Optional[PromptAdapterRequest]=None, priority: int=0, data_parallel_rank: Optional[int]=None) -> tuple[Optional[str], EngineCoreRequest]:
        ...
        processed_inputs: ProcessorInputs = self.input_preprocessor.preprocess(prompt, tokenization_kwargs=tokenization_kwargs, lora_request=lora_request, prompt_adapter_request=prompt_adapter_request, return_mm_hashes=self.use_hash)
        current_platform.validate_request(prompt=prompt, params=params, processed_inputs=processed_inputs)
        ...
        if isinstance(params, SamplingParams):
            ...
            if sampling_params.max_tokens is None:
                sampling_params.max_tokens = self.model_config.max_model_len - len(decoder_inputs['prompt_token_ids'])
        ...
        if decoder_inputs['type'] == 'multimodal':
            ...
            sorted_item_modalities, sorted_mm_positions, sorted_mm_hashes = merge_and_sort_multimodal_metadata(decoder_inputs['mm_placeholders'], decoder_inputs['mm_hashes'] if self.use_hash else None)
            ...
            if len(unique_modalities) > 1:
                ...
                orig_sorted_mm_inputs = [MultiModalKwargs.from_items([item]) for item in decoder_mm_inputs.get_items(sorted_item_modalities[0])]
            if sorted_mm_hashes is not None:
                sorted_mm_inputs = self.mm_input_cache_client.get_and_update_p0(orig_sorted_mm_inputs, sorted_mm_hashes)
            else:
                sorted_mm_inputs = orig_sorted_mm_inputs
        return (decoder_inputs.get('prompt'), EngineCoreRequest(request_id=request_id, prompt_token_ids=decoder_inputs['prompt_token_ids'], mm_inputs=sorted_mm_inputs, mm_hashes=sorted_mm_hashes, mm_placeholders=sorted_mm_positions, sampling_params=sampling_params, pooling_params=pooling_params, eos_token_id=eos_token_id, arrival_time=arrival_time, lora_request=lora_request, cache_salt=decoder_inputs.get('cache_salt'), priority=priority, data_parallel_rank=data_parallel_rank))

    def _validate_model_inputs(self, inputs: ProcessorInputs, lora_request: Optional[LoRARequest]=None):
    ...
    def _validate_model_input(self, prompt_inputs: SingletonInputs, lora_request: Optional[LoRARequest], *, prompt_type: Literal['encoder', 'decoder']):
        ...
        if len(prompt_ids) > max_prompt_len:
            if prompt_type == 'encoder' and model_config.is_multimodal_model:
                ...
                mm_processor = mm_registry.create_processor(model_config, tokenizer=tokenizer)
