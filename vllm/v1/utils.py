...
from vllm.utils import get_open_port, get_open_zmq_ipc_path, get_tcp_uri, kill_process_tree
if TYPE_CHECKING:
...
class APIServerProcessManager:
    ...
    def __init__(self, target_server_fn: Callable, listen_address: str, sock: Any, args: argparse.Namespace, num_servers: int, input_addresses: list[str], output_addresses: list[str], stats_update_address: Optional[str]=None):
        ...
        for i, in_addr, out_addr in zip(range(num_servers), input_addresses, output_addresses):
            client_config = {'input_address': in_addr, 'output_address': out_addr, 'client_index': i}
...
def report_usage_stats(vllm_config, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT) -> None:
    """Report usage statistics if enabled."""
    if not is_usage_stats_enabled():
        return
    usage_message.report_usage(get_architecture_class_name(vllm_config.model_config), usage_context, extra_kvs={'dtype': str(vllm_config.model_config.dtype), 'tensor_parallel_size': vllm_config.parallel_config.tensor_parallel_size, 'block_size': vllm_config.cache_config.block_size, 'gpu_memory_utilization': vllm_config.cache_config.gpu_memory_utilization, 'quantization': vllm_config.model_config.quantization, 'kv_cache_dtype': str(vllm_config.cache_config.cache_dtype), 'enable_lora': bool(vllm_config.lora_config), 'enable_prompt_adapter': bool(vllm_config.prompt_adapter_config), 'enable_prefix_caching': vllm_config.cache_config.enable_prefix_caching, 'enforce_eager': vllm_config.model_config.enforce_eager, 'disable_custom_all_reduce': vllm_config.parallel_config.disable_custom_all_reduce})