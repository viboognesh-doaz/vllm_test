...
from torch.nn.attention.flex_attention import BlockMask, _mask_mod_signature, _score_mod_signature, create_block_mask, flex_attention
...
def physical_to_logical_mapping(block_table: torch.Tensor, total_blocks: Optional[int]=None) -> torch.Tensor:
    """
    ...
    logical_indices = torch.arange(max_num_blocks, device=device).unsqueeze(0).expand(max_reqs, -1)
    physical_to_logical.scatter_(-1, block_table.to(torch.int64), logical_indices)
...
class FlexAttentionMetadata:
    ...
    def get_mask_mod(self) -> _mask_mod_signature:
        ...
        def final_mask_mod(b: torch.Tensor, h: torch.Tensor, q_idx: torch.Tensor, physical_kv_idx: torch.Tensor) -> torch.Tensor:
            ...
            return torch.where(is_valid, self.logical_mask_mod(b, h, logical_q_idx, logical_kv_idx), False)
        return final_mask_mod

    def build_block_mask(self) -> BlockMask:
        assert self.mask_mod is not None
        return create_block_mask_compiled(self.mask_mod, None, None, self.num_actual_tokens, self.total_cache_tokens, device=self.block_table.device)
...
class FlexAttentionMetadataBuilder(AttentionMetadataBuilder[FlexAttentionMetadata]):
    ...
    def __init__(self, kv_cache_spec: AttentionSpec, vllm_config: VllmConfig, device: torch.device):
        ...
        self.num_heads_q = self.model_config.get_num_attention_heads(vllm_config.parallel_config)
        self.num_heads_kv = self.model_config.get_num_kv_heads(vllm_config.parallel_config)
        self.headdim = self.model_config.get_head_size()
        self.block_size = kv_cache_spec.block_size
        self.kv_cache_spec = kv_cache_spec
        self.device = device

    def build(self, common_prefix_len: int, common_attn_metadata: CommonAttentionMetadata, fast_build: bool=False) -> FlexAttentionMetadata:
...
class FlexAttentionImpl(AttentionImpl):
    ...
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[list[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: AttentionType=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[str]=None) -> None:
        ...
        if sliding_window is not None:
            raise NotImplementedError('FlexAttention does not support sliding window yet.')
        else:
    ...
    def forward(self, layer: torch.nn.Module, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: FlexAttentionMetadata, output: Optional[torch.Tensor]=None, output_scale: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        query, key_cache, value_cache = map(lambda x: self.view_as_4d(x).permute(0, 2, 1, 3), (query, key_cache, value_cache))
        ...
        if current_platform.is_cuda():
            ...
            if max_shared_memory < 144 * 1024:
                extra_kernel_options['BLOCK_M'] //= 2
                extra_kernel_options['BLOCK_N'] //= 2
        out = flex_attention_compiled(query, key_cache, value_cache, attn_metadata.score_mod, attn_metadata.block_mask, self.scale, enable_gqa=enable_gqa, kernel_options={'FORCE_USE_FLEX_ATTENTION': True, **extra_kernel_options})
