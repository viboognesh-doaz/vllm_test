...
if current_platform.is_rocm():
    ...
    @triton.jit
    def _vllm_layout_trans_kernel(k_buffer_ptr, v_buffer_ptr, k_values_ptr, v_values_ptr, b_query_lens_loc, b_seq_lens_loc, block_table, block_table_stride_0, E_DIM: tl.constexpr, BLOCK_SIZE: tl.constexpr):
    ...
    def vllm_layout_trans(b_query_lens_loc, b_seq_lens_loc, block_table, k_buffer, v_buffer, max_seq_len, total_tokens):
        ...
        _vllm_layout_trans_kernel[grid](k_buffer, v_buffer, k_values, v_values, b_query_lens_loc, b_seq_lens_loc, block_table, block_table.stride(0), E_DIM=H_KV * D, BLOCK_SIZE=BLOCK_SIZE)
        return (k_values, v_values)

    def flash_attn_varlen_func_impl(q: torch.Tensor, k_cache: torch.Tensor, v_cache: torch.Tensor, out: torch.Tensor, cu_seqlens_q: torch.Tensor, cu_seqlens_k: torch.Tensor, total_tokens: int, max_seqlen_q: int, max_seqlen_k: int, softmax_scale: float, window_size: Optional[list[int]], alibi_slopes: Optional[list[float]], block_table: torch.Tensor) -> torch.Tensor:
        k, v = vllm_layout_trans(cu_seqlens_q, cu_seqlens_k, block_table, k_cache, v_cache, max_seqlen_k, total_tokens)
        output = aiter.flash_attn_varlen_func(q=q, k=k, v=v, cu_seqlens_q=cu_seqlens_q, max_seqlen_q=max_seqlen_q, min_seqlen_q=1, cu_seqlens_k=cu_seqlens_k, max_seqlen_k=max_seqlen_k, softmax_scale=softmax_scale, causal=True, alibi_slopes=alibi_slopes, window_size=window_size, out=out)
        return output
    ...
    direct_register_custom_op('flash_attn_varlen_func', flash_attn_varlen_func_impl, ['out'], flash_attn_varlen_func_fake, dispatch_key=current_platform.dispatch_key)
...
class AiterFlashAttentionMetadataBuilder:
    ...
    def build(self, common_prefix_len: int, common_attn_metadata: CommonAttentionMetadata, fast_build: bool=False) -> 'AiterFlashAttentionMetadata':
        ...
        cu_seq_lens = torch.zeros(seq_lens.shape[0] + 1, dtype=torch.int32, device=self.device)
...
class AiterFlashAttentionImpl(AttentionImpl):
    ...
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[list[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: AttentionType=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[int]=None, use_irope: bool=False) -> None:
        ...
        if is_quantized_kv_cache(self.kv_cache_dtype):
            raise NotImplementedError('AiterFlashAttention does not support fp8 kv-cache on this device.')

    def forward(self, layer: torch.nn.Module, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: AiterFlashAttentionMetadata, output: Optional[torch.Tensor]=None, output_scale: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if self.kv_sharing_target_layer_name is None:
            torch.ops._C_cache_ops.reshape_and_cache_flash(key, value, key_cache, value_cache, attn_metadata.slot_mapping, self.kv_cache_dtype, layer._k_scale, layer._v_scale)
        ...
        if not attn_metadata.use_cascade:
            ...
            workspace_buffer = torch.empty(num_seqs * num_heads * max_num_partitions * head_size * nbyes_per_qo_elem + 2 * (num_seqs * num_heads * max_num_partitions) * 4, dtype=torch.uint8, device=output.device)
            aiter.paged_attention_v1(output[:num_actual_tokens], workspace_buffer, query[:num_actual_tokens], key_cache, value_cache, self.scale, block_table, cu_seqlens_q, seqused_k, max_seqlen_k, self.alibi_slopes, self.kv_cache_dtype, 'NHD', self.logits_soft_cap, layer._k_scale, layer._v_scale, None, _PARTITION_SIZE_ROCM)
            return output
        else:
