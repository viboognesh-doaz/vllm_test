...
class BatchUpdateBuilder:
    ...
    * self.pop_removed() and self.peek_removed() both return
    ...
    def __init__(self, removed: Optional[list[RemovedRequest]]=None, moved: Optional[list[MovedRequest]]=None, added: Optional[list[AddedRequest]]=None) -> None:
    ...
    def get_and_reset(self, batch_size: int) -> Optional[BatchUpdate]:
        ...
        if not any((self._removed, self.moved, self.added)):
            return None
        batch_update = BatchUpdate(batch_size=batch_size, removed=self._removed, moved=self.moved, added=self.added)
        self._removed = []
        self.moved = []
        self.added = []
        return batch_update
...
class LogitsProcessorManager:
    ...
    argmax_invariant: list[LogitsProcessor] = field(default_factory=list)
    non_argmax_invariant: list[LogitsProcessor] = field(default_factory=list)
...
class MinPLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, max_num_reqs: int, pin_memory: bool, device: DeviceLikeType):
        super().__init__()
        self.min_p_count: int = 0
        self.min_p_cpu_tensor = torch.zeros((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
    ...
    def update_state(self, batch_update: Optional[BatchUpdate]):
        ...
        if self.min_p_count:
            ...
            for adx, bdx, direct in batch_update.moved:
                change = (min_p_a := self.min_p_cpu[adx]) != (min_p_b := self.min_p_cpu[bdx])
                needs_update |= change
                if change:
...
class LogitBiasLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, pin_memory: bool, device: torch.device):
        ...
        self.logits_slice = (self._device_tensor([], torch.int32), self._device_tensor([], torch.int32))
    ...
    def update_state(self, batch_update: Optional[BatchUpdate]):
        ...
        for index, params, _ in batch_update.added:
            if isinstance(params, SamplingParams) and (lb := params.logit_bias):
    ...
    def _device_tensor(self, data: list, dtype: torch.dtype) -> torch.Tensor:
        return torch.tensor(data, device='cpu', dtype=dtype, pin_memory=self.pin_memory).to(device=self.device, non_blocking=True)
...
class MinTokensLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, pin_memory: bool, device: torch.device):
        ...
        self.device = device
        self.pin_memory = pin_memory
        self.logits_slice: tuple[torch.Tensor, torch.Tensor] = (self._device_tensor([], torch.int32), self._device_tensor([], torch.int32))
    ...
    def update_state(self, batch_update: Optional[BatchUpdate]):
        ...
        if batch_update:
            ...
            for index, params, output_tok_ids in batch_update.added:
                if isinstance(params, SamplingParams) and (min_tokens := params.min_tokens) and (len(output_tok_ids) < min_tokens):
                    self.min_toks[index] = (min_tokens, output_tok_ids, params.all_stop_token_ids)
                else:
                ...
                for a_index, b_index, direct in batch_update.moved:
                    if direct == MoveDirectionality.UNIDIRECTIONAL:
                        ...
                        if a_entry is not None:
                            ...
                            needs_update = True
        if self.min_toks:
            to_remove = tuple((index for index, (min_toks, out_tok_ids, _) in self.min_toks.items() if len(out_tok_ids) >= min_toks))
            if to_remove:
                needs_update = True
...
def init_builtin_logitsprocs(pin_memory_available: bool, max_num_reqs: int, device: torch.device) -> LogitsProcessorManager:
    ...
    """
    min_tokens_logitproc = MinTokensLogitsProcessor(pin_memory=pin_memory_available, device=device)
    logit_bias_logitproc = LogitBiasLogitsProcessor(pin_memory=pin_memory_available, device=device)
    min_p_logitproc = MinPLogitsProcessor(pin_memory=pin_memory_available, device=device, max_num_reqs=max_num_reqs + 1)
    return LogitsProcessorManager(non_argmax_invariant=[min_tokens_logitproc, logit_bias_logitproc], argmax_invariant=[min_p_logitproc])