...
import torch
DEFAULT_SAMPLING_PARAMS = dict(temperature=-1.0, min_p=0.0, top_k=0, top_p=1.0)
...
class TPUSupportedSamplingMetadata:
    ...
    @classmethod
    def from_input_batch(cls, input_batch: InputBatch, padded_num_reqs: int, xla_device: torch.device, generate_params_if_all_greedy: bool=False) -> 'TPUSupportedSamplingMetadata':
        """
        ...
        return cls(temperature=input_batch.temperature_cpu_tensor[:padded_num_reqs].to(xla_device), all_greedy=input_batch.all_greedy, top_p=input_batch.top_p_cpu_tensor[:padded_num_reqs].to(xla_device), top_k=input_batch.top_k_cpu_tensor[:padded_num_reqs].to(xla_device), min_p=input_batch.min_p_cpu_tensor[:padded_num_reqs].to(xla_device), logprobs=needs_logprobs)