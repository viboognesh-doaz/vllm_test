...
class LogicalCPUInfo:
    ...
    def json_decoder(obj_dict: dict):
        ...
        if not (id is None or physical_core is None or numa_node is None):
            return LogicalCPUInfo(id=LogicalCPUInfo._int(id), physical_core=LogicalCPUInfo._int(physical_core), numa_node=LogicalCPUInfo._int(numa_node))
        else:
            return obj_dict
...
class CpuPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if (scheduler_config.chunked_prefill_enabled or cache_config.enable_prefix_caching) and cache_config.cache_dtype != 'auto':
            raise RuntimeError('Chunked-prefill and prefix-cache on the CPU backend is not compatible with FP8 KV cache.')
        ...
        if kv_cache_space >= 0:
            if kv_cache_space == 0:
                cache_config.cpu_kvcache_space_bytes = 4 * GiB_bytes
                logger.warning('Environment variable VLLM_CPU_KVCACHE_SPACE (GiB) for CPU backend is not set, using 4 by default.')
            else:
                cache_config.cpu_kvcache_space_bytes = kv_cache_space * GiB_bytes
        else:
            raise RuntimeError(f'Invalid environment variable VLLM_CPU_KVCACHE_SPACE {kv_cache_space}, expect a positive integer value.')
        parallel_config = vllm_config.parallel_config
        if parallel_config.world_size > 1 and parallel_config.distributed_executor_backend is not None and (parallel_config.distributed_executor_backend != 'mp'):
            logger.warning('%s is not supported on CPU, fallback to mp distributed executor backend.', parallel_config.distributed_executor_backend)
        ...
        if vllm_config.compilation_config.level == CompilationLevel.PIECEWISE:
            ...
            compilation_config.inductor_compile_config.update({'dce': True, 'size_asserts': False, 'nan_asserts': False, 'memory_planning': True, 'epilogue_fusion': True})
        ...
        os.environ['LOCAL_WORLD_SIZE'] = str(vllm_config.parallel_config.tensor_parallel_size)
        if model_config is not None and model_config.use_mla:
            logger.info('MLA is enabled on a non-GPU platform; forcing chunked prefill and prefix caching to be disabled.')
            vllm_config.scheduler_config.enable_chunked_prefill = False
            vllm_config.scheduler_config.chunked_prefill_enabled = False
            vllm_config.scheduler_config.max_num_batched_tokens = max(vllm_config.scheduler_config.max_model_len, DEFAULT_MAX_NUM_BATCHED_TOKENS)

    @classmethod
    def get_allowed_cpu_memory_node_list(cls) -> tuple[list[int], list[LogicalCPUInfo]]:
    ...
    def default_v1(cls, model_config) -> bool:
        ...
        return cls.supports_v1(model_config) and arch in (CpuArchEnum.X86, CpuArchEnum.POWERPC, CpuArchEnum.ARM)