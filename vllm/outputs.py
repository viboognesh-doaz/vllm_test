...
from vllm.sequence import PromptLogprobs, RequestMetrics, SampleLogprobs, SequenceGroup, SequenceGroupBase, SequenceStatus
import time
import torch
...
class CompletionOutput:
    ...
    def __repr__(self) -> str:
        return f'CompletionOutput(index={self.index}, text={self.text!r}, token_ids={self.token_ids}, cumulative_logprob={self.cumulative_logprob}, logprobs={self.logprobs}, finish_reason={self.finish_reason}, stop_reason={self.stop_reason})'
...
class RequestOutput:
    ...
    @classmethod
    def from_seq_group(cls, seq_group: SequenceGroup, use_cache: bool, seq_id_to_seq_group: dict[str, SequenceGroupBase]) -> Optional['RequestOutput']:
        ...
        if seq_group.request_id in seq_id_to_seq_group:
            ...
            return cls.from_seq_group(assembled_seq_group, use_cache, seq_id_to_seq_group)
        sampling_params = seq_group.sampling_params
        if sampling_params is None:
            raise ValueError('Sampling parameters are missing for a CompletionRequest.')
        if sampling_params.output_kind == RequestOutputKind.FINAL_ONLY and (not finished):
            return None
        if use_cache and seq_group.cached_request_output is None:
            seq_group.cached_request_output = RequestOutput(request_id='', prompt=None, prompt_token_ids=[], prompt_logprobs=None, outputs=[], finished=False)
        ...
        for i, seq in enumerate(top_n_seqs):
            ...
            num_output_tokens = 1 if isinstance(output_token_ids, int) else len(output_token_ids)
            ...
            if use_cache:
                ...
                if i >= len(cached_outputs):
                    cached_outputs.append(CompletionOutput(index=i, text='', token_ids=[], cumulative_logprob=None, logprobs=None, finish_reason=None, stop_reason=None))
                ...
                output.cumulative_logprob = seq.get_cumulative_logprob() if include_logprobs else None
                ...
                output = CompletionOutput(top_n_seqs.index(seq), output_text, [output_token_ids] if isinstance(output_token_ids, int) else output_token_ids, seq.get_cumulative_logprob() if include_logprobs else None, output_logprobs, SequenceStatus.get_finished_reason(seq.status), seq.stop_reason)
        ...
        init_kwargs = {'request_id': seq_group.request_id, 'prompt': prompt, 'prompt_token_ids': prompt_token_ids, 'prompt_logprobs': prompt_logprobs, 'outputs': outputs, 'finished': finished, 'metrics': seq_group.metrics, 'lora_request': seq_group.lora_request, 'encoder_prompt': encoder_prompt, 'encoder_prompt_token_ids': encoder_prompt_token_ids, 'num_cached_tokens': num_cached_tokens, 'multi_modal_placeholders': seq_group.multi_modal_placeholders}
        if use_cache:
            request_output = seq_group.cached_request_output
            request_output.__init__(**init_kwargs)
        else:
            request_output = cls(**init_kwargs)
        return request_output

    def __repr__(self) -> str:
        return f'RequestOutput(request_id={self.request_id}, prompt={self.prompt!r}, prompt_token_ids={self.prompt_token_ids}, encoder_prompt={self.encoder_prompt!r}, encoder_prompt_token_ids={self.encoder_prompt_token_ids}, prompt_logprobs={self.prompt_logprobs}, outputs={self.outputs}, finished={self.finished}, metrics={self.metrics}, lora_request={self.lora_request}, num_cached_tokens={self.num_cached_tokens}, multi_modal_placeholders={self.multi_modal_placeholders})'
...
class PoolingRequestOutput(Generic[_O]):
    ...
    def from_seq_group(seq_group: SequenceGroup) -> 'PoolingRequestOutput':
        ...
        prompt_token_ids = seq_group.prompt_token_ids
        finished = seq_group.is_finished()
        return PoolingRequestOutput(seq_group.request_id, output, prompt_token_ids, finished)

    def __repr__(self):
        return f'{type(self).__name__}(request_id={self.request_id!r}, outputs={self.outputs!r}, prompt_token_ids={self.prompt_token_ids}, finished={self.finished})'

class RequestOutputFactory:

    @staticmethod
    def create(seq_group: SequenceGroup, seq_id_to_seq_group: dict[str, SequenceGroupBase], use_cache: bool=False):
        if seq_group.pooled_data is not None:
            return PoolingRequestOutput.from_seq_group(seq_group)
        else:
            return RequestOutput.from_seq_group(seq_group, use_cache, seq_id_to_seq_group)
...
class EmbeddingRequestOutput(PoolingRequestOutput[EmbeddingOutput]):
    ...
    def from_base(request_output: PoolingRequestOutput):
        return EmbeddingRequestOutput(request_id=request_output.request_id, outputs=EmbeddingOutput.from_base(request_output.outputs), prompt_token_ids=request_output.prompt_token_ids, finished=request_output.finished)
...
class ClassificationRequestOutput(PoolingRequestOutput[ClassificationOutput]):
    ...
    @staticmethod
    def from_base(request_output: PoolingRequestOutput):
        return ClassificationRequestOutput(request_id=request_output.request_id, outputs=ClassificationOutput.from_base(request_output.outputs), prompt_token_ids=request_output.prompt_token_ids, finished=request_output.finished)
...
class ScoringRequestOutput(PoolingRequestOutput[ScoringOutput]):
    ...
    @staticmethod
    def from_base(request_output: PoolingRequestOutput):
        return ScoringRequestOutput(request_id=request_output.request_id, outputs=ScoringOutput.from_base(request_output.outputs), prompt_token_ids=request_output.prompt_token_ids, finished=request_output.finished)