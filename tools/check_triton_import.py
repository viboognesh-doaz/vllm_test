...
import subprocess
import sys
FORBIDDEN_IMPORT_RE = re.compile('^(from|import)\\s+triton(\\s|\\.|$)')
ALLOWED_LINES = {'from vllm.triton_utils import triton', 'from vllm.triton_utils import tl', 'from vllm.triton_utils import tl, triton'}
...
def parse_diff(diff: str) -> list[str]:
    ...
    for line in diff.splitlines():
        if line.startswith('+++ b/'):
            ...
            if match:
                current_lineno = int(match.group(1)) - 1
