...
class ConfigValidator(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node):
        decorators = [id for d in node.decorator_list if isinstance(d, ast.Name) and ((id := d.id) == 'config' or id == 'dataclass') or (isinstance(d, ast.Call) and (isinstance(d.func, ast.Name) and (id := d.func.id) == 'dataclass'))]
    ...
    for stmt in class_node.body:
        if isinstance(stmt, ast.AnnAssign):
            ...
            if isinstance(stmt.target, ast.Name):
                ...
                if isinstance(stmt.annotation, ast.Subscript) and isinstance(stmt.annotation.value, ast.Name) and (stmt.annotation.value.id == 'Union') and isinstance(stmt.annotation.slice, ast.Tuple):
                    ...
                    if len(literal_args) > 1:
                        fail(f"Field '{field_name}' in {class_node.name} must use a single Literal type. Please use 'Literal[Literal1, Literal2]' instead of 'Union[Literal1, Literal2]'.", stmt)
