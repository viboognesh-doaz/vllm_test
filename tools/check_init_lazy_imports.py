    ...
    class Visitor(ast.NodeVisitor):
        ...
        def visit_Import(self, node: ast.Import) -> None:
            ...
            for alias in node.names:
                ...
                if _is_internal(module_name) and module_name not in ALLOWED_IMPORTS:
                    violations.append((node.lineno, f"import '{module_name}' must be inside typing.TYPE_CHECKING"))
        ...
        def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
            ...
            if _is_internal(node.module, level=node.level) and module_as_written not in ALLOWED_FROM_MODULES:
                violations.append((node.lineno, f"from '{module_as_written}' import ... must be inside typing.TYPE_CHECKING"))
    Visitor().visit(tree)
    if violations:
        _fail(violations)
