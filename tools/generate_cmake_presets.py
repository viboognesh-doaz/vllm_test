...
def generate_presets(output_path='CMakeUserPresets.json'):
    ...
    if not nvcc_path:
        nvcc_path_input = input("Could not automatically find 'nvcc'. Please provide the full path to nvcc (e.g., /usr/local/cuda/bin/nvcc): ")
    ...
    if python_executable:
        ...
        python_executable_prompt = 'Could not automatically find Python executable. Please provide the full path to your Python executable for vLLM development (typically from your virtual environment, e.g., /home/user/venvs/vllm/bin/python): '
    ...
    if not os.path.isabs(python_executable) and which(python_executable):
        ...
        print(f"Warning: Python executable '{python_executable}' is not an absolute path and not found in PATH. CMake might not find it.")
    cache_variables = {'CMAKE_CUDA_COMPILER': nvcc_path, 'CMAKE_BUILD_TYPE': 'Release', 'VLLM_PYTHON_EXECUTABLE': python_executable, 'CMAKE_INSTALL_PREFIX': '${sourceDir}', 'CMAKE_CUDA_FLAGS': '', 'NVCC_THREADS': str(nvcc_threads)}
    if which('sccache'):
    ...
    configure_preset = {'name': 'release', 'binaryDir': '${sourceDir}/cmake-build-release', 'cacheVariables': cache_variables}
    if which('ninja'):
    ...
    presets = {'version': 6, 'cmakeMinimumRequired': {'major': 3, 'minor': 26, 'patch': 1}, 'configurePresets': [configure_preset], 'buildPresets': [{'name': 'release', 'configurePreset': 'release', 'jobs': cmake_jobs}]}
