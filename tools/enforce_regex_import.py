...
import regex as re
import subprocess
FORBIDDEN_PATTERNS = re.compile('^\\s*(?:import\\s+re(?:$|\\s|,)|from\\s+re\\s+import)')
ALLOWED_PATTERNS = [re.compile('^\\s*import\\s+regex\\s+as\\s+re\\s*$'), re.compile('^\\s*import\\s+regex\\s*$')]
...
def main() -> int:
    ...
    if total_violations > 0:
        ...
        print("   Also replace 'from re import ...' with 'from regex import ...'")
