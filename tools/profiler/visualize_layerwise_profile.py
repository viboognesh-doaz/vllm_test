...
def get_entries_at_depth(depth: int, entries_and_traces: list[tuple[Any, Any]], node: dict, curr_depth: int=0, trace=()):
...
def abbreviate_known_names(name: str) -> str:
    abbreviations = {'MergedColumnParallelLinear': 'MCPLinear', 'QKVParallelLinear': 'QKVPLinear', 'RowParallelLinear': 'RPLinear', 'weight=': 'w=', 'bfloat16': 'bf16', 'float16': 'f16'}
...
def attempt_to_make_names_unique(entries_and_traces):
    ...
    for name in non_unique_names:
        ...
        first_trace_difference = next((i for i, trace_eles in enumerate(zipped_traces) if not all_the_same(trace_eles)), None)
...
def group_trace_by_operations(trace_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def is_cutlass_gemm_op(op_name: str):
        return 'void cutlass::Kernel' in op_name or 'void cutlass::device_kernel' in op_name

    def is_gemm_op(op_name: str):
        if is_quant(op_name):
            return False
        return is_cutlass_gemm_op(op_name) or 'xmma_gemm' in op_name or 'gemv2T_kernel' in op_name or ('splitKreduce' in op_name) or ('s16816gemm' in op_name)
    ...
    if len(custom_ar_all_reduce_ops):
        trace_df['custom_ar_all_reduce_ops'] = trace_df[custom_ar_all_reduce_ops].agg('sum', axis=1)
...
def plot_trace_df(traces_df: pd.DataFrame, plot_metric: str, plot_title: str, output: Optional[Path]=None):
    ...
    for bar in ax.patches:
        if bar.get_height() != 0:
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() / 2 + bar.get_y(), f'{round(bar.get_height(), 2)}', ha='center', color='w', weight='bold', size=5)
...
def main(json_trace: Path, output_directory: Path, depth: int, plot_metric: str, make_names_unique: bool, top_k: int, json_nodes_to_fold: list[str]):
    ...
    def make_plot_title_suffix(profile_json: dict) -> str:
        ...
        run_type = f"Run {context['num_steps']} steps" if context['num_steps'] else f"Complete {context['complete_num_requests_per_step']} per step; Run till completion"
        return f"{context['engine_args']['model']}\nBatch={context['batch_size']}, PromptLen={context['prompt_len']}, NumGpus={context['engine_args']['tensor_parallel_size']}{(', Sparsity ' + sparsity if sparsity else '')}\nRun Type: {run_type}"
    profile_json = None
    ...
    plot_trace_df(decode_traces, plot_metric, 'decodes ' + plot_title_suffix, output_directory / Path('decode_steps.png'))
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--json-trace', type=str, required=True, help='json trace file output by                               examples/offline_inference/profiling.py')
    ...
    parser.add_argument('--fold-json-node', nargs='+', default=['Sampler', 'LogitsProcessor'], help='Do not plot the children of these nodes. Let,                               the node represent the aggregate of all its                               children')
    parser.add_argument('--plot-metric', type=str, default='cuda_time_ms', help='Metric to plot. some options are cuda_time_ms,                                 pct_cuda_time')
    parser.add_argument('--step-plot-interval', type=int, default=4, help='For every `step_plot_interval` steps, plot 1 step')
    ...
    main(Path(args.json_trace), output_directory, depth, args.plot_metric, make_names_unique, args.top_k, args.fold_json_node)