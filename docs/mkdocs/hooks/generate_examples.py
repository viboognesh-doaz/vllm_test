...
def fix_case(text: str) -> str:
    subs = {'api': 'API', 'cli': 'CLI', 'cpu': 'CPU', 'llm': 'LLM', 'mae': 'MAE', 'tpu': 'TPU', 'aqlm': 'AQLM', 'gguf': 'GGUF', 'lora': 'LoRA', 'rlhf': 'RLHF', 'vllm': 'vLLM', 'openai': 'OpenAI', 'lmcache': 'LMCache', 'multilora': 'MultiLoRA', 'mlpspeculator': 'MLPSpeculator', 'fp\\d+': lambda x: x.group(0).upper(), 'int\\d+': lambda x: x.group(0).upper()}
    for pattern, repl in subs.items():
        text = re.sub(f'\\b{pattern}\\b', repl, text, flags=re.IGNORECASE)
    return text
