...
latency_results = []
latency_column_mapping = {'test_name': 'Test name', 'gpu_type': 'GPU', 'avg_latency': 'Mean latency (ms)', 'P50': 'Median latency (ms)', 'P99': 'P99 latency (ms)'}
throughput_results = []
throughput_results_column_mapping = {'test_name': 'Test name', 'gpu_type': 'GPU', 'num_requests': '# of req.', 'total_num_tokens': 'Total # of tokens', 'elapsed_time': 'Elapsed time (s)', 'requests_per_second': 'Tput (req/s)', 'tokens_per_second': 'Tput (tok/s)'}
serving_results = []
serving_column_mapping = {'test_name': 'Test name', 'gpu_type': 'GPU', 'completed': '# of req.', 'request_throughput': 'Tput (req/s)', 'total_token_throughput': 'Total Token Tput (tok/s)', 'output_throughput': 'Output Tput (tok/s)', 'total_input_tokens': 'Total input tokens', 'total_output_tokens': 'Total output tokens', 'mean_ttft_ms': 'Mean TTFT (ms)', 'median_ttft_ms': 'Median TTFT (ms)', 'p99_ttft_ms': 'P99 TTFT (ms)', 'mean_tpot_ms': 'Mean TPOT (ms)', 'median_tpot_ms': 'Median', 'p99_tpot_ms': 'P99', 'mean_itl_ms': 'Mean ITL (ms)', 'median_itl_ms': 'Median ITL (ms)', 'p99_itl_ms': 'P99 ITL (ms)'}
...
def results_to_json(latency, throughput, serving):
    return json.dumps({'latency': latency.to_dict(), 'throughput': throughput.to_dict(), 'serving': serving.to_dict()})
...
if __name__ == '__main__':
    ...
    platform_data = {'Physical cores': [psutil.cpu_count(logical=False)], 'Total cores': [psutil.cpu_count(logical=True)], 'Total Memory': [get_size_with_unit(svmem.total)]}
    ...
    platform_md_table = tabulate(platform_results, headers='keys', tablefmt='pipe', showindex=True)
    with open(results_folder / 'benchmark_results.md', 'w') as f:
        results = read_markdown('../.buildkite/nightly-benchmarks/' + 'performance-benchmarks-descriptions.md')
        results = results.format(latency_tests_markdown_table=latency_md_table, throughput_tests_markdown_table=throughput_md_table, serving_tests_markdown_table=serving_md_table, platform_markdown_table=platform_md_table, benchmarking_results_in_json_string=processed_results_json)
        f.write(results)
