...
def ref_dynamic_per_token_quant(x: torch.tensor, quant_dtype: torch.dtype, scale_ub: Optional[torch.tensor]=None) -> tuple[torch.tensor, torch.tensor]:
    ...
    qtype_traits_max = ROCM_FP8FNUZ_MAX if current_platform.is_rocm() and current_platform.is_fp8_fnuz() else qtype_traits.max
    qtype_traits_min = -ROCM_FP8FNUZ_MAX if current_platform.is_rocm() and current_platform.is_fp8_fnuz() else qtype_traits.min
...
def ref_dynamic_per_tensor_fp8_quant(x: torch.tensor) -> tuple[torch.tensor, torch.tensor]:
    ...
    fp8_traits_max = ROCM_FP8FNUZ_MAX if current_platform.is_rocm() and current_platform.is_fp8_fnuz() else fp8_traits.max
    fp8_traits_min = -ROCM_FP8FNUZ_MAX if current_platform.is_rocm() and current_platform.is_fp8_fnuz() else fp8_traits.min
...
def native_w8a8_block_matmul(A: torch.Tensor, B: torch.Tensor, As: torch.Tensor, Bs: torch.Tensor, block_size: list[int], output_dtype: torch.dtype, compute_type: torch.dtype=torch.float32) -> torch.Tensor:
...
def native_per_token_group_quant_int8(x, group_size, eps=1e-10, dtype=torch.int8):
    ...
    amax = x_.abs().max(dim=-1, keepdim=True)[0].clamp(min=eps).to(torch.float32)
    x_s = amax / int8_max
    x_q = (x_.to(torch.float32) / x_s).round().clamp(min=int8_min, max=int8_max).to(dtype)
    x_q = x_q.reshape(x.shape)
    x_s = x_s.reshape(x.shape[:-1] + (x.shape[-1] // group_size,))
    return (x_q, x_s)
DEFAULT_BLOCK_SHAPE = [128, 128]

def per_block_cast_to_fp8(x: torch.Tensor, block_shape: list[int]=DEFAULT_BLOCK_SHAPE) -> tuple[torch.Tensor, torch.Tensor]:
...
def per_block_cast_to_int8(x: torch.Tensor, block_shape: list[int]=DEFAULT_BLOCK_SHAPE) -> tuple[torch.Tensor, torch.Tensor]:
    block_m, block_n = block_shape
    assert x.dim() == 2
    m, n = x.shape
    x_padded = torch.zeros((round_up(m, block_m), round_up(n, block_n)), dtype=x.dtype, device=x.device)
...
def dequant(t: torch.Tensor, scale: Optional[torch.Tensor], block_shape: Optional[list[int]], per_act_token_quant: bool, out_dtype: Optional[torch.dtype]=torch.float32) -> torch.Tensor:
...
def batched_dequant(t: torch.Tensor, scale: Optional[torch.Tensor], block_shape: Optional[list[int]], per_act_token_quant: bool, out_dtype: Optional[torch.dtype]=torch.float32) -> torch.Tensor:
    if scale is not None:
...
def native_batched_masked_quant_matmul(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor, num_expert_tokens: torch.Tensor, A_scale: Optional[torch.Tensor]=None, B_scale: Optional[torch.Tensor]=None, block_shape: Optional[list[int]]=None, per_act_token_quant: bool=False) -> torch.Tensor:
