...
from vllm.utils import STR_BACKEND_ENV_VAR, STR_FLASH_ATTN_VAL, STR_XFORMERS_ATTN_VAL, make_tensor_with_pad
import itertools
import pytest
import random
import torch
...
def torch_experts(a: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, topk_weight: torch.Tensor, topk_ids: torch.Tensor, global_num_experts: int=-1, expert_map: Optional[torch.Tensor]=None, w1_scale: Optional[torch.Tensor]=None, w2_scale: Optional[torch.Tensor]=None, a1_scale: Optional[torch.Tensor]=None, a2_scale: Optional[torch.Tensor]=None, quant_dtype: Optional[torch.dtype]=None, per_act_token_quant=False, block_shape: Optional[list[int]]=None, apply_router_weights_on_input: bool=False) -> torch.Tensor:
    assert global_num_experts == -1 or (global_num_experts == w1.shape[0] and expert_map is None) or (expert_map is not None and global_num_experts == expert_map.shape[0])
    M, K = a.shape
...
def torch_moe(a: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, score: torch.Tensor, topk: int, global_num_experts: int=-1, expert_map: Optional[torch.Tensor]=None) -> torch.Tensor:
