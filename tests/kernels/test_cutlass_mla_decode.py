...
if not current_platform.has_device_capability(100):
    pytest.skip(reason='Cutlass MLA Requires compute capability of 10 or above.', allow_module_level=True)

def ref_mla(out: Tensor, query: Tensor, kv_cache: Tensor, scale: float, block_tables: Tensor, seq_lens: Tensor):
...
def test_cutlass_mla_decode(dtype: torch.dtype, mean_seq_len: int, bs: int, varlen: bool, block_size: int):
    ...
    q_nope = q[:, :, :dv].clone()
    q_pe = q[:, :, dv:].clone()
    ops.cutlass_mla_decode(out_ans, q_nope, q_pe, kv_cache, seq_lens, block_table, scale)
    torch.testing.assert_close(out_ans, out_ref, atol=0.01, rtol=0.01)