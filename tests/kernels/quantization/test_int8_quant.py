...
HIDDEN_SIZES = [16, 67, 768, 5137, 8193]
HIDDEN_SIZES += list(range(1024, 1033))
...
def opcheck_int8_quant_dynamic(output, input, symmetric=True):
    ...
    if symmetric:
        ...
        azp = torch.empty((input.numel() // input.shape[-1], 1), device=input.device, dtype=torch.int32)
...
@torch.inference_mode()
def test_static_scaled_int8_quant(num_tokens: int, hidden_size: int, dtype: torch.dtype, seed: int, scale: float) -> None:
    current_platform.seed_everything(seed)
    int8_traits = torch.iinfo(torch.int8)
    x = torch.rand(num_tokens, hidden_size, dtype=dtype, device='cuda') * 1000
...
@torch.inference_mode()
def test_static_scaled_int8_azp_quant(num_tokens: int, hidden_size: int, dtype: torch.dtype, seed: int, scale: float, azp: int) -> None:
    current_platform.seed_everything(seed)
    int8_traits = torch.iinfo(torch.int8)
    x = torch.rand(num_tokens, hidden_size, dtype=dtype, device='cuda') * 1000 - 300
