...
MNK_FACTORS = [(1, 256, 128), (1, 16384, 1024), (1, 24576, 496), (16, 256, 496), (16, 16384, 128), (16, 24576, 4096), (32, 8192, 4096), (32, 16384, 4096), (33, 1024, 1024), (33, 8192, 128), (64, 2048, 496), (64, 16384, 1024), (100, 8192, 496), (128, 32768, 4096), (256, 4096, 4096), (512, 256, 1024), (512, 8192, 4096), (512, 16384, 128), (512, 24576, 128)]
...
def cutlass_fp8_gemm_helper(m: int, n: int, k: int, a_scale_group_shape: tuple, b_scale_group_shape: tuple, use_bias: bool, out_dtype: type[torch.dtype]=torch.bfloat16, device: str='cuda'):
    ...
    torch.testing.assert_close(out, baseline, rtol=0.01, atol=0.15)
    opcheck(torch.ops._C.cutlass_scaled_mm, (out, a, b, scale_a, scale_b, bias))

def cutlass_int8_gemm_helper(m: int, n: int, k: int, a_scale_group_shape: tuple, b_scale_group_shape: tuple, use_bias: bool, out_dtype: type[torch.dtype]=torch.bfloat16, device: str='cuda'):
    ...
    out = ops.cutlass_scaled_mm(a, b, scale_a, scale_b, out_dtype, bias)
    baseline = baseline_scaled_mm(a, b, scale_a, scale_b, out_dtype, bias)
    torch.testing.assert_close(out, baseline, rtol=0.1, atol=1.0)
    opcheck(torch.ops._C.cutlass_scaled_mm, (out, a, b, scale_a, scale_b, bias))
...
@pytest.mark.skipif(not current_platform.has_device_capability(90), reason='FP8 blockwise is not supported on this GPU type.')
def test_cutlass_fp8_blockwise_scale_gemm(m: int, n: int, k: int, a_scale_group_shape, b_scale_group_shape, use_bias: bool):
...
@pytest.mark.parametrize('use_bias', [True, False])
def test_cutlass_int8_gemm_output_dtype(a_scale_group_shape, b_scale_group_shape, out_dtype: type[torch.dtype], use_bias: bool):
    cutlass_int8_gemm_helper(512, 512, 512, a_scale_group_shape, b_scale_group_shape, use_bias, out_dtype=out_dtype)
...
@pytest.mark.parametrize('use_bias', [True, False])
@pytest.mark.skipif(not current_platform.has_device_capability(89), reason='FP8 is not supported on this GPU type.')
def test_cutlass_fp8_gemm_output_dtype(a_scale_group_shape, b_scale_group_shape, out_dtype: type[torch.dtype], use_bias: bool):
    cutlass_fp8_gemm_helper(512, 512, 512, a_scale_group_shape, b_scale_group_shape, use_bias, out_dtype=out_dtype)

@pytest.mark.parametrize('a_scale_group_shape,b_scale_group_shape', [((1, 128), (128, 128))])
@pytest.mark.parametrize('out_dtype', [torch.bfloat16, torch.float16])
@pytest.mark.parametrize('use_bias', [False])
@pytest.mark.skipif(not current_platform.has_device_capability(90), reason='FP8 blockwise is not supported on this GPU type.')
def test_cutlass_fp8_blockwise_scale_gemm_dtype(a_scale_group_shape, b_scale_group_shape, out_dtype: type[torch.dtype], use_bias: bool):
    cutlass_fp8_gemm_helper(512, 512, 512, a_scale_group_shape, b_scale_group_shape, use_bias, out_dtype=out_dtype)
...
def test_cutlass_fp8_gemm_devices(a_scale_group_shape, b_scale_group_shape, use_bias: bool, device: str):
    cutlass_fp8_gemm_helper(512, 512, 512, a_scale_group_shape, b_scale_group_shape, use_bias, torch.bfloat16, device)
...
def test_cutlass_int8_gemm_devices(a_scale_group_shape, b_scale_group_shape, use_bias: bool, device: str):
    cutlass_int8_gemm_helper(512, 512, 512, a_scale_group_shape, b_scale_group_shape, use_bias, out_dtype=torch.bfloat16, device=device)
...
def test_cutlass_int8_azp_bias_fold(m: int, n: int, k: int, out_dtype: torch.dtype):
    ...
    azp_a = azp_aq_i8.to(dtype=torch.float32) * scale_a
    ...
    baseline_q = (scale_a.to(device='cpu') * scale_b.to(device='cpu') * ((aq_i32 + azp_aq_i8).to(device='cpu') @ bq_i32.to(device='cpu'))).to(dtype=out_dtype, device='cuda')
    out = ops.cutlass_scaled_mm(aq_i8, bq_i8, scale_a, scale_b, out_dtype=out_dtype, bias=azp_bias[0, :])
    torch.testing.assert_close(out, baseline_dq, rtol=0.01, atol=1.0)
    torch.testing.assert_close(out, baseline_q, rtol=0.01, atol=1.0)
...
def test_cutlass_int8_azp(m: int, n: int, k: int, out_dtype: torch.dtype, use_bias: bool, azp_per_token: bool):
    ...
    azp_a = azp_aq_i8.to(dtype=torch.float32) * scale_a
    a_dq = scale_a * (aq_i32 - azp_aq_i8).to(dtype=torch.float32)
    torch.testing.assert_close(a_dq, scale_a * aq_f32 - azp_a, rtol=0.0001, atol=0.001)
    if use_bias:
    ...
    rtol = 0.01 if out_dtype == torch.bfloat16 else 0.001
    atol = 0.001
    ...
    if azp_per_token:
        ...
        opcheck(torch.ops._C.cutlass_scaled_mm_azp, (out, aq_i8, bq_i8, scale_a, scale_b, azp_with_adj_i32, None, func_bias))
...
def test_cutlass_fp8_group_gemm(num_experts: int, per_act_token: bool, per_out_ch: bool, use_bias: bool):
    ...
    ops.cutlass_moe_mm(out_tensors_stacked, a_tensors_stacked, b_tensors_stacked, a_scales_tensors_stacked, b_scales_tensors_stacked, expert_offsets[:-1], problem_sizes, ab_strides, ab_strides, c_strides, per_act_token, per_out_ch)
    for g in range(num_experts):
        ...
        print(c)
        print('*')
        torch.testing.assert_close(c, baseline, rtol=0.01, atol=0.0005)