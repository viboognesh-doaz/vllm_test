...
K = [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 6144, 8192]
N = [1, 2, 3, 4]
SEEDS = [0]
...
@pytest.mark.parametrize('n', N)
@pytest.mark.parametrize('k', K[1:] + [14336, 24576, 32768])
@pytest.mark.parametrize('m', M + [28672])
@pytest.mark.parametrize('dtype', DTYPES)
@pytest.mark.parametrize('seed', SEEDS)
@pytest.mark.skipif(not (current_platform.is_rocm() and current_platform.supports_fp8()), reason='only test for rocm fp8')
