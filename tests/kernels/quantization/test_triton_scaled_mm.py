...
device = 'cuda'
triton_scaled_mm_module = importlib.import_module('vllm.model_executor.layers.quantization.compressed_tensors.triton_scaled_mm')
triton_scaled_mm = triton_scaled_mm_module.triton_scaled_mm

def torch_scaled_mm(a: torch.Tensor, b: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, out_dtype: type[torch.dtype], bias: Optional[torch.Tensor]=None) -> torch.Tensor:
...
def test_rocm_compressed_tensors_w8a8(vllm_runner, example_prompts, model_path, max_tokens, num_logprobs):
    dtype = 'bfloat16'
...
def test_scaled_mm(M, N, K, in_dtype, out_dtype, use_scalar_scale_a, use_scalar_scale_b, use_bias):
    ...
    torch.testing.assert_close(c_check, c_actual, rtol=0.1, atol=0.1)