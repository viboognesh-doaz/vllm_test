...
from vllm.model_executor.layers.quantization.utils.machete_utils import query_machete_supported_group_sizes
...
MNK_SHAPES = [(1, 128, 128), (1, 512, 1024), (1, 4096, 4096), (1, 8192, 28672), (13, 8192, 4096), (26, 4096, 8192), (64, 4096, 4096), (64, 8192, 28672), (257, 128, 4096), (257, 4224, 4160), (257, 4096, 4096), (1024, 4096, 8192), (1024, 8192, 4096)]
...
TestTypeTuple = tuple[list[torch.dtype], ScalarType, Optional[torch.dtype], Optional[torch.dtype], bool]
TEST_TYPES = [*(TypeConfig(act_type=a_type, weight_type=w_type, output_type=None, group_scale_type=a_type, group_zero_type=None, channel_scale_type=None, token_scale_type=None) for w_type in [scalar_types.uint4b8, scalar_types.uint8b128] for a_type in [torch.float16, torch.bfloat16]), *(TypeConfig(act_type=a_type, weight_type=w_type, output_type=None, group_scale_type=a_type, group_zero_type=a_type, channel_scale_type=None, token_scale_type=None) for w_type in [scalar_types.uint4, scalar_types.uint8] for a_type in [torch.float16, torch.bfloat16]), *(TypeConfig(act_type=torch.int8, weight_type=scalar_types.uint4b8, output_type=torch.float16, group_scale_type=group_scale_type, group_zero_type=None, channel_scale_type=torch.float, token_scale_type=torch.float) for group_scale_type in [None, torch.float16]), *(TypeConfig(act_type=torch.float8_e4m3fn, weight_type=scalar_types.uint4b8, output_type=torch.float16, group_scale_type=group_scale_type, group_zero_type=None, channel_scale_type=torch.float, token_scale_type=torch.float) for group_scale_type in [None, torch.float16])]
...
def machete_quantize_and_pack(atype: torch.dtype, w: torch.Tensor, wtype: ScalarType, stype: Optional[torch.dtype], group_size: Optional[int], zero_points: bool=False):
...
def create_test_tensors(shape: tuple[int, int, int], types: TypeConfig, group_size: Optional[int], subset_stride_factor: Optional[int]=None) -> Tensors:
    m, n, k = shape
    ...
    w_ref, w_q_packed, w_s, w_zp = machete_quantize_and_pack(a.dtype, w, types.weight_type, types.group_scale_type, group_size, types.group_zero_type is not None)
    ...
    return Tensors(w_ref=w_ref, a_ref=a_ref, a=a, w_q=w_q_packed, w_g_s=w_s, w_g_zp=maybe_convert_zeropoints(w_zp, w_s), w_ch_s=w_ch_s, w_tok_s=w_tok_s)

def machete_mm_test_helper(types: TypeConfig, tensors: Tensors, group_size: Optional[int]=None, schedule: Optional[str]=None):
    ...
    output = ops.machete_mm(a=tensors.a, b_q=tensors.w_q, b_type=types.weight_type, b_group_scales=tensors.w_g_s, b_group_zeros=tensors.w_g_zp, b_group_size=group_size, b_channel_scales=tensors.w_ch_s, a_token_scales=tensors.w_tok_s, out_type=types.output_type, schedule=schedule)
    print(output)
    print(output_ref)
    atol = 1 if tensors.w_g_zp is not None else min(0.05 * math.sqrt(tensors.a.shape[1]), 1)
    rtol = 0.1 if tensors.a.element_size() >= 2 else 0.2
...
def test_machete_all_schedules(shape, types: TypeConfig):
    ...
    for group_size in group_sizes:
        ...
        print(f'MNK = {shape}')
        for schedule in ops.machete_supported_schedules(types.act_type, types.weight_type, group_scales_type=types.group_scale_type, group_zeros_type=types.group_scale_type, out_type=types.output_type):
...
def test_machete_devices(device: str):
    group_size = 128
    type_config = TypeConfig(act_type=torch.float16, weight_type=scalar_types.uint4b8, output_type=None, group_scale_type=torch.float16, group_zero_type=None, channel_scale_type=None, token_scale_type=None)
...
def test_machete_cuda_graph():
    ...
    model = MacheteLayer(b_q=w_q_packed, b_type=wtype, b_group_scales=w_s, b_group_zeros=maybe_convert_zeropoints(w_zp, w_s), b_group_size=group_size)
    ...
    output.zero_()
    g.replay()
    atol = 1 if zero_points else min(0.05 * math.sqrt(k), 1)
    torch.testing.assert_close(output, output_ref, rtol=0.1, atol=atol)