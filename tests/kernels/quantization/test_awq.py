...
@pytest.mark.skipif(not hasattr(torch.ops._C, 'awq_dequantize'), reason='AWQ is not supported on this GPU type.')
def test_awq_dequantize_opcheck(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        ...
        thx = 0
        thy = 0
        opcheck(torch.ops._C.awq_dequantize, (qweight, scales, zeros, split_k_iters, thx, thy))

@pytest.mark.skip(reason='Not working; needs investigation.')
@pytest.mark.skipif(not hasattr(torch.ops._C, 'awq_gemm'), reason='AWQ is not supported on this GPU type.')
