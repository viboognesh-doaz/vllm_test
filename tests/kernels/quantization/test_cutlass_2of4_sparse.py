...
def check_compress_decompress_invariance(dtype: torch.dtype, b: torch.Tensor, b_compressed: torch.Tensor, b_metadata: torch.Tensor):
...
def make_rand_sparse_tensors(dtype: torch.dtype, m: int, n: int, k: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
@pytest.mark.skipif(not sparse_cutlass_supported(), reason='Sparse CUTLASS is not supported on this GPU type.')
...
MNK_FACTORS = [(1, 256, 128), (1, 16384, 1024), (1, 24576, 512), (16, 256, 512), (16, 16384, 128), (16, 24576, 4096), (32, 8192, 4096), (32, 16384, 4096), (33, 1024, 1024), (33, 8192, 128), (64, 2048, 512), (64, 16384, 1024), (100, 8192, 512), (128, 32768, 4096), (256, 4096, 4096), (512, 256, 1024), (512, 8192, 4096), (512, 16384, 128), (512, 24576, 128)]
...
def test_cutlass_sparse_gemm(m: int, k: int, n: int, dtype: type[torch.dtype], use_bias: bool):
    ...
    torch.testing.assert_close(out, baseline, rtol=0.01, atol=0.3)
...
def test_cutlass_sparse_int8_gemm(m: int, n: int, k: int, per_act_token: bool, per_out_ch: bool, use_bias: bool):
    ...
    out = ops.cutlass_scaled_sparse_mm(a, b_comp, e, scale_a, scale_b, out_dtype=out_dtype, bias=bias)
    baseline = baseline_scaled_mm(a, b, scale_a, scale_b, out_dtype=out_dtype, bias=bias)
    torch.testing.assert_close(out, baseline, rtol=1.0, atol=2.0)