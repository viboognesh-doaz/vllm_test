from vllm.model_executor.layers.quantization.awq_triton import AWQ_TRITON_SUPPORTED_GROUP_SIZES, awq_dequantize_triton, awq_gemm_triton
...
def reverse_awq_order(t: torch.Tensor):
    ...
    t = t[:, reverse_order_tensor] & 15
    return t

def awq_dequantize_torch(qweight: torch.Tensor, scales: torch.Tensor, qzeros: torch.Tensor, group_size: int) -> torch.Tensor:
    if group_size == -1:
...
def test_dequantize(qweight_rows, qweight_cols, group_size):
    ...
    qweight = torch.randint(0, torch.iinfo(torch.int32).max, (qweight_rows, qweight_cols), dtype=qweight_dtype, device=device)
    scales = torch.rand(scales_rows, scales_cols, dtype=scales_dtype, device=device)
    zeros = torch.randint(0, torch.iinfo(torch.int32).max, (zeros_rows, zeros_cols), dtype=zeros_dtype, device=device)
...
def test_gemm(N, K, M, splitK, group_size):
    ...
    torch.testing.assert_close(output_triton.cpu(), output_torch.cpu(), atol=0.1, rtol=0.1)