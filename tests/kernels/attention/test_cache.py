...
def test_copy_blocks(kv_cache_factory, num_mappings: int, num_layers: int, num_heads: int, head_size: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, kv_cache_dtype: str, device: str) -> None:
    ...
    key_caches, value_caches = kv_cache_factory(num_blocks, block_size, num_layers, num_heads, head_size, kv_cache_dtype, dtype, seed, device)
    cloned_key_caches = [key_cache.clone() for key_cache in key_caches]
    cloned_value_caches = [value_cache.clone() for value_cache in value_caches]
    block_mapping_tensor = torch.tensor(block_mapping, dtype=torch.int64, device=device).view(-1, 2)
    opcheck(torch.ops._C_cache_ops.copy_blocks, (key_caches, value_caches, block_mapping_tensor), test_utils=DEFAULT_OPCHECK_TEST_UTILS, cond=head_size == HEAD_SIZES[0])
...
@pytest.mark.parametrize('kv_cache_dtype', KV_CACHE_DTYPE)
@torch.inference_mode()
def test_reshape_and_cache(kv_cache_factory, num_tokens: int, num_heads: int, head_size: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, device: str, kv_cache_dtype: str) -> None:
    if kv_cache_dtype == 'fp8' and head_size % 16:
        pytest.skip()
    current_platform.seed_everything(seed)
    torch.set_default_device(device)
    ...
    key_caches, value_caches = kv_cache_factory(num_blocks, block_size, 1, num_heads, head_size, kv_cache_dtype, dtype, seed, device)
    ...
    opcheck(torch.ops._C_cache_ops.reshape_and_cache, (key, value, key_cache, value_cache, slot_mapping, kv_cache_dtype, k_scale, v_scale), cond=head_size == HEAD_SIZES[0])
...
@torch.inference_mode()
def test_reshape_and_cache_flash(kv_cache_factory_flashinfer, num_tokens: int, num_heads: int, head_size: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, device: str, kv_cache_dtype: str, kv_cache_layout: str) -> None:
    current_platform.seed_everything(seed)
    torch.set_default_device(device)
    ...
    _, key, value = qkv.unbind(dim=1)
    key_caches, value_caches = kv_cache_factory_flashinfer(num_blocks, block_size, 1, num_heads, head_size, kv_cache_dtype, dtype, device=device, cache_layout=kv_cache_layout)
    key_cache, value_cache = (key_caches[0], value_caches[0])
    del key_caches
    del value_caches
    k_scale = (key.amax() / 64.0).to(torch.float32)
    ...
    opcheck(torch.ops._C_cache_ops.reshape_and_cache_flash, (key, value, key_cache, value_cache, slot_mapping, kv_cache_dtype, k_scale, v_scale), cond=head_size == HEAD_SIZES[0])
    ...
    if kv_cache_dtype == 'fp8':
        ...
        ops.convert_fp8(result_value_cache, value_cache_compact, v_scale.item(), kv_dtype=kv_cache_dtype)
    block_indices = torch.div(slot_mapping, block_size, rounding_mode='floor')
    block_indices_lst = block_indices.cpu().tolist()
    block_offsets = slot_mapping % block_size
    block_offsets_lst = block_offsets.cpu().tolist()
...
@pytest.mark.parametrize('kv_cache_dtype', KV_CACHE_DTYPE)
@torch.inference_mode()
def test_swap_blocks(kv_cache_factory, direction: tuple[str, str], num_mappings: int, num_heads: int, head_size: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, device: str, kv_cache_dtype: str) -> None:
    ...
    dist_key_caches, dist_value_caches = kv_cache_factory(num_blocks, block_size, 1, num_heads, head_size, kv_cache_dtype, dtype, seed, dst_device)
    src_key_caches_clone = src_key_caches[0].clone()
    src_value_caches_clone = src_value_caches[0].clone()
    do_opcheck = head_size == HEAD_SIZES[0]
    opcheck(torch.ops._C_cache_ops.swap_blocks, (src_key_caches[0], dist_key_caches[0], block_mapping_tensor), cond=do_opcheck)
    opcheck(torch.ops._C_cache_ops.swap_blocks, (src_value_caches[0], dist_value_caches[0], block_mapping_tensor), cond=do_opcheck)
...
@pytest.mark.parametrize('device', CUDA_DEVICES)
@torch.inference_mode()
def test_fp8_e4m3_conversion(num_heads: int, head_size: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, device: str) -> None:
    current_platform.seed_everything(seed)
    low = -224.0
    high = 224.0
...
def _create_mla_cache(num_blocks: int, block_size: int, entry_size: int, dtype: torch.dtype, kv_cache_dtype: str, device: str) -> torch.Tensor:
...
@pytest.mark.parametrize('kv_cache_dtype', KV_CACHE_DTYPE)
@torch.inference_mode()
def test_concat_and_cache_mla(kv_lora_rank: int, qk_rope_head_dim: int, num_tokens: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, device: str, kv_cache_dtype: str) -> None:
    current_platform.seed_everything(seed)
    torch.set_default_device(device)
    ...
    opcheck(torch.ops._C_cache_ops.concat_and_cache_mla, (kv_c, k_pe, kv_cache, slot_mapping, kv_cache_dtype, scale), test_utils=DEFAULT_OPCHECK_TEST_UTILS)
...
@pytest.mark.parametrize('kv_cache_dtype', KV_CACHE_DTYPE)
@torch.inference_mode()
def test_copy_blocks_mla(kv_lora_rank: int, qk_rope_head_dim: int, block_size: int, num_blocks: int, num_layers: int, dtype: torch.dtype, seed: int, device: str, kv_cache_dtype: str) -> None:
    current_platform.seed_everything(seed)
    torch.set_default_device(device)
    entry_size = kv_lora_rank + qk_rope_head_dim
    kv_caches = []
    ...
    opcheck(torch.ops._C_cache_ops.copy_blocks_mla, (kv_caches, block_mapping_tensor), test_utils=DEFAULT_OPCHECK_TEST_UTILS)
...
@pytest.mark.parametrize('kv_cache_dtype', KV_CACHE_DTYPE)
@torch.inference_mode()
def test_swap_blocks_mla(kv_lora_rank: int, qk_rope_head_dim: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int, device: str, kv_cache_dtype: str) -> None:
    current_platform.seed_everything(seed)
    torch.set_default_device(device)
    entry_size = kv_lora_rank + qk_rope_head_dim
    ...
    block_mapping = list(zip(src_blocks, dst_blocks))
    block_mapping_tensor = torch.tensor(block_mapping, dtype=torch.int64, device='cpu').view(-1, 2)
    opcheck(torch.ops._C_cache_ops.swap_blocks, (src_cache, dst_cache, block_mapping_tensor), test_utils=DEFAULT_OPCHECK_TEST_UTILS)
    ops.swap_blocks(src_cache, dst_cache, block_mapping_tensor)
    for src, dst in block_mapping:
        torch.testing.assert_close(src_cache_clone[src].cpu(), dst_cache[dst].cpu(), msg=f'Block {src} from src should have been swapped to block {dst} in dst_cache.')
...
@pytest.mark.parametrize('device', CUDA_DEVICES)
@torch.inference_mode()
def test_gather_cache_mla(kv_lora_rank, qk_rope_head_dim, block_size, num_blocks, max_seq_len, batch_size, dtype, kv_cache_dtype, device):
    entry_size = kv_lora_rank + qk_rope_head_dim
    src_cache = _create_mla_cache(num_blocks, block_size, entry_size, dtype, kv_cache_dtype, device)
    ...
    opcheck(torch.ops._C_cache_ops.gather_cache, (src_cache, dst, block_table, cu_seq_lens, batch_size, None), test_utils=DEFAULT_OPCHECK_TEST_UTILS)
...
@torch.inference_mode()
def test_concat_and_cache_mla_cpu(kv_lora_rank: int, qk_rope_head_dim: int, num_tokens: int, block_size: int, num_blocks: int, dtype: torch.dtype, seed: int) -> None:
    device = 'cpu'
    kv_cache_dtype = 'auto'
    current_platform.seed_everything(seed)
    torch.set_default_device(device)
