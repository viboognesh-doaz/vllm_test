...
def test_decode_attention(B, L, H_Q, H_KV, D_QK, D_V, CACHE_SIZE, PAGE_SIZE):
    ...
    attn_logits = torch.empty((B, H_Q, num_kv_splits, D_V + 1), dtype=torch.float32, device='cuda')
    decode_attention_fwd(q, k_buffer, v_buffer, o, req_to_token, b_seq_len, attn_logits, num_kv_splits, sm_scale)
    ...
    decode_attention_fwd(q, k_buffer, v_buffer, o1, req_to_page, b_seq_len, attn_logits, num_kv_splits, sm_scale, PAGE_SIZE)
