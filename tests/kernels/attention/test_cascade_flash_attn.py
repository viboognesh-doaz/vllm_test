...
from vllm.vllm_flash_attn import fa_version_unsupported_reason, flash_attn_varlen_func, is_fa_version_supported
import pytest
import torch
...
def test_merge_kernel(num_tokens: int, num_heads: tuple[int, int], head_size: int, dtype: torch.dtype):
...
CASES = [([(129, 871), (18, 280), (37, 988), (1023, 2304), (1, 257)], 256), ([(1, 1023), (1, 879), (1, 778), (1, 1777)] * 100, 512)]
...
@torch.inference_mode()
def test_cascade(seq_lens_and_common_prefix: tuple[list[tuple[int, int]], int], num_heads: tuple[int, int], head_size: int, dtype: torch.dtype, block_size: int, soft_cap: Optional[float], num_blocks: int, fa_version: int) -> None:
    torch.set_default_device('cuda')
    ...
    ref_output = flash_attn_varlen_func(q=query, k=key_cache, v=value_cache, cu_seqlens_q=cu_query_lens, seqused_k=kv_lens_tensor, max_seqlen_q=max_query_len, max_seqlen_k=max_kv_len, softmax_scale=scale, causal=True, window_size=window_size, block_table=block_tables, softcap=soft_cap if soft_cap is not None else 0)
