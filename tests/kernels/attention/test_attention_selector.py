...
DEVICE_MLA_BLOCK_SIZES = {'cuda': [16, 64], 'hip': [16, 1], 'cpu': []}
        ...
        if device == 'cpu':
            ...
            with patch('vllm.attention.selector.current_platform', CudaPlatform()):
                if use_mla:
                    ...
                    if use_v1:
                        backend = get_attn_backend(16, torch.float16, torch.float16, block_size, False, use_mla=use_mla)
                        assert backend.get_name() == 'FLEX_ATTENTION', 'Should fallback to FlexAttention if head size is not supported by FlashAttention'

@pytest.mark.parametrize('device', ['cpu', 'cuda'])
@pytest.mark.parametrize('use_v1', [True, False])
def test_fp32_fallback(device: str, use_v1: bool, monkeypatch: pytest.MonkeyPatch):
...
def test_invalid_env(use_v1: bool, monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m, patch('vllm.attention.selector.current_platform', CudaPlatform()):
        m.setenv('VLLM_USE_V1', '1' if use_v1 else '0')
