...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False, return_last_state=False, prev_state=None, final_state_out=None):
    """
    u: r(B D L)
    delta: r(B D L)
    A: c(D N) or r(D N)
...
def selective_scan_opcheck_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False, cu_seq_len=None, cache_indices=None, has_initial_state=None, ssm_states=None, pad_slot_id=PAD_SLOT_ID):
    ...
    if C.dim() == 2 and cu_seq_len is not None:
        C = C.unsqueeze(0)
    opcheck(torch.ops._C.selective_scan_fwd, (u, delta, A, B, C, D, z, delta_bias, delta_softplus, cu_seq_len, cache_indices, has_initial_state, ssm_states, pad_slot_id), test_utils=['test_schema', 'test_faketensor'])
...
def test_selective_scan(is_variable_B, is_variable_C, varBC_groups, has_D, has_z, has_delta_bias, delta_softplus, seqlen, itype, wtype, scan_chunks):
    if varBC_groups > 1 and (not is_variable_B or not is_variable_C):
        pytest.skip()
    device = 'cuda'
    rtol, atol = (0.0006, 0.002) if itype == torch.float32 else (0.003, 0.005)
    if itype == torch.bfloat16:
        rtol, atol = (0.03, 0.05)
    rtolw, atolw = (0.001, 0.001)
    if has_z:
    ...
    for c in range(scan_chunks):
        ...
        out = selective_scan_fn(u[..., chunk_start:chunk_end], state, delta[..., chunk_start:chunk_end], A, _B, _C, D, z=_z, delta_bias=delta_bias, delta_softplus=delta_softplus, has_initial_state=torch.ones(batch_size, device=u.device, dtype=torch.bool) if c > 0 else None)
        outs.append(out)
    if len(outs) > 1:
        out = torch.cat(outs, dim=-1)
    out_ref, state_ref, *rest = selective_scan_ref(u_ref, delta_ref, A_ref, B_ref, C_ref, D_ref, z=z_ref, delta_bias=delta_bias, delta_softplus=delta_softplus, return_last_state=True)
    ...
    selective_scan_opcheck_fn(u, delta, A, B, C, D, z, delta_bias=delta_bias, delta_softplus=delta_softplus, ssm_states=state)
...
def test_selective_state_update(dim, dstate, has_z, itype):
    device = 'cuda'
    rtol, atol = (0.0003, 0.001) if itype == torch.float32 else (0.005, 0.01)
    if itype == torch.bfloat16:
        rtol, atol = (0.01, 0.05)
...
def test_selective_scan_varlen(with_padding, is_variable_B, is_variable_C, varBC_groups, has_D, has_z, has_delta_bias, delta_softplus, return_last_state, seqlen, itype, wtype):
    if varBC_groups > 1 and (not is_variable_B or not is_variable_C):
        pytest.skip()
    device = 'cuda'
    rtol, atol = (0.0006, 0.002) if itype == torch.float32 else (0.003, 0.005)
    ...
    padded_state_indices = torch.concat([state_indices, torch.as_tensor([PAD_SLOT_ID] * padding, dtype=torch.int32, device=device)], dim=-1)
    has_initial_state = torch.randint(0, 2, (cumsum.shape[0] - 1,), dtype=torch.bool, device=u.device)
    out = selective_scan_fn(u, prev_state, delta, A, B, C, D, z, delta_bias, delta_softplus, cumsum, padded_state_indices, has_initial_state)
    outs_ref = []
    ...
    for i in range(len(seqlens[0])):
        ...
        if padded_state_indices[i] == PAD_SLOT_ID:
            continue
        out_ref_s, _ = selective_scan_ref(u_s, delta_s, A_ref, B_s, C_s, D_ref, z=z_s, delta_bias=delta_bias, delta_softplus=delta_softplus, return_last_state=return_last_state, prev_state=prev_state_ref[padded_state_indices[i]].unsqueeze(0) if has_initial_state[i] else None, final_state_out=prev_state_ref[padded_state_indices[i]].unsqueeze(0))
    ...
    selective_scan_opcheck_fn(u, delta, A, B, C, D, z, delta_bias, delta_softplus, cumsum, padded_state_indices, has_initial_state, prev_state)
...
def test_selective_state_update_with_batch_indices(with_padding, dim, dstate, has_z, itype):
    ...
    unused_states_bool = torch.ones(total_entries, dtype=torch.bool, device=device)
    unused_states_bool[state_indices] = False
    padded_state_indices = torch.concat([state_indices, torch.as_tensor([PAD_SLOT_ID] * padding, dtype=torch.int32, device=device)], dim=0)
    ...
    out = selective_state_update(state, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias, dt_softplus=True, state_batch_indices=padded_state_indices, pad_slot_id=PAD_SLOT_ID)
    out_ref = selective_state_update_ref(state_ref, x[:batch_size], dt[:batch_size], A, B[:batch_size], C[:batch_size], D=D, z=z[:batch_size], dt_bias=dt_bias, dt_softplus=True)
...
def test_selective_state_update_with_heads_with_batch_indices(dim, dstate, ngroups, has_z, tie_hdim, itype):
    device = 'cuda'
    rtol, atol = (0.0003, 0.001) if itype == torch.float32 else (0.005, 0.03)
    if itype == torch.bfloat16:
        rtol, atol = (0.1, 0.1)
    torch.random.manual_seed(0)
    batch_size = 3
    ...
    out = selective_state_update(state, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias, dt_softplus=True, state_batch_indices=state_indices, pad_slot_id=PAD_SLOT_ID)
    out_ref = selective_state_update_ref(state_ref, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias, dt_softplus=True)
