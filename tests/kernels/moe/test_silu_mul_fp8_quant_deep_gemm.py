from vllm.model_executor.layers.fused_moe.batched_deep_gemm_moe import silu_mul_fp8_quant_deep_gemm
from vllm.platforms import current_platform
import pytest
import torch
CASES = [(1, 1, 128, 64, 0), (1, 4, 128, 128, 0), (2, 4, 256, 128, 0), (32, 64, 256, 128, 0), (17, 31, 768, 128, 0)]
...
def test_silu_mul_fp8_quant_deep_gemm(E, T, H, group_size, seed):
    ...
    tokens_per_expert = torch.randint(low=0, high=T, size=(E,), dtype=torch.int32, device='cuda')
    y_q, y_s = silu_mul_fp8_quant_deep_gemm(y, tokens_per_expert, group_size=group_size, eps=1e-10)
    ...
    for e in range(E):
        ...
        torch.testing.assert_close(y_qe[:nt].to(torch.float32), ref_q[:nt].to(torch.float32), atol=2, rtol=0.2)