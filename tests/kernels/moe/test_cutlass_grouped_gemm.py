...
@pytest.mark.parametrize('num_groups, expected_m_per_group, k, n', [(4, 8192, 7168, 4096), (4, 8192, 2048, 7168), (8, 4096, 7168, 4096), (8, 4096, 2048, 7168), (32, 1024, 7168, 4096), (32, 1024, 2048, 7168)])
@pytest.mark.parametrize('out_dtype', [torch.float16])
@pytest.mark.skipif((lambda x: x is None or x.to_int() != 100)(current_platform.get_device_capability()), reason='Block Scaled Grouped GEMM is only supported on SM100.')
def test_cutlass_grouped_gemm(num_groups: int, expected_m_per_group: int, k: int, n: int, out_dtype: torch.dtype):
    device = 'cuda'
    alignment = 128
    ...
    ops.cutlass_blockwise_scaled_grouped_mm(out, x_fp8[0], y_fp8[0], x_fp8[1], y_fp8[1], problem_sizes, expert_offsets[:-1])
    torch.testing.assert_close(ref_out, out, atol=0.5, rtol=0.001)