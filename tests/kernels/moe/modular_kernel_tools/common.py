...
from vllm.model_executor.layers.fused_moe.config import FusedMoEConfig, FusedMoEParallelConfig, FusedMoEQuantConfig
...
from .utils import make_block_quant_fp8_weights, make_non_quant_weights, make_quant_fp8_weights, per_token_cast_to_fp8
if has_pplx():
...
class Config:
    ...
    def topk_ids_dtype(self) -> Optional[torch.dtype]:
        ...
        elif self.prepare_finalize_type in [DeepEPHTPrepareAndFinalize, DeepEPLLPrepareAndFinalize]:
    ...
    def make_env_data(self) -> tuple[VllmConfig, dict[Any, Any]]:
        ...
        env_dict = {'VLLM_ALL2ALL_BACKEND': self.all2all_backend(), 'VLLM_USE_DEEP_GEMM': str(int(self.needs_deep_gemm()))}
        if self.fused_moe_chunk_size is not None:
            env_dict.update({'VLLM_FUSED_MOE_CHUNK_SIZE': str(self.fused_moe_chunk_size)})
    ...
    def is_batched_fused_experts(self):
        return self.fused_experts_type in [CutlassExpertsFp8, BatchedDeepGemmExperts, BatchedTritonExperts, NaiveBatchedExperts, BatchedTritonOrDeepGemmExperts]

    def is_standard_fused_experts(self):
        return self.fused_experts_type in [CutlassExpertsFp8, DeepGemmExperts, TritonOrDeepGemmExperts, TritonExperts]

    def is_fe_16bit_supported(self):
        return self.fused_experts_type in [BatchedTritonExperts, BatchedTritonOrDeepGemmExperts, NaiveBatchedExperts, TritonExperts]

    def is_fe_fp8_supported(self):
        return self.fused_experts_type in [BatchedDeepGemmExperts, BatchedTritonExperts, BatchedTritonOrDeepGemmExperts, CutlassExpertsFp8, DeepGemmExperts, TritonExperts, TritonOrDeepGemmExperts, NaiveBatchedExperts]

    def is_fe_block_fp8_supported(self):
        return self.fused_experts_type in [BatchedDeepGemmExperts, BatchedTritonOrDeepGemmExperts, DeepGemmExperts, TritonExperts, TritonOrDeepGemmExperts, BatchedTritonExperts, NaiveBatchedExperts]

    def is_fe_supports_chunking(self):
        return self.fused_experts_type in [CutlassExpertsFp8, DeepGemmExperts, TritonOrDeepGemmExperts, TritonExperts]

    def needs_deep_gemm(self):
        return self.fused_experts_type in [BatchedDeepGemmExperts, DeepGemmExperts]
    ...
    def needs_all2all(self):
        return self.prepare_finalize_type in [PplxPrepareAndFinalize, DeepEPHTPrepareAndFinalize, DeepEPLLPrepareAndFinalize]

    def is_valid(self):
        if self.is_batched_prepare_finalize():
            if not self.is_batched_fused_experts():
                return False
        elif not self.is_standard_fused_experts():
            return False
        use_chunking = self.fused_moe_chunk_size is not None
        if use_chunking and (not self.is_fe_supports_chunking()):
            return False
        if int(self.is_per_act_token_quant) + int(self.is_per_tensor_act_quant) + int(self.quant_block_shape is not None) > 1:
            return False
...
class WeightTensors:
    ...
    def make(config: Config) -> 'WeightTensors':
        if config.quant_dtype is None:
            w1, w2 = make_non_quant_weights(e=config.E, n=config.N, k=config.K, dtype=config.dtype)
        ...
        if not config.is_fp8_block_quantized():
            w1, w2, w1_scale, w2_scale = make_quant_fp8_weights(e=config.E, n=config.N, k=config.K, per_out_channel_quant=config.is_per_out_ch_quant)
            return WeightTensors(w1=w1, w2=w2, w1_scale=w1_scale, w2_scale=w2_scale)
        assert config.quant_block_shape is not None
        w1, w2, w1_scale, w2_scale = make_block_quant_fp8_weights(e=config.E, n=config.N, k=config.K, block_size=config.quant_block_shape)
        return WeightTensors(w1=w1, w2=w2, w1_scale=w1_scale, w2_scale=w2_scale)
...
class RankTensors:
    ...
    @staticmethod
    def make_hidden_states(config: Config) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
    ...
    def make(config: Config, pgi: ProcessGroupInfo):
        ...
        return RankTensors(hidden_states=hidden_states, hidden_states_scale=hidden_states_scale, topk_weights=topk_weights, topk_ids=topk_ids, expert_map=expert_map, quant_config=config.quant_config)

def reference_moe_impl(config: Config, weights: WeightTensors, rank_tensors: RankTensors) -> torch.Tensor:
    return torch_experts(a=rank_tensors.hidden_states, w1=weights.w1, w2=weights.w2, topk_weight=rank_tensors.topk_weights, topk_ids=rank_tensors.topk_ids, global_num_experts=config.E, expert_map=None, w1_scale=weights.w1_scale, w2_scale=weights.w2_scale, a1_scale=rank_tensors.hidden_states_scale, quant_dtype=config.quant_dtype, per_act_token_quant=config.is_per_act_token_quant, block_shape=config.quant_block_shape, apply_router_weights_on_input=config.topk == 1)

def make_fused_experts(config: Config, moe: FusedMoEConfig, num_dispatchers: int) -> mk.FusedMoEPermuteExpertsUnpermute:
    use_fp8 = config.quant_dtype == torch.float8_e4m3fn
    batch_kwargs = {'max_num_tokens': moe.max_num_tokens, 'num_dispatchers': num_dispatchers}
    quant_kwargs = {'use_fp8_w8a8': use_fp8, 'use_int8_w8a8': False, 'use_int8_w8a16': False, 'use_int4_w4a16': False, 'block_shape': config.quant_block_shape, 'per_act_token_quant': config.is_per_act_token_quant}
    deepgemm_kwargs = {'allow_deep_gemm': has_deep_gemm()}
    if config.fused_experts_type == BatchedDeepGemmExperts:
        kwargs = batch_kwargs | {'block_shape': config.quant_block_shape, 'per_act_token_quant': config.is_per_act_token_quant}
        ...
        kwargs = {'max_experts_per_worker': num_experts, 'out_dtype': moe.in_dtype, 'per_act_token_quant': config.is_per_act_token_quant, 'per_out_ch_quant': config.is_per_out_ch_quant, 'block_shape': config.quant_block_shape, 'num_dispatchers': num_dispatchers, 'use_batched_format': use_batched_format}
        print(f'Making CutlassExpertsFp8 {kwargs} ...')
        experts = CutlassExpertsFp8(**kwargs)
    return experts

def make_modular_kernel(config: Config, vllm_config: VllmConfig) -> mk.FusedMoEModularKernel:

    def next_power_of_2(x):
        if x == 0:
            return 1
        return 2 ** math.ceil(math.log2(x))
    moe_parallel_config: FusedMoEParallelConfig = FusedMoEParallelConfig.make(tp_size_=get_tensor_model_parallel_world_size(), dp_size_=get_dp_group().world_size, vllm_parallel_config=vllm_config.parallel_config)
    moe = FusedMoEConfig(num_experts=config.E, experts_per_token=config.topk, hidden_dim=config.K, num_local_experts=config.num_local_experts, moe_parallel_config=moe_parallel_config, in_dtype=config.dtype, quant_config=config.quant_config, max_num_tokens=next_power_of_2(config.M))
