...
def make_feature_matrix(csv_file_path: str):
    ...
    def add_to_results(config: Config, success: Result, results_df: Optional[pd.DataFrame]=None):
    ...
    for m, k, n, e, topks, dtype, pf_type, experts_type, quant_config in tqdm(combinations):
        config = Config(Ms=[m], K=k, N=n, E=e, topks=topks, dtype=dtype, prepare_finalize_type=pf_type, fused_experts_type=experts_type, quant_config=quant_config, world_size=2, fused_moe_chunk_size=None)
        success = None
        if config.is_valid():
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Make ModularKernel feature matrix \nExample : python3 -m tests.kernels.moe.modular_kernel_tools.make_feature_matrix -f ./feature_matrices/feature_matrix.csv')
    parser.add_argument('-f', '--feature-matrix-csv-file-path', type=str, required=True, help='File name to Generate a .csv file')
