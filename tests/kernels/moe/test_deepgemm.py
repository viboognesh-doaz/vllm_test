...
def make_block_quant_fp8_weights(e: int, n: int, k: int, block_size: list[int]):
    ...
    fp8_max, fp8_min = (torch.finfo(torch.float8_e4m3fn).max, torch.finfo(torch.float8_e4m3fn).min)
...
def run_single_case(m, n, k, topk, num_experts, block_size):
    ...
    out_triton = fused_experts(hidden_states=tokens_bf16, w1=w1, w2=w2, topk_weights=topk_weights, topk_ids=topk_ids, inplace=False, use_fp8_w8a8=True, w1_scale=w1_s, w2_scale=w2_s, a1_scale=a1_scale, block_shape=block_size, allow_deep_gemm=False)
    out_deepgemm = fused_experts(hidden_states=tokens_bf16, w1=w1, w2=w2, topk_weights=topk_weights, topk_ids=topk_ids, inplace=False, use_fp8_w8a8=True, w1_scale=w1_s, w2_scale=w2_s, a1_scale=a1_scale, block_shape=block_size, allow_deep_gemm=True)
    diff = calc_diff(out_deepgemm, out_triton)
    assert diff < 0.001, f'Diff exceeded 1%: {diff}'
MNKs = [(1024, 512, 128), (1024, 512, 512), (2048, 512, 512), (512, 1024, 1024), (512, 2048, 2048), (4096, 4096, 1024)]
TOPKS = [2, 6]
NUM_EXPERTS = [32]
...
def test_deepgemm_vs_triton(mnk, topk, num_experts, monkeypatch):
    with monkeypatch.context() as m:
        ...
        _fused_moe_mod = importlib.import_module('vllm.model_executor.layers.fused_moe.fused_moe')
        call_counter = {'cnt': 0}
        ...
        run_single_case(m=m, n=n, k=k, topk=topk, num_experts=num_experts, block_size=BLOCK_SIZE)
        assert call_counter['cnt'] == 1, f"DeepGEMM path was not executed during the test. Call counter: {call_counter['cnt']}"