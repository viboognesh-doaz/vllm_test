...
def start_engine():
    wait_for_gpu_memory_to_clear(devices=list(range(torch.cuda.device_count())), threshold_bytes=2 * 2 ** 30, timeout_s=60)
    ...
    return AsyncLLMEngine.from_engine_args(AsyncEngineArgs(model='facebook/opt-125m', enforce_eager=True, num_scheduler_steps=num_scheduler_steps))
...
async def async_engine():
    ...
    engine = await asyncio.get_event_loop().run_in_executor(executor=None, func=start_engine)
    try:
        yield engine
    finally:
...
async def test_asyncio_run(async_engine, stop):
    ...
    async def run(prompt: str):
        sampling_params = SamplingParams(temperature=0, max_tokens=32, min_tokens=32, stop=stop)
        output_count = 0
        final_output = None
        async for output in async_engine.generate(prompt, sampling_params, request_id=uid()):
            output_count += 1
            final_output = output
        return (final_output, output_count)
    results = await asyncio.gather(run('test0'), run('test0'))
...
async def test_output_kinds(async_engine, stop):
    ...
    async def run(prompt: str, kind: RequestOutputKind):
        ...
        return (final_output.prompt_token_ids, final_output.outputs[0].token_ids, final_output.outputs[0].text, output_count)
    ...
    results = await asyncio.gather(run('common input prompt', RequestOutputKind.CUMULATIVE), run('common input prompt', RequestOutputKind.FINAL_ONLY), run_deltas('common input prompt'))
...
async def test_cancellation(async_engine, stop):
    scheduler_config = await async_engine.get_scheduler_config()
    num_scheduler_steps = scheduler_config.num_scheduler_steps
    sampling_params = SamplingParams(temperature=0, min_tokens=13, max_tokens=13, stop=stop)
    stop_at = 5 if num_scheduler_steps == 1 else 1
    request_id = uid()
    i = 0
    with pytest.raises(CancelledError):
        async for output in async_engine.generate('test2', sampling_params, request_id=request_id):
...
async def test_delayed_generator(async_engine, stop):
    ...
    sampling_params = SamplingParams(temperature=0, min_tokens=10, max_tokens=10, stop=stop)
...
async def test_invalid_argument(async_engine):
    ...
    with pytest.raises(ValueError):
        async for _ in async_engine.generate('test', sampling_params, request_id=uid(), data_parallel_rank=0):
            pass