...
def decode_value(value: AnyValue):
    field_decoders: dict[FieldName, Callable] = {'bool_value': lambda v: v.bool_value, 'string_value': lambda v: v.string_value, 'int_value': lambda v: v.int_value, 'double_value': lambda v: v.double_value, 'array_value': lambda v: [decode_value(item) for item in v.array_value.values]}
...
def test_traces(monkeypatch: pytest.MonkeyPatch, trace_service: FakeTraceService):
    with monkeypatch.context() as m:
        ...
        llm = LLM(model=model, otlp_traces_endpoint=FAKE_TRACE_SERVER_ADDRESS)
        ...
        assert len(request.resource_spans[0].scope_spans) == 1, f'Expected 1 scope span, but got {len(request.resource_spans[0].scope_spans)}'
        assert len(request.resource_spans[0].scope_spans[0].spans) == 1, f'Expected 1 span, but got {len(request.resource_spans[0].scope_spans[0].spans)}'
        ...
        assert attributes.get(SpanAttributes.GEN_AI_LATENCY_TIME_IN_QUEUE) == metrics.time_in_queue
        ...
        assert attributes.get(SpanAttributes.GEN_AI_LATENCY_TIME_IN_SCHEDULER) == metrics.scheduler_time
        assert metrics.model_forward_time is None
        assert metrics.model_execute_time is None

def test_traces_with_detailed_steps(monkeypatch: pytest.MonkeyPatch, trace_service: FakeTraceService):
    with monkeypatch.context() as m:
        m.setenv(OTEL_EXPORTER_OTLP_TRACES_INSECURE, 'true')
        sampling_params = SamplingParams(temperature=0.01, top_p=0.1, max_tokens=256)
        model = 'facebook/opt-125m'
        llm = LLM(model=model, otlp_traces_endpoint=FAKE_TRACE_SERVER_ADDRESS, collect_detailed_traces=['all'])
        prompts = ['This is a short prompt']
        outputs = llm.generate(prompts, sampling_params=sampling_params)
        timeout = 5
        if not trace_service.evt.wait(timeout):
        ...
        assert attributes.get(SpanAttributes.GEN_AI_LATENCY_TIME_IN_MODEL_FORWARD) == pytest.approx(metrics.model_forward_time / 1000)
        assert metrics.model_execute_time > 0
        assert attributes.get(SpanAttributes.GEN_AI_LATENCY_TIME_IN_MODEL_EXECUTE) == metrics.model_execute_time
