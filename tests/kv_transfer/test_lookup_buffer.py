...
def stress_test(my_rank, buf, device):
    ...
    reqs = [(torch.rand(100).to(device), torch.ones(100).bool().to(device), torch.rand(100).to(device), torch.rand(100).to(device), torch.rand(100).to(device)) for i in tqdm(range(200))]
    random.seed(my_rank)
    random.shuffle(reqs)
    torch.distributed.barrier()
    n = 0
...
if __name__ == '__main__':
    ...
    data_pipe = PyNcclPipe(local_rank=my_rank, config=config, device='cuda', port_offset=0)
    cpu_pipe = PyNcclPipe(local_rank=my_rank, config=config, device='cpu', port_offset=1)
