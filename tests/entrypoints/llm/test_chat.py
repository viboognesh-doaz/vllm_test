...
def test_chat(text_llm):
    ...
    messages = [{'role': 'system', 'content': 'You are a helpful assistant'}, {'role': 'user', 'content': prompt1}]
...
def test_multi_chat(text_llm):
    ...
    conversation1 = [{'role': 'system', 'content': 'You are a helpful assistant'}, {'role': 'user', 'content': prompt1}]
    conversation2 = [{'role': 'system', 'content': 'You are a helpful assistant'}, {'role': 'user', 'content': prompt2}]
...
@pytest.fixture(scope='function')
def vision_llm():
    llm = LLM(model='microsoft/Phi-3.5-vision-instruct', max_model_len=4096, max_num_seqs=5, enforce_eager=True, trust_remote_code=True, limit_mm_per_prompt={'image': 2}, seed=0)
    with llm.deprecate_legacy_api():
        yield weakref.proxy(llm)
        del llm
    cleanup_dist_env_and_memory()
...
def test_chat_extra_kwargs(thinking_llm, enable_thinking):
    ...
    outputs = thinking_llm.chat(messages, chat_template_kwargs={'enable_thinking': enable_thinking})
    assert len(outputs) == 1
    prompt_token_ids = outputs[0].prompt_token_ids
    assert prompt_token_ids is not None
