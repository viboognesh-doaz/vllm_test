...
def test_rerank_texts(server: RemoteOpenAIServer, model_name: str):
    ...
    rerank_response = requests.post(server.url_for('rerank'), json={'model': model_name, 'query': query, 'documents': documents})
...
def test_top_n(server: RemoteOpenAIServer, model_name: str):
    query = 'What is the capital of France?'
    documents = ['The capital of Brazil is Brasilia.', 'The capital of France is Paris.', 'Cross-encoder models are neat']
    rerank_response = requests.post(server.url_for('rerank'), json={'model': model_name, 'query': query, 'documents': documents, 'top_n': 2})
    rerank_response.raise_for_status()
    rerank = RerankResponse.model_validate(rerank_response.json())
    assert rerank.id is not None
    assert rerank.results is not None
