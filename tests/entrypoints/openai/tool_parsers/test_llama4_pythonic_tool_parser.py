...
SIMPLE_FUNCTION_CALL = FunctionCall(name='get_weather', arguments='{"city": "LA", "metric": "C"}')
MORE_TYPES_FUNCTION_OUTPUT = "[register_user(name='Doe', age=9, address={'city': 'LA', 'state': 'CA'}, role=None, passed_test=True, aliases=['John', 'Johnny'])]"
MORE_TYPES_FUNCTION_CALL = FunctionCall(name='register_user', arguments='{"name": "Doe", "age": 9, "address": {"city": "LA", "state": "CA"}, "role": null, "passed_test": true, "aliases": ["John", "Johnny"]}')
...
def test_streaming_tool_call_with_large_steps():
    mock_tokenizer = MagicMock()
    tool_parser: ToolParser = ToolParserManager.get_tool_parser('llama4_pythonic')(mock_tokenizer)
    model_output_deltas = ["<|python_start|>[get_weather(city='LA', metric='C'), get_weather(), do_something_cool(steps=[])]<|python_end|>"]
