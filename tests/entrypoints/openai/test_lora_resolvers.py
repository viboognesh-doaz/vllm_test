...
class MockLoRAResolver(LoRAResolver):
    ...
    async def resolve_lora(self, base_model_name: str, lora_name: str) -> Optional[LoRARequest]:
        if lora_name == 'test-lora':
            return LoRARequest(lora_name='test-lora', lora_int_id=1, lora_local_path='/fake/path/test-lora')
        elif lora_name == 'invalid-lora':
            return LoRARequest(lora_name='invalid-lora', lora_int_id=2, lora_local_path='/fake/path/invalid-lora')
        return None
...
def mock_serving_setup():
    ...
    serving_completion = OpenAIServingCompletion(mock_engine, mock_model_config, models, request_logger=None)
