...
def v1(run_with_both_engines):
    pass
MODELS = [{'name': 'BAAI/bge-reranker-v2-m3', 'is_cross_encoder': True}, {'name': 'BAAI/bge-base-en-v1.5', 'is_cross_encoder': False}]
DTYPE = 'half'
...
def runner(model: dict[str, Any], hf_runner):
    kwargs = {'dtype': DTYPE, 'is_cross_encoder' if model['is_cross_encoder'] else 'is_sentence_transformer': True}
...
class TestModel:
    ...
    def test_text_1_str_text_2_list(self, server: RemoteOpenAIServer, model: dict[str, Any], runner):
        ...
        score_response = requests.post(server.url_for('score'), json={'model': model['name'], 'text_1': text_1, 'text_2': text_2})
    ...
    def test_text_1_list_text_2_list(self, server: RemoteOpenAIServer, model: dict[str, Any], runner):
        text_1 = ['What is the capital of the United States?', 'What is the capital of France?']
        text_2 = ['The capital of Brazil is Brasilia.', 'The capital of France is Paris.']
        score_response = requests.post(server.url_for('score'), json={'model': model['name'], 'text_1': text_1, 'text_2': text_2})
        score_response.raise_for_status()
        score = ScoreResponse.model_validate(score_response.json())
    ...
    def test_score_max_model_len(self, server: RemoteOpenAIServer, model: dict[str, Any]):
        ...
        score_response = requests.post(server.url_for('score'), json={'model': model['name'], 'text_1': text_1, 'text_2': text_2, 'truncate_prompt_tokens': 101})
        assert score_response.status_code == 400
    ...
    def test_invocations(self, server: RemoteOpenAIServer, model: dict[str, Any]):
        text_1 = 'What is the capital of France?'
        text_2 = 'The capital of France is Paris.'
        request_args = {'model': model['name'], 'text_1': text_1, 'text_2': text_2}
        ...
        for score_data, invocation_data in zip(score_output['data'], invocation_output['data']):
            ...
            assert score_data['score'] == pytest.approx(invocation_data['score'], rel=0.01)