...
def default_server_args():
    return ['--dtype', 'bfloat16', '--max-model-len', '1024', '--enforce-eager', '--max-num-seqs', '128']

@pytest.fixture(scope='module', params=['', '--enable-chunked-prefill', '--disable-frontend-multiprocessing', f'--show-hidden-metrics-for-version={PREV_MINOR_VERSION}'])
                ...
                for suffix, expected_value in suffix_values_list:
                    ...
                    for sample in family.samples:
                        if sample.name == metric_name_w_suffix:
                            found_suffix = True
                            assert sample.value == expected_value, f'{metric_name_w_suffix} expected value of {expected_value} did not match found value {sample.value}'
                            break
...
EXPECTED_METRICS = ['vllm:num_requests_running', 'vllm:num_requests_waiting', 'vllm:gpu_cache_usage_perc', 'vllm:time_to_first_token_seconds_sum', 'vllm:time_to_first_token_seconds_bucket', 'vllm:time_to_first_token_seconds_count', 'vllm:time_per_output_token_seconds_sum', 'vllm:time_per_output_token_seconds_bucket', 'vllm:time_per_output_token_seconds_count', 'vllm:e2e_request_latency_seconds_sum', 'vllm:e2e_request_latency_seconds_bucket', 'vllm:e2e_request_latency_seconds_count', 'vllm:request_queue_time_seconds_sum', 'vllm:request_queue_time_seconds_bucket', 'vllm:request_queue_time_seconds_count', 'vllm:request_inference_time_seconds_sum', 'vllm:request_inference_time_seconds_bucket', 'vllm:request_inference_time_seconds_count', 'vllm:request_prefill_time_seconds_sum', 'vllm:request_prefill_time_seconds_bucket', 'vllm:request_prefill_time_seconds_count', 'vllm:request_decode_time_seconds_sum', 'vllm:request_decode_time_seconds_bucket', 'vllm:request_decode_time_seconds_count', 'vllm:request_prompt_tokens_sum', 'vllm:request_prompt_tokens_bucket', 'vllm:request_prompt_tokens_count', 'vllm:request_generation_tokens_sum', 'vllm:request_generation_tokens_bucket', 'vllm:request_generation_tokens_count', 'vllm:request_params_n_sum', 'vllm:request_params_n_bucket', 'vllm:request_params_n_count', 'vllm:request_params_max_tokens_sum', 'vllm:request_params_max_tokens_bucket', 'vllm:request_params_max_tokens_count', 'vllm:iteration_tokens_total', 'vllm:num_preemptions_total', 'vllm:prompt_tokens_total', 'vllm:generation_tokens_total', 'vllm:request_success_total', 'vllm:cache_config_info', 'block_size', 'cache_dtype', 'cpu_offload_gb', 'enable_prefix_caching', 'gpu_memory_utilization', 'num_cpu_blocks', 'num_gpu_blocks', 'num_gpu_blocks_override', 'sliding_window', 'swap_space_bytes']
EXPECTED_METRICS_V1 = ['vllm:num_requests_running', 'vllm:num_requests_waiting', 'vllm:gpu_cache_usage_perc', 'vllm:gpu_prefix_cache_queries', 'vllm:gpu_prefix_cache_hits', 'vllm:num_preemptions_total', 'vllm:prompt_tokens_total', 'vllm:generation_tokens_total', 'vllm:iteration_tokens_total', 'vllm:cache_config_info', 'vllm:request_success_total', 'vllm:request_prompt_tokens_sum', 'vllm:request_prompt_tokens_bucket', 'vllm:request_prompt_tokens_count', 'vllm:request_generation_tokens_sum', 'vllm:request_generation_tokens_bucket', 'vllm:request_generation_tokens_count', 'vllm:request_params_n_sum', 'vllm:request_params_n_bucket', 'vllm:request_params_n_count', 'vllm:request_params_max_tokens_sum', 'vllm:request_params_max_tokens_bucket', 'vllm:request_params_max_tokens_count', 'vllm:time_to_first_token_seconds_sum', 'vllm:time_to_first_token_seconds_bucket', 'vllm:time_to_first_token_seconds_count', 'vllm:time_per_output_token_seconds_sum', 'vllm:time_per_output_token_seconds_bucket', 'vllm:time_per_output_token_seconds_count', 'vllm:e2e_request_latency_seconds_sum', 'vllm:e2e_request_latency_seconds_bucket', 'vllm:e2e_request_latency_seconds_count', 'vllm:request_queue_time_seconds_sum', 'vllm:request_queue_time_seconds_bucket', 'vllm:request_queue_time_seconds_count', 'vllm:request_inference_time_seconds_sum', 'vllm:request_inference_time_seconds_bucket', 'vllm:request_inference_time_seconds_count', 'vllm:request_prefill_time_seconds_sum', 'vllm:request_prefill_time_seconds_bucket', 'vllm:request_prefill_time_seconds_count', 'vllm:request_decode_time_seconds_sum', 'vllm:request_decode_time_seconds_bucket', 'vllm:request_decode_time_seconds_count']
HIDDEN_DEPRECATED_METRICS: list[str] = []

@pytest.mark.asyncio
async def test_metrics_exist(server: RemoteOpenAIServer, client: openai.AsyncClient, use_v1: bool):
    await client.completions.create(model=MODEL_NAME, prompt='Hello, my name is', max_tokens=5, temperature=0.0)
    response = requests.get(server.url_for('metrics'))
    assert response.status_code == HTTPStatus.OK
...
def test_metrics_exist_run_batch(use_v1: bool):
    ...
    input_batch = '{"custom_id": "request-0", "method": "POST", "url": "/v1/embeddings", "body": {"model": "intfloat/multilingual-e5-small", "input": "You are a helpful assistant."}}'
    base_url = '0.0.0.0'
    port = '8001'
    ...
    with tempfile.NamedTemporaryFile('w') as input_file, tempfile.NamedTemporaryFile('r') as output_file:
        ...
        input_file.flush()
        proc = subprocess.Popen([sys.executable, '-m', 'vllm.entrypoints.openai.run_batch', '-i', input_file.name, '-o', output_file.name, '--model', 'intfloat/multilingual-e5-small', '--enable-metrics', '--url', base_url, '--port', port])
