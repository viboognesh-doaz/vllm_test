...
def server():
    args = ['--max-model-len', '8192', '--enforce-eager', '--reasoning-parser', 'deepseek_r1', '--enable-auto-tool-choice', '--tool-call-parser', 'hermes']
...
async def test_chat_streaming_of_tool_and_reasoning(client: openai.AsyncOpenAI):
    stream = await client.chat.completions.create(model=MODEL_NAME, messages=MESSAGES, tools=TOOLS, temperature=0.0, stream=True)
    chunks = []
...
async def test_chat_full_of_tool_and_reasoning(client: openai.AsyncOpenAI):
    tool_calls = await client.chat.completions.create(model=MODEL_NAME, messages=MESSAGES, tools=TOOLS, temperature=0.0, stream=False)
