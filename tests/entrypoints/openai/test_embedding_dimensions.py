...
MODELS = [EmbedModelInfo('intfloat/multilingual-e5-small', is_matryoshka=False), EmbedModelInfo('Snowflake/snowflake-arctic-embed-m-v1.5', is_matryoshka=True, matryoshka_dimensions=[256])]
...
def server(model_info, dtype: str):
    args = ['--task', 'embed', '--dtype', dtype, '--enforce-eager', '--max-model-len', '512']
...
async def test_matryoshka(model_info: EmbedModelInfo, server: RemoteOpenAIServer, hf_model: HfRunner):
    ...
    async def make_request_and_correctness_test(dimensions):
        ...
        embedding_response = await client.embeddings.create(model=model_info.name, input=prompts, dimensions=dimensions, encoding_format='float')
