...
import json
import pytest
LORA_MODULE = {'name': 'module2', 'path': '/path/to/module2', 'base_model_name': 'llama'}
...
def test_invalid_type_error(serve_parser):
    with pytest.raises(SystemExit):
        serve_parser.parse_args(['--lora-modules', 'invalid_format'])

def test_invalid_json_field(serve_parser):
    with pytest.raises(SystemExit):
        serve_parser.parse_args(['--lora-modules', '{"name": "module4"}'])
...
def test_multiple_valid_inputs(serve_parser):
    args = serve_parser.parse_args(['--lora-modules', 'module1=/path/to/module1', json.dumps(LORA_MODULE)])
    expected = [LoRAModulePath(name='module1', path='/path/to/module1'), LoRAModulePath(name='module2', path='/path/to/module2', base_model_name='llama')]
    assert args.lora_modules == expected
...
def test_enable_auto_choice_passes_with_tool_call_parser(serve_parser):
    ...
    args = serve_parser.parse_args(args=['--enable-auto-tool-choice', '--tool-call-parser', 'mistral'])
    validate_parsed_serve_args(args)
...
def test_enable_auto_choice_fails_with_enable_reasoning(serve_parser):
    ...
    args = serve_parser.parse_args(args=['--enable-auto-tool-choice', '--reasoning-parser', 'deepseek_r1'])
    with pytest.raises(TypeError):
        validate_parsed_serve_args(args)
...
@pytest.mark.parametrize('cli_args, expected_middleware', [(['--middleware', 'middleware1', '--middleware', 'middleware2'], ['middleware1', 'middleware2']), ([], [])])
