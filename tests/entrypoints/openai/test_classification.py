...
def test_single_input_classification(server: RemoteOpenAIServer, model_name: str):
    ...
    classification_response = requests.post(server.url_for('classify'), json={'model': model_name, 'input': input_text})
...
def test_multiple_inputs_classification(server: RemoteOpenAIServer, model_name: str):
    input_texts = ['The product arrived on time and works perfectly', "I'm very satisfied with my purchase, would buy again", 'The customer service was helpful and resolved my issue quickly', 'This product broke after one week, terrible quality', "I'm very disappointed with this purchase, complete waste of money", 'The customer service was rude and unhelpful']
    classification_response = requests.post(server.url_for('classify'), json={'model': model_name, 'input': input_texts})
    output = ClassificationResponse.model_validate(classification_response.json())
...
def test_truncate_prompt_tokens(server: RemoteOpenAIServer, model_name: str):
    long_text = 'hello ' * 600
    classification_response = requests.post(server.url_for('classify'), json={'model': model_name, 'input': long_text, 'truncate_prompt_tokens': 5})
    classification_response.raise_for_status()
    output = ClassificationResponse.model_validate(classification_response.json())
    assert len(output.data) == 1
...
def test_invalid_truncate_prompt_tokens_error(server: RemoteOpenAIServer, model_name: str):
    classification_response = requests.post(server.url_for('classify'), json={'model': model_name, 'input': 'test', 'truncate_prompt_tokens': 513})
...
def test_empty_input_error(server: RemoteOpenAIServer, model_name: str):
    classification_response = requests.post(server.url_for('classify'), json={'model': model_name, 'input': ''})
    error = classification_response.json()
    assert classification_response.status_code == 400
    assert error['object'] == 'error'
...
async def test_invocations(server: RemoteOpenAIServer):
    request_args = {'model': MODEL_NAME, 'input': 'This product was excellent and exceeded my expectations'}
