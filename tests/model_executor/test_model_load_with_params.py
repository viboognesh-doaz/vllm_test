...
def test_model_loading_with_params(vllm_runner):
    ...
    """
    with vllm_runner(model_name=MODEL_NAME, revision=REVISION, dtype='float16', max_model_len=MAX_MODEL_LEN) as vllm_model:
...
def test_roberta_model_loading_with_params(vllm_runner):
    ...
    Test parameter weight loading with tp>1.
    """
    with vllm_runner(model_name=MODEL_NAME_ROBERTA, revision=REVISION_ROBERTA, dtype='float16', max_model_len=MAX_MODEL_LEN) as vllm_model:
        output = vllm_model.embed('Write a short story about a robot that dreams for the first time.\n')
        model_config = vllm_model.model.llm_engine.model_config
        model_tokenizer = vllm_model.model.llm_engine.tokenizer
        assert model_config.encoder_config['max_seq_length'] == 512
...
def test_facebook_roberta_model_loading_with_params(vllm_runner):
    ...
    with vllm_runner(model_name=model_name, dtype='float16', max_model_len=MAX_MODEL_LEN) as vllm_model:
        output = vllm_model.embed('Write a short story about a robot that dreams for the first time.\n')
        model_tokenizer = vllm_model.model.llm_engine.tokenizer
