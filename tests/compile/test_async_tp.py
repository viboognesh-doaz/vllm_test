...
def test_async_tp_pass_replace(test_model: str, batch_size: int, seq_len: int, hidden_size: int, dtype: torch.dtype):
    num_processes = 2

    def run_torch_spawn(fn, nprocs):
        torch.multiprocessing.spawn(fn, args=(num_processes, test_model, batch_size, seq_len, hidden_size, dtype), nprocs=nprocs)
    run_torch_spawn(async_tp_pass_on_test_model, num_processes)

def async_tp_pass_on_test_model(local_rank: int, world_size: int, test_model_cls: torch.nn.Module, batch_size: int, seq_len: int, hidden_size: int, dtype: torch.dtype):
    ...
    vllm_config.compilation_config = CompilationConfig(pass_config=PassConfig(enable_async_tp=True))
...
def test_async_tp_pass_correctness(model_id: str, tp_size: int, async_tp_enabled: bool, distributed_backend: str, eager_mode: bool, num_gpus_available: int):
    ...
    common_args = ['--dtype', 'bfloat16', '--max-model-len', '2048', '--max-num-seqs', '8']
    if eager_mode:
        common_args.append('--enforce-eager')
    compilation_config = {'level': 3, 'compile_sizes': [2, 4, 8], 'splitting_ops': [], 'pass_config': {'enable_async_tp': async_tp_enabled}}
    async_tp_env = tp_env = {'VLLM_USE_V1': '1'}
    async_tp_args = [*common_args, '--tensor-parallel-size', str(tp_size), '--distributed-executor-backend', distributed_backend, '--compilation_config', json.dumps(compilation_config)]
    tp_args = [*common_args, '--tensor-parallel-size', str(tp_size), '--distributed-executor-backend', 'mp']
    compare_two_settings(model_id, async_tp_args, tp_args, async_tp_env, tp_env, method='generate')