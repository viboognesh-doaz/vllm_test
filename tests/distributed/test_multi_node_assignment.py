...
@pytest.mark.skipif(not VLLM_MULTI_NODE, reason='Need at least 2 nodes to run the test.')
    ...
    for _ in range(10):
        ...
        for bundle_id, bundle in enumerate(config.placement_group.bundle_specs):
            if not bundle.get('GPU', 0):
                continue
            scheduling_strategy = PlacementGroupSchedulingStrategy(placement_group=config.placement_group, placement_group_capture_child_tasks=True, placement_group_bundle_index=bundle_id)
            worker = ray.remote(num_cpus=0, num_gpus=1, scheduling_strategy=scheduling_strategy)(Actor).remote()
