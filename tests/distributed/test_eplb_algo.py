...
def test_basic_rebalance():
    ...
    assert phy2log.shape == (2, 16), f'Expected `phy2log` shape (2, 16), got {phy2log.shape}'
    ...
    assert logcnt.shape == (2, 12), f'Expected `logcnt` shape (2, 12), got {logcnt.shape}'
    ...
    assert torch.sum(logcnt, dim=1).sum() == num_replicas * num_layers, f'Total replicas should be {num_replicas * num_layers}'
    expected_phy2log = torch.tensor([[5, 6, 5, 7, 8, 4, 3, 4, 10, 9, 10, 2, 0, 1, 11, 1], [7, 10, 6, 8, 6, 11, 8, 9, 2, 4, 5, 1, 5, 0, 3, 1]])
...
def test_multiple_layers():
    ...
    weight = torch.tensor([[10, 20, 30, 40, 50, 60], [60, 50, 40, 30, 20, 10], [25, 25, 25, 25, 25, 25]])
    num_replicas = 8
    num_groups = 2
    num_nodes = 2
    num_gpus = 4
    ...
    for layer in range(3):
        assert torch.all(phy2log[layer] >= 0) and torch.all(phy2log[layer] < 6), f'Layer {layer} physical to logical mappingshould be in range [0, 6)'
...
def test_parameter_validation():
    ...
    with pytest.raises(AssertionError):
        rebalance_experts(weight, 7, 2, 2, 4)

def test_small_scale_hierarchical():
    """Test small-scale hierarchical load balancing"""
    weight = torch.tensor([[100, 50, 200, 75, 150, 25, 300, 80]])
    num_replicas = 12
    num_groups = 4
    num_nodes = 2
    num_gpus = 4
...
def test_additional_cases():
    ...
    weight2 = torch.tensor([[200, 150, 100, 50, 25, 12], [12, 25, 50, 100, 150, 200]])
