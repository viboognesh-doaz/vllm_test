...
from vllm.distributed.parallel_state import ensure_model_parallel_initialized, get_world_group, graph_capture, init_distributed_environment
...
def multiple_allreduce_worker_fn():
    ...
    groups = [torch.distributed.new_group(ranks=[0, 1], backend='gloo'), torch.distributed.new_group(ranks=[2, 3], backend='gloo')]
...
def all_gather_worker_fn():
    ...
    expected = torch.cat([torch.arange(num_elems, dtype=torch.float32) + r * num_elems for r in range(world_size)]).to(device)
    pynccl_comm.all_gather(result, tensor)
    torch.cuda.synchronize()
    torch.testing.assert_close(result, expected, rtol=1e-05, atol=1e-08)
...
def multiple_send_recv_worker_fn():
    device = torch.device(f'cuda:{torch.distributed.get_rank()}')
    groups = [torch.distributed.new_group(ranks=[0, 2], backend='gloo'), torch.distributed.new_group(ranks=[1, 3], backend='gloo')]
...
def broadcast_worker_fn():
    pynccl_comm = PyNcclCommunicator(get_world_group().cpu_group, device=get_world_group().device)
    recv_tensors = [torch.empty(16, 1024, 1024, dtype=torch.float32, device=pynccl_comm.device) for i in range(pynccl_comm.world_size)]
    recv_tensors[pynccl_comm.rank] = torch.ones(16, 1024, 1024, dtype=torch.float32, device=pynccl_comm.device) * pynccl_comm.rank
