...
def graph_allreduce(monkeypatch: pytest.MonkeyPatch, tp_size, pp_size, rank, distributed_init_port):
...
@ray.remote(num_gpus=1, max_calls=1)
def eager_allreduce(monkeypatch: pytest.MonkeyPatch, tp_size, pp_size, rank, distributed_init_port):
    with monkeypatch.context() as m:
        m.delenv('CUDA_VISIBLE_DEVICES', raising=False)
        device = torch.device(f'cuda:{rank}')
        torch.cuda.set_device(device)
...
def test_custom_allreduce(monkeypatch: pytest.MonkeyPatch, tp_size, pipeline_parallel_size, test_target):
