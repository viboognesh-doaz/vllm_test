...
from .conftest import MockSubscriber
from vllm.distributed.kv_events import EventBatch, EventPublisherFactory, NullEventPublisher
import msgspec
import pytest
import threading
import time
...
class EventSample(msgspec.Struct, tag=True, array_like=True):
...
def test_data_parallel_rank_tagging(publisher_config):
    ...
    if 'tcp://' in base_endpoint:
        expected_endpoint_0 = base_endpoint
        expected_endpoint_1 = base_endpoint.replace(':5557', ':5558')
    else:
        expected_endpoint_0 = base_endpoint
        expected_endpoint_1 = base_endpoint + '_dp1'
