...
def pytest_addoption(parser):
    ...
    parser.addoption('--extended', action='store_true', default=False, help='invoke extended tests requiring large GPUs')

@pytest.fixture(scope='session', params=CONFIGS.keys())
def server_config(request):
    extended = request.config.getoption('--extended')
    models = request.config.getoption('--models')
    config_keys_to_test = [key for key in CONFIGS if (models is None or key in models) and (extended or not CONFIGS[key].get('extended', False))]
