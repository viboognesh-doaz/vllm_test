...
import pytest
import regex as re
EXAMPLE_TOOLS = [{'type': 'function', 'function': {'name': 'get_current_weather', 'description': 'Get the current weather in a given location', 'parameters': {'type': 'object', 'properties': {'city': {'type': 'string', 'description': "The city to find the weather for, e.g. 'San Francisco'"}}, 'required': ['city'], 'additionalProperties': False}}, 'strict': True}, {'type': 'function', 'function': {'name': 'get_forecast', 'description': 'Get the weather forecast for a given location', 'parameters': {'type': 'object', 'properties': {'city': {'type': 'string', 'description': "The city to get the forecast for, e.g. 'New York'"}, 'days': {'type': 'integer', 'description': 'Number of days to get the forecast for (1-7)'}}, 'required': ['city', 'days'], 'additionalProperties': False}}, 'strict': True}]
...
VALID_TOOL_OUTPUTS = [([{'name': 'get_current_weather', 'parameters': {'city': 'Vienna'}}], True), ([{'name': 'get_current_weather', 'parameters': {'city': 'Vienna'}}, {'name': 'get_current_weather', 'parameters': {'city': 'Berlin'}}], True), ([{'name': 'get_forecast', 'parameters': {'city': 'Vienna', 'days': 7}}], True), ([{'name': 'get_forecast', 'parameters': {'city': 'Vienna', 'days': 7}}, {'name': 'get_current_weather', 'parameters': {'city': 'Vienna'}}], True), ([{'name': 'get_forecast', 'parameters': {'city': 'Vienna', 'days': 7}}, {'name': 'get_current_weather', 'parameters': {'city': 'Vienna'}}, {'name': 'get_forecast', 'parameters': {'city': 'Berlin', 'days': 7}}, {'name': 'get_current_weather', 'parameters': {'city': 'Berlin'}}], True)]
VALID_TOOLS = [t[0] for t in VALID_TOOL_OUTPUTS]

@pytest.mark.parametrize('sample_output, should_match', VALID_TOOL_OUTPUTS + [(None, False), ([], False), ({}, False), ([{}], False), ([{'name': 'get_current_weather'}], False), ([{'name': 'get_current_weather', 'parameters': {}}], False), ([{'name': 'get_current_weather', 'parameters': None}], False), ({'name': 'get_current_weather', 'parameters': {'city': 'Vienna'}}, False), ([{'name': 'get_current_weather', 'parameters': {'city': 'Vienna', 'extra': 'value'}}], False), ([{'parameters': {'city': 'Vienna'}, 'name': 'get_current_weather'}], False), ([{'name': 'get_forecast', 'parameters': {'city': 'Vienna'}}], False), ([{'name': 'get_weather', 'parameters': {'city': 'Vienna', 'days': 7}}], False), ([{'name': 'get_current_weather', 'parameters': {'city': 'Vienna'}}, {}], False)])
def test_guided_json(sample_output, should_match):
    _compile_and_check(tools=TypeAdapter(list[ChatCompletionToolsParam]).validate_python(EXAMPLE_TOOLS), sample_output=sample_output, should_match=should_match)

def update_parameters_none(tool: ChatCompletionToolsParam) -> ChatCompletionToolsParam:
    tool.function.parameters = None
    return tool

def update_parameters_empty_dict(tool: ChatCompletionToolsParam) -> ChatCompletionToolsParam:
    tool.function.parameters = {}
    return tool

@pytest.mark.parametrize('sample_output, should_match', [(None, False), ([], False), ({}, False), ([{}], False), ([{'name': 'get_current_weather'}], False), ([{'name': 'get_current_weather', 'parameters': None}], False), ([{'name': 'get_current_weather', 'parameters': {'extra': 'value'}}], False), ([{'name': 'get_current_weather', 'parameters': {}}], True)])
...
def test_streaming_output_valid(output, empty_params, delta_len):
    ...
    for i in range(0, len(output_json), delta_len):
        ...
        delta_message, function_name_returned = OpenAIServingChat.extract_tool_call_required_streaming(self, previous_text=previous_text, current_text=current_text, delta_text=delta_text, function_name_returned=function_name_returned)
        if delta_message:
    ...
    for message in messages:
        if message.tool_calls[0].function.name:
            if len(combined_messages) > 1:
                combined_messages += '},'
            combined_messages += '{"name": "' + message.tool_calls[0].function.name + '", "parameters": ' + message.tool_calls[0].function.arguments
        else:
