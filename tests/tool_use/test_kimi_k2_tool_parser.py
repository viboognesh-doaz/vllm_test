...
def assert_tool_calls(actual_tool_calls: list[ToolCall], expected_tool_calls: list[ToolCall]):
    ...
    for actual_tool_call, expected_tool_call in zip(actual_tool_calls, expected_tool_calls):
        ...
        assert actual_tool_call.id.split('.')[1].split(':')[0] == expected_tool_call.function.name

def test_extract_tool_calls_no_tools(kimi_k2_tool_parser):
    model_output = 'This is a test'
    extracted_tool_calls = kimi_k2_tool_parser.extract_tool_calls(model_output, request=None)
    assert not extracted_tool_calls.tools_called
    assert extracted_tool_calls.tool_calls == []
    assert extracted_tool_calls.content == model_output

@pytest.mark.parametrize(ids=['tool_call_with_content_before', 'multi_tool_call_with_content_before'], argnames=['model_output', 'expected_tool_calls', 'expected_content'], argvalues=[('I\'ll help you check the weather. <|tool_calls_section_begin|> <|tool_call_begin|>\nfunctions.get_weather:0 <|tool_call_argument_begin|> {"city": "Beijing"} <|tool_call_end|> <|tool_calls_section_end|>', [ToolCall(id='functions.get_weather:0', function=FunctionCall(name='get_weather', arguments=json.dumps({'city': 'Beijing'})), type='function')], "I'll help you check the weather. "), ('I\'ll help you check the weather. <|tool_calls_section_begin|> <|tool_call_begin|>\nfunctions.get_weather:0 <|tool_call_argument_begin|> {"city": "Beijing"} <|tool_call_end|> <|tool_call_begin|>\nfunctions.get_weather:1 <|tool_call_argument_begin|> {"city": "Shanghai"} <|tool_call_end|> <|tool_calls_section_end|>', [ToolCall(id='functions.get_weather:0', function=FunctionCall(name='get_weather', arguments=json.dumps({'city': 'Beijing'})), type='function'), ToolCall(id='functions.get_weather:1', function=FunctionCall(name='get_weather', arguments=json.dumps({'city': 'Shanghai'})), type='function')], "I'll help you check the weather. ")])
...
def test_streaming_basic_functionality(kimi_k2_tool_parser):
    ...
    result = kimi_k2_tool_parser.extract_tool_calls_streaming(previous_text="I'll help you", current_text=current_text, delta_text='<|tool_calls_section_end|>', previous_token_ids=[], current_token_ids=[], delta_token_ids=[], request=None)
...
def test_streaming_no_tool_calls(kimi_k2_tool_parser):
    ...
    result = kimi_k2_tool_parser.extract_tool_calls_streaming(previous_text='This is just regular text', current_text=current_text, delta_text=' without any tool calls.', previous_token_ids=[], current_token_ids=[], delta_token_ids=[], request=None)
