...
def test_model_runner_input():
    sampling_metadata = SamplingMetadata(['seq_group'], 'selected_token_indices', 'categorized_sample_indices', 'num_prompts')
    attn_metadata = AttentionMetadata(num_prefills=1, num_prefill_tokens=2, num_decode_tokens=3, slot_mapping=torch.zeros(1), multi_modal_placeholder_index_maps=None, enable_kv_scales_calculation=True)
    model_input = ModelInputForGPUWithSamplingMetadata(input_tokens=torch.ones(10), input_positions=torch.ones(10), sampling_metadata=sampling_metadata, attn_metadata=attn_metadata)
    assert isinstance(model_input, ModelInputForGPUWithSamplingMetadata)
    tensor_dict = model_input.as_broadcastable_tensor_dict()
    attn_backend = MockAttentionBackend()
    received_model_input = ModelInputForGPUWithSamplingMetadata.from_broadcasted_tensor_dict(tensor_dict, attn_backend=attn_backend)
    ...
    for field in dataclasses.fields(AttentionMetadata):
        assert getattr(received_model_input.attn_metadata, field.name, None) == getattr(attn_metadata, field.name, None)
    assert received_model_input.sampling_metadata.selected_token_indices == sampling_metadata.selected_token_indices
    assert received_model_input.sampling_metadata.seq_groups is None

def test_embedding_model_runner_input():
    pooling_metadata = PoolingMetadata(seq_groups=[[0]], seq_data={}, prompt_lens=[1])
    attn_metadata = AttentionMetadata(num_prefills=1, num_prefill_tokens=2, num_decode_tokens=3, slot_mapping=torch.zeros(1), multi_modal_placeholder_index_maps=None, enable_kv_scales_calculation=True)
    model_input = ModelInputForGPUWithPoolingMetadata(input_tokens=torch.ones(10), input_positions=torch.ones(10), pooling_metadata=pooling_metadata, attn_metadata=attn_metadata)
    assert isinstance(model_input, ModelInputForGPUWithPoolingMetadata)
    tensor_dict = model_input.as_broadcastable_tensor_dict()
    attn_backend = MockAttentionBackend()
    received_model_input = ModelInputForGPUWithPoolingMetadata.from_broadcasted_tensor_dict(tensor_dict, attn_backend=attn_backend)
...
def test_multi_step_model_runner_input():
    sampling_metadata = SamplingMetadata(['seq_group'], 'selected_token_indices', 'categorized_sample_indices', 'num_prompts')
    attn_metadata = AttentionMetadata(num_prefills=1, num_prefill_tokens=2, num_decode_tokens=3, slot_mapping=torch.zeros(1), multi_modal_placeholder_index_maps=None, enable_kv_scales_calculation=True)
    frozen_model_input = ModelInputForGPUWithSamplingMetadata(input_tokens=torch.ones(10), input_positions=torch.ones(10), sampling_metadata=sampling_metadata, attn_metadata=attn_metadata)
    model_input = StatefulModelInput(frozen_model_input=frozen_model_input, is_last_step=True, is_first_multi_step=False, current_step=4, last_sampled_token_ids=torch.ones((10, 1)), is_multi_step=True, num_queries=8, num_seqs=5, cached_outputs=[])
    assert isinstance(model_input, StatefulModelInput)
    tensor_dict = model_input.as_broadcastable_tensor_dict()
    attn_backend = MockAttentionBackend()
    received_model_input = StatefulModelInput.from_broadcasted_tensor_dict(tensor_dict, attn_backend=attn_backend)
    ...
    assert (received_frozen_input.input_positions == frozen_model_input.input_positions).all()
    assert received_frozen_input.multi_modal_kwargs is None
    assert frozen_model_input.multi_modal_kwargs == frozen_model_input.multi_modal_kwargs
    ...
    for field in dataclasses.fields(AttentionMetadata):
        assert getattr(received_frozen_input.attn_metadata, field.name, None) == getattr(attn_metadata, field.name, None)
    assert received_frozen_input.sampling_metadata.selected_token_indices == sampling_metadata.selected_token_indices
