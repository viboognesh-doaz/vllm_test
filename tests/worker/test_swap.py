...
def test_swap() -> None:
    engine_args = EngineArgs(model='distilbert/distilgpt2', dtype='half', load_format='dummy')
    ...
    execute_model_req = ExecuteModelRequest(seq_group_metadata_list=[], blocks_to_swap_in=[], blocks_to_swap_out=blocks_to_swap_out, blocks_to_copy=[])
    ...
    execute_model_req.blocks_to_swap_in = [(19, 45), (67, 23), (12, 78), (40, 99), (1, 71)]
    worker.execute_model(execute_model_req=execute_model_req)
    for i in range(num_layers):
        gpu_key_cache, gpu_value_cache = gpu_cache[i]
        cpu_key_cache, cpu_value_cache = cpu_cache[i]
