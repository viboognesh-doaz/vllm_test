...
@pytest.mark.skipif(current_platform.is_cpu(), reason='CPU backend is not currently supported with encoder/decoder models')
def test_encoder_decoder_e2e(hf_runner, vllm_runner, example_encoder_decoder_prompts, model: str, dtype: str, max_tokens: int, num_logprobs: int, decoder_prompt_type: DecoderPromptType, enforce_eager: bool, attn_backend: _Backend) -> None:
    """
    ...
    with global_force_attn_backend_context_manager(attn_backend):
        ...
        with hf_runner(model, dtype=dtype, auto_cls=AutoModelForSeq2SeqLM) as hf_model:
            hf_outputs = hf_model.generate_encoder_decoder_greedy_logprobs_limit(test_case_prompts, max_tokens, num_logprobs, **hf_kwargs)
        with vllm_runner(model, dtype=dtype, enforce_eager=enforce_eager) as vllm_model:
            vllm_outputs = vllm_model.generate_encoder_decoder_greedy_logprobs(test_case_prompts, max_tokens, num_logprobs)
