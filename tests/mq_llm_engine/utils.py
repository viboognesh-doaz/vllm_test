...
async def generate(client: MQLLMEngineClient, request_id: str, num_tokens: int, return_output: bool=False) -> Union[RequestOutput, tuple[int, str]]:
    final_output = None
    count = 0
    async for out in client.generate(request_id=request_id, prompt='Hello my name is Robert and', sampling_params=SamplingParams(max_tokens=num_tokens, temperature=0)):
        count += 1
        final_output = out
...
class RemoteMQLLMEngine:
    ...
    def __init__(self, engine_args: AsyncEngineArgs, ipc_path: str, run_fn: Callable=run_normal) -> None:
