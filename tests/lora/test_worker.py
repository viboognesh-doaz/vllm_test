...
from vllm.config import CacheConfig, DeviceConfig, LoadConfig, LoRAConfig, ModelConfig, ParallelConfig, SchedulerConfig, VllmConfig
...
def test_worker_apply_lora(sql_lora_files):
    ...
    def set_active_loras(worker: Union[Worker, V1Worker], lora_requests: list[LoRARequest]):
    ...
    vllm_config = VllmConfig(model_config=ModelConfig('meta-llama/Llama-2-7b-hf', task='auto', tokenizer='meta-llama/Llama-2-7b-hf', tokenizer_mode='auto', trust_remote_code=False, seed=0, dtype='float16', revision=None, enforce_eager=True), load_config=LoadConfig(download_dir=None, load_format='dummy'), parallel_config=ParallelConfig(pipeline_parallel_size=1, tensor_parallel_size=1, data_parallel_size=1), scheduler_config=SchedulerConfig('generate', 32, 32, 32), device_config=DeviceConfig('cuda'), cache_config=CacheConfig(block_size=16, swap_space=0, cache_dtype='auto'), lora_config=LoRAConfig(max_lora_rank=8, max_cpu_loras=NUM_LORAS, max_loras=NUM_LORAS))
    worker = worker_cls(vllm_config=vllm_config, local_rank=0, rank=0, distributed_init_method=f'file://{tempfile.mkstemp()[1]}')
    worker.init_device()
    worker.load_model()
