...
from vllm.lora.layers import ColumnParallelLinearWithLoRA, MergedColumnParallelLinearWithLoRA, RowParallelLinearWithLoRA
from vllm.lora.lora import LoRALayerWeights, PackedLoRALayerWeights
from vllm.lora.models import LoRAMapping, LoRAModel, LoRAModelManager, LRUCacheLoRAModelManager
...
def test_from_lora_tensors(sql_lora_files, device):
    ...
    lora_model = LoRAModel.from_lora_tensors(1, tensors, peft_helper=peft_helper, device=device, embeddings=new_embeddings, embedding_modules=EMBEDDING_MODULES, embedding_padding_modules=EMBEDDING_PADDING_MODULES)
...
def create_lora(lora_id: int, model: nn.Module, sub_modules: list[str], device: torch.device) -> LoRAModel:
    ...
    for name in sub_modules:
        ...
        loras[name] = LoRALayerWeights(name, 8, 16, torch.rand([w.shape[1], 8], device=device), torch.rand([8, w.shape[0]], device=device))
    return LoRAModel(lora_id, 8, loras)

def create_packed_lora(lora_id: int, model: nn.Module, module_name, replaced_module_names, device: torch.device, empty_replaced_module_name=None) -> LoRAModel:
    w = model.get_submodule(module_name).weight
    loras: dict[str, LoRALayerWeights] = {}
    for replaced_module_name in replaced_module_names:
        if replaced_module_name == empty_replaced_module_name:
            continue
        loras[replaced_module_name] = LoRALayerWeights(replaced_module_name, 8, 16, torch.rand([w.shape[1], 8], device=device), torch.rand([8, w.shape[0] // len(replaced_module_names)], device=device))
    return LoRAModel(lora_id, 8, loras)

def test_replace_submodules(dist_init, dummy_model):
    model = dummy_model
    manager = LoRAModelManager(model, 1, 1, 1, LoRAConfig(max_lora_rank=8, max_cpu_loras=8, max_loras=8, lora_dtype=DEFAULT_DTYPE), torch.device(DEVICES[0]))
    model = manager.model
...
def test_lora_model_manager(dist_init, dummy_model, device):
    ...
    manager = LoRAModelManager(model, 2, 2, 2, LoRAConfig(max_lora_rank=8, max_cpu_loras=3, max_loras=2, lora_dtype=DEFAULT_DTYPE), device=device)
    ...
    assert sorted(manager.supported_lora_modules) == ['dense1', 'dense2', 'lm_head', 'output']
...
def test_lora_lru_cache_model_manager(dist_init, dummy_model, device):
    ...
    model_lora2 = create_lora(2, model, ['dense1', 'dense2', 'lm_head'], device=device)
    model_lora3 = create_lora(3, model, ['dense1', 'dense2', 'lm_head'], device=device)
    manager = LRUCacheLoRAModelManager(model, 2, 2, 2, LoRAConfig(max_lora_rank=8, max_cpu_loras=3, max_loras=2, lora_dtype=DEFAULT_DTYPE), device=device)
    assert all((x is None for x in manager.lora_index_to_id))
    assert manager.add_adapter(model_lora1)
    assert manager.activate_adapter(1)
    assert manager.lora_index_to_id[0] == 1
...
def test_lru_lora_model_manager(dist_init, dummy_model, device):
    ...
    manager = LRUCacheLoRAModelManager(model, 2, 2, 2, LoRAConfig(max_lora_rank=8, max_cpu_loras=2, max_loras=2, lora_dtype=DEFAULT_DTYPE), device=device)
    assert all((x is None for x in manager.lora_index_to_id))
    assert manager.add_adapter(model_lora1)
    assert manager.add_adapter(model_lora2)
    assert manager.activate_adapter(1)
...
@pytest.mark.parametrize('device', DEVICES)
def test_lru_cache_worker_adapter_manager(llama_2_7b_model_extra_embeddings, sql_lora_files, device):
    lora_config = LoRAConfig(max_lora_rank=8, max_cpu_loras=4, max_loras=4, lora_dtype=DEFAULT_DTYPE)
    worker_adapter_manager = LRUCacheWorkerLoRAManager(4, 2, llama_2_7b_model_extra_embeddings.unpadded_vocab_size - lora_config.lora_extra_vocab_size, lora_config, device, EMBEDDING_MODULES, EMBEDDING_PADDING_MODULES)
    worker_adapter_manager.create_lora_manager(llama_2_7b_model_extra_embeddings)
    mapping = LoRAMapping([], [])
    worker_adapter_manager.set_active_adapters([LoRARequest('1', 1, sql_lora_files), LoRARequest('2', 2, sql_lora_files)], mapping)
    ...
    worker_adapter_manager.set_active_adapters([LoRARequest('1', 1, sql_lora_files), LoRARequest('3', 3, sql_lora_files), LoRARequest('4', 4, sql_lora_files)], mapping)
    ...
    worker_adapter_manager.set_active_adapters([LoRARequest('1', 1, sql_lora_files), LoRARequest('2', 2, sql_lora_files), LoRARequest('5', 5, sql_lora_files)], mapping)
    assert worker_adapter_manager.list_adapters() == {1, 2, 4, 5}
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[0] == 1
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[1] == 2
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[2] == 5
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[3] == 4
    worker_adapter_manager.set_active_adapters([LoRARequest('1', 1, sql_lora_files), LoRARequest('1', 1, sql_lora_files), LoRARequest('1', 1, sql_lora_files)], mapping)
    assert worker_adapter_manager.list_adapters() == {1, 2, 4, 5}
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[0] == 1
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[1] == 2
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[2] == 5
    assert worker_adapter_manager._adapter_manager.lora_index_to_id[3] == 4
    worker_adapter_manager.set_active_adapters([LoRARequest('6', 6, sql_lora_files), LoRARequest('7', 7, sql_lora_files), LoRARequest('8', 8, sql_lora_files)], mapping)
    ...
    with pytest.raises(RuntimeError):
        worker_adapter_manager.set_active_adapters([LoRARequest('10', 10, sql_lora_files), LoRARequest('11', 11, sql_lora_files), LoRARequest('12', 12, sql_lora_files), LoRARequest('13', 13, sql_lora_files), LoRARequest('14', 14, sql_lora_files)], mapping)
    assert worker_adapter_manager.device == device
    assert worker_adapter_manager._adapter_manager.punica_wrapper.device == device

@pytest.mark.parametrize('device', DEVICES)
def test_worker_adapter_manager(llama_2_7b_model_extra_embeddings, sql_lora_files, device):
    lora_config = LoRAConfig(max_lora_rank=8, max_cpu_loras=4, max_loras=4, lora_dtype=DEFAULT_DTYPE)
    worker_adapter_manager = WorkerLoRAManager(4, 2, llama_2_7b_model_extra_embeddings.unpadded_vocab_size - lora_config.lora_extra_vocab_size, lora_config, device, EMBEDDING_MODULES, EMBEDDING_PADDING_MODULES)
    worker_adapter_manager.create_lora_manager(llama_2_7b_model_extra_embeddings)
    mapping = LoRAMapping([], [])
    worker_adapter_manager.set_active_adapters([LoRARequest('1', 1, sql_lora_files), LoRARequest('2', 2, sql_lora_files)], mapping)
    assert worker_adapter_manager.list_adapters() == {1, 2}
...
def test_packed_loras(dist_init, dummy_model_gate_up, device):
    ...
    model_lora = create_packed_lora(1, model, module_name='gate_up_proj', replaced_module_names=['gate_proj', 'up_proj'], device=device)
    model_lora1 = create_packed_lora(2, model, module_name='gate_up_proj', replaced_module_names=['gate_proj', 'up_proj'], device=device, empty_replaced_module_name='gate_proj')
    manager = LoRAModelManager(model, 2, 2, 2, LoRAConfig(max_lora_rank=8, max_cpu_loras=2, max_loras=2, lora_dtype=DEFAULT_DTYPE), device=device)
    model = manager.model
