...
def do_sample(llm: vllm.LLM, lora_path: str, lora_id: int, tensorizer_config_dict: Union[dict, None]=None) -> list[str]:
    ...
    if tensorizer_config_dict is not None:
        outputs = llm.generate(prompts, sampling_params, lora_request=LoRARequest(str(lora_id), lora_id, lora_path, tensorizer_config_dict=tensorizer_config_dict) if lora_id else None)
    else:
...
def generate_and_test(llm, sql_lora_files, tensorizer_config_dict: Union[dict, None]=None):
    ...
    assert do_sample(llm, sql_lora_files, tensorizer_config_dict=tensorizer_config_dict, lora_id=0) == EXPECTED_NO_LORA_OUTPUT
    print('lora 1')
    assert do_sample(llm, sql_lora_files, tensorizer_config_dict=tensorizer_config_dict, lora_id=1) == EXPECTED_LORA_OUTPUT
    print('no lora')
    assert do_sample(llm, sql_lora_files, tensorizer_config_dict=tensorizer_config_dict, lora_id=0) == EXPECTED_NO_LORA_OUTPUT
    print('lora 2')
...
def test_llama_lora(sql_lora_files):
    llm = vllm.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=13, max_loras=4, enable_chunked_prefill=True)
    generate_and_test(llm, sql_lora_files)

@multi_gpu_test(num_gpus=4)
@create_new_process_for_each_test()
def test_llama_lora_tp4(sql_lora_files):
    llm = vllm.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=4, enable_chunked_prefill=True)
    generate_and_test(llm, sql_lora_files)

@multi_gpu_test(num_gpus=4)
@create_new_process_for_each_test()
def test_llama_lora_tp4_fully_sharded_loras(sql_lora_files):
    llm = vllm.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=4, fully_sharded_loras=True, enable_chunked_prefill=True)
    generate_and_test(llm, sql_lora_files)
...
def test_tp2_serialize_and_deserialize_lora(tmp_path, sql_lora_files, sql_lora_huggingface_id):
    ...
    suffix = 'test'
    try:
        result = subprocess.run([sys.executable, f'{VLLM_PATH}/examples/others/tensorize_vllm_model.py', '--model', MODEL_PATH, '--lora-path', lora_path, '--tensor-parallel-size', str(tp_size), 'serialize', '--serialized-directory', str(tmp_path), '--suffix', suffix, '--serialization-kwargs', '{"limit_cpu_concurrency": 4}'], check=True, capture_output=True, text=True)
    ...
    loaded_vllm_model = LLM(model=model_ref, load_format='tensorizer', enable_lora=True, enforce_eager=True, model_loader_extra_config=tensorizer_config, max_num_seqs=13, tensor_parallel_size=2, max_loras=2)
    tc_as_dict = tensorizer_config.to_serializable()
    print('lora adapter created')
    assert do_sample(loaded_vllm_model, sql_lora_files, tensorizer_config_dict=tc_as_dict, lora_id=0) == EXPECTED_NO_LORA_OUTPUT
    print('lora 1')
    assert do_sample(loaded_vllm_model, sql_lora_files, tensorizer_config_dict=tc_as_dict, lora_id=1) == EXPECTED_LORA_OUTPUT