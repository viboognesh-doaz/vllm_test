...
LLAMA_LORA_MODULES = ['qkv_proj', 'o_proj', 'gate_up_proj', 'down_proj', 'embed_tokens', 'lm_head']
...
def test_load_checkpoints_from_huggingface(lora_fixture_name, request):
    ...
    lora_model = LoRAModel.from_local_checkpoint(lora_path, expected_lora_modules, peft_helper=peft_helper, lora_model_id=1, device='cpu', embedding_modules=embedding_modules, embedding_padding_modules=embed_padding_modules)
