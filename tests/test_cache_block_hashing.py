...
def test_auto_prefix_caching(model: str, block_size: int, max_num_seqs: int, concurrent_lora_int_ids: list[Optional[int]]):
    tokenizer = TokenizerGroup(tokenizer_id='facebook/opt-125m', enable_lora=False, max_num_seqs=max_num_seqs, max_input_length=None)
    hashes: list[list[list[int]]] = []
    for prefix in prefixes:
        for lora_int_id in concurrent_lora_int_ids:
            lora_request = None
            if lora_int_id is not None:
                lora_request = LoRARequest(f'example_lora_{lora_int_id}', lora_int_id, f'example/path/to/lora_{lora_int_id}')
            hashes.append([])
            prompts = [prefix + prompt for prompt in sample_prompts]
            for seq_id, prompt in enumerate(prompts):
                hashes[-1].append([])
                prompt_token_ids = tokenizer.encode(prompt)
                seq = Sequence(seq_id, inputs=token_inputs(prompt_token_ids, prompt=prompt), block_size=block_size, eos_token_id=tokenizer.tokenizer.eos_token_id, lora_request=lora_request)
