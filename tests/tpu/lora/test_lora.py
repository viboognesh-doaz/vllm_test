...
def setup_vllm(num_loras: int) -> vllm.LLM:
    return vllm.LLM(model='Qwen/Qwen2.5-3B-Instruct', num_scheduler_steps=1, max_model_len=256, max_seq_len_to_capture=256, max_num_seqs=8, enable_lora=True, max_loras=num_loras, max_lora_rank=8)
...
def test_single_lora():
    ...
    output = llm.generate(prompt, sampling_params=vllm.SamplingParams(max_tokens=256, temperature=0), lora_request=lora_request)[0].outputs[0].text
