...
def sequence_with_eos(text: str, eos_token: str, eos_token_id: int) -> Sequence:
    ...
    """
    seq = Sequence(seq_id=0, inputs=token_inputs([]), block_size=16, eos_token_id=eos_token_id)
...
@pytest.mark.parametrize(['text_wo_eos', 'eos_token', 'eos_token_id'], [('This text ends with EOS token', '</s>', 2)])
@pytest.mark.parametrize('ignore_eos', [True, False])
@pytest.mark.parametrize('include_stop_str_in_output', [True, False])
@pytest.mark.skip_global_cleanup
def test_stop_on_eos_token(text_wo_eos: str, eos_token: str, eos_token_id: int, ignore_eos: bool, include_stop_str_in_output: bool):
    """
    ...
    stop_checker = StopChecker(max_model_len=1024, get_tokenizer_for_seq=get_tokenizer_for_seq)
    seq = sequence_with_eos(text=text_wo_eos, eos_token=eos_token, eos_token_id=eos_token_id)
    new_char_count = len(eos_token)
    sampling_params = SamplingParams(min_tokens=1, ignore_eos=ignore_eos, include_stop_str_in_output=include_stop_str_in_output)
    stop_checker.maybe_stop_sequence(seq=seq, new_char_count=new_char_count, sampling_params=sampling_params)
    if ignore_eos:
