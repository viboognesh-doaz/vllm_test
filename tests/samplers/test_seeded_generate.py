...
def test_random_sample_with_seed(vllm_model, example_prompts, seed: int) -> None:
    set_random_seed(seed)
    sampling_params = SamplingParams(temperature=3.0, top_p=min(random.random() + 0.3, 1), top_k=random.randint(5, 20), n=random.randint(1, 10), presence_penalty=random.randint(0, 1), max_tokens=8, ignore_eos=True)
    ...
    for prompt in example_prompts:
        for params in (sampling_params, sampling_params_seed_1, sampling_params_seed_2, sampling_params, sampling_params_seed_1, sampling_params_seed_2):
    ...
    for i in range(0, len(example_prompts), 6):
        ...
        for output_a, output_b in combinations((outputs[0], outputs[1], outputs[2], outputs[3]), 2):
