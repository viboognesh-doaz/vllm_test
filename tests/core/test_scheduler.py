...
def test_scheduler_add_seq_group():
    block_size = 4
    scheduler_config = SchedulerConfig('generate', max_num_batched_tokens=100, max_num_seqs=64, max_model_len=1)
...
def test_scheduler_prefill_prioritized():
    ...
    scheduler_config = SchedulerConfig('generate', max_num_batched_tokens=max_batched_num_tokens, max_num_seqs=2, max_model_len=max_model_len)
    cache_config = CacheConfig(block_size, 1.0, 1, 'auto')
...
def test_scheduler_schedule_preempt_abort():
    block_size = 4
    max_model_len = 16
    scheduler_config = SchedulerConfig('generate', max_num_batched_tokens=64, max_num_seqs=2, max_model_len=max_model_len)
    cache_config = CacheConfig(block_size, 1.0, 1, 'auto')
    ...
    assert out.num_batched_tokens == block_size * 2
    assert not out.blocks_to_copy and (not out.blocks_to_swap_in) and (not out.blocks_to_swap_out)
    ...
    seq_group_meta, out = schedule_and_update_computed_tokens(scheduler)
    assert get_sequence_groups(out) == [seq_group_b]
    assert out.num_batched_tokens == 5
    assert not out.blocks_to_copy and (not out.blocks_to_swap_in) and (not out.blocks_to_swap_out)
    assert len(seq_group_meta) == 1
...
def test_scheduler_max_seqs():
    ...
    max_seq_group = 2
    max_model_len = 16
    scheduler_config = SchedulerConfig('generate', max_num_batched_tokens=64, max_num_seqs=max_seq_group, max_model_len=max_model_len)
    cache_config = CacheConfig(block_size, 1.0, 1, 'auto')
    cache_config.num_cpu_blocks = 8
    cache_config.num_gpu_blocks = 8
    scheduler = Scheduler(scheduler_config, cache_config, None)
...
def test_scheduler_delay_factor():
    block_size = 4
    scheduler_config = SchedulerConfig('generate', max_num_batched_tokens=100, max_num_seqs=64, max_model_len=16, delay_factor=0.5)
    cache_config = CacheConfig(block_size, 1.0, 1, 'auto')
    cache_config.num_cpu_blocks = 8
    cache_config.num_gpu_blocks = 8
    scheduler = Scheduler(scheduler_config, cache_config, None)
    seq_group_meta, seq_group = create_dummy_prompt('0', prompt_length=block_size, block_size=block_size)
    scheduler.add_seq_group(seq_group)
    seq_group_meta, out = schedule_and_update_computed_tokens(scheduler)
    ...
    append_new_token(out, 1)
    time.sleep(1)
    seq_group_meta, seq_group = create_dummy_prompt('1', prompt_length=block_size, block_size=block_size)
    scheduler.add_seq_group(seq_group)
    seq_group_meta, out = schedule_and_update_computed_tokens(scheduler)
    assert out.num_prefill_groups == 0
    assert seq_group_meta[0].request_id == '0'
...
def initialize_scheduler(*, max_num_seqs=1000, max_token_budget=1000, max_model_len=1000, lora_config=None, block_size=4, num_cpu_blocks=8, num_gpu_blocks=8, enable_prefix_caching=False, enable_chunked_prefill=False):
    block_size = block_size
    scheduler_config = SchedulerConfig('generate', max_num_batched_tokens=max_token_budget, max_num_seqs=max_num_seqs, max_model_len=max_model_len, enable_chunked_prefill=enable_chunked_prefill)
    cache_config = CacheConfig(block_size, 1.0, 1, 'auto', enable_prefix_caching=enable_prefix_caching)
    cache_config.num_cpu_blocks = num_cpu_blocks
    cache_config.num_gpu_blocks = num_gpu_blocks
    scheduler = Scheduler(scheduler_config, cache_config, lora_config)
    return scheduler

def create_token_budget(token_budget: int=10000, max_num_seqs: int=10000) -> SchedulingBudget:
    return SchedulingBudget(token_budget=token_budget, max_num_seqs=max_num_seqs)

def add_token_budget(budget: SchedulingBudget, num_batched_tokens: int=0, num_curr_seqs: int=0):
...
def test_prefill_schedule_token_budget():
    ...
    """
    block_size = 4
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=64, num_gpu_blocks=64)
    ...
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=16, num_gpu_blocks=16)
    budget = create_token_budget(token_budget=60)
...
def test_prefill_schedule_max_lora():
    ...
    scheduler = initialize_scheduler(lora_config=lora_config, block_size=block_size, num_cpu_blocks=64, num_gpu_blocks=64)
    budget = create_token_budget(token_budget=120)
    curr_loras: set[int] = set()
    for i in range(2):
        _, seq_group = create_dummy_prompt(str(i), prompt_length=60, block_size=block_size, lora_request=LoRARequest(lora_name=str(i), lora_int_id=i + 1, lora_path='abc'))
        scheduler.add_seq_group(seq_group)
    for i in range(2, 4):
        _, seq_group = create_dummy_prompt(str(i), prompt_length=60, block_size=block_size)
        scheduler.add_seq_group(seq_group)
...
def test_prefill_schedule_no_block_manager_capacity():
    ...
    """
    block_size = 4
    scheduler = initialize_scheduler(block_size=block_size, num_gpu_blocks=128, num_cpu_blocks=128)
    budget = create_token_budget()
    for i in range(3):
        _, seq_group = create_dummy_prompt(str(i), prompt_length=60, block_size=block_size)
        scheduler.add_seq_group(seq_group)
...
def test_schedule_swapped_max_loras():
    block_size = 4
    lora_config = LoRAConfig(max_lora_rank=8, max_loras=1)
    scheduler = initialize_scheduler(lora_config=lora_config, block_size=block_size, num_cpu_blocks=32, num_gpu_blocks=32)
    curr_loras: set[int] = set()
...
def test_schedule_swapped_cannot_swap_in():
    block_size = 4
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=32, num_gpu_blocks=32)
    curr_loras = None
    blocks_to_swap_out: list[tuple[int, int]] = []
    for i in range(2):
        _, seq_group = create_dummy_prompt(str(i), prompt_length=60, block_size=block_size)
...
def test_prefix_caching_aware_prefills(enable_prefix_caching):
    ...
    max_seq_group = 3
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=16, num_gpu_blocks=16, max_token_budget=max_num_batched_tokens, max_num_seqs=max_seq_group, max_model_len=max_num_batched_tokens, enable_prefix_caching=enable_prefix_caching)
    seqA_tokens = list(range(8))
    num_shared_tokens = 4
    seqB_tokens = seqA_tokens[:num_shared_tokens] + list(range(12, 16))
    seqC_tokens = seqA_tokens[:num_shared_tokens] + list(range(16, 20))
    seqA, seqA_group = create_dummy_prompt('0', prompt_tokens=seqA_tokens, block_size=block_size)
    seqB, seqB_group = create_dummy_prompt('1', prompt_tokens=seqB_tokens, block_size=block_size)
    seqC, seqC_group = create_dummy_prompt('2', prompt_tokens=seqC_tokens, block_size=block_size)
    scheduler.add_seq_group(seqA_group)
    metas, out, _ = scheduler.schedule()
    assert len(out.scheduled_seq_groups) == 1 and out.scheduled_seq_groups[0].seq_group == seqA_group
    ...
    if enable_prefix_caching:
        ...
        assert set([out.scheduled_seq_groups[0].seq_group, out.scheduled_seq_groups[1].seq_group]) == set([seqB_group, seqC_group])
        assert len(metas) == 2
        for meta in metas:
            assert meta.token_chunk_size == 8
            assert len(meta.computed_block_nums) == num_shared_tokens // block_size
    else:
        assert len(out.scheduled_seq_groups) == 1
        assert len(metas) == 1
        assert metas[0].token_chunk_size == 8
        assert len(metas[0].computed_block_nums) == 0
...
def test_no_multiple_partial_prefills_with_chunked_prefill_and_prefix_caching():
    ...
    max_seq_group = 3
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=16, num_gpu_blocks=16, max_token_budget=max_num_batched_tokens, max_num_seqs=max_seq_group, max_model_len=100, enable_prefix_caching=True, enable_chunked_prefill=True)
    seqA_tokens = list(range(8))
    ...
    assert metas[seqA_group.request_id].token_chunk_size == 1
    assert metas[seqB_group.request_id].token_chunk_size == 8
    assert metas[seqC_group.request_id].token_chunk_size == 6, 'A partial prefix of C (4 tokens) should be prefilled, with the '
...
def test_no_batches_mixed_with_prompt_tokens_and_prompt_embeds():
    ...
    block_size = 2
    max_seq_group = 3
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=16, num_gpu_blocks=16, max_num_seqs=max_seq_group, max_model_len=100, enable_prefix_caching=True)
    seq_length = 7
    embedding_size = 5
    num_seqs = 11
    ...
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens[i], prompt_embeds=seq_embeds[i], block_size=block_size) for i in range(len(seq_tokens))]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    while not all((seq.is_finished() for seq, _ in seq_and_seq_groups)):
        unfinished_seq_groups = [seq_group for _, seq_group in seq_and_seq_groups if not seq_group.is_finished()]
        _, out = schedule_and_update_computed_tokens(scheduler)
        assert len(out.scheduled_seq_groups) > 0
        batch_is_prompt_embeds = out.scheduled_seq_groups[0].seq_group.uses_prompt_embeds()
        expected_scheduled_seq_groups = [seq_group for seq_group in unfinished_seq_groups if seq_group.uses_prompt_embeds() == batch_is_prompt_embeds]
        assert len(out.scheduled_seq_groups) == min(max_seq_group, len(expected_scheduled_seq_groups))
        assert all((scheduled_seq_group.seq_group.uses_prompt_embeds() == batch_is_prompt_embeds for scheduled_seq_group in out.scheduled_seq_groups))
...
def test_remove_seq_from_computed_blocks_tracker():
    ...
    blocks_to_swap_out: list[tuple[int, int]] = []
    curr_loras: set[int] = set()
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=64, num_gpu_blocks=16, max_num_seqs=max_seq_group, enable_prefix_caching=True)
    budget = create_token_budget(token_budget=15)
    seq_length = 16
    num_seqs = 3
    for i in range(num_seqs):
        seq_tokens_with_swapped.append([i] * seq_length)
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens_with_swapped[i], block_size=block_size) for i in range(len(seq_tokens_with_swapped))]
    for _, seq_group in seq_and_seq_groups:
        scheduler._allocate_and_set_running(seq_group)
        scheduler._swap_out(seq_group, blocks_to_swap_out)
        scheduler._add_seq_group_to_swapped(seq_group)
    scheduler._schedule_swapped(budget, curr_loras)
    seq_id_to_num_tokens_computed = scheduler.block_manager._computed_blocks_tracker._seq_id_to_num_tokens_computed.get(1)
    assert seq_id_to_num_tokens_computed is None
    block_size = 4
    lora_config = LoRAConfig(max_lora_rank=8, max_loras=1)
    scheduler = initialize_scheduler(lora_config=lora_config, block_size=block_size, num_cpu_blocks=64, num_gpu_blocks=64, enable_prefix_caching=True)
    budget = create_token_budget(token_budget=120)
    num_seqs = 2
    for i in range(num_seqs):
        _, seq_group = create_dummy_prompt(str(i), prompt_length=seq_length, block_size=block_size, lora_request=LoRARequest(lora_name=str(i), lora_int_id=i + 1, lora_path='abc'))
        scheduler.add_seq_group(seq_group)
    ...
    seq_embeds.append(torch.rand(embedding_size))
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens_with_embedding[i], prompt_embeds=seq_embeds[i], block_size=block_size) for i in range(len(seq_tokens_with_embedding))]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    ...
    scheduler = initialize_scheduler(block_size=block_size, max_token_budget=8, num_cpu_blocks=16, num_gpu_blocks=16, max_num_seqs=max_seq_group, max_model_len=5, enable_prefix_caching=True)
    seq_length = 4
    num_seqs = 3
    for i in range(num_seqs):
        seq_tokens_prefill_budget.append([i] * seq_length)
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens_prefill_budget[i], block_size=block_size) for i in range(len(seq_tokens_prefill_budget))]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    scheduler._schedule_default()
    seq_id_to_num_tokens_computed = scheduler.block_manager._computed_blocks_tracker._seq_id_to_num_tokens_computed.get(2)
    assert seq_id_to_num_tokens_computed is None
    block_size = 2
    max_seq_group = 3
    scheduler = initialize_scheduler(block_size=block_size, max_token_budget=30, num_cpu_blocks=16, num_gpu_blocks=16, max_num_seqs=max_seq_group, max_model_len=30, enable_prefix_caching=True)
    seq_length = 16
    num_seqs = 3
    seq_tokens_prefill_budget_waiting: list[list[int]] = []
    for i in range(num_seqs):
        seq_tokens_prefill_budget_waiting.append(list(range(seq_length)))
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens_prefill_budget_waiting[i], block_size=block_size) for i in range(len(seq_tokens_prefill_budget_waiting))]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    scheduler._schedule_default()
    seq_id_to_num_tokens_computed = scheduler.block_manager._computed_blocks_tracker._seq_id_to_num_tokens_computed.get(1)
    ...
    block_size = 2
    max_seq_group = 3
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=16, num_gpu_blocks=16, max_num_seqs=max_seq_group, max_model_len=30, enable_prefix_caching=True)
    seq_length = 31
    seq_tokens_prompt_limit: list[list[int]] = []
    seq_tokens_prompt_limit.append(list(range(seq_length)))
    seq_and_seq_groups = [create_dummy_prompt('0', prompt_tokens=seq_tokens_prompt_limit[0], block_size=block_size)]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    scheduler._schedule_default()
    seq_id_to_num_tokens_computed = scheduler.block_manager._computed_blocks_tracker._seq_id_to_num_tokens_computed.get(0)
    assert seq_id_to_num_tokens_computed is None
    block_size = 2
    max_seq_group = 3
    scheduler = initialize_scheduler(block_size=block_size, num_cpu_blocks=160, num_gpu_blocks=160, max_num_seqs=max_seq_group, max_model_len=320, enable_prefix_caching=True)
    seq_length = 320
    num_seqs = 1
    seq_tokens_never: list[list[int]] = []
    for i in range(num_seqs):
        seq_tokens_never.append(list(range(seq_length)))
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens_never[i], block_size=block_size) for i in range(len(seq_tokens_never))]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    scheduler._schedule_default()
    seq_id_to_num_tokens_computed = scheduler.block_manager._computed_blocks_tracker._seq_id_to_num_tokens_computed.get(0)
    ...
    seq_and_seq_groups = [create_dummy_prompt(f'{i}', prompt_tokens=seq_tokens_later[i], block_size=block_size) for i in range(len(seq_tokens_later))]
    for _, seq_group in seq_and_seq_groups:
        scheduler.add_seq_group(seq_group)
    scheduler._schedule_default()
    seq_id_to_num_tokens_computed = scheduler.block_manager._computed_blocks_tracker._seq_id_to_num_tokens_computed.get(1)
