...
class TestNaiveBlockAllocator:
    ...
    def test_naive_block_get_num_full_blocks_touched(num_blocks, block_size):
        ...
        full blocks touched.
        """
        allocator_src = NaiveBlockAllocator(create_block=NaiveBlock, num_blocks=num_blocks, block_size=block_size)
        allocator_dst = NaiveBlockAllocator(create_block=NaiveBlock, num_blocks=num_blocks, block_size=block_size)
        allocate_block = TestNaiveBlockAllocator.create_allocate_lambda('immutable', allocator_src, prev_block=None, token_ids=list(range(block_size)))
        ...
        allocate_non_full_block = TestNaiveBlockAllocator.create_allocate_lambda('mutable', allocator_src, prev_block=src_blocks[-1], token_ids=[])
