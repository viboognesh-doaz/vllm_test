...
@pytest.mark.parametrize('common_llm_kwargs', [{'model': MODEL, 'enforce_eager': True, 'block_size': BLOCK_SIZE, 'num_gpu_blocks_override': 100000 // BLOCK_SIZE}])
...
def test_sliding_window_retrieval(baseline_llm_generator, test_llm_generator, batch_size, seed, backend, monkeypatch):
    """
...
..." and then
    ...
    sampling_params = SamplingParams(max_tokens=1024, ignore_eos=True, temperature=0.0)
    prompts, answer, indices = prep_prompts(batch_size)
    baseline_texts = get_text_from_llm_generator(baseline_llm_generator, prompts, sampling_params, llm_cb=check_window(prompts))
    check_answers(indices, answer, baseline_texts)
    print('Getting token ids from block manager v2')
    test_texts = get_text_from_llm_generator(test_llm_generator, prompts, sampling_params)
...
@pytest.mark.parametrize('seed', [1])
@pytest.mark.parametrize('backend', ['FLASH_ATTN', 'FLASHINFER', 'XFORMERS'])
def test_sliding_window_chunked_prefill(test_llm_generator, batch_size, seed, backend, monkeypatch):
    """
    ...
    if backend == 'XFORMERS' and current_platform.is_rocm():
        pytest.skip('Xformers does not support ROCm/HIP.')
    override_backend_env_variable(monkeypatch, backend)
    sampling_params = SamplingParams(max_tokens=10, ignore_eos=True, temperature=0.0)
    prompts, answer, indices = prep_prompts(batch_size)
    test_texts = get_text_from_llm_generator(test_llm_generator, prompts, sampling_params, llm_cb=check_window(prompts))
    check_answers(indices, answer, test_texts)
...
def check_window(prompts: list[str]):
    ...
    def inner(llm: LLM):
        ...
        assert any((len(llm.get_tokenizer().tokenize(prompt)) > sliding_window for prompt in prompts))
    return inner