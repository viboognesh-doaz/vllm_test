...
def test_allocate_naive(block_size: int, sequence_len: int):
    ...
    assert block_size > 1
    num_gpu_blocks = 1024
    allocator = CpuGpuBlockAllocator.create(allocator_type='naive', num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, block_size=block_size)
...
def test_allocate_prefix_caching(block_size: int, sequence_len: int):
    ...
    assert block_size > 1
    num_gpu_blocks = 1024
    allocator = CpuGpuBlockAllocator.create(allocator_type='prefix_caching', num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, block_size=block_size)
    token_ids = list(range(sequence_len))
...
def test_allocate_free(block_size: int, sequence_len: int, allocator_type: str, device: str):
    ...
    allocator = CpuGpuBlockAllocator.create(allocator_type=allocator_type, num_gpu_blocks=num_device_blocks, num_cpu_blocks=num_device_blocks, block_size=block_size)
    token_ids = list(range(sequence_len))
    num_blocks_per_alloc = len(list(chunk_list(token_ids, block_size)))
...
@pytest.mark.parametrize('allocator_type', ['naive', 'prefix_caching'])
def test_append_token_ids_allocation(block_size: int, sequence_len: int, append_len: int, allocator_type: str):
    ...
    """
    num_gpu_blocks = 1024
    allocator = CpuGpuBlockAllocator.create(allocator_type=allocator_type, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, block_size=block_size)
    token_ids = list(range(sequence_len))
...
@pytest.mark.parametrize('allocator_type', ['naive', 'prefix_caching'])
def test_ensure_num_empty_slots_allocation(block_size: int, sequence_len: int, num_empty_slots: int, allocator_type: str):
...
@pytest.mark.parametrize('allocator_type', ['naive', 'prefix_caching'])
def test_append_token_ids_correct_content(block_size: int, sequence_len: int, append_len: int, allocator_type: str, append_size: int):
...
def test_fork(seq_len: int, block_size: int, allocator_type: str):
    ...
    """
    num_gpu_blocks = 1024
    allocator = CpuGpuBlockAllocator.create(allocator_type=allocator_type, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=0, block_size=block_size)
...
@pytest.mark.parametrize('allocator_type', ['naive', 'prefix_caching'])
def test_cow(block_size: int, sequence_len: int, append_len: int, allocator_type: str, appender: str):
    ...
    token_ids = list(range(sequence_len))
    token_ids_to_append = list(range(append_len))
    original_block_table = BlockTable(block_size=block_size, block_allocator=allocator)
    ...
    if appender == 'forked':
        ...
        raise ValueError(f'unknown test config appender={appender!r}')
...
@pytest.mark.parametrize('appender', ['forked', 'original'])
@pytest.mark.parametrize('allocator_type', ['naive', 'prefix_caching'])
def test_cow_lookahead_simple(block_size: int, sequence_len: int, append_len: int, lookahead_slots: int, allocator_type: str, appender: str):
...
@pytest.mark.parametrize('allocator_type', ['naive', 'prefix_caching'])
def test_num_blocks_touched_by_append_slots(block_size: int, sequence_len: int, num_new_tokens: int, num_lookahead_slots: int, allocator_type: str):
    ...
    _ = block_table.fork()
    expected_num_touched_blocks = block_table.get_num_blocks_touched_by_append_slots(token_ids=token_ids_to_append, num_lookahead_slots=num_lookahead_slots)
    ...
    num_consumed_blocks = num_free_blocks_before_append - allocator.get_num_free_blocks(Device.GPU)
