from ..utils import create_dummy_prompt, create_seq_group, create_seq_group_encoder_decoder
from vllm.core.block.utils import STR_NOT_IMPL_ENC_DEC_PREFIX_CACHE, STR_NOT_IMPL_ENC_DEC_SWA
...
def test_can_allocate_seq_group(block_size: int, num_seqs_per_group: int, num_gpu_blocks: int, watermark: float):
    block_manager = SelfAttnBlockSpaceManager(block_size=block_size, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, watermark=watermark)
    ...
    for num_prompt_blocks in range(1, num_gpu_blocks - num_output_blocks):
        seq_group = create_seq_group(seq_prompt_len=block_size * num_prompt_blocks, seq_output_lens=[block_size * num_output_blocks_per_seq for _ in range(num_seqs_per_group)])
...
@pytest.mark.parametrize('num_seqs_per_group', [1, 4])
@pytest.mark.parametrize('watermark', [0.0, 0.5])
def test_can_allocate_seq_group_encoder_decoder(block_size: int, num_seqs_per_group: int, num_gpu_blocks: int, watermark: float):
    block_manager = SelfAttnBlockSpaceManager(block_size=block_size, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, watermark=watermark)
    num_watermark_blocks = int(watermark * num_gpu_blocks)
    num_output_blocks_per_seq = 1
    num_output_blocks = num_output_blocks_per_seq
    for bdx, num_prompt_blocks in enumerate(range(1, num_gpu_blocks - num_output_blocks)):
        ...
        seq_group = create_seq_group_encoder_decoder(seq_prompt_len=block_size * num_prompt_blocks, seq_output_lens=[block_size * num_output_blocks_per_seq for _ in range(num_seqs_per_group)], request_id=str(bdx))
        assert num_prompt_blocks + num_output_blocks <= num_gpu_blocks
        can_allocate_result = block_manager.can_allocate(seq_group)
...
@pytest.mark.parametrize('watermark', [0.0, 0.5])
def test_can_allocate_encoder_decoder_fails_with_swa(block_size: int, num_seqs_per_group: int, num_gpu_blocks: int, watermark: float):
    """
    ...
    with pytest.raises((NotImplementedError, AssertionError)) as exc_info:
        block_manager = SelfAttnBlockSpaceManager(block_size=block_size, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, watermark=watermark, sliding_window=5)
        num_output_blocks_per_seq = 1
        num_prompt_blocks = 1
        num_output_blocks = num_output_blocks_per_seq
        seq_group = create_seq_group_encoder_decoder(seq_prompt_len=block_size * num_prompt_blocks, seq_output_lens=[block_size * num_output_blocks_per_seq for _ in range(num_seqs_per_group)], request_id='0')
        assert num_prompt_blocks + num_output_blocks <= num_gpu_blocks
...
@pytest.mark.parametrize('num_seqs_per_group', [1])
@pytest.mark.parametrize('watermark', [0.0, 0.5])
def test_can_allocate_encoder_decoder_fails_with_prefix_cache(block_size: int, num_seqs_per_group: int, num_gpu_blocks: int, watermark: float):
    block_manager = SelfAttnBlockSpaceManager(block_size=block_size, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=1024, watermark=watermark, enable_caching=True)
    num_output_blocks_per_seq = 1
    num_prompt_blocks = 1
    num_output_blocks = num_output_blocks_per_seq
    seq_group = create_seq_group_encoder_decoder(seq_prompt_len=block_size * num_prompt_blocks, seq_output_lens=[block_size * num_output_blocks_per_seq for _ in range(num_seqs_per_group)], request_id='0')
...
def test_append_slots(block_size, prompt_len, num_slots_to_append, num_lookahead_slots):
    ...
    num_gpu_blocks = 1024
    watermark = 0.1
    block_manager = SelfAttnBlockSpaceManager(block_size=block_size, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=0, watermark=watermark)
    seq_group = create_seq_group(seq_prompt_len=prompt_len, seq_output_lens=[0])
    ...
    expected_consumed_blocks = len(list(chunk_list(list(range(prompt_len + num_slots_to_append + num_lookahead_slots)), block_size))) - len(list(chunk_list(list(range(prompt_len)), block_size)))
...
def test_swap(block_size, num_cpu_blocks, num_gpu_blocks, num_lookahead_slots, enable_caching):
    """Verify blocks number on src/desc device is correct after swapping in/out
        sequence group (not missing or extra blocks).
    """
    block_manager = SelfAttnBlockSpaceManager(block_size, num_cpu_blocks, num_gpu_blocks, watermark=0, enable_caching=enable_caching)
...
def test_can_swap(block_size, num_gpu_blocks, num_lookahead_slots, enable_caching):
    ...
    block_manager = SelfAttnBlockSpaceManager(block_size, num_cpu_blocks, num_gpu_blocks, watermark=0, enable_caching=enable_caching)
    prompt, seq_group = create_dummy_prompt('1', prompt_length=(num_gpu_blocks - 1) * block_size - 1)
    prompt.status = SequenceStatus.WAITING
    block_manager.allocate(seq_group)
    prompt.status = SequenceStatus.RUNNING
    gpu_blocks = block_manager.get_block_table(prompt)
    ...
    if num_lookahead_slots <= block_size:
        assert block_manager.can_swap_in(seq_group, num_lookahead_slots) == AllocStatus.OK
    else:
        assert block_manager.can_swap_in(seq_group, num_lookahead_slots) == AllocStatus.NEVER
    prompt2_len = 2 * block_size - 1
    prompt2, seq_group2 = create_dummy_prompt('2', prompt_length=prompt2_len, prompt_tokens=[10000 + i for i in range(prompt2_len)])
    prompt2.status = SequenceStatus.WAITING
    block_manager.allocate(seq_group2)
    if num_lookahead_slots <= block_size:
        assert block_manager.can_swap_in(seq_group, num_lookahead_slots) == AllocStatus.LATER
    else:
        assert block_manager.can_swap_in(seq_group, num_lookahead_slots) == AllocStatus.NEVER
...
def test_swap_in_infeasible(num_lookahead_slots, enable_caching):
    ...
    prompt.status = SequenceStatus.SWAPPED
    num_unseen_tokens = 1
    if num_lookahead_slots + num_unseen_tokens + prompt_length <= block_size * num_gpu_blocks:
        assert block_manager.can_swap_in(seq_group, num_lookahead_slots) == AllocStatus.OK
    else:
        assert block_manager.can_swap_in(seq_group, num_lookahead_slots) == AllocStatus.NEVER
...
def test_sliding_window(block_size, prompt_len, num_slots_to_append, sliding_window):
    ...
    num_gpu_blocks = 1024
    watermark = 0.1
    block_manager = SelfAttnBlockSpaceManager(block_size=block_size, num_gpu_blocks=num_gpu_blocks, num_cpu_blocks=0, watermark=watermark, sliding_window=sliding_window)
