...
def create_model_dummy_weights(repo: str, model_arch: str) -> Iterable[tuple[str, torch.Tensor]]:
    """
...
def test_hf_model_weights_mapper(model_arch: str):
    ...
    assert not weights_missing and (not weights_unmapped), f'Following weights are not mapped correctly: {weights_unmapped}, Missing expected weights: {weights_missing}.'