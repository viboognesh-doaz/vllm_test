...
import pytest
PROMPTS = [{'prompt': '<|startoftranscript|><|en|><|transcribe|><|notimestamps|>', 'multi_modal_data': {'audio': AudioAsset('mary_had_lamb').audio_and_sample_rate}}, {'encoder_prompt': {'prompt': '', 'multi_modal_data': {'audio': AudioAsset('winning_call').audio_and_sample_rate}}, 'decoder_prompt': '<|startoftranscript|><|en|><|transcribe|><|notimestamps|>'}]
...
def run_test(vllm_runner: type[VllmRunner], model: str, *, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None) -> None:
    prompt_list = PROMPTS * 10
    expected_list = EXPECTED[model] * 10
    with vllm_runner(model, dtype='half', max_model_len=448, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend) as vllm_model:
...
@pytest.mark.parametrize('model', ['openai/whisper-small', 'openai/whisper-large-v3-turbo'])
@create_new_process_for_each_test()
def test_models(vllm_runner, model) -> None:
    run_test(vllm_runner, model, tensor_parallel_size=1)
...
@create_new_process_for_each_test()
def test_models_distributed(vllm_runner, model, distributed_executor_backend) -> None:
    run_test(vllm_runner, model, tensor_parallel_size=2, distributed_executor_backend=distributed_executor_backend)