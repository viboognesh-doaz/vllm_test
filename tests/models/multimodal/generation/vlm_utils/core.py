...
def run_test(*, hf_runner: type[HfRunner], vllm_runner: type[VllmRunner], inputs: list[PromptWithMultiModalInput], model: str, dtype: str, max_tokens: int, num_logprobs: int, enforce_eager: bool, max_model_len: int, max_num_seqs: int, hf_output_post_proc: Optional[Callable[[RunnerOutput, str], Any]], vllm_output_post_proc: Optional[Callable[[RunnerOutput, str], Any]], auto_cls: type[_BaseAutoModelClass], use_tokenizer_eos: bool, comparator: Callable[..., None], get_stop_token_ids: Optional[Callable[[AnyTokenizer], list[int]]], stop_str: Optional[list[str]], limit_mm_per_prompt: dict[str, int], vllm_runner_kwargs: Optional[dict[str, Any]], hf_model_kwargs: Optional[dict[str, Any]], patch_hf_runner: Optional[Callable[[HfRunner], HfRunner]], task: TaskOption='auto', distributed_executor_backend: Optional[str]=None, tensor_parallel_size: int=1, vllm_embeddings: Optional[torch.Tensor]=None):
    ...
    vllm_outputs_per_mm = []
    hf_outputs_per_mm = []
    vllm_runner_kwargs_: dict[str, Any] = {'disable_mm_preprocessor_cache': True}
    ...
    with vllm_runner(model, max_model_len=max_model_len, max_num_seqs=max_num_seqs, dtype=dtype, limit_mm_per_prompt=limit_mm_per_prompt, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=enforce_eager, task=task, **vllm_runner_kwargs_) as vllm_model:
        ...
        for prompts, image_data, video_data, audio_data in vllm_inputs:
            ...
            vllm_output = vllm_model.generate_greedy_logprobs(prompts, max_tokens, num_logprobs=num_logprobs, **vllm_kwargs_with_mm_data)
    ...
    with hf_model, torch.no_grad():
        ...
        for prompts, image_data, video_data, audio_data in inputs:
            ...
            hf_output = hf_model.generate_greedy_logprobs_limit(prompts, max_tokens, num_logprobs=num_logprobs, tokenizer=tokenizer, **hf_kwargs_with_mm_data)
            hf_outputs_per_mm.append(hf_output)
    hf_outputs_per_mm, vllm_outputs_per_mm = process_runner_outputs(model, first_runner_outputs=hf_outputs_per_mm, second_runner_outputs=vllm_outputs_per_mm, first_runner_processor=hf_output_post_proc, second_runner_processor=vllm_output_post_proc)
    for hf_outputs, vllm_outputs in zip(hf_outputs_per_mm, vllm_outputs_per_mm):
        comparator(outputs_0_lst=hf_outputs, outputs_1_lst=vllm_outputs, name_0='hf', name_1='vllm')

def process_runner_outputs(model, first_runner_outputs, second_runner_outputs, first_runner_processor=None, second_runner_processor=None):
