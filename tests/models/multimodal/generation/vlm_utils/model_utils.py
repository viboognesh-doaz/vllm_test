...
from vllm.model_executor.models.h2ovl import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values_h2ovl
from vllm.model_executor.models.internvl import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values_internvl, video_to_pixel_values_internvl
from vllm.model_executor.models.skyworkr1v import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values_skyworkr1v
...
def qwen_vllm_to_hf_output(vllm_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    """Sanitize vllm output [qwen models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_output
    hf_output_str = output_str + '<|endoftext|>'
    return (output_ids, hf_output_str, out_logprobs)

def qwen2_vllm_to_hf_output(vllm_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    """Sanitize vllm output [qwen2 models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_output
    hf_output_str = output_str + '<|im_end|>'
    return (output_ids, hf_output_str, out_logprobs)

def kimiv_vl_vllm_to_hf_output(vllm_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    """Sanitize vllm output [kimi_vl models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_output
    hf_output_str = output_str + '<|im_end|>[EOS]'
    return (output_ids, hf_output_str, out_logprobs)

def llava_image_vllm_to_hf_output(vllm_output: RunnerOutput, model: str) -> RunnerOutput:
...
def _llava_vllm_to_hf_output(vllm_output: RunnerOutput, model: str, mm_token_id: int) -> RunnerOutput:
    """Sanitize vllm output [Llava models] to be comparable with hf output."""
    output_ids, output_str, out_logprobs = vllm_output
    tokenizer = AutoTokenizer.from_pretrained(model)
    eos_token_id = tokenizer.eos_token_id
    hf_output_ids = [token_id for idx, token_id in enumerate(output_ids) if token_id != mm_token_id or output_ids[idx - 1] != mm_token_id]
    assert output_str[0] == ' '
...
def llava_onevision_vllm_to_hf_output(vllm_output: RunnerOutput, model: str) -> RunnerOutput:
    ...
    tokenizer = AutoTokenizer.from_pretrained(model)
    eos_token_id = tokenizer.eos_token_id
    hf_output_ids = [token_id for idx, token_id in enumerate(output_ids) if token_id != video_token_id or output_ids[idx - 1] != video_token_id]
    hf_output_str = output_str
    if hf_output_ids[-1] == eos_token_id:
        hf_output_str = hf_output_str + tokenizer.decode(eos_token_id)
    return (hf_output_ids, hf_output_str, out_logprobs)
...
def qwen_prompt_path_encoder(tmp_path: PosixPath, prompt: str, assets: Union[list[ImageAsset], ImageTestAssets]) -> str:
...
def deepseekvl2_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def processor(*args, text='', images=None, **kwargs):
        ...
        inputs = {k: inputs[k] for k in inputs.keys() if k not in ('seq_lens', 'sft_format')}
...
def glm4v_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def processor(*args, text='', images=None, **kwargs):
        ...
        contents = re.findall('<\\|begin_of_image\\|><\\|endoftext\\|><\\|end_of_image\\|>(.*?)<\\|assistant\\|>', text)
        assert len(contents) == len(images)
        return hf_processor.apply_chat_template([{'role': 'user', 'image': image, 'content': content} for image, content in zip(images, contents)], add_generation_prompt=True, tokenize=True, return_dict=True, **kwargs)
    hf_model.processor = processor
...
def h2ovl_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    class H2OVLProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]], **kwargs):
            images = [images] if isinstance(images, Image) else images
            pixel_values = [image_to_pixel_values_h2ovl(image, input_size=self.image_size, min_num=self.min_num, max_num=self.max_num, use_thumbnail=self.use_thumbnail, use_msac=self.use_msac) for image in images]
...
def skyworkr1v_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    class SkyworkR1VProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]], **kwargs):
            images = [images] if isinstance(images, Image) else images
            pixel_values = [image_to_pixel_values_skyworkr1v(image, input_size=self.image_size, min_num=self.min_num, max_num=self.max_num, use_thumbnail=self.use_thumbnail) for image in images]
            num_patches_list = [pixel_value.shape[0] for pixel_value in pixel_values]
            pixel_values = torch.cat(pixel_values, dim=0)
            for num_patches in num_patches_list:
                context_tokens = IMG_CONTEXT * self.num_image_token * num_patches
...
def internvl_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    class InternVLProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]]=None, videos: Union[npt.NDArray, list[npt.NDArray]]=None, **kwargs):
            images = [images] if isinstance(images, Image) else images
            videos = [videos] if isinstance(videos, np.ndarray) else videos
            if images is not None:
                pixel_values_images = [image_to_pixel_values_internvl(image, input_size=self.image_size, min_num=self.min_num, max_num=self.max_num, use_thumbnail=self.use_thumbnail) for image in images]
                num_patches_images = [pixel_value.shape[0] for pixel_value in pixel_values_images]
            else:
                pixel_values_images, num_patches_images = ([], [])
            if videos is not None:
                pixel_values_videos = [video_to_pixel_values_internvl(video, input_size=self.image_size, min_num=1, max_num=1, use_thumbnail=False) for video in videos]
...
def _internvl_generate(self, pixel_values: torch.FloatTensor, input_ids: torch.FloatTensor, attention_mask: Optional[torch.LongTensor]=None, **generate_kwargs) -> torch.LongTensor:
...
def mantis_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def _generate(self, *args, **kwargs):
        return orig_generate(*args, **kwargs, eos_token_id=[tokenizer.eos_token_id, tokenizer.convert_tokens_to_ids('<|eot_id|>')])
    hf_model.model.generate = types.MethodType(_generate, hf_model.model)
    return hf_model
...
def minicpmv_25_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def _generate(self, *args, input_ids=None, pixel_values=None, image_sizes=None, image_bound=None, tgt_sizes=None, **kwargs):
        model_inputs = {'input_ids': input_ids, 'pixel_values': pixel_values, 'image_sizes': image_sizes, 'image_bound': image_bound, 'tgt_sizes': tgt_sizes}
...
def molmo_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def _generate(self, max_new_tokens=None, do_sample=None, **kwargs):
        batch = {k: kwargs.pop(k).unsqueeze(0) for k in ('input_ids', 'images', 'image_input_idx', 'image_masks') if k in kwargs}
        batch = BatchFeature(batch).to(dtype=self.dtype)
        return self.generate_from_batch(batch, generation_config=GenerationConfig(max_new_tokens=max_new_tokens, stop_strings='<|endoftext|>', do_sample=do_sample), **kwargs)
    hf_model.model.generate = types.MethodType(_generate, hf_model.model)
    return hf_model
...
def ovis_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    ...
    def processor(*args, text='', images=None, **kwargs):
        ...
        images = [images] if isinstance(images, Image) else images
        prompt_start_and_end = {'qwen2': ('<|im_start|>user\n', '<|im_end|>\n'), 'llama': ('<|start_header_id|>user<|end_header_id|>\n\n', '<|eot_id|>'), 'gemma2': ('<start_of_turn>user\n', '<end_of_turn>\n')}
        ...
        inputs = {'inputs': input_ids.unsqueeze(0), 'pixel_values': pixel_values.unsqueeze(0), 'attention_mask': attention_mask.unsqueeze(0)}
        return BatchFeature(data=inputs, tensor_type='pt')
    hf_model.processor = processor
    return hf_model
