...
def multi_image_multi_aspect_ratio_inputs(formatter: Callable[[str], str]):
    ...
    img_prompts = ['<image><image>\nDescribe 2 images.', '<image><image>\nDescribe 2 images.', '<image><image><image><image>\nDescribe 4 images.', '<image>\nWhat is the season?']
    formatted_prompts = [formatter(prompt) for prompt in img_prompts]
    aspect_ratio_images = [[stop_sign, cherry_blossom], [rescale_image_size(stop_sign, 0.1), stop_sign], [stop_sign, rescale_image_size(stop_sign, 0.25), cherry_blossom.resize((183, 488)), cherry_blossom.resize((488, 183))], cherry_blossom]
    return [PromptWithMultiModalInput(prompts=formatted_prompts, image_data=aspect_ratio_images)]
...
def multi_video_multi_aspect_ratio_inputs(formatter: Callable[[str], str], num_frames: int=16):
    ...
    video_prompts = ['<video><video>\nDescribe 2 videos.', '<video><video>\nDescribe 2 videos.', '<video><video><video><video>\nDescribe 4 videos.', '<video>\nWhy is this video funny?']
    formatted_prompts = [formatter(prompt) for prompt in video_prompts]
    aspect_ratio_videos = [[video, video], [rescale_video_size(video, 0.1), video], [video, rescale_video_size(video, 0.25), resize_video(video, (183, 488)), resize_video(video, (488, 183))], video]
    return [PromptWithMultiModalInput(prompts=formatted_prompts, video_data=aspect_ratio_videos)]

def different_patch_input_cases_internvl():
    images = [asset.pil_image.resize((896, 896)) for asset in IMAGE_ASSETS]
    formatter = lambda img_prompt: f'<|im_start|>User\n{img_prompt}<|im_end|>\n<|im_start|>Assistant\n'
    single_img_prompts = ["<image>\nWhat's the content in the center of the image?", '<image>\nWhat is the season?']
    multi_img_prompts = ['Image-1: <image>\nImage-2: <image>\nDescribe the two images in detail.\n']
    ...
    return [build_single_image_inputs(images, formatted_sprompts, wrapped_sf), build_multi_image_inputs([images], formatted_mprompts, wrapped_sf)]
