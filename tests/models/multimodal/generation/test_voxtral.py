...
def server(request, audio_assets: AudioTestAssets):
    args = ['--enforce-eager', '--limit-mm-per-prompt', json.dumps({'audio': len(audio_assets)})] + MISTRAL_FORMAT_ARGS
...
def test_models_with_multiple_audios(vllm_runner, audio_assets: AudioTestAssets, dtype: str, max_tokens: int, num_logprobs: int) -> None:
    vllm_prompt = _get_prompt(audio_assets, MULTI_AUDIO_PROMPT)
    run_multi_audio_test(vllm_runner, [(vllm_prompt, [audio.audio_and_sample_rate for audio in audio_assets])], MODEL_NAME, dtype=dtype, max_tokens=max_tokens, num_logprobs=num_logprobs, tokenizer_mode='mistral')
...
async def test_online_serving(client, audio_assets: AudioTestAssets):
    ...
    messages = [{'role': 'user', 'content': [*audio_chunks, {'type': 'text', 'text': f"What's happening in these {len(audio_assets)} audio clips?"}]}]
