...
def _validate_image_max_tokens_one(processor: BaseMultiModalProcessor, max_tokens: int, failed_size_excs: list[tuple[ImageSize, Exception]], image_size: ImageSize) -> None:
    info = processor.info
...
def test_processor_max_tokens(model_id):
    ctx = build_model_context(model_id, mm_processor_kwargs=None, limit_mm_per_prompt={'image': 1})
    ...
    validate_one = partial(_validate_image_max_tokens_one, processor, info.get_max_image_tokens(), failed_size_excs)
    pqdm(image_sizes, validate_one, n_jobs=8, desc='Validating image sizes')
    if failed_size_excs:
        msg = 'Found failing image sizes:' + '\n========\n'.join((f'[{size}]\n{exc}' for size, exc in failed_size_excs))
        raise AssertionError(msg)

def _validate_image_prompt_replacements_one(processor: BaseMultiModalProcessor, num_imgs: int, failed_size_excs: list[tuple[ImageSize, Exception]], image_size: ImageSize) -> None:
...
def _test_image_prompt_replacements(processor, *, num_imgs: int, image_sizes: list[ImageSize]) -> None:
    """
    Ensure LlavaOnevisionMultiModalProcessor
    handles prompt replacement properly for input images.
    """
    failed_size_excs = list[tuple[ImageSize, Exception]]()
    validate_one = partial(_validate_image_prompt_replacements_one, processor, num_imgs, failed_size_excs)
    pqdm(image_sizes, validate_one, n_jobs=8, desc='Validating image sizes')
    if failed_size_excs:
        msg = 'Found failing image sizes:' + '\n========\n'.join((f'[{size}]\n{exc}' for size, exc in failed_size_excs))
        raise AssertionError(msg)
...
def test_processor_prompt_replacements_regression(model_id, num_imgs):
    ctx = build_model_context(model_id, mm_processor_kwargs=None, limit_mm_per_prompt={'image': num_imgs})
    processor = MULTIMODAL_REGISTRY.create_processor(ctx.model_config)
    image_ratios = [(171, 152), (184, 161), (198, 176), (333, 296), (369, 328), (488, 183), (2560, 1669)]
    image_sizes = [size for w, h in image_ratios for size in [ImageSize(w, h), ImageSize(h, w)]]
    _test_image_prompt_replacements(processor, num_imgs=num_imgs, image_sizes=image_sizes)
