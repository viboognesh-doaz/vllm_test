...
def test_processor_override(image_assets: ImageTestAssets, model_id: str, mm_processor_kwargs: dict, num_imgs: int, disable_mm_preprocessor_cache: bool, tokenized_prompt: bool):
    """Ensure llama4 processor works properly."""
    ctx = build_model_context(model_id, mm_processor_kwargs=mm_processor_kwargs, limit_mm_per_prompt={'image': num_imgs}, disable_mm_preprocessor_cache=disable_mm_preprocessor_cache)
    processor = MULTIMODAL_REGISTRY.create_processor(ctx.model_config)
    config = processor.info.get_hf_config()
    tokenizer = processor.info.get_tokenizer()
    hf_processor = processor.info.get_hf_processor()
    vocab = tokenizer.get_vocab()
    prompt = '<|begin_of_text|><|header_start|>user<|header_end|>' + '<|image|>' * num_imgs + '<|eot|><|header_start|>assistant<|header_end|>'
    ...
    assert [img_loc.offset for img_loc in img_locs] == [i for i, v in enumerate(prompt_token_ids) if v == config.boi_token_index]
    num_patches_per_chunk = processor.info.get_patch_per_chunk(config.vision_config)
    assert prompt_token_ids.count(config.image_token_index) == mm_kwargs['patches_per_image'].sum() * num_patches_per_chunk
