...
@pytest.mark.parametrize(('mm_processor_kwargs', 'expected_toks_per_img'), [({'dynamic_hd': 4}, 1329), ({'dynamic_hd': 16}, 4433), ({}, 9585)])
@pytest.mark.parametrize('num_imgs', [1, 2])
@pytest.mark.parametrize('kwargs_on_init', [True, False])
def test_processor_override(image_assets: ImageTestAssets, model_id: str, mm_processor_kwargs: dict[str, int], expected_toks_per_img: int, num_imgs: int, kwargs_on_init: bool):
