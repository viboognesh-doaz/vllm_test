...
HF_TEXT_PROMPTS = [('Query: Find me an everyday image that matches the given caption: The label of the object is stop sign', Image.new('RGB', (56, 56))), ('Query: Retrieve an image of this caption: cherry blossom', Image.new('RGB', (56, 56)))]
HF_IMAGE_PROMPTS = IMAGE_ASSETS.prompts({'stop_sign': 'What is shown in this image?', 'cherry_blossom': 'What is shown in this image?'})
MODELS = ['MrLight/dse-qwen2-2b-mrl-v1']

def get_messages(image: Image.Image, text: str, embed_text: bool):
    if embed_text:
        messages = [{'role': 'user', 'content': [{'type': 'image', 'image': Image.new('RGB', (56, 56)), 'resized_height': 1, 'resized_width': 1}, {'type': 'text', 'text': text}]}]
    else:
        messages = [{'role': 'user', 'content': [{'type': 'image', 'image': image}, {'type': 'text', 'text': text}]}]
    return messages

def apply_chat_template_and_add_eos(messages: list[dict], apply_chat_template_fn: Callable):
    prompt = apply_chat_template_fn(messages, tokenize=False, add_generation_prompt=True) + '<|endoftext|>'
    return prompt

def _run_test(hf_runner: type[HfRunner], vllm_runner: type[VllmRunner], input_texts: list[str], input_images: PromptImageInput, embed_texts: list[bool], model: str, *, dtype: str) -> None:
    """SET PYTHONPATH"""
    with vllm_runner(model, task='embed', dtype=dtype, enforce_eager=True, max_model_len=8192) as vllm_model:
        tokenizer = vllm_model.model.get_tokenizer()
        texts = [apply_chat_template_and_add_eos(get_messages(image, text, False), apply_chat_template_fn=tokenizer.apply_chat_template) for text, image in zip(input_texts, input_images)]
    ...
    with hf_runner(model, dtype=dtype, auto_cls=Qwen2VLForConditionalGeneration) as hf_model:
        ...
        with torch.no_grad():
            ...
            for inputs in all_inputs:
                inputs = hf_model.model.prepare_inputs_for_generation(**inputs, cache_position=torch.arange(1), use_cache=False)
...
def test_models_text(hf_runner, vllm_runner, image_assets, model: str, dtype: str) -> None:
    ...
    _run_test(hf_runner, vllm_runner, input_texts, input_images, embed_texts, model, dtype=dtype)
