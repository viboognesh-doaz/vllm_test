...
def run_intern_vit_test(image_assets: ImageTestAssets, model_id: str, *, dtype: str):
    ...
    hf_model = AutoModel.from_pretrained(model, torch_dtype=torch_dtype, trust_remote_code=True).to('cuda')
    ...
    vllm_outputs_per_image = [vllm_model(pixel_values=pixel_value.to('cuda')) for pixel_value in pixel_values]
    del vllm_model
    cleanup_dist_env_and_memory()
...
@pytest.mark.parametrize('model_id', ['OpenGVLab/InternViT-300M-448px', 'OpenGVLab/InternViT-6B-448px-V1-5'])
