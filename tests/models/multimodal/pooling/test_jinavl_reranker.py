...
def vllm_reranker(vllm_runner: type[VllmRunner], model_name: str, dtype: str, query_strs: list[str], document_strs: list[str], query_type: str='text', doc_type: str='text'):
    ...
    with vllm_runner(model_name, task='score', dtype=dtype, max_num_seqs=2, max_model_len=2048, mm_processor_kwargs=mm_processor_kwargs, limit_mm_per_prompt=limit_mm_per_prompt) as vllm_model:
        outputs = vllm_model.model.score(query, documents)
    return [output.outputs.score for output in outputs]

def hf_reranker(hf_runner: type[HfRunner], model_name: str, dtype: str, query_strs: list[str], document_strs: list[str], query_type: str='text', doc_type: str='text'):
    checkpoint_to_hf_mapper = {'visual.': 'model.visual.', 'model.': 'model.language_model.'}
    data_pairs = [[query_strs[0], d] for d in document_strs]
    with hf_runner(model_name, dtype=dtype, trust_remote_code=True, auto_cls=AutoModel, model_kwargs={'key_mapping': checkpoint_to_hf_mapper}) as hf_model:
...
def test_model_text_image(hf_runner, vllm_runner, model_name, dtype):
    query = ['slm markdown']
    documents = ['https://raw.githubusercontent.com/jina-ai/multimodal-reranker-test/main/handelsblatt-preview.png', 'https://raw.githubusercontent.com/jina-ai/multimodal-reranker-test/main/paper-11.png']
    hf_outputs = hf_reranker(hf_runner, model_name, dtype, query, documents, 'text', 'image')
    vllm_outputs = vllm_reranker(vllm_runner, model_name, dtype, query, documents, 'text', 'image')
...
def test_model_image_text(hf_runner, vllm_runner, model_name, dtype):
    ...
    vllm_outputs = vllm_reranker(vllm_runner, model_name, dtype, query, documents, 'image', 'text')
    assert hf_outputs[0] == pytest.approx(vllm_outputs[0], rel=0.02)
    assert hf_outputs[1] == pytest.approx(vllm_outputs[1], rel=0.02)
...
def test_model_image_image(hf_runner, vllm_runner, model_name, dtype):
    ...
    vllm_outputs = vllm_reranker(vllm_runner, model_name, dtype, query, documents, 'image', 'image')
    assert hf_outputs[0] == pytest.approx(vllm_outputs[0], rel=0.02)
    assert hf_outputs[1] == pytest.approx(vllm_outputs[1], rel=0.02)