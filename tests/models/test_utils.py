...
def test_module_skip_prefix():
    ...
    def weight_generator():
        redundant_weights = {'prefix.bn.weight': torch.Tensor([1, 2]), 'prefix.bn.bias': torch.Tensor([3, 4])}
...
def test_module_skip_substr():
    ...
    def weight_generator():
        redundant_weights = {'nested_mod.0.substr.weight': torch.Tensor([1, 2]), 'nested_mod.0.substr.bias': torch.Tensor([3, 4]), 'nested_mod.substr.weight': torch.Tensor([1, 2]), 'nested_mod.substr.bias': torch.Tensor([3, 4])}
        yield from (mod.state_dict() | redundant_weights).items()
    new_mod = ModuleWithNestedBatchNorm()
    assert not torch.all(new_mod.nested_mod.bn.running_mean == mod.nested_mod.bn.running_mean)
    assert not torch.all(new_mod.nested_mod.bn.running_var == mod.nested_mod.bn.running_var)
