...
def check_outputs_equal(*, outputs_0_lst: Sequence[TokensText], outputs_1_lst: Sequence[TokensText], name_0: str, name_1: str):
    """
...
TokensTextLogprobsPromptLogprobs = tuple[list[int], str, Optional[Union[list[dict[int, float]], SampleLogprobs]], Optional[Union[list[Optional[dict[int, float]]], PromptLogprobs]]]

def check_logprobs_close(*, outputs_0_lst: Sequence[Union[TokensTextLogprobs, TokensTextLogprobsPromptLogprobs, TextTextLogprobs]], outputs_1_lst: Sequence[Union[TokensTextLogprobs, TokensTextLogprobsPromptLogprobs, TextTextLogprobs]], name_0: str, name_1: str, num_outputs_0_skip_tokens: int=0, warn_on_mismatch: bool=True, always_check_logprobs: bool=False) -> None:
    ...
    * `always_check_logprobs == False`: highest-logprob token ids are
    ...
    for prompt_idx, (outputs_0, outputs_1) in enumerate(zip(outputs_0_lst, outputs_1_lst)):
        ...
        if len(outputs_0) == 3:
            ...
            raise ValueError(f'Outputs tuple must have 3 or 4 elements but {len(outputs_0)} elements were provided: {outputs_0}')
...
def build_model_context(model_id: str, task: TaskOption='auto', dtype: Union[str, torch.dtype]='auto', model_config_kwargs: Optional[dict[str, Any]]=None, mm_processor_kwargs: Optional[dict[str, Any]]=None, limit_mm_per_prompt: Optional[dict[str, int]]=None, disable_mm_preprocessor_cache: bool=True):
    ...
    model_config = ModelConfig(model_id, task=task, tokenizer=model_info.tokenizer or model_id, tokenizer_mode=model_info.tokenizer_mode, trust_remote_code=model_info.trust_remote_code, dtype=dtype, seed=0, mm_processor_kwargs=mm_processor_kwargs, limit_mm_per_prompt=limit_mm_per_prompt, disable_mm_preprocessor_cache=disable_mm_preprocessor_cache, hf_overrides=model_info.hf_overrides, **model_config_kwargs)
    return InputContext(model_config)

def check_embeddings_close(*, embeddings_0_lst: Sequence[list[float]], embeddings_1_lst: Sequence[list[float]], name_0: str, name_1: str, tol: float=0.001) -> None:
