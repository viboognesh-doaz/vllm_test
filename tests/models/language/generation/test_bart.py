from ....conftest import DecoderPromptType, ExplicitEncoderDecoderPrompt, HfRunner, VllmRunner
...
def vllm_to_hf_output(vllm_output: tuple[list[int], str, Optional[SampleLogprobs]], decoder_prompt_type: DecoderPromptType):
...
def run_test(hf_runner: type[HfRunner], vllm_runner: type[VllmRunner], prompts: list[ExplicitEncoderDecoderPrompt[str, str]], decoder_prompt_type: DecoderPromptType, model: str, *, dtype: str, max_tokens: int, num_logprobs: int, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None) -> None:
    """
    ...
    * vLLM will (1) tokenize the None prompt as [<BOS>], (2) append decoder-
    ...
    """
    with vllm_runner(model, dtype=dtype, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=True) as vllm_model:
    ...
    check_logprobs_close(outputs_0_lst=hf_outputs, outputs_1_lst=[vllm_to_hf_output(vllm_output, decoder_prompt_type) for vllm_output in vllm_outputs], name_0='hf', name_1='vllm', num_outputs_0_skip_tokens=hf_skip_tokens)

@pytest.mark.parametrize('model', [pytest.param('facebook/bart-base', marks=[pytest.mark.core_model, pytest.mark.cpu_model]), pytest.param('facebook/bart-large-cnn')])
@pytest.mark.parametrize('dtype', ['float', 'bfloat16'])
@pytest.mark.parametrize('max_tokens', [64])
@pytest.mark.parametrize('num_logprobs', [5])
@pytest.mark.parametrize('decoder_prompt_type', list(DecoderPromptType))
def test_models(hf_runner, vllm_runner, example_encoder_decoder_prompts, model, dtype, max_tokens, num_logprobs, decoder_prompt_type) -> None:
    run_test(hf_runner, vllm_runner, example_encoder_decoder_prompts[decoder_prompt_type], decoder_prompt_type, model, dtype=dtype, max_tokens=max_tokens, num_logprobs=num_logprobs, tensor_parallel_size=1)
...
def test_models_distributed(hf_runner, vllm_runner, example_encoder_decoder_prompts, distributed_executor_backend, model, dtype, max_tokens, num_logprobs, decoder_prompt_type) -> None:
    run_test(hf_runner, vllm_runner, example_encoder_decoder_prompts[decoder_prompt_type], decoder_prompt_type, model, dtype=dtype, max_tokens=max_tokens, num_logprobs=num_logprobs, tensor_parallel_size=2, distributed_executor_backend=distributed_executor_backend)