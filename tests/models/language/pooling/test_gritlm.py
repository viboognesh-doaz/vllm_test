...
def test_find_array():
    model_config = ModelConfig(MODEL_NAME, task='embed', tokenizer=MODEL_NAME, tokenizer_mode='auto', trust_remote_code=False, dtype='bfloat16', seed=0)
...
async def run_client_embeddings(client: openai.AsyncOpenAI, queries: list[str], instruction: str) -> list[list[float]]:
    outputs = await client.embeddings.create(model=MODEL_NAME, input=[instruction + q for q in queries])
...
def get_test_data():
    ...
    """
    q_instruction = gritlm_instruction("Given a scientific paper title, retrieve the paper's abstract")
...
def validate_embed_output(q_rep: list[list[float]], d_rep: list[list[float]]):
    ...
    assert cosine_sim_q1_d0 == pytest.approx(0.12, abs=0.001)
    cosine_sim_q1_d1 = 1 - cosine(q_rep[1], d_rep[1])
    assert cosine_sim_q1_d1 == pytest.approx(0.534, abs=0.001)

def test_gritlm_offline_embedding(vllm_runner):
    queries, q_instruction, documents, d_instruction = get_test_data()
    with vllm_runner(MODEL_NAME, task='embed', max_model_len=MAX_MODEL_LEN) as vllm_model:
...
async def test_gritlm_api_server_embedding():
    ...
    with RemoteOpenAIServer(MODEL_NAME, args) as server:
        ...
        d_rep = await run_client_embeddings(client_embedding, documents, d_instruction)
        q_rep = await run_client_embeddings(client_embedding, queries, q_instruction)
    validate_embed_output(q_rep, d_rep)

def test_gritlm_offline_generate(monkeypatch: pytest.MonkeyPatch, vllm_runner):
    input = '<|user|>\nWhat is the capital of France?\n<|assistant|>\n'
    with vllm_runner(MODEL_NAME, task='generate', max_model_len=MAX_MODEL_LEN) as vllm_model:
        llm = vllm_model.model
...
async def test_gritlm_api_server_generate():
    ...
    with RemoteOpenAIServer(MODEL_NAME, args) as server:
        ...
        outputs = await client_generate.completions.create(model=MODEL_NAME, prompt=input, max_tokens=256, temperature=0.0)
