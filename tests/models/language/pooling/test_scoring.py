...
CROSS_ENCODER_MODELS = ['cross-encoder/ms-marco-MiniLM-L-6-v2', 'BAAI/bge-reranker-v2-m3']
...
def test_cross_encoder_1_to_1(vllm_runner, hf_runner, model_name):
    ...
    with vllm_runner(model_name, task='score', dtype=DTYPE, max_model_len=None) as vllm_model:
...
def test_embedding_1_to_1(vllm_runner, hf_runner, emb_model_name):
    ...
    with vllm_runner(emb_model_name, task='embed', dtype=DTYPE, max_model_len=None) as vllm_model:
        vllm_outputs = vllm_model.score(text_pair[0], text_pair[1])
    assert len(vllm_outputs) == 1
    assert len(hf_outputs) == 1
    assert hf_outputs[0] == pytest.approx(vllm_outputs[0], rel=0.01)
