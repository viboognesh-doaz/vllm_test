...
RERANK_MODELS = [RerankModelInfo('BAAI/bge-reranker-v2-gemma', architecture='GemmaForSequenceClassification')]
    ...
    def predict(self, prompts: list[list[str]], *args, **kwargs) -> torch.Tensor:
        ...
        def get_inputs(pairs, tokenizer, prompt=None):
            ...
            for query, passage in pairs:
                query_inputs = tokenizer(f'A: {query}', return_tensors=None, add_special_tokens=False, truncation=True)
                passage_inputs = tokenizer(f'B: {passage}', return_tensors=None, add_special_tokens=False, truncation=True)
                item = tokenizer.prepare_for_model([tokenizer.bos_token_id] + query_inputs['input_ids'], sep_inputs + passage_inputs['input_ids'], truncation='only_second', padding=False, return_attention_mask=False, return_token_type_ids=False, add_special_tokens=False)
...
def test_rerank_models_mteb(vllm_runner, model_info: RerankModelInfo, monkeypatch) -> None:
    monkeypatch.setenv('VLLM_USE_V1', '0')
    assert model_info.architecture == 'GemmaForSequenceClassification'
    vllm_extra_kwargs: dict[str, Any] = {'hf_overrides': {'architectures': ['GemmaForSequenceClassification'], 'classifier_from_token': ['Yes'], 'method': 'no_post_processing'}}
    mteb_test_rerank_models(GemmaRerankerHfRunner, vllm_runner, model_info, vllm_extra_kwargs, vllm_mteb_encoder=GemmaMtebEncoder)