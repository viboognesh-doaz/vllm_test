...
class ModelPair:
    model_gptq: str
model_pairs = [ModelPair(model_gptq='hxbgsyxh/opt-125m-4bit-128g')]
...
def test_models(vllm_runner, example_prompts, model_pair: ModelPair, dtype: str, max_tokens: int, num_logprobs: int) -> None:
    with vllm_runner(model_pair.model_gptq, dtype=dtype, quantization='bitblas') as bitblas_model:
    ...
    check_logprobs_close(outputs_0_lst=gptq_outputs, outputs_1_lst=bitblas_outputs, name_0='gptq', name_1='gptq_bitblas')