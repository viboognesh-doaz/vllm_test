...
def can_initialize(model_arch: str, monkeypatch: pytest.MonkeyPatch, EXAMPLE_MODELS: HfExampleModels):
    ...
    if model_arch in ('Glm4vForConditionalGeneration', 'GraniteSpeechForConditionalGeneration', 'KimiVLForConditionalGeneration'):
    ...
    def hf_overrides(hf_config: PretrainedConfig) -> PretrainedConfig:
        ...
        text_config.update({'num_layers': 1, 'num_hidden_layers': num_hidden_layers, 'num_experts': num_experts, 'num_experts_per_tok': 2, 'num_local_experts': num_experts, 'first_k_dense_replace': 0, 'n_routed_experts': num_experts, 'num_kv_shared_layers': 1})
    ...
    def _initialize_kv_caches_v1(self, vllm_config):
        ...
        scheduler_kv_cache_config = get_kv_cache_config(vllm_config, kv_cache_specs[0], 10 * GiB_bytes)
        return (1, 0, scheduler_kv_cache_config)
    with patch.object(V0LLMEngine, '_initialize_kv_caches', _initialize_kv_caches_v0), patch.object(V1EngineCore, '_initialize_kv_caches', _initialize_kv_caches_v1), monkeypatch.context() as m:
        if model_info.v0_only:
            m.setenv('VLLM_USE_V1', '0')
        if model_arch == 'Phi4FlashForCausalLM':
            m.setenv('VLLM_ATTENTION_BACKEND', 'DIFFERENTIAL_FLASH_ATTN')
        LLM(model_info.default, tokenizer=model_info.tokenizer, tokenizer_mode=model_info.tokenizer_mode, revision=model_info.revision, speculative_config={'model': model_info.speculative_model, 'num_speculative_tokens': 1} if model_info.speculative_model else None, trust_remote_code=model_info.trust_remote_code, max_model_len=model_info.max_model_len, gpu_memory_utilization=0.8, load_format='dummy', hf_overrides=hf_overrides)
