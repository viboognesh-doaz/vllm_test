...
def _create_neuron_model_runner(model: str, *args, **kwargs) -> NeuronModelRunner:
    ...
    vllm_config = VllmConfig(model_config=engine_config.model_config, parallel_config=engine_config.parallel_config, scheduler_config=engine_config.scheduler_config, device_config=engine_config.device_config)
...
def test_update_neuron_sampling_params_not_full_batch():
    ...
    model_runner = _create_neuron_model_runner('facebook/opt-125m', seed=0, dtype='float16', max_num_seqs=2)
    ...
    if current_platform.use_transformers_neuronx():
        ...
        seq_group_metadata_list = [SequenceGroupMetadata(request_id='test_0', is_prompt=True, seq_data={0: SequenceData.from_seqs([1, 2, 3])}, sampling_params=SamplingParams(temperature=0.5, top_k=1, top_p=0.5), block_tables={0: [1]})]
        ...
        assert neuron_sampling_params.top_k == [model_runner._MAX_NEURON_SAMPLING_TOP_K, 1]
        assert neuron_sampling_params.top_p == [1.0, 0.5]
        model_mock.model.update_generation_config.assert_called_once_with(neuron_sampling_params)
