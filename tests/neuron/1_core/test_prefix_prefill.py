...
from vllm.attention.ops.nki_flash_attn import flash_attn_varlen_nkifunc, reorder_context_mask
...
def ref_masked_attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, scale: float, attn_mask: Optional[torch.Tensor]=None, return_max_reduce: Optional[bool]=False) -> torch.Tensor:
    ...
    if return_max_reduce:
        return (out, cached_max, cached_sum_reciprocal, norm_score, masked_score, scaled_qk)
    else:
        return (out,)

def ref_context_attention(query, key, value, query_lens, seq_lens, head_size, num_queries_per_kv, return_max_reduce=False):
    ...
    output, *debug_tensors = ref_masked_attention(query, key, value, scale, attn_mask, return_max_reduce=return_max_reduce)
    output = output.unsqueeze(1)
    if return_max_reduce:
        cached_max, cached_sum_reciprocal, lse, masked_score, scaled_qk = debug_tensors
        return (output, cached_max, cached_sum_reciprocal, lse, masked_score, scaled_qk)
    else:
        return output

def sample_inputs(prefill_batch_size, decode_batch_size, min_query_len, max_query_len, min_ctx_len, max_ctx_len, block_size, num_heads, num_kv_heads, head_size, dtype):
    ...
    query_lens = torch.randint(min_query_len, max_query_len + 1, (prefill_batch_size,), dtype=torch.long).tolist() + [1 for _ in range(decode_batch_size)]
    ...
    return (query, k, v, kv_cache, block_table, key, value, query_lens, seq_lens)
...
def get_active_block_tables(block_tables, query_lens, seq_lens, block_size, num_blocks):
    ...
    return F.pad(torch.tensor(active_blocks, dtype=torch.int32), (0, num_blocks - len(active_blocks)), 'constant', 0)

@pytest.mark.parametrize('prefill_batch_size,decode_batch_size,block_size,large_tile_size,num_heads,num_queries_per_kv,head_size,mixed_precision', [(1, 199, 1, 512, 4, 2, 8, False), (1, 199, 1, 512, 4, 2, 8, True), (4, 12, 32, 2048, 32, 8, 64, False), (4, 12, 32, 2048, 16, 4, 32, True), (4, 12, 256, 8192, 8, 1, 128, False), (4, 12, 256, 8192, 64, 8, 64, True), (2, 24, 64, 4096, 12, 4, 96, False), (8, 8, 128, 2048, 24, 2, 48, True), (1, 128, 16, 1024, 4, 2, 16, False), (16, 4, 8, 1024, 4, 2, 128, True), (4, 12, 32, 2048, 16, 1, 32, True), (4, 12, 32, 2048, 16, 16, 32, True)])
@torch.inference_mode()
def test_contexted_kv_attention(monkeypatch: pytest.MonkeyPatch, prefill_batch_size: int, decode_batch_size: int, num_heads: int, num_queries_per_kv: int, head_size: int, block_size: int, large_tile_size, mixed_precision: bool) -> None:
    assert large_tile_size % block_size == 0
    device = xm.xla_device()
    compiler_flags_str = ' '.join(['-O1', '--retry_failed_compilation'])
    with monkeypatch.context() as m:
        ...
        active_block_table = get_active_block_tables(block_table.cpu(), torch.tensor(query_lens).cpu(), torch.tensor(seq_lens).cpu(), block_size, num_active_blocks)
        prior_mask, active_mask = BlockDiagonalCausalFromBottomRightMask.from_seqlens(query_lens, seq_lens, block_size=block_size)
        prior_mask_padded = F.pad(prior_mask, (0, context_kv_len - prior_mask.shape[1], 0, max_num_queries - prior_mask.shape[0]), 'constant', 0).bool()
        ...
        input_args = (query.to(device=device), k.to(device=device), v.to(device=device), kv_cache.to(device=device), active_block_table.to(device=device), attn_mask.to(device=device))
        input_kwargs = dict(n_kv_head=num_kv_heads, head_size=head_size, mixed_precision=mixed_precision, LARGE_TILE_SZ=large_tile_size)
        ...
        torch.testing.assert_close(output_nki, output_ref, atol=0.01, rtol=0)