...
def all_gather_test_worker(index, tp_degree, distributed_init_method):
    init_distributed_environment(tp_degree, index, distributed_init_method, index, backend='xla')
    ...
    all_tensors = [torch.arange(total_size, dtype=torch.float32, device='xla').reshape(tensor_size) * (r + 1) for r in range(tp_degree)]
...
def all_reduce_test_worker(index, tp_degree, distributed_init_method):
    ...
    ensure_model_parallel_initialized(tp_degree, 1)
    num_elements = 8
    all_tensors = [torch.arange(num_elements, dtype=torch.float32, device='xla') * (r + 1) for r in range(tp_degree)]
...
@pytest.mark.parametrize('test_target', [all_reduce_test_worker, all_gather_test_worker])
...
def test_neuron_multi_process_tensor_parallel(monkeypatch, tp_size, test_target):
    with patch('torch_xla._XLAC._xla_runtime_is_initialized', return_value=False):
        ...
        monkeypatch.setenv('NEURON_PJRT_PROCESSES_NUM_DEVICES', ','.join(['1' for _ in range(tp_size)]))
