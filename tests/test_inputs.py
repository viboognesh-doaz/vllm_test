...
import pytest
STRING_INPUTS = ['', 'foo', 'foo bar', 'foo baz bar', 'foo bar qux baz']
TOKEN_INPUTS = [[-1], [1], [1, 2], [1, 3, 4], [1, 2, 4, 3]]
INPUTS_SLICES = [slice(None, None, -1), slice(None, None, 2), slice(None, None, -2)]
...
def test_parse_single_batch_string_consistent(string_input: str):
    assert parse_and_batch_prompt(string_input) == parse_and_batch_prompt([string_input])
...
def test_parse_single_batch_string_slice(inputs_slice: slice):
    assert parse_and_batch_prompt(STRING_INPUTS)[inputs_slice] == parse_and_batch_prompt(STRING_INPUTS[inputs_slice])

@pytest.mark.parametrize('mm_processor_kwargs,expected_mm_kwargs', [(None, [{}, {}]), ({}, [{}, {}]), ({'foo': 100}, [{'foo': 100}, {'foo': 100}]), ([{'foo': 100}, {'bar': 200}], [{'foo': 100}, {'bar': 200}])])
