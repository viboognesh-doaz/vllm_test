...
def test_enable_prompt_embeds(hf_runner, model: str, enable_prompt_embeds: bool):
    ...
    ctx = nullcontext() if enable_prompt_embeds else pytest.raises(ValueError, match='set `--enable-prompt-embeds`')
    llm = LLM(model=model, enable_prompt_embeds=enable_prompt_embeds, enforce_eager=True)
    with ctx:
