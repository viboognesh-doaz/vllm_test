...
from vllm.sequence import CompletionSequenceGroupOutput, Logprob, SequenceOutput, SequenceStatus
...
def test_appends_token_ids(num_new_tokens: int, seq_output_len: int):
    ...
    seq_counter = Counter()
    output_processor = MultiStepOutputProcessor(detokenizer=detokenizer, scheduler=[scheduler], seq_counter=seq_counter, get_tokenizer_for_seq=lambda _: mock_tokenizer(), stop_checker=stop_checker)
    seq_group = create_seq_group(seq_prompt_len=1024, seq_output_lens=[seq_output_len], sampling_params=SamplingParams(max_tokens=seq_output_len + num_new_tokens, ignore_eos=True))
    seq = seq_group.get_seqs()[0]
    seq.status = SequenceStatus.RUNNING
    new_token_ids = list(range(num_new_tokens))
    outputs = [CompletionSequenceGroupOutput(samples=[SequenceOutput(parent_seq_id=seq.seq_id, output_token=output_token, logprobs={output_token: Logprob(0.0)})], prompt_logprobs=None) for output_token in new_token_ids]
...
def test_respects_max_tokens(num_new_tokens: int, seq_prompt_len: int, seq_output_len: int, max_tokens: int):
    ...
    seq_counter = Counter()
    output_processor = MultiStepOutputProcessor(detokenizer=detokenizer, scheduler=[scheduler], seq_counter=seq_counter, get_tokenizer_for_seq=lambda _: mock_tokenizer(), stop_checker=stop_checker)
    seq_group = create_seq_group(seq_prompt_len=seq_prompt_len, seq_output_lens=[seq_output_len], sampling_params=SamplingParams(max_tokens=max_tokens))
    seq = seq_group.get_seqs()[0]
    seq.status = SequenceStatus.RUNNING
    new_token_ids = list(range(num_new_tokens))
    outputs = [CompletionSequenceGroupOutput(samples=[SequenceOutput(parent_seq_id=seq.seq_id, output_token=output_token, logprobs={output_token: Logprob(0.0)})], prompt_logprobs=None) for output_token in new_token_ids]
...
def test_respects_eos_token_id(num_new_tokens: int, seq_prompt_len: int, seq_output_len: int, seed: int):
    ...
    seq_counter = Counter()
    eos_token_id = 100
    output_processor = MultiStepOutputProcessor(detokenizer=detokenizer, scheduler=[scheduler], seq_counter=seq_counter, get_tokenizer_for_seq=lambda _: mock_tokenizer(eos_token_id), stop_checker=stop_checker)
    seq_group = create_seq_group(seq_prompt_len=seq_prompt_len, seq_output_lens=[seq_output_len], sampling_params=SamplingParams(max_tokens=seq_output_len + num_new_tokens))
    seq = seq_group.get_seqs()[0]
    seq.status = SequenceStatus.RUNNING
    new_token_ids = list(range(num_new_tokens))
...
def test_ignores_eos_token_id(num_new_tokens: int, seq_prompt_len: int, seq_output_len: int, seed: int):
    ...
    eos_token_id = 100
    output_processor = MultiStepOutputProcessor(detokenizer=detokenizer, scheduler=[scheduler], seq_counter=seq_counter, get_tokenizer_for_seq=lambda _: mock_tokenizer(eos_token_id), stop_checker=stop_checker)
    seq_group = create_seq_group(seq_prompt_len=seq_prompt_len, seq_output_lens=[seq_output_len], sampling_params=SamplingParams(max_tokens=seq_output_len + num_new_tokens, ignore_eos=True))
    seq = seq_group.get_seqs()[0]
    seq.status = SequenceStatus.RUNNING
    new_token_ids = list(range(num_new_tokens))
    assert eos_token_id not in new_token_ids
