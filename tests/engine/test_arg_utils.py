...
from vllm.engine.arg_utils import EngineArgs, contains_type, get_kwargs, get_type, get_type_hints, is_not_builtin, is_type, literal_to_kwargs, optional_type, parse_type
from vllm.utils import FlexibleArgumentParser
import json
import pytest

@pytest.mark.parametrize(('type', 'value', 'expected'), [(int, '42', 42), (float, '3.14', 3.14), (str, 'Hello World!', 'Hello World!'), (json.loads, '{"foo":1,"bar":2}', {'foo': 1, 'bar': 2})])
...
@pytest.mark.parametrize(('type_hint', 'type', 'expected'), [(int, int, True), (int, float, False), (list[int], list, True), (list[int], tuple, False), (Literal[0, 1], Literal, True)])
def test_is_type(type_hint, type, expected):
    assert is_type(type_hint, type) == expected

@pytest.mark.parametrize(('type_hints', 'type', 'expected'), [({float, int}, int, True), ({int, tuple[int]}, int, True), ({int, tuple[int]}, float, False), ({str, Literal['x', 'y']}, Literal, True)])
def test_contains_type(type_hints, type, expected):
    assert contains_type(type_hints, type) == expected

@pytest.mark.parametrize(('type_hints', 'type', 'expected'), [({int, float}, int, int), ({int, float}, str, None), ({str, Literal['x', 'y']}, Literal, Literal['x', 'y'])])
def test_get_type(type_hints, type, expected):
    assert get_type(type_hints, type) == expected

@pytest.mark.parametrize(('type_hints', 'expected'), [({Literal[1, 2]}, {'type': int, 'choices': [1, 2]}), ({Literal[1, 'a']}, Exception)])
...
@pytest.mark.parametrize(('type_hint', 'expected'), [(Annotated[int, 'annotation'], {int}), (Optional[int], {int, type(None)}), (Annotated[Optional[int], 'annotation'], {int, type(None)}), (Optional[Annotated[int, 'annotation']], {int, type(None)})], ids=['Annotated', 'Optional', 'Annotated_Optional', 'Optional_Annotated'])
...
@pytest.mark.parametrize(('arg', 'expected'), [(None, dict()), ('{"video": {"num_frames": 123} }', {'video': {'num_frames': 123}}), ('{"video": {"num_frames": 123, "fps": 1.0, "foo": "bar"}, "image": {"foo": "bar"} }', {'video': {'num_frames': 123, 'fps': 1.0, 'foo': 'bar'}, 'image': {'foo': 'bar'}})])
...
def test_compilation_config():
    ...
    args = parser.parse_args(['-O', '{"level": 3, "cudagraph_capture_sizes": [1, 2, 4, 8], "use_inductor": false}'])
    assert args.compilation_config.level == 3 and args.compilation_config.cudagraph_capture_sizes == [1, 2, 4, 8] and (not args.compilation_config.use_inductor)
    args = parser.parse_args(['--compilation-config={"level": 3, "cudagraph_capture_sizes": [1, 2, 4, 8], "use_inductor": true}'])
    assert args.compilation_config.level == 3 and args.compilation_config.cudagraph_capture_sizes == [1, 2, 4, 8] and args.compilation_config.use_inductor
...
def test_human_readable_model_len():
    ...
    assert args.max_model_len == 1000000
    args = parser.parse_args(['--max-model-len', '10k'])
    assert args.max_model_len == 10000
