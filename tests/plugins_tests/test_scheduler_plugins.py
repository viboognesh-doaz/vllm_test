...
def test_scheduler_plugins_v0(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        ...
        with pytest.raises(Exception) as exception_info:
            engine_args = EngineArgs(model='facebook/opt-125m', enforce_eager=True, scheduler_cls=DummyV0Scheduler)
...
def test_scheduler_plugins_v1(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        ...
        with pytest.raises(Exception) as exception_info:
            engine_args = EngineArgs(model='facebook/opt-125m', enforce_eager=True, scheduler_cls=DummyV1Scheduler)
