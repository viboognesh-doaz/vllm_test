...
MODEL_QUANT = [('ModelCloud/Qwen1.5-1.8B-Chat-GPTQ-4bits-dynamic-cfg-with-lm_head-symTrue', True), ('ModelCloud/Qwen1.5-1.8B-Chat-GPTQ-4bits-dynamic-cfg-with-lm_head-symFalse', False)]

@pytest.mark.parametrize('model_id, use_marlin_kernel', MODEL_QUANT)
def test_gptq_with_dynamic(vllm_runner, model_id: str, use_marlin_kernel: bool, monkeypatch):
    ...
    for name, submodule in vllm_model.model.llm_engine.model_executor.driver_worker.model_runner.model.named_modules():
        if name == 'lm_head':
