...
@pytest.mark.skipif(not is_quant_method_supported('ptpc_fp8'), reason='PTPC FP8 is not supported on this GPU type.')
...
def test_ptpc_fp8_rocm(vllm_runner, dtype: str, kv_cache_dtype: str) -> None:
    try:
        with vllm_runner('facebook/opt-125m', dtype=dtype, quantization='ptpc_fp8', kv_cache_dtype=kv_cache_dtype) as llm:
        ...
        if str(e) == 'Currently torch._scaled_mm (hipBLASLt) rowwise gemm only support output dtype of bfloat16. torch.float16 is specified.':
            pass
        else:
            raise