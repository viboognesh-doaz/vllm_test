...
from vllm.model_executor.layers.quantization.quark.quark import QuarkLinearMethod, QuarkW8A8Fp8, QuarkW8A8Int8
...
QUARK_MXFP4_AVAILABLE = importlib.util.find_spec('quark') is not None and version.parse(importlib.metadata.version('amd-quark')) >= version.parse('0.8.99')
...
def test_quark_fp8_w_per_tensor_a_per_tensor(vllm_runner, kv_cache_dtype, tp):
    ...
    with vllm_runner(model_path, kv_cache_dtype=kv_cache_dtype, tensor_parallel_size=tp) as llm:
...
def test_quark_fp8_parity(vllm_runner):
    ...
    llm_kwargs = {'tensor_parallel_size': 1, 'enforce_eager': True, 'gpu_memory_utilization': 0.1}
    with vllm_runner(quark_model_id, **llm_kwargs) as quark_handle, vllm_runner(fp8_model_id, **llm_kwargs) as fp8_handle:
        ...
        fp8_model = fp8_handle.model.llm_engine.model_executor.driver_worker.model_runner.model
...
class GSM8KAccuracyTestConfig:
    ...
    def get_model_args(self) -> str:
        return f'pretrained={self.model_name},dtype=auto,add_bos_token=True,tensor_parallel_size=8,gpu_memory_utilization=0.7,max_model_len=38768'
ACCURACY_CONFIGS = [GSM8KAccuracyTestConfig(model_name='amd/DeepSeek-R1-WMXFP4-AMXFP4-Scale-UINT8-MoE-Quant', excepted_value=0.96)]

@pytest.mark.parametrize('config', ACCURACY_CONFIGS)
@pytest.mark.skipif(not QUARK_MXFP4_AVAILABLE, reason='amd-quark>=0.9 is not available')
@pytest.mark.skipif(not HF_HUB_AMD_ORG_ACCESS, reason='Read access to huggingface.co/amd is required for this test.')
def test_mxfp4_gsm8k_correctness(config: GSM8KAccuracyTestConfig):
    ...
    results = lm_eval.simple_evaluate(model='vllm', model_args=config.get_model_args(), tasks=task, batch_size=64, num_fewshot=8)
...
@pytest.mark.parametrize('scalings', [[2.3, 0.03, 7.3, 0.1, 0.004, 17.3, 10000.0, 0.0001]])
...
def test_mxfp4_dequant_kernel_match_quark(float_dtype: torch.dtype, scalings: list[int]):
    qspec = FP4PerGroupSpec(ch_axis=-1, group_size=32, scale_format='e8m0', scale_calculation_mode='even', is_dynamic=False).to_quantization_spec()
    weight_quantizer = StaticScaledRealQuantizer(qspec=qspec, quantizer=None, reorder=False, real_quantized=True, float_dtype=float_dtype, device='cuda')
