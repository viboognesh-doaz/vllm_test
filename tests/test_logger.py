...
from vllm.logger import _DATE_FORMAT, _FORMAT, _configure_vllm_root_logger, enable_trace_function_call, init_logger
...
@patch('vllm.logger.VLLM_CONFIGURE_LOGGING', 1)
@patch('vllm.logger.VLLM_LOGGING_CONFIG_PATH', '/if/there/is/a/file/here/then/you/did/this/to/yourself.json')
...
@patch('vllm.logger.VLLM_CONFIGURE_LOGGING', 1)
@pytest.mark.parametrize('unexpected_config', ('Invalid string', [{'version': 1, 'loggers': []}], 0))
def test_an_error_is_raised_when_custom_logging_config_is_unexpected_json(unexpected_config: Any):
    """This test calls _configure_vllm_root_logger again to test custom logging
    config behavior, however it fails before any change in behavior or
    configuration occurs."""
    with NamedTemporaryFile(encoding='utf-8', mode='w') as logging_config_file:
...
def test_custom_logging_config_is_parsed_and_used_when_provided():
    ...
    config behavior, however mocks are used to ensure no changes in behavior or
    configuration occur."""
    valid_logging_config = {'loggers': {'vllm.test_logger.logger': {'handlers': [], 'propagate': False}}, 'version': 1}
    with NamedTemporaryFile(encoding='utf-8', mode='w') as logging_config_file:
        logging_config_file.write(json.dumps(valid_logging_config))
        logging_config_file.flush()
        with patch('vllm.logger.VLLM_LOGGING_CONFIG_PATH', logging_config_file.name), patch('vllm.logger.dictConfig') as dict_config_mock:
            _configure_vllm_root_logger()
...
def test_custom_logging_config_causes_an_error_if_configure_logging_is_off():
    ...
    config behavior, however mocks are used to ensure no changes in behavior or
    configuration occur."""
    valid_logging_config = {'loggers': {'vllm.test_logger.logger': {'handlers': []}}, 'version': 1}
    with NamedTemporaryFile(encoding='utf-8', mode='w') as logging_config_file:
        logging_config_file.write(json.dumps(valid_logging_config))
        logging_config_file.flush()
        with patch('vllm.logger.VLLM_LOGGING_CONFIG_PATH', logging_config_file.name):
            ...
            expected_message_snippet = 'VLLM_CONFIGURE_LOGGING evaluated to false, but VLLM_LOGGING_CONFIG_PATH was given.'
