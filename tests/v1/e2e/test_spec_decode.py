...
def test_eagle_correctness(monkeypatch: pytest.MonkeyPatch, test_prompts: list[list[dict[str, Any]]], sampling_config: SamplingParams, model_setup: tuple[str, str, str, int]):
    ...
    with monkeypatch.context() as m:
        ...
        torch.cuda.empty_cache()
        cleanup_dist_env_and_memory()
        spec_llm = LLM(model=model_name, trust_remote_code=True, tensor_parallel_size=tp_size, speculative_config={'method': method, 'model': spec_model_name, 'num_speculative_tokens': 3, 'max_model_len': 2048}, max_model_len=2048)
        spec_outputs = spec_llm.chat(test_prompts, sampling_config)
        matches = 0
        misses = 0
        for ref_output, spec_output in zip(ref_outputs, spec_outputs):
