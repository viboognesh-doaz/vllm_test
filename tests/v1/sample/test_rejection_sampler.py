...
def create_sampling_metadata(all_greedy: bool, temperature: Optional[torch.Tensor]=None, top_k: Optional[torch.Tensor]=None, top_p: Optional[torch.Tensor]=None, generators: Optional[dict[int, Any]]=None) -> SamplingMetadata:
...
def test_perfect_match(rejection_sampler):
    ...
    output = rejection_sampler(spec_decode_metadata, draft_probs=None, target_logits=logits, bonus_token_ids=bonus_token_tensor, sampling_metadata=metadata)
...
def test_early_mismatch(rejection_sampler):
    ...
    spec_decode_metadata = SpecDecodeMetadata.make_dummy(spec_tokens, device=logits.device)
    output = rejection_sampler(spec_decode_metadata, draft_probs=None, target_logits=logits, bonus_token_ids=bonus_token_tensor, sampling_metadata=metadata)
    expected = torch.tensor([[1, 5, PLACEHOLDER_TOKEN_ID, PLACEHOLDER_TOKEN_ID]], dtype=torch.int, device=logits.device)
    assert torch.equal(output, expected)
...
def test_multiple_mismatches(rejection_sampler):
    ...
    output_tokens = [[1, 2, 7, 6], [4, 8, 6, 9]]
    metadata = create_sampling_metadata(all_greedy=True)
    logits = create_logits_tensor(output_tokens)
    bonus_token_tensor = torch.tensor([output_tokens[0][-1], output_tokens[1][-1]], device=logits.device)
    spec_decode_metadata = SpecDecodeMetadata.make_dummy(spec_tokens, device=logits.device)
    output = rejection_sampler(spec_decode_metadata, draft_probs=None, target_logits=logits, bonus_token_ids=bonus_token_tensor, sampling_metadata=metadata)
    expected = torch.tensor([[1, 2, 7, PLACEHOLDER_TOKEN_ID], [4, 8, PLACEHOLDER_TOKEN_ID, PLACEHOLDER_TOKEN_ID]], dtype=torch.int, device=logits.device)
    assert torch.equal(output, expected)

@pytest.mark.parametrize('spec_tokens,output_tokens,expected', [([[1, 2]], [[1, 2, 3]], [[1, 2, 3]]), ([[1]], [[2, 3]], [[2, PLACEHOLDER_TOKEN_ID]]), ([[1, 2], [3, 4]], [[1, 5, 6], [3, 4, 7]], [[1, 5, PLACEHOLDER_TOKEN_ID], [3, 4, 7]])])
...
def test_deterministic_when_seeded(rejection_sampler, k: int, vocab_size: int, batch_size: int, frac_seeded: float, n_rep: int):
    ...
    for _ in range(n_rep):
        ...
        spec_decode_metadata = SpecDecodeMetadata.make_dummy(draft_token_ids.tolist(), device=DEVICE)
        rep_result = rejection_sampler(spec_decode_metadata, draft_probs=draft_probs, target_logits=target_logits, bonus_token_ids=bonus_token_ids, sampling_metadata=sampling_metadata)
...
def test_rejection_sampling_approximates_target_distribution():
    ...
    reference_probs = F.softmax(torch.rand(num_reference_probs, vocab_size, dtype=torch.float32), dim=-1)
...
def estimate_rejection_sampling_pdf(draft_probs: torch.Tensor, target_logits: torch.Tensor, k: int, vocab_size: int, num_samples: int) -> torch.Tensor:
    ...
    spec_decode_metadata = SpecDecodeMetadata.make_dummy(draft_token_ids.tolist(), device=bonus_token_ids.device)
    output_token_ids = rejection_sampler(spec_decode_metadata, draft_probs=draft_probs, target_logits=target_logits, bonus_token_ids=bonus_token_ids, sampling_metadata=sampling_metadata)
    output_token_ids = output_token_ids[:, :-1].flatten()
    hist = torch.histogram(output_token_ids.to(dtype=torch.float, device='cpu'), bins=vocab_size, range=(0, vocab_size), density=True)
    return hist.hist

def _test_masked_logits(rejection_sampler, batch_size: int, num_draft_tokens: int, vocab_size: int, target_logits: torch.Tensor, unmasked_indices: torch.Tensor, sampling_metadata: SamplingMetadata):
    ...
    spec_decode_metadata = SpecDecodeMetadata.make_dummy(draft_token_ids, device=DEVICE)
    output_token_ids = rejection_sampler(spec_decode_metadata, draft_probs=draft_probs, target_logits=target_logits, bonus_token_ids=bonus_token_ids, sampling_metadata=sampling_metadata)
...
def test_top_k(rejection_sampler, top_k):
    ...
    temperature = torch.ones(batch_size, dtype=torch.float32, device=DEVICE)
    sampling_metadata = create_sampling_metadata(all_greedy=False, temperature=temperature, top_k=torch.tensor([top_k] * batch_size, device=DEVICE, dtype=torch.int64))
    _test_masked_logits(rejection_sampler, batch_size=batch_size, num_draft_tokens=num_draft_tokens, vocab_size=vocab_size, target_logits=target_logits, unmasked_indices=top_k_indices, sampling_metadata=sampling_metadata)
...
def test_top_p(rejection_sampler, top_p):
    ...
    sampling_metadata = create_sampling_metadata(all_greedy=False, temperature=temperature, top_p=torch.tensor([top_p] * batch_size, device=DEVICE, dtype=torch.float32))
    _test_masked_logits(rejection_sampler, batch_size=batch_size, num_draft_tokens=num_draft_tokens, vocab_size=vocab_size, target_logits=target_logits, unmasked_indices=top_p_indices, sampling_metadata=sampling_metadata)