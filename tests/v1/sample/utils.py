...
def create_prompt_tokens_tensor(prompt_token_ids: list[list[int]], vocab_size: int, device: torch.device) -> torch.Tensor:
...
class LogitsprocsTestFakes(NamedTuple):
    ...
    def get_logitsprocs_by_cls(self, cls: type[LogitsProcessor]) -> Iterator[LogitsProcessor]:
...
def fake_update_logitsprocs_state(test_fakes: LogitsprocsTestFakes, batch_update: BatchUpdate) -> None:
...
def fake_apply_logitsprocs(test_fakes: LogitsprocsTestFakes, slice_indices: list[int]) -> torch.Tensor:
