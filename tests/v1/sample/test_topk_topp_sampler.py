...
from vllm.v1.sample.ops.topk_topp_sampler import apply_top_k_top_p, is_flashinfer_available
import pytest
import torch
...
def test_flashinfer_sampler():
    ...
    p_values = torch.rand((BATCH_SIZE,), generator=generator) * 0.5 + 0.5
    k_values.masked_fill_(torch.randint(0, 2, (BATCH_SIZE,), generator=generator, dtype=torch.bool), VOCAB_SIZE)
    p_values.masked_fill_(torch.randint(0, 2, (BATCH_SIZE,), generator=generator, dtype=torch.bool), 1.0)
    python_logits = apply_top_k_top_p(logits=logits.clone(), k=k_values, p=p_values)
    ...
    assert torch.allclose(python_probs, flashinfer_probs, atol=0.02), 'FlashInfer and Python sampling implementations do not match!'