...
def vllm_model(vllm_runner, request) -> Generator[VllmRunner, None, None]:
    with vllm_runner(MODEL, dtype=DTYPE, max_logprobs=7, max_num_batched_tokens=16, max_num_seqs=16, max_model_len=128, enforce_eager=True, enable_prefix_caching=request.param, gpu_memory_utilization=0.4) as vllm_model:
        yield vllm_model
...
def _run_and_validate(vllm_model: VllmRunner, test_prompts: list[str], vllm_sampling_params: SamplingParams, hf_logprobs: list[list[torch.Tensor]], hf_outputs: list[tuple[list[int], str]], logprob_prompt_logprob_list: BatchLogprobsSpecType, temperature: float, max_tokens: int, do_apc: bool) -> None:
    vllm_results = vllm_model.model.generate(test_prompts, sampling_params=vllm_sampling_params)
    for vllm_result, hf_logprob, hf_output, logprob_prompt_logprob in zip(vllm_results, hf_logprobs, hf_outputs, logprob_prompt_logprob_list):
        num_top_logprobs, num_top_prompt_logprobs = logprob_prompt_logprob
        if temperature == 0.0:
            assert vllm_result.prompt_token_ids + vllm_result.outputs[0].token_ids == hf_output[0]
        else:
            assert vllm_result.prompt_token_ids == hf_output[0][:len(vllm_result.prompt_token_ids)]
        if num_top_logprobs is not None:
            ...
            for logprobs, token_id in zip(vllm_result.outputs[0].logprobs, vllm_result.outputs[0].token_ids):
            ...
            torch.testing.assert_close(vllm_result.outputs[0].cumulative_logprob, compute_correct_cumulative_logprob(vllm_result.outputs[0]), atol=1e-06, rtol=1e-06)
        else:
        ...
        if num_top_prompt_logprobs is not None:
            ...
            for prompt_logprobs, prompt_token_id in zip(vllm_result.prompt_logprobs[1:], vllm_result.prompt_token_ids[1:]):
...
def test_get_logprobs_and_prompt_logprobs(hf_model, vllm_model, batch_logprobs_composition: BatchLogprobsComposition, temperature: float, example_prompts: list[str], monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    * The generated (prompt)logprobs are consistent with HuggingFace
    ...
    with monkeypatch.context() as m:
        ...
        vllm_sampling_params = [SamplingParams(max_tokens=max_tokens, logprobs=num_lp, prompt_logprobs=num_plp, temperature=temperature, seed=1984) for num_lp, num_plp in logprob_prompt_logprob_list]
        for _ in range(2 if do_apc else 1):
            _run_and_validate(vllm_model=vllm_model, test_prompts=test_prompts, vllm_sampling_params=vllm_sampling_params, hf_logprobs=hf_logprobs, hf_outputs=hf_outputs, logprob_prompt_logprob_list=logprob_prompt_logprob_list, temperature=temperature, max_tokens=max_tokens, do_apc=do_apc)
...
def test_max_logprobs(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        m.setenv('VLLM_USE_V1', '1')
        runner = VllmRunner('facebook/opt-125m', max_logprobs=1, enable_prefix_caching=False, gpu_memory_utilization=0.15, max_model_len=256)
...
def test_none_logprobs(vllm_model, example_prompts, monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        results_logprobs_none = vllm_model.model.generate(example_prompts, sampling_params=sampling_params_logprobs_none)
...
def test_zero_logprobs(vllm_model, example_prompts, monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        results_logprobs_zero = vllm_model.model.generate(example_prompts, sampling_params=sampling_params_logprobs_zero)
