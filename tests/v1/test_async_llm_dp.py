...
engine_args = AsyncEngineArgs(model='ibm-research/PowerMoE-3b', enforce_eager=True, disable_log_requests=True, tensor_parallel_size=int(os.getenv('TP_SIZE', 1)), data_parallel_size=DP_SIZE)
...
async def generate(engine: AsyncLLM, request_id: str, prompt: PromptType, output_kind: RequestOutputKind, max_tokens: int, prompt_logprobs: Optional[int]=None, data_parallel_rank: Optional[int]=None) -> tuple[int, str]:
    await asyncio.sleep(0.2)
    count = 0
    sampling_params = SamplingParams(max_tokens=max_tokens, ignore_eos=True, output_kind=output_kind, temperature=0, prompt_logprobs=prompt_logprobs)
    async for out in engine.generate(request_id=request_id, prompt=prompt, sampling_params=sampling_params, data_parallel_rank=data_parallel_rank):
...
async def test_load(output_kind: RequestOutputKind, data_parallel_backend: str):
    ...
    with ExitStack() as after:
        ...
        for sl in stats_loggers.values():
            ...
            assert slogger.finished_req_count > NUM_REQUESTS // (DP_SIZE + 1), f'requests are imbalanced: {stats_loggers}'