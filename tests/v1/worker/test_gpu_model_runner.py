...
def initialize_kv_cache(runner: GPUModelRunner):
    ...
    """
    attn_spec = FullAttentionSpec(block_size=BLOCK_SIZE, num_kv_heads=runner.model_config.get_num_kv_heads(runner.parallel_config), head_size=runner.model_config.get_head_size(), dtype=runner.kv_cache_dtype, use_mla=False)
    tensor_size = attn_spec.page_size_bytes * NUM_BLOCKS
    kv_cache_config = KVCacheConfig(num_blocks=NUM_BLOCKS, kv_cache_tensors=[KVCacheTensor(size=tensor_size, shared_by=['layer.0'])], kv_cache_groups=[KVCacheGroupSpec(layer_names=['layer.0'], kv_cache_spec=attn_spec)])
    runner.kv_cache_config = kv_cache_config
    runner.input_batch = InputBatch(max_num_reqs=runner.max_num_reqs, max_model_len=runner.max_model_len, max_num_batched_tokens=runner.max_num_tokens, device=runner.device, pin_memory=runner.pin_memory, vocab_size=runner.model_config.get_vocab_size(), block_sizes=[kv_cache_config.kv_cache_groups[0].kv_cache_spec.block_size])
    runner.initialize_attn_backend(kv_cache_config)

def get_vllm_config():
    scheduler_config = SchedulerConfig(max_num_seqs=10, max_num_batched_tokens=512, max_model_len=512)
    model_config = ModelConfig(model='facebook/opt-125m', task='generate', tokenizer='facebook/opt-125m', tokenizer_mode='auto', trust_remote_code=True, dtype='float16', seed=42)
    cache_config = CacheConfig(block_size=BLOCK_SIZE, gpu_memory_utilization=0.9, swap_space=0, cache_dtype='auto')
    parallel_config = ParallelConfig()
    vllm_config = VllmConfig(model_config=model_config, cache_config=cache_config, scheduler_config=scheduler_config, parallel_config=parallel_config)
    return vllm_config
...
def _schedule_new_request(*req_ids: str) -> SchedulerOutput:
    ...
    for req_id in req_ids:
        new_reqs.append(NewRequestData(req_id=req_id, prompt_token_ids=[1, 2, 3], mm_inputs=[], mm_hashes=[], mm_positions=[], sampling_params=SamplingParams(), pooling_params=None, block_ids=([0],), num_computed_tokens=0, lora_request=None))
        num_scheduled_tokens[req_id] = 3
        total_num_scheduled_tokens += num_scheduled_tokens[req_id]
    return SchedulerOutput(scheduled_new_reqs=new_reqs, scheduled_cached_reqs=CachedRequestData.make_empty(), num_scheduled_tokens=num_scheduled_tokens, total_num_scheduled_tokens=total_num_scheduled_tokens, scheduled_spec_decode_tokens={}, scheduled_encoder_inputs={}, num_common_prefix_blocks=0, finished_req_ids=set(), free_encoder_input_ids=[], structured_output_request_ids={}, grammar_bitmask=None)
...
def test_update_states_request_finished(model_runner, dist_init):
    ...
    assert _is_req_added(model_runner, req_id)
    assert _is_req_scheduled(model_runner, req_id)
    scheduler_output = SchedulerOutput(scheduled_new_reqs=[], scheduled_cached_reqs=CachedRequestData.make_empty(), num_scheduled_tokens={}, total_num_scheduled_tokens=0, scheduled_spec_decode_tokens={}, scheduled_encoder_inputs={}, num_common_prefix_blocks=0, finished_req_ids={req_id}, free_encoder_input_ids=[], structured_output_request_ids={}, grammar_bitmask=None)
    metadata_before = model_runner.input_batch.sampling_metadata
    model_runner._update_states(scheduler_output)
    assert _is_sampling_metadata_changed(model_runner, metadata_before)
...
def test_update_states_request_resumed(model_runner, dist_init):
    ...
    assert _is_req_added(model_runner, req_id)
    assert _is_req_scheduled(model_runner, req_id)
    scheduler_output = SchedulerOutput(scheduled_new_reqs=[], scheduled_cached_reqs=CachedRequestData.make_empty(), num_scheduled_tokens={}, total_num_scheduled_tokens=0, scheduled_spec_decode_tokens={}, scheduled_encoder_inputs={}, num_common_prefix_blocks=0, finished_req_ids=set(), free_encoder_input_ids=[], structured_output_request_ids={}, grammar_bitmask=None)
    model_runner._update_states(scheduler_output)
    assert _is_req_added(model_runner, req_id)
    assert not _is_req_scheduled(model_runner, req_id)
    cached_req_data = CachedRequestData(req_ids=[req_id], resumed_from_preemption=[False], new_token_ids=[[]], new_block_ids=([[0]],), num_computed_tokens=[0])
    scheduler_output = SchedulerOutput(scheduled_new_reqs=[], scheduled_cached_reqs=cached_req_data, num_scheduled_tokens={req_id: 1}, total_num_scheduled_tokens=1, scheduled_spec_decode_tokens={}, scheduled_encoder_inputs={}, num_common_prefix_blocks=0, finished_req_ids=set(), free_encoder_input_ids=[], structured_output_request_ids={}, grammar_bitmask=None)
    metadata_before = model_runner.input_batch.sampling_metadata
    model_runner._update_states(scheduler_output)
    assert _is_sampling_metadata_changed(model_runner, metadata_before)
    assert _is_req_added(model_runner, req_id)
...
def test_get_nans_in_logits(model_runner, dist_init):
    ...
    model_runner._update_states(scheduler_output)
    logits = torch.tensor([[1.0, 2.0, 3.0], [3.0, 2.0, 1.0]], device=DEVICE)
    result = model_runner._get_nans_in_logits(logits)
    assert result == {'req_0': 0, 'req_1': 0}
    logits = torch.tensor([[1.0, float('nan'), 3.0], [4.0, float('nan'), float('nan')]], device=DEVICE)
    result = model_runner._get_nans_in_logits(logits)
    assert result == {'req_0': 1, 'req_1': 2}
    logits = torch.tensor([[1.0, 2.0, 3.0], [4.0, float('nan'), float('nan')]], device=DEVICE)
    result = model_runner._get_nans_in_logits(logits)
    assert result == {'req_0': 0, 'req_1': 2}
    result = model_runner._get_nans_in_logits(logits=None)
    assert result == {'req_0': 0, 'req_1': 0}
    logits = torch.tensor([[1.0, float('nan'), 3.0]], device=DEVICE)
    result = model_runner._get_nans_in_logits(logits)
    assert result == {'req_0': 1, 'req_1': 0}
    logits = torch.tensor([[float('nan'), float('nan'), 2.0], [1.0, 2.0, 3.0], [float('nan'), 2.0, 3.0]], device=DEVICE)
    result = model_runner._get_nans_in_logits(logits)
...
def test_kv_cache_stride_order(monkeypatch, model_runner):
    ...
    expected_kv_cache_shape = [2, NUM_BLOCKS, BLOCK_SIZE, n_heads, model_runner.model_config.get_head_size()]
...
def test_init_kv_cache_with_kv_sharing_invalid_target_layer_order():
    ...
    with pytest.raises(ValueError, match=error_msg):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_0, kv_sharing_target_layer_name=layer_1), layer_1: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_1)}
...
def test_init_kv_cache_with_kv_sharing_target_layer_not_exist():
    ...
    with pytest.raises(ValueError, match=error_msg):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_1, kv_sharing_target_layer_name=invalid_layer)}
        assert fwd_context is not None
...
def test_init_kv_cache_with_kv_sharing_target_same_as_current():
    ...
    with pytest.raises(ValueError, match=error_msg):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_1, kv_sharing_target_layer_name=layer_1)}
        assert fwd_context is not None
...
def test_init_kv_cache_without_kv_sharing():
    ...
    with set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_1)}
        assert fwd_context is not None
    vllm_config.model_config.max_model_len = 3000000
...
def test_init_kv_cache_with_kv_sharing_valid():
    ...
    vllm_config = get_vllm_config()
    with set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=64, scale=1.0, prefix=layer_1, kv_sharing_target_layer_name='model.layers.0.self_attn.attn')}
        assert fwd_context is not None
    vllm_config.model_config.max_model_len = 3000000
    vllm_ctx = vllm_config.compilation_config.static_forward_context
    runner = GPUModelRunner(vllm_config, DEVICE)
