...
def _construct_expected_sampling_metadata(reqs: list[CachedRequestState], req_ids_retained: set[int], req_id_index_in_input_batch: dict[str, int], device: torch.device) -> SamplingMetadata:
    """
...
def _construct_cached_request_state(req_id_suffix: int):
    ...
    return CachedRequestState(req_id=f'req_id_{req_id_suffix}', prompt_token_ids=prompt_token_ids, sampling_params=_create_sampling_params(), pooling_params=None, mm_inputs=[], mm_positions=[], block_ids=([],), generator=None, num_computed_tokens=len(output_token_ids), output_token_ids=output_token_ids)
...
def test_sampling_metadata_in_input_batch(device: str, batch_size: int):
    ...
    """
    input_batch: InputBatch = InputBatch(max_num_reqs=batch_size, max_model_len=1024, max_num_batched_tokens=1024, device=torch.device(device), pin_memory=is_pin_memory_available(), vocab_size=1024, block_sizes=[1])
    ...
    expected_sampling_metadata = _construct_expected_sampling_metadata(reqs, req_ids_retained, input_batch.req_id_to_index, device=torch.device(device))
    ...
    assert torch.allclose(expected_sampling_metadata.frequency_penalties, sampling_metadata.frequency_penalties)
    assert torch.allclose(expected_sampling_metadata.presence_penalties, sampling_metadata.presence_penalties)
    assert torch.allclose(expected_sampling_metadata.repetition_penalties, sampling_metadata.repetition_penalties)
...
def test_swap_states_in_input_batch(device: str, batch_size: int, swap_list: list):
    ...
    """
    input_batch: InputBatch = InputBatch(max_num_reqs=batch_size, max_model_len=1024, max_num_batched_tokens=1024, device=torch.device(device), pin_memory=is_pin_memory_available(), vocab_size=1024, block_sizes=[1])
    ref_input_batch: InputBatch = InputBatch(max_num_reqs=batch_size, max_model_len=1024, max_num_batched_tokens=1024, device=torch.device(device), pin_memory=is_pin_memory_available(), vocab_size=1024, block_sizes=[1])
    reqs: list[CachedRequestState] = []
    req_id_reqs = {}
    req_id_output_token_ids = {}
    for req_index in range(batch_size):
    ...
    for swap_pair in swap_list:
        reordered_reqs[swap_pair[0]], reordered_reqs[swap_pair[1]] = (reordered_reqs[swap_pair[1]], reordered_reqs[swap_pair[0]])
