...
@pytest.mark.parametrize('failing_method', ['forward', 'load_weights'])
def test_llm_startup_error(monkeypatch, model: str, tensor_parallel_size: int, enable_multiprocessing: bool, failing_method: str) -> None:
    ...
    with monkeypatch.context() as m:
        ...
        monkeypatch.setattr(LlamaForCausalLM, failing_method, evil_method)
        with pytest.raises(Exception, match='initialization failed' if enable_multiprocessing else 'Simulated Error in startup!'):
            _ = LLM(model=model, enforce_eager=True, tensor_parallel_size=tensor_parallel_size)
        wait_for_gpu_memory_to_clear(devices=list(range(tensor_parallel_size)), threshold_bytes=SHUTDOWN_TEST_THRESHOLD_BYTES)