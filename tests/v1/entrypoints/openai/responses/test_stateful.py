...
async def test_background_error(client: openai.AsyncOpenAI):
    with pytest.raises(openai.BadRequestError, match='background can only be used when `store` is true'):
        _ = await client.responses.create(input='What is 13 * 24?', background=True, store=False)

@pytest.mark.asyncio
async def test_background_cancel(client: openai.AsyncOpenAI):
    response = await client.responses.create(input='Write a long story about a cat.', background=True)
    assert response.status == 'queued'
...
async def test_previous_response_id(client: openai.AsyncOpenAI):
    response1 = await client.responses.create(instructions='You are tested on your ability to retrieve the correct information from the previous response.', input='Hello, my name is John.')
    response2 = await client.responses.create(input='Actually, my name is not John. My real name is Mark.', previous_response_id=response1.id)
    response3 = await client.responses.create(input='What is my real name again? Answer in one word.', previous_response_id=response2.id)
    print(response3)
...
async def test_two_responses_with_same_prev_id(client: openai.AsyncOpenAI):
    response1 = await client.responses.create(instructions='You are tested on your ability to retrieve the correct information from the previous response.', input='Hello, my name is John.')
    response2 = client.responses.create(input='Actually, my name is not John. My name is Mark.', previous_response_id=response1.id)
    response3 = client.responses.create(input='What is my name again? Answer in one word.', previous_response_id=response1.id)
    _ = await response2
