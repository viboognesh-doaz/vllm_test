...
from vllm.multimodal.inputs import MultiModalBatchedField, MultiModalFieldElem, MultiModalFlatField, MultiModalKwargs, MultiModalKwargsItem, MultiModalSharedField, NestedTensors
...
def test_encode_decode(monkeypatch: pytest.MonkeyPatch):
    ...
    with monkeypatch.context() as m:
        ...
        obj = MyType(tensor1=torch.randint(low=0, high=100, size=(1024,), dtype=torch.int32), a_string='hello', list_of_tensors=[torch.rand((1, 10), dtype=torch.float32), torch.rand((3, 5, 4000), dtype=torch.float64), torch.tensor(1984), torch.rand((3, 5, 1000), dtype=torch.bfloat16), torch.tensor([float('-inf'), float('inf')] * 1024, dtype=torch.bfloat16)], numpy_array=np.arange(512), unrecognized=UnrecognizedType(33), small_f_contig_tensor=torch.rand(5, 4).t(), large_f_contig_tensor=torch.rand(1024, 4).t(), small_non_contig_tensor=torch.rand(2, 4)[:, 1:3], large_non_contig_tensor=torch.rand(1024, 512)[:, 10:20], empty_tensor=torch.empty(0))
...
def test_multimodal_kwargs():
    d = {'foo': torch.zeros(20000, dtype=torch.float16), 'bar': [torch.zeros(i * 1000, dtype=torch.int8) for i in range(3)], 'baz': [torch.rand(256, dtype=torch.float16), [torch.rand((1, 12), dtype=torch.float32), torch.rand((3, 5, 7), dtype=torch.float64)], [torch.rand((4, 4), dtype=torch.float16)]]}
...
def test_multimodal_items_by_modality():
    ...
    e2 = MultiModalFieldElem('video', 'v0', [torch.zeros(1000, dtype=torch.int8) for _ in range(4)], MultiModalFlatField([[slice(1, 2, 3), slice(4, 5, 6)], [slice(None, 2)]], 0))
    e3 = MultiModalFieldElem('image', 'i0', torch.zeros(1000, dtype=torch.int32), MultiModalSharedField(4))
    e4 = MultiModalFieldElem('image', 'i1', torch.zeros(1000, dtype=torch.int32), MultiModalFlatField([slice(1, 2, 3), slice(4, 5, 6)], 2))
...
def test_custom_class_serialization_allowed_with_pickle(monkeypatch: pytest.MonkeyPatch):
