from vllm.v1.metrics.reader import Counter, Gauge, Histogram, Vector, get_metrics_snapshot
...
def test_gauge_metric(test_registry, num_engines):
    g = prometheus_client.Gauge('vllm:test_gauge', 'Test gauge metric', labelnames=['model', 'engine_index'], registry=test_registry)
...
def test_counter_metric(test_registry, num_engines):
    c = prometheus_client.Counter('vllm:test_counter', 'Test counter metric', labelnames=['model', 'engine_index'], registry=test_registry)
    for i in range(num_engines):
...
def test_histogram_metric(test_registry, num_engines):
    h = prometheus_client.Histogram('vllm:test_histogram', 'Test histogram metric', labelnames=['model', 'engine_index'], buckets=[10, 20, 30, 40, 50], registry=test_registry)
    for i in range(num_engines):
...
def test_vector_metric(test_registry, num_engines):
    c = prometheus_client.Counter('vllm:spec_decode_num_accepted_tokens_per_pos', 'Vector-like counter metric', labelnames=['position', 'model', 'engine_index'], registry=test_registry)
    for i in range(num_engines):
