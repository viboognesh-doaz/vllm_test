...
from vllm.config import CacheConfig, CompilationConfig, DeviceConfig, LoadConfig, ModelConfig, ModelDType, ParallelConfig, SchedulerConfig, VllmConfig
...
def create_common_attn_metadata(batch_spec: BatchSpec, block_size: int, device: torch.device, max_block_idx: int=1000) -> CommonAttentionMetadata:
...
def get_attention_backend(backend_name: _Backend):
    ...
    """
    backend_map = {_Backend.FLASH_ATTN_VLLM_V1: 'vllm.v1.attention.backends.flash_attn.FlashAttentionBackend', _Backend.FLASHINFER_VLLM_V1: 'vllm.v1.attention.backends.flashinfer.FlashInferBackend', _Backend.FLEX_ATTENTION: 'vllm.v1.attention.backends.flex_attention.FlexAttentionBackend', _Backend.TRITON_ATTN_VLLM_V1: 'vllm.v1.attention.backends.triton_attn.TritonAttentionBackend'}
...
def create_standard_kv_cache_spec(vllm_config: VllmConfig) -> FullAttentionSpec:
    ...
    return FullAttentionSpec(block_size=vllm_config.cache_config.block_size, num_kv_heads=vllm_config.model_config.get_num_kv_heads(vllm_config.parallel_config), head_size=vllm_config.model_config.get_head_size(), dtype=vllm_config.model_config.dtype, use_mla=vllm_config.model_config.use_mla, sliding_window=vllm_config.model_config.get_sliding_window())

def create_vllm_config(model_name: str='meta-llama/Meta-Llama-3-8B', tensor_parallel_size: int=1, max_model_len: int=1024, dtype: Union[ModelDType, torch.dtype]='auto', block_size: int=16, max_num_seqs: int=256, max_num_batched_tokens: int=8192, add_mock_model_methods: bool=True) -> VllmConfig:
    """Create a VllmConfig for testing with reasonable defaults."""
    model_config = ModelConfig(model=model_name, tokenizer=model_name, trust_remote_code=False, dtype=dtype, seed=0, max_model_len=max_model_len)
    ...
    if add_mock_model_methods:
        ...
        model_config.get_sm_scale_for_layer = types.MethodType(lambda self, i: 1.0 / model_config.get_head_size() ** 0.5, model_config)
    return VllmConfig(model_config=model_config, cache_config=cache_config, parallel_config=parallel_config, scheduler_config=scheduler_config, device_config=device_config, load_config=load_config, compilation_config=compilation_config)
...
def create_dummy_kv_cache(block_size: int, num_kv_heads: int, head_size: int, dtype: torch.dtype, device: torch.device, num_blocks: int=100) -> torch.Tensor:
    ...
    kv_cache = torch.randn(num_blocks, 2, block_size, num_kv_heads, head_size, dtype=dtype, device=device)
    return kv_cache