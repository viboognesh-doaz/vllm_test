...
def ref_attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, scale: float) -> torch.Tensor:
    """
...
def test_mha_attn_forward(batch_size: int, seq_len: int, num_heads: int, num_kv_heads: int, head_size: int, device: str):
    ...
    ref_output = ref_attention(q, k, v, scale=scale).reshape(batch_size, seq_len, num_heads * head_size)
    torch.testing.assert_close(output, ref_output, atol=0.01, rtol=0.001)