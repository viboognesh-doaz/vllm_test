...
def setup_environment():
    engine_args = EngineArgs(model='Qwen/Qwen2-1.5B-Instruct', max_model_len=64, max_num_batched_tokens=64, max_num_seqs=4)
...
def test_xla_qkv_linear(bias, mesh, device):
    ...
    qkv_linear = QKVParallelLinear(hidden_size=4096, head_size=128, total_num_heads=32, total_num_kv_heads=8, bias=bias, params_dtype=torch.bfloat16, return_bias=False)
