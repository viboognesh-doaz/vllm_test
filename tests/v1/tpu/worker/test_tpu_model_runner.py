...
from vllm.config import CacheConfig, ModelConfig, SchedulerConfig, VllmConfig, set_current_vllm_config
...
from vllm.v1.worker.tpu_model_runner import TPUModelRunner, _get_padded_num_reqs_with_upper_limit, _get_padded_token_len, _get_req_paddings, _get_token_paddings
import pytest

def get_vllm_config():
    scheduler_config = SchedulerConfig(max_num_seqs=10, max_num_batched_tokens=512, max_model_len=512)
    model_config = ModelConfig(model='facebook/opt-125m', task='generate', tokenizer='facebook/opt-125m', tokenizer_mode='auto', trust_remote_code=True, dtype='bfloat16', seed=42)
    cache_config = CacheConfig(block_size=16, gpu_memory_utilization=0.9, swap_space=0, cache_dtype='auto')
    vllm_config = VllmConfig(model_config=model_config, cache_config=cache_config, scheduler_config=scheduler_config)
    return vllm_config
...
def _schedule_new_request(*req_ids: str) -> SchedulerOutput:
    ...
    for req_id in req_ids:
        new_reqs.append(NewRequestData(req_id=req_id, prompt_token_ids=[1, 2, 3], mm_inputs=[], mm_hashes=[], mm_positions=[], sampling_params=SamplingParams(), pooling_params=PoolingParams(), block_ids=([0],), num_computed_tokens=0, lora_request=None))
...
def test_update_states_request_resumed(model_runner):
    ...
    assert _is_req_added(model_runner, req_id)
    assert not _is_req_scheduled(model_runner, req_id)
    cached_req_data = CachedRequestData(req_ids=[req_id], resumed_from_preemption=[False], new_token_ids=[[]], new_block_ids=[([],)], num_computed_tokens=[0])
...
def test_init_kv_cache_with_kv_sharing_invalid_target_layer_order(model_runner):
    ...
    with pytest.raises(ValueError, match=error_msg), set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_0, kv_sharing_target_layer_name=layer_1), layer_1: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_1)}
...
def test_init_kv_cache_with_kv_sharing_target_layer_not_exist(model_runner):
    ...
    vllm_config = model_runner.vllm_config
    with pytest.raises(ValueError, match=error_msg), set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_1, kv_sharing_target_layer_name=invalid_layer)}
        assert fwd_context is not None
...
def test_init_kv_cache_with_kv_sharing_target_same_as_current(model_runner):
    ...
    vllm_config = model_runner.vllm_config
    with pytest.raises(ValueError, match=error_msg), set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_1, kv_sharing_target_layer_name=layer_1)}
        assert fwd_context is not None
...
def test_init_kv_cache_without_kv_sharing():
    ...
    with set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_1)}
        assert fwd_context is not None
    vllm_config.model_config.max_model_len = 1000000
...
def test_init_kv_cache_with_kv_sharing_valid():
    ...
    vllm_config = get_vllm_config()
    with set_current_vllm_config(vllm_config):
        fwd_context = {layer_0: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_0), layer_1: Attention(num_heads=8, head_size=128, scale=1.0, prefix=layer_1, kv_sharing_target_layer_name='model.layers.0.self_attn.attn')}
        assert fwd_context is not None
