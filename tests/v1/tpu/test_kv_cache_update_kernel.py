...
import torch
import torch_xla
import vllm.v1.attention.backends.pallas
...
def test_kv_cache_update_kernel(page_size: int, combined_kv_head_num: int, head_dim: int, num_slices_per_block: int):
    ...
    kv_cache_cpu = torch.zeros((page_num * page_size, combined_kv_head_num, head_dim), dtype=torch.bfloat16, device='cpu')
    kv_cache_xla = kv_cache_cpu.to(torch_xla.device())
    new_kv_cpu = torch.randn((padded_num_tokens, combined_kv_head_num, head_dim), dtype=torch.bfloat16, device='cpu')
    ...
    kv_cache_start_indices = np.array([page_size * 2 - 7, page_size * 2, page_size * 3, page_size * 4 + 6, page_size * 5 + 7, page_size * 6 + 8, page_size * 15 + 3], dtype=np.int32)
    ...
    padded_size = (slot_mapping.shape[0] + num_slices_per_block - 1) // num_slices_per_block * num_slices_per_block
    slot_mapping = np.pad(slot_mapping, [[0, padded_size - slot_mapping.shape[0]], [0, 0]], constant_values=0)
    ...
    new_kv_cache_xla = torch.ops.xla.kv_cache_update_op(new_kv_xla, slot_mapping_xla, kv_cache_xla, num_kv_update_slices_xla, page_size, num_slices_per_block)
    ...
    assert torch.allclose(kv_cache_xla.cpu(), kv_cache_cpu, atol=0.0001, rtol=0.0001)