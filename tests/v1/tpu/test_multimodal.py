...
async def test_basic_vision(model_name: str, base64_encoded_image: dict[str, str]):
    ...
    def whats_in_this_image_msg(b64):
        return [{'role': 'user', 'content': [{'type': 'text', 'text': "What's in this image?"}, {'type': 'image_url', 'image_url': {'url': f'data:image/jpeg;base64,{b64}'}}]}]
    server_args = ['--max-model-len', '1024', '--max-num-seqs', '16', '--gpu-memory-utilization', '0.95', '--trust-remote-code', '--max-num-batched-tokens', '576', '--disable_chunked_mm_input']
    with RemoteOpenAIServer(model_name, server_args, max_wait_seconds=600) as remote_server:
        ...
        for image_url in TEST_IMAGE_URLS:
            ...
            chat_completion_from_base64 = await client.chat.completions.create(model=model_name, messages=whats_in_this_image_msg(image_base64), max_completion_tokens=24, temperature=0.0)
