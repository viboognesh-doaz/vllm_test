...
def test_ragged_paged_attention():
    ...
    attn_impl = PallasAttentionBackendImpl(num_heads=num_heads, head_size=head_size, scale=scale, num_kv_heads=num_kv_heads, alibi_slopes=None, sliding_window=sliding_window, kv_cache_dtype='auto', logits_soft_cap=logits_soft_cap, attn_type=AttentionType.DECODER)
    ...
    attn_metadata = PallasMetadata(slot_mapping=slot_mapping, block_tables=block_tables, context_lens=context_lens, query_start_loc=query_start_loc, num_seqs=num_seqs, num_kv_update_slices=num_kv_update_slices, num_slices_per_kv_cache_update_block=8)
    with patch('torch.ops.xla.ragged_paged_attention') as mock_ragged_paged_attention:
        attn_impl.forward(layer=layer, query=query, key=key, value=value, kv_cache=kv_cache, attn_metadata=attn_metadata)
        mock_ragged_paged_attention.assert_called_once_with(ANY, ANY, ANY, ANY, ANY, ANY, num_kv_pages_per_block=None, num_queries_per_block=None, vmem_limit_bytes=None, use_kernel=True, sm_scale=scale, sliding_window=sliding_window, soft_cap=logits_soft_cap, k_scale=1.0, v_scale=1.0)