...
def create_scheduler(model: str='facebook/opt-125m', max_num_seqs: int=16, max_num_batched_tokens: int=8192, enable_prefix_caching: Optional[bool]=None, long_prefill_token_threshold: int=0, disable_chunked_mm_input: bool=False, use_kv_connector: bool=False, num_blocks: int=10000, block_size: int=16, max_model_len: Optional[int]=None, num_speculative_tokens: Optional[int]=None, skip_tokenizer_init: bool=False, async_scheduling: bool=False) -> Union[Scheduler, AsyncScheduler]:
    ...
    model_config = ModelConfig(model=model, task='auto', tokenizer=model, tokenizer_mode='auto', trust_remote_code=True, dtype='float16', seed=42, skip_tokenizer_init=skip_tokenizer_init)
    ...
    return scheduler_cls(vllm_config=vllm_config, kv_cache_config=kv_cache_config, log_stats=True, structured_output_manager=StructuredOutputManager(vllm_config))

def create_requests(num_requests: int, num_tokens: int=10, mm_positions: Optional[list[PlaceholderRange]]=None, max_tokens: int=16, stop_token_ids: Optional[list[int]]=None, prompt_logprobs: Optional[int]=None, same_prompt: bool=False) -> list[Request]:
    ...
    for i in range(num_requests):
        ...
        request = Request(request_id=f'{i}', prompt_token_ids=prompt_token_ids, sampling_params=sampling_params, pooling_params=None, multi_modal_inputs=mm_inputs, multi_modal_placeholders=mm_position, multi_modal_hashes=None, eos_token_id=EOS_TOKEN_ID)
