...
from vllm.distributed.kv_transfer.kv_connector.v1.shared_storage_connector import SharedStorageConnector
...
def create_vllm_config(model: str='facebook/opt-125m', max_num_seqs: int=16, max_num_batched_tokens: int=64, block_size: int=16) -> VllmConfig:
    ...
    cache_config = CacheConfig(block_size=block_size, gpu_memory_utilization=0.9, swap_space=0, cache_dtype='auto', enable_prefix_caching=True)
    kv_transfer_config = KVTransferConfig(kv_connector='NixlConnector', kv_role='kv_both')
    return VllmConfig(scheduler_config=scheduler_config, model_config=model_config, cache_config=cache_config, kv_transfer_config=kv_transfer_config, device_config=DeviceConfig('cpu'))

def create_scheduler(vllm_config: VllmConfig, num_blocks: int=10000) -> Scheduler:
...
def create_request(request_id: int, num_tokens: int=10, max_tokens: int=16, do_remote_decode: bool=False, do_remote_prefill: bool=False, use_all_1s_for_prompt_tokens: bool=False, num_remote_blocks: int=3) -> Request:
    ...
    if do_remote_decode:
        ...
        kv_transfer_params = dict(do_remote_prefill=True, do_remote_decode=False, remote_engine_id='my-engine-id', remote_block_ids=list(range(num_remote_blocks)), remote_host='my-host', remote_port=1234)
    ...
    req = Request(request_id=f'id-{request_id}', prompt_token_ids=prompt_token_ids, sampling_params=sampling_params, pooling_params=None, multi_modal_inputs=None, multi_modal_placeholders=None, multi_modal_hashes=None, eos_token_id=EOS_TOKEN_ID)
    req.kv_transfer_params = kv_transfer_params
    return req

def create_model_runner_output(reqs: list[Request], finished_sending: Optional[list[str]]=None, finished_recving: Optional[list[str]]=None, use_eos: bool=False) -> ModelRunnerOutput:
    """Make dummy model runner output for testing."""
    req_ids = [req.request_id for req in reqs]
    req_id_to_index = {req_id: idx for idx, req_id in enumerate(req_ids)}
    sampled_token = EOS_TOKEN_ID if use_eos else 0
    sampled_token_ids = [[sampled_token] for _ in req_ids]
    return ModelRunnerOutput(req_ids=req_ids, req_id_to_index=req_id_to_index, sampled_token_ids=sampled_token_ids, spec_token_ids=None, logprobs=None, prompt_logprobs_dict={}, pooler_output=None, finished_sending=finished_sending, finished_recving=finished_recving)
...
class TestSharedStorageConnector(SharedStorageConnector):
    ...
    def __getattribute__(self, name):
        if name in ('_connector', 'call_record', 'name', '_event_file', '__class__', '__dict__', '__getattribute__', '__init__'):
