...
class DummyModelRunnerOutput(ModelRunnerOutput):
    ...
    def __init__(self, finished_sending: Optional[set[str]]=None, finished_recving: Optional[set[str]]=None):
...
def test_aggregate_workers_output():
    ...
    assert aggregated.finished_recving is None
    output1 = DummyModelRunnerOutput(finished_sending=None, finished_recving=None)
    output2 = DummyModelRunnerOutput(finished_sending={'req1'}, finished_recving={'req2'})
    aggregated = aggregator.aggregate([output1, output2])
    assert aggregated is output1
    assert aggregated.finished_sending is None
