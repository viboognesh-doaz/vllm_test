...
from vllm.distributed.kv_transfer.kv_connector.v1.nixl_connector import KVConnectorRole, NixlAgentMetadata, NixlConnector, NixlConnectorMetadata, NixlConnectorWorker
...
def test_basic_interface():
    ...
    for block_id, block in zip(req_meta.local_block_ids, scheduler.kv_cache_manager.coordinator.single_type_managers[0].req_to_blocks[request_id]):
...
def test_prompt_less_than_block_size():
    ...
    request = create_request(request_id=1, num_tokens=NUM_TOKENS, do_remote_prefill=True, num_remote_blocks=0)
    scheduler.add_request(request)
    scheduler_output = scheduler.schedule()
    kv_connector_metadata = scheduler_output.kv_connector_metadata
    assert kv_connector_metadata is not None
...
class FakeNixlConnectorWorker(NixlConnectorWorker):
    ...
    def _nixl_handshake(self, host: str, port: int, remote_tp_size: int, expected_engine_id: str) -> dict[int, str]:
        ...
        remote_agent_name = self.add_remote_agent(NixlAgentMetadata(engine_id=self.REMOTE_ENGINE_ID, agent_metadata=FakeNixlWrapper.AGENT_METADATA, kv_caches_base_addr=[0], num_blocks=1, block_len=self.block_len, attn_backend_name=self.backend_name), remote_tp_size=remote_tp_size)
...
class TestNixlHandshake:
    ...
    def test_multi_xfer_one_engine(self, dist_init):
        ...
        while True:
            ...
            if num_xfers > 0:
                num_xfers -= 1
                metadata.add_new_req(request_id=request_id, local_block_ids=[num_xfers + 1, num_xfers + 2, num_xfers + 3], kv_transfer_params={'remote_block_ids': [num_xfers + 4, num_xfers + 5, num_xfers + 6], 'remote_engine_id': FakeNixlConnectorWorker.REMOTE_ENGINE_ID, 'remote_host': 'localhost', 'remote_port': 1234, 'remote_tp_size': 1})
            connector.bind_connector_metadata(metadata)
            dummy_ctx = ForwardContext(no_compile_layers={}, attn_metadata={}, virtual_engine=0)
    ...
    @patch('vllm.distributed.kv_transfer.kv_connector.v1.nixl_connector.NixlWrapper', FakeNixlWrapper)
    @pytest.mark.parametrize('decode_tp_size, prefill_tp_size', [(1, 1), (2, 1), (4, 2), (4, 4)])
    def test_async_load_kv(self, dist_init, decode_tp_size, prefill_tp_size):
        ...
        metadata = NixlConnectorMetadata()
        metadata.add_new_req(request_id='id', local_block_ids=[1, 2, 3], kv_transfer_params={'remote_block_ids': [4, 5, 6], 'remote_engine_id': FakeNixlConnectorWorker.REMOTE_ENGINE_ID, 'remote_host': 'localhost', 'remote_port': 1234, 'remote_tp_size': prefill_tp_size})
        connector.bind_connector_metadata(metadata)
        timeout = 2.5
    ...
    @patch('vllm.distributed.kv_transfer.kv_connector.v1.nixl_connector.NixlWrapper', FakeNixlWrapper)
    def test_concurrent_load_kv(self, dist_init):
        ...
        for i in range(total_reqs):
            metadata.add_new_req(request_id=f'id_{i}', local_block_ids=[1, 2, 3], kv_transfer_params={'remote_block_ids': [4, 5, 6], 'remote_engine_id': FakeNixlConnectorWorker.REMOTE_ENGINE_ID, 'remote_host': 'localhost', 'remote_port': 1234, 'remote_tp_size': 1})
        connector.bind_connector_metadata(metadata)
...
def test_abort_timeout_on_prefiller(monkeypatch, distributed_executor_backend):
    ...
    if distributed_executor_backend == 'ray':
        runtime_env = {'working_dir': _make_stub_pkg(), 'env_vars': {'VLLM_NIXL_ABORT_REQUEST_TIMEOUT': str(timeout)}}
        ray.init(runtime_env=runtime_env)
    llm = LLM(model=model_name, enforce_eager=True, gpu_memory_utilization=0.5, kv_transfer_config=kv_transfer_config, distributed_executor_backend=distributed_executor_backend)
    remote_prefill_opts = {'do_remote_decode': True, 'do_remote_prefill': False, 'remote_engine_id': None, 'remote_block_ids': None, 'remote_host': None, 'remote_port': None}
    sampling_params = SamplingParams(temperature=0.0, max_tokens=1, extra_args={'kv_transfer_params': remote_prefill_opts})
