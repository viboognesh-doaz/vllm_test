...
def make_request(params: SamplingParams, prompt_tokens_ids: Optional[list[int]]=None) -> EngineCoreRequest:
    if not prompt_tokens_ids:
        prompt_tokens_ids = PROMPT_TOKENS
    return EngineCoreRequest(request_id=str(uuid.uuid4()), prompt_token_ids=prompt_tokens_ids, mm_inputs=None, mm_hashes=None, mm_placeholders=None, sampling_params=params, pooling_params=None, eos_token_id=None, arrival_time=time.time(), lora_request=None, cache_salt=None, data_parallel_rank=None)
...
def test_engine_core_client(monkeypatch: pytest.MonkeyPatch, multiprocessing_mode: bool):
    with monkeypatch.context() as m:
        ...
        with set_default_torch_num_threads(1):
            client = EngineCoreClient.make_client(multiprocess_mode=multiprocessing_mode, asyncio_mode=False, vllm_config=vllm_config, executor_class=executor_class, log_stats=False)
        MAX_TOKENS = 20
        ...
        for req_id in request_ids:
            assert len(outputs[req_id]) == MAX_TOKENS, f'outputs[req_id]={outputs[req_id]!r}, MAX_TOKENS={MAX_TOKENS!r}'
        'Abort Request Cycle.'
        ...
        for idx, req_id in enumerate(request_ids):
            if idx % 2 == 0:
                assert len(outputs[req_id]) < MAX_TOKENS, f'len(outputs[req_id])={len(outputs[req_id])!r}, MAX_TOKENS={MAX_TOKENS!r}'
            else:
                assert len(outputs[req_id]) == MAX_TOKENS, f'len(outputs[req_id])={len(outputs[req_id])!r}, MAX_TOKENS={MAX_TOKENS!r}'
...
async def test_engine_core_client_asyncio(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        ...
        m.setattr(EngineCore, 'echo', echo, raising=False)
        engine_args = EngineArgs(model=MODEL_NAME, enforce_eager=True)
        vllm_config = engine_args.create_engine_config(usage_context=UsageContext.UNKNOWN_CONTEXT)
        executor_class = Executor.get_class(vllm_config)
        with set_default_torch_num_threads(1):
            client = EngineCoreClient.make_client(multiprocess_mode=True, asyncio_mode=True, vllm_config=vllm_config, executor_class=executor_class, log_stats=True)
        try:
            MAX_TOKENS = 20
            params = SamplingParams(max_tokens=MAX_TOKENS)
            'Normal Request Cycle.'
...
@pytest.mark.parametrize('multiprocessing_mode,publisher_config', [(True, 'tcp'), (False, 'inproc')], indirect=['publisher_config'])
def test_kv_cache_events(monkeypatch: pytest.MonkeyPatch, multiprocessing_mode: bool, publisher_config):
    with monkeypatch.context() as m:
        m.setenv('VLLM_USE_V1', '1')
        block_size = 16
        num_blocks = 2
        engine_args = EngineArgs(model=MODEL_NAME, enforce_eager=True, enable_prefix_caching=True, block_size=block_size)
...
@pytest.mark.asyncio
@pytest.mark.parametrize('multiprocessing_mode,publisher_config', [(True, 'tcp')], indirect=['publisher_config'])
@multi_gpu_test(num_gpus=4)
async def test_kv_cache_events_dp(monkeypatch: pytest.MonkeyPatch, multiprocessing_mode: bool, publisher_config):
    with monkeypatch.context() as m:
        m.setenv('VLLM_USE_V1', '1')
        block_size = 16
        num_blocks = 2
        dp_size = 2
        tp_size = 2
        engine_args = EngineArgs(model=MODEL_NAME, enforce_eager=True, enable_prefix_caching=True, data_parallel_size=dp_size, tensor_parallel_size=tp_size, block_size=block_size)
        engine_args.kv_events_config = publisher_config
        vllm_config = engine_args.create_engine_config(UsageContext.UNKNOWN_CONTEXT)
        executor_class = Executor.get_class(vllm_config)
        with set_default_torch_num_threads(1):
            client = EngineCoreClient.make_client(multiprocess_mode=multiprocessing_mode, asyncio_mode=True, vllm_config=vllm_config, executor_class=executor_class, log_stats=False)
...
def test_startup_failure(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m, pytest.raises(Exception) as e_info:
        ...
        _core_client = EngineCoreClient.make_client(multiprocess_mode=True, asyncio_mode=True, vllm_config=vllm_config, executor_class=executor_class, log_stats=True)
...
def test_engine_core_proc_instantiation_cuda_empty(monkeypatch: pytest.MonkeyPatch):
    ...
    def create_mock_executor(vllm_config):
        ...
        mock_spec = FullAttentionSpec(block_size=16, num_kv_heads=1, head_size=64, dtype=torch.float16, use_mla=False)
    ...
    with monkeypatch.context() as m:
        ...
        def mock_startup_handshake(self, handshake_socket, on_head_node, parallel_config):
            return EngineZmqAddresses(inputs=['tcp://127.0.0.1:5555'], outputs=['tcp://127.0.0.1:5556'], coordinator_input=None, coordinator_output=None)
        m.setattr(EngineCoreProc, 'startup_handshake', mock_startup_handshake)
        vllm_config = EngineArgs(model='deepseek-ai/DeepSeek-V2-Lite', trust_remote_code=True).create_engine_config()
        engine_core_proc = EngineCoreProc(vllm_config=vllm_config, local_client=True, handshake_address='tcp://127.0.0.1:12345', executor_class=mock_executor_class, log_stats=False, engine_index=0)
