...
def _vllm_model(apc: bool, vllm_runner: type[VllmRunner], monkeypatch: pytest.MonkeyPatch, *, skip_tokenizer_init: bool=False):
    """Set up VllmRunner instance."""
    monkeypatch.setenv('VLLM_USE_V1', '1')
    return vllm_runner(MODEL, dtype=DTYPE, max_model_len=128, enforce_eager=True, enable_prefix_caching=apc, gpu_memory_utilization=0.5, skip_tokenizer_init=skip_tokenizer_init)
...
def vllm_model_skip_tokenizer_init(vllm_runner, request, monkeypatch):
    """VllmRunner test fixture with APC."""
    with _vllm_model(request.param, vllm_runner, monkeypatch, skip_tokenizer_init=True) as vllm_model:
        yield vllm_model

def _get_test_sampling_params(prompt_list: list[str], seed: Optional[int]=42, structured_outputs: bool=False) -> tuple[list[SamplingParams], list[int]]:
    ...
    return ([SamplingParams(temperature=0.95, top_p=0.95, n=n, seed=seed, guided_decoding=GuidedDecodingParams(regex='[0-9]+') if structured_outputs else None) for n in n_list], n_list)

def test_compatibility_with_skip_tokenizer_init(vllm_model_skip_tokenizer_init: VllmRunner, example_prompts: list[str]):
    sampling_params_list, _ = _get_test_sampling_params(example_prompts, structured_outputs=True)
...
def test_engine_metrics(vllm_runner, monkeypatch, example_prompts):
    max_tokens = 100
    speculative_config = {'method': 'ngram', 'prompt_lookup_max': 5, 'prompt_lookup_min': 3, 'num_speculative_tokens': 5}
    monkeypatch.setenv('VLLM_USE_V1', '1')
    with vllm_runner(MODEL, speculative_config=speculative_config, disable_log_stats=False) as vllm_model:
        model: LLM = vllm_model.model
        ...
        num_accepted_tokens_per_pos = find_metric('vllm:spec_decode_num_accepted_tokens_per_pos')
